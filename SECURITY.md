...
**Source of Truth**: `server-rs/src/security.rs`, `server-rs/src/audit.rs`, `SECURITY.md`
**Last Hardened**: 2026-04-05
**Standard Compliance**: ECC-SEC (Enhanced Contextual Clarity - Security Standards)
...
> - **Audit Trail**: Every action is SHA-256 linked in the **Merkle Audit Trail** (`audit.rs`).
...
Security guidance remains zero-trust and now reflects the latest runtime hardening:
- **Scanner/Redaction Alignment**: Secret-scanning and redaction rules remain enforced as the authoritative boundary for shell/code execution.
- **Path Safety Continuity**: Filesystem validation remains centralized through the canonical path-validation layer used across persistence, workspace access, and tool execution.
- **Operational Drift Closure**: Documentation has been refreshed to stay synchronized with the current scanner, permissions, request-id, and audit-linked runtime modules flagged by parity checks.

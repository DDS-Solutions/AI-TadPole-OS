...
> - **Subsystem**: Project Root / ROADMAP
> - **Architecture**: `@docs ARCHITECTURE:Documentation`
> - **Failure Path**: Information drift, legacy terminology, or documentation mismatch.
> - **Observability**: Traceability via `execution/parity_guard.py` (`[ROADMAP]`)
...
- [x] **Symbol Graph & ADG Unification** `ADG-02`: Integrate the markdown/comment-based ADG validation into the AST-based [graph_query.rs](server-rs/src/bin/graph_query.rs) system. By compiling standard docstrings (JSDoc/Rustdoc) directly into `symbol_graph.json` metadata, we eliminate custom comment blocks and maintain a single AST-backed source of truth.
...
> *Note: These connectors will be built and distributed via the `AI-Tadpole-OS-Industry-Templates` remote repository to decouple release cycles and enable community contribution.*

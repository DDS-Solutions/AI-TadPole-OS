...
- [ ] **Symbol Graph & ADG Unification** `ADG-02`: Integrate the markdown/comment-based ADG validation into the AST-based [graph_query.rs](file:///D:/TadpoleOS-Dev/server-rs/src/bin/graph_query.rs) system. By compiling standard docstrings (JSDoc/Rustdoc) directly into `symbol_graph.json` metadata, we eliminate custom comment blocks and maintain a single AST-backed source of truth.

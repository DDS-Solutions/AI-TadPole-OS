...
- [ ] **Database & CRM/SaaS Blueprint** — Template MCP server for querying relational/document databases, lightweight CRMs, and SMB accounting tools
...
- [ ] Add `mirror_mode: bool` flag to `AppConfig` / `.env`
- [ ] When `mirror_mode = true`, agents consume connector data but **do not act** unless
  drift from expected state is detected
- [ ] Drift detection: compare real connector data against agent's expected state, emit
  `oversight:drift` WebSocket event for human review
- [ ] Add `GET /v1/engine/mirror/status` route returning current drift alerts
...
- [ ] Add `shadows_human_id: Option<String>` to `EngineAgent`
- [ ] Extend agent config panel with "Shadows Real Person" field

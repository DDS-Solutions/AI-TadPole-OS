...
pub struct CodeSymbolGraph {
    ...
    pub parse_cache: HashMap<String, (Vec<crate::utils::parser::Symbol>, Vec<crate::utils::parser::Reference>)>,
    root: PathBuf,
}
    ...
    pub fn build(&mut self, salt: &str) {
        ...
        let files: Vec<PathBuf> = WalkDir::new(&self.root)
            ...
            .filter_entry(|e| {
                ...
                name != "target" && name != "node_modules" && name != ".git" && name != "dist" && name != "scratch"
            })
            .filter_map(|e| e.ok())
        ...
        if files_to_parse.is_empty() && !has_deleted && !self.index.is_empty() {
            tracing::info!("✅ [Graph] Knowledge graph is already up-to-date. (Nodes: {}, Edges: {})", self.graph.node_count(), self.graph.edge_count());
            return;
        }
            ...
            .map_init(SymbolExtractor::new, |extractor, path| {
                ...
                match std::fs::read_to_string(path) {
                    Ok(content) => {
                        ...
                        if let Ok(m) = std::fs::metadata(path) {
                            if let (Ok(mtime), size) = (m.modified(), m.len()) {
                                return (path.clone(), rel_path, Some((symbols, refs, mtime, size)));
                            }
                        }
        ...
        for (rel_path, (symbols, _)) in &self.parse_cache {
            ...
            self.obfuscated_to_real_path.insert(obf_path, rel_path.to_string());
            for sym in symbols {
...
mod tests {
    ...
    fn test_incremental_ast_caching() {
        ...
        assert!(graph.index.contains_key(&("a.rs".to_string(), "helper".to_string())));
        assert!(graph.index.contains_key(&("b.rs".to_string(), "main".to_string())));

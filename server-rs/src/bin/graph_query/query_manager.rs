...
//! - **Subsystem**: Sovereign Engine / query_manager
//!
...
pub fn calculate_pagerank(
    ...
    // Apply IPC / Contract Boundary Boost once after convergence (1.2x multiplier for routes, commands, and schemas)
    for idx in graph.graph.node_indices() {
        ...
        let real_path = graph
            ...
            .map(|p| p.as_str())
            .unwrap_or(&node.path);
        if real_path.contains("routes")
            || real_path.contains("commands")
            || real_path.contains("schemas")
        {
            scores[idx.index()] *= 1.2;
        }
    }
...
pub fn build_audit_context(graph: &CodeSymbolGraph, root: &Path, limit: usize) -> AuditContext {
    ...
    AuditContext {
        ...
        top_connected,
        anomalies: anomalies.into_iter().take(limit).collect(),

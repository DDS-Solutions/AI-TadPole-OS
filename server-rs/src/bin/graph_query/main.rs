...
#[path = "../../intelligence/graph/mod.rs"]
pub mod graph;
...
pub enum Commands {
    ...
    Blast {
        ...
        depth: u32,
        #[arg(long, default_value = "text", value_parser = ["text", "mermaid", "html"])]
        format: String,
    },
...
fn run() -> Result<(), GraphQueryError> {
    ...
    match args.command {
        ...
        Commands::Lookup {
            ...
            emit_query(&contexts, json, out.as_ref())?;
        }
        Commands::File {
            path,
            limit,
            ...
            emit_query(&payload, json, out.as_ref())?;
        }
        Commands::Blast {
            name,
            path,
            ...
            let file_path = path_utils::normalize_query_path(&root, &path)?;
            let blast_radius = graph.calculate_blast_radius(&name, &file_path, depth as usize);
            ...
            if format == "mermaid" {
                ...
                emit_query(&blast_radius, json, out.as_ref())?;
            }
        }
        Commands::Validate {
            strict,
            ...
            doc_guard::validate_graph_docstrings(
                &graph,
                &root,
                doc_guard::DocGuardOptions {
                    strict,
                    out: out.as_deref(),
                    diff_only: diff,
                    fix_enabled: fix,
...
fn setup_graph(root_path: &Path) -> Result<(CodeSymbolGraph, PathBuf), GraphQueryError> {
    let root = root_path.canonicalize().map_err(GraphQueryError::Io)?;
...
mod tests {
    ...
    fn test_validate_graph_docstrings_success() {
        ...
        let file_path = root.join("main.rs");
        let mut file = fs::File::create(&file_path).unwrap();
        writeln!(file, "/// Refers to `test_alpha`").unwrap();
        writeln!(file, "fn test_alpha() {{}}").unwrap();
        drop(file);
        ...
        let res = validate_graph_docstrings(
            &graph,
            &root,
            crate::doc_guard::DocGuardOptions {
                strict: true,
                out: None,
                diff_only: false,
                fix_enabled: false,
        ...
        assert!(
            res.is_ok(),
            "Validation should pass since 'test_alpha' exists in the file"
        );
    }
    ...
    fn test_validate_graph_docstrings_auto_fix() {
        ...
        let file_path = root.join("main.rs");
        let mut file = fs::File::create(&file_path).unwrap();
        writeln!(file, "/// Refers to `test_alph`").unwrap();
        writeln!(file, "fn test_alpha() {{}}").unwrap();
        drop(file);
        ...
        let res_no_fix = validate_graph_docstrings(
            &graph,
            &root,
            crate::doc_guard::DocGuardOptions {
                strict: true,
        ...
        let res_fix = validate_graph_docstrings(
            &graph,
            &root,
            crate::doc_guard::DocGuardOptions {
                strict: true,
        ...
        // 3. Read file back to verify the docstring has been replaced with `test_alpha`.
        let content = fs::read_to_string(&file_path).unwrap();
        assert!(content.contains("/// Refers to `test_alpha`"));
    }

    #[test]
    fn test_pagerank_boost_single_application() {
        let temp = tempfile::tempdir().unwrap();
        let root = temp.path().to_path_buf();

        let routes_dir = root.join("routes");
        fs::create_dir_all(&routes_dir).unwrap();
        let file_a = routes_dir.join("system.rs");
        let mut f_a = fs::File::create(&file_a).unwrap();
        writeln!(f_a, "fn route_handler() {{}}").unwrap();
        drop(f_a);

        let utils_dir = root.join("utils");
        fs::create_dir_all(&utils_dir).unwrap();
        let file_b = utils_dir.join("helper.rs");
        let mut f_b = fs::File::create(&file_b).unwrap();
        writeln!(f_b, "fn helper_fn() {{}}").unwrap();
        drop(f_b);
        ...
        let ranked = crate::query_manager::calculate_pagerank(&graph, 0.85, 20);

        let route_node = ranked
            .iter()
            .find(|(idx, _)| graph.graph[*idx].name == "route_handler")
            .expect("route_handler node should exist");
        let helper_node = ranked
            .iter()
            .find(|(idx, _)| graph.graph[*idx].name == "helper_fn")
            .expect("helper_fn node should exist");

        // The boosted node (in "routes") should have a score of roughly 1.2x the unboosted score,
        // NOT 1.2^20 ≈ 38.3x.
        let ratio = route_node.1 / helper_node.1;
        assert!(
            (ratio - 1.2).abs() < 1e-3,
            "Expected boost ratio of ~1.2, got {ratio}"
        );
    }

    #[test]
    fn test_git_error_propagation_in_non_git_dir() {
        let temp = tempfile::tempdir().unwrap();
        let non_git_root = temp.path();

        let res = crate::path_utils::get_git_modified_files(non_git_root);
        assert!(
            res.is_err(),
            "get_git_modified_files must return Err when executed in a non-git directory"
        );
    }

    #[test]
    fn test_html_visualizer_script_injection_escaped() {
        let node = crate::graph::SymbolNode {
        ...
        let mut graph = CodeSymbolGraph::new(PathBuf::from("."));
        let salt = "salt".to_string();
        let _ = graph.build(&salt);

        let html = crate::visualizer::generate_html_visualizer(&[node], &graph);
        assert!(!html.contains("</script><script>alert(1)</script>"));
        assert!(html.contains("\\u003c/script\\u003e"));
    }

    #[test]
    fn test_mermaid_quotes_escaped() {
        let node = crate::graph::SymbolNode {
            name: "symbol_with_\"quotes\"".to_string(),
            path: "src/\"quoted\"/file.rs".to_string(),
            kind: "function".to_string(),
            signature: "fn()".to_string(),
            start_line: 1,
            end_line: 5,
        ...
        let mermaid = crate::visualizer::generate_mermaid_diagram(&[node], &graph);
        assert!(mermaid.contains("#quot;quotes#quot;"));
        assert!(mermaid.contains("#quot;quoted#quot;"));
    }
}

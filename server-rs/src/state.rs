...
impl AppState {
    ...
    pub async fn new_mock() -> Self {
        ...
        let pool = crate::db::init_db(database_url).await.expect("Failed to init test DB");
        ...
        Self {
            ...
            registry: Arc::new(RegistryHub {
                ...
                hooks: Arc::new(crate::agent::hooks::HooksManager::new(&std::path::PathBuf::from("data"))),
            }),
            ...
            resources: Arc::new(ResourceHub {
                ...
                audio_cache: Arc::new(crate::agent::audio_cache::BunkerCache::new("data/test_audio.db".into()).await.unwrap()),
    ...
    pub async fn new() -> Self {
        ...
        let base_dir = std::env::var("WORKSPACE_ROOT")
            ...
            .unwrap_or_else(|_| {
                if std::env::current_dir().unwrap_or_default().ends_with("server-rs") {
                    std::env::current_dir().unwrap_or_default().parent().unwrap_or(&std::env::current_dir().unwrap_or_default()).to_path_buf()
                } else {
        ...
        let mcp_config_opt = if mcp_config_path.exists() { Some(mcp_config_path) } else { None };
        ...
        let registry = Arc::new(RegistryHub {
            ...
            models,
            nodes: DashMap::new(),
            skills: Arc::new(crate::agent::script_skills::ScriptSkillsRegistry::new().await.unwrap()),
            skill_registry: Arc::new(crate::agent::skill_manifest::SkillRegistry::load_all()),
            mcp_host: Arc::new(crate::agent::mcp::McpHost::new(event_tx.clone(), mcp_config_opt)),
...
impl Default for AppState {
    ...
    fn default() -> Self {
        ...
        let registry = Arc::new(RegistryHub {
            ...
            skills: Arc::new(crate::agent::script_skills::ScriptSkillsRegistry::mock()),
            skill_registry: Arc::new(crate::agent::skill_manifest::SkillRegistry::new()),
            mcp_host: Arc::new(crate::agent::mcp::McpHost::new(broadcast::channel(1).0, None)),

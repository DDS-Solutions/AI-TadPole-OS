...
pub fn is_local_model_or_provider(provider: Option<&str>, model_id: &str) -> bool {
    if let Some(p) = provider {
        ...
        if p_lower == "ollama" || p_lower == "local" || p_lower == "localhost" {
            return true;
        }
    }

    let id_lower = model_id.to_lowercase();
    if id_lower.starts_with("ollama/")
        || id_lower.starts_with("local/")
        || id_lower.contains(":latest")
        || id_lower.contains("localhost")
    {
        return true;
    }

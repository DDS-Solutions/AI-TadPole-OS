...
//! - **Subsystem**: Sovereign Engine / Agent Runner / rates
//!
...
/// Checks if a model or provider represents a local, self-hosted deployment (e.g. Ollama, local weights),
/// which incurs zero API usage cost.
pub fn is_local_model_or_provider(provider: Option<&str>, model_id: &str) -> bool {
    ...
    if model_id
        .get(..7)
        .map_or(false, |s| s.eq_ignore_ascii_case("ollama/"))
        || model_id
            .get(..6)
            .map_or(false, |s| s.eq_ignore_ascii_case("local/"))
    {
        return true;
    }

    if model_id.contains(":latest") || model_id.contains("localhost") {
        return true;
    }

    if model_id.bytes().any(|b| b.is_ascii_uppercase()) {
        let id_lower = model_id.to_ascii_lowercase();
        return id_lower.contains(":latest") || id_lower.contains("localhost");
    }

    false
}

/// Calculates the cost in USD for a given token usage, model, and optional provider.
///
/// If the model is hosted locally (e.g. Ollama, localhost), cost is strictly 0.0 USD.
/// If the model is in the registry, its specific rate is applied.
/// If unknown and external, fallback rates ($0.002 / $0.006 per 1k) are applied with a warning.
pub fn calculate_cost_with_provider(
    ...
    if is_local_model_or_provider(provider, model_id) {
        return 0.0;
    }

    let rate = match MODEL_RATES.get(model_id) {
        Some(r) => r,
        None => {
            tracing::warn!(
                "⚠️ [rates] Unknown model '{}' (provider: {:?}) not in pricing table; applying default fallback rate ($0.008/1k).",
                model_id,
                provider
            );
            &ModelRate {
...
pub fn calculate_cost(model_id: &str, input_tokens: u64, output_tokens: u64) -> f64 {
    calculate_cost_with_provider(None, model_id, input_tokens, output_tokens)
}
...
mod tests {
    ...
    #[test]
    fn test_calculate_cost_ollama_local_zero() {
        // Explicit provider = "ollama"
        let cost_provider =
            calculate_cost_with_provider(Some("ollama"), "llama3.1:latest", 100_000, 50_000);
        assert_eq!(cost_provider, 0.0);

        // Fallback detection from tag ":latest"
        let cost_tag = calculate_cost("qwen2.5:latest", 500_000, 200_000);
        assert_eq!(cost_tag, 0.0);

        // Explicit provider = "local"
        let cost_local =
            calculate_cost_with_provider(Some("local"), "gemma4:e4b", 1_000_000, 500_000);

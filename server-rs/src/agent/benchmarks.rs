...
pub async fn list_benchmarks(pool: &SqlitePool) -> Result<Vec<BenchmarkResult>, AppError> {
    let results = sqlx::query_as::<_, BenchmarkResult>(
        "SELECT id, name, category, test_id, mean_ms, p95_ms, p99_ms, target_value, status, metadata, CAST(created_at AS TEXT) as created_at FROM benchmarks ORDER BY created_at DESC LIMIT 100"
    )
    .fetch_all(pool)
    .await?;
    ...
    match test_id {
        ...
        "BM-TOKEN-01" => {
            ...
            for i in 0..1000 {
                ...
                let _ = crate::agent::tokenizer::TokenizerService::count_tokens(model, sample_payload);
            }

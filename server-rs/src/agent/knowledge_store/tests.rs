...
/// Hierarchical 1-hop parent concept expansion (GraphRAG) must augment search result descriptions.
#[tokio::test]
async fn test_expand_parent_concepts() {
    let pool = init_test_db().await;
    let store = KnowledgeStore::new(pool);

    // Insert parent concept
    sqlx::query(
        "INSERT INTO knowledge_store_meta (id, text, title, description, content_hash, topic, created_at, updated_at, concept_type) \
         VALUES ('id-parent', 'Parent platform architectural blueprint', 'Platform Core', 'Core architecture overview', 'hash-p', 'arch', unixepoch(), unixepoch(), 'concept')"
    )
    .execute(&store.pool)
    .await
    .unwrap();

    // Insert child concept with parent_id pointing to 'id-parent'
    sqlx::query(
        "INSERT INTO knowledge_store_meta (id, text, title, description, content_hash, topic, created_at, updated_at, concept_type, parent_id) \
         VALUES ('id-child', 'Authentication microservice patterns', 'Auth Subsystem', 'JWT and OAuth flows', 'hash-c', 'arch', unixepoch(), unixepoch(), 'concept', 'id-parent')"
    )
    .execute(&store.pool)
    .await
    .unwrap();
    ...
    assert_eq!(hits.len(), 1);
    assert_eq!(hits[0].id, "id-child");
    let desc = hits[0]
        .description
        .as_ref()
        .expect("description should be present");
    assert!(desc.contains("JWT and OAuth flows"));
    assert!(desc.contains("[Parent Concept: 'Platform Core' (ID: id-parent)]"));
}

/// Confirming an entry must sync the CONCEPT node and any parent DEPENDS_ON relation to TrustGraph.
#[tokio::test]
async fn test_confirm_syncs_to_trust_graph() {
    let pool = init_test_db().await;
    let store = KnowledgeStore::new(pool.clone());

    // Insert parent and child candidate entries
    sqlx::query(
        "INSERT INTO knowledge_store_meta (id, text, title, description, content_hash, topic, created_at, updated_at, concept_type, parent_id) \
         VALUES ('cand-child', 'Caching layer guidelines', 'Distributed Cache', 'Redis caching rules', 'hash-cand', 'cache', unixepoch(), unixepoch(), 'concept', 'parent-sys')"
    )
    .execute(&store.pool)
    .await
    .unwrap();
    ...
    // Confirm candidate
    let confirmed = store.confirm("cand-child").await.unwrap();
    assert!(confirmed.human_confirmed);
    assert_eq!(confirmed.confidence, 1.0);

    // Verify TrustGraphEngine has vertices and DEPENDS_ON edge
    let trust_engine = crate::agent::trustgraph::TrustGraphEngine::new(pool);
    let subgraph = trust_engine
        .traverse_subgraph(&["concept:cand-child".to_string()], 1)
        .await
        .unwrap();
    ...
    assert!(subgraph
        .nodes
        .iter()
        .any(|n| n.id == "concept:cand-child" && n.r#type == "CONCEPT"));
    assert!(subgraph
        .nodes
        .iter()
        .any(|n| n.id == "concept:parent-sys" && n.r#type == "CONCEPT"));

    assert_eq!(subgraph.relations.len(), 1);
    assert_eq!(subgraph.relations[0].relation_type, "DEPENDS_ON");
    assert_eq!(subgraph.relations[0].source_entity_id, "concept:cand-child");
    assert_eq!(subgraph.relations[0].target_entity_id, "concept:parent-sys");
}

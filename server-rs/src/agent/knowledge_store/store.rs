...
//! - `[Behavioral]` Human confirmation clears TTL, sets confidence to 1.0, and synchronizes concept to TrustGraph (enforced_by: `test_confirm_syncs_to_trust_graph`, `test_confirm_is_idempotent`).
//!
//! ### 🔍 Debugging & Observability
//! - **Local Errors**: none
//! - **Telemetry Targets**: `[IKS]`
//! - **Witness Tests**: `test_confirm_syncs_to_trust_graph`, `test_confirm_is_idempotent`
...
impl KnowledgeStore {
    ...
    pub async fn confirm(&self, id: &str) -> Result<KnowledgeEntry, AppError> {
        ...
        let entry = self.get_by_id_internal(id, false).await?.ok_or_else(|| {
            AppError::NotFound(format!("[IKS] Entry {} not found after confirm", id))
        })?;

        // ── Sync Confirmed Concept to TrustGraph ───────────────────
        let trust_engine = crate::agent::trustgraph::TrustGraphEngine::new(self.pool.clone());
        let concept_node = crate::agent::trustgraph::TrustGraphNode {
            id: format!("concept:{}", entry.id),
            name: entry.title.clone().unwrap_or_else(|| entry.id.clone()),
        ...
        if let Err(e) = trust_engine.add_node(&concept_node).await {
            ...
            if !parent_id.trim().is_empty() {
                ...
                let rel = crate::agent::trustgraph::TrustGraphRelation {
                    id: rel_id,
                    source_entity_id: concept_node.id,
                    target_entity_id: parent_node.id,

...
use super::types::{AddKnowledgeRequest, KnowledgeEntry, DEFAULT_TTL_DAYS};
...
impl KnowledgeStore {
    ...
    pub(super) fn entry_from_row(
        ...
        Ok(KnowledgeEntry {
            ...
            security_tier: row.try_get("security_tier").unwrap_or_else(|_| "BRONZE_ADHOC".to_string()),
    ...
    pub async fn add_entry(
        ...
        let security_tier = req
            .security_tier
            .unwrap_or_else(|| "BRONZE_ADHOC".to_string())
            .to_uppercase();

        // ── SQLite Transaction Atomicity Guard ──────────────────────────────
        ...
        #[cfg(feature = "vector-memory")]
        {
            ...
            if let Err(e) = lance.add_memory(&id, &req.text, &topic, vector).await {
                // Return error prior to tx.commit(), triggering automatic SQLite rollback
                return Err(AppError::InternalServerError(format!("[IKS] LanceDB insert failed: {}", e)));
            }
        }

        // Commit SQLite write only after vector storage succeeds
    ...
    pub async fn get_peers(
        ...
        #[cfg(feature = "vector-memory")]
        {
            ...
            // Security: Sanitize ID to prevent predicate injection
            if !id.chars().all(|c| c.is_alphanumeric() || c == '-' || c == '_') {
                return Err(AppError::InternalServerError("[IKS] Invalid ID format for vector query predicate".to_string()));
            }

            // Retrieve top peers (limit + 1 to account for self-exclusion)
            // L2 distance limit of 0.5 corresponds to high similarity
            let predicate = format!(
                "id != '{}' AND confidence >= 0.0 AND (ttl IS NULL OR ttl > {})",
                id,

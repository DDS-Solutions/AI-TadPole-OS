...
//! - `[Behavioral]` GraphRAG search expands 1-hop parent concepts to enrich description context (enforced_by: `test_expand_parent_concepts`).
//! - `[Behavioral]` SQLite search fallback accurately filters hits by text and metadata query (enforced_by: `test_sqlite_fallback_search_filters_by_query`).
//!
//! ### 🔍 Debugging & Observability
//! - **Local Errors**: none
//! - **Telemetry Targets**: `[IKS]`
//! - **Witness Tests**: `test_expand_parent_concepts`, `test_sqlite_fallback_search_filters_by_query`
...
impl KnowledgeStore {
    ...
    pub async fn search(
        ...
        #[cfg(feature = "vector-memory")]
        {
            ...
            self.expand_parent_concepts(&mut results).await;
            return Ok(results);
        }
    ...
    pub(crate) async fn search_sqlite_fallback(
        ...
        let mut results: Vec<KnowledgeEntry> = candidate_rows
            .into_iter()
    ...
    /// Hierarchical 1-hop parent concept expansion (GraphRAG).
    /// Enriches search result descriptions with parent concept titles and IDs.
    pub(crate) async fn expand_parent_concepts(&self, results: &mut [KnowledgeEntry]) {
        ...
        for chunk in parent_ids.chunks(100) {
            let mut qb: sqlx::QueryBuilder<sqlx::Sqlite> =
                sqlx::QueryBuilder::new("SELECT id, title FROM knowledge_store_meta WHERE id IN (");
        ...
        for entry in results.iter_mut() {
            if let Some(ref pid) = entry.parent_id {
                if let Some(parent_title_opt) = parent_map.get(pid) {
                    let title_display = parent_title_opt
                        .as_deref()
                        .filter(|t| !t.trim().is_empty())
                        .unwrap_or("Untitled Concept");
                    let annotation =
                        format!("\n[Parent Concept: '{}' (ID: {})]", title_display, pid);
                    if let Some(ref mut desc) = entry.description {
                        if !desc.contains(&annotation) {
                            desc.push_str(&annotation);
                        }
                    } else {
                        entry.description = Some(annotation);
                    }
                }
            }
        }

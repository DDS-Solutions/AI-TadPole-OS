...
use crate::agent::types::{
    ...
    EngineAgent, ModelEntry, ProviderConfig, RoleBlueprint, SyncManifest, TokenUsage,
};
    ...
    for row in rows {
        ...
        let agent = EngineAgent {
            ...
            models: AgentModels {
                ...
                planning_slot: row
                    ...
                    .map(|mut m: crate::agent::types::ModelConfig| {
                        if let Some(id) = row.get::<Option<String>, _>("model_2") {
                            m.model_id = id;
                        }
                        m
                    }),
                execution_slot: row
                    ...
                    .and_then(|s| serde_json::from_str(&s).ok())
                    .map(|mut m: crate::agent::types::ModelConfig| {
                        if let Some(id) = row.get::<Option<String>, _>("model_3") {
                            m.model_id = id;
                        }
                        m
                    }),
                active_model_slot: row.get::<Option<i32>, _>("active_model_slot").map(
                    |s| match s {
            ...
            health: AgentHealth {
                ...
                last_failure_at: row
                    .get::<Option<chrono::DateTime<chrono::Utc>>, _>("last_failure_at"),
            ...
            capabilities: AgentCapabilities {
                ...
                workflows: serde_json::from_str(&row.get::<String, _>("workflows"))
                    .unwrap_or_default(),
                mcp_tools: serde_json::from_str(&row.get::<String, _>("mcp_tools"))
                    .unwrap_or_default(),
                skill_manifest: None,
            },
            state: AgentState {
...
pub async fn save_agent_db(pool: &SqlitePool, agent: &EngineAgent) -> Result<(), AppError> {
    ...
    execute_save_agent(&mut conn, agent)
        .await?;
...
pub async fn load_sync_manifests(
    pool: &SqlitePool,
) -> Result<Vec<SyncManifest>, AppError> {
...
pub async fn get_all_sync_manifests(pool: &SqlitePool) -> Result<Vec<SyncManifest>, AppError> {
    let manifests = sqlx::query_as::<_, SyncManifest>("SELECT * FROM sync_manifest")
        .fetch_all(pool)
        .await?;
    Ok(manifests)
}

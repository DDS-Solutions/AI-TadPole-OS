...
pub async fn load_agents_db(pool: &SqlitePool) -> anyhow::Result<Vec<EngineAgent>> {
    let rows = sqlx::query(
        "SELECT
            id,
            name,
...
async fn execute_save_agent<'c, E>(executor: E, agent: &EngineAgent) -> anyhow::Result<()>
where
...
pub async fn save_agent_db(pool: &SqlitePool, agent: &EngineAgent) -> anyhow::Result<()> {
...
pub async fn load_blueprints(pool: &SqlitePool) -> Result<Vec<RoleBlueprint>> {
...
pub async fn save_blueprint(pool: &SqlitePool, blueprint: &RoleBlueprint) -> anyhow::Result<()> {
    execute_save_blueprint(pool, blueprint).await
}

/// Transaction-compatible variant of `save_blueprint`.
pub async fn execute_save_blueprint<'c, E>(executor: E, blueprint: &RoleBlueprint) -> anyhow::Result<()>
where
    E: sqlx::Executor<'c, Database = sqlx::Sqlite>,
{
    sqlx::query(
...
pub async fn delete_blueprint<'c, E>(executor: E, id: &str) -> anyhow::Result<()>
where
    E: sqlx::Executor<'c, Database = sqlx::Sqlite>,
{

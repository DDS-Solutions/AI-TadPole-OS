//! Agent Storage — Database persistence and recovery
//!
//! Synchronizes in-memory registries with persistent SQLite and JSON storage. 
//! Ensures mission continuity and prevents resource exhaustion.
//!
//! @docs ARCHITECTURE:Persistence
//!
//! ### AI Assist Note
//! **Agent Reaping**: Features `reap_stale_agents()`, which resets `busy` 
//! agents to `idle` if their heartbeat exceeds a specified threshold. 
//! This prevents "zombie agents" from blocking mission clusters.
...
pub async fn save_agent_db(pool: &SqlitePool, agent: &EngineAgent) -> Result<()> {
    ...
    sqlx::query("INSERT INTO agents (id, name, role, department, description, model_id, tokens_used, status, theme_color, budget_usd, cost_usd, metadata, skills, workflows, mcp_tools, connector_configs, model_2, model_3, model_config2, model_config3, active_model_slot, voice_id, voice_engine, failure_count, last_failure_at, heartbeat_at, active_mission, provider, api_key, base_url, system_prompt, temperature, category, requires_oversight, working_memory)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)

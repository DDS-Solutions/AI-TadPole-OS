...
pub async fn load_agents_db(pool: &SqlitePool) -> Result<Vec<EngineAgent>> {
    ...
    for row in rows {
        ...
        let agent = EngineAgent {
            ...
            voice_id: Some(row.try_get("voice_id").unwrap_or_else(|_| "alloy".to_string())),
            metadata,
            ...
            category: row.try_get("category").unwrap_or_else(|_| "user".to_string()),

...
pub async fn load_agents_db(pool: &SqlitePool) -> Result<Vec<EngineAgent>> {
    ...
    for row in rows {
        ...
        let provider_str: String = row.try_get("provider").unwrap_or_else(|_| "google".to_string());
...
pub async fn save_agent_db_in_tx(conn: &mut sqlx::SqliteConnection, agent: &EngineAgent) -> Result<()> {
    let metadata_json = serde_json::to_string(&agent.metadata)?;
    let active_mission_json = agent
        .active_mission
        .as_ref()

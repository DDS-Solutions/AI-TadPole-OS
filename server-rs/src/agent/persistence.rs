...
/// ### 🔄 Lifecycle & Atomic Persistence: save_agent_db
/// Persists an agent's current architectural state to the database.
/// 
/// ### 🧬 Workflow: State Polarization
/// 1. Maps complex in-memory HashMaps (Metadata) and vectors (Skills) to JSON strings for SQLite compatibility.
/// 2. Performs an ATOMIC `INSERT OR REPLACE` (upsert) using `ON CONFLICT(id) DO UPDATE`.
/// 3. Ensures that critical financial telemetry (Cost/Tokens) is never lost during high-frequency heartbeat updates.
/// 4. Synchronizes data ingestion manifests (Phase 2) to ensure the `SyncWorker` is aware of new connector configurations.

...
//! - **Subsystem**: Sovereign Engine / Agent Runner / sanitizer
//!
...
static OVERRIDE_PATTERNS_RAW: &[&str] = &[
    r"(?i)ignore all previous instructions",
    r"(?i)system override",
    r"(?i)disregard your directives",
    r"(?i)you are now",
    r"(?i)act as an?",
    r"(?i)new role:",
];

static OVERRIDE_REGEX_SET: Lazy<RegexSet> = Lazy::new(|| {
    RegexSet::new(OVERRIDE_PATTERNS_RAW).expect("Static override pattern set MUST be valid regex.")
});
...
static SSN_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"\b\d{3}-\d{2}-\d{4}\b").expect("Static SSN regex MUST be valid"));
...
impl Sanitizer {
    ...
    pub fn sanitize_content(text: &str) -> String {
        let res = EMAIL_REGEX.replace_all(text, "[REDACTED_EMAIL]");
        let res = SSN_REGEX.replace_all(&res, "[REDACTED_SSN]");
        let res = API_KEY_REGEX.replace_all(&res, "[REDACTED_API_KEY]");
        let mut final_res = res;
        for re in SENSITIVE_PATTERNS.iter() {
            if re.is_match(&final_res) {
                final_res = Cow::Owned(re.replace_all(&final_res, "[REDACTED_TOKEN]").into_owned());
            }
        }
        final_res.into_owned()
    }
    ...
    pub fn sanitize_for_prompt(input: &str) -> String {
        ...
        let stripped = DIRECTIVE_TAG_REGEX
    ...
    fn scan_recursive(text: &str, depth: usize) -> SanitizationResult {
        ...
        let matches = OVERRIDE_REGEX_SET.matches(&normalized);
...
mod tests {
    ...
    fn test_sanitize_directive_case_insensitive() {
        // Lowercase
        let output =
            Sanitizer::sanitize_directive("[user directive start]payload[user directive end]");
        assert!(!output.to_lowercase().contains("user directive"));
        assert!(output.contains("payload"));

        // Mixed case
        let output =
            Sanitizer::sanitize_directive("<User Directive Start>inner<User Directive End>");
        assert!(!output.to_lowercase().contains("user directive"));

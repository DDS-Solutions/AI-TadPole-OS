...
async fn test_e2e_oversight_rest_decision_database_update() {
    ...
    let payload_str = format!("ovs:v1|{}|{}|{}|{}", entry_id, decision_str, timestamp, nonce);
    ...
    .await
    .expect("decide_oversight handler should succeed");
    ...
    let row_updated: (String, String, String) = sqlx::query_as(
        "SELECT status, decision, decided_by FROM oversight_log WHERE id = ?"
    )
    .bind(&entry_id)
    .fetch_one(&state.resources.pool)
    .await
...
async fn test_hardened_signature_verification() {
    use crate::routes::oversight::{verify_oversight_signature, resolve_oversight_decision};
    use crate::agent::types::OversightDecision;
    use ed25519_dalek::{SigningKey, Signer};
    use rand::RngExt;
    ...
    assert!(res.is_ok(), "Valid signature verification failed: {:?}", res);

    // 2. Expired Timestamp (e.g., -6 minutes / -360_000 ms)
    let expired_timestamp = timestamp - 360_000;
    let expired_payload = format!("ovs:v1|{}|{}|{}|{}", entry_id, decision, expired_timestamp, nonce);
    ...
    let overflow_payload = format!("ovs:v1|{}|{}|{}|{}", entry_id, decision, overflow_timestamp, nonce);
    ...
    assert!(res_overflow.is_err(), "Overflowing signature should fail safely");
    ...
    sqlx::query(setup_query).execute(&state.resources.pool).await.unwrap();
    ...
    sqlx::query(entry_insert).bind(entry_id).execute(&state.resources.pool).await.unwrap();
    ...
    state.comms.oversight_queue.insert(entry_id.to_string(), oversight_entry);
    ...
    sqlx::query(entry_insert).bind(entry_id_replay).execute(&state.resources.pool).await.unwrap();
    ...
    state.comms.oversight_queue.insert(entry_id_replay.to_string(), oversight_entry_replay);

    let replay_payload_str = format!("ovs:v1|{}|{}|{}|{}", entry_id_replay, decision, timestamp, nonce);
    ...
    let replay_res = resolve_oversight_decision(&state, entry_id_replay, &replay_decision_payload).await;
    assert!(replay_res.is_err(), "Replay attempt with duplicate nonce should fail");
    assert!(replay_res.unwrap_err().to_string().contains("nonce already used"));
}
...
async fn test_admin_role_authorization() {
    ...
    let req = Request::builder().header("Authorization", "Bearer invalid-token").body(()).unwrap();
    let (mut parts, _) = req.into_parts();
    let res = RequireAdmin::from_request_parts(&mut parts, &state).await;
    assert!(res.is_err(), "Should fail with invalid token");

    // Test with correct token (admin_token) -> should succeed
    let req = Request::builder().header("Authorization", format!("Bearer {}", state.security.admin_token)).body(()).unwrap();
    let (mut parts, _) = req.into_parts();
    let res = RequireAdmin::from_request_parts(&mut parts, &state).await;
    assert!(res.is_ok(), "Should succeed with valid admin token: {:?}", res.err());
}
...
async fn test_pinned_public_key_verification() {
    ...
    assert!(res1.is_ok(), "Verification with pinned key should succeed: {:?}", res1);
    ...
    assert!(res2.unwrap_err().contains("does not match the pinned OVERSIGHT_PUBLIC_KEY"));

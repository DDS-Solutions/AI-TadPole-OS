//! Voice Cache — Optimized storage for synthesized engine speech
//!
...
impl BunkerCache {
    ...
    pub async fn get(&self, text: &str) -> Result<Option<Vec<u8>>> {
        ...
        let row: Option<(Vec<u8>,)> =
            sqlx::query_as::<sqlx::Sqlite, (Vec<u8>,)>("SELECT audio_data FROM audio_cache WHERE hash = ?")
                .bind(hash)
    ...
    pub async fn set(&self, text: &str, audio_data: Vec<u8>) -> Result<()> {
        let hash = format!("{:x}", md5::compute(text));
        sqlx::query::<sqlx::Sqlite>("INSERT OR REPLACE INTO audio_cache (hash, audio_data) VALUES (?, ?)")
            .bind(hash)
            .bind(audio_data)
            .execute(&self.pool)
            .await?;

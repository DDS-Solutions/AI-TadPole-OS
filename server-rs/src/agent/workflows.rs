...
pub async fn load_workflow(
    ...
    let agent_workflows_path = base_dir.join(".agent").join("workflows").join(&file_name);
    ...
    {
        canonical_path
    } else if tokio::fs::try_exists(&agent_workflows_path)
        .await
        .map_err(AppError::Io)?
    {
        agent_workflows_path
        ...
        .map_err(AppError::Io)?
    {
        ...
        return Err(AppError::NotFound(format!(
            "Workflow file '{}' was not found in directives/, .agent/workflows/, or data/workflows/",
            file_name
        )));
    };
...
mod tests {
    ...
    #[tokio::test]
    async fn loads_canonical_orchestrate_workflow() {
        let workspace = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .unwrap();
        let workflow = load_workflow(workspace, "orchestrate").await.unwrap();

        assert_eq!(workflow.workflow_name, "orchestrate");
        assert_eq!(workflow.steps.len(), 8);
        assert!(workflow.steps[0].title.contains("Gap-Finding Phase"));
        assert!(workflow.steps[7].title.contains("Swarm Monitoring"));
    }

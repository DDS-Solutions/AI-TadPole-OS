...
impl McpClient {
    pub async fn spawn(command_line: &str) -> Result<Self, AppError> {
        ...
        let stdin = child
            .stdin
            .take()
            .ok_or_else(|| AppError::InternalServerError("Failed to open stdin".to_string()))?;
        let stdout = child
            .stdout
            .take()
            .ok_or_else(|| AppError::InternalServerError("Failed to open stdout".to_string()))?;
    ...
    pub async fn call(&mut self, method: &str, params: Value) -> Result<Value, AppError> {
        ...
        loop {
            ...
            if let Ok(raw_val) = serde_json::from_str::<Value>(&response_line) {
                // If it is a notification (contains method and does not contain id), we skip it.
                ...
                if let Some(resp_id) = raw_val.get("id") {
                    if resp_id == &target_id {
                        ...
                        return response.result.ok_or_else(|| AppError::InfrastructureError {
                            provider_id: "mcp_client".to_string(),
                            detail: "Missing result in MCP response".to_string(),
                            help_link: None,
                        });
                    } else {
                        debug!("<< [MCP] Mismatched JSON-RPC response ID (expected {:?}, got {:?}): {}", target_id, resp_id, response_line.trim());
                    }
                }
            } else {
                debug!("<< [MCP] Malformed JSON-RPC message: {}", response_line.trim());
            }
        }
    }

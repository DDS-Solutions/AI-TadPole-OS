...
/// Acts as the central dispatcher for both **Native Rust Handlers** (high performance)
/// and **External MCP Servers** (modular extensions). 
pub struct McpHost {
    /// Registry of all statically and dynamically loaded tool handlers.
    pub registry: Arc<Mutex<McpRegistry>>,
    /// Multi-agent telemetry hub for tracking tool performance.
    pub stats: Arc<dashmap::DashMap<String, McpToolStats>>,
    event_tx: broadcast::Sender<serde_json::Value>,
    mcp_config_path: Option<PathBuf>,
    /// Global sovereign permission gateway (SEC-04).
    pub policy: Arc<PermissionPolicy>,
    /// Optional UI hook for user approval prompts.
    pub prompter: Option<Arc<dyn PermissionPrompter>>,
    /// Active connections to external MCP servers.
...
impl McpHost {
    ...
    pub async fn list_tools(
        ...
        if let Some(ref path) = self.mcp_config_path {
            // SEC-03: Harden path access to prevent uncontrolled path expression vulnerabilities.
            // We authorize config loading only from the current execution context or project root.
    ...
    /// 
    /// ### 🛡️ Sovereign Security Gateway (SEC-04)
    /// Every tool call MUST pass through this gateway. It enforces 
    /// **Permission Polarization**:
    /// 1. `Deny`: Immediate fatal rejection for unsafe tools.
    /// 2. `Prompt`: Suspends execution to await UI/User confirmation.
    /// 3. `Allow`: Transparent execution for authorized system tools.
    ...
    async fn execute_tool_internal(
        ...
        // 3. External MCP Tooling (Dynamic Bridge)
    ...
    async fn get_or_spawn_client(&self, server_name: &str) -> anyhow::Result<Arc<Mutex<client::McpClient>>> {
        ...
        // Spawn new client
        let config_path = self.mcp_config_path.as_ref().ok_or_else(|| anyhow::anyhow!("MCP config path not set"))?;

...
//! handlers (e.g., `recruit_specialist`) and external MCP servers
...
impl McpHost {
    ...
    pub async fn list_tools(
        ...
        if let Some(ref path) = self.mcp_config_path {
            ...
            if let Ok(safe_path) =
                crate::utils::security::validate_path(&authorized_base, &path.to_string_lossy())
            {
            ...
            PermissionMode::Prompt => {
                if let Some(ref prompter) = self.prompter {
                    let decision = prompter
                        .prompt_user(tool_name, &arguments.to_string())
            ...
            if parts.len() >= 3 {
                ...
                let result = client_lock
                    ...
                    .map_err(|e| AppError::InfrastructureError {
                        ...
                        detail: format!("MCP server '{}' tool execution failed: {}", server_name, e),
                        help_link: None,
                    })?;
    ...
    async fn get_or_spawn_client(
        ...
        let mut client = client::McpClient::spawn(&full_command).await.map_err(|e| {
    ...
    fn update_stats(&self, tool_name: &str, is_success: bool, latency: u64) {
        ...
        if entry.avg_latency_ms == 0 {
            ...
            let prev_count = entry.invocations.saturating_sub(1);
            entry.avg_latency_ms =
                ((entry.avg_latency_ms * prev_count) + latency) / entry.invocations;
        }
    }

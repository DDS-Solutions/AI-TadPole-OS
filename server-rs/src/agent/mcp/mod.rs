...
impl McpHost {
    ...
    fn update_stats(&self, tool_name: &str, is_success: bool, latency: u64) {
        ...
        if entry.avg_latency_ms == 0 {
            ...
            entry.avg_latency_ms = ((entry.avg_latency_ms * prev_count) + latency) / entry.invocations;
        }
    }
    ...
    async fn execute_legacy_skill(
        ...
        let allowed_binaries = ["python", "python3", "node", "sh", "bash", "cmd", "powershell", "pwsh", "echo", "ls"];

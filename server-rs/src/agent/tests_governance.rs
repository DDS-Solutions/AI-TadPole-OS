...
async fn test_mandatory_oversight_gate() {
        ...
        for _ in 0..300 {
            ...
            if let Some(entry_id) = entry_id_opt {
                ...
                if let Some((_, tx)) = state_clone.comms.oversight_resolvers.remove(&entry_id) {
                    ...
                    let _ = tx.send(crate::agent::types::OversightResolution { approved: true, override_slot: None });
                    return;
                }
            }
    ...
    let result = runner
        .execute_tool(&ctx, &fc, "user message")
        .await;
    ...
    assert!(result.is_ok());
    let (_res_text, _res_usage) = result.unwrap();
}
...
async fn test_mandatory_oversight_rejection() {
        ...
        for _ in 0..300 {
            ...
            if let Some(entry_id) = entry_id_opt {
                ...
                if let Some((_, tx)) = state_clone.comms.oversight_resolvers.remove(&entry_id) {
                    ...
                    let _ = tx.send(crate::agent::types::OversightResolution { approved: false, override_slot: None });
                    return;
                }
            }
            tokio::time::sleep(std::time::Duration::from_millis(100)).await;
    ...
    use crate::agent::runner::service_traits::ToolExecutor;
    if let Ok((res_text, _res_usage)) = runner
        .execute_tool(&ctx, &fc, "user message")
        .await
    {
        output_str = res_text;

...
//! - **Subsystem**: Sovereign Engine / Agent Runner / tests_governance
//!
...
async fn test_mandatory_oversight_gate() {
    ...
    let result = runner.execute_tool(&ctx, &fc, "user message").await;
...
async fn test_mandatory_oversight_rejection() {
    ...
    use crate::agent::runner::service_traits::ToolExecutor;
    if let Ok((res_text, _res_usage)) = runner.execute_tool(&ctx, &fc, "user message").await {
        output_str = res_text;
    }

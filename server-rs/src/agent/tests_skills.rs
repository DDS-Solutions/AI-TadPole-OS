...
//! - **Subsystem**: Sovereign Engine / Agent Runner / tests_skills
//!
...
mod recruitment_tests {
    ...
    async fn test_ensure_sub_agent_exists_provider_fallback() {
        ...
        state
            .registry
            .agents
            .insert(agent_id.to_string(), mock_agent.clone());
        ...
        let resolved_agent = runner
            .ensure_sub_agent_exists(&mut conn, opts)
            .await
            .unwrap();
        if let Some(agent) = resolved_agent.staged_agent {
            state.registry.agents.insert(resolved_agent.id, agent);
        }
        ...
        assert_eq!(
            resolved.models.model.provider,

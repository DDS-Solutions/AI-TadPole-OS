    ...
    pub fn apply_to(&self, agent: &mut EngineAgent) -> bool {
        ...
        if let Some(mc) = &self.model_config {
            ...
            if let Some(top_p) = self.top_p {
                let mut extras = agent.models.model.extra_parameters.clone().unwrap_or_default();
...
mod tests {
    ...
    fn test_agent_config_sampling_params_merge() {
        ...
        let extras = agent.models.model.extra_parameters.as_ref().expect("Extras missing");
        assert_eq!(extras.get("top_p").unwrap(), &serde_json::json!(0.9f32));
        assert_eq!(extras.get("top_k").unwrap(), &serde_json::json!(50));
        
        assert_eq!(agent.models.model.provider, crate::agent::types::ModelProvider::Anthropic);
        assert_eq!(agent.models.model.model_id, "claude-3");
    }
}

...
//! - **Subsystem**: Sovereign Engine / Agent Runner / merge
//!
...
use crate::agent::types::{ConnectorConfig, EngineAgent, ModelConfig, ModelProvider};
    ...
    pub fn apply_to(&self, agent: &mut EngineAgent) -> bool {
        ...
        if let Some(mc) = &self.model_config {
            ...
            if let Some(top_p) = self.top_p {
                let mut extras = agent
                    .models
                    .model
                    .extra_parameters
                    .clone()
                    .unwrap_or_default();
                extras.insert("top_p".to_string(), serde_json::json!(top_p));
                agent.models.model.extra_parameters = Some(extras);
                changed = true;
            }
            if let Some(top_k) = self.top_k {
                let mut extras = agent
                    ...
                    .clone()
                    .unwrap_or_default();
                extras.insert("top_k".to_string(), serde_json::json!(top_k));
                agent.models.model.extra_parameters = Some(extras);
                changed = true;
            }
        ...
        if let Some(m2) = &self.model_2 {
            if let Some(slot) = agent.models.planning_slot.as_mut() {
                slot.model_id = m2.clone();
            }
            changed = true;
        }
        if let Some(m3) = &self.model_3 {
            if let Some(slot) = agent.models.execution_slot.as_mut() {
                slot.model_id = m3.clone();
            }
            changed = true;
        }
        if let Some(active_slot) = self.active_model_slot {
            agent.models.active_model_slot = Some(match active_slot {
        ...
        if let Some(mc2) = &self.model_config2 {
            agent.models.planning_slot = Some(mc2.clone());
            changed = true;
        }
        if let Some(mc3) = &self.model_config3 {
            agent.models.execution_slot = Some(mc3.clone());
            changed = true;
        }
        ...
        if let Some(it) = self.input_tokens {
            agent.economics.token_usage.input_tokens = it as u64;
            changed = true;
        }
        if let Some(ot) = self.output_tokens {
            agent.economics.token_usage.output_tokens = ot as u64;
            changed = true;
        }
        if let Some(tt) = self.total_tokens {
            agent.economics.token_usage.total_tokens = tt as u64;
            changed = true;
        }
        if let Some(tu) = self.tokens_used {
            agent.economics.tokens_used = tu as u64;
            changed = true;
        }
...
mod tests {
    ...
    fn test_nested_model_config_deserialization() {
        ...
        let update: AgentConfigUpdate =
            serde_json::from_str(json).expect("Nested deserialization failed");
    ...
    fn test_nested_model_config_slots_2_3_deserialization() {
        ...
        let update: AgentConfigUpdate =
            serde_json::from_str(json).expect("Nested slots 2/3 deserialization failed");
        ...
        assert_eq!(
            update.model_config3.as_ref().unwrap().model_id,
            "gemini-1.5-pro"
        );
    }
    ...
    fn test_agent_config_sampling_params_merge() {
        ...
        let extras = agent
            ...
            .extra_parameters
            .as_ref()
            .expect("Extras missing");
        assert_eq!(extras.get("top_p").unwrap(), &serde_json::json!(0.9f32));
        assert_eq!(extras.get("top_k").unwrap(), &serde_json::json!(50));

        assert_eq!(
            agent.models.model.provider,
            crate::agent::types::ModelProvider::Anthropic
        );
        assert_eq!(agent.models.model.model_id, "claude-3");
    }
}

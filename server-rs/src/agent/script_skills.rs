...
impl ScriptSkillsRegistry {
    ...
    pub async fn reload_all(&self) -> Result<(), AppError> {
        ...
        // Apply loaded maps using synchronized retain and insert
        self.skills.retain(|k, _| temp_skills.contains_key(k));
        for (k, v) in temp_skills {
            self.skills.insert(k, v);
        }

        self.workflows.retain(|k, _| temp_workflows.contains_key(k));
        for (k, v) in temp_workflows {
            self.workflows.insert(k, v);
        }

        self.hooks.retain(|k, _| temp_hooks.contains_key(k));

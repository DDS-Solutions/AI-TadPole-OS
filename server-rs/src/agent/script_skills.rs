...
impl ScriptSkillsRegistry {
    ...
    pub async fn reload_all(&self) -> Result<(), AppError> {
        ...
        match fs::read_dir(&self.skills_dir).await {
                        ...
                        match read_file_safe(&path).await {
                            Ok(content) => {
                                match serde_json::from_str::<SkillDefinition>(&content) {
                                    ...
                                    Err(e) => {
                                        tracing::warn!("⚠️ Failed to parse skill JSON at {:?}: {}", path, e);
                                    }
                                }
                            }
                            Err(e) => {
            ...
            Err(e) => {
                tracing::warn!("⚠️ Failed to read skills directory {:?}: {}", self.skills_dir, e);
            }
        }
        ...
        match fs::read_dir(&self.agent_skills_dir).await {
                        ...
                        match read_file_safe(&path).await {
                            Ok(content) => {
                                match serde_json::from_str::<SkillDefinition>(&content) {
                                    ...
                                    Err(e) => {
                                        tracing::warn!("⚠️ Failed to parse agent skill JSON at {:?}: {}", path, e);
                                    }
                                }
                            }
                            Err(e) => {
                                tracing::warn!("⚠️ Failed to read agent skill file {:?}: {}", path, e);
                            }
                        }
                    }
                }
            ...
            Err(e) => {
                tracing::warn!("⚠️ Failed to read agent skills directory {:?}: {}", self.agent_skills_dir, e);
            }
        }
        ...
        match fs::read_dir(&built_in_agent_skills_dir).await {
                            ...
                            match read_file_safe(&skill_md).await {
                                Ok(content) => {
                                    if let Some(skill) = parse_skill_md(&content) {
                                        ...
                                        tracing::warn!("⚠️ Failed to parse built-in skill MD at {:?}", skill_md);
                                    }
                                }
                                Err(e) => {
                                    tracing::warn!("⚠️ Failed to read built-in skill MD file {:?}: {}", skill_md, e);
                                }
                            }
                        }
                    }
            ...
            Err(e) => {
                tracing::warn!("⚠️ Failed to read built-in agent skills directory {:?}: {}", built_in_agent_skills_dir, e);
            }
        }
        ...
        match fs::read_dir(&self.workflows_dir).await {
            ...
            Err(e) => {
                tracing::warn!("⚠️ Failed to read workflows directory {:?}: {}", self.workflows_dir, e);
            }
        }
        ...
        match fs::read_dir(&built_in_agent_workflows_dir).await {
                ...
                while let Ok(Some(entry)) = entries.next_entry().await {
                    ...
                    if path.extension().and_then(|s| s.to_str()) == Some("md") {
                        match read_file_safe(&path).await {
                            ...
                            Err(e) => {
                                tracing::warn!("⚠️ Failed to read agent workflow file {:?}: {}", path, e);
                            }
                        }
                    }
                }
            ...
            Err(e) => {
                tracing::warn!("⚠️ Failed to read built-in agent workflows directory {:?}: {}", built_in_agent_workflows_dir, e);
            }
        }
        ...
        match fs::read_dir(&self.agent_workflows_dir).await {
                ...
                while let Ok(Some(entry)) = entries.next_entry().await {
                    ...
                    if path.extension().and_then(|s| s.to_str()) == Some("md") {
                        match read_file_safe(&path).await {
                            ...
                            Err(e) => {
                                tracing::warn!("⚠️ Failed to read agent-generated workflow file {:?}: {}", path, e);
                            }
                        }
                    }
                }
            ...
            Err(e) => {
                tracing::warn!("⚠️ Failed to read agent workflows directory {:?}: {}", self.agent_workflows_dir, e);
            }
        }
        ...
        match fs::read_dir(&self.hooks_dir).await {
                        ...
                        match read_file_safe(&path).await {
                            Ok(content) => {
                                match serde_json::from_str::<HookDefinition>(&content) {
                                    ...
                                    Err(e) => {
                                        tracing::warn!("⚠️ Failed to parse hook JSON at {:?}: {}", path, e);
                                    }
                                }
                            }
                            Err(e) => {
            ...
            Err(e) => {
                tracing::warn!("⚠️ Failed to read hooks directory {:?}: {}", self.hooks_dir, e);
            }
        }
        ...
        match fs::read_dir(&self.agent_hooks_dir).await {
                        ...
                        match read_file_safe(&path).await {
                            Ok(content) => {
                                match serde_json::from_str::<HookDefinition>(&content) {
                                    ...
                                    Err(e) => {
                                        tracing::warn!("⚠️ Failed to parse agent hook JSON at {:?}: {}", path, e);
                                    }
                                }
                            }
                            Err(e) => {
                                tracing::warn!("⚠️ Failed to read agent hook file {:?}: {}", path, e);
                            }
                        }
                    }
                }
            ...
            Err(e) => {
                tracing::warn!("⚠️ Failed to read agent hooks directory {:?}: {}", self.agent_hooks_dir, e);
            }
        }

        // Apply loaded maps using synchronized retain and insert (reordered to eliminate race window)
        let temp_skill_keys: std::collections::HashSet<String> = temp_skills.keys().cloned().collect();
        for (k, v) in temp_skills {
            self.skills.insert(k, v);
        }
        self.skills.retain(|k, _| temp_skill_keys.contains(k));

        let temp_workflow_keys: std::collections::HashSet<String> = temp_workflows.keys().cloned().collect();
        for (k, v) in temp_workflows {
            self.workflows.insert(k, v);
        }
        self.workflows.retain(|k, _| temp_workflow_keys.contains(k));

        let temp_hook_keys: std::collections::HashSet<String> = temp_hooks.keys().cloned().collect();
    ...
    pub async fn save_skill(&self, mut skill: SkillDefinition) -> Result<(), AppError> {
        ...
        skill.security_report = Some(serde_json::to_value(&scan_res).unwrap_or(serde_json::json!({ "filtered_findings": [] })));
    ...
    pub async fn save_agent_workflow(
        ...
        let scan_res = crate::security::skillspector::scan_content(&workflow.content, "WORKFLOW.md")?;
        if scan_res.risk_score >= 50 {
            return Err(AppError::Forbidden(format!(
    ...
    pub async fn save_workflow(&self, mut workflow: WorkflowDefinition) -> Result<(), AppError> {
        let scan_res = crate::security::skillspector::scan_content(&workflow.content, "WORKFLOW.md")?;
        if scan_res.risk_score >= 50 {
            return Err(AppError::Forbidden(format!(
                "Security audit failed: workflow has a high-risk security score of {} (Severity: {}). Registration rejected.",
        ...
        workflow.category = "user".to_string();
        write_atomic(&path, workflow.content.as_bytes()).await?;
    ...
    pub async fn save_hook(&self, mut hook: HookDefinition) -> Result<(), AppError> {
        let scan_res = crate::security::skillspector::scan_content(&hook.content, "HOOK.json")?;
        if scan_res.risk_score >= 50 {
            return Err(AppError::Forbidden(format!(
                "Security audit failed: hook has a high-risk security score of {} (Severity: {}). Registration rejected.",
...
async fn read_file_safe(path: &Path) -> Result<String, AppError> {
    ...
    if meta.file_type().is_symlink() {
        return Err(AppError::Forbidden("Security boundary: Symlinks are not allowed".into()));
    }
...
pub fn parse_skill_md(content: &str) -> Option<SkillDefinition> {
    ...
    // Find the ending '---' at the start of a line
    let mut lines = content_trimmed.lines();
    // Skip the first line which is the starting '---'
    ...
    if !found_end {
        return None; // No ending frontmatter boundary
    }
    ...
    if yaml_str.len() > 65536 {
        tracing::warn!("⚠️ Guard: frontmatter size exceeds limit of 64 KiB ({} bytes)", yaml_str.len());
        return None;
    }

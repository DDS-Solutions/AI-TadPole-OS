...
//! - **Subsystem**: Sovereign Engine / Agent Runner / script_skills
//!
...
//! - **Witness Tests**: tests::test_parse_skill_md_basic, tests::test_parse_skill_md_title_fallback, tests::test_parse_skill_md_invalid, tests::test_parse_skill_md_with_schema, tests::test_parse_skill_md_robustness, tests::test_parse_skill_md_body_separators, tests::test_parse_skill_md_size_limit
...
impl ScriptSkillsRegistry {
                            ...
                            Ok(content) => {
                                match serde_json::from_str::<SkillDefinition>(&content) {
                                    Ok(mut skill) => {
                                        ...
                                        skill.oversight_required = true; // [R-a] Force oversight for agent-generated skills on disk load
                                        temp_skills.insert(skill.name.clone(), skill);
                                    }
                                    Err(e) => {
                                        tracing::warn!(
    ...
    async fn save_skill_internal(
        ...
        content_to_scan.push_str(&skill.name);
        content_to_scan.push('\n');
        content_to_scan.push_str(&skill.description);
        content_to_scan.push('\n');
        ...
        // 1. Primary documentation & metadata scan as Markdown
        let mut scan_res =
            crate::security::skillspector::scan_content(&content_to_scan, "SKILL.md").await?;

        // 2. [F2] Dedicated executable code scan preserving script AST type (.py / .sh / .ps1)
        if !skill.execution_command.trim().is_empty() {
            let cmd_lower = skill.execution_command.to_lowercase();
            let script_ext = if cmd_lower.contains("python")
                || cmd_lower.contains("import ")
                || cmd_lower.contains("def ")
            {
                "script.py"
            } else if cmd_lower.contains("powershell") || cmd_lower.contains(".ps1") {
                "script.ps1"
            } else {
                "script.sh"
            };
            ...
            let code_scan =
                crate::security::skillspector::scan_content(&skill.execution_command, script_ext)
                    .await?;
            if code_scan.risk_score > scan_res.risk_score {
                scan_res.risk_score = code_scan.risk_score;
                scan_res.risk_severity = code_scan.risk_severity;
                scan_res.risk_recommendation = code_scan.risk_recommendation;
            }
            scan_res
                .filtered_findings
                .extend(code_scan.filtered_findings);
        }

        // 3. [R-c] Dedicated executable code scan for verification_script if present
        if let Some(ref script) = skill.verification_script {
            if !script.trim().is_empty() {
                let script_lower = script.to_lowercase();
                let verify_ext = if script_lower.contains("python")
                    || script_lower.contains("import ")
                    || script_lower.contains("def ")
                {
                    "verify.py"
                } else if script_lower.contains("powershell") || script_lower.contains(".ps1") {
                    "verify.ps1"
                } else {
                    "verify.sh"
                };
                ...
                let verify_scan =
                    crate::security::skillspector::scan_content(script, verify_ext).await?;
                if verify_scan.risk_score > scan_res.risk_score {
                    scan_res.risk_score = verify_scan.risk_score;
                    scan_res.risk_severity = verify_scan.risk_severity;
                    scan_res.risk_recommendation = verify_scan.risk_recommendation;
                }
                scan_res
                    .filtered_findings
                    .extend(verify_scan.filtered_findings);
            }
        }

        skill.security_score = Some(scan_res.risk_score);
        skill.security_severity = Some(scan_res.risk_severity.clone());
        if scan_res.risk_score >= crate::security::skillspector::RISK_REJECT_THRESHOLD {
    ...
    async fn save_workflow_internal(
        ...
        let scan_res =
            crate::security::skillspector::scan_content(&workflow.content, "WORKFLOW.md").await?;
        if scan_res.risk_score >= crate::security::skillspector::RISK_REJECT_THRESHOLD {
            return Err(AppError::Forbidden(format!(
    ...
    async fn save_hook_internal(
        ...
        let scan_res =
            crate::security::skillspector::scan_content(&hook.content, "HOOK.json").await?;
        if scan_res.risk_score >= crate::security::skillspector::RISK_REJECT_THRESHOLD {
            return Err(AppError::Forbidden(format!(
    ...
    pub async fn save_skill(&self, skill: SkillDefinition) -> Result<(), AppError> {
        self.save_skill_internal(skill, &self.skills_dir, "user")
            .await
    }

    /// Saves a skill to the dedicated agent_generated directory.
    /// Category-Based Override: Agent-generated skills unconditionally require human oversight.
    pub async fn save_agent_skill(&self, mut skill: SkillDefinition) -> Result<(), AppError> {
        ...
        self.save_skill_internal(skill, &self.agent_skills_dir, "agent_generated")
            .await
    }

    pub async fn save_workflow(&self, workflow: WorkflowDefinition) -> Result<(), AppError> {
        self.save_workflow_internal(workflow, &self.workflows_dir, "user")
            .await
    }
    ...
    pub async fn save_agent_workflow(&self, workflow: WorkflowDefinition) -> Result<(), AppError> {
        self.save_workflow_internal(workflow, &self.agent_workflows_dir, "agent_generated")
            .await
    }
    ...
    pub async fn save_agent_hook(&self, hook: HookDefinition) -> Result<(), AppError> {
        self.save_hook_internal(hook, &self.agent_hooks_dir, "agent_generated")
            .await
    }
    ...
    pub async fn delete_skill(&self, name: &str) -> Result<(), AppError> {
        ...
        if let Ok(path) = crate::utils::security::validate_path(&self.skills_dir, &filename) {
            let _ = delete_safe(&path).await;
        }
        if let Ok(agent_path) =
            crate::utils::security::validate_path(&self.agent_skills_dir, &filename)
        {
            let _ = delete_safe(&agent_path).await;
        }
    ...
    pub async fn delete_workflow(&self, name: &str) -> Result<(), AppError> {
        ...
        if let Ok(agent_path) =
            crate::utils::security::validate_path(&self.agent_workflows_dir, &filename)
        {
            let _ = delete_safe(&agent_path).await;
        }
    ...
    pub async fn delete_hook(&self, name: &str) -> Result<(), AppError> {
        ...
        if let Ok(agent_path) =
            crate::utils::security::validate_path(&self.agent_hooks_dir, &filename)
        {
            let _ = delete_safe(&agent_path).await;
        }

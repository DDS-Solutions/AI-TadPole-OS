...
impl OpenAIProvider {
    ...
    async fn generate_with_fallback(
        ...
        let request_timeout_secs = if self.config.provider == ModelProvider::Ollama
            || self.config.provider == ModelProvider::OllamaCloud
        {
            600 // 🛡️ [Hardening] 10 minutes for local model CPU/GPU inference & prompt pre-fill
        } else {
            180 // 3 minutes for cloud APIs
        };
        ...
        let res = loop {
            ...
            match res_result {
                ...
                Err(e) => {
                    let is_ollama_offline =
                        self.config.provider == ModelProvider::Ollama && e.is_connect();
                    let is_ollama_timeout =
                        self.config.provider == ModelProvider::Ollama && e.is_timeout();

                    let app_err = if is_ollama_offline {
                        AppError::OllamaOffline(e.to_string())
                    } else if is_ollama_timeout {
                        AppError::InfrastructureError {
                            provider_id: ProviderId::Ollama,
                            kind: InfrastructureErrorKind::Timeout,
                            detail: format!(
                                "Ollama model '{}' inference timed out after {}s: {}",
                                self.config.model_id, request_timeout_secs, e
                            ),
                            help_link: Some(
                                "https://docs.tadpoleos.dev/configuration/models".to_string(),
                            ),
                        }
                    } else {
                        AppError::from(e)

...
static FUNCTION_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?s)<function=([a-zA-Z0-9_-]+)[^\{]*(\{.*?\})[^>]*>?")
        .expect("Static tool-call parser regex MUST be valid.")
});
...
impl OpenAIProvider {
    ...
    async fn generate_with_fallback(
        ...
        let mut url = if let Some(base) = &self.config.base_url {
            base.clone()
        ...
        // If running in Docker, redirect loopback to the host bridge.
        ...
        if !has_vectors {
            ...
            tracing::info!(
                "🧠 [OBLITERATUS] Steering vectors active. Stripping bulky system prompt."
            );
        }
            ...
            match res_result {
                ...
                Err(e) => {
                    ...
                    if app_err.is_retryable() && attempts < max_attempts - 1 {
                        ...
                        let delay =
                            (500 * (2u64.pow(attempts as u32)) as i64 + jitter).max(100) as u64;
        ...
        let choice = parsed.choices.first().ok_or_else(|| {
            AppError::InternalServerError("No completion return from OpenAI".to_string())
        })?;

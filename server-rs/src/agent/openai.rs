...
impl OpenAIProvider {
    ...
    /// Wrapper with recursion tracking for quantization fallback retries.
    async fn generate_with_fallback(
        ...
        // Hardening: Resolve IPv4/IPv6 localhost conflict on Windows by forcing 127.0.0.1
        ...
        if !res.status().is_success() {
            ...
            // 🧠 [OBLITERATUS] Item 6: Dynamic Quantization Fallback
            // Intercept OOM (Out Of Memory) from local providers (Ollama, etc)
            ...
            if is_local && is_oom_error(&error_text) && !is_fallback {
                ...
                // If not already quantized, try q4 fallback
                if !current_model.contains(":q") && !current_model.contains("-q") {
                    ...
                    // Create a temporary clone with the new model ID

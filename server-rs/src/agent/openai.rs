...
impl OpenAIProvider {
    ...
    async fn generate_with_fallback(
        ...
        if has_vectors {
            tracing::debug!(
                "🧠 Steering vectors active in addition to system prompt."
            );
        }
        ...
        let request_body = OpenAIRequest {
            ...
            max_tokens: {
                ...
                if limit.is_none() && (self.config.provider == ModelProvider::Ollama || self.config.provider == ModelProvider::OllamaCloud) {
                    limit = Some(2048);
                }
                limit
            },

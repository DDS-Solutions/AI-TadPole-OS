//! Phase 4: Layout-Aware Document Parsing for SME Business Documents.
//!
//! Provides structured text extraction from common business document formats
//! (CSV, PDF text-layer, Markdown, plain text). Each parser returns a
//! `ParsedDocument` with sections that preserve the original structure,
//! enabling higher-quality embeddings for the VectorMemory RAG pipeline.
//!
//! # Supported Formats
//! - `.txt` — raw text pass-through
//! - `.md`  — Markdown with header-based sectioning
//! - `.csv` — row-level chunking with header context
//! - `.pdf` — basic text-layer extraction (no OCR; future: Tesseract/Docling)

...
const CAPABILITY_PATTERNS: &[CapabilityPattern] = &[
    ...
    CapabilityPattern {
        slugs: &["gpt-4o", "gpt-5", "gpt-3.5-turbo", "gemini", "-pro", "-flash"],
        update: |c| c.supports_structured_output = true,
    },
    CapabilityPattern {
        slugs: &["reasoning", "-o1", "-o3", "deepseek-r1", "-r1", "mai-thinking"],
        update: |c| {
        ...
        D: Deserializer<'de>,
    {
        ...
        Ok(Self {
            ...
            models: AgentModels {
                model_id,
                model,
                planning_slot: wire.model_config2.map(|m| ModelConfig {
                    model_id: wire.model_2.unwrap_or_default(),
                    ..m
                }),
                execution_slot: wire.model_config3.map(|m| ModelConfig {
                    model_id: wire.model_3.unwrap_or_default(),
                    ..m
                }),
...
impl EngineAgent {
    ...
    pub fn resolve_provider_context(
        ...
        crate::agent::runner::RunContext {
            ...
            authority_level: crate::agent::types::RoleAuthorityLevel::from_role(
                &self.identity.role,
            ),

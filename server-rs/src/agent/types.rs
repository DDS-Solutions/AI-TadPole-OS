...
pub struct EngineAgent {
    ...
    #[serde(default, rename = "currentTask", skip_serializing_if = "Option::is_none")]
...
impl EngineAgent {
    pub fn resolve_provider_context(&self) -> crate::agent::runner::RunContext {
        crate::agent::runner::RunContext {
            ...
            fs_adapter: crate::adapter::filesystem::FilesystemAdapter::new(std::path::PathBuf::from("data/workspaces/default")),
            safe_mode: false,
            analysis: false,
            traceparent: None,
            user_id: None,
...
/// Request for a new system capability (skill or workflow).
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct CapabilityProposal {
    /// Type of capability (active tool or passive workflow).
    pub r#type: CapabilityType,
    /// Unique capability name (lowercase, snake_case).
    pub name: String,
...
mod tests {
    ...
    fn test_engine_agent_deserialization_defaults() {
        ...
        let agent: EngineAgent = serde_json::from_str(&agent_str).expect("Failed to deserialize agent with missing fields");
    ...
    fn test_engine_agent_deserialization_full() {
        ...
        let agent_str = agent_json.to_string();
        let agent: EngineAgent = serde_json::from_str(&agent_str).expect("Failed to deserialize full agent");

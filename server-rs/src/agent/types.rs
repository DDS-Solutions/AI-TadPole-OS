//! Agent Schemas — Primary data structures and state enums
//!
//! Defines the foundational data structures for the engine, optimized for 
//! high-performance serialization across REST and WebSocket boundaries.
//!
//! @docs ARCHITECTURE:Agent
//!
//! @state EngineAgent: (Active | Suspended | Degraded)
//!
//! ### AI Assist Note
//! **Serialization Parity**: Field names use `camelCase` via `#[serde(rename)]` 
//! to maintain parity with the TypeScript frontend. `ModelConfig` includes 
//! `supports_native_tools()` to prevent errors with smaller models.
...
impl ProviderConfig {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if !valid_protocols.contains(&proto.as_str()) {
            return Err(format!(
                "Unsupported protocol: '{}'. Must be one of: {:?}",
                self.protocol, valid_protocols
            ));
        }
...
impl ModelEntry {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if let Some(modality) = &self.modality {
            let m = modality.to_lowercase();
            let valid_modalities = ["llm", "vision", "voice", "reasoning"];
            if !valid_modalities.contains(&m.as_str()) {
                let m_str = modality.to_string();
                return Err(format!(
                    "Unsupported modality: '{}'. Must be one of: {:?}",
                    m_str, valid_modalities
                ));
            }
        }
...
pub struct EngineAgent {
    ...
    #[serde(default, rename = "connectorConfigs")]
...
/// Types of relationships between entities in the swarm graph.
...
/// Represents an edge in the swarm's knowledge graph.
#[derive(Debug, Clone, Serialize, Deserialize, sqlx::FromRow)]
pub struct MissionRelationship {
    pub id: String,
    pub from_id: String,
    pub to_id: String,

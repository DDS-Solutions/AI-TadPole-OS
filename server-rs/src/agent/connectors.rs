...
//! - **Subsystem**: Sovereign Engine / Agent Runner / connectors
//!
...
//! - **Local Errors**: none
//! - **Telemetry Targets**: `[IngestionWorker]`
...
pub trait ConnectorTrait: Send + Sync {
    ...
    ) -> Result<(Vec<IngestionItem>, i32, i64), AppError>;
}
    ...
    async fn fetch_new_items(
            ...
            .filter_map(|e| e.ok())
        {
            if entry.file_type().is_file() {
                ...
                // Only ingest relevant SME formats consistently
                if !["txt", "md", "pdf", "csv"].contains(&extension) {
                    continue;
                }

                // Reliability: Gracefully catch file deletion or permission errors on metadata extraction
                let metadata = match entry.metadata() {
                    Ok(m) => m,
                    Err(e) => {
                        tracing::warn!(
                            "⚠️ [FsConnector] Failed to read metadata for {}: {}",
                            path.display(),
                            e
                        );
                        continue;
                ...
                let modified: DateTime<Utc> = match metadata.modified() {
                    ...
                    Err(e) => {
                        tracing::warn!(
                            "⚠️ [FsConnector] Failed to get modified time for {}: {}",
                            path.display(),
                            e
                        );
                        continue;
        ...
        Ok((items, file_count, total_bytes))
    }
}
...
pub async fn start_ingestion_worker(state: std::sync::Arc<crate::state::AppState>) {
    ...
    loop {
        ...
        if stats.memory_pressure > 0.85 {
            tracing::warn!(
                "⚠️ [Resource Guardian] High memory pressure ({:.1}%) detected. Postponing data ingestion cycle.",
                stats.memory_pressure * 100.0
            );
        } else if let Err(e) = run_ingestion_cycle(&state).await {
...
async fn run_ingestion_cycle(state: &crate::state::AppState) -> Result<(), AppError> {
    ...
    for manifest in manifests {
        ...
        match connector.fetch_new_items(manifest.last_sync_at).await {
            Ok((items, file_count, total_bytes)) => {
                if items.is_empty() {
                    crate::agent::persistence::complete_sync(
                        pool,
                        &manifest.id,
                        manifest.last_sync_at.unwrap_or(Utc::now()),
                        file_count,
                        total_bytes,
                    )
                    .await?;
...
mod tests {
    ...
    async fn test_fs_connector_filtering() -> Result<(), AppError> {
        ...
        // Fetch items
        let (items, _, _) = connector.fetch_new_items(None).await?;
    ...
    async fn test_fs_connector_incremental_sync() -> Result<(), AppError> {
        ...
        let (items, _, _) = connector.fetch_new_items(Some(last_sync)).await?;
        assert_eq!(
            items.len(),
            0,
            "No items should be returned if nothing changed"
        );

...
impl FsConnector {
    ...
    /// Calculates total file count and byte size of the target directory.
    pub fn calculate_stats(&self) -> (i32, i64) {
        let mut count = 0;
        let mut size = 0;
            ...
            .filter_map(|e| e.ok())
        {
            if entry.file_type().is_file() {
                ...
                // Match the filter logic in fetch_new_items
                if matches!(ext.as_str(), "md" | "txt" | "pdf" | "csv" | "json") {
                    count += 1;
...
impl ConnectorTrait for FsConnector {
    ...
    async fn fetch_new_items(
        ...
        let (file_count, total_bytes) = self.calculate_stats();

...
mod tests {
    ...
    async fn test_vector_memory_reader_trait_bound() -> Result<()> {
        ...
        let empty_batch = RecordBatch::new_empty(schema.clone());
        let reader = Box::new(
            arrow_array::RecordBatchIterator::new(vec![Ok(empty_batch)].into_iter(), schema.clone())
        ) as Box<dyn arrow_array::RecordBatchReader + Send>;

        // This verifies the create_table call with our casted reader
        mem.conn.create_table("test_trait_table", reader).execute().await?;
    ...
    async fn test_vector_memory_add_batch_robustness() -> Result<()> {
        let dir = tempdir()?;
        let mem = Arc::new(VectorMemory::connect(dir.path().to_str().unwrap(), "test_robust").await?);
        ...
        assert!(results.len() >= 20, "Should have successfully added all concurrent records");

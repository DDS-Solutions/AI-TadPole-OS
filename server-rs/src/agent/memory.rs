...
//! and **Apache Arrow**. Orchestrates **Hybrid Search** (Vector + Keyword)
//! and **Advanced RAG Weighting** to provide precise mission context.
//! Implements **Semantic Archival**, where fragmented session memories are
//! summarized into dense historical records via Gemini. Features
//! **Automatic Orphan Cleanup** to prevent disk bloat from terminated
//! mission active-scopes.
//!
//! ### 🔍 Debugging & Observability
//! - **Failure Path**: LanceDB table lock contention, malformed Arrow
//!   batch schemas during ingestion, or 429 rate limits on embedding
//!   provider callbacks.
...
impl VectorMemory {
    ...
    /// Handles the asynchronous connection lifecycle and table registry discovery.
    #[cfg(feature = "vector-memory")]
    ...
    /// 
    /// ### 💎 Schema Enforcement
    /// We use a fixed 768-dimension vector (optimized for `text-embedding-004`).
    /// The the 'mission_id' field is critical for multi-tenant isolation 
    /// during concurrent swarm missions.
    #[cfg(feature = "vector-memory")]
    ...
    /// Ensures that the memory table exists within the LanceDB connection.
    /// 
    /// If the table is missing, it is initialized with an empty Apache Arrow 
    /// RecordBatch matching the canonical `get_schema()` footprint. This 
    /// prevents runtime panics during the first `add_memory` call.
    #[cfg(feature = "vector-memory")]
    ...
    /// ### 💎 Semantic Ingestion: add_memory
    /// Adds a single knowledge fragment to the persistent vector store.
    /// 
    /// ### 🧬 Workflow: Arrow Serialization
    /// 1. **Schema Check**: Validates the underlying table exists (lazy-init).
    /// 2. **Batch Construction**: Utilizes `arrow_array` builders to construct a 
    ///    high-performance RecordBatch containing the text, mission ID, 
    ///    timestamp, and the 768-D floating-point vector.
    /// 3. **Commit**: Appends the batch to the LanceDB table, triggering an 
    ...
    /// Uses exhaustive k-NN search via LanceDB's vector index. 
    /// This implementation ensures that high-fidelity semantic matches 
    /// are prioritized, while `limit * 2` over-fetching prevents 
    /// loss of context during dense historical retrieval.
    #[cfg(feature = "vector-memory")]
    ...
    /// ### 🔍 Semantic Retrieval: search_knowledge
    /// Performs a pure vector-based k-Nearest Neighbors (k-NN) search.
    /// 
    /// ### 🧬 Logic: Spatial Retrieval
    /// Finds the `limit` closest vectors in the 768-D latent space using L2 
    /// distance. This is the primary mechanism for RAG-01 (Retrieval 
    /// Augmented Generation) in mission reasoning loops.
    #[cfg(feature = "vector-memory")]
    ...
    /// High-Fidelity retrieval for Advanced RAG. 
    /// 
    /// ### 🧠 Advanced RAG
    /// Returns full metadata including `mission_id` and semantic `_distance`.
    /// This allows the routing layer to differentiate between general knowledge 
    /// and mission-specific ephemeral findings.
    #[cfg(feature = "vector-memory")]
    ...
    /// This is used during ingestion to prevent "Stale Echoes"—redundant 
    /// memories that would otherwise dilute the context window during RAG 
    /// retrieval.
    ...
    /// ### 🧬 Data Retrieval: get_all_memories
    /// Fetches the exhaustive list of memory IDs and raw text for a specific mission.
    /// 
    /// ### 🔍 Rationale: Context Sweeping
    /// Used by the Semantic Archiver to "sweep" all fragmented findings before 
    /// summarizing them into dense historical vaults. It filters the Apache 
    /// Arrow batches based on the `mission_id` partition key.
    #[cfg(feature = "vector-memory")]
    ...
    /// Session memories grow exponentially. This tool "compresses" the cognitive 
    /// load by summarizing fragmented findings into a structured historical record. 
    /// The original fragmented records are deleted, and only the "Dense Summary" 
    /// persists in the long-term vault (ARC-01).
    /// 
    /// ### 🔄 Transition Lifecycle
    /// 1. **Retrieval**: Pulls all mission-specific memories for the ID.
    /// 2. **Cognitive Synthesis**: Summarizes fragments via Gemini 1.5 logic 
    ///    using a specialized internal JSON schema for archival truth.
    /// 3. **Ingestion**: Computes a new 768-D vector for the summary and commits.
    /// 4. **Purge**: Atomically deletes the original fragmented Arrow records.
    #[cfg(feature = "vector-memory")]

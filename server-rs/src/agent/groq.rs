...
impl GroqProvider {
    ...
    /// Creates a GroqProvider instance.
    /// Accepts a shared `reqwest::Client` to reuse the underlying connection pool.
    /// The `ModelConfig` injected here determines the inference parameters (RPM/TPM)
    /// and the specific model ID (e.g., "llama3-70b-8192").
    ...
    /// ### 🛰️ Intelligence Orchestration: generate_internal
    /// Main generation loop for Groq high-speed inference.
    ///
    /// This function orchestrates three critical resiliency layers:
    /// 1. **Native Tool Mapping**: Translates the engine's flat tool registry 
    ///    into Groq's specialized OpenAI-compatible function schema.
    /// 2. **Intercept & In-Flight Recovery**: Monitors for `400 tool_use_failed` 
    ///    errors (hallucinations) and performs an on-the-fly regex repair pass 
    ///    before bubbling up any faults.
    /// 3. **Manual Tag Cleansing**: Actively scans Llama-based responses for 
    ///    leaked `<function>` tags to keep the UI command-clean.
    async fn generate_internal(
        ...
        if !res.status().is_success() {
            ...
            // ### 🛠️ Resiliency: In-Flight JSON Correction
            // Handle Groq Tool Call hallucination errors (400 Bad Request). 
            // Common with Llama-3 models on ultra-high speed inference where the 
            // structural JSON brackets might be fragmented or missing.
            // We intercept the `failed_generation` field and perform a 
            // local regex-based recovery pass before attempting a full retry.
        ...
        while let Some(caps) = FUNCTION_REGEX.captures(&output_text) {
            ...
            let mat = caps.get(0).expect("Groq regex match missing internal capture group 0.");
            let raw_match = mat.as_str();
    ...
    /// ### 🎙️ Multimodal: Audio Transcription (Whisper)
    /// Transcribes audio data using Groq's high-performance Whisper-large-v3 implementation.
    /// 
    /// ### ⚙️ Execution Flow
    /// 1. Constructs a `multipart/form-data` request containing the raw audio bytes.
    /// 2. Dispatches to Groq's specific Speech-to-Text (STT) endpoint.
    /// 3. Parses the `TranscriptionResponse` into a clean UTF-8 string.

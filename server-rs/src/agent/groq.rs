//! Groq-specific API bridge and transcription provider.
//!
//! Implements the `LlmProvider` trait for Groq's high-speed inference engine.
//! Includes specialized support for the Whisper-large-v3 transcription model 
//! and regex-based tool call recovery for Llama 3 models.
...
impl GroqProvider {
    ...
    async fn generate_internal(
        ...
        if !res.status().is_success() {
            ...
            return Err(anyhow::anyhow!("Groq API Error (RecoveredBody): {}", error_text));
        }

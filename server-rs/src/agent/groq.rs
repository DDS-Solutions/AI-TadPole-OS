...
//! - **Subsystem**: Sovereign Engine / Agent Runner / groq
//!
...
//! - **Local Errors**: none
//! - **Telemetry Targets**: `[Recovery]`, `[Groq]`
...
struct GroqChoice {
    ...
    #[serde(default)]
    delta: Option<GroqResponseMessage>,
}
...
struct GroqToolCall {
    #[serde(default)]
    function: Option<GroqFunctionCall>,
}
...
static FUNCTION_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?s)<function=([a-zA-Z0-9_-]+)[^\{]*(\{.*?\})[^>]*>?")
        .expect("Static Groq tool-call parser regex MUST be valid.")
});
    ...
    async fn generate_internal(
        ...
        let raw_text = res
            ...
            .map_err(|e| AppError::InfrastructureError {
                ...
                detail: format!("Failed to read Groq response body stream: {}", e),
                help_link: None,
            })?;

        let parsed: GroqResponse = match serde_json::from_str(&raw_text) {
            Ok(p) => p,
            Err(err) => {
                tracing::error!(
                    "🚨 [Groq] Failed to decode response JSON ({}). Raw response: {}",
                    err,
                ...
                return Err(AppError::InfrastructureError {
                    ...
                    kind: InfrastructureErrorKind::ApiError,
                    detail: format!(
                        "Error decoding Groq response body ({}): {}",
                        err,
        ...
        let choice = parsed
            ...
            .ok_or_else(|| AppError::InfrastructureError {
                provider_id: ProviderId::Groq,
                kind: InfrastructureErrorKind::ApiError,
                detail: "No completion choices returned from Groq provider".to_string(),
                help_link: None,
            })?;

        let message = choice.message.as_ref().or(choice.delta.as_ref());

        let mut output_text = message.and_then(|m| m.content.clone()).unwrap_or_default();
    ...
    pub async fn transcribe(
        ...
        let raw_text = res
            ...
            .map_err(|e| AppError::InfrastructureError {
                provider_id: ProviderId::Groq,
                kind: InfrastructureErrorKind::NetworkError,
                detail: format!("Failed to read transcription response: {}", e),
                help_link: None,
            })?;

        let parsed: TranscriptionResponse =
            serde_json::from_str(&raw_text).map_err(|e| AppError::InfrastructureError {
                provider_id: ProviderId::Groq,
                kind: InfrastructureErrorKind::ApiError,
                detail: format!(
                    "Error decoding transcription response ({}): {}",
                    e,
                    crate::error::safe_truncate_bytes(&raw_text, 500)
                ),
                help_link: None,
...
impl crate::agent::provider_trait::LlmProvider for GroqProvider {
    ...
    async fn embed(&self, _text: &str) -> Result<Vec<f32>, AppError> {
        Err(AppError::NotImplemented(
            "Groq does not natively support embeddings".to_string(),
        ))
    }
}

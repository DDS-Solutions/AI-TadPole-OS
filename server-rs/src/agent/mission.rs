...
mod tests {
    ...
    async fn test_merkle_audit_trail_linking() -> Result<(), AppError> {
        ...
        sqlx::query("CREATE TABLE agents (id TEXT PRIMARY KEY, name TEXT, status TEXT)").execute(&pool).await?;
        sqlx::query("CREATE TABLE mission_history (id TEXT PRIMARY KEY, agent_id TEXT, title TEXT, status TEXT, budget_usd REAL, cost_usd REAL, created_at DATETIME, updated_at DATETIME, is_degraded BOOLEAN, is_pinned BOOLEAN)").execute(&pool).await?;
        ...
        let agent_id = "agent-1";
        sqlx::query("INSERT INTO agents (id, name, status) VALUES ('agent-1', 'Agent One', 'idle')").execute(&pool).await?;
        let mission = create_mission(&pool, agent_id, "Test Mission", 10.0).await?;

        // 1. First Log (should have no prev_hash)
        let log1 = log_step(&pool, &mission.id, agent_id, "System", "First step", "info", None).await?;
        assert!(log1.prev_hash.is_none());
        assert!(log1.hash.is_some());

        // 2. Second Log (should link to first)
        let log2 = log_step(&pool, &mission.id, agent_id, "Agent", "Second step", "success", None).await?;

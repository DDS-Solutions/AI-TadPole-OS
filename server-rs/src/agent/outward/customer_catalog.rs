...
impl CustomerCatalog {
    pub fn new(business_name: impl Into<String>) -> Self {
        ...
        info!("[CustomerCatalog] Initializing Customer Knowledge Catalog for: {}", name);
        Self {
            business_name: name,
            items: Vec::new(),
    ...
    pub fn ingest_csv(&mut self, csv_data: &str) -> usize {
        ...
        for line in csv_data.lines().skip(1) {
            ...
            if parts.len() >= 3 {
                let item = CatalogItem {
                    ...
                    price_usd: parts.get(3).and_then(|p| p.parse::<f64>().ok()),
        ...
        info!("[CustomerCatalog] Successfully ingested {} CSV items", added);
        added
    }

    /// Ingest QuickBooks product/invoice JSON objects
    pub fn ingest_quickbooks_json(&mut self, json_str: &str) -> usize {
        info!("[CustomerCatalog] Ingesting QuickBooks JSON data");
        let mut added = 0;
        if let Ok(parsed_array) = serde_json::from_str::<Vec<serde_json::Value>>(json_str) {
            for val in parsed_array {
                let title = val.get("Name").or_else(|| val.get("title")).and_then(|v| v.as_str()).unwrap_or("QB Item").to_string();
                let category = val.get("Type").or_else(|| val.get("category")).and_then(|v| v.as_str()).unwrap_or("QuickBooks").to_string();
                let description = val.get("Description").or_else(|| val.get("description")).and_then(|v| v.as_str()).unwrap_or("QuickBooks Imported Item").to_string();
                let price = val.get("UnitPrice").or_else(|| val.get("price")).and_then(|v| v.as_f64());

                let mut metadata = std::collections::HashMap::new();
                metadata.insert("source".to_string(), "quickbooks".to_string());

                let item = CatalogItem {
                    id: format!("qb-item-{}", self.items.len() + 1),
                    title,
                    category,
                    description,
                    price_usd: price,
        ...
        info!("[CustomerCatalog] Successfully ingested {} QuickBooks items", added);
        added
    }
    ...
    pub fn to_llm_context(&self) -> String {
        ...
        context.push_str(&format!("Model Profile: {}\n\n", self.default_model_profile));
        ...
        for item in display_items {
            context.push_str(&format!(
                ...
                item.id,
                item.description,
                item.price_usd.map_or("N/A".to_string(), |p| format!("${:.2}", p))
            ));
        }
    ...
    pub fn to_category_context(&self, category: &str) -> String {
        let mut context = format!("# Customer Catalog: {} (Category: {})\n", self.business_name, category);
        context.push_str(&format!("Model Profile: {}\n\n", self.default_model_profile));
    ...
    pub fn search_catalog(&self, query: &str, top_k: usize) -> Vec<CatalogItem> {
        ...
        scored_items.into_iter().take(top_k).map(|(_, item)| item.clone()).collect()
    }
...
mod tests {
    ...
    fn test_customer_catalog_csv_ingestion() {
        ...
        let count = catalog.ingest_csv(csv_data);
        assert_eq!(count, 2);
    ...
    fn test_customer_catalog_quickbooks_json_ingestion() {
        ...
        let count = catalog.ingest_quickbooks_json(qb_json);
        assert_eq!(count, 2);
        assert_eq!(catalog.items[0].title, "Consulting Hour");
        assert_eq!(catalog.items[0].metadata.get("source").map(|s| s.as_str()), Some("quickbooks"));
    }

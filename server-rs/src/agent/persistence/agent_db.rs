...
fn parse_json_field<T: serde::de::DeserializeOwned + Default>(col_name: &str, agent_id: &str, raw_str: &str) -> T {
    let trimmed = raw_str.trim();
    if trimmed.is_empty() || trimmed == "null" {
        return T::default();
    }
    serde_json::from_str(trimmed).unwrap_or_else(|e| {
        tracing::warn!("⚠️ [Persistence] Failed to parse {} for agent {}: {}. Falling back to default.", col_name, agent_id, e);
        T::default()
    })
}

/// Shared column list for agent SELECT queries. Update this constant when the agents table schema changes.
const AGENT_SELECT_COLS: &str = "id, name, role, department, description, model_id, tokens_used, status, current_task,
            ...
            heartbeat_at, requires_oversight, working_memory, version";
    ...
    pub fn to_engine_agent(&self) -> EngineAgent {
        ...
        EngineAgent {
            identity: AgentIdentity {
                ...
                category: self.category.clone().unwrap_or_else(|| DEFAULT_CATEGORY.to_string()),
...
pub async fn load_agents_db(pool: &SqlitePool) -> Result<Vec<EngineAgent>, AppError> {
    let rows: Vec<FlatAgentRow> = sqlx::query_as(
        sqlx::AssertSqlSafe(format!("SELECT {} FROM agents", AGENT_SELECT_COLS)),
    )
    .fetch_all(pool)
    .await?;
...
pub async fn load_agent_by_id_db(pool: &SqlitePool, agent_id: &str) -> Result<Option<EngineAgent>, AppError> {
    let row: Option<FlatAgentRow> = sqlx::query_as(
        sqlx::AssertSqlSafe(format!("SELECT {} FROM agents WHERE id = ?", AGENT_SELECT_COLS)),
    )
    .bind(agent_id)
    .fetch_optional(pool)
    .await?;
...
pub async fn claim_agent(pool: &SqlitePool, agent_id: &str) -> Result<bool, AppError> {
    ...
    let existing_status: Option<String> = sqlx::query_scalar("SELECT status FROM agents WHERE id = ?1")
        .bind(agent_id)
        .fetch_optional(pool)
        .await?;
        ...
        None => {
            ...
            match res_insert {
                ...
                Err(_) => {
                    ...
                    let res_retry = sqlx::query("UPDATE agents SET status = 'busy' WHERE id = ?1 AND status != 'busy'")
                        .bind(agent_id)
                        .execute(pool)
                        .await?;

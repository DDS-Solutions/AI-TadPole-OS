...
impl A2AMailbox {
    ...
    pub async fn send_envelope(&self, envelope: &MailboxEnvelope) -> Result<(), AppError> {
        if envelope.target_agent_id.starts_with("http://") || envelope.target_agent_id.starts_with("https://") {
            ...
            let signature = crate::agent::runner::tools::capability::sign_a2a_envelope(&payload_str);

            // Post to the remote inbox gateway
            let url = if envelope.target_agent_id.ends_with("/send") {
                envelope.target_agent_id.clone()
            } else {
                format!("{}/v1/engine/a2a/mailbox/send", envelope.target_agent_id.trim_end_matches('/'))
            };

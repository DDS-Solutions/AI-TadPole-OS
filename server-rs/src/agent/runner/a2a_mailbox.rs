...
//! - **Witness Tests**: `test_mailbox_flow`, `test_mailbox_pagination_and_filtering`, `test_mailbox_ssrf_metadata_rejected`, `test_a2a_mailbox_replay_on_pending_directive_is_idempotent`, `test_a2a_mailbox_merkle_audit_chaining`
...
impl A2AMailbox {
    ...
    pub async fn send_envelope(
        ...
        let is_remote = envelope.target_agent_id.starts_with("http://")
        ...
        if let Some(audit) = audit_trail {
            ...
            if let Err(e) = audit
                ...
                .await
            {
                tracing::error!("🚨 Failed to record [A2A_DISPATCH] in audit trail: {:?}", e);
            }
        }

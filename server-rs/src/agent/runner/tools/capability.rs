...
static KEYRING: Lazy<Mutex<Keyring>> = Lazy::new(|| {
    ...
    Mutex::new(Keyring { keys, active_key_id })
});

static REVOCATION_LIST: Lazy<Mutex<HashSet<String>>> =
...
fn compute_signature(
    ...
    let msg = canonical_message(id, agent_id, mission_id, permissions, expires_at.timestamp());
    ...
    pub fn verify(&self, required: &Permission) -> bool {
        ...
        if now > self.expires_at + chrono::Duration::seconds(5) {
            tracing::warn!(
                ...
                self.id, self.agent_id, self.mission_id, self.expires_at, now
            );
            return false;
        }
        ...
        let verified = self.permissions.iter().any(|p| match (p, required) {
            ...
            (Permission::ShellExecute(p1), Permission::ShellExecute(p2)) => {
                ...
                let resolved_exe = resolve_executable_path(exe_name)
        ...
        if !verified {
            tracing::warn!(
                ...
                self.id, self.agent_id, self.mission_id, required
            );
        }
        verified
    }
    ...
    pub fn mint_token(
        ...
        match authority {
            ...
            | crate::agent::types::RoleAuthorityLevel::Management => {
                ...
                let cargo_exe = resolve_executable_path("cargo").unwrap_or_else(|| "cargo".to_string());
        ...
        let key = keyring.keys.get(&key_id).expect("Active key must exist in keyring");
        let signature = compute_signature(&id, agent_id, mission_id, &permissions, &expires_at, key);
...
mod tests {
    ...
    fn test_shell_execute_matching() {
        ...
        assert!(token.verify(&Permission::ShellExecute("cargo build --release".to_string())));
    ...
    fn test_network_fetch_matching() {
        let mut guard = TestCleanupGuard::new();
        let permissions = vec![Permission::NetworkFetch("https://api.github.com/*".to_string())];
        ...
        assert!(token.verify(&Permission::NetworkFetch("https://api.github.com/repos".to_string())));

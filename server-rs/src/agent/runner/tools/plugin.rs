...
//! - **Telemetry Link**: Search `[plugin]` in tracing logs.
//!
//! @docs ARCHITECTURE:Registry
//!
//! ### AI Assist Note
//! **Dynamic Plugin Subsystem**: Handles loading, parsing, and execution of
//! third-party script-based tools dynamically. Implements the standard `Tool`
//! trait. Runs script processes (Python/Node) via a secure sub-process boundary.
    ...
    async fn execute(
        ...
        let (cmd, args_list) = match ext.as_str() {
            ...
            "wasm" => {
                ...
                ("wasmer", vec!["run".to_string(), entry_path.to_string_lossy().to_string()])
            }
            _ => {
                ...
                if manifest_path.exists() {
                    if let Ok(content) = tokio::fs::read_to_string(&manifest_path).await {
                        if let Ok(manifest) = serde_json::from_str::<PluginManifest>(&content) {
                            tracing::info!("🔌 [Plugins] Discovered dynamic plugin: {}", manifest.name);

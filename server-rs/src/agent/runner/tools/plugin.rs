...
//! - `[Behavioral]` Dynamic plugin manifests deserialize correctly and expose accurate tool metadata and security flags (enforced_by: `test_plugin_manifest_serde`, `test_plugin_tool_metadata`).
//!
//! ### 🔍 Debugging & Observability
//! - **Local Errors**: none
//! - **Telemetry Targets**: none declared
//! - **Witness Tests**: `test_plugin_manifest_serde`, `test_plugin_tool_metadata`
...
mod tests {
    ...
    fn test_plugin_manifest_serde() {
        ...
        let manifest: PluginManifest =
            serde_json::from_value(manifest_json).expect("Deserialization succeeds");
        assert_eq!(manifest.name, "custom_tool");
        assert!(manifest.is_mutating);
        assert!(manifest.is_dangerous);
        assert!(!manifest.is_cacheable);
    }
    ...
    fn test_plugin_tool_metadata() {
        ...
        let tool = PluginTool {
            manifest,
            plugin_dir: PathBuf::from("/plugins/test_plugin"),
        };
        ...
        // Plugins are always treated as dangerous because they spawn external subprocesses

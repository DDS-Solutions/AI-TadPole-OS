...
//! - `[Behavioral]` Tool execution enforces dangerous composition and JSON schema validation (enforced_by: `test_dangerous_composition_invariant`, `test_validate_json_schema_basic`).
//!
//! ### 🔍 Debugging & Observability
//! - **Local Errors**: none
//! - **Telemetry Targets**: `[tools]`
//! - **Witness Tests**: `test_dangerous_composition_invariant`, `test_validate_json_schema_basic`
...
impl super::service_traits::ToolExecutor for AgentRunner {
    async fn execute_tool(
        ...
        // 1. Mint Capability Token for this specific call with explicit agent skills
            ...
            .await
        {
            ...
            Err(e) => {
                ...
                if let ToolExecutionError::AppError(ref ae) = e {
                    if matches!(
                        ae,
                        AppError::Unauthorized(_) | AppError::Forbidden(_) | AppError::RateLimit(_)
                    ) {
                        // Propagate security/billing blocks directly to halt execution
                        return Err(e.into());
                    }
                }
    ...
    fn get_tool_timeout_secs(&self) -> u64 {
        let timeout = self.get_provider_timeout_secs();
        if timeout > 0 {
            timeout.clamp(5, 600)
        } else {
            60
    ...
    fn verify_execution_witnesses(
        ...
        self.verify_execution_witnesses(records, last_mutation_ms)
    }
...
impl AgentRunner {
    ...
    async fn run_zero_trust_pipeline(
        ...
        tracing::info!(
            "📦 [tools] Executing tool: {} (call_id: {})",
            fc.name,
            call_id
        );

        // The model-facing toolbelt is not an authorization boundary by itself:
        // providers can return undeclared function names. Enforce the agent's MCP
        // capability declaration again immediately before wrapper construction.
        if crate::agent::mcp::decode_mcp_tool_name(&fc.name).is_some()
            && !crate::agent::mcp::is_mcp_tool_authorized(&ctx.mcp_tools, &fc.name)
        {
            return Err(ToolExecutionError::SecurityBlocked(format!(
                "Agent '{}' is not authorized to execute MCP tool '{}'",
                ctx.agent_id, fc.name
            )));
        }
        ...
        // We MUST record the intent before execution with correlation call_id.
        let redacted_args = self.state.security.secret_redactor.redact(&args_str);
        let intent_params = serde_json::json!({
            "call_id": &call_id,
            "args": serde_json::from_str::<serde_json::Value>(&redacted_args)
                .unwrap_or_else(|_| serde_json::json!(redacted_args)),
        })
        .to_string();
        ...
        self.execute_with_retry(
            ...
            ctx.user_id.as_deref(),
            &ctx.modified_files,
            &ctx.last_mutation_timestamp_ms,
        )
        .await
    }
    ...
    async fn execute_with_retry(
        ...
        last_mutation_timestamp_ms: &std::sync::atomic::AtomicU64,
    ) -> Result<String, ToolExecutionError> {
        ...
        loop {
            ...
            match result {
                Ok(res) => {
                    ...
                    if is_mutating {
                        if let Some(ref path) = target_path {
                            ...
                            if !mod_files.contains(&path_str) {
                                mod_files.push(path_str.clone());
                            }

                            // ── TrustGraph Mutation Hook ───────────────────
                            let tg_pool = self.state.resources.pool.clone();
                            let tg_agent = agent_id.to_string();
                            let tg_path = path_str;
                            let tg_mission = mission_id_opt.clone();
                            tokio::spawn(async move {
                                let engine =
                                    crate::agent::trustgraph::TrustGraphEngine::new(tg_pool);
                                let agent_node = crate::agent::trustgraph::TrustGraphNode {
                                    id: format!("agent:{}", tg_agent),
                                    name: tg_agent.clone(),
                                    r#type: "AGENT".to_string(),
                                    description: Some(format!("Agent {}", tg_agent)),
                                    mission_id: tg_mission.clone(),
                                };
                                if let Err(e) = engine.add_node(&agent_node).await {
                                    tracing::warn!(
                                        "Failed to record agent node in TrustGraph: {}",
                                        e
                                    );
                                    return;
                                }
                                let file_node = crate::agent::trustgraph::TrustGraphNode {
                                    id: format!("file:{}", tg_path),
                                    name: tg_path.clone(),
                                    r#type: "FILE".to_string(),
                                    description: Some(format!("File at {}", tg_path)),
                                    mission_id: tg_mission.clone(),
                                };
                                if let Err(e) = engine.add_node(&file_node).await {
                                    tracing::warn!(
                                        "Failed to record file node in TrustGraph: {}",
                                        e
                                    );
                                    return;
                                }
                                let rel = crate::agent::trustgraph::TrustGraphRelation {
                                    ...
                                    source_entity_id: agent_node.id,
                                    target_entity_id: file_node.id,
                                    relation_type: "MUTATED".to_string(),
                                    weight: 1.0,
                                    mission_id: tg_mission,
                                };
                                if let Err(e) = engine.add_relation(&rel).await {
                                    tracing::warn!(
                                        "Failed to record MUTATED relation in TrustGraph: {}",
                                        e
                                    );
                                }
                            });
                    ...
                    if is_cacheable
                        && !res.starts_with("(READ FAILED")
                        && !res.starts_with("(FAILED")
                    {
                        let mut cache = self.state.resources.tool_cache.lock();
                        cache.insert(
                            &fc.name,
                    ...
                    // Record successful completion in audit trail with output digest
                    use sha2::Digest;
                ...
                Err(e) => {
                    ...
                    // Record failure in audit trail (redacted with correlation call_id)
                    let redacted_error = self
                        .state
                        .security
                        .secret_redactor
                        .redact(&format!("Error: {}", e));
                    let failure_params = serde_json::json!({
                        "call_id": call_id,
                        "status": "failure",
    ...
    async fn validate_security_and_oversight(
        ...
        if validation.oversight_required {
            ...
            // Auto-approved by policy: record entry in oversight_log for persistent audit history & UI Action Ledger
            let entry_id = uuid::Uuid::new_v4().to_string();
            let params_json = serde_json::to_string(&fc.args).unwrap_or_default();
            let payload_desc = if user_message.is_empty() {
                format!("Auto-approved by policy for tool: {}", fc.name)
            } else {
                format!("User Context: '{}' | Auto-approved by policy", user_message)
            };
            let _ = sqlx::query(
                "INSERT INTO oversight_log (id, mission_id, agent_id, entry_type, skill, params, status, decision, decided_at, decided_by, payload) VALUES (?, ?, ?, 'tool_call', ?, ?, 'auto_approved', 'auto_approved', datetime('now'), 'auto_policy', ?)"
            )
            .bind(&entry_id)
            .bind(&mission_id_opt)
            .bind(&ctx.agent_id)
...
mod tests {
    ...
    async fn test_execute_tool_cbs_block() {
        ...
        ctx.skills = vec!["allowed_skill".to_string(), "forbidden_skill".to_string()];
    ...
    async fn test_execute_tool_mirror_mode() {
        let mut mock_state = AppState::new_minimal_mock().await;
        mock_state.mirror_mode = true;
    ...
    async fn test_execute_tool_policy_deny() {
        ...
        let runner = AgentRunner::new(state.clone());
        let mut ctx = RunContext::default();
        ctx.skills = vec!["risky_tool".to_string()];
    ...
    async fn test_unknown_mutating_tool_default_denied() {
        ...
        ctx.mcp_tools = vec!["git:create_repository".to_string()];

        let mut agent = EngineAgent::default();
        agent.identity.id = ctx.agent_id.clone();
        agent.capabilities.skills = vec!["mcp_git_create_repository".to_string()];
        agent.capabilities.mcp_tools = ctx.mcp_tools.clone();
        state.registry.agents.insert(ctx.agent_id.clone(), agent);
    ...
    async fn test_unknown_non_mutating_tool_allowed_past_gates() {
        ...
        let mut ctx = RunContext::default();
        ctx.agent_id = "test-agent".to_string();
        ctx.skills = vec!["mcp_docs_query_docs".to_string()];
        ctx.mcp_tools = vec!["docs:query_docs".to_string()];
    ...
    #[tokio::test]
    async fn test_undeclared_mcp_tool_blocked_before_dispatch() {
        let state = Arc::new(AppState::new_minimal_mock().await);
        let runner = AgentRunner::new(state);
        let mut ctx = RunContext::default();
        ctx.agent_id = "least-privilege-agent".to_string();

        let fc = ToolCall {
            name: "mcp_docs_query_docs".to_string(),
            args: serde_json::json!({}),
        };
        let token = ZeroTrustGuard::mint_token(
            &ctx.agent_id,
        ...
        assert!(matches!(
            result,
            Err(ToolExecutionError::SecurityBlocked(message))
                if message.contains("not authorized to execute MCP tool")
        ));
    }
    ...
    fn test_is_dangerous_native_operation() {
        ...
        assert!(is_dangerous_native_operation("resolve_x402_challenge"));
        ...
        assert!(!is_dangerous_native_operation("propose_asset_transaction"));

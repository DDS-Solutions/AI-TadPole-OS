...
impl AgentRunner {
    ...
    async fn run_zero_trust_pipeline(
        ...
        // Retrieve tool from registry (optional early lookup, fail later at execution time)
        let tool = self.state.registry.tool_registry.get(&fc.name);
        ...
        let required_permission = match fc.name.as_str() {
            ...
            _ => Permission::FileRead(".".to_string()),
        };

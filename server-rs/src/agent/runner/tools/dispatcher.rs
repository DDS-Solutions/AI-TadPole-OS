...
impl CategoricalHandler for MissionHandler {
    async fn handle(
        ...
        match name {
            ...
            "search_mission_knowledge" => {
                runner.handle_search_mission_knowledge(&run_ctx, &fc).await
            }
            ...
            "update_working_memory" => {
                ...
                runner
                    .handle_update_working_memory(&run_ctx, &fc, &mut output)
                    .await?;
                Ok(output)
            }
            "query_financial_logs" => {
                runner
                    .handle_query_financial_logs(&run_ctx, &fc, usage)
                    .await
            }
            _ => Err(ToolExecutionError::ExecutionFailed(format!(
                "MissionHandler cannot handle '{}'",
                name
            ))),
        }
    }
    ...
    async fn handle(
        ...
        match name {
            "read_file" | "get_file_contents" => {
                runner.handle_read_file(&run_ctx, &fc, usage).await
            }
...
impl CategoricalHandler for SwarmHandler {
    async fn handle(
        ...
        match name {
            ...
            "issue_alpha_directive" => runner
                .handle_alpha_directive(&run_ctx, &fc)
                .await
    ...
    async fn handle(
        ...
        match name {
            ...
            "script_builder" => {
                let mut output = String::new();
                runner
                    .handle_script_builder(&run_ctx, &fc, &mut output, usage, "")
                    .await?;
                Ok(output)
            }
            ...
            "execute_shell" => {
                let mut output = String::new();
                runner
                    .handle_execute_shell(&run_ctx, &fc, &mut output)
                    .await?;
                Ok(output)
            }
            _ => Err(ToolExecutionError::ExecutionFailed(format!(
                "AuxHandler cannot handle '{}'",
                name
            ))),
        }
    }
    ...
    async fn handle(
        ...
        match name {
            ...
            "refactor_synthesized_skill" => {
                runner
                    .handle_refactor_synthesized_skill(&run_ctx, &fc)
                    .await
            }
            _ => Err(ToolExecutionError::ExecutionFailed(format!(
                "EvolutionHandler cannot handle '{}'",
                name
            ))),
        }
    }

...
//! - `[Behavioral]` Every registered tool in the core tool manifest maps to a deterministic categorical handler (enforced_by: `test_manifest_matches_dispatcher`).
//!
//! ### 🔍 Debugging & Observability
//! - **Local Errors**: none
//! - **Telemetry Targets**: none declared
//! - **Witness Tests**: `test_manifest_matches_dispatcher`
...
impl Dispatcher {
    pub fn new() -> Self {
        ...
        tracing::info!(
            "🔧 [dispatcher] Initializing Tool Dispatcher with {} tools",
            manifest_len
        );
        ...
        for meta in manifest {
            let handler: Arc<dyn CategoricalHandler> =
                if mission_tools.contains(&meta.name.as_str()) {
                    ...
                    tracing::warn!(
                        "⚠️ [dispatcher] Manifest tool '{}' not matched to any categorical handler",
                        meta.name
                    );
                    continue;
                };
        ...
        tracing::info!(
            "🔧 [dispatcher] Successfully registered {}/{} manifest tools",
            registered_count,
            manifest_len
        );
...
crate::define_categorical_handler!(MissionHandler, {
    ...
    "pin_mission" => handle_pin_mission (),
    ...
    "query_symbol_graph" => handle_query_symbol_graph (),
    ...
    "request_model_switch" => handle_request_model_switch (output),
    "send_agent_envelope" => handle_send_agent_envelope (),
    "propose_asset_transaction" => handle_propose_asset_transaction (),
    "confirm_asset_transaction" => handle_confirm_asset_transaction (),
    "cancel_asset_transaction" => handle_cancel_asset_transaction (),
    "resolve_x402_challenge" => handle_resolve_x402_challenge (),
    "add_mission_task" => handle_add_mission_task (),
    "update_mission_task" => handle_update_mission_task (),
...
crate::define_categorical_handler!(FsHandler, {
    ...
    "restore_file_version" => handle_restore_file_version (),
    "get_file_history" => handle_get_file_history (),
});

crate::define_categorical_handler!(SwarmHandler, {
    "spawn_subagent" => handle_spawn_subagent (usage),
    "issue_alpha_directive" => handle_alpha_directive (usage),

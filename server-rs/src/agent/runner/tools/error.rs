...
//! - `[Behavioral]` User-safe messages sanitize prompt injection attempts, strip control characters, and truncate safely on unicode boundaries (enforced_by: `test_sanitize_for_llm_strips_injection`, `test_sanitize_for_llm_truncates_unicode_safely`).
//!
//! ### 🔍 Debugging & Observability
//! - **Local Errors**: none
//! - **Telemetry Targets**: none declared
//! - **Witness Tests**: `test_sanitize_for_llm_strips_injection`, `test_sanitize_for_llm_truncates`, `test_sanitize_for_llm_truncates_unicode_safely`, `test_user_safe_message_sanitized`
...
impl ToolExecutionError {
    ...
    pub fn recovery_strategy(&self) -> RecoveryAction {
        match self {
            ...
            Self::AppError(e) => match e {
                ...
                // SQLite lock contention (BUSY/LOCKED) is transient — retry with short backoff.
                // This fires during high-concurrency swarm spawning when multiple agents compete
                // for the same SQLite write lock in the batched pre-flight transaction.
                AppError::Sqlx(sqlx_err) if is_sqlite_transient(sqlx_err) => {
                    RecoveryAction::RetryWithBackoff { seconds: 1 }
                }
    ...
    pub fn user_safe_message(&self) -> String {
        match self {
            ...
            Self::HierarchyBlocked(msg) => {
                format!("Hierarchy Violation: {}", sanitize_for_llm(msg))
            }
            Self::Validation(msg) => {
                format!("Parameter Validation Error: {}", sanitize_for_llm(msg))
            }
            Self::ExecutionFailed(_) => "Runtime execution failed. Refer to logs.".to_string(),
            Self::ToolNotFound { name } => {
                format!("Tool '{}' not found in registry", sanitize_for_llm(name))
            }
            Self::InvalidParameters { name, reason } => {
                format!(
                    "Invalid parameters for tool '{}': {}",
                    sanitize_for_llm(name),
                    sanitize_for_llm(reason)
                )
            }
            ...
            Self::AppError(e) => match e {
                AppError::Forbidden(msg) => {
                    format!("Security Violation: {}", sanitize_for_llm(msg))
                }
                AppError::NotFound(msg) => format!("Not Found: {}", sanitize_for_llm(msg)),
                AppError::RateLimit(msg) => {
                    format!("Rate limit exceeded: {}", sanitize_for_llm(msg))
                }
...
fn sanitize_for_llm(s: &str) -> String {
    ...
    let boundary = s.floor_char_boundary(MAX_LEN.min(s.len()));
...
/// Returns `true` for SQLite errors that are inherently transient (lock contention, busy).
/// These occur during high-concurrency swarm spawning and should be retried automatically.
fn is_sqlite_transient(err: &sqlx::Error) -> bool {
...
impl From<sqlx::Error> for ToolExecutionError {
    fn from(err: sqlx::Error) -> Self {
        // Promote SQLite busy/locked errors to Transient so the retry loop
        // in tools/mod.rs catches them instead of aborting immediately.
        if is_sqlite_transient(&err) {
            ToolExecutionError::Transient {
                message: format!("SQLite transient lock error: {}", err),
                retry_after: Some(1),
            }
        } else {
...
impl From<ToolExecutionError> for AppError {
    fn from(err: ToolExecutionError) -> Self {
        match err {
            ToolExecutionError::AppError(ae) => ae,
            ToolExecutionError::SecurityBlocked(msg)
            | ToolExecutionError::HierarchyBlocked(msg) => AppError::Forbidden(msg),
            ToolExecutionError::Validation(msg)
            | ToolExecutionError::InvalidParameters { reason: msg, .. } => {
                AppError::BadRequest(msg)
            }
            ToolExecutionError::ToolNotFound { name } => {
                AppError::NotFound(format!("Tool '{}' not found in registry", name))
            }
            ToolExecutionError::FilesystemError { details, .. } => {
                AppError::Io(std::io::Error::other(details))
            }
            ToolExecutionError::Transient { message, .. }
            | ToolExecutionError::ExecutionFailed(message)
            | ToolExecutionError::CommandFailed { reason: message } => {
...
mod tests {
    ...
    #[test]
    fn test_sanitize_for_llm_truncates_unicode_safely() {
        let input = format!("{}{}", "a".repeat(199), "👋");
    ...
    fn test_user_safe_message_sanitized() {
        let err =
            ToolExecutionError::SecurityBlocked(r#"test"; DROP TABLE agents; --"#.to_string());

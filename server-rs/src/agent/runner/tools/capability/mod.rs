...
//! - **Failure Path**: Security gate rejection, expired token, or untrusted signature.
...
pub use crypto::{compute_hmac, set_active_key_id, set_key, sign_a2a_envelope, verify_a2a_envelope};
#[allow(unused_imports)]
...
#[cfg(test)]
mod tests {
    use super::crypto::{compute_hmac, compute_signature, reset_keyring_for_test, set_key, KEYRING};
    use super::guard::{clear_revocations, is_revoked, revoke_token, ZeroTrustGuard};
    ...
    fn test_token_verify_allowed_files() {
        ...
        assert!(token.verify(&Permission::ToolExec("read_file".to_string())));
    ...
    fn test_token_serialization() {
        ...
        assert!(deserialized.verify(&Permission::ToolExec("read_file".to_string())));
    }
    ...
    fn test_wildcard_toolexec_rejected() {
        ...
        assert!(token.verify(&Permission::ToolExec("read_file".to_string())));
        assert!(token.verify(&Permission::ToolExec("grep_search".to_string())));

...
//! - **Subsystem**: Sovereign Engine / Agent Runner / guard
//!
//! ### ⚠️ Invariants & Non-Negotiables
//! - `[Behavioral]` Zero-trust token verification bounds tool and path permissions and rejects revoked tokens (enforced_by: `test_token_verify_allowed_files`, `test_token_revocation`).
//!
//! ### 🔍 Debugging & Observability
//! - **Local Errors**: none
//! - **Telemetry Targets**: `[guard]`, `[capability]`
//! - **Witness Tests**: `test_token_verify_allowed_files`, `test_token_revocation`
...
pub const CLOCK_SKEW_GRACE_SECS: i64 = 5;
pub const EXEC_TOKEN_TTL_HOURS: i64 = 2;
pub const MGMT_TOKEN_TTL_HOURS: i64 = 1;
pub const DEFAULT_TOKEN_TTL_MINS: i64 = 30;
pub const DEFAULT_REVOCATION_RETENTION_HOURS: i64 = 24;
...
pub fn revoke_token_with_expiry(id: &str, expires_at: chrono::DateTime<chrono::Utc>) {
    tracing::warn!("[guard] REVOKED: token_id={} expires_at={}", id, expires_at);
...
pub fn revoke_token(id: &str) {
    let default_exp =
        chrono::Utc::now() + chrono::Duration::hours(DEFAULT_REVOCATION_RETENTION_HOURS);
...
pub fn is_revoked(id: &str) -> bool {
    tracing::trace!("[guard] Checking revocation status for token_id={}", id);
...
#[cfg(test)]
#[allow(dead_code)]
pub fn unrevoke_token(id: &str) {
    REVOCATION_LIST.lock().remove(id);
}
...
impl CapabilityToken {
    ...
    pub fn verify(&self, required: &Permission) -> bool {
        ...
        // 3. Check token expiration (allow grace window for clock skew)
        let now = chrono::Utc::now();
        if now > self.expires_at + chrono::Duration::seconds(CLOCK_SKEW_GRACE_SECS) {
            tracing::warn!(
        ...
        let verified = self.permissions.iter().any(|p| match (p, required) {
            ...
            (Permission::ShellExecute(p1), Permission::ShellExecute(p2)) => {
                let tokens = crate::utils::security::parse_command_tokens(p2);
                let exe_name = tokens.first().map(|s| s.as_str()).unwrap_or("");
                if let Some(resolved_exe) = resolve_executable_path(exe_name) {
                    ...
                    p1 == exe_name || p1 == p2 || matches_glob(p1, p2)
                }
            }
...
impl ZeroTrustGuard {
    ...
    pub fn mint_token(
        ...
        // Explicit tool execution grants for safe non-filesystem, non-network memory & coordination tools
        let safe_tools = [
        ...
        if let Some(skills) = allowed_skills {
            for skill in skills {
                if skill == "*" {
                    tracing::warn!("[capability] Wildcard skill '*' ignored during token minting; explicit skills required");
                    continue;
                }
        ...
        if matches!(
            ...
            if let Some(files) = allowed_files {
                // Least privilege: an explicit empty list grants NO write access.
                ...
                // Unconstrained task: default to workspace root
        ...
        let duration = match authority {
            crate::agent::types::RoleAuthorityLevel::Executive => {
                chrono::Duration::hours(EXEC_TOKEN_TTL_HOURS)
            }
            crate::agent::types::RoleAuthorityLevel::Management => {
                chrono::Duration::hours(MGMT_TOKEN_TTL_HOURS)
            }
            _ => chrono::Duration::minutes(DEFAULT_TOKEN_TTL_MINS),
        };

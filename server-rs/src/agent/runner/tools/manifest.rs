...
//! - `[Behavioral]` Tool names in manifest are strictly unique and schemas conform to JSON schema specification (enforced_by: `test_manifest_uniqueness`, `test_manifest_schema_validity`).
//! - `[Behavioral]` Manifest definitions match dispatcher registrations with pinned security classifications (enforced_by: `test_manifest_matches_dispatcher`, `test_tool_security_classifications_pinned`).
//!
//! ### 🔍 Debugging & Observability
//! - **Local Errors**: none
//! - **Telemetry Targets**: `[manifest]`
//! - **Witness Tests**: `test_manifest_uniqueness`, `test_manifest_schema_validity`, `test_manifest_matches_dispatcher`, `test_tool_security_classifications_pinned`
...
fn build_core_tool_manifest() -> Vec<ToolDefinitionData> {
    vec![
        ...
        def(
            "spawn_subagent",
            "Spawns one or more specialized sub-agents to handle tasks in parallel. DELEGATION CONTRACT: 1. Sub-agents have NO access to your prior conversation history — you must supply all necessary context in the message. 2. Before delegating, tell the sub-agent what you have already done so it does not repeat completed work. 3. The sub-agent's response must be concise and actionable, citing file paths and symbol names. 4. Do not delegate your primary mission — only delegate independent sub-tasks or parallel research.",
            serde_json::json!({
                "type": "object",
                "properties": {
                    ...
                    "model_tier": { "type": "string", "enum": ["fast", "reasoning", "creative"], "description": "Optional model tier for delegation: 'fast' (cheaper, rapid), 'reasoning' (deep analysis), 'creative' (standard execution)." }
                },
                "required": ["message"]
            }),
        ),
        def(
            "ask_user_question",
            "Ask the user a question when clarification or a decision is needed before proceeding. Only ask when the answer meaningfully changes what you do next.\n\nRules:\n- Provide 0-5 mutually exclusive choices. Each must be a genuinely distinct outcome.\n- Every option must commit the user to a specific outcome. Do not add options like 'Other' or 'Something else'.\n- If you have enough context to recommend one option, put it first with the exact suffix ' (Recommended)'.",
            serde_json::json!({
                "type": "object",
                "properties": {
                    ...
                    "options": {
                        "type": "array",
                        "items": { "type": "string" },
                        "description": "0-5 mutually exclusive choices, each mapping to a distinct outcome."
                    },
                    "context": { "type": "string", "description": "Brief context explaining why this decision is needed." }
                },
                "required": ["question"]
            }),
        ),
        ...
        def(
            "query_symbol_graph",
            "Queries the codebase AST symbol graph. Action 'lookup' inspects symbol definitions, locations, callers, and callees. Action 'blast_radius' computes downstream dependent symbols that would be impacted by modifying the symbol.",
            serde_json::json!({
                "type": "object",
                "properties": {
                    "action": { "type": "string", "enum": ["lookup", "blast_radius"], "description": "Graph query mode: 'lookup' for symbol definition and caller/callee context, or 'blast_radius' for downstream impact analysis. Defaults to 'lookup'." },
                    "symbol": { "type": "string", "description": "Name of the target function, struct, class, type, or module to inspect." },
                    "path": { "type": "string", "description": "Optional file path relative to workspace root (e.g. 'src/routes/system.rs') to disambiguate symbols or scope search." },
                    "max_depth": { "type": "integer", "description": "Optional traversal depth limit for blast radius traversal (1..=50). Defaults to 20." }
                },
                "required": ["symbol"]
            }),
        ),
        ...
        def(
            "recruit_specialist",
            "Recruits a specialist agent subagent for a mission task. DELEGATION CONTRACT: 1. Sub-agents have NO access to your prior conversation history — you must supply all necessary context in the message. 2. Before delegating, tell the sub-agent what you have already done so it does not repeat completed work. 3. The sub-agent's response must be concise and actionable, citing file paths and symbol names. 4. Do not delegate your primary mission — only delegate independent sub-tasks or parallel research.",
            serde_json::json!({
                "type": "object",
                "properties": {

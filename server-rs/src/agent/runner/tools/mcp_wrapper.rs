...
//! - **Subsystem**: Sovereign Engine / Agent Runner / mcp_wrapper
//!
//! ### ⚠️ Invariants & Non-Negotiables
//! - `[Behavioral]` MCP tools enforce the dangerous composition invariant where every mutating tool is classified dangerous (enforced_by: `test_dangerous_composition_invariant`, `test_mcp_underscore_server_mutation_classification`).
//!
//! ### 🔍 Debugging & Observability
//! - **Local Errors**: none
//! - **Telemetry Targets**: none declared
//! - **Witness Tests**: `test_dangerous_composition_invariant`, `test_mcp_underscore_server_mutation_classification`

use super::policy::{
    is_cacheable_tool_name, is_dangerous_mcp_operation, is_dangerous_native_operation,
...
impl Tool for McpToolWrapper {
    ...
    async fn execute(
        ...
        let declarations = ctx
            ...
            .agents
            .get(&ctx.agent_id)
            .map(|a| a.capabilities.mcp_tools.clone());
        ...
        match result {
            ...
            crate::agent::mcp::McpResult::Structured(value) => serde_json::to_string(&value)
                .map_err(|error| ToolExecutionError::ExecutionFailed(error.to_string())),

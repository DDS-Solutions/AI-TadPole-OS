...
impl AgentRunner {
    ...
    pub(crate) async fn handle_execute_shell(
            ...
            .filter(|s| !s.trim().is_empty())
        {
            ...
            let env_map: Option<std::collections::HashMap<String, String>> = fc
                .args
                .get("envs")
        ...
        match self.state.security.shell_scanner.scan(&full_command_display) {
        ...
        if executable == "cd" {
            ...
            if resolved.exists() && resolved.is_dir() {
                ...
                let workspace_canonical = ctx.workspace_root
                    .canonicalize()
        ...
        match child {
            Ok(mut child) => {
                ...
                *output_text = format!("(PROCESS OUTPUT of '{}'):\n\n{}", full_command_display, truncated);
            }
            Err(e) => {

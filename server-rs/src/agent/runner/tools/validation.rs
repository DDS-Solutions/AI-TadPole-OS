...
//! - **Subsystem**: Sovereign Engine / Agent Runner / validation
//!
//! ### ⚠️ Invariants & Non-Negotiables
//! - `[Behavioral]` JSON arguments are validated against required parameter schemas and property types before execution (enforced_by: `test_validate_json_schema_basic`).
//!
//! ### 🔍 Debugging & Observability
//! - **Local Errors**: none
//! - **Telemetry Targets**: none declared
//! - **Witness Tests**: `test_validate_json_schema_basic`
    ...
    if let serde_json::Value::Object(map) = args {
        ...
        if let Some(v) = path_val {
            if let Some(path_str) = v.as_str() {
                ...
                let root_cow = workspace_root.to_string_lossy();
                let root_str = root_cow.as_ref();
                if p_str.starts_with(root_str) {
                    p_str = &p_str[root_str.len()..];
                } else if p_str
                    .get(..root_str.len())
                    .map_or(false, |prefix| prefix.eq_ignore_ascii_case(root_str))
                {
                    p_str = &p_str[root_str.len()..];
                }
                ...
                let safe_path = crate::utils::security::validate_path(workspace_root, clean_path)
                    .map_err(|e| {
                    ToolExecutionError::SecurityBlocked(format!("Path validation failed: {}", e))
                })?;

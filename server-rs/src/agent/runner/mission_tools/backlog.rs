...
impl AgentRunner {
    pub(crate) async fn handle_add_mission_task(
        ...
        let dependencies = if let Some(deps) = fc.args.get("dependencies").and_then(|v| v.as_array()) {
        ...
        if let Some(backlog_arc) = &ctx.backlog {
            ...
            tracing::info!(
                ...
                ctx.agent_id, task_id, ctx.mission_id
            );
            ...
            Ok(format!("Task added successfully. Task ID: {}\n\nCurrent Backlog:\n{}", task_id, backlog.progress_report()))
        } else {
    ...
    pub(crate) async fn handle_update_mission_task(
        ...
        let status = match status_str.to_lowercase().as_str() {
            ...
            "inprogress" | "in_progress" | "in-progress" => crate::agent::backlog::TaskStatus::InProgress,
            ...
            _ => return Err(ToolExecutionError::AppError(AppError::BadRequest(
                format!("Invalid status: {}", status_str)
            ))),
        };
        
        let result = fc.args.get("result").and_then(|v| v.as_str()).map(|s| s.to_string());
        ...
        if let Some(backlog_arc) = &ctx.backlog {
            ...
            tracing::info!(
                ...
                ctx.agent_id, task_id, status_str
            );
            ...
            Ok(format!("Task updated successfully.\n\nCurrent Backlog:\n{}", backlog.progress_report()))
        } else {
            Err(ToolExecutionError::AppError(AppError::InternalServerError(
                "Mission backlog not initialized for this run context.".to_string(),
            )))

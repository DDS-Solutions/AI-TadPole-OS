...
//! - **Subsystem**: Sovereign Engine / Agent Runner / codebase
//!
//! ### ⚠️ Invariants & Non-Negotiables
//! - `[Structural]` Type-safe state handling and bounded execution without unhandled panics.
//! - `[Behavioral]` Path traversal is strictly contained within workspace boundaries (enforced_by: `test_require_path_safety`).
//! - `[Behavioral]` Codebase AST symbol graph query yields definitions, callers, and blast radius (enforced_by: `test_handle_query_symbol_graph`).
//!
//! ### 🔍 Debugging & Observability
//! - **Local Errors**: none
//! - **Telemetry Targets**: `[Context]`
//! - **Witness Tests**: `test_handle_query_symbol_graph`, `test_require_path_safety`, `test_extract_symbols_rust`
...
/// Resolves the file path argument from a tool call, accepting both `path` and
/// `filename` as valid keys. Agents occasionally emit `filename` instead of
/// `path`; this alias prevents a hard `[FAILURE]` on an otherwise valid call.
///
/// Priority: `path` is checked first; `filename` is used as a fallback.
fn require_path(
    ctx: &RunContext,
    args: &serde_json::Value,
    tool_name: &str,
) -> Result<String, ToolExecutionError> {
    // Priority: canonical "path" > "filename" > "file_name" > "file"
    for alias in &["path", "filename", "file_name", "file"] {
        if let Some(val) = args.get(*alias).and_then(|v| v.as_str()) {
            if !val.trim().is_empty() {
                if *alias != "path" {
                    tracing::debug!(
                        "[codebase] Agent {} used '{}' alias instead of 'path' in '{}'. Accepting.",
                        ctx.agent_id,
                        alias,
                        tool_name
                    );
                }
                return require_str(ctx, args, alias, tool_name);
            }
        }
    }
    // Neither key present — delegate to require_str for a canonical error message
    require_str(ctx, args, "path", tool_name)
}
...
impl AgentRunner {
    ...
    pub(crate) async fn handle_read_codebase_file(
        ...
        let path_str = require_path(ctx, &fc.args, "read_codebase_file")?;
        ...
        match self
            .read_codebase_file_helper(ctx, &final_path, &path_str)
            .await
        {
            Ok(content) => {
                // Slicing logic
                ...
                let truncated = self.safe_truncate(&sliced_content, CODEBASE_READ_MAX_CHARS);
    ...
    pub(crate) async fn handle_list_file_symbols(
        ...
        let path_str = require_path(ctx, &fc.args, "list_file_symbols")?;
    ...
    pub(crate) async fn handle_get_symbol_body(
        ...
        let path_str = require_path(ctx, &fc.args, "get_symbol_body")?;
    ...
    /// Handles `query_symbol_graph`: queries the in-memory AST symbol graph for symbol lookup or blast radius analysis.
    pub(crate) async fn handle_query_symbol_graph(
        &self,
        _ctx: &RunContext,
        fc: &crate::agent::types::ToolCall,
    ) -> Result<String, ToolExecutionError> {
        ...
        if symbol_opt.is_none() && path_opt.is_none() {
            return Ok(
                "(QUERY FAILED: Please provide at least a 'symbol' name or 'path' to inspect the graph.)"
                    .to_string(),
            );
        }

        // 1. Ensure symbol graph is built
        let graph_lock = self.state.resources.get_symbol_graph().await;
        let is_empty = {
        ...
        if guard.graph.node_count() == 0 {
            return Ok(
                "(SYMBOL GRAPH EMPTY: No symbols currently indexed in the codebase graph.)"
                    .to_string(),
            );
        }
        ...
        match action {
            "blast_radius" => {
                let symbol_name = match symbol_opt {
                    Some(s) => s,
                    None => {
                        return Ok(
                            "(QUERY FAILED: 'symbol' is required for action 'blast_radius'.)"
                                .to_string(),
                        );
                    }
                };

                let mut target_node: Option<crate::intelligence::graph::SymbolNode> = None;
                for idx in guard.graph.node_indices() {
                    if let Some(node) = guard.graph.node_weight(idx) {
                        if node.name == symbol_name {
                            if let Some(p) = path_opt {
                                if node.path.ends_with(p) || p.ends_with(&node.path) {
                ...
                let target = match target_node {
                    ...
                    None => {
                        return Ok(format!(
                            "(SYMBOL '{}' NOT FOUND in symbol graph for blast radius analysis)",
                            symbol_name
                        ));
                    }
                };

                let affected = guard.calculate_blast_radius(&target.name, &target.path, max_depth);
                let mut out = format!(
                    "(BLAST RADIUS FOR '{}' in {} [Depth Limit: {}]):\n\nAffected downstream symbols ({}):\n",
                    target.name,
                    target.path,
                    max_depth,
                    affected.len()
                ...
                if affected.is_empty() {
                    out.push_str("  (No downstream dependents affected)\n");
                } else {
                    for (i, node) in affected.iter().take(30).enumerate() {
                        out.push_str(&format!(
                            "  {}. [{}] {} ({}:{}-{})\n",
                            i + 1,
                            node.kind,
            ...
            _ => {
                // "lookup" action (default)
                use petgraph::Direction;

                let mut matches_found = Vec::new();
                for idx in guard.graph.node_indices() {
                    if let Some(node) = guard.graph.node_weight(idx) {
                        let name_matches = match symbol_opt {
                            Some(s) => node.name.eq_ignore_ascii_case(s),
                            None => true,
                        };
                        let path_matches = match path_opt {
                            Some(p) => node.path.ends_with(p) || p.ends_with(&node.path),
                            None => true,
                        };
                        if name_matches && path_matches {
                            let callers: Vec<String> = guard
                                .graph
                                .edges_directed(idx, Direction::Incoming)
                                .filter_map(|e| guard.graph.node_weight(e.source()))
                                .map(|n| format!("{} ({})", n.name, n.path))
                                .take(10)
                                .collect();

                            let callees: Vec<String> = guard
                                .graph
                                .edges_directed(idx, Direction::Outgoing)
                                .filter_map(|e| guard.graph.node_weight(e.target()))
                                .map(|n| format!("{} ({})", n.name, n.path))
                                .take(10)
                                .collect();

                            matches_found.push((node.clone(), callers, callees));
                            if matches_found.len() >= 10 {
                                break;
                            }
                        }
                    }
                ...
                if matches_found.is_empty() {
                    let query_desc = match (symbol_opt, path_opt) {
                        (Some(s), Some(p)) => format!("symbol '{}' in path '{}'", s, p),
                        (Some(s), None) => format!("symbol '{}'", s),
                        (None, Some(p)) => format!("path '{}'", p),
                        (None, None) => "query".to_string(),
                    };
                    return Ok(format!(
                        "(NO SYMBOLS MATCHED: {} in the codebase symbol graph)",
                        query_desc
                    ));
                }

                let mut out = format!(
                    "(SYMBOL GRAPH LOOKUP RESULTS - {} match(es)):\n\n",
                    matches_found.len()
                );
                for (node, callers, callees) in matches_found {
                    out.push_str(&format!(
                        "• [{}] {} at {}:{}-{}\n  Signature: {}\n",
                        node.kind,
                        node.name,
                    ...
                    if !callers.is_empty() {
                        ...
                        out.push_str("  Callers: None (direct or external entrypoint)\n");
                    }
                    if !callees.is_empty() {
                        out.push_str(&format!(
                            "  Callees ({}): {}\n",
    ...
    pub(crate) async fn require_path_safety(
        ...
        let root = &self.state.base_dir;
        let mut clean_path_str = path_str.to_string();
        let path = std::path::Path::new(path_str);
        if path.is_absolute() {
            if let Ok(relative) = path.strip_prefix(root) {
                if let Some(rel_str) = relative.to_str() {
                    clean_path_str = rel_str.to_string();
                }
            }
        }

        let target_path = match crate::utils::security::validate_path(root, &clean_path_str) {
            Ok(p) => p,
            Err(e) => {
        ...
        match ext {
            ...
            "py" => {
                let re = PYTHON_RE.get_or_init(|| {
                    regex::Regex::new(r"(?m)^[ \t]*(def|class)\s+([a-zA-Z_][a-zA-Z0-9_]*)").unwrap()
                });
                for cap in re.captures_iter(content) {
                    symbols.push(format!("[{}] {}", &cap[1], &cap[2]));
                }
    ...
    pub(crate) fn extract_symbol_body(
        &self,
        content: &str,
        symbol: &str,
        path: &str,
    ...
    pub(crate) async fn read_codebase_file_helper(
        ...
        if final_path.is_dir() {
            return Err(format!(
                "Path '{}' is a directory, not a file. For directories, please use directory listing or search tools instead.",
                path_str
            ));
        }
            ...
            if state.escaped {
                ...
                if let Some((_, '{')) = chars.peek() {
                    ...
                    if state.template_literal_brace_depths.len() < MAX_TEMPLATE_LITERAL_DEPTH {
                        state
                            .template_literal_brace_depths
                            .push(state.current_depth);
                    }
                    state.in_template_literal = false;
                }
            }
...
"#;
    ...
    async fn test_require_path_safety() {
        ...
        let res = runner
            .require_path_safety(&ctx, "src/agent/runner/mission_tools.rs")
            .await;
        assert!(res.is_ok());
    }

    #[tokio::test]
    async fn test_handle_query_symbol_graph() {
        ...
        let graph_lock = state.resources.get_symbol_graph().await;
        {
            ...
            let node_a = SymbolNode {
                ...
                signature: "fn parse_tokens() -> Vec<Token>".to_string(),
                start_line: 10,
                end_line: 25,
                docstring: Some("Parses lexical tokens".to_string()),
                docstring_range: None,
            };
            let node_b = SymbolNode {
                name: "execute_plan".to_string(),
                path: "src/runner.rs".to_string(),
                kind: "function".to_string(),
                signature: "fn execute_plan()".to_string(),
                start_line: 40,
                end_line: 80,
                docstring: None,
                docstring_range: None,
            };
            let idx_a = guard.graph.add_node(node_a);
            let idx_b = guard.graph.add_node(node_b);
            // node_b calls node_a (outgoing from b to a; incoming to a from b)
            guard.graph.add_edge(
                idx_b,
                idx_a,
                crate::intelligence::graph::SymbolEdge {
                    kind: "call".to_string(),
                },
            );
            guard
                .index
                .insert("src/parser.rs\x01parse_tokens".to_string(), idx_a);
            guard
                .index
                .insert("src/runner.rs\x01execute_plan".to_string(), idx_b);
        }

        // 1. Test lookup
        let lookup_call = crate::agent::types::ToolCall {
            name: "query_symbol_graph".to_string(),
            args: serde_json::json!({
                "action": "lookup",
                "symbol": "parse_tokens"
            }),
        };
        let lookup_res = runner
            .handle_query_symbol_graph(&ctx, &lookup_call)
            .await
            .unwrap();
        assert!(lookup_res.contains("SYMBOL GRAPH LOOKUP RESULTS"));
        assert!(lookup_res.contains("parse_tokens"));
        assert!(lookup_res.contains("src/parser.rs:10-25"));
        assert!(lookup_res.contains("execute_plan (src/runner.rs)"));

        // 2. Test blast radius
        let blast_call = crate::agent::types::ToolCall {
            name: "query_symbol_graph".to_string(),
            args: serde_json::json!({
                "action": "blast_radius",
                "symbol": "parse_tokens",
        ...
        let blast_res = runner
            .handle_query_symbol_graph(&ctx, &blast_call)
            .await
            .unwrap();
        assert!(blast_res.contains("BLAST RADIUS FOR 'parse_tokens' in src/parser.rs"));
        assert!(blast_res.contains("execute_plan"));
    }

    #[tokio::test]
    async fn test_require_path_polymorphic_aliases() {
        let ctx = RunContext::default();

        let args_path = serde_json::json!({ "path": "src/main.rs" });
        assert_eq!(
            super::require_path(&ctx, &args_path, "test").unwrap(),
            "src/main.rs"
        );

        let args_filename = serde_json::json!({ "filename": "src/lib.rs" });
        assert_eq!(
            super::require_path(&ctx, &args_filename, "test").unwrap(),
            "src/lib.rs"
        );

        let args_file_name = serde_json::json!({ "file_name": "src/App.tsx" });
        assert_eq!(
            super::require_path(&ctx, &args_file_name, "test").unwrap(),
            "src/App.tsx"
        );

        let args_file = serde_json::json!({ "file": "Cargo.toml" });
        assert_eq!(
            super::require_path(&ctx, &args_file, "test").unwrap(),
            "Cargo.toml"
        );

        // Priority check: path beats filename beats file_name beats file
        let args_multi = serde_json::json!({ "path": "p.rs", "filename": "fn.rs", "file": "f.rs" });
        assert_eq!(
            super::require_path(&ctx, &args_multi, "test").unwrap(),
            "p.rs"
        );

        // Missing check
        let args_empty = serde_json::json!({ "unrelated": 123 });
        assert!(super::require_path(&ctx, &args_empty, "test").is_err());
    }
}

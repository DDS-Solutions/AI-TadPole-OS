...
fn require_path(
    ...
    if args.get("path").and_then(|v| v.as_str()).filter(|s| !s.trim().is_empty()).is_some() {
        return require_str(ctx, args, "path", tool_name);
    }
    // Fall back to "filename" alias (commonly produced by LLM outputs)
    if args.get("filename").and_then(|v| v.as_str()).filter(|s| !s.trim().is_empty()).is_some() {
        tracing::debug!(
            ...
            Ok(content) => {
                ...
                let (sliced_content, header) = if start_idx < end_idx {
                    (
                        ...
                        format!("(FILE CONTENT OF {} - Lines {} to {} of {}):\n\n", path_str, start_idx + 1, end_idx, total_lines)
                    )
                } else {
                    (
                        "".to_string(),

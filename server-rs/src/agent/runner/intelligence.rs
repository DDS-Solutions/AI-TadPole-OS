...
use super::{service_traits, AgentRunner, IntelligenceOutput, RunContext};
...
impl AgentRunner {
    async fn handle_specification_generation_phase(
        ...
        let _slot_kind = if payload.active_model_slot.as_deref() == Some("default") {
            super::service_traits::SlotKind::Default
        } else {
    ...
    fn activate_model_slot(
        ...
        let selection =
            self.model_router
                .select_model_slot(&self.state, &ctx.agent_models, preferred_kind);
        if selection.kind != super::service_traits::SlotKind::Default
...
mod tests {
    ...
    fn test_resolve_mission_state() {
        ...
        // No spec marker, not safe, not fast path -> SpecificationGeneration
    ...
    async fn test_privacy_mode_prefers_local_default_for_planning_slot() {
        ...
        let selection =
            runner
                .model_router
                .select_model_slot(&runner.state, &agent.models, SlotKind::Planning);

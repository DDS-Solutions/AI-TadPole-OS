    ...
    match val {
        serde_json::Value::Object(map) => {
            ...
            for key in sorted_keys {
                let normalized_val = normalize_json(&map[key]);
                parts.push(format!("\"{}\":{}", key, normalized_val));
            }
            format!("{{{}}}", parts.join(","))
        }
        serde_json::Value::Array(arr) => {
            let parts: Vec<_> = arr.iter().map(|v| normalize_json(v)).collect();
            format!("[{}]", parts.join(","))
        }
        _ => val.to_string(),
    }
...
impl DoomLoopDetector {
    ...
    fn has_loop(&self) -> bool {
        ...
        for period in 1..=4 {
            ...
            let window = &self.signatures[start_idx..];
...
impl AgentRunner {
    ...
    pub(crate) async fn execute_intelligence_loop(
        ...
        let system_prompt = self.build_system_prompt(ctx, &payload.message).await;
        ...
        // --- 📑 [System 2] Specification Check ---
        // If this is a coding mission and no spec exists, we must generate one first.
        let spec =
            crate::agent::mission::get_mission_context(&self.state.resources.pool, &ctx.mission_id)
                .await
                .map_err(|e| (e, None))?;
        if !spec.contains("--- [ROOM: system::spec] ---") && !ctx.safe_mode {
            // --- 🎭 [System 2] Planning Slot Activation ---
            if let Some(agent) = self.state.registry.agents.get(&ctx.agent_id) {
                if let Some(plan_config) = &agent.models.planning_slot {
                    active_ctx.model_config = plan_config.clone();
                    self.broadcast_agent(
                        ctx,
                        &format!(
                            "🧠 Orchestrator: Switching to High-Density Slot ({}) for Planning",
                            plan_config.model_id
                        ),
                        "pulse",
                    );
                }
        ...
        // --- 🎭 [System 2] Execution Slot Activation ---
        if let Some(entry) = self.state.registry.agents.get(&ctx.agent_id) {
            let agent = entry.value();
            if let Some(exec_config) = &agent.models.execution_slot {
                active_ctx.model_config = exec_config.clone();
                self.broadcast_agent(
                    ctx,
                    &format!(
                        "⚡ Orchestrator: Switching to High-Speed Slot ({}) for Tactical Execution",
                        exec_config.model_id
                    ),
                    "pulse",
                );
            }
        ...
        let mut loop_detector = DoomLoopDetector::new();
        ...
        while turn_count < max_turns {
            ...
            while reasoning_turn < ctx.reasoning_depth && !reasoning_halted {
                ...
                let provider_res = self
                    .call_provider(&active_ctx, &system_prompt, &current_prompt, Some(tools))
                    .await;
                ...
                let slot = if active_ctx.model_config.model_id == ctx.model_config.model_id {
                    "default"
                } else {
                    if active_ctx.model_config.model_id.contains("mini")
                        || active_ctx.model_config.model_id.contains("flash")
                    {
                        "execution"
                    } else {
                        "planning"
                ...
                if reasoning_turn < ctx.reasoning_depth && !reasoning_halted {
                    ...
                    for fc in function_calls {
                        ...
                        futures.push(async move {
                            ...
                            (fc_name, fc_args_str, result, local_text, local_usage)
                        });
                    }
                    ...
                    while let Some((name, args_str, result, mut local_text, local_usage)) = futures.next().await {
                        self.accumulate_usage(&mut usage, local_usage);

                        // Hashed Loop & Error Cycle Detection
                        if loop_detector.check(&name, &args_str, &local_text) {
                            tracing::warn!(
                        ...
                        if name == "complete_mission" {
                            ...
                            if !self.verify_mission_success(&observation_buffer) && !ctx.safe_mode {
                                self.broadcast_agent(ctx, "🚨 Sentinel Gate: Finalization BLOCKED. No proof of verification found.", "warning");
...
fn deterministic_fallback_summarize(history: &str) -> String {
    ...
    async fn compress_monologue(
        ...
        let (older_turns, tail_turns) = if monologue_len > tail_count {
            ...
            (monologue[..split_idx].to_vec(), monologue[split_idx..].to_vec())
        } else {
...
mod tests {
    ...
    async fn test_compress_monologue() {
        ...
        let input_with_large_log = "Some reasoning\n```json\n".to_string() + &"a".repeat(2500) + "\n```";

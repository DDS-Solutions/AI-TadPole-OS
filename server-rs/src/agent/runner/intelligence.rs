        ...
        match output {
            ...
            Err(e) => {
                if let Err(rollback_err) = state_tx.rollback().await {
                    tracing::error!("❌ [StateTransaction] Explicit rollback failed: {:?}", rollback_err);
                }
                Err(e)
            }
        }
...
impl AgentRunner {
    ...
    pub(crate) fn is_fast_path_query(message: &str) -> bool {
        ...
        let complex_triggers = [
            "implement", "build", "refactor", "debug", "compile", "run tests", 
            "migrate", "deploy", "stress test", "benchmark", "integrate", "solve"
        ];
        ...
        let simple_triggers = [
            "what is", "get status", "read file", "list files", "check budget",
            "show metrics", "get metrics", "health", "system details", "who are you"
        ];
        let is_short = message.len() < 150;
        is_short || simple_triggers.iter().any(|trigger| lower.contains(trigger))
    }
    ...
    async fn handle_reasoning_phase(
        ...
        if is_fast_path {
            tracing::info!("⚡ [Fast-Path] Simple/read query detected. Engaging System 1 fast execution.");

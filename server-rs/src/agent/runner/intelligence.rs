...
impl AgentRunner {
    ...
    pub(crate) async fn execute_intelligence_loop(
        ...
        while turn_count < MAX_TURNS {
            turn_count += 1;
            tracing::debug!("🎯 [Intelligence] Start Turn {}/{} for agent {}", turn_count, MAX_TURNS, ctx.agent_id);
            ...
            if let Some(budget_msg) = self.check_budget(ctx, cumulative_cost, &output_text).await.map_err(|e| (e, usage.clone()))? {
                return Ok(IntelligenceOutput { text: budget_msg, usage });
            }
            ...
            if mission_completed {
                ...
                tracing::info!("🏁 [Intelligence] Agent {} signaled mission completion.", ctx.agent_id);
                break;
            }
        }
    ...
    async fn enforce_sentinel_gate(
        ...
        if !is_orchestrator && function_calls.is_empty() && !output_text.contains("complete_mission") {
            ...
            let swarm_tool = self.build_tools(ctx).await;
            let sentinel_result = self.call_provider(
                ctx,
                system_prompt,
                &sentinel_directive,
                Some(vec![swarm_tool]),
...
mod tests {
    ...
    async fn test_enforce_sentinel_gate_orchestrator_bypass() {
        ...
        let _ = runner.enforce_sentinel_gate(
            &ctx, 
            "System", 
            "Report status", 
            &mut output_text, 

...
impl AgentRunner {
    ...
    async fn finalize_mission_persistence(
        ...
        let receipt_status = if is_branch {
...
mod tests {
    ...
    async fn branch_finalization_does_not_close_parent_mission() {
        ...
        runner
            .finalize_mission_persistence(&ctx, "branch result", &None)
            .await
            .unwrap();
    ...
    async fn partial_root_result_pauses_for_human_review() {
        ...
        runner
            .finalize_mission_persistence(&ctx, "partial result", &None)
            .await
            .unwrap();

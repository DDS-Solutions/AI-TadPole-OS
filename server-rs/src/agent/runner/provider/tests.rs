...
mod tests {
    ...
    async fn test_privacy_mode_local_routing() {
        ...
        // Post-refactor: Privacy Shield fails closed when no local Ollama is reachable.
        // Accept either: OpenAI(Ollama) when Ollama runs with a <=15B model, OR
        // Null(PrivacyModeEnforced) when Ollama is offline (e.g. in CI / local test environments).
        match &variant {
            ProviderVariant::OpenAI(ref p) => {
                assert_eq!(p.config.provider, ModelProvider::Ollama);
                assert!(!p.config.model_id.is_empty());
                assert_eq!(p.api_key, "ollama");
            }
            ProviderVariant::Null(ref null_p) => {
                assert_eq!(
                    null_p.reason.as_str(),
                    "privacy_mode_enforced",
                    "Expected PrivacyModeEnforced null when no local Ollama model is available"
                );
            }
            _ => panic!(
                "Expected OpenAI/Ollama or PrivacyModeEnforced null for cloud model under privacy shield"
            ),
        }

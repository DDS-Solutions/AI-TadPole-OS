...
//!
//! ### AI Assist Note
//! **Provider Module**: Decouples the engine from specific LLM vendors (Groq, Anthropic, Gemini, OpenAI) and proxies usage.
//!
//! ### 🔍 Debugging & Observability
//! - **Failure Path**: Model alignment check errors or budget calculation mismatch.
//!
...
impl AgentRunner {
    ...
    pub(crate) async fn check_budget(
        ...
        if let Some(sub_budget) = ctx.sub_budget_usd {
            let current_db_cost = match crate::agent::mission::get_mission_by_id(&self.state.resources.pool, &ctx.mission_id).await {
                Ok(Some(m)) => m.cost_usd,
                _ => ctx.current_cost_usd,
            };
            let accumulated_by_sub_agent = ((current_db_cost + step_cost) - ctx.current_cost_usd).max(0.0);

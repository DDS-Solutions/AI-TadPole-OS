...
impl AgentRunner {
    ...
    pub(crate) async fn resolve_provider(
        ...
        // OpenRouter Model Resolution: Ensure exact canonical 'stealth/ox-alpha' is preserved and sent to OpenRouter
        if active_config.provider == ModelProvider::Openrouter {
            let lower = active_config.model_id.to_lowercase();
            let is_stealth_alias = lower.contains("stealth")
                || lower.contains("ox-alpha")
                || lower.contains("oxalpha")
                || lower.is_empty();

            if is_stealth_alias {
                let canonical = "stealth/ox-alpha".to_string();
                if active_config.model_id != canonical {
                    tracing::info!(
                        "🔄 [OpenRouter Resolution] Normalized alias '{}' to canonical OpenRouter model '{}'",
                        active_config.model_id,
                        canonical
                    );
                    active_config.model_id = canonical;
                }
            } else if let Ok(override_model) = std::env::var("OPENROUTER_DEFAULT_MODEL") {

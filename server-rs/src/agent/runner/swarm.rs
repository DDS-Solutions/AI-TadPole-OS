...
impl AgentRunner {
    ...
    pub(crate) async fn handle_spawn_subagent(
        ...
        for sub_agent_id in target_ids {
            ...
            // Clone values for the sub-agent existence check
            let model_config_clone = ctx.model_config.clone();
            let extra_skills_clone = extra_skills.cloned();
            let extra_workflows_clone = extra_workflows.cloned();
            let extra_mcp_clone = extra_mcp.cloned();
            let role_override_clone = role_override.map(|s| s.to_string());

            swarm_tasks.push(async move {
                // Ensure sub-agent exists
                if let Err(e) = runner.ensure_sub_agent_exists(
                    &sub_id_clone,
                    &model_config_clone,
                    extra_skills_clone.as_ref(),
                    extra_workflows_clone.as_ref(),
                    extra_mcp_clone.as_ref(),
                    role_override_clone.as_deref(),
                ).await {
                    return (sub_id_clone, format!("RECRUITMENT FAILURE: {}", e));
                }
        ...
        if results.is_empty() {
             *output_text = "ERROR: No sub-agents were spawned (Protocol Violation or empty IDs).".to_string();
             return Ok(());
        }
...
mod tests {
    ...
    async fn test_parallel_swarm_recruitment_logic() {
        ...
        let _ = runner.handle_spawn_subagent(&ctx, &fc, &mut output, &mut usage).await;

...
//!
//! This module implements the "hive mind" skills of Tadpole OS, 
...
impl AgentRunner {
    ...
    pub(crate) async fn handle_spawn_subagent(
        ...
        self.state.broadcast_sys(format!("🐝 Agent {}: Handing off to sub-agent {}...", ctx.agent_id, sub_agent_id).as_str(), "info");
        ...
        self.ensure_sub_agent_exists(sub_agent_id, &ctx.model_config, extra_skills, extra_workflows, extra_mcp)
            .await?;
    ...
    pub(crate) async fn ensure_sub_agent_exists(
        ...
        if let Some(skills) = extra_skills {
            for s in skills {
                if let Ok(name) = self.state.registry.skills.register_capability("skill", s.clone(), "ai").await {
        ...
        if let Some(workflows) = extra_workflows {
            for w in workflows {
                if let Ok(name) = self.state.registry.skills.register_capability("workflow", w.clone(), "ai").await {
                    tracing::info!("Registered discovered workflow: {}", name);
                }
            }
        }
        // TODO: Implement MCP registration in McpHost if needed

...
impl AgentRunner {
    ...
    pub(crate) async fn handle_spawn_subagent(
        ...
        if let Ok(mut tx) = self.state.resources.pool.begin().await {
            for sub_agent_id in &target_ids {
                ...
                {
                    tracing::warn!(
                        "⚠️ [Swarm] Failed to ensure sub-agent {} pre-flight: {}",
                        sub_agent_id,
                        e
                    );
                }
        ...
        if let Some(ref plan) = ctx.conductor_plan {
            tracing::info!("🧠 [Conductor] Executing topologically sorted Conductor plan for agent '{}'", ctx.agent_id);
            let sorted_steps = match Self::topological_sort_conductor_steps(&plan.steps) {
                Ok(s) => s,
                Err(e) => {
                    tracing::error!("Conductor topological sort failed: {:?}", e);
                    return Ok(format!("ERROR: Conductor plan topological sort failed: {:?}", e));
                }
            };

            let mut step_results: std::collections::HashMap<u32, String> = std::collections::HashMap::new();
            for step in sorted_steps {
                if ctx.lineage.contains(&step.target_agent) || ctx.agent_id == step.target_agent {
                    tracing::warn!("🛡️ [Swarm] Conductor recursion block for {} vs {:?}", step.target_agent, ctx.lineage);
                ...
                let primary_goal_text = ctx.primary_goal.as_deref().unwrap_or("See mission scope for details.");
        ...
        if let Some(ref vt) = ctx.visible_transcript {
            vt.lock().push(format!("OBSERVATION (Spawn Sub-agents): {}", final_text));
        }
        Ok(final_text)
    }
    ...
    pub(crate) async fn handle_alpha_directive(
        ...
        if let Some(ref vt) = ctx.visible_transcript {
            vt.lock().push(format!("OBSERVATION (Alpha Directive): {}", sub_result));
        }

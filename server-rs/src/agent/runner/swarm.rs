...
impl AgentRunner {
    ...
    pub(crate) async fn handle_spawn_subagent(
        ...
        if ctx.depth >= max_swarm_depth {
            ...
            *output_text = format!("PROTOCOL_VIOLATION: Swarm depth limit exceeded (current depth: {}). You cannot spawn more sub-agents.", ctx.depth);
            return Ok(());
        }
    ...
    pub(crate) async fn ensure_sub_agent_exists(
        ...
        let registry_match = self
            ...
            .find(|kv| {
                let a = kv.value();
                a.identity.category == "user"
                    && (a.identity.name.eq_ignore_ascii_case(opts.agent_id)
...
mod tests {
    ...
    async fn test_tiered_recruitment_priority() {
        ...
        let mut tx1 = state.resources.pool.begin().await.unwrap();
        runner
        ...
        let mut tx2 = state.resources.pool.begin().await.unwrap();
        runner
            .ensure_sub_agent_exists(
                &mut *tx2,
                SubAgentOptions {
        ...
        let mut tx3 = state.resources.pool.begin().await.unwrap();
        runner
            .ensure_sub_agent_exists(
                &mut *tx3,
                SubAgentOptions {

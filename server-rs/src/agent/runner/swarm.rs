...
impl AgentRunner {
    ...
    pub(crate) async fn handle_spawn_subagent(
        ...
        let sub_err = match self.ensure_sub_agent_exists(
            sub_agent_id,
            &ctx.model_config,
            extra_skills,
            extra_workflows,
    ...
    pub(crate) async fn ensure_sub_agent_exists(
        ...
        let (initial_role, initial_dept, initial_desc) = if let Some(r) = role_override {
            (r.to_string(), "Tactical Operations".to_string(), format!("Specialized agent with role override: {}", r))
        } else {
            match sub_agent_id.to_lowercase().as_str() {
    ...
    pub(crate) async fn handle_alpha_directive(
        ...
        self.broadcast_agent(
            ctx,
            "🧬 Issuing directive to Tadpole Alpha...",
            "info",
        );
...
mod tests {
    ...
    async fn test_proactive_recursion_block_parent() {
        ...
        let mut ctx = RunContext {
            ...
            lineage: vec!["2".to_string(), "alpha".to_string(), "weather_agent".to_string()],
            ..RunContext::default()
        };
        ...
        let result = runner.handle_spawn_subagent(&ctx, &fc, &mut output, &mut usage).await;

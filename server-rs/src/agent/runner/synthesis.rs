...
struct PromptAssemblerContext<'a> {
    directives: &'a str,
    reviews: &'a str,
    identity: &'a str,
    memory: &'a str,
...
impl AgentRunner {
    ...
    pub async fn build_system_prompt(&self, ctx: &RunContext, payload_message: &str) -> String {
        ...
        let (pruned_repo_map, pruned_swarm_context_str) =
            self.prune_context(ctx, &identity, &memory, &repo_map_text, &swarm_context_str);
    ...
    async fn fetch_external_context(&self, ctx: &RunContext) -> (String, String) {
        let pending_directives = super::swarm_persistence::get_pending_directives(
            &self.state.resources.pool,
            &ctx.agent_id,
        )
        .await
        .unwrap_or_default();
        let pending_reviews = super::swarm_persistence::get_pending_reviews(
            &self.state.resources.pool,
            &ctx.agent_id,
        )
        .await
    ...
    async fn fetch_mission_context(
        &self,
        ctx: &RunContext,
    ...
    fn assemble_prompt_variables(
        ...
        let compressed_swarm_context = self.aaak_encode(p_ctx.swarm_context);
        ...
        let compressed_findings = self.aaak_encode(
            ctx.recent_findings
                .as_deref()
        ...
        vars.insert(
            "breadcrumbs",
            self.generate_breadcrumbs_display(ctx).to_string(),
        );
    ...
    pub(crate) fn get_hierarchy_label(
        &self,
        level: crate::agent::types::RoleAuthorityLevel,
    ) -> &str {
        match level {
    ...
    fn generate_swarm_protocols(&self, ctx: &RunContext) -> String {
        let mut swarm_protocols = self.state.resources.acl.get_role_protocols(
            &ctx.agent_id,
            &ctx.role,
            ctx.authority_level,
        );
    ...
    async fn gather_global_intelligence(
        &self,
        ctx: &RunContext,
        query: &str,
    ) -> Result<String, crate::error::AppError> {
    ...
    pub(crate) async fn build_tools(&self, ctx: &RunContext) -> ToolDefinition {
        ...
        for tool in tool_list {
            if self.state.resources.acl.is_tool_allowed(
                &ctx.agent_id,
                &ctx.role,
                ctx.authority_level,
                &tool.name,
                ...
                if (tool.name == "read_file"
                    ...
                    || tool.name == "delete_file")
                    && !self.has_file_system_capability(ctx)
                {
                    continue;
                }
...
mod tests {
    ...
    async fn test_context_pruning_basic() {
        ...
        let (pruned_map, pruned_swarm) =
            runner.prune_context(&ctx, &identity, &memory, &repo_map, &swarm_context);
    ...
    async fn test_build_tools_ceo_restriction() {
        ...
        let has_read_file = tools
            .function_declarations
            .iter()
            .any(|f| f.name == "read_file");
        assert!(!has_read_file);

        // CEO SHOULD have issue_alpha_directive
        let has_directive = tools
            .function_declarations
            .iter()
            .any(|f| f.name == "issue_alpha_directive");
    ...
    async fn test_build_tools_observer_restriction() {
        ...
        assert!(
            !has_write,
            "Observer should NOT be able to write even if skill is assigned"
        );
    }
    ...
    async fn test_system_failure_context_injection() {
        ...
        assert!(
            prompt.contains("SYSTEM WARNING"),
            "Prompt should contain system warning alert"
        );
        assert!(
            prompt.contains("IDENTITY_FAILURE"),
            "Prompt should mention IDENTITY_FAILURE"
        );
        assert!(
            prompt.contains("MEMORY_FAILURE"),
            "Prompt should mention MEMORY_FAILURE"
        );
    }
}

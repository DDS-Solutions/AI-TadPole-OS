...
impl AgentRunner {
    ...
    fn generate_structural_components(
        ...
        let cluster_directory = if cluster_specialists.is_empty() {
            ...
            let is_orchestrator =
                ctx.agent_id == "1" || ctx.agent_id == "2" || ctx.agent_id == "alpha";
    ...
    fn add_core_tools(
        &self,
        ctx: &RunContext,
        function_declarations: &mut Vec<crate::agent::gemini::GeminiFunctionDeclaration>,
    ) {
        if !ctx.safe_mode {
            if ctx.agent_id != "1" {
                function_declarations.push(crate::agent::gemini::GeminiFunctionDeclaration {

...
impl AgentRunner {
    ...
    pub(crate) async fn build_system_prompt(
        ...
        for skill_name in &ctx.skills {
            if let Some(skill_entry) = self.state.registry.capabilities.skills.get(skill_name) {
        ...
        format!(
            "{safe_mode_prefix}{tool_mode_prefix}You are {} (ID: {}, Role: {}) at the {} level of the swarm hierarchy.\n\
             ...
             2. COO PROTOCOL: If you are the COO, you MUST delegate the mission to the Alpha Node. Use 'spawn_subagent' with agentId 'alpha'. The Alpha Node has full filesystem and coding capabilities. Direct specialist recruitment is SYSTEM-BLOCKED.\n\
    ...
    pub(crate) fn build_tools(&self, ctx: &RunContext) -> crate::agent::gemini::GeminiTool {
        if !ctx.model_config.supports_native_tools() {
            return crate::agent::gemini::GeminiTool { function_declarations: vec![] };
        }
        ...
        let has_fs_skill = ctx.skills.iter().any(|s| {
            s == "filesystem" || s == "read_file" || s == "write_file" || s == "list_files" || s == "delete_file"
        });
        ...
        if ctx.skills.contains(&"propose_capability".to_string()) {
            function_declarations.push(crate::agent::gemini::GeminiFunctionDeclaration {
        ...
        if !ctx.safe_mode {
            let mcp_tools = self
                .state.registry
                .mcp_host
                .list_tools(&ctx.skills, &self.state.registry.capabilities.skills);

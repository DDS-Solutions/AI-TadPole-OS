...
impl AgentRunner {
    ...
    pub(crate) async fn build_system_prompt(
        ...
        let swarm_context_result = crate::agent::mission::get_mission_context(
        ...
        let findings_str = ctx.recent_findings.as_deref().unwrap_or("No recent findings inherited.");
        ...
        tracing::info!("🏁 [Runner] Synthesizing system prompt for mission: {} (Depth: {})", ctx.mission_id, ctx.depth);
        system_prompt
    }
    ...
    fn prune_context(
        ...
        if est_tokens > safe_limit {
            ...
            tracing::info!("✅ [Runner] Context token load ({}) is safely below the limit.", est_tokens);
        }
    ...
    fn generate_structural_components(&self, ctx: &RunContext) -> (
        ...
        let cluster_directory = if cluster_specialists.is_empty() {
            ...
            let is_orchestrator = ctx.agent_id == "1" || ctx.agent_id == "2" || ctx.agent_id == "alpha";
            
            cluster_specialists.into_iter().map(|id| {
                if is_orchestrator {
                    id
                } else {
                    match id.as_str() {
                        "1" => "[SYSTEM_OVERLORD]".to_string(),
                        "2" => "[SYSTEM_ORCHESTRATOR]".to_string(),
                        "alpha" => "[SYSTEM_COMMANDER]".to_string(),
        ...
        let lineage_display = if ctx.lineage.is_empty() {
            ...
            let path_parts: Vec<String> = ctx.lineage.iter().map(|id| {
                if is_orchestrator {
                    id.clone()
                } else {
                    match id.as_str() {
                        "1" => "OVERLORD".to_string(),
                        "2" => "ORCHESTRATOR".to_string(),
                        "alpha" => "COMMANDER".to_string(),
                        _ => id.clone()
                    }
                }
            }).collect();
    ...
    fn generate_protocols(&self, ctx: &RunContext) -> String {
        ...
        let is_manager = ctx.agent_id == "1" || ctx.agent_id == "2" || ctx.role.contains("Orchestrator") || ctx.role.contains("Overlord");
        ...
        if ctx.agent_id == "1" {
            ...
            let identity_label = if is_manager {
                format!("Specialist-{}", &ctx.agent_id[..std::cmp::min(ctx.agent_id.len(), 4)])
            } else {
                ctx.agent_id.clone()
            };
            ...
            priority_protocols.push("2. COMMANDER IS BUSY: You are under the supervision of the Alpha Node. Do NOT attempt to recruit 'alpha', '2', or '1' for assistance. Resolve the task yourself.".to_string());
        }
        ...
        priority_protocols.push("4. COMPLETION: Complete ALL steps autonomously before reporting back.".to_string());
        ...
        let priority_str = priority_protocols.iter().enumerate().map(|(i, p)| format!("{}. {}", i+1, p)).collect::<Vec<_>>().join("\n");
        priority_str
    }
    ...
    fn add_core_tools(&self, ctx: &RunContext, function_declarations: &mut Vec<crate::agent::gemini::GeminiFunctionDeclaration>) {
        if !ctx.safe_mode {
            if ctx.agent_id != "1" {
                function_declarations.push(crate::agent::gemini::GeminiFunctionDeclaration {
                    name: "spawn_subagent".to_string(),
                    description: "Spawns a specialized sub-agent to handle a specific sub-task in parallel. Use this for research, coding, or auditing while you orchestrate. If you are the COO, you MUST use 'alpha' as the agent_id.".to_string(),
                    parameters: serde_json::json!({
                        "type": "object",
                        "properties": {
                            "agent_id": { "type": "string", "description": "The ID of the specialist agent to recruit (e.g., 'researcher', 'coder'). If a specialist exists in the CLUSTER DIRECTORY, use their ID to resume work." },
                            "message": { "type": "string", "description": "The specific instruction or question for the sub-agent." },
                            "role": { "type": "string", "description": "Optional: override the agent's functional role/personality (e.g., 'Senior Security Auditor')." }
                        },
    ...
    fn add_filesystem_tools(&self, ctx: &RunContext, function_declarations: &mut Vec<crate::agent::gemini::GeminiFunctionDeclaration>) {
        if ctx.agent_id == "1" { return; } // HIERARCHY GUARD: Agent 1 (CEO) is blocked.
    ...
    fn add_advanced_tools(&self, ctx: &RunContext, function_declarations: &mut Vec<crate::agent::gemini::GeminiFunctionDeclaration>) {
        function_declarations.push(crate::agent::gemini::GeminiFunctionDeclaration {
        ...
        if ctx.skills.contains(&"propose_skill".to_string()) || ctx.skills.contains(&"propose_capability".to_string()) {
            function_declarations.push(crate::agent::gemini::GeminiFunctionDeclaration {
...
mod tests {
    ...
    async fn test_ceo_tool_block() {
        ...
        ctx.model_config.provider = ModelProvider::Groq; // Supports tools

        let tools = runner.build_tools(&ctx).await;
        
        // Verify no filesystem tools
        let has_filesystem = tools.function_declarations.iter().any(|t| 
            t.name == "read_file" || t.name == "write_file"
        );
        assert!(!has_filesystem, "CEO (Agent 1) should NOT have direct filesystem access");
    }
    ...
    async fn test_specialist_masking() {
        ...
        let mut ctx = RunContext::default();
        ctx.role = "Orchestrator".to_string(); // Managers get masking
    ...
    async fn test_aaak_encoding_fidelity() {
        ...
        assert!(encoded.contains("temp"), "Should encode temperature to temp");
    }
}

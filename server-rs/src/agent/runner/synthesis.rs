...
impl AgentRunner {
    ...
    pub(crate) async fn build_tools(&self, ctx: &RunContext) -> ToolDefinition {
        ...
        // Bounded targeted eviction to prevent memory growth without blocking map clears
        if TOOL_CACHE.len() > 64 {
            let keys_to_remove: Vec<_> = TOOL_CACHE
                .iter()
                .take(16)
                .map(|kv| kv.key().clone())
                .collect();
    ...
    fn add_core_tools(
        ...
        // 🧠 [Mythos] Early Halting via confidence score (Adaptive Computation Time)
        function_declarations.push(FunctionDeclaration {

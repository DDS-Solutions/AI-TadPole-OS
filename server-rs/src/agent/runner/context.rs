...
impl AgentRunner {
    ...
    pub(crate) async fn resolve_agent_context(
        ...
        let target_model_id = payload
            .model_id
            .clone()
            .or_else(|| a.models.model_id.clone())
            .unwrap_or_else(|| a.models.model.model_id.clone());
            ...
            self.state.registry.models.get(&target_model_id)
        {
            ...
            let base_config = ModelConfig {
                ...
                model_id: if provider_config.protocol == crate::agent::types::ModelProvider::Ollama
                    && Uuid::parse_str(&model_id).is_ok()
                {
                    tracing::debug!(
                        "🔄 [Context] Resolving Ollama UUID {} to tag: {}",
                        model_id,
                        model_entry.name
                    );
                ...
                system_prompt: a.models.model.system_prompt.clone(),
                temperature: a.models.model.temperature,
                max_tokens: a.models.model.max_tokens,
                ...
                reasoning_depth: a.models.model.reasoning_depth,
                act_threshold: a.models.model.act_threshold,
                max_turns: a.models.model.max_turns,
                connector_configs: None,
                extra_parameters: None,
            };

            // Option B: Strip bulky system prompt from tokens if steering vectors will natively handle persona mapping
            let mut base_config = base_config;
            if provider_config.supports_steering_vectors {
                base_config.system_prompt = Some("".to_string());
            }
            ...
            .find(|kv| kv.value().name.to_lowercase() == target_model_id.to_lowercase())
        {
            ...
            let mut cfg = a.models.model.clone();
        ...
        if safe_mode {
            // Strip ALL tools to ensure a pure conversational "Private Brainstorming" session.
            // This prevents specialists from being forced into tool-use loops.
        ...
        let (budget_usd, current_cost_usd) = if mission_id != "system-internal"
            && !mission_id.is_empty()
        {

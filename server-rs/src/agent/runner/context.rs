...
impl AgentRunner {
    ...
    pub(crate) async fn resolve_agent_context(
        ...
        if resolved_config.base_url.is_none() && resolved_config.provider == crate::agent::types::ModelProvider::Openai {
            if let Some(detected) = crate::agent::types::ModelProvider::from_model_id(&resolved_config.model_id) {
    ...
    pub(crate) async fn prepare_run_context(
        ...
        if mission_id != "system-internal" && !mission_id.is_empty() {
            if let Ok(Some(m)) = crate::agent::mission::get_mission_by_id(&self.state.resources.pool, mission_id).await {
    ...
    pub fn build_sandboxed_transcript(&self, raw_history: &[String]) -> Vec<String> {
        ...
        for msg in raw_history {
            ...
            if (msg.contains("[SystemOnly]") || msg.contains("[Private]"))
                && !self.role.eq_ignore_ascii_case("CEO")
                && !self.role.eq_ignore_ascii_case("Alpha")
            {
                continue;
            }

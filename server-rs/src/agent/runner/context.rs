...
impl AgentRunner {
    ...
    pub(crate) async fn prepare_run_context(
        ...
        if !ctx.safe_mode {
            let blueprint_svc = crate::services::blueprint_service::BlueprintService::new(
                ctx.workspace_root.clone(),
            );
            if let Ok(blueprint) = blueprint_svc.scan_workspace().await {
            ...
            if let Ok(vec) = provider.embed(&initial_prompt).await {
                    ...
                    .await
                {
                    ...
                    for (i, text) in top_results.into_iter().enumerate() {
                        let _ = mission_mem
                            .add_memory(&format!("mem-{}", i), &text, &ctx.mission_id, vec.clone())
                            .await;
                    }

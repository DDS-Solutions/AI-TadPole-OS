...
impl AgentRunner {
    ...
    pub(crate) async fn resolve_agent_context(
        ...
        let target_model_id = payload
            .model_id
            .as_ref()
            .or_else(|| match a.models.active_model_slot.as_deref() {
        ...
        let slot_cfg = match a.models.active_model_slot.as_deref() {
        ...
        Ok(RunContext {
            ...
            commands_run: std::sync::Arc::new(parking_lot::Mutex::new(std::collections::HashSet::new())),
    ...
    pub(crate) async fn prepare_run_context(
        ...
        let backlog = self.state.registry.mission_backlogs
            .entry(mission_id.to_string())
            .or_insert_with(|| std::sync::Arc::new(parking_lot::Mutex::new(crate::agent::backlog::MissionBacklog::new(mission_id))))
            .clone();
    ...
    fn construct_registry_config(
        ...
        let model_id = if let Some(pm_id) = &m.provider_model_id {
            pm_id.clone()
        } else if provider_config.protocol == crate::agent::types::ModelProvider::Ollama {
            // For Ollama, we prefer names/tags over UUIDs if possible
            if Uuid::parse_str(&m.id).is_ok() {
                m.name.clone()
            } else {
                m.id.clone()
            }
    ...
    async fn synchronize_rag_memory(&self, ctx: &mut RunContext, initial_prompt: &str) {
        ...
        #[cfg(feature = "vector-memory")]
        {
            ...
            for entry in &top_results {
                embedding_futures.push(provider.embed(&entry.text));
            }
...
impl RunContext {
    ...
    pub fn build_sandboxed_transcript(&self, raw_history: &[String]) -> Vec<String> {
        ...
        for msg in raw_history {
            let mut clean_msg = msg.replace("<halting_signal/>", "")
                .replace("<halt/>", "")
                .replace("<thinking>", "")
                .replace("</thinking>", "")
                .trim()
            ...
            if (clean_msg.starts_with("OBSERVATION:") || clean_msg.starts_with("TOOL OUTPUT:"))
                && clean_msg.len() > 300
            {
                clean_msg = format!("{}... [TRUNCATED TOOL OUTPUT]", &clean_msg[..300]);
            }

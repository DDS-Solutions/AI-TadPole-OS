...
impl AgentRunner {
    ...
    pub(crate) async fn handle_pin_mission(
        ...
        Ok(
            "(MISSION PINNED: This mission will now bypass the 48h Swarm Reaper cycle.)"
                .to_string(),
        )
    }
    ...
    pub(crate) async fn run_verification_script(
        ...
        let mut child = tokio::process::Command::new("powershell")
            ...
            .spawn()
            .map_err(|e| {
                AppError::InternalServerError(format!("Failed to spawn verification script: {}", e))
            })?;
    ...
    pub(crate) async fn handle_get_symbol_body(
        ...
        if path_str.is_empty() || symbol_name.is_empty() {
            return Ok(
                "(GET SYMBOL FAILED: 'path' and 'symbol' arguments are required)".to_string(),
            );
        }
    ...
    fn extract_symbols(&self, content: &str, path: &str) -> Vec<String> {
        ...
        match ext {
            ...
            "py" => {
                let re = regex::Regex::new(r"(?m)^[ \t]*(def|class)\s+([a-zA-Z_][a-zA-Z0-9_]*)")
                    .unwrap();
                for cap in re.captures_iter(content) {
                    symbols.push(format!("[{}] {}", &cap[1], &cap[2]));
                }
    ...
    pub(crate) async fn handle_archive_to_global_vault(
        ...
        #[cfg(feature = "vector-memory")]
        {
            ...
            if let Ok(vec) =
                crate::agent::memory::get_gemini_embedding(&http_client, &api_key, content).await
            {
                if let Ok(vault) = crate::agent::memory::VectorMemory::connect(
                ...
                .await
                {
                    ...
                    if let Ok(_) = vault.add_memory(&id, content, &ctx.mission_id, vec).await {
                        self.broadcast_agent(
                            ctx,
    ...
    pub(crate) async fn handle_store_knowledge(
        ...
        let text = fc.args.get("text").and_then(|v| v.as_str()).unwrap_or("");
        let topic = fc.args.get("topic").and_then(|v| v.as_str()).unwrap_or("");
        if text.is_empty() || topic.is_empty() {
            return Ok("(STORE KNOWLEDGE FAILED: 'text' and 'topic' are required)".to_string());
        }

        // Topic-ACL Check
        const SENSITIVE_TOPICS: &[&str] = &["finance", "legal", "payroll", "pii", "medical"];
        let topic_lower = topic.to_lowercase();
        let is_sensitive = SENSITIVE_TOPICS.iter().any(|&s| topic_lower.contains(s));
                ...
                .submit_oversight(
                    crate::agent::types::ToolCallAudit {
                        ...
                        description: format!(
                            ...
                            topic,
                            if text.len() > 120 { &text[..120] } else { text }
                        ),
                        timestamp: chrono::Utc::now().to_rfc3339(),
                    },
                    Some(ctx.mission_id.clone()),
        ...
        #[cfg(feature = "vector-memory")]
        {
            let cluster_id = fc.args.get("cluster_id").and_then(|v| v.as_str()).map(|s| s.to_string());
            let confidence = fc.args.get("confidence").and_then(|v| v.as_f64()).map(|f| f as f32);
            ...
            match self.state.resources.get_knowledge_store().await {
                Ok(ks) => {
                    match ks.add(req).await {
                        Ok((id, dedup_hit)) => {
                            let msg = if dedup_hit {
                                format!("(STORE KNOWLEDGE SUCCESS): Duplicate entry detected. Returned existing entry ID: {}", id)
                            } else {
                                format!("(STORE KNOWLEDGE SUCCESS): New entry stored with ID: {}", id)
                            };
                            self.broadcast_agent(
                                ctx,
                ...
                Err(e) => {
                    return Ok(format!("(STORE KNOWLEDGE FAILED: Could not acquire store: {})", e));
                }
            }
        }
    ...
    pub(crate) async fn handle_search_knowledge(
        ...
        let query = fc.args.get("query").and_then(|v| v.as_str()).unwrap_or("");
        if query.is_empty() {
            return Ok("(SEARCH KNOWLEDGE FAILED: 'query' argument is missing)".to_string());
        }
        ...
        #[cfg(feature = "vector-memory")]
        {
            let topic = fc.args.get("topic").and_then(|v| v.as_str()).map(|s| s.to_string());
            let limit = fc.args.get("limit").and_then(|v| v.as_u64()).map(|n| n as usize);
            ...
            match self.state.resources.get_knowledge_store().await {
                Ok(ks) => {
                    match ks.search(req).await {
                        Ok(results) => {
                            if results.is_empty() {
                                return Ok(format!("(IKS SEARCH): No relevant institutional knowledge found for '{}'.", query));
                            } else {
                ...
                Err(e) => {
                    return Ok(format!("(SEARCH KNOWLEDGE FAILED: Could not acquire store: {})", e));
                }
            }
        }

//! Mission Tools — High-level task management and reporting tools
//!
...
impl AgentRunner {
    ...
    /// Handles `propose_skill`: submits a new skill or workflow proposal to the Oversight Gate.
    pub(crate) async fn handle_propose_skill(
        &self,
        ctx: &RunContext,
        fc: &crate::agent::types::GeminiFunctionCall,
        output_text: &mut String,
        ...
        if cap_type == crate::agent::types::SkillType::Skill {
        ...
        let proposal = crate::agent::types::SkillProposal {
        ...
        tracing::info!(
            "💡 [Sovereignty] Agent {} proposing a new skill: {} ({})",
            ctx.agent_id,
            name,
            cap_type_str
        );
        ...
        if approved {
            if cap_type == crate::agent::types::SkillType::Skill {
                let skill = crate::agent::script_skills::SkillDefinition {
                    id: None,
                ...
                if let Err(e) = self.state.registry.skills.save_skill(skill).await {
                ...
                let workflow = crate::agent::script_skills::WorkflowDefinition {
                    id: None,
                    name: proposal.name.clone(),
                ...
                if let Err(e) = self.state.registry.skills.save_workflow(workflow).await {
            ...
            *output_text = format!(
                "(Skill Proposal for {} REJECTED by Oversight) {}",
                name, output_text
            );
        }

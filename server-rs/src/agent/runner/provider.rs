        ...
        match result {
            Ok((text, tool_calls, usage)) => {
                ...
                self.state
                    .registry
                    .provider_health
                    .insert(provider_id, ProviderStatus::Green);
            ...
            Err(e) => {
                ...
                self.state
                    .registry
                    .provider_health
                    .insert(provider_id.clone(), new_status);
...
mod tests {
    ...
    async fn test_ollama_default_routing() {
        ...
        let variant = runner.resolve_provider(&ctx, client);

        if let ProviderVariant::OpenAI(p) = variant {

...
impl AgentRunner {
    ...
    pub(crate) fn resolve_provider(&self, ctx: &RunContext, client: reqwest::Client) -> ProviderVariant {
        tracing::info!("🔍 [Provider] Resolving provider '{}' for agent '{}'", ctx.provider_name, ctx.agent_id);
        ...
        if self.state.governance.privacy_mode.load(std::sync::atomic::Ordering::Relaxed) {
            ...
            if !p_lower.contains("ollama") && !p_lower.contains("local") {
                ...
                return ProviderVariant::Null(NullProvider::new(&ctx.agent_id, NullReason::PrivacyModeEnforced));
            }
        }
        ...
        match p_name.as_str() {
            ...
            unknown if unknown.contains("ollama") => {
                tracing::info!("📡 [Runner] Fuzzy-matched Ollama protocol for provider '{}'", unknown);
            ...
            unknown if unknown.contains("openai") || unknown.contains("groq") => {
                tracing::info!("📡 [Runner] Fuzzy-matched OpenAI protocol for provider '{}'", unknown);
                 match resolve_api_key(&ctx.model_config, "OPENAI_API_KEY") {
                    Some(key) => ProviderVariant::OpenAI(crate::agent::openai::OpenAIProvider::new(
                        client,
                        key,
                        ctx.model_config.clone(),
                    )),
                    None => {
                        ProviderVariant::Null(NullProvider::new(
                            &ctx.agent_id,
                            NullReason::MissingApiKey { env_var: "OPENAI_API_KEY" },
                        ))
                    }
                }
            }
        ...
        if limiter.is_active() {
            if let Ok((_, _, Some(ref usage))) = &result {
                ...
                self.state.governance
                    .tpm_accumulator
                    .fetch_add(usage.total_tokens as usize, std::sync::atomic::Ordering::Relaxed);
            }
        }
    ...
    pub(crate) async fn check_budget(
        ...
        if let Some(mission) =
            crate::agent::mission::get_mission_by_id(&self.state.resources.pool, &ctx.mission_id).await?
        {

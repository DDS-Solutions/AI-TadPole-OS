...
pub fn assemble_prompt_variables(
    ...
    let raw_findings = ctx.recent_findings
        .as_deref()
        .unwrap_or("No recent findings inherited.");
    let deduplicated_findings = crate::utils::deduplicator::SwarmDeduplicator::deduplicate_syntax(raw_findings);
    ...
    let deduplicated_memory = crate::utils::deduplicator::SwarmDeduplicator::deduplicate_syntax(p_ctx.memory);
    vars.insert("memory", deduplicated_memory);
    
    vars.insert("graph_context", ctx.graph_context.clone().unwrap_or_default());
    vars.insert(
        "working_memory",
        serde_json::to_string_pretty(&ctx.working_memory).unwrap_or_else(|_| "{}".to_string()),
    );
    
    let raw_history = ctx.summarized_history
        .as_deref()
        .unwrap_or("No history summarized yet.");
    let deduplicated_history = crate::utils::deduplicator::SwarmDeduplicator::deduplicate_syntax(raw_history);
    vars.insert(
        "history",
        aaak_encode(&deduplicated_history).to_string(),
    );

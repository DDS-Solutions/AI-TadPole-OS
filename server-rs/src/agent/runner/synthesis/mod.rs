...
impl super::AgentRunner {
    ...
    pub async fn generate_repo_map(
        ...
        let should_include_repo_map = is_technical_role || (payload_message.len() > 10 && contains_code_keywords);
...
mod tests {
    ...
    async fn test_generate_repo_map_heuristics() {
        ...
        let (summary, text) = runner.generate_repo_map(&ctx, "3 day weather in zipcode 19702").await;
        ...
        ctx.department = "Executive".to_string();
        ctx.role = "CEO".to_string();
        let (summary3, text3) = runner.generate_repo_map(&ctx, "fix a bug in rust files").await;

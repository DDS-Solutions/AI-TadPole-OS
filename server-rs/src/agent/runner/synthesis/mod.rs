...
pub fn assemble_prompt_variables(
    ...
    vars.insert("breadcrumbs", generate_breadcrumbs_display(ctx).to_string());
    ...
    vars.insert("history", aaak_encode(&deduplicated_history).to_string());
...
impl super::AgentRunner {
    ...
    pub async fn build_system_prompt(&self, ctx: &RunContext, payload_message: &str) -> String {
        ...
        let _global_intel_str = match self::context::gather_global_intelligence(
            ctx,
            &self.state,
            payload_message,
        )
        ...
        let role_lower = ctx.role.to_lowercase();
        let is_alpha_commander = ctx.depth <= 1 && (
            role_lower.contains("alpha")
            || role_lower.contains("commander")
            || role_lower.contains("coo")
            || role_lower.contains("ceo")
            || ctx.agent_id == "1"
            || ctx.agent_id == "2"
        );

        if is_alpha_commander {
    ...
    pub async fn generate_repo_map(
        ...
        // Strictly check if the agent role, department, or prompt context requires codebase knowledge
        ...
        let should_include_repo_map = (is_technical_role
            ...
            && !ctx.safe_mode
            // Depth guard: sub-agents at depth ≥ 2 receive targeted context from their parent.
            // Injecting the full repo map compounds token usage exponentially per swarm level.
            && ctx.depth < 2;
    ...
    pub fn shorten_system_tokens(&self, text: &str) -> String {
        text.replace("STATUS: ok", "*ok*")
            .replace("STATUS: failed", "*err*")
            .replace("STATUS: completed", "*done*")
            .replace("FINDING:", "FND:")

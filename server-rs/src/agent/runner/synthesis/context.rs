...
pub async fn gather_global_intelligence(
    ...
    #[cfg(feature = "vector-memory")]
    {
        ...
        if let Ok(ks) = _state.resources.get_knowledge_store().await {
            ...
            if let Ok(results) = ks.search(iks_req).await {
                if !results.is_empty() {
                    let iks_lines: Vec<String> = results.into_iter()
                        .map(|entry| format!("- [Topic: '{}', Conf: {:.2}] {}", entry.topic, entry.confidence, entry.text))
                        .collect();
                    intelligence_parts.push(format!(

...
///
/// ### ⚠️ Tokenizer Note
/// Uses `cl100k_base` (OpenAI/GPT-4 vocabulary). This is inaccurate for non-OpenAI
/// models (Gemma4, Llama, Mistral, etc.) which have different vocabularies and token
/// densities. The character-based hard cap below serves as a model-agnostic fallback.
pub fn prune_context(
    ...
    // ── Model-Agnostic Character Hard Cap ─────────────────────────────────────
    // cl100k_base is inaccurate for non-GPT models (Gemma4, Llama, Mistral).
    // ~4 chars per token is a conservative cross-model estimate. 120k chars ≈ 30k tokens.
    // This cap fires AFTER the token-based pruning as a final safety net.
    const CHAR_HARD_CAP: usize = 120_000;
    let total_chars = identity.len() + memory.len() + pruned_repo_map.len() + pruned_swarm_context.len();
    if total_chars > CHAR_HARD_CAP {
        tracing::warn!(
            "⚠️ [Pruning] Character hard cap triggered ({} chars > {} limit). \
             This may indicate cl100k undercount for non-GPT model. Truncating swarm context.",
            total_chars, CHAR_HARD_CAP
        );
        let overhead = identity.len() + memory.len() + pruned_repo_map.len();
        let swarm_budget = CHAR_HARD_CAP.saturating_sub(overhead).max(500);
        if pruned_swarm_context.len() > swarm_budget {
            pruned_swarm_context = safe_truncate(&pruned_swarm_context, swarm_budget);
        }
    }

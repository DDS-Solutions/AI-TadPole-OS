    ...
    pub(crate) async fn run_deterministic_workflow(
        ...
        while let Some(step) = workflow.current_step() {
            ...
            if !accumulated_results.is_empty() {
                step_prompt.push_str(&format!("### PREVIOUS STEPS FINDINGS:\n{}\n\n", accumulated_results));
            }

            step_prompt.push_str("### TASK:\nFollow the instruction above to achieve the global mission. ");
            step_prompt.push_str("DO NOT repeat work that has already been accomplished in previous steps. ");
            step_prompt.push_str("Provide a concise report of your actions and findings for this specific step.");
            ...
            match self.execute_intelligence_loop(&ctx, &step_payload).await {
                Ok(out) => {
                    ...
                    accumulated_results.push_str(&format!("\n[Step {}: {}]\n{}\n", workflow.current_step_index + 1, step.title, out.text));

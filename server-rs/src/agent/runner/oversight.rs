...
//! - **Witness Tests**: `test_e2e_oversight_approval_loop`, `test_e2e_oversight_rest_decision_database_update`
...
impl AgentRunner {
    ...
    pub async fn submit_oversight_resolution(
        ...
        // Record proposal in Merkle audit trail
    ...
    pub(crate) fn broadcast_agent_status(&self, agent_id: &str, mission_id: &str, status: &str) {
        let (task, tokens_used) = self
            ...
            .agents
            .get(agent_id)
            .map(|a| (a.state.current_task.clone(), a.economics.tokens_used))
            .unwrap_or((None, 0));
    ...
    pub(crate) fn broadcast_agent_message(
        ...
        // 3. Live Logs & Terminal broadcast (comms.tx)
    ...
    pub(crate) fn verify_mission_success(&self, observation: &str) -> bool {
        // Exclude system/sentinel injected feedback lines so sentinel rejection messages
        // cannot self-satisfy the test/success keywords on subsequent retry turns.
        let cleaned_obs = observation
            .lines()
            .filter(|line| {
                let trimmed = line.trim();
                !trimmed.starts_with("[SENTINEL GATE]:")
                    && !trimmed.contains("You must run a verification test")
            })
            .collect::<Vec<&str>>()
            .join("\n")
            .to_lowercase();
        ...
        let has_test_trigger = cleaned_obs.contains("test")
            || cleaned_obs.contains("pytest")
            || cleaned_obs.contains("cargo test");
        let has_success_indicator = cleaned_obs.contains("passed")
            || cleaned_obs.contains("success")
            || cleaned_obs.contains("ok (")
            || cleaned_obs.contains("test result: ok");

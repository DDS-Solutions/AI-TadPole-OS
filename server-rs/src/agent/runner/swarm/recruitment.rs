...
use crate::agent::runner::{AgentRunner, swarm::SubAgentOptions};
...
impl AgentRunner {
    ...
    pub(crate) async fn ensure_sub_agent_exists(
        ...
        if let Err(missing) = crate::security::dependency_guard::check_skill_dependencies(&target_skills) {
            let err_msg = format!("Dependency check failed for recruited agent '{}': {:?}", target_id, missing);

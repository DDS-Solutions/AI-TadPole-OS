...
impl AgentRunner {
    ...
    pub(crate) async fn generate_conductor_plan(
        ...
        tracing::info!("🧠 [Conductor] Generating structured execution plan (DAG) for mission: {}", ctx.mission_id);
        ...
        let (plan_text, _, _) = self.call_provider(ctx, system_prompt, &user_message, None).await?;

        // Extract JSON string robustly by isolating everything between the first '{' and last '}'
        let clean_json = plan_text.trim();
        let plan_json_to_parse = if let (Some(start_idx), Some(end_idx)) = (clean_json.find('{'), clean_json.rfind('}')) {
        ...
        // Parse JSON output
        let plan: ConductorPlan = serde_json::from_str(plan_json_to_parse)
            .map_err(|e| AppError::InfrastructureError {
                ...
                detail: format!("Failed to parse Conductor plan JSON: {}. Response excerpt: {}", e, plan_json_to_parse),
                help_link: None,
            })?;
        ...
        Ok(ConductorPlan { steps: sorted_steps })
    }
    ...
    pub(crate) fn topological_sort_conductor_steps(
        ...
        let step_map: std::collections::HashMap<u32, &ConductorStep> = steps
            .iter()
            .map(|s| (s.step_id, s))
            .collect();
        ...
        for step in steps {
            if !visited.contains(&step.step_id) {
                ...
                visit(step.step_id, &step_map, &mut visited, &mut temp_set, &mut sorted)
                    .map_err(|e| AppError::InfrastructureError {
                        provider_id: crate::error::ProviderId::Runner,
                        kind: crate::error::InfrastructureErrorKind::Other,

...
//! - **Subsystem**: Sovereign Engine / Agent Runner / turn
//!
//! ### ⚠️ Invariants & Non-Negotiables
//! - `[Structural]` Tool execution steps are emitted with reasoning step's `current_step_id` as their `parent_id`.
//! - `[Behavioral]` Imperative action requests are strictly excluded from fast-path query execution.
//!   - enforced_by: `test_is_fast_path_query_heuristics`
//! - `[Behavioral]` Internal monologue compression respects `active_ctx` local model overrides.
//!   - enforced_by: `test_compress_monologue`
//!
//! ### 🔍 Debugging & Observability
//! - **Local Errors**: none
//! - **Telemetry Targets**: `[Intelligence]`
//! - **Witness Tests**: `test_is_fast_path_query_heuristics`, `test_compress_monologue`
...
impl AgentRunner {
    pub(crate) async fn handle_specification_generation_phase(
        ...
        let mut usage = ctx
            .planning_usage
            .clone()
            .or_else(|| Some(crate::agent::types::TokenUsage::default()));
        ...
        state_tx.record_mission_spec_change(&ctx.mission_id, &ctx.agent_id, None);
    ...
    pub(crate) fn is_fast_path_query(message: &str) -> bool {
        ...
        // Clear inquiries/explanations where complex words are referenced rather than commanded
        let is_inquiry = lower.starts_with("what is")
            ...
            || lower.starts_with("could you explain");

        // Informational inquiries under 150 chars are fast-path unless explicitly commanded
        if is_short && is_inquiry {
            return true;
        }

        static SIMPLE_TRIGGERS: &[&str] = &[
            "what is",
            "get status",
            "read file",
            "list files",
        ...
        // Tokenize words to prevent substring collisions (e.g. "address" matching "add")
        let words: Vec<&str> = lower
            .split(|c: char| !c.is_alphanumeric() && c != '_' && c != '-')
            .filter(|s| !s.is_empty())
            .collect();

        static COMPLEX_WORDS: &[&str] = &[
            "implement",
            "build",
            "refactor",
            "debug",
        ...
        static COMPLEX_PHRASES: &[&str] = &["run tests", "stress test"];
        ...
        // Short read-only / interrogative queries without complex action words
        let is_interrogative = lower.starts_with("why")
    ...
    pub(crate) async fn handle_reasoning_phase(
        ...
        // 🛠️ [Task 1.1: OpenToolCallCloser] Close any dangling tool calls from interrupted prior sessions
        crate::agent::runner::context::close_unresolved_tool_calls(&mut conversation_history);
        ...
        while turn_count < max_turns {
                ...
                .await?
            {
                ...
                TurnOutcome::Completed(out) => return Ok(out),
                TurnOutcome::BudgetExceeded(mut out) => {
                    tracing::warn!(
                        "💰 [Intelligence] Reasoning halted due to budget limit for agent {}",
                        ctx.agent_id
                    );
                    if !out.text.contains("Budget Exceeded") {
                        out.text = format!(
                            "{}\n\n> ⚠️ [Budget Exceeded] Execution halted due to financial budget cap.",
                            out.text
                        );
                    }
                    return Ok(out);
                ...
                .unwrap_or(false)
        {
            ...
            match self
                ...
                .await
            {
                Ok((turn_text, _, turn_usage)) => {
                    self.accumulate_usage(&mut usage, turn_usage);
                    output_text = scrub_mythos_tags(&turn_text);
                }
                Err(e) => {
                    tracing::warn!(
                        "⚠️ [Intelligence] Final conversational recovery turn failed for agent {}: {:?}",
                        ctx.agent_id,
                        e
                    );
                }
    ...
    pub(crate) async fn run_agent_turn(
        ...
        let base_prompt = conversation_history.join("\n\n");
        ...
        while reasoning_turn < ctx.reasoning_depth && !reasoning_halted {
            ...
            // --- 📊 [AI-Tadpole-OS Rollup] Record turn execution metrics ---
            ctx.execution_metrics.record_turn();
            if let Some(ref u) = turn_usage {
                ctx.execution_metrics.record_tokens(
                    u.input_tokens,
                    u.output_tokens,
                    u.input_tokens + u.output_tokens,
                );
                let turn_cost = crate::agent::rates::calculate_cost_with_provider(
                    Some(&active_ctx.model_config.provider.to_string()),
                    &active_ctx.model_config.model_id,
                    u.input_tokens,
                    u.output_tokens,
                );
                ctx.execution_metrics.record_cost(turn_cost);
            }
            let metrics_snap = ctx
                .execution_metrics
                .snapshot(&ctx.mission_id, &ctx.agent_id);
            let _ = self.state.comms.telemetry_tx.send(serde_json::json!({
                "type": "agent:execution_metrics",
            ...
            // Gap 1: Emit a dedicated tool-step event after tools are identified (as child of reasoning step)
            if !function_calls.is_empty() {
                let tool_names: Vec<String> =
                    function_calls.iter().map(|fc| fc.name.clone()).collect();
                self.broadcast_tool_step(ctx, Some(current_step_id.clone()), tool_names);
            }
            ...
            *parent_id = Some(current_step_id);
            ...
            if reasoning_turn < ctx.reasoning_depth && !reasoning_halted {
                ...
                // [Context Hygiene] Summarize monologue if it grows too large (respecting active_ctx privacy overrides)
                // Fix 13: Log compression failures instead of silently swallowing
                if let Err(e) = self
                    .compress_monologue(active_ctx, &mut internal_monologue)
                    .await
                {
                    tracing::warn!(
                ...
                if !orch_res.observation_buffer.is_empty() {
                    let (compacted_obs, _) =
                        crate::agent::runner::turn_compactor::compact_and_offload_observation(
                            "tool",
                            &orch_res.observation_buffer,
                            &ctx.workspace_root,
                        )
                        .await;
                    conversation_history.push(compacted_obs);
                }
    ...
    pub(crate) fn broadcast_reasoning_step(
        ...
        let cost_usd = crate::agent::rates::calculate_cost_with_provider(
            Some(&ctx.model_config.provider.to_string()),
    ...
    fn activate_model_slot(
        ...
        let selection = self.model_router.select_model_slot(
            &self.state,
            &ctx.agent_models,
            preferred_kind,

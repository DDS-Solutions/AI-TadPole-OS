...
//! - **Witness Tests**: `test_router_economic_zone_and_limits`, `test_unconfigured_agent_default_spend_limit`
...
impl A2APaymentAdapter for LocalMockAdapter {
    async fn transfer_funds(
        ...
        let tx_id =
            A2ATransactionCoordinator::commit_transaction(&mut tx, &lock_id, signing_key, None)
                .await?;
...
impl A2APaymentAdapter for L3HybridAdapter {
    async fn transfer_funds(
        ...
        if let Address::Web3(ref wallet_addr) = to {
            ...
            let _ = A2ATransactionCoordinator::rollback_transaction(&mut tx, &lock_id, None).await;

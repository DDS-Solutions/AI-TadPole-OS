...
mod tests {
    ...
    async fn test_mailbox_flow() {
        ...
        mailbox.send_envelope(&env, None).await.unwrap();
    ...
    async fn test_mailbox_ssrf_metadata_rejected() {
        ...
        let res = mailbox.send_envelope(&env, None).await;
        assert!(res.is_err());
    ...
    async fn test_ledger_prepare_commit_flow() {
        ...
        let mut tx = pool.begin().await.unwrap();
        let tx_id = A2ATransactionCoordinator::commit_transaction(&mut tx, &lock_id, None, None)
            .await
            .unwrap();
        tx.commit().await.unwrap();
    ...
    async fn test_ledger_prepare_rollback_flow() {
        ...
        let mut tx = pool.begin().await.unwrap();
        A2ATransactionCoordinator::rollback_transaction(&mut tx, &lock_id, None)
            .await
            .unwrap();
        tx.commit().await.unwrap();
    ...
    async fn test_commit_expired_lock_rejected() {
        ...
        let commit_res =
            A2ATransactionCoordinator::commit_transaction(&mut tx_commit, &lock_id, None, None)
                .await;
    ...
    async fn test_a2a_mailbox_replay_on_completed_directive_is_noop_and_warns() {
        ...
        mailbox.send_envelope(&initial_env, None).await.unwrap();
        ...
        mailbox.send_envelope(&replay_env, None).await.unwrap();
    ...
    async fn test_a2a_mailbox_replay_on_pending_directive_is_idempotent() {
        ...
        mailbox.send_envelope(&updated_env, None).await.unwrap();
    ...
    async fn test_a2a_mailbox_chokepoint_bounds_and_ssrf_validation() {
        ...
        let res = mailbox.send_envelope(&oversized_instruction, None).await;
        assert!(res.is_err());
        assert!(res
            .unwrap_err()
            .to_string()
        ...
        let res = mailbox.send_envelope(&oversized_trace, None).await;
        assert!(res.is_err());
        assert!(res
            .unwrap_err()
            .to_string()
        ...
        let res = mailbox.send_envelope(&too_many_artifacts, None).await;
        assert!(res.is_err());
        assert!(res
            .unwrap_err()
            .to_string()
        ...
        let res = mailbox.send_envelope(&oversized_artifact_item, None).await;
        assert!(res.is_err());
        assert!(res
            .unwrap_err()
            .to_string()
    ...
    #[tokio::test]
    async fn test_a2a_merkle_audit_chaining() {
        let pool = setup_test_db().await;
        let audit = crate::security::audit::MerkleAuditTrail::mock_async()
            .await
            .unwrap();

        // 1. Fund buyer
        sqlx::query("INSERT INTO agent_balances (agent_id, asset_type, balance) VALUES ('local:buyer_audit', 'USDC', 1000)")
            .execute(&pool)
            .await
            .unwrap();

        let buyer = Address::Local("buyer_audit".to_string());
        let seller = Address::Local("seller_audit".to_string());

        // 2. Prepare and commit with audit trail
        let mut tx1 = pool.begin().await.unwrap();
        let lock_id1 = A2ATransactionCoordinator::prepare_transaction(
            &mut tx1, &buyer, &seller, 300, None, None, None,
        )
        .await
        .unwrap();
        let tx_id1 =
            A2ATransactionCoordinator::commit_transaction(&mut tx1, &lock_id1, None, Some(&audit))
                .await
                .unwrap();
        tx1.commit().await.unwrap();
        assert!(!tx_id1.is_empty());

        // 3. Prepare and rollback with audit trail
        let mut tx2 = pool.begin().await.unwrap();
        let lock_id2 = A2ATransactionCoordinator::prepare_transaction(
            &mut tx2, &buyer, &seller, 200, None, None, None,
        )
        .await
        .unwrap();
        A2ATransactionCoordinator::rollback_transaction(&mut tx2, &lock_id2, Some(&audit))
            .await
            .unwrap();
        tx2.commit().await.unwrap();

        // 4. Verify chain integrity across both transactions
        let chain_valid = audit.verify_chain(None).await.unwrap();
        assert!(
            chain_valid,
            "Merkle audit chain must be cryptographically valid"
        );

        // 5. Verify [A2A_COMMIT] and [A2A_ROLLBACK] records exist
        let (verified, total) = audit.verify_last_n(10, None).await.unwrap();
        assert_eq!(verified, total);
        assert_eq!(total, 2);
    }

    #[tokio::test]
    async fn test_a2a_mailbox_merkle_audit_chaining() {
        let pool = setup_test_db().await;
        let mailbox = A2AMailbox::new(pool);
        let audit = crate::security::audit::MerkleAuditTrail::mock_async()
            .await
        ...
        let env = MailboxEnvelope {
            id: "msg-audit-1".to_string(),
            mission_id: "mission-audit".to_string(),
            source_agent_id: "agent-src".to_string(),
            target_agent_id: "agent-tgt".to_string(),
            instruction: "Compute cryptographic proof".to_string(),
            reasoning_trace: Some("Tracing steps".to_string()),
            status: "pending".to_string(),
            result: None,
            artifacts: None,
            timestamp: None,
        ...
        // Send envelope with audit trail
        mailbox.send_envelope(&env, Some(&audit)).await.unwrap();
        ...
        // Verify [A2A_DISPATCH] record exists
        let (verified, total) = audit.verify_last_n(10, None).await.unwrap();
        assert_eq!(verified, total);
        assert_eq!(total, 1);
    }

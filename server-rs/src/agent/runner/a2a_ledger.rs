...
//! - **Witness Tests**: `test_ledger_prepare_commit_flow`, `test_ledger_prepare_rollback_flow`, `test_self_transfer_and_zero_amount_rejected`, `test_commit_expired_lock_rejected`, `test_a2a_merkle_audit_chaining`
...
impl A2ATransactionCoordinator {
    ...
    pub async fn commit_transaction(
        ...
        let (transaction_id, seller_str, buyer_str, amount, expires_at) = match lock_opt {
            Some(l) => l,
            None => {
        ...
        // 6. Chain into Merkle Audit Trail
        if let Some(audit) = audit_trail {
            ...
            .to_string();
            if let Err(e) = audit
                .record(&buyer_str, None, None, "[A2A_COMMIT]", &params)
                .await
            {
                tracing::error!("🚨 Failed to record [A2A_COMMIT] in audit trail: {:?}", e);
            }
        }
    ...
    pub async fn rollback_transaction(
        ...
        // 5. Chain into Merkle Audit Trail
        if let Some(audit) = audit_trail {
            let params = serde_json::json!({
                "transaction_id": &transaction_id,
                "lock_id": lock_id,
            ...
            .to_string();
            if let Err(e) = audit
                .record(&buyer_str, None, None, "[A2A_ROLLBACK]", &params)
                .await
            {
                tracing::error!("🚨 Failed to record [A2A_ROLLBACK] in audit trail: {:?}", e);
            }
        }
    ...
    pub async fn sweep_expired_locks(pool: &sqlx::SqlitePool) -> Result<u32, AppError> {
        ...
        for (lock_id,) in expired_locks {
            ...
            match Self::rollback_transaction(&mut db_tx, &lock_id, None).await {
                Ok(_) => {
                    db_tx.commit().await?;
                    swept_count += 1;
                }

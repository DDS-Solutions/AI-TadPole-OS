...
/// Triggers a post-mission debrief and behavioral audit.
///
/// This is spawned in a separate thread to avoid blocking the main runner execution loop.
/// It uses Agent 99 (QA) to analyze the mission logs and output.
pub(crate) fn spawn_post_mission_analysis(
    ...
    let (cluster_name, agent_memory_dir, mission_scope_dir) = ctx.resolve_paths();
    ...
    std::thread::spawn(move || {
        ...
        rt.block_on(async move {
            ...
            if let Ok(action_vec) = crate::agent::memory::get_gemini_embedding(&runner.state.resources.http_client, &api_key, &final_output).await {
                if let Ok(identity_vec) = crate::agent::memory::get_gemini_embedding(&runner.state.resources.http_client, &api_key, &identity_context).await {
                    ...
                    if dist > drift_threshold {
                        runner.state.broadcast_sys(&format!("🚨 Behavioral Drift Detected for Agent {}! Actions diverged from core identity (distance: {:.2}).", ctx_agent_id, dist), "error");
                    }
                }
            }
            ...
            let payload = TaskPayload {
                message: "Is this mission successful? Respond with 'Mission Successful' if so.".to_string(),

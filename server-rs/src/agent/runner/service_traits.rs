...
//! - **Witness Tests**: `test_identity_service`, `test_state_transaction_rollback`, `test_truncate_observation_boundaries`, `test_sanitize_observation_content`, `test_classify_failure`, `test_offload_large_tool_response_small_unchanged`
...
/// 1. If failure, truncates to 500 characters.
/// 2. If individual output exceeds 6,000 tokens (~24,000 chars) or turn total exceeds 10,000 tokens (~40,000 chars):
///    - Dumps full payload to `.tmp/tool_overflow/tool_output_{tool}_{timestamp}.txt`.
...
pub fn offload_large_tool_response(
    ...
    if is_failure {
        if text.len() > FAILURE_MESSAGE_TRUNCATION_LENGTH {
            let boundary = text.floor_char_boundary(FAILURE_MESSAGE_TRUNCATION_LENGTH);
            return format!(
                "{}... [Failure message truncated from {} chars]",
                &text[..boundary],
                text.len()
            );
        }
    ...
    let sanitized_tool: String = tool_name
        ...
        .collect();
    let file_name = format!("tool_output_{}_{}.txt", sanitized_tool, timestamp);
...
impl ToolOrchestrator for DefaultToolOrchestrator {
    async fn execute_tools(
            ...
            futures.next().await
        {
            ...
            local_text = offload_large_tool_response(
                &name,
                &local_text,
                is_failure,

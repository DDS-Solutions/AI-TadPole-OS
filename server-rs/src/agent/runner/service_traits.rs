...
impl ModelRouter for DefaultModelRouter {
    fn select_model_slot(
        ...
        let privacy_mode_active = state
            .governance
            .is_privacy_mode_enabled(cluster_id);
...
impl ToolOrchestrator for DefaultToolOrchestrator {
    async fn execute_tools(
        ...
        // from missions that end via budget breach, max turn exhaustion, or early return (Audit 1.4)
        {
            ...
            let cutoff = std::time::Instant::now() - std::time::Duration::from_secs(LOOP_DETECTOR_TTL_SECS);
        ...
        for fc in function_calls {
            ...
            let fc_input_summary = fc.args.get("path")
                .or_else(|| fc.args.get("query"))
                .or_else(|| fc.args.get("command"))
                .or_else(|| fc.args.get("url"))
                .and_then(|v| v.as_str())
                .map(|s| s.chars().take(TOOL_INPUT_SUMMARY_MAX_LEN).collect::<String>())
                .unwrap_or_default();
            futures.push(async move {
                ...
                let timeout_secs = executor_clone.get_provider_timeout_secs();
                let tool_timeout = std::time::Duration::from_secs(timeout_secs.max(300));
                let (local_text, local_usage, success) = match tokio::time::timeout(
                    tool_timeout,
                    executor_clone.execute_tool(&ctx_clone, &fc, &user_msg_clone)
                ).await {
                    Ok(Ok((text, usage))) => (text, usage, true),
                    Ok(Err(e)) => (format!("(TOOL FAILURE: {:?})", e), None, false),
                    Err(_) => (format!("(TOOL TIMEOUT: Tool execution timed out after {} seconds)", tool_timeout.as_secs()), None, false),
                };
            ...
            futures.next().await
        {
            ...
            let has_loop = {
                ...
                detector.check(&active_ctx.agent_id, &name, &args_str, &local_text)
            };
            ...
            if local_text.len() > MAX_TOOL_OUTPUT_CHARS {
                let original_len = local_text.len();
                let mut boundary = local_text.floor_char_boundary(MAX_TOOL_OUTPUT_CHARS);
            ...
            let is_tool_yielding_runtime_error = matches!(name.as_str(), "execute_shell" | "cargo_test" | "cargo_build");
            let is_failure = !success
                || local_text.contains("(TOOL FAILURE:")
                || (is_tool_yielding_runtime_error && (

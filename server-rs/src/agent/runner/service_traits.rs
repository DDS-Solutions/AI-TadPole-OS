...
impl AgentMissionState {
    ...
    pub fn resolve(spec: &str, safe_mode: bool) -> Self {
        if safe_mode {
...
pub trait WorkflowCoordinator: Send + Sync {
    ...
    ) -> Result<super::IntelligenceOutput, (crate::error::AppError, Option<crate::agent::types::TokenUsage>)>;
}

#[async_trait::async_trait]
#[allow(dead_code)]
pub trait MissionStateManager: Send + Sync {
    async fn yield_phase_transition(&self, state: &crate::state::AppState, agent_id: &str, phase: &str);
    fn update_status(
        &self,
        state: &crate::state::AppState,
        agent_id: &str,
...
pub struct DefaultToolOrchestrator {
    loop_detectors: parking_lot::Mutex<std::collections::HashMap<String, (super::intelligence::DoomLoopDetector, std::time::Instant)>>,
}
    ...
    async fn execute_tools(
        ...
        for fc in function_calls {
            ...
            futures.push(async move {
                ...
                executor_clone.handle_tool_failure_refinement(&ctx_clone, &fc, &mut local_text_refined);

                (fc_name, fc_args_str, success, local_text_refined, local_usage)
            });
        }
            ...
            futures.next().await
        {
            ...
            let has_loop = {
                let mut guard = self.loop_detectors.lock();
                let (detector, _) = guard
                    .entry(active_ctx.mission_id.clone())
                    .or_insert_with(|| (super::intelligence::DoomLoopDetector::new(), std::time::Instant::now()));
            ...
            if let Some(ref vt) = active_ctx.visible_transcript {
                ...
                vt.lock().push(format!("OBSERVATION (Tool {}): {}", name, clean_obs));
            }
            ...
            if name == "complete_mission" {
                ...
                if !executor.verify_mission_success(&observation_buffer) && !active_ctx.safe_mode {
                    executor.broadcast_agent(active_ctx, "🚨 Sentinel Gate: Finalization BLOCKED. No proof of verification found.", "warning");
...
impl MissionStateManager for DefaultMissionStateManager {
    ...
    async fn set_mission_spec(
        ...
        crate::agent::mission::set_mission_spec(&state.resources.pool, mission_id, agent_id, spec_content).await
    }
}
...
impl UndoOp {
    pub async fn execute(&self, state: &crate::state::AppState) -> Result<(), crate::error::AppError> {
        match self {
            UndoOp::RestoreAgentRegistry { agent_id, status, task, active_mission } => {
                if let Some(mut entry) = state.registry.agents.get_mut(agent_id) {
                    let agent = entry.value_mut();
            ...
            UndoOp::DeleteMissionSpec { mission_id, agent_id } => {
...
impl StateTransaction {
    pub fn new(
        ...
        let (original_status, original_task, original_active_mission) = if let Some(entry) = state.registry.agents.get(agent_id) {
            let agent = entry.value();
            (agent.health.status.clone(), agent.state.current_task.clone(), agent.state.active_mission.clone())
        } else {
    ...
    #[allow(dead_code)]
    pub fn record_agent_status_change(&mut self, _agent_id: &str, _status: &str, _task: Option<&str>) {
    ...
    #[allow(dead_code)]
    pub fn record_mission_status_change(&mut self, mission_id: &str, status: crate::agent::types::MissionStatus) {
            ...
            tokio::spawn(async move {
                ...
                for op in ops.into_iter().rev() {
                    if let Err(e) = op.execute(&state).await {
                        tracing::error!("❌ [StateTransaction] Implicit rollback operation failed: {:?}", e);
                    }
                }
            });
        }
...
mod tests {
    ...
    async fn test_identity_service() {
        assert!(IdentityService::is_orchestrator(crate::agent::constants::AGENT_CEO));
        assert!(IdentityService::is_orchestrator(crate::agent::constants::AGENT_COO));
        assert!(IdentityService::is_orchestrator(crate::agent::constants::AGENT_ALPHA));
    ...
    async fn test_state_transaction_rollback() {
...

        {
            ...
            assert_eq!(entry.value().state.current_task.as_deref(), Some("Running..."));

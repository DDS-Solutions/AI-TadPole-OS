...
impl AgentRunner {
    ...
    pub(crate) async fn initialize_mission_state(
        ...
        if let Some(mut entry) = self.state.registry.agents.get_mut(agent_id) {
            ...
            let _ =
                crate::agent::persistence::save_agent_db(&self.state.resources.pool, agent)
                    .await;
        }
        ...
        let mission = if let Some(ref custom_id) = payload.cluster_id {
            if let Some(existing) = crate::agent::mission::get_mission_by_id(&self.state.resources.pool, custom_id).await? {
                existing
            } else {

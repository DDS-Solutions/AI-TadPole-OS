...
impl AgentRunner {
    ...
    pub(crate) async fn handle_fetch_url(
        ...
        if let Err(e) = crate::utils::security::validate_public_http_url(url).await {
            return Ok(format!("(FETCH BLOCKED: {})", e));
        }
    ...
    pub(crate) async fn handle_search_web(
            ...
            .await
        {
            Ok(r) => {
                ...
                let re =
                    regex::Regex::new(r#"class="result__snippet"[^>]*>([^<]+)</span>"#).unwrap();

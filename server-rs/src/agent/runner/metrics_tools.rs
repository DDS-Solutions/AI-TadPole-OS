...
impl AgentRunner {
    ...
    pub(crate) async fn handle_get_agent_metrics(
        ...
        tracing::info!("📊 [Governance] Agent {} fetching live metrics...", agent_id);

        // Fetch live data from the in-memory registry for maximum accuracy
        let (budget, cost, name, role, department) = if let Some(a) = self.state.registry.agents.get(agent_id) {
            let v = a.value();
            (v.budget_usd, v.cost_usd, v.name.clone(), v.role.clone(), v.department.clone())
        } else {
            // Fallback to RunContext if registry lookup fails (though this shouldn't happen during a valid run)
            (0.0, 0.0, ctx.name.clone(), ctx.role.clone(), ctx.department.clone())
        };
        ...
        *output_text = format!("(AGENT METRICS RETRIEVED):\n\n{}", serde_json::to_string_pretty(&metrics)?);

        self.broadcast_agent(
            ctx,
            &format!("📊 Governance: reviewed live metrics (${:.4} / ${:.2})", cost, budget),
            "info",
        );

...
//! - **Witness Tests**: `test_metrics_budget_status_boundary`
...
impl AgentRunner {
    ...
    pub(crate) async fn handle_get_agent_metrics(
        ...
        let exec_snap = ctx.execution_metrics.snapshot(&ctx.mission_id, agent_id);
...
#[cfg(test)]
mod tests {
    /// Verifies the inline budget status boundary: cost >= budget => BREACHED, else OK.
    #[test]
    fn test_metrics_budget_status_boundary() {
        let check_status = |cost: f64, budget: f64| -> &'static str {
            if cost >= budget {
                "BREACHED"
            } else {
                "OK"
        ...
        assert_eq!(check_status(0.0, 100.0), "OK");
        assert_eq!(check_status(99.99, 100.0), "OK");
        assert_eq!(check_status(100.0, 100.0), "BREACHED"); // Boundary: cost == budget
        assert_eq!(check_status(100.01, 100.0), "BREACHED");
        // Zero budget: any cost is a breach
        assert_eq!(check_status(0.0, 0.0), "BREACHED");
    }
}

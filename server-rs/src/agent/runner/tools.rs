...
impl AgentRunner {
    ...
    pub(crate) async fn execute_tool(
        ...
        let policy_mode = self.state.security.permission_policy.get_mode(&fc.name).await;
        match policy_mode {
            crate::security::permissions::PermissionMode::Deny => {
                *output_text = format!("(TOOL EXECUTION BLOCKED: Policy for '{}' is set to DENY)", fc.name);
                return Ok(Some(output_text.clone()));
            }
...
mod tests {
    ...
    async fn test_propose_skill_validation_rejection() {
        let state = Arc::new(crate::state::AppState::new().await.expect("Test state init failed"));

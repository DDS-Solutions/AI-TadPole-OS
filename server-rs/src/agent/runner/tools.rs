...
impl AgentRunner {
    ...
    /// 
    /// ### 🛡️ Security Protocol (Multi-Layer)
    /// 1. **CBS (Capability-Based Security)**: Verifies if the tool is in the agent's allowlist.
    /// 2. **Hierarchy Guard**: Prevents strategic nodes (CEO/COO) from direct tactical recruitment.
    /// 3. **Policy Gate**: Checks against the dynamic `PermissionPolicy` (Allow/Prompt/Deny).
    /// 4. **Oversight Gate**: Blocks and requests manual approval for sensitive "Prompt" tools.
    /// 5. **Audit Gate**: Spawns an async record in the Merkle Hash Chain before execution.
    #[tracing::instrument(
        name = "execute_tool",
    ...
    pub(crate) async fn execute_tool(
            ...
            let approved = self
                .submit_oversight(
                    crate::agent::types::ToolCallAudit {
                        ...
                        description: oversight_reason.clone(),
        ...
        let result: anyhow::Result<Option<String>> = match fc.name.as_str() {
            ...
            "pin_mission" => {
                self.handle_pin_mission(ctx, fc, output_text).await?;
                Ok(None)
            }
            "set_confidence" => {
                *output_text = "(CONFIDENCE LEVEL PERSISTED)".to_string();
                Ok(None)
            }
            "script_builder" => {
...
mod tests {
    ...
    async fn test_propose_skill_validation_rejection() {
        ...
        let fc_workflow = crate::agent::types::ToolCall {

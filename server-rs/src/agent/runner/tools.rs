...
//! and validated against Capability-Based Security (CBS) policies.
...
impl AgentRunner {
    ...
    pub(crate) async fn execute_tool(
        ...
        // 🌐 [CBS] Capability-Based Security Allowlist
        if let Some(agent) = self.state.registry.agents.get(&ctx.agent_id) {
            let allowed_skills = &agent.value().skills;
            let is_builtin = matches!(fc.name.as_str(),
                "spawn_subagent" | "issue_alpha_directive" | "share_finding" | "query_financial_logs" |
                "complete_mission" | "propose_capability" | "archive_to_vault" | "notify_discord" |
                "fetch_url" | "read_file" | "write_file" | "list_files" | "delete_file" | "search_mission_knowledge" | "read_codebase_file" |
            ...
            if !is_builtin && !allowed_skills.is_empty() && !allowed_skills.contains(&fc.name) {
                ...
                self.state.broadcast_sys(&format!("🛡️ CBS: {} attempted unauthorized skill: {}", ctx.name, fc.name), "error");
                *output_text = format!("(TOOL EXECUTION BLOCKED: Skill '{}' not in agent allowlist)", fc.name);
            ...
            if matches!(fc.name.as_str(), "spawn_subagent" | "recruit_specialist") {
                if ctx.agent_id == "1" {
                    ...
                    self.state.broadcast_sys("🛡️ Hierarchy Guard: CEO blocked from direct worker recruitment. Use 'issue_alpha_directive' instead.", "warning");
                ...
                if ctx.agent_id == "2" {
                    let target = fc.args.get("agent_id").or_else(|| fc.args.get("agentId")).and_then(|v| v.as_str()).unwrap_or("");
                    if target != "alpha" {
                        tracing::warn!("🛡️ [Hierarchy Guard] COO (ID: 2) blocked from spawning specialist '{}' directly.", target);
                        self.state.broadcast_sys("🛡️ Hierarchy Guard: COO blocked from direct worker recruitment. Use Alpha Node commander instead.", "warning");
                ...
                let approved = self
                    .submit_oversight(
                        crate::agent::types::ToolCall {
                            ...
                            description: format!(
                                "Security Gate triggered by manifest for: {}",
                                fc.name
                            ),
        ...
        let audit_params = self.state.security.secret_redactor.redact(&serde_json::to_string(&fc.args).unwrap_or_default());
        ...
        tokio::spawn(async move {
            let _ = audit_trail.record(&agent_id, Some(&mission_id), user_id.as_deref(), &action, &audit_params).await;
        });

        // 📋 [Proactive Security] Shell Safety Scanner (Script Analysis)
        let args_str = serde_json::to_string(&fc.args).unwrap_or_default();
        if let crate::security::scanner::ScannerResult::Risky(reason) = self.state.security.shell_scanner.scan(&args_str) {
            tracing::warn!("🛡️ [Security] Blocked risky tool call for '{}': {}", fc.name, reason);
            self.state.broadcast_sys(&format!("🛡️ Security: Blocked risky tool call for '{}': {}", fc.name, reason), "error");
        ...
        let result: anyhow::Result<Option<String>> = match fc.name.as_str() {
            ...
            "search_mission_knowledge" => {
                self.handle_search_mission_knowledge(ctx, fc, output_text).await?;
                Ok(None)
            }
            "read_codebase_file" => {
            ...
            _ => {
                ...
                if let Some(dynamic_skill) = self.state.registry.capabilities.skills.get(&fc.name) {
                    self.handle_dynamic_skill(ctx, fc, output_text, &dynamic_skill, usage).await?;
                    Ok(None)
                } else {
                    tracing::warn!("⚠️ [Protocol] Unknown tool execution attempted: {}", fc.name);
    ...
    async fn handle_dynamic_skill(
        ...
        let result = self
            ...
            .call_tool(
                ...
                &self.state.registry.capabilities.skills,
            )
            .await;
        ...
        match result {
            Ok(crate::agent::mcp::McpResult::Raw(output)) => {
                ...
                if let Some(verify_script) = &skill.verification_script {
                    match self.run_verification_script(verify_script, &skill.name, &fc.args, &final_output, &ctx.workspace_root).await {
                        Ok(verify_res) => {
                ...
                *output_text = format!("({} EXECUTED SUCCESSFULLY):\n\n{}", skill.name, final_output);
            }
            Ok(crate::agent::mcp::McpResult::SystemDelegate(name, args)) => {
                if name == "recruit_specialist" {
                    let mut mapped_args = serde_json::Map::new();
                    if let Some(aid) = args.get("agent_id") { mapped_args.insert("agentId".to_string(), aid.clone()); }
                    if let Some(msg) = args.get("task_description") { mapped_args.insert("message".to_string(), msg.clone()); }
                    
                    let mapped_fc = crate::agent::types::GeminiFunctionCall {
                        name: "spawn_subagent".to_string(),
                        args: serde_json::Value::Object(mapped_args),
                    };
                    self.handle_spawn_subagent(ctx, &mapped_fc, output_text, _usage).await?;
                }
            }
            Err(e) => {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_propose_capability_validation_rejection() {

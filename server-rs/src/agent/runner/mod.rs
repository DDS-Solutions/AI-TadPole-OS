...
/// Record of an OS process execution witnessing command status.
...
pub(crate) struct RunContext {
    ...
    pub execution_records: std::sync::Arc<parking_lot::Mutex<Vec<ProcessExecutionRecord>>>,
    pub last_mutation_timestamp_ms: std::sync::Arc<std::sync::atomic::AtomicU64>,
    ...
    pub execution_metrics: Arc<execution_metrics::ExecutionMetrics>,
}
...
impl Default for RunContext {
    fn default() -> Self {
        ...
        Self {
            ...
            execution_metrics: Arc::new(execution_metrics::ExecutionMetrics::new()),
        }
    }
}
...
pub struct AgentRunner {
    ...
    pub model_router: service_traits::DefaultModelRouter,
    pub prompt_service: service_traits::DefaultPromptService,
    pub tool_orchestrator: Arc<service_traits::DefaultToolOrchestrator>,
    #[allow(dead_code)]
    pub mission_state_manager: Arc<dyn service_traits::MissionStateManager>,
    pub workflow_coordinator: Arc<intelligence::MissionWorkflowCoordinator>,
}
...
impl AgentRunner {
    pub fn new(state: Arc<AppState>) -> Self {
        let prompt_service_dyn = Arc::new(service_traits::DefaultPromptService);
        let mission_state_manager: Arc<dyn service_traits::MissionStateManager> =
            Arc::new(service_traits::DefaultMissionStateManager);
        let workflow_coordinator = Arc::new(intelligence::MissionWorkflowCoordinator {
            state: state.clone(),
            prompt_service: prompt_service_dyn,
            mission_state_manager: mission_state_manager.clone(),
        });

        Self {
            state,
            model_router: service_traits::DefaultModelRouter,
            prompt_service: service_traits::DefaultPromptService,
    ...
    pub(crate) fn effective_privacy_scope(ctx: &RunContext) -> Option<&str> {

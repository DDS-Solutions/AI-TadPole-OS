...
impl RunContext {
    ...
    pub fn resolve_paths(&self) -> (String, String, String) {
        let cluster_name = self.workspace_root.file_name().unwrap_or_default().to_string_lossy().to_string();
        let agent_memory_dir = self.base_dir.join("data/workspaces").join(&cluster_name).join("agents").join(&self.agent_id).join("memory.lance").to_string_lossy().to_string();
        let mission_scope_dir = self.base_dir.join("data/workspaces").join(&cluster_name).join("missions").join(&self.mission_id).join("scope.lance").to_string_lossy().to_string();
...
impl AgentRunner {
    ...
    pub(crate) fn broadcast_agent(&self, ctx: &RunContext, msg: &str, level: &str) {
        self.state.broadcast_agent(msg, level, Some(ctx.mission_id.clone()), &ctx.agent_id, &ctx.name);
    }
    ...
    pub(crate) async fn record_heartbeat(&self, agent_id: &str) {
        ...
        let _ = crate::agent::persistence::update_agent_heartbeat(&self.state.resources.pool, agent_id).await;
    }
    ...
    pub async fn run(&self, agent_id: String, mut payload: TaskPayload) -> anyhow::Result<String> {
        ...
        let _mission_id = payload.cluster_id.clone().unwrap_or_else(|| "unknown".to_string());
        ...
        let agent_data = self.state.registry.agents.get(&agent_id).map(|a| a.value().clone()).ok_or_else(|| anyhow::anyhow!("Agent not found for context resolution"))?;

        if let Some(workflow_name) = agent_data.workflows.first() {
            let msg_lower = payload.message.to_lowercase();
            let workflow_requested = msg_lower.contains(&workflow_name.to_lowercase().replace("_", " ")) || msg_lower.contains("workflow") || msg_lower.contains("sop");
            let is_sme = agent_data.department.to_lowercase().contains("sme") || agent_data.role.to_lowercase().contains("specialist");

            if workflow_requested || is_sme {
                if let Ok(mut state) = crate::agent::workflows::load_workflow(self.state.base_dir.as_path(), workflow_name).await {
                    return self.run_deterministic_workflow(&agent_id, payload, &mut state).await;
                }
            }
        }
        ...
        let ctx = self.prepare_run_context(&agent_id, &payload, &mission_id, depth, &lineage).await?;
...
mod tests {
    ...
    async fn validate_input_detects_circular_recursion() {
        let (runner, _) = (AgentRunner::new(Arc::new(crate::state::AppState::new().await.unwrap())), ());

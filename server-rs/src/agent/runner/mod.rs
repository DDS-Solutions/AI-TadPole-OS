...
impl AgentRunner {
    ...
    async fn prepare_run_context(
        ...
        let spawn_mission_id = mission_id.to_string();
        let runner = self.clone();
        let ctx_clone = ctx.clone();

        // Spawn async background RAG prep
        tokio::spawn(async move {
                ...
                crate::agent::memory::VectorMemory::connect(&agent_memory_dir, "memories").await
            {
                ...
                {
                    // [Security] Resolve provider in background task
                    let client = (*runner.state.resources.http_client).clone();
                    let provider = runner.resolve_provider(&ctx_clone, client);
                            ...
                            for (i, text) in results.into_iter().enumerate() {
                                let _ = mission_mem
                                    .add_memory(
                                        ...
                                        &text,
                                        &spawn_mission_id,
                                        vec.clone(), // Use proper dimensions
                                    )
                                    .await;
                            }
                        }
    ...
    async fn execute_intelligence_loop(
        ...
        ctx: &RunContext,
        payload: &TaskPayload,
    ) -> anyhow::Result<IntelligenceOutput> {
...
impl AgentRunner {
    ...
    async fn finalize_run(
        ...
        if let Some(mut entry) = self.state.registry.agents.get_mut(&ctx.agent_id) {
            ...
            let turn_cost = usage.as_ref().map(|u| u.total_tokens as f64 * 0.00001).unwrap_or(0.0);

...
impl AgentRunner {
    ...
    pub async fn run(&self, agent_id: String, payload: TaskPayload) -> anyhow::Result<String> {
            ...
            .filter(|_| !agent_data.workflows.is_empty())
        {
            ...
            match crate::agent::workflows::load_workflow(self.state.base_dir.as_path(), workflow_name).await {
                Ok(mut state) => {
                    return self
    ...
    async fn prepare_run_context(
        ...
        let mut ctx = self.resolve_agent_context(agent_id, payload, mission_id, depth, lineage)?;

        // 🧠 CONTEXT SUMMARIZATION (4k Token Threshold)
        // If the mission has a long history, we summarize it to save prompt tokens
        // and reduce latency.
        let logs =
            crate::agent::mission::get_recent_mission_logs(
                &self.state.resources.pool,
                mission_id,
                CONTEXT_LOG_LIMIT,
            )
        ...
        let prompt_words: HashSet<String> = initial_prompt
            .to_lowercase()
            .split_whitespace()
            .map(str::to_string)
            .collect();
                ...
                crate::agent::memory::VectorMemory::connect(&mission_scope_dir, "scope").await
            {
                ...
                if let Ok(vec) = provider.embed(&initial_prompt).await {
                    ...
                    if let Ok(results) = agent_mem
                        ...
                        .await
                    {
                        // Phase 2: Reranking Pass (Heuristic for now, can be replaced with model-based reranker)
                        let mut scored_results: Vec<(f32, String)> = results
                            .into_iter()
                            .map(|text| {
                                let mut score = 0.0;
                                // Keyword boost: If many words from prompt are in text, boost score
                                let text_lower = text.to_lowercase();
                                let text_words: HashSet<&str> =
    ...
    async fn run_deterministic_workflow(
        &self,
        agent_id: &str,
        payload: TaskPayload,
...
impl AgentRunner {
    ...
    fn resolve_agent_context(
        ...
        Ok(RunContext {
            ...
            last_accessed_files: std::sync::Arc::new(std::sync::Mutex::new(
                payload.context_files.clone().unwrap_or_default(),
            )),

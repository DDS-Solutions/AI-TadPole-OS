...
impl RunContext {
    ...
    pub fn derive_subtask_payload(&self, message: String) -> TaskPayload {
        ...
        TaskPayload {
            ...
            visible_transcript: self.visible_transcript.as_ref().map(|vt| vt.lock().clone()),
            ..Default::default()
        }
    }
...
impl AgentRunner {
    ...
    pub(crate) async fn verify_provider_connectivity(&self, ctx: &RunContext) -> Result<(), AppError> {
        ...
        if ctx.model_config.provider == ModelProvider::Ollama {
            return Ok(());
        }

        tracing::info!("🔍 [Connectivity Check] Pre-flight connection validation for provider {:?}...", ctx.model_config.provider);
        self.broadcast_sys(
            &format!("🔍 Pre-flight connection check for provider {:?}...", ctx.model_config.provider),
            "info",
            Some(ctx.mission_id.clone()),
        );
        ...
        match resolved_prov.generate("ping", "", None).await {
            Ok(_) => {
                tracing::info!("✅ [Connectivity Check] Provider {:?} is authenticated and responsive.", ctx.model_config.provider);
                self.broadcast_sys(
                    &format!("✅ Provider {:?} authenticated successfully.", ctx.model_config.provider),
                    "success",
                    Some(ctx.mission_id.clone()),
                );
                Ok(())

...
impl RunContext {
    ...
    pub(crate) fn derive_subtask_payload(&self, message: String) -> TaskPayload {
        ...
        TaskPayload {
            ...
            recent_findings: self.recent_findings.as_ref().map(|rf| safe_truncate_str(rf, 2048)),
            structured_output: Some(false),
            primary_goal: self.primary_goal.as_ref().map(|pg| safe_truncate_str(pg, 1024)),
...
impl AgentRunner {
    ...
    pub(crate) async fn verify_provider_connectivity(&self, ctx: &RunContext) -> Result<(), AppError> {
        ...
        let is_actually_openai = if ctx.model_config.provider == ModelProvider::Openai {
            let model_lower = ctx.model_config.model_id.to_lowercase();
            let base_url_is_openai = ctx.model_config.base_url.as_ref()
                .map(|url| url.is_empty() || url.contains("api.openai.com"))
                .unwrap_or(true);
            
            base_url_is_openai || model_lower.contains("gpt") || model_lower.contains("o1-") || model_lower.contains("o3-")
        } else {
            false
        };
        ...
        } else if is_actually_openai {
            "Openai".to_string()
        } else if ctx.model_config.model_id.is_empty() {
            format!("{:?}", ctx.model_config.provider)
        } else {
            ctx.model_config.model_id.clone()
        };

        let success_label = if is_local {
            "(local connection)".to_string()
        } else if is_actually_openai {
            "Openai".to_string()
        } else if ctx.model_config.model_id.is_empty() {
            format!("{:?}", ctx.model_config.provider)
        ...
        tracing::info!("🔍 [Connectivity Check] Pre-flight connection validation for provider {}...", provider_label);
        self.broadcast_sys(
            &format!("🔍 Pre-flight connection checking for valid provider {}...", provider_label),
            "info",
            Some(ctx.mission_id.clone()),
        );
        ...
        if is_local {
            tracing::info!("✅ [Connectivity Check] Provider {} is authenticated and responsive.", success_label);
            self.broadcast_sys(
        ...
        if let ProviderVariant::Null(ref _null_prov) = provider {
            let err_msg = format!("API key configuration missing for provider {:?}", ctx.model_config.provider);
            tracing::warn!("❌ [Connectivity Check] {}", err_msg);
            return Err(AppError::Forbidden(err_msg));
        }

        // Limit the response tokens to 1 to keep the check instant and cost-free
        let mut ping_config = ctx.model_config.clone();
        ping_config.max_tokens = Some(1);
        let ping_ctx = RunContext {
            model_config: ping_config,
            ..ctx.clone()
        };

        let resolved_prov = self.resolve_provider(&ping_ctx, (*self.state.resources.http_client).clone()).await;

        // Send a minimal validation prompt
        match resolved_prov.generate("ping", "", None).await {
            Ok(_) => {
                tracing::info!("✅ [Connectivity Check] Provider {} is authenticated and responsive.", success_label);
                self.broadcast_sys(
                    &format!("✅ Provider {} authenticated successfully.", success_label),
            ...
            Err(e) => {
                let err_msg = format!("API Key authentication failed for provider {:?}: {}", ctx.model_config.provider, e);
                tracing::warn!("❌ [Connectivity Check] {}", err_msg);
                self.broadcast_sys(
                    &format!("❌ API Key authentication failed for provider {:?}.", ctx.model_config.provider),
                    "error",
                    Some(ctx.mission_id.clone()),
                );

...
//! - **Subsystem**: Sovereign Engine / Agent Runner / trustgraph
//!
//! ### ⚠️ Invariants & Non-Negotiables
//! - `[Structural]` Type-safe state handling and bounded execution without unhandled panics.
//! - `[Behavioral]` Graph relations link only existing entities across BFS traversals (enforced_by: `test_trust_graph_lifecycle`, `test_mutation_and_resolution_hooks`).
//!
//! ### 🔍 Debugging & Observability
//! - **Local Errors**: none
//! - **Telemetry Targets**: none declared
//! - **Witness Tests**: `test_trust_graph_lifecycle`, `test_mutation_and_resolution_hooks`
...
impl TrustGraphEngine {
    ...
    pub async fn traverse_subgraph(
        ...
        // Use a Map to ensure uniqueness and O(1) lookup
        ...
        for _ in 0..max_depth {
            ...
            // Chunk current frontier to avoid excessive binder limits in SQLite (max 999 parameters)
            for chunk in current_frontier.chunks(200) {
                ...
                let mut query = sqlx::query::<sqlx::Sqlite>(sqlx::AssertSqlSafe(&*query_str));
                // Bind for both source_entity_id and target_entity_id
                ...
                for row in rows {
                    ...
                    // Queue for next BFS hop if not visited
        ...
        // B. Fetch node metadata for all visited node IDs in chunks to respect binders limit
...
mod tests {
    ...
    async fn test_trust_graph_lifecycle() -> Result<(), AppError> {
        ...
        // Verify Node Metadata rehydrated successfully
    ...
    #[tokio::test]
    async fn test_mutation_and_resolution_hooks() -> Result<(), AppError> {
        let pool = setup_in_memory_db().await?;
        let engine = TrustGraphEngine::new(pool);

        // 1. Simulate AGENT --MUTATED--> FILE hook
        let agent_node = TrustGraphNode {
            id: "agent:worker-1".to_string(),
            name: "worker-1".to_string(),
            r#type: "AGENT".to_string(),
            description: Some("Agent worker-1".to_string()),
            mission_id: Some("mission-test".to_string()),
        };
        let file_node = TrustGraphNode {
            id: "file:src/main.rs".to_string(),
            name: "src/main.rs".to_string(),
            r#type: "FILE".to_string(),
            description: Some("File at src/main.rs".to_string()),
            mission_id: Some("mission-test".to_string()),
        };
        engine.add_node(&agent_node).await?;
        engine.add_node(&file_node).await?;

        let mutated_rel = TrustGraphRelation {
            id: "rel:mutated:1".to_string(),
            source_entity_id: agent_node.id.clone(),
            target_entity_id: file_node.id.clone(),
            relation_type: "MUTATED".to_string(),
            weight: 1.0,
            mission_id: Some("mission-test".to_string()),
        };
        engine.add_relation(&mutated_rel).await?;

        // 2. Simulate MISSION --RESOLVED--> OUTCOME hook
        let mission_node = TrustGraphNode {
            id: "mission:mission-test".to_string(),
            name: "Mission mission-test".to_string(),
            r#type: "MISSION".to_string(),
            description: Some("Mission run mission-test".to_string()),
            mission_id: Some("mission-test".to_string()),
        };
        let outcome_node = TrustGraphNode {
            id: "outcome:mission-test:completed".to_string(),
            name: "Outcome: completed".to_string(),
            r#type: "OUTCOME".to_string(),
            description: Some("Mission succeeded".to_string()),
            mission_id: Some("mission-test".to_string()),
        };
        engine.add_node(&mission_node).await?;
        engine.add_node(&outcome_node).await?;

        let resolved_rel = TrustGraphRelation {
            id: "rel:resolved:1".to_string(),
            source_entity_id: mission_node.id.clone(),
            target_entity_id: outcome_node.id.clone(),
            relation_type: "RESOLVED".to_string(),
            weight: 1.0,
            mission_id: Some("mission-test".to_string()),
        };
        engine.add_relation(&resolved_rel).await?;

        // 3. Verify traversals
        let agent_subgraph = engine.traverse_subgraph(&[agent_node.id], 1).await?;
        assert_eq!(agent_subgraph.relations.len(), 1);
        assert_eq!(agent_subgraph.relations[0].relation_type, "MUTATED");
        assert_eq!(agent_subgraph.nodes.len(), 2);

        let mission_subgraph = engine.traverse_subgraph(&[mission_node.id], 1).await?;
        assert_eq!(mission_subgraph.relations.len(), 1);
        assert_eq!(mission_subgraph.relations[0].relation_type, "RESOLVED");
        assert_eq!(mission_subgraph.nodes.len(), 2);

...
mod tests {
    ...
    async fn setup_in_memory_db() -> Result<SqlitePool, AppError> {
        let pool = SqlitePool::connect("sqlite::memory:").await
            .map_err(|e| AppError::InternalServerError(format!("Failed to connect in-memory DB: {}", e)))?;
        ...
        .execute(&pool)
        .await
        .map_err(|e| AppError::InternalServerError(format!("Failed to create graph_entities: {}", e)))?;
        ...
        .execute(&pool)
        .await
        .map_err(|e| AppError::InternalServerError(format!("Failed to create graph_relations: {}", e)))?;

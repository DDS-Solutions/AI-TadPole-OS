...
impl KnowledgeStore {
    ...
    pub async fn add_entry(
        ...
        let existing = sqlx::query!(
            "SELECT id FROM knowledge_store_meta WHERE content_hash = ? LIMIT 1",
            content_hash
        )
        .fetch_optional(&self.pool)
        .await
        .map_err(|e| AppError::InternalServerError(format!("[IKS] Dedup check failed: {}", e)))?;

        if let Some(row) = existing {
            tracing::debug!(id = %row.id, "[IKS] Dedup hit — returning existing entry");
            return self
                .get_by_id(&row.id)
                .await?
    ...
    pub async fn get_by_id(&self, id: &str) -> Result<Option<KnowledgeEntry>, AppError> {
        ...
        if let Some(r) = row {
            ...
            Ok(Some(KnowledgeEntry {
                ...
                human_confirmed: r.human_confirmed != 0,
                ttl: r.ttl,
    ...
    /// Paginated list of entries with optional topic/cluster filters.
    pub async fn list(
        &self,
        topic: Option<&str>,
    ...
    pub async fn search(
        ...
        #[cfg(feature = "vector-memory")]
        {
            ...
            let hydrate_sql = format!(
                ...
                req.cluster_id
                    .as_deref()
                    .map(|c| format!("AND (cluster_id = '{}' OR cluster_id IS NULL)", c.replace('\'', "''")))
                    .unwrap_or_default(),
            );
            ...
            let rows = q
                .fetch_all(&self.pool)
                .await
                .map_err(|e| AppError::InternalServerError(format!("[IKS] search hydration failed: {}", e)))?;
            ...
            let mut results: Vec<KnowledgeEntry> = rows
                ...
                .map(|r| {
                    ...
                    let text = if sqlite_text.is_empty() {
                        hit_text.get(r.try_get::<String, _>("id").as_deref().unwrap_or("")).cloned().unwrap_or_default()
                    } else {
                        sqlite_text
                    };
                    KnowledgeEntry {
    ...
    pub async fn delete(&self, id: &str) -> Result<(), AppError> {
        sqlx::query!("DELETE FROM knowledge_store_meta WHERE id = ?", id)
            .execute(&self.pool)
            .await
            .map_err(|e| {
...
mod tests {
    ...
    async fn test_confirmed_entry_survives_eviction() {
        ...
        sqlx::query!(
            r#"INSERT INTO knowledge_store_meta (id, text, content_hash, topic, ttl, human_confirmed, created_at, updated_at)
               VALUES (?, 'hello', ?, 'general', ?, 0, unixepoch(), unixepoch())"#,
            id,
            "abc123hash",
            past_ttl,
        )
        ...
        sqlx::query!(
            r#"INSERT INTO knowledge_store_meta (id, text, content_hash, topic, ttl, human_confirmed, created_at, updated_at)
               VALUES (?, 'world', ?, 'general', ?, 0, unixepoch(), unixepoch())"#,
            "test-entry-2",
            "def456hash",
            past_ttl,
        )
    ...
    async fn test_get_by_id_returns_text() {
        ...
        sqlx::query(
            "INSERT INTO knowledge_store_meta (id, text, content_hash, topic, created_at, updated_at) \
             VALUES ('id-1', 'The quick brown fox', 'hashxyz', 'general', unixepoch(), unixepoch())",
        )
        .execute(&pool)
        .await
        .unwrap();
    ...
    async fn test_decay_is_time_aware() {
        ...
        sqlx::query(
            "INSERT INTO knowledge_store_meta (id, text, content_hash, topic, confidence, updated_at, created_at) \
             VALUES ('id-decay', 'fact', 'decayhash', 'general', 1.0, ?, unixepoch())",
        )
        .bind(ten_days_ago)
        .execute(&pool)
        .await

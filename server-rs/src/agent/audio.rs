...
impl NeuralAudioEngine {
    ...
    pub async fn speak_stream(
        ...
        Err(anyhow!("Neural Audio engine is disabled in this build. Use Browser or Cloud fallback."))
    }
    ...
    pub async fn listen(&self, _audio_data: Vec<u8>) -> Result<String> {
        Err(anyhow!("Local Whisper engine is disabled in this build. Use Cloud transcription."))
    }

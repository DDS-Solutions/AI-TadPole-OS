...
mod tests {
    ...
    fn test_model_aware_multiplier_matrix() {
        ...
        assert_eq!(qwen_count, expected_qwen, "Qwen count must match 1.05x multiplier");
        assert_eq!(llama_count, expected_llama, "Llama count must match 1.02x multiplier");
        assert_eq!(gemma_count, expected_gemma, "Gemma count must match 0.95x multiplier");
    }
    ...
    fn test_tokenizer_cache_consistency() {
        ...
        assert_eq!(qwen_cold, qwen_hot, "Cached Qwen count must match cold count");
        assert_eq!(llama_cold, llama_hot, "Cached Llama count must match cold count");
    }
    ...
    fn test_fallback_tokenizer_accuracy() {
        ...
        assert_eq!(ascii_count, (26 * 10 / 38).max(1));
    }

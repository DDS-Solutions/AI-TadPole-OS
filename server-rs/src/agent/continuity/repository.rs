...
//! - **Subsystem**: Sovereign Engine / Agent Runner / repository
//!
...
impl WorkflowRepository {
    ...
    pub async fn verify_workflow_owner(
        &self,
        workflow_id: &str,
        tenant_id: &str,
    ) -> Result<(), AppError> {
        let exists: Option<String> =
            sqlx::query_scalar("SELECT id FROM workflows WHERE id = ?1 AND tenant_id = ?2")
                .bind(workflow_id)
                .bind(tenant_id)
    ...
    pub async fn verify_run_owner(&self, run_id: &str, tenant_id: &str) -> Result<(), AppError> {
        let exists: Option<String> =
            sqlx::query_scalar("SELECT id FROM workflow_runs WHERE id = ?1 AND tenant_id = ?2")
                .bind(run_id)
                .bind(tenant_id)
                .fetch_optional(&self.pool)
                .await?;
        ...
        if exists.is_none() {
            return Err(AppError::Forbidden(
                "Workflow run not found or access denied".to_string(),
            ));
        }
        Ok(())
    }
                    ...
                    Some(s) if !s.trim().is_empty() => {
                        Some(serde_json::from_str(&s).map_err(|e| {
                            AppError::InternalServerError(format!(
                                "Invalid workflow step config JSON: {}",
                                e
                            ))
                        })?)
                    }
    ...
    pub async fn update_workflow_run_success(
        ...
        let result = sqlx::query("UPDATE workflow_runs SET completed_at = ?1, status = 'completed', context = ?2 WHERE id = ?3 AND tenant_id = ?4 AND status = 'running'")
            .bind(now)
            .bind(encrypted)
            .bind(run_id)
            .bind(tenant_id)
    ...
    #[allow(dead_code)]
    pub async fn update_workflow_run_failed(
        &self,
        run_id: &str,
        tenant_id: &str,
    ) -> Result<(), AppError> {
    ...
    #[allow(dead_code)]
    pub async fn cleanup_step_runs_on_cancel(
        &self,
        run_id: &str,
        tenant_id: &str,
    ) -> Result<(), AppError> {
    ...
    pub async fn get_workflow_run_tenant_and_status(
        &self,
        run_id: &str,
    ...
    pub async fn set_run_status_cancelling(
        &self,
        run_id: &str,
        tenant_id: &str,
    ) -> Result<bool, AppError> {
        let result = sqlx::query(
            "UPDATE workflow_runs SET status = 'cancelling' WHERE id = ?1 AND tenant_id = ?2 AND status = 'running'",
        )
        .bind(run_id)
        .bind(tenant_id)
        .execute(&self.pool)
...
pub fn decrypt_context(encoded: &str) -> Result<String, AppError> {
    ...
    if decoded.len() < 44 {
        return Err(AppError::InternalServerError(
            "Encrypted context too short".to_string(),
        ));
    }
    ...
    if signature.ct_eq(&expected_signature).unwrap_u8() != 1 {
        return Err(AppError::InternalServerError(
            "Encrypted context integrity check failed (MAC mismatch)".to_string(),
        ));
    }

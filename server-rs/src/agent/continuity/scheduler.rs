...
pub async fn recover_interrupted_jobs(pool: &SqlitePool) -> Result<(), AppError> {
    ...
    if jobs_recovered > 0 {
        tracing::warn!(
            "⚠️ [Continuity] Recovered {} interrupted scheduled job run(s) and marked them as failed.",
            jobs_recovered
        );
    }
    ...
    if workflows_recovered > 0 {
        tracing::warn!(
            "⚠️ [Continuity] Recovered {} interrupted workflow run(s) and marked them as failed.",
            workflows_recovered
        );
    }
    ...
    if steps_recovered > 0 {
        tracing::warn!(
            "⚠️ [Continuity] Recovered {} interrupted workflow step run(s) and marked them as failed.",
            steps_recovered
        );
    }
...
mod tests {
    ...
    async fn test_recover_interrupted_jobs_lifecycle() -> Result<(), Box<dyn std::error::Error>> {
        ...
        // 1. Create a job to satisfy foreign key constraints
        let req = CreateJobRequest {
            agent_id: "agent-1".to_string(),
            workflow_id: None,
            name: "Test Job".to_string(),
        ...
        assert_eq!(runs[0].output_summary, Some("Interrupted by system restart".to_string()));

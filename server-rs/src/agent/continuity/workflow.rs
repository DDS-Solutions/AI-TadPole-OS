//! Orchestration engine for multi-step agent workflows.
//!
//! This module enables the creation and execution of deterministic 
//! sequences of agent tasks. Each step can inject output from previous 
//! steps into its own prompt, allowing for complex, chainable reasoning.
...
impl WorkflowEngine {
    ...
    pub async fn create_workflow(&self, name: String, description: Option<String>) -> Result<Workflow> {
    ...
    pub async fn add_step(&self, workflow_id: &str, agent_id: &str, name: String, prompt_template: String, step_order: i32) -> Result<WorkflowStep> {
        let id = Uuid::new_v4().to_string();
    ...
    async fn get_workflow_steps(&self, workflow_id: &str) -> Result<Vec<WorkflowStep>> {
        ...
        for row in rows {
            steps.push(WorkflowStep {
                ...
                config: row.get::<Option<String>, _>("config").and_then(|s| serde_json::from_str(&s).ok()),
            });
        }
        Ok(steps)
    }
    ...
    pub async fn run_workflow(&self, workflow_id: &str, initial_context: serde_json::Value) -> Result<String> {
        let row: sqlx::sqlite::SqliteRow = sqlx::query("SELECT * FROM workflows WHERE id = ?1")
            .bind(workflow_id)
        ...
        for (i, step) in steps.iter().enumerate() {
            tracing::info!("🔄 [Workflow] Running step {}/{} for workflow {}: {}", i + 1, steps.len(), workflow_id, step.name);
        ...
        sqlx::query("UPDATE workflow_runs SET completed_at = ?1, status = 'completed' WHERE id = ?2")
            .bind(Utc::now())
            .bind(&run_id)
            .execute(&self.state.resources.pool)
            .await?;

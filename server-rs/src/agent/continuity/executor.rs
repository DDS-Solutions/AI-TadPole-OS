...
pub async fn execute_job(state: Arc<AppState>, job: super::types::ScheduledJob) {
    ...
    let run = match create_job_run(&state.resources.pool, &job.id).await {
        ...
        Err(e) => {
            ...
            let _ = crate::agent::persistence::release_agent(&state.resources.pool, &job.agent_id).await;
            return;
        }
    };
    ...
    .await
    {
        tracing::error!("❌ [Continuity] Failed to finalise run record for job '{}': {}", job.name, e);
    }
    ...
    .await
    {
        tracing::error!("❌ [Continuity] Failed to record job tick for job '{}': {}", job.name, e);
    }

    // 5. Release agent claim so subsequent ticks/missions can claim the agent
    if let Err(e) = crate::agent::persistence::release_agent(&state.resources.pool, &job.agent_id).await {
        tracing::error!("❌ [Continuity] Failed to release agent claim for job '{}': {}", job.name, e);
    }

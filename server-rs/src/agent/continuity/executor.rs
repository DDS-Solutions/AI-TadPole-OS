...
//!
//! This module implements the core scheduler loop that periodically polls 
//! for due jobs and orchestrates their execution via the `AgentRunner`.
...
/// 1. Checks if the agent is already busy → skip (prevents concurrency conflicts).
...
async fn execute_job(state: Arc<AppState>, job: super::types::ScheduledJob) {
    ...
    // 1. Skip if agent is busy (prevents overlapping missions)
    let agent_busy = state
        .registry.agents
        .get(&job.agent_id)
        .map(|a| a.status == "busy")
        .unwrap_or(false);
    ...
    let result = if let Some(workflow_id) = &job.workflow_id {
        tracing::info!("🔄 [Continuity] Executing workflow '{}' (id: {})", job.name, workflow_id);
        ...
        workflow_engine.run_workflow(workflow_id, serde_json::json!({})).await
    } else {

...
pub async fn start_scheduler(state: Arc<AppState>) {
    ...
    if let Err(e) = crate::agent::continuity::repository::WorkflowRepository::recover_interrupted_workflow_runs(&state.resources.pool).await {
        tracing::error!("❌ [Continuity] Failed to recover interrupted workflow runs: {}", e);
    }
...
pub async fn execute_job(state: Arc<AppState>, job: super::types::ScheduledJob) {
    ...
    let (result_output, run_workflow_id) = if let Some(workflow_id) = &job.workflow_id {
        ...
        match workflow_engine.run_workflow("default", workflow_id, serde_json::json!({})).await {

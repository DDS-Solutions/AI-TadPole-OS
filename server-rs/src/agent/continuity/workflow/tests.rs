...
async fn test_workflow_context_key_collision() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(err_msg.contains("Duplicate step name / context key collision"));
...
async fn test_step_run_failed_on_config_error() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(err_str.contains("cannot mix routing, fan_out, or tournament modes"));
...
#[tokio::test]
async fn test_terminal_step_output_resolution() {
    let step_a = WorkflowStep {
        id: "step-a-id".to_string(),
        workflow_id: "wf".to_string(),
        agent_id: "1".to_string(),
        step_order: 1,
        name: "Step A".to_string(),
    ...
    let step_b = WorkflowStep {
        ...
        max_retries: 3,
        backoff_factor_secs: 2,
        depends_on: vec!["step-a-id".to_string()],
    };
    let step_c = WorkflowStep {
        id: "step-c-id".to_string(),
        workflow_id: "wf".to_string(),
        agent_id: "1".to_string(),
        step_order: 3,
        name: "Step C".to_string(),
    ...
    // B and C are terminals (nothing depends on them), A is not
    assert_eq!(terminals.len(), 2);
    let terminal_ids: HashSet<String> = terminals.iter().map(|s| s.id.clone()).collect();
    assert!(terminal_ids.contains("step-b-id"));
    assert!(terminal_ids.contains("step-c-id"));
    assert!(!terminal_ids.contains("step-a-id"));
}

#[tokio::test]
async fn test_add_step_eager_validation() -> Result<(), Box<dyn std::error::Error>> {
    let state = Arc::new(AppState::new_mock().await);
    let engine = WorkflowEngine::new(Arc::clone(&state));
    let wf = engine
        .create_workflow("default", "Eager Validation WF".to_string(), None)
        .await?;
    ...
    let step_1 = engine
        ...
        .await?;
    assert_eq!(step_1.name, "Initial Step");

    // Self dependency rejected
    let self_dep_err = engine
        .add_step(
            "default",
            &wf.id,
            "1",
            "Self Dep Step".to_string(),
            "Prompt".to_string(),
            2,
            None,
            None,
            Some(vec!["Self Dep Step".to_string()]),
        )
        .await;
    assert!(self_dep_err.is_err());
    assert!(self_dep_err
        .unwrap_err()
        .to_string()
    ...
    let missing_dep_err = engine
        .add_step(
            ...
            None,
            None,
            Some(vec!["non_existent_step_id".to_string()]),
        )
        .await;
    assert!(missing_dep_err.is_err());
    assert!(missing_dep_err
        .unwrap_err()
        .to_string()
...
#[test]
fn test_resolve_step_target_by_name() {
    let step_1 = WorkflowStep {
        id: "uuid-1111".to_string(),
        workflow_id: "wf".to_string(),
        agent_id: "1".to_string(),
        step_order: 1,
    ...
    let steps = vec![step_1];

    // By ID
    assert_eq!(
        resolve_step_target("uuid-1111", &steps),
        Some("uuid-1111".to_string())
    );
    // By exact Name
    assert_eq!(
        resolve_step_target("Analyze Code", &steps),
        Some("uuid-1111".to_string())
    );
    // By sanitized context key
    assert_eq!(
        resolve_step_target("analyze_code", &steps),
        Some("uuid-1111".to_string())
    );
    // Non-matching
    assert_eq!(resolve_step_target("unknown_step", &steps), None);
}

#[tokio::test]
async fn test_failure_metadata_preservation() -> Result<(), Box<dyn std::error::Error>> {
    let state = Arc::new(AppState::new_mock().await);
    let pool = state.resources.pool.clone();
    let engine = WorkflowEngine::new(Arc::clone(&state));

    let wf = engine
        .create_workflow("default", "Forensics Test".to_string(), None)
        .await?;
    let step = engine
        .add_step(
            "default",
            &wf.id,
            "non-existent-agent-id",
            "Failing Forensics Step".to_string(),
            "Prompt".to_string(),
            1,
            Some(0), // 0 retries = 1 attempt
            None,
            None,
        )
        .await?;
    ...
    let step_run = sqlx::query(
        "SELECT metadata, status, output_text FROM workflow_step_runs WHERE step_id = ?1",
    )
    .bind(&step.id)
    .fetch_one(&pool)
    .await?;

    let status: String = step_run.get("status");
    assert_eq!(status, "failed");

    let metadata_str: String = step_run.get("metadata");
    assert_ne!(metadata_str, "{}");
    let metadata: serde_json::Value = serde_json::from_str(&metadata_str)?;
    assert_eq!(metadata["type"], "standard");
    assert_eq!(metadata["status"], "failed");
    assert!(metadata["attempts"].as_array().is_some());
    assert!(!metadata["attempts"].as_array().unwrap().is_empty());

    Ok(())
}

#[tokio::test]
async fn test_tournament_candidate_cap_and_placeholder_validation() {
    // 0 candidates rejected
    let empty_tourney = StepConfig {
        context_keys: None,
        context_json_path: None,
        context_max_chars: None,
        routing: None,
    ...
    assert!(empty_tourney.validate().is_err());

    // >20 candidates rejected
    let mut too_many = Vec::new();
    for i in 0..21 {
        too_many.push(TournamentCandidate {
            agent_id: "1".to_string(),
            prompt_template: format!("Candidate {}", i),
        });
    }
    let overflow_tourney = StepConfig {
        context_keys: None,
        context_json_path: None,
        context_max_chars: None,
        routing: None,
    ...
    assert!(overflow_tourney.validate().is_err());

    // Out-of-bounds placeholder rejected (e.g. {{candidate_5}} when only 2 candidates exist)
    let bad_placeholder = StepConfig {
        context_keys: None,
        context_json_path: None,
        context_max_chars: None,
        routing: None,
        ...
        tournament: Some(TournamentConfig {
            candidates: vec![
                TournamentCandidate {
                    agent_id: "1".to_string(),
                    prompt_template: "Prompt 0".to_string(),
                },
                TournamentCandidate {
                    agent_id: "1".to_string(),
                    prompt_template: "Prompt 1".to_string(),
                },
            ],
            judge_agent_id: "1".to_string(),
            judge_prompt_template: "Judge {{candidate_0}} vs {{candidate_5}}".to_string(),
            min_successful: None,
        }),
        safe_mode: None,
        analysis: None,
    };
    assert!(bad_placeholder.validate().is_err());

    // Valid placeholders pass
    let valid_tourney = StepConfig {
        context_keys: None,
        context_json_path: None,
        context_max_chars: None,
        routing: None,
        ...
        tournament: Some(TournamentConfig {
            ...
            judge_agent_id: "1".to_string(),
            judge_prompt_template: "Judge {{candidate_0}} vs {{candidate_1}}".to_string(),
            min_successful: Some(1),
        }),
        safe_mode: None,
        analysis: None,
    };
    assert!(valid_tourney.validate().is_ok());
}

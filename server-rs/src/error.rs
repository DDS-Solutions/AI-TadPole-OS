    ...
    fn get_metadata(&self) -> ErrorMetadata {
        match self {
            ...
            AppError::MultiError(errors) => {
                let status_code = errors.first()
                    .map(|e| e.status_code())
                    .unwrap_or(StatusCode::INTERNAL_SERVER_ERROR);
                let severity = if errors.iter().any(|e| e.resolve_metadata().severity == "CRITICAL") {
                    "CRITICAL"
                } else {
                    "ERROR"
                };
...
impl AppError {
    /// Creates a domain error, sanitizing its detail payload and validating its help link immediately.
    pub fn new_domain_error(code: DomainCode, detail: String, help_link: Option<String>) -> Self {
        let truncated = safe_truncate_bytes(&detail, 2048);
    ...
    /// Creates an infrastructure error, sanitizing its detail payload and validating its help link immediately.
    ...
    pub fn is_rate_limit(&self) -> bool {
        match self {
            ...
            AppError::InfrastructureError { kind, .. } => *kind == InfrastructureErrorKind::RateLimit,
            _ => false,
        }
    }
    ...
    pub fn is_network_timeout(&self) -> bool {
        match self {
            ...
            AppError::InfrastructureError { kind, .. } => {
                *kind == InfrastructureErrorKind::Timeout || *kind == InfrastructureErrorKind::NetworkError
            }
            _ => false,
        }
    }
...
impl IntoResponse for AppError {
    fn into_response(self) -> Response {
        ...
        let safe_detail = if status.is_server_error() {
            ...
            let truncated = safe_truncate_bytes(&detail, 2048);
            let sanitized = sanitize_html(&truncated);
            crate::utils::security::redact_secrets(&sanitized)
        };

        // Filter / validate help_link to prevent internal link leakage
        let safe_help_link = metadata.help_link.filter(|link| is_public_and_safe_link(link));
...
mod tests {
    ...
    fn test_error_status_mapping() {
        ...
        assert_eq!(
            AppError::new_domain_error(DomainCode::InsufficientQuota, "limit".to_string(), None).status_code(),
            StatusCode::PAYMENT_REQUIRED
        );
        assert_eq!(
            AppError::new_domain_error(DomainCode::InvalidConfiguration, "cfg".to_string(), None).status_code(),
            StatusCode::BAD_REQUEST
        );
        assert_eq!(
            AppError::new_domain_error(DomainCode::AgentDeactivated, "deact".to_string(), None).status_code(),
            StatusCode::FORBIDDEN
        );
        assert_eq!(
            AppError::new_domain_error(DomainCode::InvalidStateTransition, "transition".to_string(), None).status_code(),
            StatusCode::BAD_REQUEST
        );
        assert_eq!(
            AppError::new_domain_error(DomainCode::OversightViolation, "oversight".to_string(), None).status_code(),
            StatusCode::FORBIDDEN
        );
        assert_eq!(
            AppError::new_domain_error(DomainCode::TaskConflict, "conflict".to_string(), None).status_code(),
            StatusCode::CONFLICT
        );
        assert_eq!(
            AppError::new_domain_error(DomainCode::SystemError, "sys".to_string(), None).status_code(),
            StatusCode::INTERNAL_SERVER_ERROR
        );
    }
    ...
    fn test_max_length_truncation() {
        ...
        let (_, emoji_pd) = ProblemDetails::new(
            StatusCode::BAD_REQUEST,
            "Bad Request",
            emoji_detail,
        );
    ...
    fn test_anyhow_metadata_downcast() {
        let runner_err = crate::agent::runner::error::RunnerError::RecursionBlocked("recursion loop".to_string());
    ...
    fn test_help_link_validation() {
        ...
        assert_eq!(meta_safe.help_link, Some("https://console.cloud.google.com/billing".to_string()));

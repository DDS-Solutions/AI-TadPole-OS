...
impl ProblemDetails {
    ...
    pub fn new(status: StatusCode, title: &str, detail: String) -> (StatusCode, Json<Self>) {
        ...
        let severity = if status.is_server_error() || status == StatusCode::FORBIDDEN || status == StatusCode::UNAUTHORIZED { "CRITICAL" } else { "ERROR" };
...
mod tests {
    ...
    async fn test_is_retryable() {
        ...
        let err_conn = client.get("http://127.0.0.1:54321").send().await.unwrap_err();
        ...
        let client = reqwest::Client::new();
        let response_err = client.get(format!("http://127.0.0.1:{}", port))
            .send()
            .await
            .unwrap()
            .error_for_status()

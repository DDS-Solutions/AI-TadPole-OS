...
impl ProblemDetails {
    ...
    pub fn new(status: StatusCode, title: &str, detail: String) -> (StatusCode, Json<Self>) {
        ...
        let severity = if status.is_server_error() { "CRITICAL" } else { "ERROR" };
...
impl AppError {
    ...
    pub fn type_slug(&self) -> String {
        match self {
            ...
            AppError::InfrastructureError { provider_id, .. } => {
                format!("infra:{}", provider_id).to_lowercase()
            }
            AppError::QuantizationFallback { .. } => "resource-exhaustion".to_string(),
            AppError::RecruitmentFailure { role, .. } => {
            ...
            | AppError::Reqwest(_)
            | AppError::Serde(_)
            | AppError::WalkDir(_) => "internal-error".to_string(),
        }
    }
    ...
    fn into_response(self) -> Response {
        ...
        let (help_link, error_code) = match &self {
            AppError::DomainError {
                help_link, code, ..
            } => (help_link.clone(), Some(code.clone())),
            AppError::InfrastructureError {
                help_link,
                provider_id,
                ..
            } => (help_link.clone(), Some(provider_id.clone())),

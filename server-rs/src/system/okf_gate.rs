...
fn extract_playbook_reqs(text: &str) -> std::collections::HashSet<String> {
    ...
    // 2. Safe Line-by-Line requirement extraction fallback
    if required_keys.is_empty() {
        for line in text.lines() {
            let line_lower = line.to_lowercase();
            for req in REQUIREMENTS {
                if line_lower.contains(&format!("requires: {}", req.key))
                    || line_lower.contains(&format!("requires {}", req.key))
                {
...
pub async fn validate_environments(
    ...
    let required_keys = match cached_reqs {
        Some(reqs) => reqs,
        None => {
            // Cache miss: Query SQLite database for playbook texts
            let rows = sqlx::query(
            ...
            for row in rows {
                ...
                let playbook_reqs = extract_playbook_reqs(&text);

...
static PLAYBOOK_REQS_CACHE: Lazy<DashMap<String, (Instant, Vec<String>)>> = Lazy::new(DashMap::new);
...
pub async fn get_mounted_playbooks(
    ...
    let playbooks = rows.into_iter().map(|row| {
...
fn extract_playbook_reqs(text: &str) -> std::collections::HashSet<String> {
    ...
    // 2. Safe Line-by-Line requirement extraction fallback (zero string allocation in search loop)
    if required_keys.is_empty() {
        for line in text.lines() {
            let line_lower = line.to_lowercase();
            for req in REQUIREMENTS {
                if line_lower.contains(req.pattern_colon) || line_lower.contains(req.pattern_space) {
                    // Check for negations on the same line before the requirement
                    let has_negation = ["no ", "not ", "don't ", "does not ", "without ", "free of "]
                        .iter()
                        .any(|neg| line_lower.contains(neg));
                    if !has_negation {
...
pub async fn validate_environments(
    ...
    // 1. Check in-memory cache first to avoid database query hotspots (bypassed in unit tests)
    let cached_reqs = if cfg!(test) {
        None
    } else if let Some(entry) = PLAYBOOK_REQS_CACHE.get(cluster_id) {
        let (timestamp, reqs) = entry.value();
    ...
    let (required_keys, has_gold_critical) = match cached_reqs {
        Some(reqs) => (reqs, false),
        None => {
            ...
            if !cfg!(test) {
                // Cache the requirements for this cluster
                PLAYBOOK_REQS_CACHE.insert(cluster_id.to_string(), (Instant::now(), reqs_vec.clone()));
            }
            (reqs_vec, is_gold)
        }
    };
    ...
    for req in REQUIREMENTS {
        if required_keys.contains(&req.key.to_string()) && !detected_envs.contains(&req.key.to_string()) {
...
Some instructions.', 'playbook', 'cl-test', 'YAML Playbook')"#,
    ...
    async fn test_okf_validation_negation_avoidance() {
        ...
        let res = validate_environments(&pool, "cl-neg", &[])
            .await
            .unwrap();
        assert_eq!(res.status, "nominal");
    }
    ...
    async fn test_okf_validation_gold_critical_tier() {
        ...
        let res = validate_environments(&pool, "cl-gold", &[])
            .await
            .unwrap();
        assert_eq!(res.status, "critical");
    }

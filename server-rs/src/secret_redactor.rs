...
//! - **Subsystem**: Sovereign Engine / secret_redactor
//!
...
static PATTERNS: Lazy<(RegexSet, Vec<Regex>)> = Lazy::new(|| {
    let patterns = vec![
        ...
        // 3. JSON/Config keys: "apiKey": "...", api_key=..., apiKey: ..., etc.
...
impl SecretRedactor {
    ...
    /// Checks if a string contains any of the registered environment secrets.
    ...
    pub fn is_sensitive(&self, text: &str) -> bool {
        if self.contains_registered_secret(text) {
            return true;
        }
        PATTERNS.0.is_match(text)
    }

    /// Creates a redactor initialized with explicit secret values.
...
mod tests {
    ...
    #[test]
    fn test_redact_unquoted_secret() {
        let redactor = SecretRedactor::noop();
        let input = "api_key=some-unquoted-secret-value";
        let output = redactor.redact(input);
        assert_eq!(output, "api_key=[REDACTED]");

        let input_col = "api_key: some-other-secret";
        let output_col = redactor.redact(input_col);
        assert_eq!(output_col, "api_key: [REDACTED]");
    }
}

//! Runtime Log Redaction & Data Isolation - The Neural Shield
//!
...
//!
//! ### AI Assist Note
//! **Neural Shield Protection**: This redactor uses simple substring replacement. 
//! It is NOT a cryptographic filter; it relies on the `MIN_SECRET_LEN` (8) 
//! to avoid over-redacting common words. Ensure all new API keys are added 
//! to the `from_env` list to guarantee they are scrubbed from system logs.
//!
//! ### 🔍 Debugging & Observability
//! - **Failure Path**: Short secrets (< 8 chars) being skipped, or new provider keys not being added to the registry.
//! - **Telemetry Link**: Search for `[SecretRedactor]` or `[REDACTED]` in `tracing` logs.

...
pub async fn init_db(database_url: &str) -> Result<SqlitePool> {
    ...
    if allow_reconcile && (!is_fresh || !database_url.contains(":memory:")) {
        ...
        premark_merkle_audit_trail_migration_if_needed(&pool, &migrator).await?;
        premark_institutional_knowledge_store_migration_if_needed(&pool, &migrator).await?;
        premark_iks_add_text_column_migration_if_needed(&pool, &migrator).await?;
        premark_swarm_graph_migration_if_needed(&pool, &migrator).await?;
    } else if !allow_reconcile && (!is_fresh || !database_url.contains(":memory:")) {
        tracing::info!("ℹ️ Migration reconciler is disabled (set ALLOW_MIGRATION_RECONCILE=true to enable)");
    }
    ...
    let skip_seed = skip_seed_from_url || std::env::var("SKIP_DB_SEED")
        .map(|v| v == "true")
        .unwrap_or(false);
...
async fn seed_baseline_providers() -> Result<()> {
    ...
    for filename in files_to_seed {
        ...
        if tokio::fs::metadata(&dest_path).await.is_err() {
            if let Some(src_path) = find_bundled_file(&resource_root, &format!("data/{}", filename)).await
            {

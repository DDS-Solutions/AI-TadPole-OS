...
/// Handles environments where `created_at` column was added manually or in a previous iteration
/// that resulted in a checksum mismatch for version `20260405000100`.
async fn premark_created_at_fix_migration_if_needed(
    ...
    let created_at_column_exists = sqlx::query_scalar::<_, i64>(
...
CREATE TABLE IF NOT EXISTS _sqlx_migrations (
    ...
    let already_applied =
        sqlx::query_scalar::<_, i64>("SELECT 1 FROM _sqlx_migrations WHERE version = ?1 LIMIT 1")
            .bind(CREATED_AT_FIX_MIGRATION_VERSION)
            .fetch_optional(pool)
            .await?
            .is_some();
    ...
    let Some(migration) = migrator
        .iter()
        .find(|m| m.version == CREATED_AT_FIX_MIGRATION_VERSION)
    else {
        tracing::warn!(
            "⚠️ Created-at fix migration {} not found in embedded migrations; skipping pre-mark",
            CREATED_AT_FIX_MIGRATION_VERSION
        );
        return Ok(());
    };

    if already_applied {
        // Force-update the checksum in the database to match the current embedded one.
        // This resolves the `VersionMismatch` error by synchronizing the metadata.
    ...
    tracing::warn!(
        "⚠️ Pre-marked migration {} because agents.created_at already exists",
        CREATED_AT_FIX_MIGRATION_VERSION
    );
...
/// Handles environments where `current_task` column was added manually or in a previous iteration
/// that resulted in a checksum mismatch for version `20260405000100`.
async fn premark_current_task_fix_migration_if_needed(
    ...
    let current_task_column_exists = sqlx::query_scalar::<_, i64>(
        "SELECT 1 FROM pragma_table_info('agents') WHERE name='current_task' LIMIT 1",
    )
    .fetch_optional(pool)
    .await?
    .is_some();
    if !current_task_column_exists {
        return Ok(());
    }

    let already_applied =
        sqlx::query_scalar::<_, i64>("SELECT 1 FROM _sqlx_migrations WHERE version = ?1 LIMIT 1")
            .bind(CURRENT_TASK_FIX_MIGRATION_VERSION)
            .fetch_optional(pool)
            .await?
            .is_some();
    ...
    let Some(migration) = migrator
        .iter()
        .find(|m| m.version == CURRENT_TASK_FIX_MIGRATION_VERSION)
    else {
        return Ok(());
    };
    ...
    tracing::warn!(
        "⚠️ Pre-marked migration {} because agents.current_task already exists",
        CURRENT_TASK_FIX_MIGRATION_VERSION
    );

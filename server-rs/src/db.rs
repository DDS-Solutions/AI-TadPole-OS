...
pub async fn init_db(database_url: &str) -> Result<SqlitePool> {
    ...
    let skip_seed = std::env::var("SKIP_DB_SEED")
        .map(|v| v == "true")
        .unwrap_or(false);
...
async fn seed_baseline_mcp_config() -> Result<()> {
    ...
    if !dest_path.exists() {
        if let Some(src_path) =
            find_bundled_file(&resource_root, &format!(".agent/{}", mcp_filename))
        {
            tracing::info!(
...
async fn premark_hotfix_migration(
    ...
    let query = match (config.table, config.column) {
        ("agents", "connector_configs") => "SELECT 1 FROM pragma_table_info('agents') WHERE name='connector_configs' LIMIT 1",
        ("agents", "created_at") => "SELECT 1 FROM pragma_table_info('agents') WHERE name='created_at' LIMIT 1",
        ("agents", "current_task") => "SELECT 1 FROM pragma_table_info('agents') WHERE name='current_task' LIMIT 1",
        ("mission_logs", "hash") => "SELECT 1 FROM pragma_table_info('mission_logs') WHERE name='hash' LIMIT 1",
        ("knowledge_store_meta", "id") => "SELECT 1 FROM pragma_table_info('knowledge_store_meta') WHERE name='id' LIMIT 1",
        ("knowledge_store_meta", "text") => "SELECT 1 FROM pragma_table_info('knowledge_store_meta') WHERE name='text' LIMIT 1",
        _ => return Err(anyhow::anyhow!("Unsupported hotfix check: {}.{}", config.table, config.column)),
    };

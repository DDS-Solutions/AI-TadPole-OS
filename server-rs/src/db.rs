...
//!
//! ### AI Assist Note: Recompiled for Phase 2 advanced workflows migration.
...
// Trigger recompile for new workflow_tenancy migration
...
const MERKLE_AUDIT_TRAIL_MIGRATION_VERSION: i64 = 20260516000100;
const INSTITUTIONAL_KNOWLEDGE_STORE_MIGRATION_VERSION: i64 = 20260601000100;
const IKS_ADD_TEXT_COLUMN_MIGRATION_VERSION: i64 = 20260601000101;
const SWARM_GRAPH_MIGRATION_VERSION: i64 = 20260404000100;
...
pub async fn init_db(database_url: &str) -> Result<SqlitePool> {
    ...
    let is_in_memory = clean_url.contains(":memory:");

    let pool = if is_in_memory {
        // For in-memory SQLite, closing the connection pool destroys the DB.
        // Run migrations directly on the primary pool.
        ...
        // Decouple migration connection pool to avoid statement-cache staling (SQLite 2.4/2.3.1 pattern)
        let migration_options = options.clone().statement_cache_capacity(0);
        ...
        if allow_reconcile && !is_fresh {
            premark_connector_column_fix_migration_if_needed(&migration_pool, &migrator).await?;
            premark_created_at_fix_migration_if_needed(&migration_pool, &migrator).await?;
            premark_current_task_fix_migration_if_needed(&migration_pool, &migrator).await?;
            premark_merkle_audit_trail_migration_if_needed(&migration_pool, &migrator).await?;
            premark_institutional_knowledge_store_migration_if_needed(&migration_pool, &migrator).await?;
            premark_iks_add_text_column_migration_if_needed(&migration_pool, &migrator).await?;
            premark_swarm_graph_migration_if_needed(&migration_pool, &migrator).await?;
        } else if !allow_reconcile && !is_fresh {
            tracing::info!(
                "ℹ dependency Migration reconciler is disabled (set ALLOW_MIGRATION_RECONCILE=true to enable)"
            );
        }

        migrator.run(&migration_pool).await?;
        migration_pool.close().await; // Cleanly release the temporary pool

        tracing::info!("✅ Database migrations applied successfully");

        // Now establish the final application database connection pool with statement caching enabled
...
async fn seed_baseline_agents(pool: &SqlitePool) -> Result<()> {
    ...
    if let Some(path) = agents_json_path {
        ...
        for agent_val in agents {
            ...
            let model_config2_val = agent_val.get("model_config2")
                .or_else(|| agent_val.get("modelConfig2"))
                .or_else(|| agent_val.get("planningSlot"));
            let model_config2 = model_config2_val.map(|v| serde_json::to_string(v).unwrap_or_default());

            let model_config3_val = agent_val.get("model_config3")
                .or_else(|| agent_val.get("modelConfig3"))
                .or_else(|| agent_val.get("executionSlot"));
            let model_config3 = model_config3_val.map(|v| serde_json::to_string(v).unwrap_or_default());
...
async fn premark_hotfix_migration(
    ...
    let query = match (config.table, config.column) {
        ...
        ("knowledge_store_meta", "text") => {
            "SELECT 1 FROM pragma_table_info('knowledge_store_meta') WHERE name='text' LIMIT 1"
        }
...
CREATE TABLE IF NOT EXISTS _sqlx_migrations (
    ...
    if already_applied {
        let allow_checksum_sync = std::env::var("ALLOW_CHECKSUM_SYNC")
            .map(|v| v == "1" || v.eq_ignore_ascii_case("true"))
            .unwrap_or(false);

        if config.force_checksum_sync && allow_checksum_sync {
...
/// Handles environments where `hash` column in `mission_logs` was added out-of-band.
async fn premark_merkle_audit_trail_migration_if_needed(
    pool: &SqlitePool,
    migrator: &sqlx::migrate::Migrator,
) -> Result<()> {
    premark_hotfix_migration(
...
/// Handles environments where the `knowledge_store_meta` table and its `id` column were added.
async fn premark_institutional_knowledge_store_migration_if_needed(
    pool: &SqlitePool,
    migrator: &sqlx::migrate::Migrator,
) -> Result<()> {
    premark_hotfix_migration(
        ...
        &HotfixMigration {
            ...
            label: "Institutional-knowledge-store",
            check_table_exists: true,
            force_checksum_sync: true,
        },
    )
...
/// Handles environments where the `text` column was added to `knowledge_store_meta`.
async fn premark_iks_add_text_column_migration_if_needed(
    pool: &SqlitePool,
    migrator: &sqlx::migrate::Migrator,
) -> Result<()> {
    premark_hotfix_migration(
...
/// Handles environments where `mission_relationships` table was added out-of-band or has checksum mismatch.
async fn premark_swarm_graph_migration_if_needed(
    pool: &SqlitePool,
    migrator: &sqlx::migrate::Migrator,
) -> Result<()> {
    premark_hotfix_migration(

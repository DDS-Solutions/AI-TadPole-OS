...
//! - **Subsystem**: Sovereign Engine / Security & Governance / signed_capability
//!
...
impl SignedCapabilityManifest {
    ...
    pub fn canonical_payload(
        ...
        let mut buf = Vec::with_capacity(
            MANIFEST_MAGIC_HEADER.len()
                ...
                + content_hash.len()
                + 1
                + approval_id.map_or(0, |a| 4 + a.len())
                + 1
                + expiration.map_or(0, |_| 8)
                + 4
                + rc_str.len()
                + 4
                + resource_pattern.len()
                + 4
                + mode_str.len(),
        );
    ...
    pub fn verify_signature(&self, verifying_key: &VerifyingKey) -> Result<(), AppError> {
        ...
        verifying_key.verify(&payload, &signature).map_err(|e| {
            AppError::Forbidden(format!("Ed25519 signature verification failed: {}", e))
        })
    }
    ...
    pub async fn verify_and_activate(
        ...
        // Step 5: Save active manifest to DB and activate capability mode in PermissionPolicy
        self.status = ManifestStatus::Active;
        self.save_to_db(policy.pool()).await.map_err(|e| {
            AppError::InternalServerError(format!("Failed to persist manifest status: {}", e))
        })?;

        policy.set_capability_mode_signed(self).await.map_err(|e| {
            AppError::InternalServerError(format!("Failed to activate capability: {}", e))
        })?;

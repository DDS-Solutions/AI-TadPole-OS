...
impl PermissionPolicy {
    ...
    pub async fn refresh_cache(&self) -> anyhow::Result<()> {
        ...
        for (name, mode_str) in rows {
            if let Ok(mode) = mode_str.parse::<PermissionMode>() {
                self.cache.insert(format!("::{}", name), mode);
            }
        }
    ...
    #[tracing::instrument(skip(self), fields(agent = ?agent_id, role = ?role, tool = tool_name), name = "security::get_permission_mode")]
    pub async fn get_mode(
        ...
        if let Some(aid) = agent_id {
            let res: Result<(String,), sqlx::Error> = sqlx::query_as(
        ...
        self.cache.insert(cache_key.clone(), PermissionMode::Prompt);

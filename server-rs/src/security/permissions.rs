...
impl PermissionDecision {
    pub fn new(mode: PermissionMode, source: PermissionDecisionSource, reason: Option<String>) -> Self {
        Self {
            mode,
            source,
            reason,
    ...
    pub fn with_reason(mode: PermissionMode, source: PermissionDecisionSource, reason: impl Into<String>) -> Self {
        Self {
            mode,
            source,
...
impl PermissionPolicy {
    ...
    pub fn legacy_cache_key(agent_id: Option<&str>, role: Option<&str>, tool_name: &str) -> PolicyCacheKey {
    ...
    pub async fn refresh_cache(&self) -> anyhow::Result<()> {
        ...
        for (name, mode_str) in legacy_rows {
            if let Ok(mode) = mode_str.parse::<PermissionMode>() {
                ...
                self.decision_cache.insert(key, PermissionDecision::new(mode, PermissionDecisionSource::GlobalLegacyPolicy, None));
            }
        }

        // 2. Load global capability policies
        let cap_rows: Vec<(String, String, String)> =
            sqlx::query_as("SELECT capability_class, resource_pattern, mode FROM capability_policies")
                .fetch_all(&self.pool)
                .await?;

        for (cap_str, pattern, mode_str) in cap_rows {
            if let (Ok(cap), Ok(mode)) = (cap_str.parse::<CapabilityClass>(), mode_str.parse::<PermissionMode>()) {
                let key = Self::capability_cache_key(None, None, cap, &pattern);
                self.decision_cache.insert(key, PermissionDecision::new(mode, PermissionDecisionSource::GlobalCapabilityPolicy, None));
            }
        }
    ...
    async fn fetch_policy_mode(&self, query: &'static str, binds: &[&str]) -> Result<Option<PermissionMode>, anyhow::Error> {
        ...
        match q.fetch_one(&self.pool).await {
            ...
            Err(e) => {
                tracing::error!("🚨 [Security] Database error during permission evaluation: {}", e);
    ...
    async fn evaluate_raw_capability_decision(
        ...
        if capability == CapabilityClass::Execute {
            ...
            if legacy_decision.mode != PermissionMode::Prompt {
                return legacy_decision;
            }
        }

        // Default to Prompt for unmapped capabilities (Sovereign Safety First)
        PermissionDecision::with_reason(PermissionMode::Prompt, PermissionDecisionSource::DefaultPrompt, "Unmapped capability defaults to prompt")
    }

    /// Checks if a cryptographically verified signed capability manifest (SEC-05) is currently active for resource.
    #[allow(dead_code)]
    pub async fn is_signed_capability_active(&self, capability: CapabilityClass, resource: &str) -> bool {
        let cap_str = capability.to_string();
        let res: Result<Option<(i32,)>, _> = sqlx::query_as(
            "SELECT 1 FROM signed_capability_manifests WHERE risk_class = ? AND (resource_pattern = ? OR ? LIKE resource_pattern) AND status = 'active' AND (expiration IS NULL OR expiration > CURRENT_TIMESTAMP) LIMIT 1"
        )
        .bind(cap_str)
        .bind(resource)
        .bind(resource)
    ...
    pub async fn check_capability_decision(
        ...
        let raw_decision = self.evaluate_raw_capability_decision(agent_id, role, capability, resource).await;
        let floor = capability.mandatory_floor();
        let effective_mode = raw_decision.mode.clamp_to_floor(floor);

        let final_decision = if effective_mode != raw_decision.mode {
        ...
        self.decision_cache.insert(cache_key, final_decision.clone());
        final_decision
    }
    ...
    pub async fn check_capability(
        ...
        self.check_capability_decision(agent_id, role, capability, resource).await.mode
    }
    ...
    pub async fn get_mode_decision(
        ...
        if let Some(r) = role {
            match self.fetch_policy_mode(
                ...
                Ok(Some(mode)) => {
                    let decision = PermissionDecision::with_reason(mode, PermissionDecisionSource::RolePolicy, format!("Role legacy override for {}", tool_name));
                    self.decision_cache.insert(cache_key, decision.clone());
                    return decision;
                }
                Err(_) => return PermissionDecision::with_reason(PermissionMode::Deny, PermissionDecisionSource::DatabaseFailure, "Infrastructure DB failure"),
        ...
        match self.fetch_policy_mode(
            ...
            Ok(Some(mode)) => {
                let decision = PermissionDecision::with_reason(mode, PermissionDecisionSource::GlobalLegacyPolicy, format!("Global legacy policy for {}", tool_name));
                self.decision_cache.insert(cache_key, decision.clone());
                return decision;
            }
            Err(_) => return PermissionDecision::with_reason(PermissionMode::Deny, PermissionDecisionSource::DatabaseFailure, "Infrastructure DB failure"),
        ...
        let default_decision = PermissionDecision::with_reason(PermissionMode::Prompt, PermissionDecisionSource::DefaultPrompt, "Unknown tool defaults to prompt");
        self.decision_cache.insert(cache_key, default_decision.clone());
        default_decision
    }
    ...
    pub async fn set_mode(&self, tool_name: &str, mode: PermissionMode) -> anyhow::Result<()> {
        ...
        self.decision_cache.retain(|key, _| match key {
            ...
            PolicyCacheKey::Capability { capability, resource, .. } => {
    ...
    pub async fn set_capability_mode(
        ...
        self.set_capability_mode_signed(capability, resource_pattern, mode).await
    }
    ...
    pub async fn set_capability_mode_signed(
        ...
        self.decision_cache.retain(|key, _| match key {
            PolicyCacheKey::Capability { capability: c, resource: r, .. } => {
                if *c != capability {
                    return true;
                }
...
mod tests {
    ...
    fn test_permission_mode_parsing() {
        ...
        assert_eq!("deny".parse::<PermissionMode>().unwrap(), PermissionMode::Deny);
        assert_eq!("prompt".parse::<PermissionMode>().unwrap(), PermissionMode::Prompt);
    ...
    async fn test_permission_policy_default_prompt() {
        ...
        sqlx::query("CREATE TABLE permission_policies (tool_name TEXT PRIMARY KEY, mode TEXT NOT NULL)").execute(&pool).await.unwrap();
        ...
        let mode = policy.get_mode(Some("agent-1"), Some("role-1"), "unknown_tool").await;

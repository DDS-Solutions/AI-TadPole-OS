...
impl MerkleAuditTrail {
    ...
    pub fn new(pool: SqlitePool) -> Self {
        let signing_key = std::env::var("AUDIT_PRIVATE_KEY")
            .ok()
            .and_then(|key| {
    ...
    pub async fn record(
        ...
        if let Some(m) = mission_id { hasher.update(m); }
        if let Some(u) = user_id { hasher.update(u); }
    ...
    async fn get_last_hash(&self) -> Result<String> {
        ...
        let row: Option<(String,)> = sqlx::query_as("SELECT current_hash FROM audit_trail ORDER BY timestamp DESC LIMIT 1")
    ...
    pub async fn verify_last_n(&self, n: usize, public_key_hex: Option<&str>) -> Result<bool> {
        let entries: Vec<AuditEntry> = sqlx::query_as(
    ...
    pub async fn verify_segments(&self, entries: &[AuditEntry], public_key_hex: Option<&str>) -> Result<bool> {
        ...
        for entry in entries {
            ...
            if let Some(ref m) = entry.mission_id { hasher.update(m); }
            if let Some(ref u) = entry.user_id { hasher.update(u); }
            ...
            if let (Some(pk), Some(sig_hex)) = (public_key, &entry.signature) {
                ...
                if pk.verify(entry.current_hash.as_bytes(), &signature).is_err() {
                    return Ok(false);
                }
            }
    ...
    /// Verifies a single entry's internal hash and signature.
    pub fn verify_record(&self, entry: &AuditEntry) -> bool {
        // 1. Verify intrinsic hash
        let mut hasher = Sha256::new();
        hasher.update(&entry.prev_hash);
        hasher.update(&entry.agent_id);
        if let Some(ref m) = entry.mission_id { hasher.update(m); }
...
mod tests {
    ...
    async fn test_audit_trail_chaining() {
        ...
        // Record 1
        let entry1 = audit.record("agent-1", Some("mission-1"), Some("user-1"), "tool_call", "{\"tool\":\"ls\"}").await.unwrap();
        assert_eq!(entry1.prev_hash, "0".repeat(64));
        
        // Record 2
        let entry2 = audit.record("agent-1", Some("mission-1"), Some("user-1"), "tool_call", "{\"tool\":\"cat\"}").await.unwrap();
    ...
    async fn test_audit_tamper_detection() {
        ...
        let entry2 = audit.record("agent-1", None, None, "action-2", "{}").await.unwrap();

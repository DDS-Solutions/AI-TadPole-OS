...
impl SkillSpectorResult {
    ...
    pub fn deny(reason: &str) -> Self {
        Self {
            ...
            risk_recommendation: format!("DENY: Security scanner unavailable or failed ({})", reason),
            filtered_findings: vec![SkillSpectorFinding {
                severity: "CRITICAL".to_string(),
                rule_id: "SCANNER_UNAVAILABLE".to_string(),
                location: None,
                finding: Some(reason.to_string()),
                explanation: Some("Security scanner unavailable or failed; failing closed to DENY.".to_string()),
            }],
        }
    }
}
...
pub fn scan_path(path: &Path) -> Result<SkillSpectorResult, AppError> {
    ...
    let python_bin = std::env::var("SKILLSPECTOR_PYTHON_BIN")
        ...
        .output()
    {
        ...
        Err(e) => {
            ...
            if require_scan {
                ...
                tracing::warn!("⚠️ [SkillSpector] Scanner missing but scan bypass enabled; bypassing scan.");
    ...
    if !output.status.success() {
        ...
        if require_scan {
            ...
            tracing::warn!("⚠️ [SkillSpector] Scan command failed but scan bypass enabled; bypassing scan.");
            return Ok(SkillSpectorResult::safe());
        }
    }

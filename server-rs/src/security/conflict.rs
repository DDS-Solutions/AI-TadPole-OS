...
impl ConflictManager {
    ...
    pub fn acquire_lease(&self, path: PathBuf, agent_id: String) -> Result<(), AppError> {
        ...
        self.leases.retain(|_, (_, timestamp)| {
...
mod tests {
    ...
    fn test_conflict_manager_lease() {
        ...
        assert!(manager.acquire_lease(path.clone(), "agent-a".to_string()).is_ok());
        assert!(manager.acquire_lease(path.clone(), "agent-a".to_string()).is_ok());
        ...
        assert!(manager.acquire_lease(path.clone(), "agent-b".to_string()).is_ok());
    }

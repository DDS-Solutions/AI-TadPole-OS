...
async fn test_permission_policy_persistence() {
    ...
    assert_eq!(policy.get_mode("dangerous_tool").await, PermissionMode::Prompt);
...
async fn test_permission_policy_cache_refresh() {
    ...
    // 2. Warm the cache
    assert_eq!(policy.get_mode("execute_shell").await, PermissionMode::Prompt);
    ...
    // 5. Refresh cache
    policy.refresh_cache().await.expect("Failed to refresh cache");
...
async fn test_permission_policy_unknown_tool_safety() {
    ...
    assert_eq!(policy.get_mode("zero_day_exploit_tool").await, PermissionMode::Prompt);
}

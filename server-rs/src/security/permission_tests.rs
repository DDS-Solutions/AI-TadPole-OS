...
async fn test_permission_policy_persistence() {
    ...
    assert_eq!(policy.get_mode(None, None, "read_file").await, PermissionMode::Allow);
}
...
async fn test_permission_policy_cache_refresh() {
    ...
    assert_eq!(policy.get_mode(None, None, "execute_shell").await, PermissionMode::Deny);
}
...
async fn test_agent_and_role_permissions() {
    ...
    sqlx::query("INSERT INTO agent_permission_policies (agent_id, tool_name, mode) VALUES (?, ?, ?)")
        .bind("agent-A")
        .bind("read_file")
        .bind("allow")
        .execute(&pool)
    ...
    assert_eq!(
        policy.get_mode(Some("agent-A"), Some("developer"), "read_file").await,
        PermissionMode::Allow
    );
    // Other agent defaults to global policy (prompt)
    assert_eq!(
        policy.get_mode(Some("agent-B"), Some("developer"), "read_file").await,
        PermissionMode::Prompt
    );

    // Test role-based policy
    assert_eq!(
        policy.get_mode(Some("agent-B"), Some("developer"), "write_file").await,
        PermissionMode::Prompt
    );
    // Other role defaults to global policy (deny)
    assert_eq!(
        policy.get_mode(Some("agent-B"), Some("tester"), "write_file").await,
        PermissionMode::Deny
    );
}

...
async fn test_signed_capability_valid_activation() {
    ...
    // Verify & Activate
    let res = manifest.verify_and_activate(&verifying_key, target_script, &policy).await;
    assert!(res.is_ok());
...
async fn test_signed_capability_tampered_content_fails() {
    ...
    let res = manifest.verify_and_activate(&verifying_key, tampered_script, &policy).await;
    assert!(res.is_err());
...
async fn test_signed_capability_bad_signature_fails() {
    ...
    let res = manifest.verify_and_activate(&wrong_key, target_script, &policy).await;
    assert!(res.is_err());
    let err_msg = res.unwrap_err().to_string();

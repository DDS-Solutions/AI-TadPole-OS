...
//!
//! ### AI Assist Note
//! **Evasion-Resistant Normalization**: Performs multi-pass normalizations
//! (Unicode folding, Cyrillic/Greek homoglyph resolution, markdown stripping,
//! leetspeak translation, base64 decoding) to prevent static scanning bypasses.
//!
//! ### 🔍 Debugging & Observability
//! - **Failure Path**: False-positives on base64-like strings or incorrect leetspeak substitutions.
//! - **Telemetry Link**: Search `[normalizer]` in tracing logs.
...
fn try_decode_base64(segment: &str) -> Option<String> {
    ...
    if let Ok(decoded) = BASE64_STANDARD.decode(segment) {
        if let Ok(utf8_str) = String::from_utf8(decoded) {
            ...
            if utf8_str.chars().all(|c| c.is_alphanumeric() || c.is_whitespace() || c.is_ascii_punctuation()) {

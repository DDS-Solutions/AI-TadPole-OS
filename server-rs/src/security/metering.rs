...
impl BudgetGuard {
    ...
    pub async fn flush_to_db(&self) -> Result<()> {
        ...
        if !agent_updates.is_empty() {
            ...
            // Deduct cost only after successful commit
            for (entity_id, cost) in agent_updates {
                if let Some(mut entry) = self.buffer.get_mut(&entity_id) {
                    *entry.value_mut() = (*entry.value() - cost).max(0.0);
                }
            }
        }
        ...
        for entry in self.mission_buffer.iter() {
            let cost = *entry.value();
            if cost > 0.0 {
        ...
        if !mission_updates.is_empty() {
            ...
            // Deduct cost only after successful commit
            for (cluster_id, cost) in mission_updates {
                if let Some(mut entry) = self.mission_buffer.get_mut(&cluster_id) {
                    *entry.value_mut() = (*entry.value() - cost).max(0.0);
                }
            }
        }
    ...
    async fn get_or_create_quota(&self, entity_id: &str) -> Result<Quota> {
        ...
        let r = sqlx::query("SELECT * FROM agent_quotas WHERE entity_id = ?1")
            .bind(entity_id)
            .fetch_one(&self.pool)
            .await?;
    ...
    async fn get_or_create_mission_quota(&self, cluster_id: &str) -> Result<Quota> {
        ...
        let r = sqlx::query("SELECT * FROM mission_quotas WHERE cluster_id = ?1")
            .bind(cluster_id)
            .fetch_one(&self.pool)
            .await?;

...
impl BudgetGuard {
    pub fn new(pool: SqlitePool, system_monitor: Arc<crate::security::monitoring::SecurityMonitor>) -> Self {
        Self {
            pool,
            buffer: DashMap::new(),
    ...
    pub async fn check_budget(&self, entity_id: &str, estimated_cost: f64) -> Result<bool> {
        ...
        if Utc::now() >= quota.next_reset_at && quota.reset_period != ResetPeriod::Never {
            ...
            return Ok(refreshed_used_micros + buffered_micros + estimated_micros <= refreshed_budget_micros);
        }
    ...
    async fn fetch_quota(pool: &SqlitePool, table: &str, id_col: &str, id_val: &str) -> Result<Quota> {

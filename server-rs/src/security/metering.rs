...
impl BudgetGuard {
    ...
    pub async fn check_budget(&self, entity_id: &str, estimated_cost: f64) -> Result<bool> {
        ...
        if Utc::now() >= quota.next_reset_at && quota.reset_period != ResetPeriod::Never {
            ...
            return Ok(refreshed.used_usd + estimated_cost <= refreshed.budget_usd);
        }

        Ok(quota.used_usd + estimated_cost <= quota.budget_usd)
    }

    /// Records actual cost after an operation completes.
    ...
    #[allow(dead_code)]
    pub async fn check_mission_budget(&self, cluster_id: &str, estimated_cost: f64) -> Result<bool> {
        let quota = self.get_or_create_mission_quota(cluster_id).await?;
        
        // Auto-reset if needed
        if Utc::now() >= quota.next_reset_at && quota.reset_period != ResetPeriod::Never {
            self.reset_mission_quota(&quota.id, quota.reset_period).await?;
    ...
    /// Records usage for a specific mission.
    #[allow(dead_code)]
    ...
    #[allow(dead_code)]
    pub async fn update_quota(&self, entity_id: &str, budget_usd: f64, reset_period: Option<ResetPeriod>) -> Result<()> {
        if let Some(period) = reset_period {
            ...
            sqlx::query(
                "UPDATE agent_quotas SET budget_usd = ?1 WHERE entity_id = ?2"
            )
            .bind(budget_usd)
            .bind(entity_id)
            .execute(&self.pool)
    ...
    #[allow(dead_code)]
    pub async fn update_mission_quota(&self, cluster_id: &str, budget_usd: f64, reset_period: Option<ResetPeriod>) -> Result<()> {

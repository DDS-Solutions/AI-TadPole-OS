...
impl ShellScanner {
    ...
    pub fn scan(&self, input: &str) -> ScannerResult {
        ...
        for pattern in heuristics {
            if let Ok(re) = Regex::new(pattern) {
                if let Some(mat) = re.find(input) {
                    ...
                    if !after.trim().is_empty() && !after.trim().starts_with('"') && !after.trim().starts_with('\'') {
                         // Likely a raw secret being exported
                         return ScannerResult::Risky("Prohibited raw secret export detected".to_string());
                    }
                }
            }
        }
        ...
        for (pattern, name) in injection_chars {
            if let Ok(re) = Regex::new(pattern) {
                if re.is_match(input) {
                    return ScannerResult::Risky(format!("Potential Command Injection or Redirection detected: {}", name));
                }
            }
        }

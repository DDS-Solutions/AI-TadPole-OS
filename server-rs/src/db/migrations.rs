//! Database Migrations & Hotfix Reconciler
//!
//! Orchestrates automated schema migrations via `sqlx` and pre-marks
//! out-of-band schema modifications to prevent checksum errors.
//!
//! ### AI Assist Note
//! **Database Migrations**: Manages schema versioning and migration execution.
//!
//! ### 🔍 Debugging & Observability
//! - **Failure Path**: Migration checksum failure or SQL schema syntax error.
//! - **Telemetry Link**: Search `[migrations]` in tracing logs.
...
async fn premark_hotfix_migration(
    ...
    if col_exists {
        if let Some(target_migration) = migrator
            .iter()

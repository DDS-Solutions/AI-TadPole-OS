//! Database Seeding & Baseline Initialization
//!
//! Seeds initial agent swarm state, provider configs, directives, and MCP configurations.
//!
//! ### AI Assist Note
//! **Database Seeding**: Populates initial swarm state and provider defaults.
//!
//! ### 🔍 Debugging & Observability
//! - **Failure Path**: Database seed collision or serialization error.
//! - **Telemetry Link**: Search `[seed]` in tracing logs.
...
async fn seed_baseline_mcp_config() -> Result<()> {
    ...
    if tokio::fs::metadata(&dest_path).await.is_err() {
        if let Some(src_path) = find_bundled_file(&resource_root, &format!(".agent/{}", mcp_filename)).await {

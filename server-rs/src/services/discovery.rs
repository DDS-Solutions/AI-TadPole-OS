//! Swarm Discovery — mDNS-SD node coordination and network awareness
//!
//! Orchestrates zero-configuration discovery of other Tadpole OS "Bunkers" 
//! across the Local Area Network (LAN) using the mDNS-SD protocol.
//!
//! @docs ARCHITECTURE:Networking
//!
//! ### AI Assist Note
//! **Network Awareness**: This module relies on the `mdns-sd` ServiceDaemon. 
//! If the host firewall blocks port `5353` (UDP), swarm discovery will fail silently 
//! or report 0 nodes. It resolves the primary non-loopback IP for host-to-host 
//! communication.
//!
//! ### 🔍 Debugging & Observability
//! - **Failure Path**: UDP 5353 blocked, mDNS daemon init failure (anyhow), or IP resolution conflict.
//! - **Telemetry Link**: Search for `[Discovery]` or `[mDNS]` in `tracing` logs.

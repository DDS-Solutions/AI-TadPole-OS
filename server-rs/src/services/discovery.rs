...
impl SwarmDiscoveryManager {
    ...
    pub fn start(&self) -> anyhow::Result<()> {
        ...
        // We use 10.0.0.1 to signal the daemon to find the local IP if possible, 
        // but mdns-sd might require a concrete one. For now, we'll try to get it.
        let my_ip = "10.0.0.1"; // Fallback
        ...
        tracing::info!("📡 [Discovery] mDNS Service Registered: {} on port {}", instance_name, port);
        ...
        tokio::spawn(async move {
            tracing::info!("📡 [Discovery] Swarm Browser Active: Searching for _tadpole._tcp.local...");
            while let Ok(event) = receiver.recv_async().await {
                match event {
                    ServiceEvent::ServiceResolved(info) => {
                        let id = info.get_property_val("id").flatten().map(|v| String::from_utf8_lossy(v).to_string()).unwrap_or_else(|| info.get_fullname().to_string());
                        let name = info.get_property_val("name").flatten().map(|v| String::from_utf8_lossy(v).to_string()).unwrap_or_else(|| info.get_fullname().to_string());
                        
                        let address = info.get_addresses().iter().next()
                            .map(|ip| format!("{}:{}", ip, info.get_port()))
                            .unwrap_or_else(|| format!("10.0.0.1:{}", info.get_port()));
                        ...
                        tracing::info!("🔗 [Discovery] Found swarm node: {} at {}", id, info.get_fullname());
                    }

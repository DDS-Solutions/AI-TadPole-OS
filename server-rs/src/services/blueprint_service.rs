...
impl BlueprintService {
    ...
    pub async fn scan_workspace(&self) -> Result<Blueprint> {
        if !self.root.exists() {
            return Err(anyhow::anyhow!("Workspace root directory does not exist: {:?}", self.root));
        }
        if !self.root.is_dir() {
            return Err(anyhow::anyhow!("Workspace root path is not a directory: {:?}", self.root));
        }
                ...
                Ok(content) => {
                    if let Ok(file) = syn::parse_file(&content) {
                        match entry.path().strip_prefix(&self.root) {
                            Ok(rel_path) => {
                                symbol_visitor.current_file = rel_path.to_string_lossy().to_string();
                                symbol_visitor.visit_file(&file);
                            }
                            Err(e) => {
                                partial_errors.push(format!("Path prefix error for {:?}: {}", entry.path(), e));
                            }
                        }
                    }
                }

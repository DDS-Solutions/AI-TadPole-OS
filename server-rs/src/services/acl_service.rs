...
impl AclServiceTrait for AclService {
        ...
        let allowed = match authority {
            ...
            RoleAuthorityLevel::Executive => {
                if agent_id == AGENT_CEO {
                    ...
                    // Equipped with read-only inspection tools for reality-grounded planning
                    matches!(
                        tool_name,
                        "spawn_subagent"
            ...
            RoleAuthorityLevel::Management => {
                if agent_id == AGENT_COO {
                    // COO Persona: Dispatcher with read-only grounding inspection
                    matches!(
                        tool_name,
                        "spawn_subagent"
                            | "send_mission_directive"
    ...
    fn get_role_protocols(
        ...
        match agent_id {
            ...
            AGENT_COO => {
                protocols.push("COO PROTOCOL: You MUST delegate the mission to the Alpha Node. Use 'spawn_subagent' with agent_id 'alpha'. Direct specialist recruitment is SYSTEM-BLOCKED. GROUNDING MANDATE: Before delegating or declaring mission status, verify the existence of referenced documents using 'read_file' or 'list_files'. Never fabricate reports or status.".to_string());
            }
            AGENT_ALPHA => {
...
mod tests {
    ...
    fn test_coo_acl_permissions() {
        ...
        assert!(acl.is_tool_allowed(AGENT_COO, "COO", RoleAuthorityLevel::Executive, "read_file"));
        assert!(acl.is_tool_allowed(
            AGENT_COO,
            "COO",
            RoleAuthorityLevel::Executive,

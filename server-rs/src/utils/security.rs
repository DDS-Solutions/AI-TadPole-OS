...
fn is_device_file(path: &Path) -> bool {
    ...
    for component in path.components() {
        if let std::path::Component::Normal(os_str) = component {
            ...
            if reserved.iter().any(|&r| r == base_name) {
                return true;
            }
        }
    }
...
pub fn validate_shell_command(command: &str) -> Result<(), AppError> {
    ...
    if lower.contains('*') || lower.contains('?') || lower.contains('[') || lower.contains(']') {
        return Err(AppError::Forbidden(
            "Wildcard characters (*, ?, []) are prohibited in commands to prevent sandbox bypass".to_string(),
        ));
    }
    ...
    if first_word == "git" {
        let dangerous_git = ["-c", "--git-dir", "--work-tree", "core.pager", "config", "exec", "upload-pack", "receive-pack"];
    ...
    let dangerous_flags = [
        "--erase", "--delete", "-rf", "/etc", "/root", "/var", "/bin", "/usr", "tadpole.db", ".env", ".gemini", "knowledge",
    ];

...
pub async fn validate_public_http_url(url: &str) -> Result<ValidatedUrl, AppError> {
    ...
    let mut addrs = match tokio::time::timeout(std::time::Duration::from_secs(3), lookup_future).await {
        Ok(Ok(a)) => a,
        Ok(Err(_)) => return Err(AppError::BadRequest("URL host could not be resolved".to_string())),
        Err(_) => return Err(AppError::BadRequest("URL host DNS resolution timed out".to_string())),
    };
...
fn is_device_file(path: &Path) -> bool {
    ...
    if path_str.starts_with("/dev/") || path_str.starts_with("/proc/") || path_str.starts_with("/sys/") || path_str.starts_with("/run/") {
        return true;
    }
    false
}
...
pub fn validate_path(base: &Path, user_path: &str) -> Result<SafePath, AppError> {
    ...
    let resolved = if joined.exists() {
        ...
        while let Some(p) = parent.parent() {
            ...
            if depth > MAX_PARENT_DEPTH {
                return Err(AppError::Forbidden("Path structure is too deep".to_string()));
            }
            if p.exists() {
                parent = p;
                break;
    ...
    if !resolved_norm.starts_with(&base_canon) {
        return Err(AppError::Forbidden(
...
pub fn redact_secrets(input: &str) -> String {
    ...
    static PATTERNS: Lazy<(RegexSet, Vec<(Regex, &'static str)>)> = Lazy::new(|| {
        ...
        let set = RegexSet::new(rules.iter().map(|r| r.pattern)).expect("Security patterns must be valid regex.");
        let regexes = rules
            .iter()
...
pub fn validate_shell_command(command: &str) -> Result<(), AppError> {
    ...
    if lower.contains('|') || lower.contains('<') || lower.contains(';') || lower.contains('&')
        || lower.contains('\u{FF1B}') || lower.contains('\u{FF5C}')
    {
        return Err(AppError::Forbidden(
    ...
    let first_word = match tokens.first() {
        ...
        None => return Err(AppError::Forbidden("Empty command is prohibited".to_string())),
    };
    ...
    if first_word == "find" {
        ...
        for flag in dangerous_find {
            if tokens.iter().any(|t| {
                ...
                return Err(AppError::Forbidden(format!(
                    "Unauthorized find parameter: '{}'", flag
                )));
            }
        }
            ...
            if token.starts_with('-') && !token.starts_with("--") {
                for c in token[1..].chars() {
                    if ['e', 'p', 'i', 'r'].contains(&c) {
                        return Err(AppError::Forbidden(format!(
                            "Unauthorized node option: '-{}'", c
                        )));
                    }
                }
            } else {
                let tl = token.to_lowercase();
                if tl == "--eval" || tl.starts_with("--eval=")
                    || tl == "--print" || tl.starts_with("--print=")
                    || tl == "--interactive" || tl.starts_with("--interactive=")
                    || tl == "--require" || tl.starts_with("--require=")
                {
                    return Err(AppError::Forbidden(format!(
                        "Unauthorized node option: '{}'", token
                    )));
                }
            }
        }
        ...
        for token in &tokens {
            ...
            if dangerous_git_flags.iter().any(|&flag| tl == flag || tl.starts_with(&format!("{}=", flag))) {
                return Err(AppError::Forbidden(format!(
                    "Unauthorized git parameter or option detected: '{}'",
                    token
                )));
            ...
            if tl.contains("ext::") || tl.contains("ssh::") || tl.contains("git::")
                || tl.contains("file://") || tl.contains("ssh://") || tl.contains("git://")
            {
                return Err(AppError::Forbidden(
...
mod tests {
    ...
    fn test_redact_secrets_extended_patterns() {
        ...
        assert!(redact_secrets("postgres://user:super_secret_password@localhost:5432/db").contains("[REDACTED]"));
        assert!(!redact_secrets("postgres://user:super_secret_password@localhost:5432/db").contains("super_secret_password"));

        // AWS secret keys
        assert!(redact_secrets("aws_secret_access_key=1234567890123456789012345678901234567890").contains("[REDACTED]"));
    }

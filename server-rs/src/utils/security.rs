...
pub fn redact_secrets(input: &str) -> String {
    ...
    static PATTERNS: Lazy<(RegexSet, Vec<Regex>)> = Lazy::new(|| {
        ...
        let set = RegexSet::new(&patterns).expect("Static security redaction patterns MUST be valid regex.");
        let regexes = patterns.iter().map(|p| Regex::new(p).expect("Static security redaction pattern MUST be valid regex.")).collect();
        (set, regexes)
    });

...
//! - **Subsystem**: Sovereign Engine / Codebase Intelligence / parser
//!
//! ### AI Assist Note
//! - Symbol and reference ranges use byte offsets and zero-based line numbers.
//!
...
//! - **Witness Tests**: `tests::test_markdown_reference_ranges_at_line_boundaries`
...
static BUILTIN_GLOBALS: Lazy<HashSet<String>> = Lazy::new(|| {
    ...
    // Statically include project globals from .agent/globals.json if available
    let json_str = include_str!("../../../.agent/globals.json");
    if let Ok(extra) = serde_json::from_str::<HashSet<String>>(json_str) {
        set.extend(extra);
    }
    set
});
...
impl SymbolExtractor {
    ...
    pub fn extract_symbols(&mut self, path: &Path, content: &str) -> Vec<Symbol> {
        ...
        let (mut symbols, module_doc, module_doc_range) = match ext {
            "rs" => {
                if let Some(tree) = self.rust_parser.parse(content, None) {
                    let root = tree.root_node();
                    let symbols = self.extract_rust_from_root(content, root);
                    let (doc, range) = extract_module_comments(root, content)
                        .map(|(d, r)| (Some(d), Some(r)))
                        .unwrap_or((None, None));
                    (symbols, doc, range)
                } else {
                    (Vec::new(), None, None)
            ...
            "ts" | "tsx" => {
                if let Some(tree) = self.ts_parser.parse(content, None) {
                    let root = tree.root_node();
                    let symbols = self.extract_typescript_from_root(content, root);
                    let (doc, range) = extract_module_comments(root, content)
                        .map(|(d, r)| (Some(d), Some(r)))
                        .unwrap_or((None, None));
                    (symbols, doc, range)
            ...
            "md" => (self.extract_markdown(path, content), None, None),
            _ => (Vec::new(), None, None),
        };
    ...
    fn extract_markdown_refs(&mut self, content: &str) -> Vec<Reference> {
        ...
        let line_breaks: Vec<usize> = content
            ...
            .map(|(offset, _)| offset)
            .collect();
        let line_at = |offset| line_breaks.partition_point(|&line_break| line_break < offset);
        ...
        while let Some(open_idx) = content[start_idx..].find("[[") {
            ...
            if let Some(close_idx) = content[absolute_open..].find("]]") {
                ...
                if !target.is_empty() {
                    let start_line = line_at(absolute_open);
                    let end_line = line_at(absolute_close + 2);
                    refs.push(Reference {
        ...
        for (event, range) in parser {
            if let pulldown_cmark::Event::Code(code_text) = event {
                ...
                if is_valid_code_identifier(term) {
                    let start_line = line_at(range.start);
                    let end_line = line_at(range.end);
                    refs.push(Reference {
                        name: term.to_string(),
                        range: SymbolRange {
    ...
    #[allow(dead_code)]
    pub fn extract_rust(&mut self, content: &str) -> Vec<Symbol> {
        let tree = match self.rust_parser.parse(content, None) {
            Some(t) => t,
            None => return Vec::new(),
        };
        self.extract_rust_from_root(content, tree.root_node())
    }

    fn extract_rust_from_root(&self, content: &str, root: tree_sitter::Node) -> Vec<Symbol> {
        let query_str = r#"
        ...
        self.query_symbols(content, &query, root)
    }

    #[allow(dead_code)]
    pub fn extract_typescript(&mut self, content: &str) -> Vec<Symbol> {
        let tree = match self.ts_parser.parse(content, None) {
            Some(t) => t,
            None => return Vec::new(),
        };
        self.extract_typescript_from_root(content, tree.root_node())
    }

    fn extract_typescript_from_root(&self, content: &str, root: tree_sitter::Node) -> Vec<Symbol> {
        let query_str = r#"
                ...
                if kind == "func" {
                    ...
                    while let Some(prev) = sibling.prev_sibling() {
                        ...
                        if p_kind == "attribute_item" {
                            ...
                            if is_tauri_command_attr(attr_text) {
                                is_tauri = true;
                                break;
                            }
                            sibling = prev;
...
fn is_tauri_command_attr(attr_text: &str) -> bool {
...
mod tests {
    ...
    #[test]
    fn test_markdown_reference_ranges_at_line_boundaries() {
        let mut extractor = SymbolExtractor::default();
        for newline in ["\n", "\r\n"] {
            let content =
                ["[[First]]", "[[Second]]", "`third`", "é [[Fourth]] `fifth`"].join(newline);
            let refs = extractor.extract_references(Path::new("test.md"), &content);
            for (name, line, source) in [
                ("First", 0, "[[First]]"),
                ("Second", 1, "[[Second]]"),
                ("third", 2, "`third`"),
                ("Fourth", 3, "[[Fourth]]"),
                ...
                let reference = refs
                    ...
                    .unwrap();
                assert_eq!(reference.range.start_line, line, "start line of {name}");
                assert_eq!(reference.range.end_line, line, "end line of {name}");
                assert_eq!(
                    &content[reference.range.start_byte..reference.range.end_byte],
                    source
                );
            }
        }

...
//! - **Subsystem**: Frontend Utilities / serialization
//!
...
mod tests {
    ...
    #[test]
    fn test_multibyte_string_pruning() {
        // Emojis like 🟢 are 4 bytes: [240, 159, 155, 162]
        let mut val = json!({
            "emojis": "🟢🟢🟢🟢🟢"
        });

        // Limit = 6 bytes. 🟢 is 4 bytes.
        // 6 is in the middle of the second emoji.
        // It should truncate safely at 4 bytes (the first emoji) without panicking.
        prune_large_strings(&mut val, 6);

        let s = val["emojis"].as_str().unwrap();

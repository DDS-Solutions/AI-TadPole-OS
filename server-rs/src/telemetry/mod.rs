    ...
    S: Subscriber + for<'a> tracing_subscriber::registry::LookupSpan<'a>,
{
    fn on_new_span(
        ...
        // Trace IDs are linked below when spans record an explicit `trace_id`
        // attribute. Current tracing APIs do not support reconstructing a
        // `tracing::Span` from a raw span id inside `on_new_span`.

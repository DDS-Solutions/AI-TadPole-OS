...
async fn test_telemetry_layer_snake_case_parity() {
    ...
    assert!(span_data.get("agent_id").is_some(), "Missing agent_id key in telemetry JSON");
    assert!(span_data.get("mission_id").is_some(), "Missing mission_id key in telemetry JSON");
    assert!(span_data.get("trace_id").is_some(), "Missing trace_id key in telemetry JSON");
...
async fn test_telemetry_span_update_on_close() {
    ...
    let update_event = rx.recv().await.expect("Failed to receive span update event");
    
    assert_eq!(update_event["type"], "trace:span_update");
    assert!(update_event.get("span_id").is_some(), "Missing span_id in update event");

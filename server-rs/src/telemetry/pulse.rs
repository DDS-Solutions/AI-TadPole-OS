...
pub async fn spawn_pulse_loop(state: Arc<AppState>, mut shutdown_rx: tokio::sync::watch::Receiver<bool>) {
    ...
    loop {
        ...
        for entry in state.registry.agents.iter() {
            ...
            let status = match agent.health.status.as_str() {
                "running" | "active" | "thinking" | "working" => 1,
                "failed" => 2,
                "throttled" => 3,
                _ => 0, // idle
            };
            ...
            if let Some(mission) = &agent.state.active_mission {
                    ...
                    .and_then(|v: &serde_json::Value| v.as_str())
                {
                    ...
                    pulse.edges.push(PulseConnection {
                        ...
                        target: "mission-hub".to_string(), // Always anchor to hub for high-speed pulse
                    });
                }
            }
        }

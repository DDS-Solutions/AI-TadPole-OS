...
impl MetricAggregator {
    ...
    pub async fn run(mut self, mut rx: broadcast::Receiver<Value>) {
        info!("🔭 [Telemetry] MetricAggregator started (Window: {}).", self.window_size);
    ...
    fn process_msg(&mut self, msg: Value) {
        if let Some(msg_type) = msg["type"].as_str() {
            match msg_type {
                "trace:span" => {
                    if let Some(span) = msg.get("span") {
                        ...
                        if span["name"] == "execute_tool" {
                            if let (Some(id), Some(start)) = (span["id"].as_str(), span["startTime"].as_u64()) {
                ...
                "trace:span_update" => {
                    if let (Some(id), Some(end)) = (msg["spanId"].as_str(), msg["update"]["endTime"].as_u64()) {

...
pub fn create_router(app_state: Arc<AppState>) -> Router {
    ...
    let mut app = Router::new()
        ...
        // GAP-PERF-03: Default 60s timeout for standard API routes.
...
/// These are limited to read-only connectivity checks, the pairing handshake,
/// and read-only health/pending queries. All write operations require paired-device
/// validation (SEC-01).
fn build_remote_public_routes() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route(
            "/remote/pair",
            post(routes::remote::pair_device)
                .route_layer(axum::middleware::from_fn(middleware::auth_rate_limit::auth_brute_force_limiter)),
        )
        .route(
        ...
        // SEC-01: Destructive remote operations (halt, decide, trigger-test)
        // are now gated behind paired-device validation below.
        .merge(build_remote_paired_routes())
}

/// Remote routes requiring a valid paired-device ID header.
/// These endpoints perform destructive actions (halt agents, approve/reject oversight)
/// and must not be accessible to unauthenticated network clients (SEC-01).
fn build_remote_paired_routes() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route_layer(axum::middleware::from_fn(
            middleware::paired_device_guard,
        ))
}

/// Protected remote routes — require valid auth token.
/// Post-pairing operations: device management.
...
fn build_agent_routes() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route(
            "/",
            post(routes::agent::create_agent)
                .layer(axum::extract::DefaultBodyLimit::max(128 * 1024)), // 128 KiB
        )
        .route(
            "/{id}",
        ...
        .route(
            "/{id}/tasks",
            post(routes::agent::send_task).layer(axum::extract::DefaultBodyLimit::max(128 * 1024)), // 128 KiB
        )
        ...
        .route(
            "/{id}/mission",
            post(routes::agent::sync_mission)
...
fn build_engine_protected_routes(app_state: Arc<AppState>) -> Router<Arc<AppState>> {
    // GAP-PERF-03: LLM-bound routes get an extended 600s timeout to support
    // slower local model execution without hitting the global 60s default.
    ...
    Router::new()
        ...
        .route("/engine/ws", get(routes::ws::ws_handler).layer(llm_timeout_layer))
        .route("/engine/live-voice", get(routes::ws::live_voice_handler).layer(llm_timeout_layer))
        .route("/engine/transcribe", post(routes::audio::transcribe_audio).layer(llm_timeout_layer))
        .route("/engine/speak", post(routes::audio::text_to_speech).layer(llm_timeout_layer))
        .route(
        ...
        .route("/api/pull", post(routes::model_manager::ollama_proxy_pull).layer(llm_timeout_layer))
        .route_layer(axum::middleware::from_fn_with_state(
            app_state,
            middleware::auth::validate_token,
        ))

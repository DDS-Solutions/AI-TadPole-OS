...
pub fn create_router(app_state: Arc<AppState>) -> Router {
    ...
    let mut app = Router::new()
        ...
        // Timeout (increased to 600s to support slower local agent model execution)
...
fn build_memory_routes() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route("/graph", get(routes::memory::get_markdown_memory_graph_handler))
}
...
fn build_agent_routes() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route(
            "/missions/{id}/clone",
            post(routes::agent::clone_mission),
        )
...
fn build_oversight_routes() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route("/security/snapshot", get(routes::oversight::get_security_snapshot))
        .route(
            "/security/integrity",
...
fn build_engine_public_routes() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route("/engine/a2a/mailbox/send", post(routes::a2a::receive_envelope))
}

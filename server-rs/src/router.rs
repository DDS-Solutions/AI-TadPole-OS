//! Route Assembly — Main API router and state injection
//!
//! Orchestrates the high-speed mapping of REST and WebSocket handlers 
//! while enforcing the security policy via auth and rate-limiting layers.
//!
//! @docs ARCHITECTURE:Networking
//!
//! ### AI Assist Note
//! **Routing Hierarchy**: All API traffic is versioned under `/v1`. 
//! CORS/Auth layers wrap the entire stack. Add new protected routes to 
//! `protected_routes` to automatically inherit the neural-token check.
//!
//! ### 🔍 Debugging & Observability
//! - **Failure Path**: CORS field mismatch, missing `Authorization: Bearer <TOKEN>` on /v1, or malformed JSON payloads.
//! - **Telemetry Link**: Search for `[Router]` or `[Auth]` in `tracing` logs.
...
pub fn create_router(app_state: Arc<AppState>) -> Router {
    ...
    // 4. Resolve static file serving path.

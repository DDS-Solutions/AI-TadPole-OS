...
fn build_agent_routes() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route("/chat/completions", post(routes::agent::create_chat_completion))
...
async fn health_gate(
    ...
    if path != "/engine/health" && path != "/v1/engine/health" && path != "/metrics" && path != "/v1/engine/metrics" && state.health_state() == crate::types::SystemHealthState::Degraded {
        return Err(crate::error::AppError::DegradedState(
            "System is operating in a Degraded state due to a critical service failure.".to_string()
        ));
    }

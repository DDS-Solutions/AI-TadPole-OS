...
pub fn create_router(app_state: Arc<AppState>) -> Router {
    ...
    let protected_routes = Router::new()
        ...
        .route("/search/memory", get(routes::memory::global_search))
        .route(
            "/oversight/{id}/decide",
        ...
        .route(
            "/skills/register",
            post(routes::skills::register_capability),
        )
        .route(
            "/skills/mcp-tools/{name}/execute",
            post(routes::mcp::execute_mcp_tool),
        )
        .route("/skills/scripts/{name}", put(routes::skills::post_script))
        .route(
            "/skills/scripts/{name}",
        ...
        .route("/skills/hooks/{name}", put(routes::skills::post_hook))
        .route(
            "/skills/hooks/{name}",
    ...
    let mut app = Router::new()
        ...
        // Enable gzip/brotli HTTP compression for JSON & static assets

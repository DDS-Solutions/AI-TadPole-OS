//! System Startup — Kernel boot sequence and dependency check
//!
//! Manages the transition from a static process to a living, autonomous swarm. 
//! Handles environment validation, telemetry setup, and background task spawning.
//!
//! @docs ARCHITECTURE:Networking
//!
//! ### AI Assist Note
//! **The Boot Sequence**: Orchestrates the system's thermal ramp-up. Based on 
//! `BootstrapIntent`, it warms up the **Hydra-RS Code Graph**, 
//! **mDNS Swarm Discovery**, and the **Heartbeat Loop**, which drives 
//! the frontend's "God View" telemetry.
//!
//! ### 🔍 Debugging & Observability
//! - **Failure Path**: `.env.schema` validation failure, mDNS port conflict, or Hydra-RS scan panic.
//! - **Telemetry Link**: Search for `[Bootstrap]` or `[Hydra-RS]` in `tracing` logs.
...
pub async fn spawn_background_tasks(app_state: Arc<AppState>, intent: BootstrapIntent) {
    ...
    // 8. Launch Telemetry Metric Aggregator (p95 Metrics)
    ...
    // 9. Launch Debounced Token Usage Flush (every 10 seconds)
    ...
    // 10. Launch High-Speed Swarm Pulse (100ms) - Only in Full Mode
    if intent == BootstrapIntent::Full {
...
mod tests {
    ...
    async fn test_fast_path_branching() {
        ...
        assert!(snapshot.get("CodeGraph").is_none());

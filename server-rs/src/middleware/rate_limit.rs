...
pub async fn inject_rate_limit_headers(req: Request<Body>, next: Next) -> Result<Response, StatusCode> {
    let client_ip = req
        .extensions()
    ...
    {
        let mut entry = RATE_BUCKETS.entry(client_ip.clone()).or_insert((*MAX_TOKENS, now));
        ...
        *last_refill = now;
        ...
        if *tokens < 1.0 {
            tracing::warn!("🚫 [Security] Rate limit exceeded for IP: {}. Blocked.", client_ip);
    ...
    headers.insert("X-RateLimit-Limit", (*MAX_TOKENS).to_string().parse().unwrap());
    headers.insert("X-RateLimit-Remaining", (current_tokens as u32).to_string().parse().unwrap());

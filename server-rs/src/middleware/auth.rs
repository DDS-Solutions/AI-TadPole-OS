...
pub async fn validate_token(
    ...
    if let Some(auth_str) = auth_header {
        if let Some(token) = auth_str.strip_prefix("Bearer ") {
            // Guard against empty token bypass (e.g. if deploy_token is empty/unset or input token is empty)
            if !token.is_empty()
    ...
    if is_ws_upgrade {
        ...
        for protocol in proto_header.split(',') {
            ...
            if let Some(token) = protocol.strip_prefix("bearer.") {
                has_bearer = true;
                // Guard against empty token bypass
                if !token.is_empty()
                    && !state.security.deploy_token.is_empty()
                    && constant_time_eq(token.as_bytes(), state.security.deploy_token.as_bytes())
                {
        ...
        if has_bearer {
            if is_bearer_valid {
                ...
                tracing::warn!("🚫 Unauthorized WebSocket upgrade: invalid bearer subprotocol token");
                return Err(StatusCode::UNAUTHORIZED);
            }
        } else {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_auth_websocket_protocol_post_connect() {
        let state = Arc::new(AppState::new_minimal_mock().await);
        let app = Router::new()
            .route("/", get(dummy_handler))
            .layer(from_fn_with_state(state, validate_token));

        // No bearer token in protocol (post-connect auth mode)
        let req = Request::builder()
            .uri("/")
            .header(header::UPGRADE, "websocket")
    ...
    S: Send + Sync,
{
    ...
    async fn from_request_parts(
        ...
        if let Some(auth_str) = auth_header {
            if let Some(token) = auth_str.strip_prefix("Bearer ") {
                if !token.is_empty()
                    && !app_state.security.admin_token.is_empty()
        ...
        Err(crate::error::AppError::Unauthorized("Administrative privileges required".to_string()))
    }
}

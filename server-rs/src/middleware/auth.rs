...
/// 
/// This implementation ensures that the execution time is deterministic 
/// relative to the input length by:
/// 1. Iterating through the entire input regardless of mismatches.
/// 2. Accumulating differences via XOR (`|= a[i] ^ b[i]`).
/// 3. Hiding whether the lengths matched until the final bitwise reduction.

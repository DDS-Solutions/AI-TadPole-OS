...
//! - **Local Errors**: none
//! - **Telemetry Targets**: `[Engine]`, `[Auth]`, `[Database]`, `[Registries]`, `[Engines]`, `[Hubs]`, `[Registry]`, `[Evolution]`, `[State]`, `[System]`
...
impl AppState {
    ...
    pub async fn new() -> Result<Self, AppError> {
        ...
        let audit_for_boot = security.audit_trail.clone();
        tokio::spawn(async move {
            match audit_for_boot.verify_last_n(100, None).await {
                Ok((verified, total)) => {
                    if total > 0 && verified == total {
                        ...
                        tracing::info!(
                            "🛡️ [Audit Ledger] Boot integrity verified (ledger is empty)."
                        );
                    }
                }
                Err(e) => {

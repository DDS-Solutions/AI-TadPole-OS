...
//!
//! ### AI Assist Note
//! **The Sovereign State**: This is the engine's core memory. It manages the 
//! **Telemetry Hub**, **Agent Registry**, and **Resource Pool**. All 
//! asynchronous workers MUST hold an `Arc<AppState>` to remain synchronized 
//! with the global swarm state.
//!
//! ### 🔍 Debugging & Observability
//! - **Failure Path**: Double-locking `parking_lot::RwLock` in nested callbacks, or DB pool exhaustion.
//! - **Telemetry Link**: Search for `[Engine]` or `[State]` in `tracing` logs.
//! - **Trace Scope**: `server-rs::state`
...
impl AppState {
    ...
    pub async fn new() -> anyhow::Result<Self> {
        ...
        let deploy_token = std::env::var("NEURAL_TOKEN").map_err(|_| {
            if cfg!(test) {
                anyhow::anyhow!("ci-test-token-placeholder")
            } else {
                anyhow::anyhow!("🚨 FATAL: NEURAL_TOKEN environment variable MUST be set for the engine to start.")
            }
        })?;
        ...
        let pool = match crate::db::init_db(&database_url).await {
            ...
            Err(e) => {
                tracing::error!("🚨 [Database] FATAL: Failed to initialize database pool at {}: {:?}", database_url, e);
        ...
        // Initialize Engines
        tracing::info!("🚀 [Engines] Initializing HTTP Client and VAD...");
        ...
        let audio_cache = match crate::agent::audio_cache::BunkerCache::new(audio_cache_path.clone()).await {
        ...
        let security = Arc::new(SecurityHub {
            ...
            shell_scanner: Arc::new(crate::security::scanner::ShellScanner::new(secret_redactor.clone())),
            secret_redactor,
            system_monitor: Arc::new(crate::security::monitoring::SecurityMonitor::new()),
            deploy_token,
        });

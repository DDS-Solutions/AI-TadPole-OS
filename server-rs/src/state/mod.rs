...
impl AppState {
    ...
    pub async fn new() -> Result<Self, AppError> {
        ...
        let admin_token = std::env::var("NEURAL_ADMIN_TOKEN")
            .or_else(|_| std::env::var("ADMIN_TOKEN"))
            .unwrap_or_else(|_| deploy_token.clone());
    ...
    pub fn health_state(&self) -> crate::types::SystemHealthState {
        ...
        let has_network = self.resources.initialization_registry.contains_key("Network");
        let has_codegraph = self.resources.initialization_registry.contains_key("CodeGraph");
        ...
        for sub in critical_subsystems {
            match self.resources.initialization_registry.get(sub) {
                Some(status) => match status.value() {
                    ...
                    crate::types::SubsystemStatus::Warming(_) | crate::types::SubsystemStatus::NotStarted => {
                        warming = true;
                    }

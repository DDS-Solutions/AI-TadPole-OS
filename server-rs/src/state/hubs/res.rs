...
impl ResourceHub {
    ...
    pub async fn get_knowledge_store(
        ...
        let ks = self
            .knowledge_store
            .get_or_try_init(|| async {
                let store = crate::agent::knowledge_store::KnowledgeStore::new(
                    self.pool.clone(),
                );
    ...
    pub async fn get_blueprint(&self, workspace_root: std::path::PathBuf) -> Result<Arc<crate::services::blueprint_service::Blueprint>, crate::error::AppError> {
        ...
        let blueprint = self
            .blueprint_cache
            .get_or_try_init(|| async {
                let blueprint_svc = crate::services::blueprint_service::BlueprintService::new(workspace_root);
                let blueprint = blueprint_svc.scan_workspace().await.map_err(|e| crate::error::AppError::InternalServerError(format!("Workspace blueprint scan failed: {}", e)))?;

            ...
            Err(err) => match err.downcast::<sqlx::Error>() {
                ...
                Err(err) => match err.downcast::<serde_json::Error>() {
                    ...
                    Err(err) => {
                        ...
                        } else if err_str.contains("Unauthorized") || err_str.contains("unauthorized") {
...
mod tests {
    ...
    fn test_constructor_sanitization() {
        ...
        if let AppError::DomainError { detail, .. } = &err {
            assert!(!detail.contains("<script>"), "Detail should be HTML-sanitized at construction");
    ...
    fn test_anyhow_downcast() {
        ...
        let io_err = anyhow::anyhow!(std::io::Error::new(std::io::ErrorKind::NotFound, "file missing"));

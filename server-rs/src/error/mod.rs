...
const ALLOWED_HELP_PREFIXES: &[&str] = &[
    "https://tadpole.os/",
    "https://docs.tadpole.os/",
...
impl ProblemDetails {
    /// Centralized builder for ProblemDetails that enforces secret redaction,
    /// HTML sanitization, bounded truncation, and internal error masking.
    /// Invariant: 1) Redact raw secrets -> 2) HTML sanitize -> 3) Bounded truncation <= 2048 bytes.
    pub fn build(
        status: StatusCode,
        slug: &str,
        detail: String,
        ...
        // Enforce ordering: Redact raw secrets first, sanitize HTML entities, then enforce byte cap
        let scrubbed = crate::utils::security::redact_secrets(&detail_to_process);
        let sanitized = sanitize_html(&scrubbed);
        let truncated = safe_truncate_bytes(&sanitized, 2048);
        ...
        Self {
            ...
            detail: truncated,
            instance,
            error_code: error_code.or_else(|| Some(slug.to_uppercase().replace(['-', ':'], "_"))),
            help_link: safe_help_link,
            severity,
        }
    }

    /// Creates a new ProblemDetails instance configured from status and title.
    pub fn new(status: StatusCode, title: &str, detail: String) -> Self {
        ...
        Self::build(
            ...
            None,
            severity,
            Some(slug.to_uppercase().replace(['-', ':'], "_")),
...
impl ProviderId {
    pub fn from_name(name: &str) -> Self {
        match name.to_ascii_lowercase().trim() {
            ...
            "xai" => ProviderId::Xai,
            "ollama" | "ollama-cloud" => ProviderId::Ollama,
            "mcp" => ProviderId::Mcp,
            "audio" => ProviderId::Audio,
            "runner" => ProviderId::Runner,
            other => {
                tracing::warn!(provider = %other, "Unrecognized provider identifier fallback to System");
                ProviderId::System
            }
        }
    }
...
pub enum AppError {
    ...
    #[error("Workflow Step Failed ({step_name}): {detail}")]
    WorkflowStepFailed {
        step_name: String,
        detail: String,
...
impl AppError {
    /// Attempts to downcast a generic `anyhow::Error` into a specific typed `AppError` variant (PD-007).
    pub fn from_anyhow_downcast(err: anyhow::Error) -> Self {
        let err_str = err.to_string();
        match err.downcast::<std::io::Error>() {
            Ok(io_err) => AppError::Io(io_err),
            Err(err) => match err.downcast::<sqlx::Error>() {
                Ok(sql_err) => AppError::Sqlx(sql_err),
                Err(err) => match err.downcast::<serde_json::Error>() {
                    Ok(serde_err) => AppError::Serde(serde_err),
                    Err(err) => {
    ...
    /// Creates a domain error, keeping the detail raw for central sanitization and logging (GAP-ERR-01).
    ...
    /// Creates an infrastructure error, keeping the detail raw for central sanitization and logging (GAP-ERR-01).
    ...
    pub fn is_retryable(&self) -> bool {
        match self {
            ...
            AppError::Reqwest(e) => {
                // Retry only on connection failures or timeouts to prevent replay hazards on non-idempotent operations
...
mod tests {
    ...
    fn test_error_slug_generation() {
        ...
        assert_eq!(
            AppError::OllamaOffline("offline".to_string())
                .resolve_metadata()
                .error_code,
    ...
    #[test]
    fn test_error_registry_matches_stable_metadata_codes() {
        let registry: serde_json::Value =
            serde_json::from_str(include_str!("../../../docs/ERROR_REGISTRY.json")).unwrap();
        let expected_codes = [
            "BAD_REQUEST",
            "UNAUTHORIZED",
            "FORBIDDEN",
        ...
        for code in expected_codes {
            assert!(
                ...
                "Missing registry entry: {code}"
            );
        }

        let internal = AppError::InternalServerError("hidden".to_string()).resolve_metadata();
        assert_eq!(internal.type_slug, "internal");
        assert_eq!(
            internal.error_code.as_deref(),
            Some("INTERNAL_SERVER_ERROR")
        );

        let domain = AppError::new_domain_error(
            DomainCode::InvalidConfiguration,
            "invalid".to_string(),
            None,
        )
        .resolve_metadata();
        assert_eq!(
            domain.error_code.as_deref(),
            Some("DOMAIN_ERROR:INVALID_CONFIGURATION")
        );
    }
    ...
    fn test_percent_encoding_slugs() {
        ...
        assert!(pd.type_uri.contains("https://tadpole.os/errors/"));
        assert!(pd.type_uri.contains("%26")); // '&'
        assert!(pd.type_uri.contains("%40")); // '@'
        assert!(pd.type_uri.contains("%23")); // '#'
    }

    #[test]
    fn test_max_length_truncation() {
        let long_detail = "a".repeat(3000);
        let pd = ProblemDetails::new(StatusCode::BAD_REQUEST, "Bad Request", long_detail.clone());
        assert_eq!(pd.status, 400);
        assert_eq!(pd.detail.len(), 2048);
        assert!(pd.detail.ends_with("..."));

        let emoji_detail = "🌟".repeat(1000); // 4000 bytes
        let emoji_pd = ProblemDetails::new(StatusCode::BAD_REQUEST, "Bad Request", emoji_detail);
        assert!(emoji_pd.detail.len() <= 2048);
        assert!(emoji_pd.detail.ends_with("..."));
    ...
    #[test]
    fn test_expanding_entity_truncation_cap() {
        // Thousands of & characters which expand from 1 byte to 5 bytes each (&amp;)
        let expanding = "&".repeat(3000);
        let pd = ProblemDetails::new(StatusCode::BAD_REQUEST, "Bad Request", expanding);
        assert!(
            pd.detail.len() <= 2048,
            "Detail length was {} > 2048",
            pd.detail.len()
        );
        assert!(pd.detail.ends_with("..."));
    }
    ...
    #[test]
    fn test_redaction_before_sanitization() {
        let sensitive = "token with key sk-1234567890abcdef1234567890abcdef and & <tag>";
        let pd = ProblemDetails::new(
            StatusCode::BAD_REQUEST,
            "Bad Request",
            sensitive.to_string(),
        );
        assert!(pd.detail.contains("[REDACTED]"));
        assert!(!pd.detail.contains("sk-1234567890abcdef"));
        assert!(pd.detail.contains("&amp;"));
        assert!(pd.detail.contains("&lt;tag&gt;"));
    }

    #[test]
    fn test_error_redaction_in_response() {
        ...
        assert_eq!(pd.status, 400);
        assert!(pd.detail.contains("[REDACTED]"));
        assert!(!pd.detail.contains("sk-1234567890"));
        assert_eq!(pd.severity, Severity::Error);
    }
    ...
    fn test_problem_details_structure() {
        ...
        assert_eq!(pd.type_uri, "https://tadpole.os/errors/not-found");
        assert_eq!(pd.error_code, Some("NOT_FOUND".to_string()));
        assert_eq!(pd.severity, Severity::Error);
    }

    #[test]
    fn test_problem_details_response_headers() {
        let pd = ProblemDetails::new(
            StatusCode::NOT_FOUND,
            "Not Found",
            "Item not found".to_string(),
    ...
    async fn test_is_retryable() {
        ...
        let err_conn = client.get("http://127.0.0.1:1").send().await.unwrap_err();
    ...
    fn test_html_sanitization_in_response() {
        ...
        assert!(!pd.detail.contains("<script>"));
        assert!(pd.detail.contains("&lt;script&gt;"));
        assert!(pd.detail.contains("&amp;"));
    }
    ...
    fn test_help_link_validation() {
        ...
        // Safe link with uppercase prefix
        let err_safe_upper = AppError::new_infrastructure_error(
            ProviderId::Gemini,
            InfrastructureErrorKind::ApiError,
            "error".to_string(),
            Some("https://Docs.Tadpole.OS/troubleshooting".to_string()),
        );
        let meta_safe_upper = err_safe_upper.resolve_metadata();
        assert_eq!(
            meta_safe_upper.help_link,
            Some("https://Docs.Tadpole.OS/troubleshooting".to_string())
        );
    ...
    #[test]
    fn test_no_double_html_escaping() {
        // GAP-ERR-01: Ensure raw storage and single-pass sanitization
        ...
        if let AppError::DomainError { detail, .. } = &err {
            assert_eq!(
                ...
                "Detail should remain raw in enum for clean display and logging"
            );
        } else {
            panic!("Expected AppError::DomainError");
        }

        let metadata = err.resolve_metadata();
        ...
        assert!(!pd.detail.contains("<script>"));
        // Check for 5xx masking
        assert_eq!(
            pd.detail,
            "An internal server error occurred. Please check system logs."
        );

        // 4xx domain error should sanitize once without double-escaping
        let bad_cfg = AppError::new_domain_error(
            DomainCode::InvalidConfiguration,
            "<script>bad</script>".to_string(),
            None,
        );
        let bad_cfg_meta = bad_cfg.resolve_metadata();
        let bad_cfg_pd = ProblemDetails::build(
            bad_cfg_meta.status_code,
            &bad_cfg_meta.type_slug,
            bad_cfg.to_string(),
            bad_cfg_meta.help_link,
            bad_cfg_meta.severity,
            bad_cfg_meta.error_code,
            None,
            None,
        );
        assert!(bad_cfg_pd
        ...
        assert!(!bad_cfg_pd.detail.contains("&amp;lt;"));
    }

    #[test]
    fn test_status_code_coverage_for_variants() {
        assert_eq!(
        ...
        assert_eq!(
            AppError::SecurityBoundaryViolation("outside workspace".to_string()).status_code(),
            StatusCode::FORBIDDEN
        );
        assert_eq!(
            AppError::DegradedState("degraded".to_string()).status_code(),
            StatusCode::SERVICE_UNAVAILABLE
        );
        assert_eq!(
            AppError::Conflict("conflict".to_string()).status_code(),
            StatusCode::CONFLICT
        );
        assert_eq!(
            AppError::InfrastructureError {
    ...
    fn test_anyhow_downcast() {
        // PD-007: Test Anyhow downcasting to typed variants
        let io_err = anyhow::anyhow!(std::io::Error::new(
            std::io::ErrorKind::NotFound,
            "file missing"
        ));
        let app_err = AppError::from_anyhow_downcast(io_err);
        assert!(matches!(app_err, AppError::Io(_)));

        let not_found_err = anyhow::anyhow!("Resource not found in registry");
        let app_err_nf = AppError::from_anyhow_downcast(not_found_err);
        assert!(matches!(app_err_nf, AppError::NotFound(_)));
    }
}

//! Multimedia processing endpoints for speech and transcription.
//!
//! Provides high-fidelity Text-to-Speech (TTS) via OpenAI/ElevenLabs, 
//! zero-latency streaming via Piper, and autonomous audio transcription 
//! via Groq's Whisper implementation.
...
pub async fn text_to_speech(
    ...
    let api_key = match std::env::var("OPENAI_API_KEY") {
        Ok(key) => key,
        Err(_) => {
            return Err(AppError::NotImplemented("High-tier TTS requires OPENAI_API_KEY. Defaulting to browser fallback.".to_string()));
        }
    };
    ...
    let res = client
        ...
        .send()
        .await
        .map_err(|e| AppError::InternalServerError(format!("Network Error: {}", state.security.secret_redactor.redact(&e.to_string()))))?;
    ...
    if res.status().is_success() {
        ...
        Err(AppError::InternalServerError(format!("TTS Provider Error: {}", safe_err)))
    }
}
...
pub async fn transcribe_audio(
    ...
    if engine == "local" {
        #[cfg(feature = "neural-audio")]
        {
            match state.resources.get_audio_engine().await.listen(audio_data).await {
                Ok(text) => {
                    return Ok(Json(json!({
                        "status": "success",
                        "text": text
                ...
                Err(e) => {
                    return Err(AppError::InternalServerError(format!("Local Whisper Error: {}", e)))
                }
            }
        }

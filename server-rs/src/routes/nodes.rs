...
pub async fn get_nodes(State(state): State<Arc<AppState>>) -> Result<Json<Vec<SwarmNode>>, AppError> {
    let nodes: Vec<SwarmNode> = state.registry.nodes.iter().map(|kv| kv.value().clone()).collect();
    Ok(Json(nodes))
}
...
pub async fn discover_nodes(State(state): State<Arc<AppState>>) -> Result<Json<serde_json::Value>, AppError> {

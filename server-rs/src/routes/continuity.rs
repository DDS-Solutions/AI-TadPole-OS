...
pub async fn create_workflow_handler(
    ...
    let engine = WorkflowEngine::new(state);
    let workflow = engine.create_workflow(req.name, req.description).await
        .map_err(|e| AppError::BadRequest(e.to_string()))?;
...
pub async fn add_workflow_step_handler(
    ...
    let engine = WorkflowEngine::new(state);
    let step = engine.add_step(&workflow_id, &req.agent_id, req.name, req.prompt_template, req.step_order)
        .await
        .map_err(|e| AppError::BadRequest(e.to_string()))?;

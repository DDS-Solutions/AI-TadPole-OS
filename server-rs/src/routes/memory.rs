...
pub async fn get_agent_memory(
    ...
    let path = db_path.ok_or_else(|| AppError::NotFound(format!("Memory for agent {} not found", agent_id)))?;
    ...
    match VectorMemory::connect(&path, "memories").await {
        Ok(memory) => {
            ...
            Err(AppError::InternalServerError("Failed to query memory store".to_string()))
        }
        Err(e) => {
            tracing::error!("Failed to connect to memory: {}", e);
            Err(AppError::InternalServerError(format!("Failed to connect to memory: {}", e)))
        }
    }
}
...
pub async fn delete_agent_memory(
    ...
    {
        if let Ok(table) = conn.open_table("memories").execute().await {
            table.delete(&format!("id = '{}'", row_id)).await
...
pub async fn save_agent_memory(
    ...
    let agent_ctx = state.registry.agents.get(&agent_id)
        .ok_or_else(|| AppError::NotFound(format!("Agent {} not found", agent_id)))?;

    // Use unified provider abstraction
    let runner = crate::agent::runner::AgentRunner::new(state.clone());
    let provider = runner.resolve_provider(&agent_ctx.resolve_provider_context(), (*state.resources.http_client).clone());
    let embedding = provider.embed(&payload.text).await
    ...
    match VectorMemory::connect(&path, "memories").await {
        Ok(memory) => {
            ...
            memory.add_memory(&id, &payload.text, "manual", embedding).await
                .map_err(|e| AppError::InternalServerError(format!("Failed to persist memory: {}", e)))?;
        ...
        Err(e) => {
            tracing::error!("Failed to connect to memory: {}", e);
            Err(AppError::InternalServerError("Failed to connect to memory store".to_string()))
        }
    }
}
...
pub async fn global_search(
    ...
    if params.query.is_empty() {
        return Ok((StatusCode::OK, Json(MemoryResponse { status: "success".to_string(), entries: vec![] })));
    }
    ...
    let provider = if let Some(agent_ctx) = state.registry.agents.iter().next() {
        runner.resolve_provider(&agent_ctx.resolve_provider_context(), (*state.resources.http_client).clone())
    } else {
        return Err(AppError::InternalServerError("No agents configured for search".to_string()));
    };

    let query_vec = provider.embed(&params.query).await
                        ...
                        for agent in agent_entries.flatten() {
                            ...
                            if db_path.exists() {
                                if let Ok(mem) = VectorMemory::connect(&db_path.to_string_lossy(), "memories").await {
                                    if let Ok(hits) = mem.search_knowledge(query_vec.clone(), 3).await {
                                        for hit in hits {

...
impl TryFrom<&arrow_array::RecordBatch> for Vec<MemoryEntry> {
    ...
    fn try_from(batch: &arrow_array::RecordBatch) -> Result<Self, Self::Error> {
        ...
        let mission_col = batch
            ...
            .ok_or_else(|| {
                AppError::InternalServerError(
                    "Missing 'mission_id' column in memory batch".to_string(),
                )
            })?;
        ...
        let ts_col = batch
            ...
            .ok_or_else(|| {
                AppError::InternalServerError(
                    "Missing 'timestamp' column in memory batch".to_string(),
                )
            })?;
...
pub async fn get_agent_memory(
    ...
    {
        let memory_path =
            resolve_agent_memory_path(&state.base_dir, &agent_id).ok_or_else(|| {
                        ...
                        if let Ok(mut results) = table.query().limit(100).execute().await {
                            ...
                            while let Some(batch_result) = results.next().await {
                                if let Ok(batch) = batch_result {
                                    if let Ok(mut batch_entries) =
                                        Vec::<MemoryEntry>::try_from(&batch)
                                    {

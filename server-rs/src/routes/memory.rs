...
pub async fn get_markdown_memory_graph_handler() -> Result<impl IntoResponse, AppError> {
    ...
    let graph = crate::intelligence::markdown_graph::MarkdownMemoryGraph::build_from_directory(&memory_dir)
        .map_err(|e| AppError::InternalServerError(format!("Failed to build memory graph: {}", e)))?;

//! Standardized error handling and RFC 9457 compliance layer.
//!
//! This module implements the `AppError` type, which ensures consistent 
//! JSON error responses across the entire Tadpole OS API. It integrates 
//! with the `SecretRedactor` to prevent PII/API key leakage in log traces.
...
impl IntoResponse for AppError {
    fn into_response(self) -> Response {
        let (status, title, detail, type_uri) = match self {
            AppError::BadRequest(d) => (StatusCode::BAD_REQUEST, "Bad Request", d, "https://tadpole.os/errors/bad-request"),
            AppError::NotFound(d) => (StatusCode::NOT_FOUND, "Not Found", d, "https://tadpole.os/errors/not-found"),
            AppError::Unauthorized(d) => (StatusCode::UNAUTHORIZED, "Unauthorized", d, "https://tadpole.os/errors/unauthorized"),
            AppError::Forbidden(d) => (StatusCode::FORBIDDEN, "Forbidden", d, "https://tadpole.os/errors/forbidden"),
            AppError::ValidationError(d) => (StatusCode::UNPROCESSABLE_ENTITY, "Validation Error", d, "https://tadpole.os/errors/validation"),
            ...
            AppError::NotImplemented(d) => (StatusCode::NOT_IMPLEMENTED, "Not Implemented", d, "https://tadpole.os/errors/not-implemented"),
            AppError::Anyhow(d) => (
                StatusCode::INTERNAL_SERVER_ERROR,
                "Internal Server Error",
                format!("{}", d),

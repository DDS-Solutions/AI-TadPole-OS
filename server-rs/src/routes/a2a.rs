...
/// Authenticates the sender cryptographically using X-A2A-Signature.
...
mod tests {
    ...
    use crate::agent::runner::tools::capability::{sign_a2a_envelope, set_key, reset_keyring_for_test};
    ...
    async fn test_receive_envelope_signature_validation() {
        ...
        let req = Request::builder()
            .method("POST")
            .uri("/engine/a2a/mailbox/send")
            .header("X-A2A-Signature", "test_a2a_key:00112233445566778899aabbccddeeff00112233445566778899aabbccddeeff")
            .header("Content-Type", "application/json")
            .body(Body::from(payload_str.clone()))
            .unwrap();
        let response = app.clone().oneshot(req).await.unwrap();

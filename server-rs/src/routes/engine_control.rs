...
pub async fn kill_agents(State(state): State<Arc<AppState>>) -> Result<impl IntoResponse, AppError> {
    ...
    let pending_ids: Vec<String> = state.comms
        .oversight_queue
        .iter()
        .map(|e| e.key().clone())
        .collect();
...
pub async fn shutdown_engine(State(state): State<Arc<AppState>>) -> Result<impl IntoResponse, AppError> {

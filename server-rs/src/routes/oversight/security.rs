...
pub async fn update_policy(
    ...
    if let Err(e) = state
        .security
        .audit_trail
        .record("admin", None, None, "[POLICY_UPDATE]", &audit_params)
        .await
    {
        tracing::error!(
            "🚨 Failed to record [POLICY_UPDATE] in audit trail: {:?}",
            e
        );
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_get_audit_trail_pagination_and_verification() {
        let state = Arc::new(AppState::new_minimal_mock().await);

        // Record policy update and quota update into audit trail
        state
            .security
            .audit_trail
            .record(
                ...
                None,
                "[POLICY_UPDATE]",
                "{\"tool_name\":\"write_file\",\"mode\":\"allow\"}",
            )
            .await
            .unwrap();
        ...
        state
            ...
            .record(
                ...
                None,
                "[QUOTA_UPDATE]",
                "{\"entity_id\":\"agent-1\",\"budget_usd\":50.0}",
            )
            .await
            .unwrap();
        ...
        let res = get_audit_trail(State(state), Query(params))
            .await
        ...
        let items = parsed["data"].as_array().expect("Expected data array");
        assert_eq!(items.len(), 2);
        assert!(items[0]["is_verified"].as_bool().unwrap());
        assert!(items[1]["is_verified"].as_bool().unwrap());
    }
}

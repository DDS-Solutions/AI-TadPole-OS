...
pub async fn trigger_deploy(
    ...
    // --- Async Process Execution with 5-minute Timeout ---
    ...
    match result {
        ...
        Err(_) => {
            tracing::error!("❌ Deployment script execution timed out after 5 minutes");
            Ok((
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(DeployResponse {
                    status: "error".to_string(),
                    output: None,
                    error: Some("Deployment script execution timed out after 5 minutes.".to_string()),
                }),
            ))
        }
    }

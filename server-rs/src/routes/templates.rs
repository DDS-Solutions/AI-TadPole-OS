...
pub async fn install_template(
    ...
    if mcps_src.exists() {
        if let Ok(content) = tokio::fs::read_to_string(&mcps_src).await {
            if let Ok(incoming_config) = serde_json::from_str::<crate::agent::mcp::McpConfig>(&content) {
                let mcp_config_path = PathBuf::from(".agent/mcp_config.json");
                let mut current_config = if mcp_config_path.exists() {
                    tokio::fs::read_to_string(&mcp_config_path).await
                        .ok()
                        .and_then(|c| serde_json::from_str::<crate::agent::mcp::McpConfig>(&c).ok())
                        .unwrap_or_else(|| crate::agent::mcp::McpConfig { mcp_servers: std::collections::HashMap::new() })
                } else {
                    crate::agent::mcp::McpConfig { mcp_servers: std::collections::HashMap::new() }
                };

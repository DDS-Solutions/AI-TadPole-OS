...
pub async fn install_template(
        ...
        .await
    {
        Ok(s) => s,
        Err(e) => {
            return Err(AppError::InternalServerError(format!("Failed to execute git: {}", e)));
        }
    };

    if !status.success() {
        let _ = tokio::fs::remove_dir_all(&temp_dir).await;
        return Err(AppError::InternalServerError("Failed to clone template repository".to_string()));
    }

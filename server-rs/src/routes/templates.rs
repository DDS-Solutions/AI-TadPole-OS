        ...
        if let Some(ks) = state.resources.get_knowledge_store().await.ok() {
            ...
            if knowledge_json_src.exists() {
                if let Ok(content) = tokio::fs::read_to_string(&knowledge_json_src).await {
                    if let Ok(requests) = serde_json::from_str::<Vec<crate::agent::knowledge_store::AddKnowledgeRequest>>(&content) {
                        for req in requests {
                ...
                if let Ok(mut entries) = tokio::fs::read_dir(&knowledge_dir).await {
                    while let Ok(Some(entry)) = entries.next_entry().await {
                        ...
                        if path.is_file() && path.extension().and_then(|s| s.to_str()) == Some("md") {
                            if let Ok(content) = tokio::fs::read_to_string(&path).await {
                                let (fm_title, fm_uri, fm_tags, fm_desc, body_content) = parse_frontmatter(&content);
                                ...
                                let req = crate::agent::knowledge_store::AddKnowledgeRequest {
                                    ...
                                    title: Some(title),
                                    description: fm_desc.or_else(|| Some(format!("Imported from template {}", payload.path))),
                                    resource_uri: fm_uri,
...
#[cfg(feature = "vector-memory")]
fn parse_frontmatter(content: &str) -> (Option<String>, Option<String>, Option<String>, Option<String>, String) {
    ...
    for line in frontmatter_text.lines() {
        ...
        if let Some((key, val)) = line.split_once(':') {
            ...
            let clean_val = if (val.starts_with('"') && val.ends_with('"')) || (val.starts_with('\'') && val.ends_with('\'')) {

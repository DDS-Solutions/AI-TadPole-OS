...
pub async fn get_templates_catalog(
    ...
    let response = match state.resources.http_client.get(url).send().await {
        Ok(res) => res,
        Err(e) => {
            tracing::warn!("⚠️ [Templates] Failed to fetch remote template catalog: {:?}", e);
            let offline_catalog = vec![
                TemplateCatalogEntry {
                    id: "marketing-swarm".to_string(),
                    name: "Marketing Swarm (Offline Fallback)".to_string(),
                    description: "Offline fallback for the marketing automation swarm. Run locally.".to_string(),
                    repository_url: "https://github.com/DDS-Solutions/AI-Tadpole-OS-Industry-Templates.git".to_string(),
    ...
    let catalog = response
        ...
        .map_err(|e| {
            AppError::InternalServerError(format!(
                "Failed to parse template catalog JSON: {}",
                e
            ))
        })?;
...
pub async fn install_template(
            ...
            while let Ok(Some(entry)) = entries.next_entry().await {
                if let Ok(file_type) = entry.file_type().await {
                    if !file_type.is_symlink() && file_type.is_file() {
                        ...
                        let dest_file = crate::utils::security::validate_path(&agents_dest, &file_name.to_string_lossy())?.to_path_buf();
                        if dest_file.exists() {
    ...
    if workflows_src.exists() {
        ...
        if let Ok(mut entries) = tokio::fs::read_dir(&workflows_src).await {
            while let Ok(Some(entry)) = entries.next_entry().await {
                if let Ok(file_type) = entry.file_type().await {
                    if !file_type.is_symlink() && entry.path().extension().and_then(|s| s.to_str()) == Some("md") {
                        let file_name = entry.file_name();
                        let dest_file = crate::utils::security::validate_path(&workflows_dest, &file_name.to_string_lossy())?.to_path_buf();
                        if dest_file.exists() {
                            return Err(AppError::Forbidden(format!(
            ...
            while let Ok(Some(entry)) = entries.next_entry().await {
                if let Ok(file_type) = entry.file_type().await {
                    let ext = entry
                        ...
                        .unwrap_or("")
                        .to_lowercase();
                    if !file_type.is_symlink() && (ext == "json" || ext == "py" || ext == "js" || ext == "ts") {
                        let file_name = entry.file_name();
                        let dest_file = crate::utils::security::validate_path(&skills_dest, &file_name.to_string_lossy())?.to_path_buf();
                        if dest_file.exists() {
                            return Err(AppError::Forbidden(format!(

...
pub async fn list_mcp_tools(State(state): State<Arc<AppState>>) -> Result<impl IntoResponse, AppError> {
    // For the global list, we return all skills registered in the engine
    let all_agent_skills: Vec<String> = state.registry.capabilities.skills
        .iter()
        .map(|kv| kv.key().clone())
        .collect();

    let tools = state.registry
        .mcp_host
        .list_tools(&all_agent_skills, &state.registry.capabilities.skills);
...
pub async fn execute_mcp_tool(
    ...
    match state.registry.mcp_host.call_tool(&name, arguments, workspace_root, &state.registry.capabilities.skills).await {

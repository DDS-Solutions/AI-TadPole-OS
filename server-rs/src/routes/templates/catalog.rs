...
pub async fn fetch_catalog_from_url(
    ...
    tracing::info!(
        "📥 [Templates] Fetching template catalog from {}",
        catalog_url
    );
    ...
    let catalog_fut = async {
        ...
        if !res.status().is_success() {
            return Err(format!(
                "Remote catalog returned HTTP status {}",
                res.status()
            ));
        }
    ...
    match tokio::time::timeout(CATALOG_FETCH_TIMEOUT, catalog_fut).await {
        ...
        Ok(Err(err_msg)) => {
            tracing::warn!(
                "⚠️ [Templates] Remote catalog error: {}, serving offline fallback",
                err_msg
            );
            get_offline_catalog()
        }
...
pub(crate) mod tests {
    ...
    async fn test_fetch_catalog_falls_back_on_invalid_url() {
        ...
        let catalog =
            fetch_catalog_from_url(&client, "http://127.0.0.1:9/non_existent_url.json").await;

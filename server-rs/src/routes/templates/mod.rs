...
pub use installed::{archive_and_remove_swarm, list_installed_swarms, read_installed_manifest, write_installed_manifest};
pub use mcp_store::{merge_incoming_mcp_config, merge_mcp_config_data, prepare_incoming_mcp_config, prune_mcp_servers};
pub use naming::{apply_swarm_namespace, apply_workflow_namespace, get_installed_swarm_id, sanitize_agent_filename, sanitize_workflow_filename};
pub use validate::{is_allowed_skill_extension, validate_agent_payload, validate_template_assets, validate_workflow_content, ValidatedAgent, ValidatedTemplateAssets, ALLOWED_SKILL_EXTENSIONS};
...
async fn register_validated_agents(
    ...
    for agent in agents {
        ...
        state.registry.agents.insert(agent.agent.identity.id.clone(), ea_clone);
    }
    warnings
}
...
pub async fn install_template(
    ...
    // 1. Clone repository
    source::clone_template_repository(&payload.repository_url, payload.git_ref.as_deref(), &temp_dir).await?;

    // 2. Validate template path
    let source_path = match crate::utils::security::validate_path(&temp_dir, &payload.path) {
        Ok(p) => p,
        Err(e) => return Err(AppError::BadRequest(format!("Invalid template path: {}", e))),
    };
    if !source_path.exists() {
        return Err(AppError::NotFound(format!("Template path '{}' not found in repo", payload.path)));
    }
    ...
    for agent in &validated.agents {
        ...
        let replaced = transaction.write_new(&destination, &agent.serialized, payload.overwrite).await?;
        if replaced {
            replaced_agents += 1;
        }
    ...
    if let Some(ref swarm_manifest) = validated.swarm_manifest {
        let destination = workspace_root
            ...
            .join(&installed_id)
            .join("swarm.json");
        transaction.copy_new(swarm_manifest, &destination, payload.overwrite).await?;
    }
    ...
    for (filename, workflow_path) in &validated.workflows {
        let destination = workspace_root
            .join("directives")
            .join(filename);
        let replaced = transaction.copy_new(workflow_path, &destination, payload.overwrite).await?;
        if replaced {
    ...
    for skill in &validated.skills {
        ...
        let destination = workspace_root
            .join("execution")
            .join(skill_name);
        let replaced = transaction.copy_new(skill, &destination, payload.overwrite).await?;
        if replaced {
            replaced_skills += 1;
        }
    ...
    if let Some(ref merged_mcp_config) = validated.merged_mcp_config {
        ...
        transaction.replace_atomically(&mcp_path, merged_mcp_config).await?;
    }
    ...
    transaction.write_new(&manifest_dest, &manifest_bytes, true).await?;
    ...
    let receipt = InstallationReceipt {
        template_path: payload.path.clone(),
        source_revision,
        agents: InstallAssetReceipt::new(validated.agents.len(), validated.agents.len(), replaced_agents),
        workflows: InstallAssetReceipt::new(validated.workflows.len(), validated.workflows.len(), replaced_workflows),
        skills: InstallAssetReceipt::new(validated.skills.len(), validated.skills.len(), replaced_skills),
        swarm_manifest: InstallAssetReceipt::complete(usize::from(validated.swarm_manifest.is_some())),
        mcp_servers: InstallAssetReceipt::new(validated.mcp_server_count, validated.mcp_server_count, validated.mcp_replaced_count),
    };
...
pub async fn import_template(
    ...
    let (merged_mcp_config, mcp_count, mcp_server_names, replaced_mcp) = if let Some(ref mcp_val) = payload.mcps {
        let incoming: crate::agent::mcp::McpConfig =
            serde_json::from_value(mcp_val.clone()).map_err(|error| {
                AppError::BadRequest(format!("Invalid incoming MCP configuration: {}", error))
            })?;
        let (bytes, count, names, replaced) =
            mcp_store::merge_incoming_mcp_config(&incoming, &workspace_root, payload.overwrite).await?;
    ...
    for agent in &validated_agents {
        ...
        let replaced = transaction.write_new(&agent_path, &agent.serialized, payload.overwrite).await?;
        if replaced {
            replaced_agents += 1;
        }
    }
    ...
    for (filename, content) in &validated_workflows {
        ...
        let replaced = transaction.write_new(&wf_path, content.as_bytes(), payload.overwrite).await?;
        if replaced {
            replaced_workflows += 1;
        }
    }
    ...
    if let Some(ref mcp_bytes) = merged_mcp_config {
        ...
        transaction.replace_atomically(&mcp_config_path, mcp_bytes).await?;
    }
    ...
    let swarm_dest = workspace_root
        ...
        .join(&installed_id)
        .join("swarm.json");
    transaction.write_new(&swarm_dest, &swarm_bytes, true).await?;
    ...
    let receipt = InstallationReceipt {
        ...
        agents: InstallAssetReceipt::new(validated_agents.len(), validated_agents.len(), replaced_agents),
        workflows: InstallAssetReceipt::new(validated_workflows.len(), validated_workflows.len(), replaced_workflows),
...
pub async fn uninstall_template(
    ...
    if !tokio::fs::try_exists(&installed_dir).await.map_err(AppError::Io)? {
        return Err(AppError::NotFound(format!("Installed swarm '{}' not found", clean_id)));
    }

...
use super::naming::{
    apply_swarm_namespace, apply_workflow_namespace, sanitize_agent_filename, sanitize_error_str,
...
pub async fn validate_template_assets(
    ...
    let (merged_mcp_config, mcp_server_count, mcp_server_names, mcp_replaced_count) =
        super::mcp_store::prepare_incoming_mcp_config(source_path, workspace_root, allow_overwrite)
            .await?;
    ...
    if !allow_overwrite {
        ...
        for (filename, _) in &workflows {
            destinations.push(workspace_root.join("directives").join(filename));
        }
        for skill in &skills {
            destinations.push(
                workspace_root
...
mod tests {
    ...
    async fn test_validate_template_assets_collision_refusal_when_overwrite_false() {
        ...
        let ws_agent_dest = temp_workspace
            .path()
            .join("data/swarm_config/agents")
            .join("worker.json");
        tokio::fs::create_dir_all(ws_agent_dest.parent().unwrap())
            .await
            .unwrap();

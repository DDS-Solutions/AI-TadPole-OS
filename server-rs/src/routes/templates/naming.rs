...
pub fn apply_workflow_namespace(
    clean_base: &str,
    namespace: Option<&str>,
) -> String {
    let stem = clean_base
...
pub(crate) mod tests {
    ...
    fn test_sanitize_agent_filename() {
        ...
        assert_eq!(sanitize_agent_filename("path/to/my_agent.json").unwrap(), "my_agent");
    ...
    fn test_sanitize_workflow_filename() {
        assert_eq!(sanitize_workflow_filename("daily_sync.md").unwrap(), "daily_sync");
        assert_eq!(sanitize_workflow_filename("DAILY_SYNC.MD").unwrap(), "DAILY_SYNC");
        assert_eq!(sanitize_workflow_filename("../../../evil.md").unwrap(), "evil");
    ...
    fn test_apply_swarm_namespace() {
        ...
        let mut agent: crate::agent::types::EngineAgent = serde_json::from_value(agent_json).unwrap();

        let (id, filename) = apply_swarm_namespace(&mut agent, "dispatcher", Some("field_services"));

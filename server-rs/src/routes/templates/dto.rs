...
pub fn parse_model_provider(s: &str) -> Result<crate::agent::types::ModelProvider, AppError> {
    match s.trim().to_lowercase().as_str() {
        ...
        "ollama-cloud" | "ollama_cloud" | "ollamacloud" => {
            Ok(crate::agent::types::ModelProvider::OllamaCloud)
        }
...
pub(crate) mod tests {
    ...
    fn test_parse_model_provider() {
        assert_eq!(
            parse_model_provider("openai").unwrap(),
            crate::agent::types::ModelProvider::Openai
        );
        assert_eq!(
            parse_model_provider("openrouter").unwrap(),
            crate::agent::types::ModelProvider::Openrouter
        );
        assert_eq!(
            parse_model_provider("ollama").unwrap(),
            crate::agent::types::ModelProvider::Ollama
        );
        assert_eq!(
            parse_model_provider("google").unwrap(),
            crate::agent::types::ModelProvider::Google
        );
        assert_eq!(
            parse_model_provider("gemini").unwrap(),
            crate::agent::types::ModelProvider::Google
        );
        assert_eq!(
            parse_model_provider("ollama-cloud").unwrap(),
            crate::agent::types::ModelProvider::OllamaCloud
        );
    ...
    fn test_install_template_request_deserialization() {
        let json_data = r#"{"repository_url": "https://github.com/example/repo.git", "path": "templates/marketing", "git_ref": "v1.0.0", "model_override": {"provider": "ollama-cloud", "model_id": "gemma4:31b-cloud"}, "overwrite": true, "namespace": "mkt"}"#;

...
pub fn merge_mcp_config_data(
    ...
    for (server_name, server_config) in &incoming.mcp_servers {
        ...
        merged.mcp_servers.insert(server_name.clone(), server_config.clone());
    }
...
pub async fn prune_mcp_servers(
    ...
    let content = tokio::fs::read_to_string(&mcp_path).await.map_err(AppError::Io)?;
    ...
    let serialized = serde_json::to_vec_pretty(&config).map_err(|e| {
        AppError::InternalServerError(format!("Failed to serialize pruned MCP configuration: {}", e))
    })?;

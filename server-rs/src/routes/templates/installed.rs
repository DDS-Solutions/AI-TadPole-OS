...
pub async fn list_installed_swarms(
    ...
    if !tokio::fs::try_exists(&installed_root)
        .await
        .map_err(AppError::Io)?
    {
        return Ok(Vec::new());
    ...
    while let Some(entry) = entries.next_entry().await.map_err(AppError::Io)? {
        ...
        if tokio::fs::try_exists(&swarm_json_path)
            .await
            .map_err(AppError::Io)?
        {
...
pub async fn write_installed_manifest(
    ...
    tokio::fs::write(&manifest_dest, manifest_bytes)
        .await
        .map_err(AppError::Io)?;
    Ok(())
}
...
pub async fn archive_and_remove_swarm(
    ...
    for agent_id in &summary.agents {
        ...
        if let Some((_, runner)) = state.comms.active_runners.remove(&clean_agent_id) {
            runner.abort_handle.abort();
        }
        if let Err(err) =
            crate::agent::persistence::delete_agent_cascade(&state.resources.pool, &clean_agent_id)
                .await
        {
    ...
    tokio::fs::remove_dir_all(&installed_dir)
        .await
        .map_err(AppError::Io)?;
...
pub(crate) mod tests {
    ...
    async fn test_read_manifest_corrupt_fails_loudly() {
        ...
        tokio::fs::write(&manifest_path, b"{ invalid json")
            .await
            .unwrap();

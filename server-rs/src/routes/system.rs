...
#[tracing::instrument(skip(state), fields(trace_id = tracing::field::Empty, request_id = tracing::field::Empty, http_status = tracing::field::Empty), name = "system::compute_profile")]
...
fn resolve_workspace_dir(base_dir: &std::path::Path, source_uri: &str) -> Result<std::path::PathBuf, AppError> {
    let clean_uri = source_uri.trim_start_matches('/').trim_start_matches('\\');
    let rel_path = if clean_uri.contains("data/workspaces") {
        clean_uri.to_string()
    } else if clean_uri.contains("workspaces") {
        clean_uri.replace("workspaces/", "data/workspaces/")
    } else {
        format!("data/workspaces/{}", clean_uri)
    };

    let safe_path = crate::utils::security::validate_path(base_dir, &rel_path)?;
    Ok(safe_path.to_path_buf())
}

/// Retrieves the synchronization status and metrics for all workspaces/connectors.
#[tracing::instrument(skip(state), fields(trace_id = tracing::field::Empty, request_id = tracing::field::Empty, http_status = tracing::field::Empty), name = "system::workspaces_status")]
pub async fn get_workspaces_status(
    ...
    for m in manifests {
        ...
        if let Ok(workspace_dir) = resolve_workspace_dir(&state.base_dir, &m.source_uri) {
            ...
            tracing::warn!(
                "Skipping playbook mount for unauthorized workspace URI: {}",
                m.source_uri
            );
        }
...
#[tracing::instrument(skip(state), fields(trace_id = tracing::field::Empty, request_id = tracing::field::Empty, http_status = tracing::field::Empty), name = "system::workspace_files")]

//! Swarm Control & Agent Orchestration — API
//!
//! Orchestrates the management of autonomous agents and the dispatching of 
//! high-level tasks to the swarm. Includes registration, state 
//! synchronization, and mission lifecycle management.
//!
//! @docs ARCHITECTURE:Networking
//!
//! ### AI Assist Note
//! **Async Task Dispatch**: `send_task` returns `202 ACCEPTED` immediately 
//! after spawning a `tokio::spawn` task runner. Agents should monitor 
//! real-time telemetry (WebSockets) or polling for task completion status. 
//! Ensure `traceparent` headers are propagated for consistent observability.

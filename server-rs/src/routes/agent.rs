    ...
    F: FnOnce(&mut EngineAgent),
{
    ...
    // Optimistic Concurrency: Increment version in memory but save with old version to DB
    let old_version = agent.version;
    agent.version += 1;

    // Sync to DB
    let mut agent_clone = agent.clone();
    agent_clone.version = old_version; // The DB query binds this to the WHERE clause

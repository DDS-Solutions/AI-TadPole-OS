...
#[tracing::instrument(skip(state), name = "agent_registry::get_all")]
...
#[tracing::instrument(skip(state, new_agent), fields(agent_id = %new_agent.id), name = "agent_registry::create")]
...
pub async fn update_agent(
    ...
    if let Some(m2) = update.model_2 {
        entry.model_2 = Some(m2);
    }
    if let Some(m3) = update.model_3 {

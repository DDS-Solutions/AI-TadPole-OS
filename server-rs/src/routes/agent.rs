    ...
    F: FnOnce(&mut EngineAgent),
{
    ...
    tokio::spawn(async move {
        let agents: Vec<EngineAgent> = state_ref
            ...
            .collect();
        if let Err(e) = crate::agent::persistence::save_agents_json(&state_ref.base_dir, agents).await {
            tracing::error!("⚠️ [Gateway] Failed to update agents.json asynchronously: {:?}", e);
        }
    });
...
pub async fn send_task(
    ...
    if let Some(req_id_val) = headers.get("x-request-id").and_then(|v| v.to_str().ok()) {
        ...
        // Clean up expired entries (>30 seconds old) amortized to avoid per-request latency spikes
        if state.comms.recent_requests.len() > 100 {
...
pub async fn create_chat_completion(
    ...
    let run_res = match runner.run(agent_id.clone(), payload).await {
        Ok(res) => res,
        Err(err) => {
...
async fn reset_agent_to_idle_safe(state: &Arc<AppState>, agent_id: &str) {
    let mut clone = state.registry.agents.get(agent_id).map(|e| e.value().clone());
...
mod tests {
    ...
    async fn test_list_agents_empty() {
        let state = Arc::new(AppState::new_minimal_mock().await);
        let result = get_agents(State(state), Query(crate::routes::pagination::PaginationParams::default())).await;
        assert!(result.is_ok());
    }
}

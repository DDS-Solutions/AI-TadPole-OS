...
pub async fn get_agents(
    ...
    let agents: Vec<EngineAgent> = state.registry.agents.iter().map(|kv| kv.value().clone()).collect();
    Ok(Json(PaginatedResponse::from_vec(agents, &params, "/v1/agents")))
}
...
pub async fn create_agent(
    ...
    state.registry.agents.insert(new_agent.id.clone(), new_agent.clone());
...
pub async fn update_agent(
    ...
    let mut entry = state.registry.agents.get_mut(&agent_id)
    ...
    if let Some(voice_id) = update.voice_id { entry.voice_id = Some(voice_id); }
    if let Some(voice_engine) = update.voice_engine { entry.voice_engine = Some(voice_engine); }
    if let Some(base_url) = update.base_url { entry.model.base_url = Some(base_url); }
...
pub async fn reset_agent(
    ...
    Ok(Json(serde_json::json!({ "status": "ok", "message": "Failure count reset." })))
}
...
pub async fn sync_mission(
    ...
    let mut agent = state.registry.agents.get_mut(&id)

...
pub async fn send_task(
    ...
    if payload.traceparent.is_none() {
        if let Some(tp) = headers.get("traceparent").and_then(|v| v.to_str().ok()) {
            ...
            if TRACEPARENT_REGEX.is_match(tp) {
                ...
                let sanitized_tp: String = tp.chars().filter(|c| c.is_ascii_alphanumeric() || *c == '-').take(64).collect();
                tracing::warn!("Blocked invalid traceparent header payload: {}", sanitized_tp);
            }
        }
    }
    ...
    match state.registry.agents.get(&agent_id) {
        ...
        Some(agent) => {
            ...
            if agent.economics.cost_usd >= agent.economics.budget_usd && agent.economics.budget_usd > 0.0 {
                return Err(AppError::BadRequest(format!(
    ...
    // Spawn Runner with AbortHandle registration (F-03)
    ...
    let join_handle = tokio::spawn(async move {
        ...
        state_clone.comms.active_runners.remove_if(&agent_id_for_spawn, |_, stored_runner| {
            stored_runner.task_id == task_id_clone
        });
    });
    ...
    state
        .comms
        .active_runners
        .insert(agent_id.clone(), crate::state::hubs::comm::RunnerHandle {
                ...
                if let Some(task) = agent.state.current_task.clone() {
                    if !task.is_empty() {
                        ...
                        if let Some(mut entry) = state.registry.agents.get_mut(&agent.identity.id) {
                            ...
                            if let Err(e) = crate::agent::persistence::save_agent_db(&state.resources.pool, a).await {
                                tracing::error!("❌ [State Recovery] Failed to reset agent {} to idle: {}", agent.identity.id, e);
                            }
                        }
                    }
                } else {

...
async fn spawn_app() -> (String, Arc<AppState>) {
    ...
    let app = Router::new()
        ...
        .route("/engine/live-voice", get(crate::routes::ws::live_voice_handler))
...
#[tokio::test]
async fn test_live_voice_authorized_success() {
    let (base_url, state) = spawn_app().await;
    let url = format!("{}/engine/live-voice", base_url);
    ...
    // Mock the GOOGLE_API_KEY environment variable so that the live voice socket connection
    // attempt doesn't fail during upgrade.
    std::env::set_var("GOOGLE_API_KEY", "mock-api-key");
    ...
    match result {
        ...
        Err(tokio_tungstenite::tungstenite::Error::Http(response)) => {
            panic!("Expected successful 101 upgrade, got HTTP {}", response.status());
        }
        Err(e) => {
            tracing::info!("Connection closed during/after upgrade: {:?}", e);
        }
    }
}

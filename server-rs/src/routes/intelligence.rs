...
///
/// Returns test files impacted by changes to a given symbol or file in the blast radius.
///
/// @docs API_REFERENCE:GetImpactedTests
#[tracing::instrument(skip(state), name = "intelligence::get_impacted_tests")]
pub async fn get_impacted_tests(
    ...
    let lock_clone = Arc::clone(&graph_lock);
    let query_path = query.path.clone();
    let query_name = query.name.clone().unwrap_or_default();

    let impacted_tests = tokio::task::spawn_blocking(move || {
        let guard = lock_clone.read();
        ...
        let affected = guard.calculate_blast_radius(&query_name, &resolved_path, 100);
        ...
        tests.into_iter().collect::<Vec<String>>()
    })
    .await
    .map_err(|e| {
        AppError::InternalServerError(format!("Impacted tests computation worker failed: {}", e))
    })?;

    Ok(Json(impacted_tests))
}
...
mod tests {
    ...
    #[tokio::test]
    async fn test_get_impacted_tests_path_traversal() {
        let state = Arc::new(AppState::new_minimal_mock().await);
        let query = ImpactedTestsQuery {
            name: None,
            path: "../../../etc/passwd".to_string(),
        };

        let result = get_impacted_tests(State(state), Query(query)).await;
        assert!(result.is_err());
        if let Err(AppError::BadRequest(msg)) = result {
            assert!(msg.contains("Invalid path boundary"));
        } else {
            panic!("Expected AppError::BadRequest for path traversal");
        }
    }
}

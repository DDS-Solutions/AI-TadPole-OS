...
//! and endpoint reactivity across diverse protocols (OpenAI, 
//! Anthropic, Google). Implements **Secure Secret Redaction**: 
//! ensures that `api_key` values are stripped from frontend 
//! responses while maintaining functional status indicators 
//! (`has_api_key`). AI agents should use these endpoints to 
//! dynamically scale the engine's cognitive capacity by pulling 
//! new models or updating high-tier provider configurations (LLM-03).
//! 
...
use crate::{
    agent::types::{ModelEntry, ProviderConfig, Validatable},
...
pub async fn get_providers(
    ...
    let sanitized: Vec<serde_json::Value> = state
        ...
        .iter()
        .map(|kv| {
            let mut val = serde_json::to_value(kv.value()).unwrap_or_else(|_| serde_json::Value::Object(serde_json::Map::new()));
            if let Some(obj) = val.as_object_mut() {
                // Replace actual key with a boolean flag so dashboard knows if it's configured
                let has_key = obj
                    .get("api_key")
                    .or_else(|| obj.get("apiKey")) // Backward compatibility check during transition
                    .and_then(|v| v.as_str())
                    .is_some_and(|s| !s.trim().is_empty());
                
                // Remove legacy camelCase key if present
                obj.remove("apiKey");

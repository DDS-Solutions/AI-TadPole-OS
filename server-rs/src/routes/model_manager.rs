...
pub async fn pull_model(
    ...
    tracing::info!("📥 [ModelStore] Requesting pull of {} on node {}", payload.tag, payload.node_id);

    // 1. Locate the node
    let node = state.registry.nodes.get(&payload.node_id)
        .ok_or_else(|| AppError::NotFound(format!("Node '{}' not found in swarm", payload.node_id)))?;
    ...
    let resp = state.resources.http_client.post(&ollama_url)
        .header("Authorization", format!("Bearer {}", state.security.deploy_token))
        .json(&ollama_payload)
        .send()
        .await
        .map_err(|e| AppError::InternalServerError(format!("Failed to reach Bunker node: {}", e)))?;


    if resp.status().is_success() {
        tracing::info!("✅ [ModelStore] Successfully initiated pull on {}", node.name);
        Ok((StatusCode::OK, Json(serde_json::json!({ "status": "success", "message": format!("Model {} is being pulled to {}", payload.tag, node.name) }))))
    } else {
        let status = resp.status();
        let err = resp.text().await.unwrap_or_else(|_| "Unknown error".to_string());
        tracing::error!("❌ [ModelStore] Pull failed on {} ({}): {}", node.name, status, err);
        Err(AppError::InternalServerError(format!("Bunker node '{}' reported error ({}): {}", node.name, status, err)))
    }
}
...
pub async fn ollama_proxy_pull(
    ...
    let ollama_host = std::env::var("OLLAMA_HOST").unwrap_or_else(|_| "http://10.0.0.1:11434".to_string());
    let ollama_url = format!("{}/api/pull", ollama_host);

    tracing::info!("🔄 [ModelStore] Proxying pull request to local Ollama: {}", ollama_url);

    let resp = state.resources.http_client.post(&ollama_url)
        .json(&payload)
        .send()
        .await
        .map_err(|e| AppError::InternalServerError(format!("Failed to reach local Ollama: {}", e)))?;
    ...
    if status.is_success() {
        ...
        tracing::error!("❌ [ModelStore] Local Ollama pull failed ({}): {}", status, body);
        Err(AppError::InternalServerError(format!("Ollama reported error: {}", body)))
    }
}

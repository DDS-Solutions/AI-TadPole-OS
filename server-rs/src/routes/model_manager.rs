...
pub async fn get_providers(
    ...
    let sanitized: Vec<serde_json::Value> = state
        ...
        .iter()
        .map(|kv| {
            let mut val = serde_json::to_value(kv.value())
...
pub async fn sync_provider_models(
    ...
    // 1. Resolve URL (Logic mirrored from test_provider but specialized for lists)
    let url = match protocol.as_str() {
        "google" => {
            if base_url.is_empty() {
                "https://generativelanguage.googleapis.com/v1/models".to_string()
        ...
        "openai" | "inception" | "deepseek" | "ollama" | "groq" | "openrouter" => {
            ...
            if protocol == "ollama" && !base.contains("/v1") {
                format!("{}/tags", base) // Ollama native list
            } else {
                format!("{}/models", base) // Standard OpenAI list
            }
        }
        _ => return Err(AppError::BadRequest(format!("Sync protocol '{}' not yet implemented in IMR-01.", protocol))),
    };
    ...
    if protocol == "ollama" && url.contains("/tags") {

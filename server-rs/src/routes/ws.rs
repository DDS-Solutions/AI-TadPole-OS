                ...
                Ok(Message::Text(text)) => {
                    if let Ok(val) = serde_json::from_str::<serde_json::Value>(&text) {
                        if val.get("type").and_then(|v| v.as_str()) == Some("oversight:decision") {
                            if let (Some(id), Some(decision), Some(sig), Some(pk)) = (
                                ...
                                if super::oversight::verify_oversight_signature(id, decision, sig, pk).is_ok() {
                                    let approved = decision == "approved";
                                    let removed_entry = ws_state.comms.oversight_queue.remove(id).map(|(_, entry)| entry);
                                    if let Some(entry) = removed_entry {
                                        if let Some((_, shooter)) = ws_state.comms.oversight_resolvers.remove(id) {
                                            let _ = shooter.send(approved);
                                        }
                                        let decision_label = if approved { "APPROVED" } else { "REJECTED" };
                                        ws_state.broadcast_sys(
                                            &format!("⚖️ Oversight: Decision for {} recorded as {}.", entry.id, decision_label),
                                            if approved { "success" } else { "warning" },
                                            entry.mission_id.clone(),
                                        );
                                        
                                        let audit = ws_state.security.audit_trail.clone();
                                        ...
                                        tokio::spawn(async move {
                                            ...
                                            let _ = audit.record("oversight", mission_id.as_deref(), None, "oversight_decision", &params).await;
                                        });

                                        ws_state.emit_event(serde_json::json!({
                                    ...
                                    tracing::warn!("🚫 WS: Oversight signature verification failed for entry {}", id);
                                }
                            }
                        }
                    }

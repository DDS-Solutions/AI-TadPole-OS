...
pub async fn update_settings(
    ...
    if let Some(val) = payload.privacy_mode {
        state.governance.privacy_mode.store(val, std::sync::atomic::Ordering::Relaxed);
        state.broadcast_sys(
            &format!("🛡️ Privacy Shield: Mode set to {}.", if val { "ON (Local-First)" } else { "OFF (Hybrid)" }),
...
pub async fn get_mission_quotas(
    ...
    let quotas: Vec<Quota> = rows.into_iter().map(|r| {
        use sqlx::Row;
...
pub async fn update_mission_quota(
    ...
    state.security.budget_guard.update_mission_quota(
        &cluster_id,
        payload.budget_usd,
        payload.reset_period
    ).await
    .map_err(|e| AppError::InternalServerError(format!("Failed to update mission quota: {}", e)))?;

    tracing::info!("🛡️ [Budget] Quota updated for mission {}: ${}", cluster_id, payload.budget_usd);
    Ok((StatusCode::OK, Json(serde_json::json!({ "status": "ok" }))))
}

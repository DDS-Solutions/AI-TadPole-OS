//! Agent Oversight — Security and audit logging API
//!
//! Provides the primary interface for human-in-the-loop verification, budget 
//! quota management, and global engine governance settings.
//!
//! @docs ARCHITECTURE:Networking
//!
//! ### AI Assist Note
//! **Governance Propagation**: Any updates to global governance settings 
//! (e.g., max-depth, budget-usd) take effect immediately across all active 
//! agent missions. Agents should monitor `OversightDecision` results via 
//! WebSockets for real-time task resolution.
...
pub async fn update_settings(
    ...
    (
        ...
        Json(serde_json::json!({
            ...
            "privacyMode": state.governance.privacy_mode.load(std::sync::atomic::Ordering::Relaxed)
        })),
    )
}
...
pub async fn get_security_quotas(State(state): State<Arc<AppState>>) -> impl IntoResponse {
...
pub async fn get_mission_quotas(State(state): State<Arc<AppState>>) -> impl IntoResponse {
...
pub async fn get_agent_health(State(state): State<Arc<AppState>>) -> impl IntoResponse {

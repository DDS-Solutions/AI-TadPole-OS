...
pub(crate) fn verify_oversight_signature(
    ...
    let payload_str = format!("{}:{}", entry_id, decision);
...
pub async fn decide_oversight(
    ...
    if let (Some(sig), Some(pk)) = (&payload.signature, &payload.verifying_key) {
        if let Err(e) = verify_oversight_signature(&entry_id, &payload.decision, sig, pk) {
            return Err(AppError::Forbidden(format!("Cryptographic signature verification failed: {}", e)));
        }
    } else {

...
pub async fn get_security_quotas(State(state): State<Arc<AppState>>) -> Result<impl IntoResponse, AppError> {
...
pub async fn get_mission_quotas(State(state): State<Arc<AppState>>) -> Result<impl IntoResponse, AppError> {
...
#[tracing::instrument(skip(state, payload), name = "security_governance::update_mission_quota")]
...
pub async fn get_agent_health(State(state): State<Arc<AppState>>) -> Result<impl IntoResponse, AppError> {
...
pub async fn get_policies(State(state): State<Arc<AppState>>) -> Result<impl IntoResponse, AppError> {
    let rows: Vec<(String, String)> = sqlx::query_as("SELECT tool_name, mode FROM permission_policies")
        .fetch_all(&state.resources.pool)
        .await
        .map_err(|e| AppError::InternalServerError(format!("Database error: {}", e)))?;

    let policies: Vec<serde_json::Value> = rows.into_iter().map(|(name, mode)| {
...
pub async fn update_policy(
    ...
    // Refresh cache
    state.security.permission_policy.refresh_cache().await
        .map_err(|e| AppError::InternalServerError(format!("Failed to refresh permission cache: {}", e)))?;

    state.broadcast_sys(
        &format!("🛡️ Security Policy Updated: {} -> {}", payload.tool_name, payload.mode),
        "info",
        None,
    );

...
pub async fn update_settings(
    ...
    if let Some(val) = payload.default_model {
        *state.governance.default_model.write() = val;
    }
    if let Some(val) = payload.default_provider {
        *state.governance.default_provider.write() = val;
    }
    ...
    tracing::info!(
        ...
        state.governance.default_model.read(),
        state.governance.default_provider.read()
    );

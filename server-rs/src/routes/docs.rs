...
pub fn find_doc_path(relative: &str) -> Option<PathBuf> {
    ...
    if let Ok(resource_root) = std::env::var("RESOURCE_ROOT") {
        let r = PathBuf::from(resource_root);
        paths.push(r.join(relative));
        paths.push(r.join("docs").join(relative));
        paths.push(r.join("_up_").join(relative));
        paths.push(r.join("_up_").join("docs").join(relative));
    }

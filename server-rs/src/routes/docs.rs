...
pub async fn get_knowledge_doc(
    ...
    if category.contains("..") || name.contains("..") {
        return Err(AppError::BadRequest("Invalid path: directory traversal detected".to_string()));
    }

    let base_path = match find_doc_path("src/data/knowledge") {
        Some(p) => p,
        None => {
            return Err(AppError::NotFound("Knowledge base directory not found".to_string()));
        }
    };
...
pub async fn get_operations_manual() -> Result<impl IntoResponse, AppError> {
    let path = match find_doc_path("OPERATIONS_MANUAL.md") {
        ...
        None => {
            ...
            return Err(AppError::NotFound("Operations Manual not found".to_string()));
        }
    };
    ...
    match fs::read_to_string(&path) {
        ...
        Err(e) => {
            ...
            Err(AppError::NotFound("Operations Manual not found".to_string()))
        }
    }
}

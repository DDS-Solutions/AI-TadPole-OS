//! Startup Environment Validation & Security Policy - The Gatekeeper
//!
//! Reads the `.env.schema` file and validates that all required environment
//! variables are set. Provides a clear startup report for operators and 
//! coordinates with the secret redactor for sensitive field isolation.
//!
//! @docs ARCHITECTURE:EnvironmentSecurity
//!
//! ### AI Assist Note
//! **Strict Validation**: In production mode, missing `@required` variables 
//! will trigger an `anyhow::bail!` and halt the engine. In `debug_assertions` 
//! (development) mode, it only logs a warning. This prevents accidental 
//! outages while allowing rapid local iteration.
//!
//! ### 🔍 Debugging & Observability
//! - **Failure Path**: Missing `.env.schema` file, malformed schema decorators, or missing `NEURAL_TOKEN`.
//! - **Telemetry Link**: Search for `[EnvSchema]` in `tracing` logs.
...
pub fn validate_and_report(schema_path: &Path) -> anyhow::Result<()> {
    ...
    if !missing_required.is_empty() {
        ...
        if cfg!(debug_assertions) {
            ...
            // SPECIAL CASE: For portable/sidecar deployments, provide a default NEURAL_TOKEN if missing
            if missing_required.contains(&"NEURAL_TOKEN".to_string()) {
                let default_token = "tadpole-os-sidecar-default-2026";
                std::env::set_var("NEURAL_TOKEN", default_token);
                tracing::warn!("⚠️  [EnvSchema] NEURAL_TOKEN was missing. Injected default sidecar token for portability.");
                
                // If it was the only missing one, we can continue

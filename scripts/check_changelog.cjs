 ...
 * - [Structural] A single Unreleased section is always first.
 * - [Structural] Published release sections are unique and immutable.
 * - [Structural] A product-version transition introduces exactly one matching release section.
 *
...
function compareSemver(left, right) {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    if (leftNumeric && rightNumeric) return BigInt(leftParts[index]) > BigInt(rightParts[index]) ? -1 : 1;

...
function loadJsonOrFallback(filePath, fallback, label) {
    if (!fs.existsSync(filePath)) {
        console.log(`[WARN] ${label} not found at ${filePath}. Using fallback registry.`);
        return fallback;
    }
    ...
    try {
        ...
        console.log(`[WARN] Failed to parse ${label} at ${filePath}: ${error.message}`);
        return fallback;
    }
}

// Load Registries
const errorRegistry = loadJsonOrFallback(ERROR_REGISTRY_PATH, {}, 'ERROR_REGISTRY.json');
const telemetryMap = loadJsonOrFallback(TELEMETRY_MAP_PATH, {}, 'TELEMETRY_MAP.json');
...
function scanDir(dir) {
    ...
    for (const file of files) {
        ...
        if (relativePath.startsWith('src/services') || relativePath.startsWith('src/stores')) {
            if (!content.includes('### AI Context Alignment') && !content.includes('### AI Assist Note')) {
                errors.push(`[MISSING_HEADER] ${relativePath}: Missing '### AI Context Alignment'`);
            }

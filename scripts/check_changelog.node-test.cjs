 ...
 * - [Structural] Published fixture sections remain byte-for-byte immutable.
 *
...
test('orders complete prerelease identifiers and large numeric components without precision loss', () => {
  for (const versions of [
    ['2.0.0-alpha-z.2', '2.0.0-alpha-z.1', '2.0.0-alpha-a.9'],
    ['1.0.0-9007199254740993', '1.0.0-9007199254740992'],
    ['9007199254740993.0.0', '9007199254740992.0.0'],
  ]) {
    const changelog = (releases) => `## [Unreleased]\n\n${releases.map((version) => `## [${version}] - 2026-09-12\n`).join('\n')}`;
    assert.deepEqual(parseChangelog(changelog(versions)).releases, versions);
    assert.throws(() => parseChangelog(changelog([...versions].reverse())), /newest first/);
  }
});

...
test('a product bump updates every package surface and increments Android once', () => {
  const root = fixture();
  const baselineAuthority = JSON.parse(fs.readFileSync(path.join(root, 'version.json'), 'utf8'));
  const expectedAndroidCode = baselineAuthority.android_version_code + 1;
  const result = run({
    rootDir: root,
  ...
  assert.equal(result.authority.android_version_code, expectedAndroidCode);
  assert.equal(result.authority.api_document_version, baselineAuthority.api_document_version);
  assert.match(fs.readFileSync(path.join(root, 'apps/mobile-android/app/build.gradle.kts'), 'utf8'), new RegExp(`versionCode = ${expectedAndroidCode}[\\s\\S]*versionName = "1\\.1\\.999"`));
  assert.equal(JSON.parse(fs.readFileSync(path.join(root, 'package-lock.json'), 'utf8')).packages[''].version, '1.1.999');
  assert.match(fs.readFileSync(path.join(root, 'docs/openapi.yaml'), 'utf8'), new RegExp(`version: ${baselineAuthority.api_document_version}`));
  assert.doesNotThrow(() => run({ rootDir: root, options: options({ check: true }) }));
});

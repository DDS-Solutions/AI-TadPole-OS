 ...
 *
 * ### AI Assist Note
 * - **Subsystem**: Developer Tooling / Environment Diagnostics
 * - Keep the Node.js check aligned with package.json and the locked build/test tools.
 *
 * ### 🔍 Debugging & Observability
 * - Reports categorized PASS, WARN, and FAIL results; exits nonzero on failures.
 * - Runtime boundary witnesses: scripts/doctor.node-test.cjs.
 */
...
const [majorVersion, minorVersion, patchVersion] = nodeVersion.replace(/^v/, '').split('.').map(Number);
const nodeRequirement = '^22.22.2 || ^24.15.0 || >=26.0.0';
const supportedNode = !nodeVersion.includes('-') && ((majorVersion === 22 && (minorVersion > 22 || (minorVersion === 22 && patchVersion >= 2)))
    || (majorVersion === 24 && minorVersion >= 15)
    || majorVersion >= 26);
if (supportedNode) {
    logResult('PASS', 'Node.js', `Runtime version ${nodeVersion} meets requirement (${nodeRequirement})`);
} else {
    logResult('FAIL', 'Node.js', `Runtime version ${nodeVersion} is unsupported. Require Node.js ${nodeRequirement}`);
}
...
// 3. SQLite Database Check (Canonical: data/tadpole.db)
const dbDir = path.join(rootDir, 'data');
const dbPath = path.join(dbDir, 'tadpole.db');
if (fs.existsSync(dbPath)) {
    try {
        fs.accessSync(dbPath, fs.constants.R_OK | fs.constants.W_OK);
        logResult('PASS', 'Database', `Canonical tadpole.db accessible at ${sanitizePath(dbPath)}`);
    } catch {
        logResult('FAIL', 'Database', `Canonical tadpole.db exists but lacks read/write permissions at ${sanitizePath(dbPath)}`);
    }
} else {
    logResult('PASS', 'Database', `Canonical database data/tadpole.db not yet created (will initialize automatically on engine boot)`);
}

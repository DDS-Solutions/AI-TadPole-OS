...
Before committing or deploying, run the complete verification suite to validate security scans, lints, type checks, tests, and documentation gates:
```bash
...
To verify symbol-to-header parity and path existence in documentation/directives without running the full suite:
```bash
# Verify symbol gate & markdown paths
python execution/symbol_gate.py .

# Verify API route & env example parity

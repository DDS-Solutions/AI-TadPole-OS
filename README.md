...
- **Neural Voice Integration (Hybrid/Infrastructure):**
  - **Sovereign TTS (Piper)**: Local-first, ONNX-accelerated speech synthesis.
  - **Sovereign STT (Whisper)**: Local neural transcription.
...
- **Swarm Template Ecosystem:**
  ...
  - **Sapphire Shield**: Strict zero-trust security gate requiring human approval for sensitive skills (e.g., `shell:execute`, `budget:spend`).
...
- **Bunker Infrastructure (Local Swarm):**
...
- **Unified Persistence**: All agents and skill templates persist strictly via a unified SQLite schema—no disparate JSON registry or manual file synchronization is required.

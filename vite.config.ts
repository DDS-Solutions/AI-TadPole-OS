 ...
 * - **Subsystem**: System Core / vite.config
 *
...
export default defineConfig({
  ...
  server: {
    host: '127.0.0.1', // Loopback only. For LAN device testing: npx vite --host 0.0.0.0
    port: 5173,
    watch: {
      ignored: [
        '**/coverage/**',
  ...
  test: {
    ...
    exclude: ['**/node_modules/**', '**/.git/**', '**/dist/**', '**/.tmp/**', '**/target/**', '**/3rdparty/**', 'docs/.vitepress/**', '**/setup.ts', 'tests/e2e/**', '**/external/**'],
    deps: {
      optimizer: {
        web: {

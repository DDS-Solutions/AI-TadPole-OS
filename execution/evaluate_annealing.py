...
- **Subsystem**: Infrastructure Automation / evaluate_annealing
- **Primary Entrypoints**: `get_unresolved_faults`, `propose_annealing`, `create_proposal`
...
- **Local Errors**: none
- **Telemetry Targets**: `[SEARCH]`, `[OK]`, `[EVAL]`, `[FAIL]`
...
DATABASE_VALUE = os.getenv("DATABASE_URL", "sqlite://data/tadpole.db")
DATABASE_PATH = Path(DATABASE_VALUE.removeprefix("sqlite://").removeprefix("sqlite:"))
DB_PATH = DATABASE_PATH if DATABASE_PATH.is_absolute() else ROOT / DATABASE_PATH
DIRECTIVES_PATH = ROOT / "directives"

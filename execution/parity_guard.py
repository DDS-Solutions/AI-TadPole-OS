...
def scan_router(router_path):
    ...
    m_root_v1 = re.search(r'fn build_protected_v1_routes\s*\([^)]*\)\s*->\s*Router[^{]*\{', content)
    if m_root_v1:
        start_idx = m_root_v1.end() - 1
        brace_count = 0
        end_idx = start_idx
        for i in range(start_idx, len(content)):
            char = content[i]
...
def check_env_vars(root, env_vars_in_code=None):
    print(f"\nScanning for Environment Variables...")
    if env_vars_in_code is None:
...
def check_api_docs_parity(root, fix=False):
    ...
    if fix:
        ...
        result = subprocess.run([sys.executable, str(gen_script), "--fix"], capture_output=True, text=True, cwd=str(root))
        if result.returncode == 0:
            print_result("DOCS-PARITY", True, "Successfully synced API_REFERENCE.md")
            return 0
        else:
            print_result("DOCS-PARITY", False, f"Sync failed: {result.stderr or result.stdout}")
            return 1
    else:
        openapi_file = root / "docs" / "openapi.yaml"
        api_ref_file = root / "docs" / "API_REFERENCE.md"
        if not openapi_file.is_file():
            print_result("DOCS-PARITY", False, "docs/openapi.yaml missing")
            return 1
        if not api_ref_file.is_file():
            print_result("DOCS-PARITY", False, "docs/API_REFERENCE.md missing")
            return 1

        openapi_mtime = os.path.getmtime(openapi_file)
        api_ref_mtime = os.path.getmtime(api_ref_file)
...
def check_parity(root_dir, fix=False):
    ...
    print(f"\nScanning Server Code for Environment Variables and Documentation Tags (@docs)...")
    env_vars_in_code = set()
    doc_tags = {}
    for root_dir_walk, _, files in os.walk(root / "server-rs" / "src"):
        for file in files:
    ...
    for tag, info in doc_tags.items():
        ...
        if doc_path_str not in doc_content_cache:
            try:
                doc_content_cache[doc_path_str] = doc_file.read_text(encoding='utf-8')
            except Exception:
                doc_content_cache[doc_path_str] = ""

        doc_content = doc_content_cache[doc_path_str]
        # Semantic check: does the doc contain this tag or section reference?
        if f"{doc_name}:{section}" in doc_content or f"@docs {doc_name}:{section}" in doc_content or section in doc_content:
            print_result("ADG-TAG", True, f"{tag} synchronized")
        else:
            doc_mtime = os.path.getmtime(doc_file)
            if info["mtime"] > doc_mtime and (info["mtime"] - doc_mtime > 300):
    ...
    errors += check_env_vars(root, env_vars_in_code=env_vars_in_code)

...
def scan_router(router_path):
    ...
    with open(router_path, 'r', encoding='utf-8') as f:
        ...
        matches = re.finditer(r'\.route\("([^"]+)",\s*([a-z]+)\(([^)]+)\)\)', content)
...
def check_capabilities(root):
    print(f"\nScanning Capabilities & Skills...")
...
def check_parity(root_dir):
    ...
    errors += check_capabilities(root)

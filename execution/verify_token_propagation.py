...
- **Subsystem**: Infrastructure Automation / verify_token_propagation
- **Primary Entrypoints**: `print_header`, `print_result`, `TelemetryVerifier`, `main`
...
- **Local Errors**: none
- **Telemetry Targets**: `[verify_token_propagation]`, `[SIMULATION]`, `[VERIFICATION]`
...
class TelemetryVerifier:
    ...
    def generate_report(self, before, after, mode="Simulated", mission_id="N/A", endpoints_verified=False):
        report_path = ROOT / ".tmp" / "token_telemetry_report.md"
...
"""
        ...
        endpoint_status = "PASS" if endpoints_verified else "NOT RUN"
        md_content += f"""
...
- `GET /v1/agents` - **Status: {endpoint_status}**.
- `GET /v1/agents/{{id}}` - **Status: {endpoint_status}**.
...
def main() -> int:
    ...
    parser.add_argument("--token", type=str, default=os.getenv("NEURAL_TOKEN", ""), help="Auth token (defaults to NEURAL_TOKEN)")
    parser.add_argument("--simulate", action="store_true", help="Explicitly simulate agent runs by mutating SQLite")
    args = parser.parse_args()

    if not args.simulate and not args.token:
        print_result("AUTH", False, "NEURAL_TOKEN or --token is required for real verification")
        return 2
    ...
    # 2. Check server
    server_online = False if args.simulate else verifier.check_server_health()
    ...
    if args.simulate:
        ...
        if not server_online:
            print_result("REST-API", False, "Server is offline; no database simulation was performed")
            return 1
        success = verifier.dispatch_real_mission()
        if not success:
            print_result("DISPATCH", False, "Real dispatch failed; no database simulation was performed")
            return 1
    ...
    verifier.generate_report(
        ...
        mode,
        mission_id,
        endpoints_verified=server_online,
    )
    return 0

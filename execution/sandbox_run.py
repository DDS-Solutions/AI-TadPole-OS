...
- **Subsystem**: Infrastructure Automation / sandbox_run
- **Primary Entrypoints**: `create_junction`, `main`
...
- **Local Errors**: none
- **Telemetry Targets**: `[sandbox_run]`, `[Sandbox]`
...
def main():
    ...
    try:
        ...
        # Step 5: Execute command in the sandbox directory with Process Group isolation
        print(f"[Sandbox] Executing command in isolated process group: {command_to_run}")
        
        # Configure OS-specific process group creation flags
        kwargs = {}
        if os.name == 'nt':
        ...
        try:
            ...
            # Wait for execution with safety timeout (default 300 seconds)
            exit_code = proc.wait(timeout=300)
        except subprocess.TimeoutExpired:
            print(f"[Sandbox] Execution timed out after 300s! Terminating process group...")
            if proc:
                if os.name == 'nt':

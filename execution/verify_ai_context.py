...
ADG_MANIFEST_PATH = ROOT / 'adg.manifest.json'
...
# Cache of the ADG v2 manifest (loaded once per run)
_ADG_MANIFEST: Optional[Dict[str, Any]] = None

def get_adg_manifest() -> Optional[Dict[str, Any]]:
    """Load and cache the ADG v2 manifest if it exists."""
    global _ADG_MANIFEST
    if _ADG_MANIFEST is not None:
        return _ADG_MANIFEST
    if ADG_MANIFEST_PATH.exists():
        try:
            with open(ADG_MANIFEST_PATH, 'r', encoding='utf-8') as f:
                _ADG_MANIFEST = json.load(f)
            return _ADG_MANIFEST
        except Exception:
            return None
    return None

def get_manifest_witnessed_files() -> Set[str]:
    """Returns the set of source file paths that have witnesses in adg.manifest.json."""
    manifest = get_adg_manifest()
    if not manifest:
        return set()
    witness_map = manifest.get('claims', {}).get('witness_map', {})
    # Normalize paths to forward slashes for cross-platform comparison
    return {k.replace('\\', '/') for k in witness_map.keys()}
...
def strip_comments_and_strings(content: str, file_ext: str) -> str:
    ...
    if file_ext == '.rs':
        ...
        text = re.sub(r'(?<![a-zA-Z0-9_])r(#*)"[\s\S]*?"\1', '""', text)
...
def extract_js_test_names(content: str) -> Set[str]:
    """Read named test calls while skipping comments and unrelated string literals."""
    tokens = re.compile(r'''
        //[^\r\n]* | /\*[\s\S]*?\*/
        | (?<![\w.$])(?:test|it)\s*\(\s*(?P<quote>["'`])(?P<name>[a-zA-Z0-9_]+)(?P=quote)
        | "(?:\\.|[^"\\])*" | '(?:\\.|[^'\\])*' | `(?:\\.|[^`\\])*`
    ''', re.VERBOSE | re.DOTALL)
    return {match.group('name') for match in tokens.finditer(content) if match.group('name')}
            ...
            if ext in {'.rs', '.ts', '.tsx', '.js', '.jsx', '.py'}:
                try:
                    ...
                    if ext == '.rs':
                        ...
                        tests.update(extract_js_test_names(raw_text))
...
def verify_symbol_defined_in_file(symbol: str, content: str, file_ext: str) -> bool:
    ...
    if file_ext == '.py':
        ...
        patterns = [
            ...
            rf'\bexport\s*\{{[^}}]*\b{re.escape(clean_sym)}\b[^}}]*\}}',
        ]
        return any(re.search(p, stripped) for p in patterns)
...
def tag_is_emitted(tag: str, code_content: str) -> bool:
    ...
    emitter_pattern = '|'.join(f'(?:{e})' for e in EMITTERS)
    if re.search(rf'(?:{emitter_pattern})[^;}}]*?{re.escape(tag_fmt)}', code_content, re.DOTALL):
        return True
    if re.search(rf'{re.escape(tag_fmt)}[^;}}]*?(?:{emitter_pattern})', code_content, re.DOTALL):
        return True
    return False
...
def extract_header_block(content: str, file_ext: str = "") -> str:
    ...
    if file_ext == '.py' or norm.strip().startswith('"""') or norm.strip().startswith("'''"):
        ...
        for line in lines:
            s = line.strip()
            if s.startswith('//') or s.startswith('/*') or s.startswith('*') or s.startswith('*/') or s.startswith('#!') or not s:
                header_lines.append(line)
            else:
                break
        return '\n'.join(header_lines)
...
def verify_file(file_path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        #    ADG v2 UPGRADE: When the 'Witness Tests' header field has been stripped by
        #    `adg codemod`, this gate falls through to adg.manifest.json for resolution.
        #    The manifest is the single author of record for witness claims.
        enforced_witnesses = {inv["witness"] for inv in meta["invariants"] if inv.get("witness")}
        declared_witnesses = set(meta["witness_tests"])
        manifest_witnessed = get_manifest_witnessed_files()

        # Check 6A: Declared witness tests must exist in repository test suite
        #           (only runs if the file still has Witness Tests: header — pre-codemod)
        ...
        # Check 6B: Any invariant with an enforced_by link must be cross-listed in the
        #           header's 'Witness Tests' section OR be resolvable via adg.manifest.json
        for enf_w in enforced_witnesses:
            if enf_w and enf_w not in declared_witnesses:
                # ADG v2 fallback: if manifest exists and the witness is in the test suite, skip
                if manifest_witnessed and enf_w in all_known_tests:
                    continue

...
def main():
    ...
    for target in TARGET_DIRS:
        ...
        for r, _, files in os.walk(target_path):
            if any(part in r for part in ['.git', 'node_modules', 'target', 'dist', '.tmp', 'reports', 'coverage', '3rdparty', 'wiki', '.vitepress', 'public', 'archive', '.gemini', '.cargo', 'scratch']):
                continue
                
            for f in files:
                if f in ['API_REFERENCE.md', 'Benchmark_Spec.md']:
                    continue

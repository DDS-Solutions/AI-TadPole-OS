...
def restore_agents(json_path, db_path):
    ...
    for agent in agents:
        ...
        # modelConfig mapping
        model_config = agent.get('modelConfig', {})
        model_id = agent.get('model', model_config.get('modelId'))
        provider = model_config.get('provider', 'gemini')
        if not provider or provider == "": provider = "gemini"
        
        api_key = model_config.get('apiKey')
        base_url = model_config.get('baseUrl')
        system_prompt = model_config.get('systemPrompt')
        ...
        # Additional fields
        theme_color = agent.get('themeColor')
        budget_usd = agent.get('budgetUsd', 10.0)
        cost_usd = agent.get('costUsd', 0.0)
        voice_id = agent.get('voiceId')
        voice_engine = agent.get('voiceEngine')
        ...
        # tokens
        tokens_used = agent.get('tokensUsed', 0)
        failure_count = agent.get('failureCount', 0)
        
        # Model 2/3
        model_2 = agent.get('model_2')
        model_3 = agent.get('model_3')
        model_config2 = json.dumps(agent.get('modelConfig2')) if agent.get('modelConfig2') else None
        model_config3 = json.dumps(agent.get('modelConfig3')) if agent.get('modelConfig3') else None
    ...
    conn.commit()
    conn.close()
    print(f"Successfully restored {count} agents to {db_path}.")

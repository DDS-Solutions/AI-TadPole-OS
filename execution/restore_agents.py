...
- **Subsystem**: Infrastructure Automation / restore_agents
- **Primary Entrypoints**: `print_result`, `restore_agents`
...
def restore_agents(json_path, db_path):
    ...
    for agent in agents:
        ...
        # Whitelist of allowed columns for security (SEC-02)
        ALLOWED_COLUMNS = {
            "id", "name", "role", "department", "description", "model_id", "status",
            "theme_color", "budget_usd", "cost_usd", "metadata", "skills", "workflows",
            "mcp_tools", "model_2", "model_3", "model_config2", "model_config3",
            "voice_id", "voice_engine", "provider", "api_key", "base_url",
            "system_prompt", "temperature", "category", "requires_oversight",
            "working_memory", "tokens_used", "failure_count", "created_at"
        }
        
        # Only use fields that exist in both the input and the DB schema, and are whitelisted
        active_fields = {k: v for k, v in fields.items() if k in db_cols and k in ALLOWED_COLUMNS}

...
> - **Observability**: Traceability via `execution/parity_guard.py` (`[product_owner]`)
...
- **The Final Arbiter**: When the `tadpole-backend-specialist` and `frontend-specialist` disagree on a trade-off, the PO decides based on the Strategic Roadmap.

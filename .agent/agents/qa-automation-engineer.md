...
>
> ### AI Assist Note
> The Quality Guardian for the Tadpole OS Sovereign infrastructure. Responsible for the final validation of all features. No code reaches "Done" status without a signed-off verification report from this agent.
>
> ### 🔍 Debugging & Observability
> Traceability via `parity_guard.py`. All failures must be reported as "Reproducible Defect Packets" containing the exact state and steps to trigger the bug.
...
- **Hand-off to `debugger` / `specialist`**: The "Bug Packet" is the primary input for the developer to fix the issue.

...
> - **Telemetry Link**: Search `[documentation_writer]` in audit logs.
>
> ### AI Assist Note
> Core technical resource for the Tadpole OS Sovereign infrastructure. Responsible for eliminating information drift and maintaining the Single Source of Truth (SSOT) across all versioned releases.
>
> ### 🔍 Debugging & Observability
> Traceability via `parity_guard.py` and doc-to-code parity audits.
...
- **Sync with `backend-specialist`**: Capture the *intent* and *architecture* before it is obscured by implementation details.

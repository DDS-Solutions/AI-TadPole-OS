...
> - **Telemetry Link**: Cross-reference results with `execution/parity_guard.py`.
>
> ### AI Assist Note
...
- **The Rubric**: Every candidate solution must be audited against:
    ...
    - **Parity Check**: Does this change break existing functionality (refer to `parity_guard.py`)?
- **Outcomes**:
...
- **Constraint**: Do not rewrite the entire solution if the core logic is sound; surgically fix the identified flaws.

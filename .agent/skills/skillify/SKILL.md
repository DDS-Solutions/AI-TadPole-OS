...
> **Skill Authoring Rules (Pocock Discipline)**:
> 1. **Model Invocation vs Context Load**: If the skill is ONLY manually invoked by users, set `disable-model-invocation: true` to prevent system prompt context bloat.
> 2. **Checkable Completion Criteria**: Every step MUST end on an explicit, checkable criterion (e.g. "Every modified model tested", not "Produce changes") to prevent premature agent turn completion.

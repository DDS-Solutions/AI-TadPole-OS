...
## 🚫 Anti-Patterns Checklist (Avoid "AI Design Tells")

Refer to the project's root [`DESIGN.md`](file:///d:/TadpoleOS-Dev/DESIGN.md) contract for design tokens. Always avoid these 6 common AI design tells:

1. **Card Inception**: Never nest a card component (`bg-slate-900 border-slate-800`) inside an identical container card. Use dividers, spatial padding, or background tone steps.
2. **Untinted Grays & Pure Black**: Never use `#000000` or raw untinted grays. Always use HSL-tinted obsidian surfaces (e.g. `#090d16`, `slate-950`, `zinc-900`).
3. **Low-Contrast Badge Text**: Never use dark gray text on colored badge backgrounds. Ensure badge text uses high-contrast text (`text-emerald-400` on `bg-emerald-500/10`).
4. **Icon Tiles Above Headings**: Avoid placing rounded-square icon tiles above every section heading. Keep headings clean and typography-driven.
5. **Single-Font Uniformity**: Do not rely exclusively on default Inter. Pair crisp Sans headers with JetBrains Mono for system metrics, execution logs, and trace IDs.
6. **Unlabelled Icon Buttons**: Always provide `aria-label` or `title` on icon-only interactive elements.

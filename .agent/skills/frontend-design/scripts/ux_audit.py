...
- **Subsystem**: Developer Scripts / ux_audit
- **Primary Entrypoints**: `UXAuditor`, `main`
...
- **Local Errors**: none
- **Telemetry Targets**: `[ux_audit]`, `[:10]`, `[:15]`
...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        # Von Restorff
        if re.search(r'<button\b', content, re.IGNORECASE) and not re.search(r'primary|bg-primary|Button.*primary|variant=["\']primary', content, re.IGNORECASE):
        ...
        if len(size_values) > 2:
            ...
            for ratio in ratios:  # Check all ratios
        ...
        # --- 7. ACCESSIBILITY ---
        if re.search(r'<img(?![^>]*\balt\b)[^>]*>', content, re.IGNORECASE):
    ...
    def audit_directory(self, directory: str) -> None:
        ...
        skip_dirs = {'node_modules', '.git', 'dist', 'build', '.next', 'coverage',
                     '__pycache__', '.vite', 'target', '.vitepress', '.agent', 'docs', 'playwright-report'}

...
def run_tests(cmd: list, cwd: Path) -> dict:
    ...
    try:
        ...
        if "passed" in output.lower() and "failed" in output.lower():
            import re
            # Extract the summary section specifically to avoid "Test Files" counts
            summary_match = re.search(r'Tests\s+(.*?)\n', output, re.IGNORECASE)
            summary_text = summary_match.group(1) if summary_match else output
            
            match = re.search(r'(\d+)\s+passed', summary_text, re.IGNORECASE)
            if match:
                result["tests_passed"] = int(match.group(1))
            match = re.search(r'(\d+)\s+failed', summary_text, re.IGNORECASE)
            if match:

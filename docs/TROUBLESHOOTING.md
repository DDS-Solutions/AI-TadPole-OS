...
> 1. **[`docs/ERROR_REGISTRY.json`](./ERROR_REGISTRY.json)** — search by symptom keyword or error code. It maps codes to source files and remediation paths.
> 2. **[`docs/TELEMETRY_MAP.json`](./TELEMETRY_MAP.json)** — trace by tag (e.g., `[VaultStore]`, `[AgentService]`) to find the log emitter location.
> 3. **For Sovereign events** (`BUDGET_BREACH`, `STASIS_ACTIVE`, `LOGIC_BLOCKER`, `COMPLIANCE_DRIFT`): consult `ERROR_REGISTRY.json` — all 4 codes are now registered with directive references and remediation paths.
>
> Only proceed to the sections below if both assets yield no result.

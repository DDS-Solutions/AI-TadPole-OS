...
## 7. Filesystem & Execution Security Guards (`security/path_guard.rs`, `security/ssrf_guard.rs`, `security/command_guard.rs`, `utils/security.rs`)

Tadpole OS implements specialized defense primitives decomposed across dedicated security submodules:

- **Path Integrity & Workspace Boundary Enforcement (`security/path_guard.rs`)**: Centralized path validation (`validate_path`) and non-forgeable `SafePath` wrappers applied to ALL filesystem interactions involving user-controlled data. Components are broken down and verified against illegal traversal tokens (`..`, root prefixes), symlink-escape attacks, and device paths, strictly locking access within authorized roots (`WORKSPACE_ROOT`, Execution Dir, or Codebase Root). Also provides identifier sanitization (`sanitize_id()`).
- **SSRF Prevention & Outbound URL Boundaries (`security/ssrf_guard.rs`)**: Strict pre-flight URL verification (`validate_outbound_url`) blocking private and loopback IP ranges (IPv4 RFC 1918, link-local, loopback, IPv6 RFC 4193/4291) to prevent DNS rebinding, internal subnet port scanning, and cloud metadata exfiltration (e.g., AWS 10.0.0.1).
- **Command Injection & Binary Whitelisting (`security/command_guard.rs`)**: Enforces explicit executable binary whitelists and safe argument slicing (`validate_command_tokens`), preventing subshell injection (`|`, `;`, `&&`, `$()`) during external tool calls and template installation.
- **Unified Security Facade (`utils/security.rs`)**: Provides backward-compatible re-exports and core utility functions across the engine.

...
## 🛡️ Active Documentation Guard CLI (`tools/adg/adg.mjs`)

The zero-dependency Node.js CLI engine that enforces **ADG v2** across the TypeScript and frontend codebase. Replaces manual comment claims with machine-computed manifests and executable invariant testing.

### Commands & npm Scripts

| Command | npm Script | Description |
|:--- |:--- |:--- |
| `lint-headers <dir>` | `npm run adg:lint` | Scans files for forbidden v1 header fields (`Witness Tests:`, `Invariants:`, etc.) |
| `codemod [--write] <dir>` | `npm run adg:codemod` / `npm run adg:codemod:write` | Strips legacy v1 fact-fields while preserving `@docs` architectural guidance |
| `generate [--check] <dir>` | `npm run adg:generate` / `npm run adg:check` | Computes witness maps, parses CSP, checks ratchets, and updates or validates `adg.manifest.json` |
| `verify <dir>` | `npm run adg:verify` / `npm run test:witness` | Executes full verification pipeline (must-fail fixtures, lint, CSP grammar, manifest integrity) |

### Usage Examples
```powershell
# Lint source files for legacy v1 header anti-patterns
npm run adg:lint

# Regenerate the machine manifest and enforce ratchet coverage
npm run adg:generate

# Verify manifest matches repository state with zero drift
npm run adg:check

# Run full ADG v2 verification suite
npm run adg:verify
```

...
spacing:
  ...
  sidebar-width: "260px"
animations:
  spring-panel: { type: "spring", stiffness: 380, damping: 30 }
  spring-dropdown: { type: "spring", stiffness: 450, damping: 40 }
  spring-badge: { type: "spring", stiffness: 500, damping: 25 }
  transition-standard: "all 200ms cubic-bezier(0.16, 1, 0.3, 1)"
components:
  sovereign-panel:
...
- **Hover**: Transition to Zinc-700 border $\rightarrow$ 200ms ease-in-out (`transition-standard`).
- **Active/Focus**: Implementation of `focus-ring` $\rightarrow$ 2px outline offset for high-contrast keyboard visibility (Sovereign Focus).
- **Animations**: Framer Motion transitions should consume the standard spring models (`spring-panel`, `spring-dropdown`, `spring-badge`) to maintain mechanical kinetic feedback.

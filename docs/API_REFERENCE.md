...
> - **Tool Flow**: `POST /v1/agents/:id/tasks` -> `AgentRunner` -> `ToolInvocation` -> `Yield (Oversight)` -> `Result`.
...
sequenceDiagram
    ...
    participant Tool as McpHost/Skill
...
| `agent:update` | `{ agent_id, data: EngineAgent }` | Full agent state sync |
...
| `trace:span` | `{ id, name, parentId, startTime, missionId, attributes }` | New OTel-compliant span initialized |
| `trace:span_update` | `{ id, endTime, status, attributes }` | Span completion or update (set result/error) |

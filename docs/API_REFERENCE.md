...
> **Version**: 1.7.1
> **Last Hardened**: 2026-04-16 (OBLITERATUS Integration)
...
| Method | Path | Auth | Description |
|--------|------|------|-------------|
| `GET` | `/v1/system/compute-profile` | ✓ | Get real-time hardware telemetry (CPU/RAM) |
...
> [!NOTE]
> **OOM Autonomic Fallback**: If the provider returns an Out-of-Memory (OOM) error, the engine automatically appends `:q4_K_M` to the `model_id` and transparently retries the task up to 1 time.
...
| `POST` | `/v1/infra/providers/{id}/sync` | ✓ | Discover & Sync Models (IMR-01) |
...
| `GET` | `/v1/system/compute-profile` | ✓ | Get Hardware Telemetry |
...
| `recruit_specialist(agent_id: string, message: string)` | `{ "agent_id": "...", "message": "..." }` | Request assistance from a specialized agent within the same cluster. Spawns ephemeral sub-agents in parallel via `FuturesUnordered`. |

## 🛡️ Sovereign Error Protocol (RFC 9457)

Tadpole OS uses standardized **Problem Details for HTTP APIs** (RFC 9457). All errors return `application/problem+json`.

### Response Format
```json
{
  "type": "https://tadpole.os/errors/resource-exhaustion",
  "title": "Resource Exhaustion (VRAM)",
  "status": 507,
  "detail": "VRAM limit exceeded during model load.",
  "error_code": "OOM_QUANTIZATION_FALLBACK:llama3",
  "help_link": "https://docs.tadpole.os/troubleshooting/quantization#q4_K_M"
}
```

### Machine-Readable Error Codes
The `error_code` field enables UI-level autonomous remediation.

| Code | Status | Remediation Suggestion |
| :--- | :--- | :--- |
| `OOM_QUANTIZATION_FALLBACK:{model}` | 507 | Suggest fallback to 4-bit quantization. |
| `RECRUITMENT_FAILED:{role}` | 503 | Suggest manual recruitment or node check. |
| `INFRA_ERROR:{provider}` | 502 | Verify API key and network connectivity. |
| `VALIDATION_ERROR` | 422 | Fix request payload constraints. |
| `NOT_FOUND` | 404 | Resource has drifted or been deleted. |
...
> [!IMPORTANT]
> **Runtime requirement**: `NEURAL_TOKEN` must be explicitly set. The engine will refuse to start if the variable is missing, and the frontend now expects a real configured token instead of a baked-in placeholder.

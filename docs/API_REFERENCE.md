...
**Source of Truth**: `server-rs/src/router.rs`, `server-rs/src/routes/mod.rs`
**Last Hardened**: 2026-04-05
**Standard Compliance**: ECC-API (Enhanced Contextual Clarity - API Standards)
...
## 2026-04-05 Parity Refresh

The agent API contract has been refreshed to match current runtime behavior:
- `POST /v1/agents` emits registry creation events consumed by the live socket layer.
- `PUT /v1/agents/{id}` now applies `mcp_tools`, `connector_configs`, `category`, `created_at`, `last_pulse`, and token usage updates consistently.
- Agent payloads support compatibility across legacy and current client shapes, including string-or-object model input and camelCase/snake_case aliases for lifecycle and token fields.
...
```mermaid
sequenceDiagram
    participant UI as ops_dashboard
    participant Gateway as Axum Gateway
    participant Runner as agent_runner
    participant Tool as mcp_host/Skill
    participant OTel as Telemetry (OTel)

    UI->>Gateway: POST /v1/agents/:id/tasks (Auth/Trace)
    Gateway->>OTel: Start Root Trace
    Gateway->>Runner: Spawn Mission Task
    Runner->>OTel: Create Phase Span (Setup)
    Runner->>Tool: Request Tool Execution
    Tool->>OTel: Create Tool Span
    alt Requires Oversight
        Tool->>UI: [WS] oversight:new
        UI-->>Tool: User Approve
    end
    Tool-->>Runner: Result String
    Runner-->>Gateway: Final Synthesis
    Gateway-->>UI: 200 OK (Problem Details on Error)
```
...
| `GET` | `/v1/agents/graph` | ✓ | Get Swarm Topology Graph (Nodes & Edges) |
...
| `swarm:pulse` | `[Binary MessagePack]` | **The Pulse**: High-speed (10Hz) binary updates for the Swarm Visualizer. Prefixed with header `0x02`. |
| `agent:create` | `{ agentId, data: engine_agent }` | New agent registered in the live registry |
| `agent:status` | `{ agent_id, status }` | Agent status change (thinking, idle, etc.) |
| `agent:message` | `{ agent_id, text }` | Agent output text |
| `agent:update` | `{ agentId, data: engine_agent }` | Full agent state sync |

...
### Phase 2.5: Mythos Recurrence (Internal Reason)
If the agent is configured with `reasoning_depth > 1`, the engine enters a recurrent loop.
- **Neural Pulse**: The runner broadcasts `current_reasoning_turn` at 10Hz for real-time visualization.
- **ACT (Adaptive Computation Time)**: Reasoning continues until a halting signal (`<halt/>`) or the `set_confidence` tool is triggered.
- **Hygiene**: The engine summary-prunes monologues exceeding 8k characters to ensure context window stability.
- **RAII Safety**: A state guard ensures all telemetry resets regardless of the loop's exit path.

...
- [1. Security & SSH Setup](#-1-security--ssh-setup)
...
```mermaid
graph TD
    A[Windows Dev Machine] -->|Package tar| B(Remote Bunker)
    B -->|Unpack| C{Docker Build}
    C -->|Success| D[Run tadpole-os:latest]
    C -->|Failure| E[Rollback to tadpole-os:rollback]
```
...
The deployment scripts in `scripts/deploy/` automate the entire distribution lifecycle:

### Pre-Flight Checks
Before triggering a deployment, always run the integrity suite on your development machine:
1. `python execution/verify_all.py` — Ensures the engine and services are stable.
2. `python execution/parity_guard.py` — Confirms that all routing changes are reflected in the documentation.

### Execution
Run the specific script for your target bunker from the workspace root:
```powershell
# Deploy to Bunker 1 (Standard)
./scripts/deploy/deploy-bunker-1.ps1

# Deploy to Bunker 2 (High RAM)
./scripts/deploy/deploy-bunker-2.ps1
```
...
- **Persistent Data**: Ensure your Docker Compose configuration maps a host volume to the container's `/data/` directory. Tadpole OS uses a **Split-Brain Database** architecture:
    - `tadpole.db` (SQLite): Persists agents, missions, logs, budgets, and sync manifests.
    - `memory.lance` (LanceDB): Manages high-dimensional vector embeddings for long-term memory.
    - `data/workflows/` (Markdown): Contains deterministic SOP workflow definitions for the Workflow Engine.
...
- **Swarm Insight**: The engine broadcasts a 10Hz binary pulse (`0x02`) for the **Swarm_Visualizer**. Ensure the target host has sufficient CPU to aggregate and broadcast MessagePack telemetry under high agent counts.

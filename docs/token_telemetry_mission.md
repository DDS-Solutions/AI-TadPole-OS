...
> - **Subsystem**: Architecture & Documentation / Core Docs / token_telemetry_mission
> - **Architecture**: `@docs ARCHITECTURE:Documentation`
> - **Failure Path**: Information drift, legacy terminology, or documentation mismatch.
> - **Observability**: Traceability via `execution/parity_guard.py` (`[token_telemetry_mission]`)
...
1. The **Swarm Dispatcher** (defined in [dispatcher.rs](../server-rs/src/agent/runner/swarm/dispatcher.rs)) manages execution loops.
2. Upon completion of each agent's execution turn, [finalize.rs](../server-rs/src/agent/runner/finalize.rs) is called:
   ...
   - **Mission Aggregation**: Updates the `mission_history` table's `cost_usd` column (via `update_mission` in [mission.rs](../server-rs/src/agent/mission.rs)).
...
2. The [useDashboardData](../src/hooks/use_dashboard_data.ts) hook fetches the agent registry into the Zustand `agent_store`.
...
4. The [Ops_Dashboard.tsx](../src/pages/Ops_Dashboard.tsx) renders these values in key telemetry cards at the top of the command panel.

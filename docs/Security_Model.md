...
- **Isolated Ledger Database**: Financial records are stored in a dedicated `agent_transactions` table with transaction-level SQL isolation, separate from standard agent configuration and memory tables, ensuring non-repudiability and prevention of double-spending or unauthorized balance drift.

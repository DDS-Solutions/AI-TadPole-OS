...
- [`swarm.json` Specification](#swarmjson-specification)
  - [roster](#roster)
  - [Agent Definition File (`agents/*.json`)](#agent-definition-file-agentsjson)
...
│   ├── archivist.json
│   └── researcher.json
├── skills/                  # Local tool scripts (.json) or documentation (.md)
│   ├── search_db.json
│   └── custom_logic.md      # New: Markdown-based skill definitions
...
```json
{
  ...
  "budget_usd": 50.00,
  "working_memory": {
    "mission_milestones": ["Initial research complete", "Drafting brief"]
  },
  "skills": ["issue_alpha_directive", "delegate"],
  "workflows": ["workflows/contract_intake.md"],
  "visualizer_color": "#00ffcc"
}
```

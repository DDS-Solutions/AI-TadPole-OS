...
**Source of Truth**: `server-rs/src/agent/runner/swarm.rs`, `server-rs/src/agent/runner/mod.rs`
...
- **Defensive Ceilings**: Background tasks adhere strictly to assigned `budgetUSD` ceilings. If an autonomous task encounters `maxFailures` consecutive LLM or tool crashing states, the Continuity Scheduler will automatically suspend the job to prevent unbounded token expenditure.## 4d. Conductor DAG Planning & Topological Scheduling

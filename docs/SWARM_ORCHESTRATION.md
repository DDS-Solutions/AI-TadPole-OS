...
**Source of Truth**: `server-rs/src/agent/runner/swarm/dispatcher.rs`, `server-rs/src/agent/runner/mod.rs`
...
graph TD
    User["Human Operator"]
    CEO["CEO: Agent 1 (Strategist)"]
    COO["COO: Agent 2 (Tactician)"]
    Specialist["Specialist: (e.g. Coder, Researcher)"]
    
    User -- "Global Objective" --> CEO
    CEO -- "Strategic Intent" --> COO
    COO -- "Decomposed Task" --> Specialist
    Specialist -- "Task Result" --> COO
    COO -- "Tactical Synthesis" --> CEO
    CEO -- "Strategic Verified Result" --> User

    subgraph "Recursive Safety"
        DepthLimit["Depth Limit: 5"]
        LineageCheck["Lineage Trace (No Loops)"]
    end
```
...
- [📡 Advanced Swarm Observability ("God View")](#7-advanced-swarm-observability-god-view)
...
- [📁 Workspace File Operations](#11-workspace-file-operations-in-swarms)
...
> [!IMPORTANT]
> **Neural Lineage Schema Required** *(IDENTITY.md Directive #2)*: Every `issue_alpha_directive` handoff must include all 5 fields of the Neural Lineage Schema. See [`directives/neural_handoff.md`](../directives/neural_handoff.md) for the mandatory format. Partial handoffs that omit `[Verified Facts]` or `[Pending Blockers]` are a protocol violation.
...
- **Bounded fan-out**: A direct `spawn_subagent` call accepts at most eight unique targets, and a model turn accepts at most sixteen parallel tool calls.
- **Global capacity**: `MAX_CONCURRENT_RUNNERS` applies to root agents and nested branches alike, so internal recruitment cannot bypass the operator's hardware ceiling.
- **Shared budget allocation**: Sibling branch allowances divide the parent's remaining budget; their aggregate allocation cannot exceed that remainder.
- **Structured fan-in**: Every branch returns an explicit `completed`, `failed`, `blocked`, or `cancelled` outcome before synthesis. Failure text is never silently treated as success.
...
## 4d. Conductor DAG Planning & Topological Scheduling
Tadpole OS introduces the **Conductor Engine** (`server-rs/src/agent/runner/conductor.rs`) to orchestrate multi-agent swarms using a directed acyclic graph (DAG) representation of subtasks.

### Conductor DAG Planning
When a high-level mission objective is received, the Conductor decomposes it into a list of structured steps (`ConductorStep`). Each step defines:
- **Title & Description**: The exact sub-mission prompt.
- **Model / Agent Assignment**: The agent template or ID assigned to execute it.
- **Dependencies**: A list of step IDs or names that must complete successfully *before* this step can start.

This allows the engine to map out a clear dependency tree instead of guessing step-by-step in an unstructured loop.

### Topological Swarm Scheduling
To prevent circular dependency deadlocks and execute subtasks in the correct logical sequence, the Conductor runs a **Topological Sort** (`topological_sort_conductor_steps`) utilizing Kahn's/DFS sorting on step dependencies.
- **Cycle Detection**: If the planning phase creates a circular dependency (e.g., Step A depends on Step B, which depends on Step A), the validator immediately errors, preventing runtime deadlocks.
- **Sequential & Parallel Execution Scheduling**: Steps with zero dependencies are run immediately. As steps finish, their dependents are queued, enabling both concurrent parallel branches (when steps share no mutual dependencies) and strict sequential ordering (when a step relies on predecessor outcomes).
- **Failure propagation**: A failed or cancelled dependency marks downstream steps `blocked`; successful independent branches are preserved for operator review instead of being discarded.
- **Plan validation and accounting**: Plans are limited to sixteen validated steps, and the model usage spent creating the plan is included in mission cost and token totals.

## 4e. Strategic Context Sandboxing
To avoid context explosion and information leakage, the swarm implements **Strategic Context Sandboxing** when spawning sub-agents:
- **Transcript Stripping**: The parent agent's internal monologue, raw tool executions, and system-level telemetry are stripped out.
- **Observation Injection**: Only the `visible_transcript` observations (e.g., critical user messages, verified tool findings, and explicit handoff results) are injected into the sub-agent's prompt window.
- **Data Leakage Prevention**: Sub-agents remain decoupled from the parent's sensitive internal deliberations, improving model accuracy by eliminating irrelevant telemetry clutter.
...
> `recruit_specialist(agent_id: "researcher_b", message: "Analyze theoretical cryptography breakthroughs")`
...
### 1. The Budget Gate *(IDENTITY.md Directive #7)*
Every `WorkflowRun` is assigned a cumulative `budget_usd`. The **Budget Guard** uses **Debounced Persistence** to track the aggregate real-time spend of all agents. Upon threshold breach, the system enters **`STASIS` mode** — it does NOT simply terminate the mission:
- All active Specialist Nodes are suspended (no new tool calls).
- All current task states are snapshotted to the Merkle Audit Trail.
- A `BUDGET_BREACH` alert is emitted to Entity 0 with: current spend, threshold breached, and active task inventory.
- **The system will NOT self-resume.** Entity 0 must issue an explicit `RESUME` (with reallocated budget) or `TERMINATE` command.

> See `docs/ERROR_REGISTRY.json` entries `BUDGET_BREACH` and `STASIS_ACTIVE` for remediation paths.
...
- **Parent ownership**: Child specialists write results and costs to the shared mission ledger but cannot mark the parent mission completed or failed. Partial fan-in pauses the parent for an explicit human retry, resume, or close decision.
...
- **Cost Awareness**: Always set a `budget_usd` for hierarchical swarms. Recursive spawning can consume tokens quickly. **Real-time USD burn and token usage** are tracked per-node on the agent card.
...
- **Bulk Capability Syncing**: Operators can perform a **Sovereignty Sync** from the Skills Hub (orchestrated client-side by invoking individual agent update and tool registration endpoints sequentially), pushing a single MCP tool or workflow to an entire department cluster in one turn. This ensures architectural parity across the swarm without manual node-by-node configuration.
...
### 6. Shared Tool Cache & Conflict Manager
To optimize performance and maintain integrity in high-concurrency environments:
- **Shared Idempotent Tool Caching**: Implements a `SharedToolCache` for local, read-only tools. It caches output strings and token usages, checking file modification times (`mtime`) and sizes before declaring a hit. Any write/delete operation invalidates the cache entries. Network tools are excluded to prevent stale responses.
- **Concurrent Lease Locks**: Mutating tools lease workspace file paths via a `ConflictManager` to prevent race conditions. If a path is already leased, other agents receive retryable conflict errors. Active leases feature a 30-second TTL auto-expiration window to protect against starvation or deadlocks in case of agent runner exceptions.

## 7. Advanced Swarm Observability ("God View")

Tadpole OS provides a real-time, hierarchical overview of the entire swarm execution via a high-performance binary telemetry pipeline.

### 7.1 Swarm Pulse: The Binary Pipeline (`0x02`)
To maintain sub-millisecond parity between the backend and UI, the engine utilizes a custom binary protocol hosted over the primary WebSocket hub.
- **MessagePack Serialization**: Telemetry frames are encoded using `rmp-serde` for maximum density and zero-allocation parsing in the UI.
- **The Binary Header**: Pulse packets are prefixed with `0x02`, allowing the frontend multiplexer to route them directly to the high-speed `Swarm_Visualizer` buffer.
- **10Hz Cadence**: The pulse broadcasts at 100ms intervals, ensuring the "God View" reflects agent state, battery, signal, and mission relationships with zero perceived lag.

### 7.2 The God View (Swarm Visualizer)
Located in the **Engine Dashboard** (formerly Operations), the Swarm Visualizer uses a high-performance 2D force-graph to map the swarm's topology.
- **Real-time Topology**: Visualizes agents as nodes and mission-recruitment events as dynamic edges.
- **Status Pulsing**: Nodes exhibit neon glow behaviors based on their binary status (Cyan: Thinking, Zinc: Idle, Red: Error).
- **Detach & Recall**: Operators can detach the visualizer into a specialized **Portal Window** for multi-monitor command center setups. A "Recall" button on the Engine Dashboard ensures seamless re-attachment.
- **Interactive Telepresence**: Clicking a node in the graph instantly focuses the agent and their mission logs in the **Sovereign_Chat**, providing a seamless transition from aggregate oversight to individual coordination.
...
The Mission Cluster detail view is enhanced with a **Neural Map** for real-time visual feedback on cluster connectivity.

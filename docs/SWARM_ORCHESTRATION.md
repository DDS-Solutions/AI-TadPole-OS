...
The Alpha merges the researcher data in the **synthesis phase**.
...
### 1. The Budget Gate
Every `WorkflowRun` is assigned a cumulative `budget_usd`. The `BudgetGuard` tracks the aggregate spend of all agents within the workflow. If Step 2 exhausts the budget, Step 3 is automatically cancelled.
...
- **Skill Manifests (Sapphire Phase 1)**: All capabilities are constrained by strongly-typed JSON schema requirements (`skill.json`).
- **Agent-Written Skills (Sapphire Phase 2)**: Agents can autonomously propose and physicalize new capabilities using the `propose_capability` tool. Upon human approval via the Oversight Dashboard, the skill manifest is instantly saved to disk and loaded into the `CapabilitiesRegistry`, expanding the swarm's abilities permanently.
- **Oversight Auto-Gate**: Tools marked with permissions for `budget:spend` or `shell:execute`, as well as all agent-written skills, automatically force an oversight lock, preventing agents from destroying host files or blowing API budgets without human approval via the **Oversight & Compliance** dashboard.
- **MCP Standardization**: All capabilities (skills, workflows, recruitment) are now routed through the **McpHost**. This ensures that every tool call follows a consistent protocol, providing better error handling and future-proof extensibility for external MCP servers.
...
The Tadpole Dashboard now provides **High-Level Telemetry Cards** for real-time monitoring of swarm health:
- **Recruitment Velocity**: Tracks how quickly new agents are being instantiated (Recruits Per Minute).

### 7.1. RESTful Observability (HATEOAS)

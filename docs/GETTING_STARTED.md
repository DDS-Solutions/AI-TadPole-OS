...
> **Prerequisites**: Tadpole OS running locally (`start_tadpole.bat`) or deployed on your Swarm Bunker. Node.js 20+ and Rust 1.80+ are required for the framework's modernized high-performance engine.
...
1. Open Tadpole OS in your browser:
   ...
   - **Production (Bunker)**: `http://<bunker-ip>:8000`
...
4. Click **Save Changes** — the dashboard auto-reconnects immediately without a page refresh.
...
6. The top-tier **PageHeader** should show **🟢 ONLINE** and display server-side telemetry.

> [!TIP]
> **Multi-Monitor Setup**: Tadpole OS supports **Detachable Portals**. Click the **External Link** icon on any tab in the Multi-Tab Bar to "pop out" that context into a dedicated window. This allows you to spread your command center across multiple screens.


> **Windows Users**: For reliable database persistence, set your `DATABASE_URL` using an **absolute path** (e.g., `sqlite:D:\TadpoleOS-Dev\tadpole.db`). Relative paths in Rust on Windows can sometimes fail to resolve across different startup contexts.
...
For rapid SME deployment, Tadpole OS: 1. Choose a **Sovereign Starter Kit** (Marketing, Customer Success, Finance). See the full [Starter Kits Guide](docs/STARTER_KITS.md) for instructions on "one-click" swarm installation.

---

## Step 4: Local Intelligence (Local LLMs via Ollama)
2. Follow the [Qwen3.5-9B Local Integration Guide](docs/QWEN_LOCAL_INTEGRATION.md) for detailed setup.
3. Once configured, you can add local models similarly to the steps below.
...
## Step 4.5: Bulk Capability Assignment (Skills Hub)
...
## Step 4.6: Importing External Capabilities (.md)
...
8.  **Hierarchical Recruitment**: High-level agents (Alphas) can recruit ephemeral sub-agents. The engine uses the **`recruit_specialist`** MCP tool to delegate tasks with strategic context handoffs.
...
10. **Neural Map Visualization**: Toggle the "Neural Map" icon on the mission dashboard to see a real-time SVG visualization of cluster connectivity, featuring animated traces and node status glows.
11. **Neural Swarm Optimization**: When you type an objective, you may see a "Neural Swarm Optimization Proffered" banner. This is the AI suggesting a "Best Practice" setup. 
12. **Recursion Guard**: To prevent infinite token burn, the engine enforces a maximum **Swarm Depth of 5**.
13. **Mission Analysis**: Toggle the **"Analysis"** switch next to the Run button to trigger a post-mission debrief by the Success Auditor (Agent 99).
...
## Step 7b: SME Data Intelligence (Connectors & Workflows)
...
## Step 8: Send a Task & Get Results
...
> [!NOTE]
> Replace `YOUR_NEURAL_TOKEN` with the value from your `.env` file. If running locally in dev mode, the default is `tadpole-dev-token-2026`.
...
- **Swarm Velocity**: Track recruitment speed (Recruits Per Minute).
- **Swarm Depth**: See hierarchical nesting levels as they happen.
...
## Step 10: The Swarm Template Ecosystem (New)

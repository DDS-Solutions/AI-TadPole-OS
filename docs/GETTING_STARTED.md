...
| :--- | :--- | :--- |
| `DATABASE_URL` | Path to `tadpole.db` | **Absolute path REQUIRED on Windows** (e.g., `sqlite:D:\TadpoleOS-Dev\data\tadpole.db`). Defaults to `data/tadpole.db` relative to engine CWD. |
...
| `ANTHROPIC_API_KEY` | Claude Provider Key | Optional |
...
| `GOOGLE_API_KEY` | Google Gemini Provider Key | Optional |
| `INCEPTION_API_KEY` | Inception AI Provider Key | Optional |
...
3. Click **CSV Catalog**, **QuickBooks**, or **PDF Pricelist** to ingest business data into the outward catalog.
4. Preview your published `agent-card.json` in the live **A2A Card Preview** card and copy your public endpoint URL.

...
> **Version**: 1.1.396  
> **Last Hardened**: 2026-08-08 (SEC-05 & SEC-06 Granular Capability Governance update)  
...
A dedicated, siloed management hub for SMB owners to configure public customer-facing AI agents and expose `a2a-protocol.org` compliant agent cards (`agent-card.json`).
...
- **1-Click SMB Data Ingestion**:
    - **CSV Catalog Importer**: Ingests product/service CSV rows with quote-aware line splitting.
    - **QuickBooks Schema Importer**: Parses QuickBooks invoice/product JSON payloads into catalog items.
    - **PDF Pricelist Parser**: Converts text/PDF pricelists into structured catalog entries.
- **Published A2A Agent Card Preview (`A2A_Card_Preview`)**: Live visual preview displaying exposed agent skills, version, copyable well-known JSON endpoint URL, and active local model profile.

...
Operational expectations were refreshed to match the live agent lifecycle:
- **Create vs Update Semantics**: New agents are now persisted through the dedicated create route rather than the update path, preventing silent partial registration.
- **Registry Telemetry**: The frontend now reacts to both `agent:create` and `agent:update`, so newly provisioned agents appear immediately in live views.
- **Lifecycle Timestamps**: `created_at` and `last_pulse` are treated as authoritative runtime fields for UI recency, ordering, and operator observability.
- **Token Accounting**: Partial token updates now recompute totals consistently, so displayed usage and persisted usage stay in sync during live operations.
- **Runner Path Resolution**: Swarm orchestration now documents the runner-owned workspace/base-dir resolution flow used to keep mission scope, agent memory, and filesystem context deterministic.

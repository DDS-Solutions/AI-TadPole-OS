...
> - **Subsystem**: Architecture & Documentation / Core Docs / OPERATIONS_MANUAL
> - **Architecture**: `@docs ARCHITECTURE:Documentation`
> - **Failure Path**: Information drift, legacy terminology, or documentation mismatch.
> - **Observability**: Traceability via `execution/parity_guard.py` (`[OPERATIONS_MANUAL]`)
...
- **Mode-Switched Utility Cluster** [DEFINE: Mode-Switched Utility Cluster]: A consolidated multi-role agent team template with strict operational boundaries and sub-routine mode switching, designed to minimize database index bloat and state synchronization latency across the swarm.
- **Team Cluster Preset** [DEFINE: Team Cluster Preset]: A reusable workspace configuration detailing a team cluster name, badge tag, default USD budget, department, and assigned multi-role agent collaborators.
...
- [3. Shared Folder & Asset Management](#3-shared-folder--asset-management)
  ...
  - [3.2 Dynamic Environment Detection & OKF Governance](#32-dynamic-environment-detection--okf-governance)
  - [3.3 Task Draft Branching Workflow](#33-task-draft-branching-workflow)
...
- [5. Security, Approvals & Budget Safeguards](#5-security-approvals--budget-safeguards)
  - [5.1 Oversight Gate & Approval Queue](#51-oversight-gate--approval-queue-oversight)
    ...
    - [5.1.3 Passive Mirror Mode (Digital Twin)](#513-passive-mirror-mode-digital-twin)
...
- [6. The Workspace Heartbeat (Observability)](#6-the-workspace-heartbeat-observability)
  ...
  - [6.4 Latency Benchmarks & Dual-Trace Analytics](#64-latency-benchmarks--dual-trace-analytics-benchmark_analytics)
...
### 1.4 Neural Map & Cognition Visualizer Operations

The **Neural Map** provides real-time 2D force-graph visualization of structural codebase references (Tree-sitter AST symbol nodes) and semantic OKF knowledge networks. Clicking any node opens the **Cognition Sidebar Inspector** (`CognitionSidebar.tsx`), displaying high-fidelity operational metadata:

- **Topic Space** (`<hash>/<relative_path>`):
  - Displays the obfuscated workspace hash prefix followed by the relative file path.
  - Enforces **Security Policy SEC-05 (Path Obfuscation)**: Absolute local paths are salted and hashed on the backend before UI rendering to prevent filesystem path leakage.
- **Concept Type** (`general`, `sop`, `playbook`, `metric`, `symbol`):
  - Categorizes the node according to the Open Knowledge Format (OKF v0.2) taxonomy.
  - Denoted by monochromatic Lucide iconography in the sidebar.
- **Confidence Score** (`0%` to `100%`):
  - The Multi-Factor Scoring (MFS) confidence decay metric.
  - Newly ingested structural AST nodes start at `0%` baseline until validated by active agent mission execution. Human-authored directives and active SOPs carry high confidence (80%–100%).
- **Governance State** (`Confirmed` vs `Pending Review`):
  - Evaluates Sapphire Shield Zero-Trust operator verification (`human_confirmed` flag).
  - **🟢 Confirmed**: Manually verified by an operator (`human_confirmed = true`).
  - **🟡 Pending Review**: Autonomously ingested node awaiting human operator sign-off (`human_confirmed = false`).

#### 1.4.1 Static Code Anomalies Panel (`AnomalyPanel.tsx`)

Embedded directly within the Neural Map HUD, the **Anomalies Panel** tracks structural code health and static analysis findings detected by the Tree-sitter AST engine during workspace indexing.

* **Primary Purpose**:
  - **Dead Code & Drift Detection**: Identifies unused symbols (`UNUSED_SYMBOL`), unreferenced functions, dangling imports, and disconnected nodes across Rust and TypeScript files.
  - **Automated Refactoring Prompts**: Features a one-click **"Copy Prompt"** button (`anomaly_prompt_template`) that compiles all active workspace anomalies into a formatted refactoring instruction set, ready for submission to an AI agent swarm.
  - **Interactive Node Jump**: Clicking any listed anomaly centers and highlights the corresponding node directly on the 2D Neural Map graph for visual blast-radius inspection.

**Interactive Elements:**
- **[Button] Copy Prompt**
  - *Default state*: Active (`anomaly_prompt_template`).
  - *Visible location*: Top header controls of `AnomalyPanel.tsx`.
  - *Observable side effect*: Copies formatted static analysis anomalies into the clipboard, ready to paste into `SovereignChat` for automated refactoring.
- **[Card] Anomaly Node Item**
  - *Default state*: Unselected list row.
  - *Visible location*: Scrollable list inside the Anomalies Panel HUD.
  - *Observable side effect*: Centers and highlights the corresponding AST symbol node on the 2D Neural Map graph for visual blast-radius inspection.
- **[Filter] Anomaly Severity Selector**
  - *Default state*: All Severities.
  - *Visible location*: Header filter row of `AnomalyPanel.tsx`.
  - *Observable side effect*: Filters displayed static code findings by severity classification (Critical, Warning, Info).
...
#### 1.3.5 Outward Customer Catalog & A2A Gateway (`/customer-catalog`)
A dedicated, siloed management hub for SMB owners to configure public customer-facing AI agents and expose `a2a-protocol.org` compliant agent cards (`company-agent-card.json`).

**Core Features & Interfaces:**
- **Silo Isolation Barrier**: Completely isolates customer-facing agent interactions from internal engineering codebase symbol graphs and system files.
- **Outward Local Model Runner**: Binds customer agent execution to local model profiles (`gemma4:e4b`, `gemma4:e8b`, `gemma4:full`) for low-latency, zero-cloud-leak customer inquiries.
- **Catalog Import Boundary**: CSV and QuickBooks JSON are validated before shared catalog state is locked. Imports are capped at 512 KiB per field, malformed or empty payloads return RFC 9457 `BAD_REQUEST`, and a failed import leaves the catalog unchanged. PDF ingestion is unavailable and the interface directs operators to export CSV or QuickBooks JSON instead of reporting fabricated success.
- **Authentication & Profile Sync**: `POST /a2a/v1/catalog/import` and `PUT /a2a/v1/profile` require the configured bearer token. The manager saves profile changes through the shared authenticated API client after a 300 ms debounce and cancels superseded requests. Public discovery and search remain token-free and use a 60-request-per-minute IP fixed window.
- **Business FAQ & Operating Info Card Manager (`Company_Info_Card_Manager.tsx`)**:
    - **Interactive CRUD Cards**: Create, edit, and delete business knowledge cards (FAQ, Operating Info, Policies, Custom) dynamically.
    - **Tabbed Category Filter**: Filter knowledge cards instantly by category tab (`All`, `FAQ`, `Operating Info`, `Policies`, `Custom`).
    - **Real-Time PII & Luhn Algorithm Guard**: Live form warning alert detecting credit card numbers (validated via Luhn Mod-10 algorithm), Social Security Numbers, and secret API keys before saving.
    - **Delete Confirmation Shield**: Employs system-wide `Confirm_Dialog` modal for safe deletion confirmation.
- **Published A2A Company Agent Card Preview (`A2A_Card_Preview`)**: Live visual preview displaying exposed agent skills compiled from knowledge cards, version, copyable well-known JSON endpoint URL (`http://localhost:8000/a2a/v1/company-agent-card.json`), and active local model profile.
...
Child specialists share the parent mission ledger but cannot close it independently. Each branch records its own cost and structured outcome (`completed`, `failed`, `blocked`, or `cancelled`). If any branch is incomplete, the Department Lead preserves successful work, pauses the parent mission, and presents it for human retry, resume, or closure. Independent Conductor steps may run concurrently in bounded waves; steps whose dependencies fail remain visibly blocked instead of being reported as successful.

#### Workspace Cluster Limits & Concurrent Execution
To prevent system overload on local nodes, operators can run **multiple concurrent active clusters** simultaneously. Active clusters process missions in parallel up to the `MAX_CLUSTERS` engine cap (default: `10`). If the active workspace count matches this limit:
...
#### 2.1.1 Mode-Switched Utility Clusters & Team Management Studio
To reduce database index sizes, configuration loading times, and state synchronization latency across large swarms, operators can group multi-role agents into **Mode-Switched Utility Clusters**:
- **Concurrent Swarm Execution**: Multiple mission clusters can be toggled to `ACTIVE` simultaneously, executing parallel swarm operations across isolated work folders without forced single-active deactivations.
- **Cluster Studio (`[CLUSTERS]`)**: Accessible via the `CLUSTERS` studio button next to `+ NEW MISSION` on the Missions sidebar. Opens the **Utility Cluster Configuration Studio** modal ([Cluster_Manager_Modal.tsx](../src/components/missions/Cluster_Manager_Modal.tsx)) to define reusable team presets with custom badge tags (e.g. `SEC-AUDIT`, `ENG-SWARM`), budget allocations, and multi-role agent memberships.
- **Deploying Team Presets (`+ ADD TEAM CLUSTER PRESET`)**: Inside the `NEW MISSION` creation drawer, operators can expand the team preset gallery to populate pre-configured team parameters in a single click.
- **Editable Pre-fill Collaborator Toggle**: Selecting a team preset pre-fills all defined multi-role agents into an interactive checkbox list, allowing operators to fine-tune assigned collaborators before creating the project workspace.
- **Sidebar Team Badges**: Active project workspace cards displaying multi-role team presets render a high-visibility `🏷️ [TEAM: BADGE]` tag below the budget readout for instant visual identification.
...
- **Environment Detection**: Automatically detects host runtime sandboxes, developer shells, and container configurations.

→ *See §4.3 for local data synchronization details.*

### 3.2 Dynamic Environment Detection & OKF Governance
...
- **Dynamic Environment Detection**:
  - Automatically probes the host for sandboxes (`docker`, `wasm_sandbox`), execution interfaces (`tauri_shell`, `vs_code`, `jupyter_lab`), and orchestration environments (`k8s_node`, `headless`).
  - Displays detected platforms on the Workspaces dashboard using monochromatic Zinc-scaled badges to minimize visual clutter while retaining full environment observability.

- **OKF Playbook Safety Gates**:
  - Active workspaces are checked against security rules declared in OKF playbooks (e.g., `requires: docker` or `requires: k8s_node`).
  - If a required host environment is missing (e.g., Docker daemon is offline but a playbook requires containerization), the validation state transitions to `warning` or `critical`, presenting a high-contrast safety warning banner on the dashboard in semantic colors (`cyber-amber` or `cyber-red`).

- **Playbook Auto-Mounting & Caching**:
  - The backend engine resolves active OKF playbooks assigned to a cluster's ID and automatically mounts them to the cluster's directory on disk as a read-only local JSON file (`playbooks.json`), allowing local agents to access playbooks and constraints without direct database queries.
  - Active playbooks are rendered on the dashboard as clickable badges that deep-link to the **Semantic OKF Graph** page for visual inspection of Institutional Knowledge Store (IKS) relationships.

**Interactive Elements:**

- **[Card] OKF Playbook Badge**
  - *Default state*: Populated with the mounted playbook's title.
  - *Visible location*: "OKF Governance" card inside the Workspace Cluster details.
  - *Observable side effect*: Clicking deep-links to `/knowledge?node=<id>` to display its node relationships in the Semantic OKF Graph.
  - *Keyboard shortcut*: None.
  - *Missing in build*: Always present.

- **[Banner] Safety Alert Banner**
  - *Default state*: Hidden (Nominal).
  - *Visible location*: Middle section of the "OKF Governance" card inside the Workspace Cluster details when validation status is `warning` or `critical`.
  - *Observable side effect*: Renders error details and missing environment requirements.
  - *Keyboard shortcut*: None.
  - *Missing in build*: Always present.

### 3.3 Task Draft Branching Workflow
...
> [!CAUTION]
> **Browser Secure Context Requirement (`http://192.168.x.x:5173`)**:
> The Web Crypto API (`crypto.subtle`) strictly requires a **Secure Context** (`https://` or `http://localhost`). Accessing the dashboard remotely over plain HTTP (e.g., `http://10.0.0.1:5173`) will cause browser security policies to disable Web Crypto, preventing the Neural Vault from unlocking. For remote deployment access, configure HTTPS/TLS certificates or connect through a local SSH tunnel or reverse proxy.
...
> - `api.openai.com` (`OPENAI_API_KEY`) — cited in [resolution.rs:L384](../server-rs/src/agent/runner/provider/resolution.rs#L384) & [model_manager.rs:L172](../server-rs/src/routes/model_manager.rs#L172)
...
> - `api.inceptionlabs.ai` (`INCEPTION_API_KEY`) — key resolution cited in [resolution.rs:L393](../server-rs/src/agent/runner/provider/resolution.rs#L393)
> - `api.deepseek.com` (`DEEPSEEK_API_KEY`) — key resolution cited in [resolution.rs:L396-414](../server-rs/src/agent/runner/provider/resolution.rs#L396-L414)
...
#### 4.1.3 Remote Companion Pairing and Revocation

Open **Remote Oversight & Mobile Mesh Settings** while authenticated with the
desktop `NEURAL_TOKEN`, then select **Display Pairing QR Code**. Token creation
must succeed on the engine before the QR dialog opens; there is no client-side
fallback token. Scan the QR code in the companion app, then select **Refresh
Pairing Status**. The desktop lists a device only after the engine confirms its
registered Ed25519 public key.

Edits and revocations are also server-confirmed. If an operation fails, the
device remains unchanged in the interface and the error banner identifies the
failure. Revoked companions immediately lose signed-route access. Companion
requests require `X-Device-Id`, `X-Timestamp`, `X-Nonce`, and `X-Signature`;
stale or replayed requests are denied.

On Android, both approval and rejection decisions require biometric or device
credential confirmation. A failed registration, signature, or server command
must leave the displayed authoritative state unchanged; retry after confirming
the desktop address and that the device has not been revoked.
...
- **Governance Tab**: Set individual budget caps, toggle the "Requires Oversight" setting, and configure A2A Economic Zone daily spend limit caps to restrict micropayment transfers.
...
- **Emergency Kill Switch (`POST /v1/engine/kill`)** — `@docs OPERATIONS_MANUAL:EmergencyKill`
  - Halts all active agent execution loops and tool invocations, resets statuses to `idle`, clears in-flight mission/task handles, purges pending oversight items, and flushes persistent state.
- **[Button] Halt Agents**
...
#### 5.1.3 Passive Mirror Mode (Digital Twin)
- **Passive Observation**: When the system is operated as a digital twin shadowing live systems, the operator can enable Mirror Mode by setting `MIRROR_MODE=true` in the environment (`.env`).
- **Mirror Mode Status API (`GET /v1/engine/mirror/status`)** — `@docs OPERATIONS_MANUAL:MirrorStatus`
  - Returns current active mirror mode configuration, passive intercept states, and recent drift alerts.
- **Drift Alerts & Interception**: Under Mirror Mode, any attempt by an agent to execute a mutating or dangerous tool (such as modifying filesystem files, running terminal scripts, or writing to external connections) is intercepted. Instead of executing the mutation:
  - The tool execution is blocked safely.
  - A drift alert entry is registered in the engine state.
  - An `oversight:drift` event is broadcasted over WebSockets to notify the dashboard in real-time.
  - A warning message is returned to the agent's context explaining that the passive twin boundary was enforced.
- **Mirror Mode Status API**: Operators can query `GET /v1/engine/mirror/status` to fetch the current mirror mode configuration and list of active drift alerts.

### 5.2 Security Dashboard & Cost Quotas
- **Governance Settings & Policy API (`GET /v1/oversight/settings`, `PUT /v1/oversight/security/policies`)** — `@docs OPERATIONS_MANUAL:GovernanceSettings`
  - Governs global engine rate limits, auto-approve safe skills toggle, default agent and mission USD budgets, maximum swarm depth, and stasis threshold configurations.
...
- **[Metric] Swarm Health & Agent Metrics (`GET /v1/oversight/security/health`)** — `@docs OPERATIONS_MANUAL:AgentHealth`
...
- **OPEN**: Triggered when a service namespace encounters 5 consecutive failures (`VITE_SYSTEM_BREAKER_FAILURE_THRESHOLD` [RISK: HIGH]). In this state, further calls to the namespace are instantly rejected on the client side with a `CircuitBreakerOpenError` to prevent UI freezing.
  - *UI Visibility when OPEN*: The dashboard header ticker updates to **🔴 DISCONNECTED (RETRYING IN 10s)**, a toast notification alerts `🚨 Circuit Breaker OPEN: Service [namespace] temporarily offline`, and action buttons requiring that service namespace enter a disabled retry state until probing completes in `HALF_OPEN`.
...
2. **Environmental Drift**: Flags Rust logic calling `std::env::var("VAR")` that is not declared in `.env.example`.
...
### 6.4 Latency Benchmarks & Dual-Trace Analytics (`Benchmark_Analytics`)

The **Performance Analytics Hub** ([Benchmark_Analytics.tsx](../src/pages/Benchmark_Analytics.tsx)) provides real-time hardware profiling and execution trace visualization across all autonomous agent nodes in the swarm.

#### 6.4.1 Hardware Latency Benchmarks (`/benchmarks`)
- **Interactive Performance Profiling**: Displays mean execution latencies, P95/P99 distributions, target values, and pass/fail statuses across all registered test categories (`BM-RUN`, `BM-DB`, `BM-RL`, `BM-MCP`, `BM-WS`, `BM-FS`, `BM-HTTP`, `BM-SEC`).
- **On-Demand Execution**: Allows operators to trigger execution profiling with single-click runner buttons (`BM-RUN-01`, `BM-DB-01`, `BM-RL-01`) calling `POST /v1/benchmarks/run/:test_id`.
- **Historical Comparison**: Selecting multiple historical benchmark runs opens a side-by-side variance analysis showing latency deltas and percentage performance shifts.

#### 6.4.2 Dual-Trace Swarm Analytics (`/analytics`)
- **Side-by-Side Telemetry Playback**: Utilizes the `DualTracePlayback` inspector to compare unoptimized linear baseline directives against 100% certified World Model execution state machines.
- **Trace Event Inspection**: Visualizes hypothesis generation, certified backtest parity, discriminative probes, latency variance observations, and execution logs step-by-step.
- **Automated Comparison**: Navigating to `/analytics` automatically selects the latest two benchmark runs for immediate side-by-side inspection.

> [!TIP]
> **Technical Specification**: For Criterion benchmark setups, CLI usage (`cargo bench`), and hardware claim criteria, refer to [Benchmark_Spec.md](./Benchmark_Spec.md).
...
- `DEFAULT_MISSION_BUDGET_USD` [RISK: MEDIUM] — Type: number; Default: `5.0`. The default spending budget limit allocated to newly created missions.
...
- `MAX_CONCURRENT_RUNNERS` [RISK: MEDIUM] — Type: number; Default: `10`. Global upper limit across route-started agents and every nested swarm branch; excess branches fail with a retryable capacity response instead of bypassing the operator's resource ceiling.

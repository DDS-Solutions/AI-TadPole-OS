...
- **Local Access**: `localhost` and `10.0.0.1` are considered secure by default.
...
- **[Section] Governance & Oversight**:
    ...
    - **[Toggle] Privacy Shield (Shield)**: High-security "Hard Gate" that blocks all external cloud traffic (Gemini, OpenAI, etc.) at the engine level. Agents are restricted to local providers only. Displays "Air-Gap Verified" status in the global header when active.
...
| Workspace Root | `WORKSPACE_ROOT` | `.` | Absolute path to the project root for resolving `execution/` and `directives/`. |

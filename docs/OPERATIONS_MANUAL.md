...
Operational expectations have been hardened for **Level 5 AI Awakening**:
- **State Machine Sync**: 100% parity between `axum` state transitions and UI status indicators.
- **Detachable Portals**: State-preserved window detachment for all high-fidelity components.
- **Transaction Visibility**: Real-time token accounting with debounced SQLite persistence.
...
- [4. Intelligence Layer](#4-intelligence-layer)
  ...
  - [4.4 Agent-Generated Artifacts](#44-agent-generated-artifacts)
  - [4.5 System Telemetry](#45-system-telemetry)
  - [4.6 Performance Analysis](#46-performance-analysis)
  - [4.7 SME Data Intelligence](#47-sme-data-intelligence)
...
- [9. Maintenance & System Reliability](#9-maintenance--system-reliability)
- [10. Automated Mission Debriefing (Agent 99)](#10-automated-mission-debriefing-agent-99)
- [11. Tool Reference](#11-tool-reference)
- [12. Governance](#12-governance)
- [13. Communication Hub](#13-communication-hub)
- [14. Registry Hub](#14-registry-hub)
- [15. Security Hub](#15-security-hub)
...
### 2.1 Operations Center (`Ops_Dashboard`)
...
*   **Operational Note**: Real-time throughput and cost metrics are primarily monitored via the **Operations Center (`src/pages/Ops_Dashboard.tsx`)**.
...
- **Transmission Protocol**:
    ...
    - **[Button] Sync Models (IMR-01)**: Triggers the Intelligent Model Registry discovery process. This manually pings the provider and auto-populates the registry with inferred capabilities.

#### 4.1.2 Automated Model Discovery (IMR-01)
Tadpole OS implements automated feature inference for all discovered models. When a provider is synced, the engine analyzes model identifiers and cross-references them against the **Capability Matrix**.

**Capability Micro-Icons:**
- 👁️ **Vision**: Model supports image input and multimodal processing.
- 🛠️ **Tools**: Model is optimized for function calling and structured tool interaction.
- 🧠 **Reasoning**: Model utilizes internal reasoning chains (e.g., DeepSeek-R1, o1-style).

These icons appear in the **Agent Manager** and **Hierarchy** dropdowns to help operators select the most capable node for a specific mission.

    > [!NOTE]
> For local Ollama setup, see the [Qwen3.5-9B Local Integration Guide](./QWEN_LOCAL_INTEGRATION.md) for endpoint and key placeholder details.
...
### 4.4 Agent-Generated Artifacts (`agent_generated/`)
The **`agent_generated/`** root directory is the designated workspace for all code, scripts, and documentation synthesized by the swarm.

**Operational Procedures:**
- **Automatic Routing**: Agents are instructed to write new tool scripts or technical assets into this folder to maintain structural parity.
- **Review Protocol**: Assets in this directory should be audited before being promoted to the `directives/` or `execution/` folders.
- **Provenance Awareness**: This folder allows human operators to instantly distinguish between static system files and dynamic agent-authored content.
...
- **[Component] God View (Swarm Visualizer):** A high-performance, real-time 2D force-graph visualization of the swarm's topology.
    - **Live Binary Pulse**: Driven by a 10Hz MessagePack stream (`0x02` header) for sub-millisecond status parity (0.1s resolution).
    ...
    - **Detach Capability**: Use the **External Link** icon to pop the visualizer into a dedicated window for multi-monitor oversight.
...
| **System Neural Engine** | Tadpole OS | `data/models/` |
...
### 9.2 Autonomic Recovery (OBLITERATUS)
The engine implements **Recursive Quantization Fallback** for local LLM nodes.
- **Process**: If an agent receives an OOM (Out Of Memory) error from a local provider, it automatically appends `:q4_K_M` to the model name and retries.
- **Operator Awareness**: A system warning is logged, but the mission continues automatically, ensuring zero-touch persistence in low-VRAM environments.
...
## 10. Automated Mission Debriefing (Agent 99)

Tadpole OS features a self-annealing "Wisdom Extraction" system designed to convert tactical mission outcomes into persistent institutional knowledge.

### 10.1 The Wisdom Extraction Loop
Upon completion of a mission, the engine (or an operator via CLI) triggers **Agent 99** to perform a post-mission debrief.
1. **Log Analysis**: Agent 99 queries the `mission_history` in `tadpole.db` to retrieve the full execution trace.
2. **Key Learning Synthesis**: Using high-fidelity models (e.g., Llama-3.3-70b via Groq), the system identifies architectural discoveries, blockers, and optimization patterns.
3. **Memory Persistence**: The synthesized "Key Learnings" are automatically appended to `directives/LONG_TERM_MEMORY.md`.

### 10.2 Triggering a Debrief (CLI)
For operators who wish to manually trigger a debrief or run it on historical missions, the system provides a deterministic Python tool:

```powershell
# Run debrief for the most recent mission
python execution/debrief_mission.py --commit

# Run debrief for a specific mission ID
python execution/debrief_mission.py --mission_id <ID> --commit

# Dry-run (view insights without writing to memory)
python execution/debrief_mission.py
```

### 10.3 Institutional Memory (`LONG_TERM_MEMORY.md`)
This file is the **Source of Truth** for the swarm's evolving intelligence. It is automatically injected into every agent's system prompt, ensuring that discoveries made in Mission A (e.g., a specific API constraint) are known to the swarm in Mission B.

---

## Tool Reference

Tadpole OS provides agents with a high-fidelity suite of **Dynamic Skills**. These are classified into **Sovereignty Domains** to ensure rapid cognitive mapping during mission decomposition.

### 11.1 System & File Sovereignty
Direct interface with the local OS and filesystem, governed by the `filesystem` skill permit.
- **`filesystem`**: The master gateway skill for disk access.
- **`list_files` / `read_file` / `write_file` / `delete_file`**: Standard CRUD operations on the local workspace.
- **`resource_check`**: Performance monitoring of CPU/RAM/VRAM.

### 11.2 Complete Skill Manifest
The following skills are registered and available for agent recruitment:
`analyze_budget`, `analyze_feedback`, `api_test`, `archive_to_vault`, `brainstorming`, `check_server_health`, `code_audit`, `code_generation`, `code_review`, `cold_call`, `conflict_resolution`, `content_generation`, `contract_review`, `copywriting`, `customer_chat`, `database_query`, `data_analysis`, `deep_research`, `delete_file`, `edit_content`, `employee_onboarding`, `expense_tracking`, `fetch_url`, `figma_sync`, `filesystem`, `generate_image`, `git_push`, `issue_alpha_directive`, `keyword_research`, `knowledge_base_search`, `lead_qualification`, `list_files`, `market_research`, `monitor_mentions`, `notify_discord`, `performance_tracking`, `post_update`, `query_financial_logs`, `read_file`, `refactoring`, `resource_check`, `risk_analysis`, `scan_vulnerabilities`, `schedule_meeting`, `seo_analysis`, `system_audit`, `task_prioritization`, `test_skill`, `ticket_triage`, `ui_audit`, `unit_testing`, `update_crm`, `user_interview`, `view_logs`, `web_search`, `write_file`, `write_spec`.

---

## Governance

### 12.1 Dynamic Policy Governance
The Governance layer manages the high-level operational parameters of the swarm via a dynamic SQLite-backed registry.
- **Tool Permission Policies**: Define whether a tool is `allow`, `deny`, or `prompt` (oversight gate).
- **Runtime Updates**: Security policies can be modified in real-time through the **Governance Hub** without requiring a server restart.
- **Recruitment Limits**: Controls for maximum swarm depth and agent density.
- **Autonomous Scheduling**: Management of Jobs and Continuity workflows.
- **Self-Healing Loop**: Automated agent recovery and failure monitoring.

### 12.2 Asynchronous Oversight
Tadpole OS supports **Parallel Cognitive Continuity**, allowing agents to remain productive while awaiting human approval.
- **Task Parking**: When a gated tool (e.g., `git_push`) is triggered, the specific sub-task enters a **Yield Phase**.
- **Context Preservation**: The agent runner preserves the mission state and can be configured to pivot to alternative, non-gated tasks (if applicable) or wait asynchronously without blocking the rest of the engine.
- **Deterministic Resumption**: Upon approval, the engine re-activates the specific `oneshot` channel, and the agent resumes execution exactly where it yielded.

---

## Communication Hub

### 13.1 Neural Channels
- **Neural Chat**: The primary NLI (Natural Language Interface).
- **Voice Sync**: Real-time STT/TTS coordination.
- **Lineage Stream**: The historical audit of every neural transmission.

---

## Registry Hub

### 14.1 Infrastructure Inventory
- **Provider Manager**: Vault for API keys and infrastructure URLs.
- **Model Forge**: Inventory management for cognitive nodes.
- **Agent Registry**: The directory of all autonomous identities within the swarm.

---

## Security Hub

### 15.1 Defensive Layers
- **Oversight Gate**: The "Yield-to-Human" protocol for tool approval.
- **Quota Enforcement**: Daily and weekly budget caps in USD.
- **Merkle Ledger**: Cryptographically linked audit trail of all actions.

...
## 6.1 God View (Swarm Visualizer & Code Graph)
High-performance force-graph visualization of the swarm's topology and codebase dependency graph.
- **Live Binary Pulse**: Driven by a 10Hz MessagePack stream.
- **Agent Telepresence**: Click nodes to focus agent chat contexts.

### 6.1.1 Codebase Knowledge Graph Navigation HUD
When viewing the Codebase Knowledge Graph (under the Intelligence tab), the operator has access to a specialized HUD command toolbar at the top:
- **Back / Forward Navigation**: Implements browser-style state-tracking. Clicking a node focuses on it and pushes it onto a history stack, enabling operators to step backward and forward through their analysis path using the HUD controls.
- **Export PNG**: Captures a double-scale high-resolution screenshot directly from the active canvas context, allowing the operator to download a crisp representation of the dependency diagram.
- **Find Path (BFS Pathfinder)**: Triggers the BFS Pathfinder dialog modal to trace dependencies.
- **Copy AI Instruction**: Embedded in the Anomaly Panel, this allows operators to copy a pre-formatted instructional prompt containing AST syntax errors/cycles to paste directly into an AI agent chat window.

### 6.1.2 BFS Pathfinder Dialog
- **Bidirectional Traversal**: Allows the operator to select a **Source Symbol** and a **Target Symbol**.
- **Breadth-First Pathing**: Computes the shortest connection path of imports, inheritance, or calls between the symbols.
- **Visual Highlight Overlay**: Highlights the resulting path on the canvas with distinct rendering states (glowing halos and pulsating edges).
- **Interactive Steps**: Shows each step of the path as a list item. Clicking any step in the path will fly and zoom the viewport directly to that symbol on the active canvas.

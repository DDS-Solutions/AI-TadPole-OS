...
**Version**: 1.3.4
**Last Hardened**: 2026-09-09 (GEV Port 3000 MCP Conformance Contract, PreferHttp dual-transport state machine, fail-closed fallback matrix, bounded JSON/SSE parsing, catalog enforcement, operation identity, header injection guards, and secret redaction)
**Standard Compliance**: ECC-ARA (Enhanced Contextual Clarity)  
**Last Code/Docs Parity Check**: 2026-09-09 (Rust formatting, strict Clippy, 20 MCP 2026-07-28 conformance tests across gates 1-19, and zero-trust verification)
...
│   │   ├── mcp/         # Decomposed MCP Hub (host, authz, config, types, native, client/{stdio,http/{limits,headers,classify,body,sse,client},jsonrpc,port3000_conformance})

...
> - **Execution**: AgentRunner (unner/mod.rs) handles the stateful intelligence loop.
> - **Memory**: Hybrid approach using SQLite (Deterministic) and LanceDB (Vector/RAG).
> - **Safety**: Non-bypassable Merkle Audit Trail (udit.rs) and Budget Guard (metering.rs).
...
graph TD
    subgraph "Sovereign Layer (Frontend)"
        Dashboard["OpsDashboard (React 19)"]
        Registry["AgentStore (Zustand)"]
        Vault["NeuralVault (SubtleCrypto)"]
    end

    subgraph "Intelligence Layer (Backend)"
        Axum["Axum Gateway (0.8)"]
        State["AppState (state.rs)"]
        Runner["AgentRunner (runner/mod.rs)"]

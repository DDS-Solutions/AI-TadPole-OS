...
> - **Sovereign Hub**: state.rs (AppState) is the single source of truth for the swarm.
...
This refresh aligns the architecture guide with the current backend/frontend contract:
- **Agent Contract Symmetry**: `EngineAgent` now supports legacy and current payload shapes for reads while serializing a frontend-friendly response shape for `model`, `model_config`, `model2`, `model3`, `created_at`, and `last_pulse`.
- **Persistence Coverage**: SQLite agent persistence now round-trips `created_at` in addition to canonical primary model storage and existing mission/runtime metadata.
- **Realtime Registry Events**: The socket/event layer now treats `agent:create` and `agent:update` as first-class registry events and normalizes both `agent_id` and `agentId`.
- **Packaging Reliability**: Tauri bundle paths were corrected so release packaging resolves the checked-in binary and env schema resources deterministically.
...
graph TD
    ...
    subgraph "Intelligence Layer (Backend)"
        ...
        Memory["LanceDB RAG (memory.rs)"]
        Audit["Merkle Ledger (audit.rs)"]
    end

    subgraph "Persistence & Nodes"
        SQLite[("tadpole.db (sqlx)")]
        Bunker["Remote Bunker (mDNS)"]
    end

    Dashboard -- "WebSocket/REST" --> Axum
    Axum --> State
    State --> Runner
    Runner --> Memory
    Runner --> Audit
...
- [🏗️ Core Components](#core-components)
  ...
  - [8. filesystem_adapter](#8-filesystemadapter)
...
- **Hierarchical Tracing**: Standard OTel `trace:span` events still provide the deep "Chain of Thought" audit trail, while the Force-Graph provides the aggregate swarm topology.
...
| `tab_store` | **Multi-Tab Orchestration** | `tabs`, `activeTabId`, `addTab()`, `removeTab()` |
...
- **Unified API Layer (`use_api_request.ts`)**: A centralized hook that injects `NEURAL_TOKEN` authorization headers and handles standardized error parsing. This ensures consistency across 20+ different management panels.

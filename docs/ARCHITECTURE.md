...
> - **Resilience**: OBLITERATUS-inspired **Adaptive Quantization Fallback** for local OOM recovery.

> - **Discovery**: mDNS-SD (discovery.rs) for local Bunker nodes.
> - **Hardware Telemetry**: Proactive **Compute Profiler** (`profiler.rs`) for dashboard visualization.
...
### 🛰️ Telemetry Hub v2 (OBLITERATUS Implementation)
The engine now features a high-performance **Hardware Profiler** (`system/profiler.rs`) that aggregates low-level system metrics at the kernel layer:
- **Compute Profiles**: Gathers CPU load (per-core and aggregate), memory pressure (Used/Total), and active process count.
- **REST Streaming**: Metrics are exposed via `GET /v1/system/compute-profile`, enabling the dashboard's `Hardware_Load` widget to provide real-time resource visibility.
- **Performance Budgeting**: Enables the future `ResourceGuard` to pause swarm execution if system memory approaches critical thresholds (OOM Prevention).
...
### 🧠 Autonomic Self-Healing (OBLITERATUS Item 6)
To ensure mission continuity in resource-constrained environments (local Ollama/LLM nodes), the runner implements **Adaptive Quantization Fallback**:
1. **OOM Interception**: The `OpenAIProvider` specifically monitors for "out of memory" or "insufficient vram" keywords in the provider error text.
2. **Recursive Fallback**: Upon OOM detection, the engine automatically appends a quantization suffix (e.g., `:q4_K_M`) to the `model_id` and transparently retries the mission.
3. **Mission Continuity**: This allows the swarm to "downgrade" its precision in real-time to survive hardware pressure without human intervention.

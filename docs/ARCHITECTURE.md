...
> **Engineering Standard**: High-Performance Rust
...
- **Application State Hubs (`state.rs`)**: The engine's state is decomposed into five specialized hubs to ensure modularity and thread-safe access:
    ...
    - **`registry`**: Manages the identities and configurations of agents, providers, models, and discovered Swarm nodes.
    - **`security`**: Orchestrates Merkle auditing, shell scanning, resource monitoring, and the Privacy Shield "Hard Gate".
...
### 5. Financial Control (Budget Guard) (`server-rs/src/security/metering.rs`)

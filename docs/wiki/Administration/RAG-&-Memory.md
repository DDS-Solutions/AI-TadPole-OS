...
## 2. Vector Memory & Feature Flag Strategy (LanceDB)

> [!IMPORTANT]
> Vector memory requires the `vector-memory` [RISK: MEDIUM] build feature flag to be enabled at compile time. Without it, the engine operates on **Graph Intelligence + BM25 Lexical Retrieval**.

### 2.1 Why `vector-memory` is an Optional Feature Flag (Why vs. Why Not)

| Configuration | Technology Stack | Best For | Why Choose This? |
|:---|:---|:---|:---|
| **Default Build** (`default = []`) | **Graph Intelligence + BM25 + SQLite** | Standard CLI, Edge Workstations, Air-gapped Bunkers | **Zero compilation friction**: Compiles pure Rust without C++/Protoc/libclang prerequisites. Instant execution, zero embedding token costs, 100% deterministic AST code search and SOP compliance. |
| **Enhanced Cognitive Build** (`--features vector-memory`) | **LanceDB (Arrow) + Graph + BM25** | Deep Multi-Agent Swarms, Semantic Catalogs | **Fuzzy episodic recall**: Enables natural language similarity across unbounded mission monologues, past debugging logs, and semantic mapping across the 223-agent catalog even with zero keyword overlap. |

### 2.2 Graph Intelligence vs. Vector Memory (GraphRAG Synergy)
- **Graphs** solve *structural determinism* (AST call hierarchies, blast radius, RBAC boundaries).
- **Vectors (LanceDB)** solve *latent semantic proximity* (unstructured conversation summaries, conceptual analogy).
- Together, they form a **Graph-Augmented Vector Search (GraphRAG)** pipeline.
...
| **Knowledge Store** | `ResourceHub::knowledge_store` | Persistent cross-cluster Institutional Knowledge Store (IKS), supporting OKF-compliant frontmatter schemas (stored in SQLite columns `concept_type`, `title`, `description`, `resource_uri`, `tags`). |
...
### 5.1 Force-Graph View Modes (HUD Toggle)
The React dashboard supports swapping between visualizers directly from the HUD:
- **Codebase Symbols Graph**: Visualizes functions, traits, structs, and dependency relationships.
- **Semantic OKF Graph**: Visualizes IKS nodes mapped to Open Knowledge Format specifications. Employs monochromatic Zinc-scaled design tokens (`DESIGN_SYNERGY.md`) for concept types and reserves vibrant accents (`cyber-green`, `cyber-amber`, `cyber-red`) for live node verification and broken link anomalies.
...
## 7. Swarm Context Compaction

Tadpole OS implements dialogue context compaction in the `ContextManager` to keep LLM context within hardware boundaries:
- **$O(N)$ Linear Compactor**: Iteratively filters dialogue history in linear time, stripping large code logs from older messages while leaving the last 4 reasoning turns fully intact.
- **Resilient Fallback**: Employs a regex-based parser if LLM summarization fails, guaranteeing zero loss of structural turn coherence.

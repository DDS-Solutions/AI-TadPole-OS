...
| `TOKIO_THREAD_STACK_SIZE` | [RISK: LOW] | number | `16777216` | Tokio worker thread stack size in bytes (defaults to 16 MB in debug mode, 4 MB in release mode to prevent `STATUS_STACK_OVERFLOW` on Windows). |

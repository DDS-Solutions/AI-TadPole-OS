...
> Tadpole OS is built on a local-first guarantee. All system indexes, agent reasoning states, credential vaults, and communication histories are processed and stored entirely on your local machine. No data is transmitted externally unless you explicitly configure and enable a cloud LLM provider.

...
To prevent system overload on local nodes, the maximum number of concurrent projects is capped by the `MAX_CLUSTERS` [RISK: MEDIUM] engine setting (default: `10` in code [`state/mod.rs:L552-556`](../../../server-rs/src/state/mod.rs#L552-L556) and [`.env.example`](../../.env.example)).

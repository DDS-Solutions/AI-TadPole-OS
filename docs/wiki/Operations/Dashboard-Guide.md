...
- **Cognition Sidebar Inspector (`CognitionSidebar`)**: Clicking any graph node opens the Cognition Inspector panel detailing:
  - **Topic Space** (`<salted_hash>/<path>`): Salted workspace hash + relative file path enforcing SEC-05 path obfuscation security policy.
  - **Concept Type** (`general`, `sop`, `playbook`, `metric`, `symbol`): Open Knowledge Format (OKF v0.2) taxonomy classification.
  - **Confidence Score** (`0%`–`100%`): Multi-Factor Scoring (MFS) confidence decay metric.
  - **Governance State** (`Confirmed` vs `Pending Review`): Sapphire Shield Zero-Trust operator sign-off status.
- **Anomalies Panel (`AnomalyPanel`)**: HUD widget surfacing static analysis findings (unused symbols, dead code, dangling imports). Features a one-click **"Copy Prompt"** button to export refactoring instructions directly to AI swarms and interactive node jump targeting.

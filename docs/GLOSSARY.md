...
> - **Subsystem**: Architecture & Documentation / Core Docs / GLOSSARY
> - **Architecture**: `@docs ARCHITECTURE:Documentation`
> - **Failure Path**: Information drift, legacy terminology, or documentation mismatch.
> - **Observability**: Traceability via `execution/parity_guard.py` (`[GLOSSARY]`)
...
- drift_alerts (`Arc<DashMap<String, serde_json::Value>>`): Definition: Live registry of active architectural drift alerts keyed by subsystem name.
- base_dir (PathBuf): Definition: The root directory for persistent data and workspace sandboxes.
- session_id (`String`): Definition: Unique session ID generated at boot, used to correlate engine:boot / engine:shutdown events.
Return Value: N/A
Side Effects: N/A
Failure Conditions: N/A
...
- recent_requests (`DashMap<String, std::time::Instant>`): Definition: Sliding window cache of recently processed X-Request-Ids to prevent duplicate task execution.
...
- default_model (`RwLock<String>`): Definition: Default AI intelligence model used when none is explicitly configured.
- default_provider (`RwLock<String>`): Definition: Default model provider used when none is explicitly configured.
...
- privacy_mode (`AtomicBool`): Definition: Global Privacy Shield: When true, all external cloud provider traffic is blocked across all clusters.
- cluster_privacy_policies (`DashMap<String, bool>`): Definition: Per-Cluster Privacy Policies: Maps `cluster_id` to a boolean flag enforcing 100% local air-gapped execution for specific mission clusters when global privacy mode is hybrid.
...
- base_dir (`PathBuf`): Definition: Base directory for relative path resolution.
...
- payment_router (`Arc<crate::agent::runner::a2a_router::PaymentRouter>`): Definition: Payment router for economic zone and spend limits management.
Return Value: N/A
Side Effects: N/A
Failure Conditions: N/A
...
### Mode-Switched Utility Cluster
[Tier 2: Admin]
Type: Terminology
Purpose: A consolidated multi-role agent team architecture with strict operational boundaries and sub-routine mode switching, designed to minimize database index bloat and state synchronization latency across large swarms.
Implementation Hook: `Team_Cluster_Preset` / `Cluster_Manager_Modal.tsx` / `use_workspace_store`

### Mission Cluster
[Tier 1: Operator]
> [!NOTE]
> **Renamed Term**: This term has been renamed to **Project Workspace**. See [Project Workspace](#project-workspace) for the canonical definition.

### Neural Vault
[Tier 3: Security]
> [!NOTE]
> **Renamed Term**: This term has been renamed to **Secure Credentials Vault**. See [Secure Credentials Vault](#secure-credentials-vault) for the canonical definition.
...
[Tier 1: Operator]
> [!NOTE]
> **Renamed Term**: This term has been renamed to **Private Swarm Environment**. See [Private Swarm Environment](#private-swarm-environment) for the canonical definition.
...
[Tier 1: Operator]
Type: Terminology
Purpose: A reusable project workspace configuration specifying a preset cluster name, badge tag, default USD budget, department, and assigned multi-role agent collaborators.
Implementation Hook: `Team_Cluster_Preset` / `add_team_preset` / `use_workspace_store`
...
### Active Documentation Guard (ADG v2)
[Tier 1: Developer Tooling & Sovereign Governance]
Type: System / CLI Architecture
Purpose: Machine-enforced integrity engine that replaces manual, unverified comment claims (`Witness Tests:`, `Invariants:`) with deterministic machine verification. Partitions repository knowledge into a 3-Layer Claim Taxonomy (P-Class, M-Class, E-Class), computes co-located test witness maps into `adg.manifest.json`, validates CSP directives, and executes automated invariant test suites.
Implementation Hook: `tools/adg/adg.mjs`, `adg.manifest.json`, `tests/invariants/`
CLI: `node tools/adg/adg.mjs verify src/`, `npm run adg:verify`

### P-Class Claim (Prose & Guidance)
[Tier 1: Developer Tooling & Sovereign Governance]
Type: Metadata Classification (ADG v2)
Purpose: High-level architectural intent, subsystem boundaries, entrypoints, and documentation links (`@docs`) stored in top-of-file JSDoc comments. P-class claims provide human and AI context but must never assert empirical test or security facts.
Implementation Hook: JSDoc comments in `src/**/*.{ts,tsx}`

### M-Class Claim (Machine-Derived Fact)
[Tier 1: Developer Tooling & Sovereign Governance]
Type: Metadata Classification (ADG v2)
Purpose: Empirical facts about the codebase derived deterministically by static analysis tools (e.g., co-located test witness maps, parsed CSP directives). Stored in `adg.manifest.json` and governed by ratchet rules to prevent regression.
Implementation Hook: `adg.manifest.json`, `tools/adg/adg.mjs`

### E-Class Claim (Executable Invariant)
[Tier 1: Developer Tooling & Sovereign Governance]
Type: Metadata Classification (ADG v2)
Purpose: Critical safety, security, cryptography, and governance rules asserted by executable unit tests rather than English comments. Fails CI immediately if implementation code deviates from policy.
Implementation Hook: `tests/invariants/*.test.ts`
...
Implementation Hook: `server-rs/src/intelligence/graph/models.rs`
...
### Unidirectional Telemetry Flow Protocol
[Tier 3: Security & Telemetry]
Type: Architectural Protocol
Purpose: Enforces single-direction telemetry emission (`Client -> event_bus -> telemetry_engine`). Back-pressure or reverse UI blocking from telemetry receivers is strictly forbidden to prevent main thread blocking or event loop degradation.
Implementation Hook: `src/services/event_bus.ts`, `server-rs/src/telemetry/`

### Canonical Three-State Status Model
[Tier 2: Admin & UI]
Type: UI & State Specification
Purpose: Standardizes operational status indicators across all dashboards and monitoring services:
- `ACTIVE` (Emerald/Cyan): Subsystem or agent is operating within normal telemetry thresholds.
- `DEGRADED` (Amber): Heartbeat or response latency exceeds amber threshold or fallback retry is in progress.
- `OFFLINE` (Zinc/Red): Node unresponsive, killed, or privacy shield hard-blocked.
Implementation Hook: `src/hooks/use_engine_status.ts`, `server-rs/src/routes/oversight/mod.rs`
...
Type: API Endpoint Contract
Purpose: All backend REST endpoints exposed by `server-rs` strictly adhere to the `/v1/` route prefix hierarchy (e.g., `/v1/agents`, `/v1/oversight`, `/v1/infra/providers`, `/v1/continuity/jobs`). Higher non-existent API prefixes (such as `/v3/`) are forbidden.
Implementation Hook: `server-rs/src/router.rs`

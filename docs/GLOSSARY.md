...
> **Status**: Stable  
> **Version**: 1.1.17  
> **Last Updated**: 2026-04-19 (Mythos Hardening Pass)  
...
Algorithm allowing agents to halt reasoning early upon reaching high confidence. Halting can be triggered via `<halt/>` markers or the `set_confidence` tool.
**Implementation Hook**: `AgentRunner::execute_intelligence_loop` in `server-rs/src/agent/runner/intelligence.rs`
...
### Recurrence Depth
**(Domain: Swarm Architecture/Intelligence)**  
The maximum number of internal reasoning turns (1-16) an agent performs before emitting a final response.
**Implementation Hook**: `ModelConfig.reasoning_depth` in `server-rs/src/agent/types.rs`
...
**(Domain: Intelligence/Performance)**  
Recursive summarization of internal reasoning chains that exceed context limits (8k chars), preserving technical continuity while reclaiming token space.
**Implementation Hook**: `AgentRunner::compress_monologue` in `server-rs/src/agent/runner/intelligence.rs`

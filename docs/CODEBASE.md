...
- **`server-rs/`**: The core **Tadpole OS** engine.
    ...
    - **`src/startup.rs`**: System initialization and background tasks.
    ...
    - **`src/agent/`**: The Intelligence Layer.
        ...
        - `persistence.rs`: Unified SQLite state, replacing legacy file registries.
    ...
    - **`src/security/`**: The Core Defense Layer.
        ...
        - `monitoring.rs`: Resource Guard (RAM/CPU) and Sandbox Awareness.

...
> - **Subsystem**: Architecture & Documentation / Core Docs / QWEN_LOCAL_INTEGRATION
...
4.  Run a mission and monitor the **Swarm_Visualizer** in the Operations Center. Local token generation is broadcast via the 10Hz binary pulse, allowing you to see Qwen's "Thinking" status in real-time.
...
*   **Swarm Pulse Interleaving**: Local inference events are interleaved with the global **Swarm Pulse** (0x02), ensuring sub-millisecond parity between the local Ollama state and the frontend visualizer.

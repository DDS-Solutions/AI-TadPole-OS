...
- **Benchmark Suites**: Files stored inside `server-rs/benches/` directory.
...
Add the following to `server-rs/Cargo.toml`:
```toml
[dev-dependencies]
...
To prevent performance degradation over time, the benchmark suite is integrated directly into the CI pipeline:
- **Execution Cadence**:
...
- [Agent Runner Engine](file:///d:/TadpoleOS-Dev/server-rs/src/agent/runner/mod.rs)
...
The following files represent the required benchmark files in the codebase repository:
- [ ] `server-rs/benches/runner_bench.rs` (Create)
- [ ] `server-rs/benches/rate_limiter_bench.rs` (Create)
- [ ] `server-rs/benches/persistence_bench.rs` (Create)
- [ ] `server-rs/benches/mcp_bench.rs` (Create)
- [ ] `server-rs/benches/ws_bench.rs` (Create)
- [ ] `server-rs/benches/filesystem_bench.rs` (Create)
- [ ] `server-rs/benches/http_bench.rs` (Create)

> [!NOTE]
> **Micro-Benchmarks**: The `BM-SEC` (Security) and `BM-QUAL` (Quality) categories do not live as separate files under `server-rs/benches/`. Instead, they are compiled directly as internal tests and telemetry hooks inside the `server-rs/src/agent/benchmarks.rs` module, allowing them to utilize internal structs and private schemas.

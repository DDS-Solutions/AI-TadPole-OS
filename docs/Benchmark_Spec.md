...
Version: 1.1.3 | Status: Verified | Last Updated: March 20, 2026
Scope: Development-phase performance and security benchmarking for the Tadpole Engine (Rust backend + React frontend)
...
Benchmark Categories (with setup + step-by-step execution)

BM-RUN — Agent Runner
BM-RL — Rate Limiter
BM-DB — Persistence Layer
BM-MCP — MCP Host
BM-WS — WebSocket & Event Bus
BM-FS — FilesystemAdapter & Security
BM-HTTP — HTTP Client & Providers
BM-FE — Frontend
BM-SEC — Security & Integrity
...
WebSocket throughput | RAF batching maintains 60fps under load | 60fps at ≥ 500 events/sec
...
Purpose: Validate capability cache performance, zero-downtime hot-swap behavior, and **McpToolStats** telemetry accuracy.
Setup Parameters
...
Tests & Step-by-Step Execution
BM-MCP-01 — Cold start capability load
bash# Step 1: Clear capability cache
# Step 2: Time the server startup to first capability response
...
Purpose: Verify real-time event throughput and that the frontend maintains 60fps under swarm telemetry load.
Setup Parameters
...
# If latency grows linearly: buffer is not truly O(1) — investigate eventBus.ts
...
# Expected: vendor-react.js, vendor-framer.js, vendor-reactflow.js, etc.
...
# - Each vendor (react, framer-motion, reactflow, lucide, zustand) in own chunk

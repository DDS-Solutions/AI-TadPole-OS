...
- **Outward A2A Gateway & Customer Knowledge Catalog**: Zero-trust public gateway (`/a2a/v1`) with rate-limited agent card publishing, RFC 4180 CSV / QuickBooks imports, and real-time PII sanitization.
- **Decomposed Template Store & Transactional FS**: Specialized submodules (`catalog`, `source`, `naming`, `validate`, `mcp_store`, `installed`) with atomic `OpenOptions::create_new(true)` filesystem staging and LIFO rollback.

...
> - **Subsystem**: Architecture & Documentation / Core Docs / TEST_MISSIONS
> - **Architecture**: `@docs ARCHITECTURE:Documentation`
> - **Failure Path**: Information drift, legacy terminology, or documentation mismatch.
> - **Observability**: Traceability via `execution/parity_guard.py` (`[TEST_MISSIONS]`)

# 🐸 AI-Tadpole-OS: Test Missions Playbook
...
This document provides a set of curated "Small Missions" to help you verify and explore the core skills of **AI-Tadpole-OS**. Each mission is designed to exercise a specific layer of the engine, from basic connectivity to complex autonomous swarms.
...
- **Mission 6 (Continuity)**: **Continuity Scheduler**. Done via the Continuity Scheduler UI to check background execution.
- **Mission 7 (Governance)**: Recommended as **Missions** to observe agent-level oversight queues.
- **Mission 8 (Telemetry)**: Requires opening the **Operations Center** and visualizer.
...
| **10. The Grounding Gate** | Anti-Hallucination | Sentinel Reality Check & File Grounding |
...
*   **Prompt**: `"Identify yourself and summarize the current state of AI-Tadpole-OS based on your IDENTITY.md."`
*   **Verification**:
...
**Goal**: Test agent-level security (Junior Mode/Requires Oversight) where all tool calls are forced into the Approval Queue.

*   **Setup**: Select an agent (e.g., a custom specialist or departmental agent) and enable the **Requires Oversight** flag in their configuration (or choose/create a role blueprint with `requires_oversight` enabled).
*   **Prompt**: `"Read the contents of WORKSPACE_TEST.md."` (or any simple/safe tool execution).
*   **Verification**:
    - [ ] Even though the tool (e.g., `read_file`) is low-risk and normally auto-approved, the engine pauses execution because the agent has `requires_oversight` enabled.
    - [ ] An "Oversight Required" alert appears in the Security Dashboard.
    - [ ] You (The Overlord) must Approve or Reject the action.
    - [ ] Once approved, the tool executes and the agent completes the turn.
...
## 10. The Grounding Gate (Anti-Hallucination & Reality Anchor)
**Goal**: Verify that the engine intercepts ungrounded milestone assertions, non-existent report citations, and fictitious API versions, forcing orchestrator agents to verify on-disk reality using read-only inspection tools.

*   **Agent**: `COO` (ID: `"2"`) or `CEO` (ID: `"1"`).
*   **Prompt**:
    ```text
    "As the COO, review the current state of the system and report on the Glossary Compliance Audit findings. Confirm if the Discovery Phase is complete and cite the drift report."
    ```
*   **Verification**:
    - [ ] **Sentinel Gate Intercept**: If the agent claims `Glossary_Drift_Report.md` is complete without the file existing on disk, the Sentinel intercepts the turn with `SYSTEM_SENTINEL: REALITY COLLAPSE DETECTED`.

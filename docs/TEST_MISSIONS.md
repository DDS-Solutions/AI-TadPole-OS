...
## 7. The Governance Gate (Agent-Level Oversight)
**Goal**: Verify the "Requires Oversight" (Junior Mode) toggle at the agent level.

*   **Setup**: Open an agent's **Neural Node Configuration**, go to the **Governance** tab, and enable **Requires Oversight**.
*   **Prompt**: `"List the files in your current workspace."` (Use a safe tool like `list_dir`).
*   **Verification**:
    - [ ] Even though `list_dir` is typically a safe tool, the engine pauses execution.
    - [ ] An "Oversight Required: Junior Agent Node" alert appears in the Security Dashboard.

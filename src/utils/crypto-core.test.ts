 ...
 * - **Subsystem**: Frontend Utilities / crypto-core.test
 *
...
describe('crypto-core', () => {
    ...
    describe('encrypt_raw', () => {
        ...
        it('throws an error if crypto.getRandomValues is undefined (fail-closed)', async () => {
            vi.stubGlobal('crypto', { subtle: subtle_mock }); // No getRandomValues
    ...
    describe('decrypt_raw', () => {
        ...
        it('handles malformed or invalid json payloads without throwing unhandled exceptions', async () => {
             await expect(decrypt_raw('not valid json', 'pass'))
                 .rejects.toThrow('Decryption failed');
             await expect(decrypt_raw('{"missing": "fields"}', 'pass'))
                 .rejects.toThrow('Decryption failed');
             await expect(decrypt_raw('{"salt":"zz","iv":"zz","data":"zz"}', 'pass'))
                 .rejects.toThrow('Decryption failed');
        });
    });
});

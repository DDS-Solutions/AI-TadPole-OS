 ...
 * - **Subsystem**: Frontend Utilities / model_utils
 * - **Primary Entrypoints**: `resolve_technical_model_id`, `resolve_provider`, `parse_active_model_slot`, `resolve_agent_model_config`
 *
...
export function resolve_provider(model_id: string): string {
    ...
    if (lower.includes(':')) {
        ...
        const known_providers = ['openai', 'anthropic', 'google', 'gemini', 'groq', 'mistral', 'deepseek', 'xai', 'ollama-cloud', 'ollama', 'openrouter'];
        if (known_providers.includes(prefix)) {
            return prefix === 'gemini' ? 'google' : prefix;
        }
    }

    // 2. Cloud-hosted Ollama & Gemma4 Cloud Routing
    if (lower.includes('ollama-cloud') || lower.includes('gemma4') || lower.includes('-cloud') || lower.includes(':cloud')) return 'ollama-cloud';

    // 3. OpenRouter Routing
    if (lower.includes('openrouter') || lower.includes('stealth')) return 'openrouter';

    // 4. Core Provider Keywords
...
export function get_model_color(model_name: string): string {
    ...
    // Ollama Cloud / Gemma4 - Amber
    if (lower.includes('ollama-cloud') || lower.includes('gemma4') || lower.includes('-cloud')) {
        return 'text-amber-400 border-amber-900 bg-amber-900/10';
    }

    // OpenRouter - Violet
    if (lower.includes('openrouter') || lower.includes('stealth')) {
        return 'text-violet-400 border-violet-900 bg-violet-900/10';
    }

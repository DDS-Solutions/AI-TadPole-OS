...
export function resolve_agent_model_config(agent: Agent, global_default_model?: string): { model_id: string, provider: string } {
    ...
    const is_agent_default = (!has_id && is_generic) || 
                             (has_id && !has_key && config?.provider === 'google');
    
    // 1. Determine base model_id: Use global override if agent is using generic system defaults
    let raw_id = has_id ? config!.modelId : (agent.model || 'gemini-1.5-flash');
    ...
    if (agent.active_model_slot === 2 && agent.model_config2) {
        ...
        model_id = resolve_technical_model_id(c2.modelId || agent.model_2 || model_id);
        provider = c2.provider || resolve_provider(model_id);
    } else if (agent.active_model_slot === 3 && agent.model_config3) {
        const c3 = agent.model_config3;
        model_id = resolve_technical_model_id(c3.modelId || agent.model_3 || model_id);
        provider = c3.provider || resolve_provider(model_id);
    }

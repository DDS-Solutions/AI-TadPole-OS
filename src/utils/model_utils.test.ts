    ...
    describe('resolve_agent_model_config', () => {
        ...
        it('handles slot 2 correctly', () => {
            const agent = {
                ...
                model_config: { modelId: 'gpt-4o', provider: 'openai' },
                active_model_slot: 2,
                model_config2: { modelId: 'gemini-1.5-pro', provider: 'google' }
            };
            const result = resolve_agent_model_config(agent as any);
        ...
        it('handles slot 3 correctly', () => {
            const agent = {
                ...
                active_model_slot: 3,
                model_3: 'grok-1.5',
                model_config3: { modelId: 'grok-1.5', provider: 'xai' }
            };
            const result = resolve_agent_model_config(agent as any);
        ...
        it('overrides generic Gemini Flash models if a global default is provided', () => {
            const agent = {
        ...
        it('respects specialized Gemini Pro models even if global default is provided', () => {
            const agent = {
        ...
        it('ignores legacy provider "google" if global default is active for a Gemini Flash agent', () => {
            const agent = {
                model: 'gemini-1.5-flash',
                model_config: { provider: 'google', modelId: 'gemini-1.5-flash' },
                active_model_slot: 1
            };
            const global_default = 'Global Default (Ollama)';
            const result = resolve_agent_model_config(agent as any, global_default);
        ...
        it('aggressively overrides any agent with NO API KEY if it is generic system default', () => {
            const agent = {
                name: 'SystemAgent',
                model: 'gemini-1.5-flash',
                model_config: { 

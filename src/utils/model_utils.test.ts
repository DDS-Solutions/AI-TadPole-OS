 ...
 * 
 * ### AI Assist Note
 * **Infrastructure**: Manages the model utility tests. 
 * Part of the Tadpole-OS core infrastructure.
 * 
 * ### 🔍 Debugging & Observability
 * - **Failure Path**: Unit test regression (failure in Vitest runner) or logic collision in provider resolution.
...
describe('model_utils', () => {
    ...
    describe('resolve_provider', () => {
        ...
        it('identifies "Global Default (Ollama)" as ollama', () => {
            expect(resolve_provider('Global Default (Ollama)')).toBe('ollama');
    ...
    describe('resolve_agent_model_config', () => {
        ...
        it('overrides generic Gemini models if a global default is provided', () => {
            const agent = {
                model: 'Gemini 1.5 Pro', // Standard specialist default
                active_model_slot: 1
            };
            const global_default = 'Global Default (Ollama)';
            const result = resolve_agent_model_config(agent as any, global_default);
            
            expect(result.model_id).toBe('Global Default (Ollama)');
            expect(result.provider).toBe('ollama');
        });

        it('respects specialized models even if global default is provided', () => {
            const agent = {
                model_config: { 
                    model_id: 'gpt-4o', 
                    provider: 'openai',
                    api_key: 'sk-test' // Explicit key prevents hijacking
                },
                active_model_slot: 1
            };
            const global_default = 'ollama';
            const result = resolve_agent_model_config(agent as any, global_default);
        ...
        it('ignores legacy provider "google" if global default is active for a Gemini default agent', () => {
            const agent = {
                model: 'Gemini 1.5 Pro',
                model_config: { provider: 'google', model_id: 'gemini-1.5-pro' },
                active_model_slot: 1
            };
            const global_default = 'Global Default (Ollama)';
            const result = resolve_agent_model_config(agent as any, global_default);
            ...
            expect(result.model_id).toBe('Global Default (Ollama)');
            expect(result.provider).toBe('ollama'); // Should be ollama, NOT google
        });

        it('aggressively overrides any agent with NO API KEY if it is Gemini-based', () => {
            const agent = {
                name: 'CustomAgent',
                model: 'Custom-Legacy-Brain',
                model_config: { 
                    model_id: 'gemini-1.5-pro', // Gemini ID triggers hijack if no key
                    provider: 'google',
                    // NO api_key
                },
                active_model_slot: 1

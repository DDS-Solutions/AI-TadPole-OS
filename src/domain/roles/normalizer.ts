...
export const normalize_role_blueprint = (dto: Role_Blueprint_Dto): Role => {
    return {
        ...
        mcp_tools: parse_json_array(dto.mcpTools || dto.mcp_tools || '[]'),
...
const parse_json_array = (json_str: string | string[]): string[] => {
    if (Array.isArray(json_str)) return json_str;
    if (typeof json_str !== 'string' || !json_str.trim()) return [];
    try {

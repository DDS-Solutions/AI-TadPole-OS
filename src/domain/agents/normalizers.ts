 ...
 * - **Subsystem**: System Core / normalizers
 * - **Primary Entrypoints**: `normalize_agent_dto`, `normalize_agent_memory_entry`
 *
 ...
 */
interface RobustAgentDto extends Omit<Partial<AgentDto>, 'inventory' | 'planningSlot' | 'executionSlot'> {
    department?: string;
    tokens_used?: number;
...
export const normalize_agent_dto = (dto: AgentDto, workspace_path?: string, existing_agent?: Agent): Agent => {
    ...
    const resolve_slot_value = <T>(
        incoming_slot: unknown,
        incoming_field: unknown,
        existing_val: T | undefined,
        extractor: (slot: Record<string, unknown>) => T | undefined
    ): T | undefined => {
        if (incoming_slot === null) return undefined;
        if (incoming_slot !== undefined) {
            return typeof incoming_slot === 'object' && incoming_slot !== null ? extractor(incoming_slot as Record<string, unknown>) : undefined;
        }
        if (incoming_field === null) return undefined;
        if (incoming_field !== undefined) {
            return typeof incoming_field === 'object' && incoming_field !== null ? extractor(incoming_field as Record<string, unknown>) : (incoming_field as T);
        }
        return existing_val;
    };
    ...
    const model_2 = resolve_slot_value(
        planningSlotVal,
        d.model2 ?? d.model_2 ?? (rawD.modelConfig2 as Record<string, unknown> | undefined)?.modelId as string | undefined ?? (rawD.model_config2 as Record<string, unknown> | undefined)?.model_id as string | undefined,
        existing_agent?.model_2,
        (slot) => (slot as Record<string, unknown>).modelId as string | undefined ?? (slot as Record<string, unknown>).model_id as string | undefined
    );

    const model_3 = resolve_slot_value(
        executionSlotVal,
        d.model3 ?? d.model_3 ?? (rawD.modelConfig3 as Record<string, unknown> | undefined)?.modelId as string | undefined ?? (rawD.model_config3 as Record<string, unknown> | undefined)?.model_id as string | undefined,
    ...
    const model_config2 = resolve_slot_value(
        ...
        (slot) => slot as unknown as ModelConfigDto
    );
    ...
    return {
        ...
        active_model_slot: parse_active_model_slot(get_val('activeModelSlot', 'active_model_slot', 1)),

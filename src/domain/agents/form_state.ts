 ...
 * - **Subsystem**: System Core / form_state
 * - **Primary Entrypoints**: `buildAgentFormState`, `serializeFormState`
 *
...
import { resolve_provider, parse_active_model_slot } from '../../utils/model_utils';

/**
 * buildAgentFormState
 * Converts a flat Domain Agent object into structured Form state.
 */
export function buildAgentFormState(agent: Agent): AgentFormState {
    ...
    return {
        main_tab: 'cognition',
        active_tab: active_slot === 2 ? 'secondary' : active_slot === 3 ? 'tertiary' : 'primary',
        identity: {
            name: agent.name,
            role: agent.role,
        ...
        governance: {
            ...
            shadows_human_id: agent.shadows_human_id || '',
            economic_zone: agent.economic_zone || 'DEV',
            daily_spend_limit: agent.daily_spend_limit || 0
        },
        ui: {
            direct_message: '',
            saving: false,
...
export const serializeFormState = (state: AgentFormState): AgentPatch => {
    ...
    return {
        ...
        economic_zone: governance.economic_zone,
        daily_spend_limit: governance.daily_spend_limit,

 ...
 * - **Subsystem**: System Core / App
 * - **Primary Entrypoints**: `App`
 *
...
const Detached_Chat = lazy(() => import('./components/SovereignChat').then(module => ({ default: module.SovereignChat })));
...
function TabSync(): null {
  ...
  const last_active_tab_id = useRef(active_tab_id);
  ...
  useEffect(() => {
    ...
    // 2. Guard: If active_tab_id itself changed, it means the tab store state was updated first
    // (e.g. via tab click or BroadcastChannel sync from another tab/window), and the URL is currently
    // lagging behind and will be updated by useLayoutNavigation. In this case, we must not call open_tab
    // because that would overwrite/revert the active tab back to matching the old location.pathname.
    if (last_active_tab_id.current !== active_tab_id) {
      last_active_tab_id.current = active_tab_id;
      return;
    }
...
export default function App(): React.ReactElement {
  ...
  useEffect(() => {
    const cleanup = use_sovereign_store.getState().initialize_sync_listeners();
    return cleanup;
  }, []);
      ...
      <Error_Boundary name="Global OS Hub">
        <Suspense fallback={<RouteLoading />}>
          <Routes>
            ...
            <Route path="/detached/chat" element={<Detached_Chat isDetachedView={true} />} />

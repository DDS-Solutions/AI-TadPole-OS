...
export function Forge_Item({ model, is_editing, on_edit, on_cancel, on_save, on_delete }: Forge_Item_Props): React.ReactElement {
    ...
    const [prev_is_editing, set_prev_is_editing] = useState(is_editing);
    if (is_editing !== prev_is_editing) {
        set_prev_is_editing(is_editing);
        if (is_editing) {
            set_edit_name(model.name);
            set_edit_modality(model.modality || 'llm');
            const custom = !['llm', 'vision', 'voice', 'reasoning'].includes(model.modality || 'llm');
            set_is_custom_modality(custom);
            set_custom_modality(model.modality || '');
            set_limits({
                rpm: model.rpm || 10,
                tpm: model.tpm || 100000,
                rpd: model.rpd || 1000,
            ...
            <div className="p-4 bg-emerald-500/[0.03] space-y-4 animate-in fade-in duration-300">
                ...
                {is_custom_modality && (
                    <input
                        ...
                        onChange={e => set_custom_modality(e.target.value)}
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                        ...
                        <input
                            ...
                            value={limits.rpm}
                            onChange={e => set_limits({ ...limits, rpm: parseInt(e.target.value) || 0 })}
                    ...
                    <div className="space-y-1">
                        ...
                        <input
                            ...
                            className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-1.5 text-xs text-zinc-300 font-mono focus:border-emerald-500/40 outline-none"
                            value={limits.tpm}
                            onChange={e => set_limits({ ...limits, tpm: parseInt(e.target.value) || 0 })}
                    ...
                    <div className="space-y-1">
                        ...
                        <input
                            ...
                            className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-1.5 text-xs text-zinc-300 font-mono focus:border-emerald-500/40 outline-none"
                            value={limits.rpd}
                            onChange={e => set_limits({ ...limits, rpd: parseInt(e.target.value) || 0 })}
                    ...
                    <div className="space-y-1">
                        ...
                        <input
                            ...
                            className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-1.5 text-xs text-zinc-300 font-mono focus:border-emerald-500/40 outline-none"
                            value={limits.tpd}
                            onChange={e => set_limits({ ...limits, tpd: parseInt(e.target.value) || 0 })}
                    ...
                    <Tooltip content={i18n.t('provider.forge_item.tooltip_save')} position="bottom">
                        <button
                            onClick={() => {
                                ...
                                on_save(model.id, edit_name, model.provider, final_modality, limits);
                            }}

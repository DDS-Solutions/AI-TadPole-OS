 ...
 * - **Subsystem**: UI Components / Provider / Forge_Item
 * - **Primary Entrypoints**: `Forge_Item`
 *
...
export function Forge_Item({ model, is_editing, on_edit, on_cancel, on_save, on_delete }: Forge_Item_Props): React.ReactElement {
        ...
        return (
            <div className="p-4 bg-emerald-500/[0.03] space-y-4 animate-in fade-in duration-300">
                <div className="flex gap-3">
                    <input
                        className="bg-[color:var(--color-background)] border border-emerald-500/30 rounded-lg px-3 py-1.5 text-xs text-zinc-100 flex-1 focus:outline-none focus:border-emerald-500 font-mono"
                        value={edit_name}
                        onChange={e => set_edit_name(e.target.value)}
                        aria-label={i18n.t('provider.forge_name_label')}
                    />
                    <select
                        className="bg-[color:var(--color-background)] border border-emerald-500/30 rounded-lg px-3 py-1.5 text-[11px] text-zinc-400 focus:outline-none uppercase font-bold"
                ...
                {is_custom_modality && (
                    <input
                        className="bg-[color:var(--color-background)] border border-emerald-500/30 rounded-lg px-3 py-1.5 text-xs text-zinc-100 w-full focus:outline-none focus:border-emerald-500 font-mono"
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                        ...
                        <input
                            ...
                            min="0"
                            step="1"
                            className="w-full bg-[color:var(--color-background)] border border-[color:var(--color-border)] rounded-lg px-3 py-1.5 text-xs text-zinc-300 font-mono focus:border-emerald-500/40 outline-none"
                            value={limits.rpm}
    ...
    return (
        <div className="p-3 px-4 flex items-center justify-between group hover:bg-[color:var(--color-surface)]/50 transition-all">
                ...
                <div className={`p-1.5 rounded-lg border flex items-center justify-center transition-colors ${model.modality === 'vision' ? 'bg-amber-500/10 border-amber-500/20 text-amber-500' :
                    model.modality === 'voice' ? 'bg-green-500/10 border-green-500/20 text-green-500' :
                        model.modality === 'reasoning' ? 'bg-green-500/10 border-green-500/20 text-green-500' :
                            'bg-[color:var(--color-surface)] border-[color:var(--color-border)] text-zinc-500 group-hover:text-emerald-500'
                    }`}>
            ...
            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
                <Tooltip content={i18n.t('provider.forge_item.tooltip_edit')} position="left">
                    <button
                        onClick={on_edit}
                        aria-label={i18n.t('provider.forge_item.tooltip_edit')}
                ...
                <Tooltip content={i18n.t('provider.forge_item.tooltip_delete')} position="left">
                    <button
                        onClick={on_delete}
                        aria-label={i18n.t('provider.forge_item.tooltip_delete')}

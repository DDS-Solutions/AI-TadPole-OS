 ...
 * - **Subsystem**: UI Components / Provider / Model_Forge
 * - **Primary Entrypoints**: `Model_Forge`
 *
...
export function Model_Forge({
    ...
    return (
        <section className="space-y-4">
            ...
            <div className="bg-[color:color-mix(in_srgb,var(--color-background)_50%,transparent)] border border-[color:var(--color-border)] rounded-2xl overflow-hidden divide-y divide-zinc-800/50">
                {is_forge_adding && (
                    <div className="p-4 bg-green-500/5 space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                        <div className="flex gap-3">
                            <input
                                id="forge-model-name"
                                className="bg-[color:var(--color-background)] border border-green-500/30 rounded-lg px-3 py-1.5 text-xs text-zinc-100 flex-1 focus:outline-none focus:border-green-500 font-mono"
                            ...
                            <select
                                className="bg-[color:var(--color-background)] border border-green-500/30 rounded-lg px-3 py-1.5 text-xs text-zinc-100 focus:outline-none cursor-pointer font-mono"
                        ...
                        {is_forge_custom_modality && (
                            <input
                                ...
                                className="bg-[color:var(--color-background)] border border-green-500/30 rounded-lg px-3 py-1.5 text-xs text-zinc-100 w-full focus:outline-none focus:border-green-500 font-mono"
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1">
                                ...
                                <input
                                    ...
                                    min="0"
                                    step="1"
                                    className="w-full bg-[color:var(--color-background)] border border-[color:var(--color-border)] rounded-lg px-3 py-1.5 text-xs text-zinc-300 font-mono focus:border-green-500/40 outline-none"
                        ...
                        <div className="flex justify-end gap-2 pt-2 border-t border-[color:var(--color-border)]/50">
                            <Tooltip content={i18n.t('provider.commit_node_tooltip')} position="bottom">
                                <button onClick={handle_forge_add} aria-label={i18n.t('provider.commit_node_tooltip', { defaultValue: 'Commit node' })} className="p-1.5 bg-green-500/20 text-green-400 rounded hover:bg-green-500/30 transition-colors">
                                    <Check size={16} />
                                </button>
                            </Tooltip>
                            <Tooltip content={i18n.t('provider.cancel_node_tooltip')} position="bottom">
                                <button onClick={() => set_is_forge_adding(false)} aria-label={i18n.t('provider.cancel_node_tooltip', { defaultValue: 'Cancel' })} className="p-1.5 bg-zinc-800 text-zinc-500 rounded hover:bg-zinc-700 transition-colors">
                                    <X size={16} />
                                </button>
                            </Tooltip>
                        </div>
                ...
                {provider_models.length === 0 ? (
                    <div className="p-8 text-center">
                        <div className="inline-flex p-3 rounded-full bg-[color:var(--color-surface)] border border-[color:var(--color-border)] text-zinc-600 mb-3">
                            <Cpu size={24} />
                        </div>

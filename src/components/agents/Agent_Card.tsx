 ...
 * - **Subsystem**: UI Components / Agents / Agent_Card
 * - **Primary Entrypoints**: `Agent_Card_Memo`
 *
...
const Agent_Card: React.FC<AgentCardProps> = ({ agent, on_select }) => {
    ...
    return (
        <div
            ...
            className="group sovereign-card cursor-pointer p-4 hover:sovereign-glow-green relative overflow-hidden"
            aria-label={i18n.t('agent_manager.aria_configure_agent', { name: agent.name })}
        >
            <div className="absolute top-0 right-0 p-3 flex items-center gap-1 z-20" onClick={(e) => e.stopPropagation()}>
                <Tooltip content={health_tooltip} position="left">
                    <button
                        onClick={(e) => { e.stopPropagation(); set_is_health_open(!is_health_open); }}
                        aria-label={health_tooltip}
                        aria-expanded={is_health_open}
                        className={`p-1.5 rounded-lg border transition-all ${
                            is_health_open 
                                ? 'bg-zinc-800 border-zinc-700 ' + health_color 
                                : 'bg-zinc-900/80 border-white/5 text-zinc-500 hover:' + health_color + ' hover:border-zinc-700'
                        }`}
                    >
                        <Shield size={14} fill={is_health_open ? 'currentColor' : 'none'} className={failure_count >= 3 ? 'animate-pulse' : ''} />
                    </button>
                </Tooltip>
                <Tooltip content={i18n.t('agent_manager.tooltip_configure')} position="left">
                    <button
                        onClick={(e) => { e.stopPropagation(); on_select(agent); }}
                        aria-label={i18n.t('agent_manager.tooltip_configure')}
                        className="p-1.5 rounded-lg border bg-zinc-900/80 border-white/5 text-zinc-500 hover:text-green-400 hover:border-zinc-700 transition-colors"
                    >
                        <Sliders size={14} />
                    </button>
                </Tooltip>

 ...
 * - **Subsystem**: UI Components / Agents / Agent_Manager_Header
 * - **Primary Entrypoints**: `Agent_Manager_Header`
 *
        ...
        <div className="py-4 px-6 border-b border-[color:var(--color-border)] bg-[color:color-mix(in_srgb,var(--color-background)_80%,transparent)] backdrop-blur-xl sticky top-0 z-40 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
                ...
                <div className="flex items-center p-1 bg-[color:var(--color-surface)] rounded-lg border border-[color:var(--color-border)] self-center">
                    ...
                    >
                        <Users size={12} />
                        {i18n.t('agent_manager.tab_user')}
                        <span className={`px-1 rounded ${active_tab === 'user' ? 'bg-zinc-950/20 text-white' : 'bg-zinc-800 text-zinc-600'}`}>
                            {user_count}
                        </span>
                    </button>
                    <button
                    ...
                    >
                        <Cpu size={12} />
                        {i18n.t('agent_manager.tab_ai')}
                        <span className={`px-1 rounded ${active_tab === 'ai' ? 'bg-zinc-950/20 text-white' : 'bg-zinc-800 text-zinc-600'}`}>
                            {ai_count}
                        </span>
                    </button>
                </div>
                    ...
                    <Tooltip content={i18n.t('agent_manager.tooltip_search')} position="bottom">
                        <div className="relative w-full">
                            ...
                            <input
                                ...
                                className="bg-[color:var(--color-surface)] border border-[color:var(--color-border)] text-zinc-200 text-xs rounded-lg pl-9 pr-3 py-2 w-full focus:outline-none focus:border-green-500/50 transition-colors placeholder:text-zinc-600"
                            />
                        </div>
                    </Tooltip>
                    ...
                    <Tooltip content={i18n.t('agent_manager.tooltip_filter')} position="bottom">
                        <div className="relative w-full">
                            ...
                            <select
                                ...
                                className="bg-[color:var(--color-surface)] border border-[color:var(--color-border)] text-zinc-200 text-xs rounded-lg pl-9 pr-8 py-2 w-full appearance-none focus:outline-none focus:border-green-500/50 cursor-pointer uppercase font-bold tracking-wider"
                            >
                                <option value="all">{i18n.t('agent_manager.filter_all')}</option>
                                {filter_roles.map(role => (
                                    <option key={role} value={role}>{role}</option>
                                ))}
                                <option value="health_status">{i18n.t('agent_manager.sort_health_status')}</option>
                            </select>
                        </div>
                    </Tooltip>
                </div>

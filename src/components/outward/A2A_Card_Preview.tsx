 ...
 * - **Subsystem**: UI Components / Outward / A2A_Card_Preview
 * - **Primary Entrypoints**: `A2A_Card_Preview`, `A2aSkill`, `A2aAgentCardProps`, `ModelProfile`
 *
...
export const A2A_Card_Preview: React.FC<A2aAgentCardProps> = ({
  ...
  url = 'http://localhost:8000/a2a/v1/company-agent-card.json',
    ...
    <div className="w-full max-w-2xl bg-zinc-900/60 backdrop-blur-xl border border-white/5 rounded-2xl p-6 text-zinc-100 font-mono shadow-2xl overflow-hidden flex flex-col justify-between relative">
      ...
      <div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-zinc-800/80 pb-4 mb-4">
          ...
          {/* Model Profile Badge */}
          <div className="flex items-center space-x-2 text-xs bg-zinc-950/40 px-3 py-1.5 rounded-xl border border-zinc-800 shrink-0 self-start sm:self-auto shadow-sm">
        ...
        <p className="text-xs sm:text-sm text-zinc-300 mb-6 leading-relaxed bg-zinc-950/40 p-3.5 rounded-xl border border-zinc-800/60">
          {description}
        </p>
        ...
        <div className="space-y-3">
          ...
          {skills.length === 0 ? (
            <div className="p-4 text-xs text-zinc-500 italic bg-zinc-950/30 rounded-xl border border-zinc-800/60 text-center">
            ...
            <div className="grid grid-cols-1 gap-3">
              {skills.map((skill) => (
                <div
                  key={skill.id}
                  className="p-4 bg-zinc-950/40 rounded-xl border border-zinc-800/60 hover:border-zinc-700/80 transition-all"
                >
      ...
      {/* Cloner & AI Discovery Guidance Banner */}
      <div className="mt-4 p-3 bg-emerald-950/20 rounded-xl border border-emerald-500/20 text-xs text-zinc-300 font-mono space-y-1.5">
        <div className="flex items-center justify-between font-semibold text-emerald-400">
          <span className="flex items-center gap-1.5">
            <Globe className="w-3.5 h-3.5 shrink-0" />
            GitHub Pages & AI Auto-Discovery Guide
          </span>
          <span className="text-[10px] text-zinc-400 font-normal">Scenario 2 Setup</span>
        </div>
        <p className="text-[11px] text-zinc-400 leading-relaxed">
          <strong className="text-zinc-200">Local Dev:</strong> Serves live on <code className="text-emerald-300 bg-zinc-950/40 px-1 py-0.5 rounded">http://localhost:8000/a2a/v1/company-agent-card.json</code>.<br />
          <strong className="text-zinc-200">Cloners & Forkers:</strong> Fork this repository, enable GitHub Pages in <code className="text-zinc-300">Settings -&gt; Pages</code>, and your agent card automatically publishes online at <code className="text-emerald-300 bg-zinc-950/40 px-1 py-0.5 rounded">https://&lt;username&gt;.github.io/&lt;repo&gt;/a2a/v1/company-agent-card.json</code>.<br />
          <strong className="text-zinc-200">AI Auto-Discovery:</strong> Web agents &amp; crawlers automatically find your card via <code className="text-cyan-300 bg-zinc-950/40 px-1 py-0.5 rounded">/.well-known/agent.json</code> and HTML head meta tags.
        </p>
      </div>

...
export function Hardware_Load() {
    ...
    useEffect(() => {
        ...
        const fetch_profile = async () => {
            try {
                // Determine base URL, assuming default port or proxy
                const base_url = localStorage.getItem('backend_url') || 'http://localhost:8000';
                
                const response = await fetch(`${base_url}/api/v1/system/compute-profile`, {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('tadpole_neural_token') || 'local-dev-bypass'}`,
                    }
                });
            ...
            <div className="space-y-6 relative z-10">
                ...
                <div>
                    ...
                    <div className="h-1.5 w-full bg-zinc-900 rounded-full overflow-hidden">
                        <div 
                            className={`h-full transition-all duration-500 ${profile.cpu_usage > 80 ? 'bg-red-500' : profile.cpu_usage > 50 ? 'bg-yellow-500' : 'bg-indigo-500'}`}

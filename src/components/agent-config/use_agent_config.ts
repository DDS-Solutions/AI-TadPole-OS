 ...
 */
export function use_agent_config(
    ...
    const [state, dispatch] = useReducer(config_reducer, {
        ...
        slots: {
            ...
            secondary: {
                provider: agent?.model_config2?.provider || resolve_provider(agent?.model2 ?? 'Claude Opus 4.5'),
            ...
            tertiary: {
                provider: agent?.model_config3?.provider || resolve_provider(agent?.model3 ?? 'LLaMA 4 Maverick'),

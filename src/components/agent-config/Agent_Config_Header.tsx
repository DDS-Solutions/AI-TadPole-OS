...
interface Agent_Config_Header_Props {
    name: string;
    role: string;
    department: string;
    theme_color: string;
    ...
    on_close: () => void;
    on_detach?: () => void;
    on_update_identity: (field: 'name' | 'role' | 'department', value: string) => void;
    on_update_theme_color: (color: string) => void;
    handle_role_change: (role: string) => void;
    is_detached?: boolean;
}
 ...
 */
export function Agent_Config_Header({
    name,
    role,
    department,
    theme_color,
        ...
        <div className="p-6 border-b border-zinc-800 bg-zinc-900/80 backdrop-blur-md flex items-start justify-between shrink-0 relative overflow-hidden">
                    ...
                    <Tooltip content="Select a custom color profile for this agent node's signature." position="top">
                        <div
                            ...
                            style={{
                                ...
                                boxShadow: `0 0 20px ${theme_color}10`
                            }}
                        >
                            <Sliders size={20} style={{ color: theme_color }} />
                            <input
                                type="color"
                                value={theme_color}
                                onChange={(e) => on_update_theme_color(e.target.value)}
                                className="absolute inset-0 opacity-0 cursor-pointer w-full h-full"
                                aria-label={i18n.t('agent_config.aria_theme_color')}
                            />
                        </div>
                    </Tooltip>
                    <div className="absolute -bottom-1 -right-1 w-3 h-3 rounded-full border border-black/50 shadow-sm" style={{ backgroundColor: theme_color }} />
                </div>
                <div className="space-y-1">
                    <h2 className="text-[11px] font-bold text-emerald-500 tracking-[0.2em] uppercase opacity-80">
                        {is_new ? i18n.t('agent_config.init_new') : i18n.t('agent_config.init_config')}
                    </h2>
                    <input
                        value={name}
                        onChange={(e) => on_update_identity('name', e.target.value)}
                    ...
                    <div className="flex items-center gap-3 pt-1">
                        <div className="relative group/role">
                            <select
                                value={role}
                                onChange={(e) => handle_role_change(e.target.value)}
                        ...
                        <div className="relative group/dept">
                            <select
                                value={department}
                                onChange={(e) => on_update_identity('department', e.target.value)}
                                aria-label={i18n.t('agent_config.aria_dept_selector')}
                                className="appearance-none bg-zinc-900/80 border border-zinc-700/50 rounded px-2 py-0.5 text-xs font-bold text-blue-400 uppercase tracking-[0.2em] cursor-pointer hover:border-blue-500/50 hover:text-blue-300 transition-all focus:outline-none pr-6"
                            >
                            ...
                            <ChevronDown size={10} className="absolute right-1.5 top-1/2 -translate-y-1/2 text-zinc-600 group-hover/dept:text-blue-400 pointer-events-none" />
                        </div>

                        <span className="text-[11px] text-zinc-500 font-mono tracking-tighter opacity-50">
                            {agent_id ? i18n.t('agent_config.neural_node_id', { id: agent_id.substring(0, 8).toUpperCase() }) : i18n.t('agent_config.id_pending')}
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div className="flex items-center gap-2 z-10 shrink-0">
                ...
                <Tooltip content={i18n.t('agent_config.aria_close_panel')} position="bottom">
                    <button onClick={on_close} className="p-3 w-11 h-11 flex items-center justify-center rounded-lg hover:bg-zinc-800 transition-colors text-zinc-500 hover:text-white" aria-label={i18n.t('agent_config.aria_close_panel')}>
                        <X size={20} />
                    </button>
                </Tooltip>
            </div>

 ...
 * - **Subsystem**: UI Components / Agent-Config / ModelSlotConfig
 * - **Primary Entrypoints**: `ModelSlotConfig`
 *
        ...
        <div className="space-y-5 animate-in fade-in duration-300" key={slotKey}>
            <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                    ...
                    <select
                        ...
                        className="w-full bg-[color:var(--color-surface)] border border-[color:var(--color-border)] rounded-lg px-3 py-2 text-xs text-zinc-200 focus:outline-none focus:border-zinc-700 cursor-pointer font-bold appearance-none transition-all"
                        style={{ borderLeft: `2px solid ${themeColor}40` }}
                    >
                        {providers.map(p => (
                            <option key={p.id} value={p.id} className="bg-[color:var(--color-background)]">
                                {p.name.toUpperCase()}
                            </option>
                        ))}
                    </select>
                ...
                <div className="space-y-1.5">
                    ...
                    <select
                        ...
                        className="w-full bg-[color:var(--color-surface)] border border-[color:var(--color-border)] rounded-lg px-3 py-2 text-xs text-zinc-200 focus:outline-none focus:border-zinc-700 font-mono cursor-pointer appearance-none transition-all"
                        style={{ borderLeft: `2px solid ${themeColor}40` }}
                    >
                        {filteredModels.map(m => (
                            <option key={m.id || `${m.provider}-${m.name}`} value={m.name} className="bg-[color:var(--color-background)]">
            ...
            <div>
                ...
                <textarea
                    ...
                    rows={6}
                    className="w-full bg-[color:var(--color-surface)] border border-[color:var(--color-border)] rounded-lg px-3 py-2 text-xs text-zinc-200 focus:outline-none focus:border-zinc-700 font-mono resize-none custom-scrollbar transition-all"
                    style={{ borderLeft: `3px solid ${themeColor}20` }}
                />
            </div>

            {/* Skills & Workflows Row */}
            <div className="grid grid-cols-2 gap-4 pt-4 border-t border-[color:var(--color-border)]/50">
                <div className="flex flex-col h-48">
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar border border-[color:var(--color-border)] rounded-lg bg-[color:var(--color-surface)]/30 p-2 space-y-2">
                        {allSkills.map(skillName => {
                            ...
                            const getDangerColor = (level?: string) => {
                                if (level === 'critical') return 'text-danger-text bg-danger-bg border-red-500/20';
                                ...
                                >
                                    <div className="flex items-start justify-between gap-2 overflow-hidden">
                                        <div className="flex items-center gap-2 min-w-0">
                                            <div 
                                                className={`w-3 h-3 rounded-full border shrink-0 transition-colors ${isSelected ? '' : 'border-zinc-600 bg-[color:var(--color-background)]'}`}
                                        ...
                                        <div className="flex items-center gap-1.5 shrink-0">
                                            ...
                                            {manifest ? (
                                                ...
                                                <span className="text-[8px] font-bold px-1 py-0.5 rounded bg-zinc-800 text-zinc-400 border border-zinc-700 uppercase tracking-tighter">{i18n.t('agent_config.label_script')}</span>
                ...
                <div className="flex flex-col h-48">
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar border border-[color:var(--color-border)] rounded-lg bg-[color:var(--color-surface)]/30 p-2 space-y-1">
                        {allWorkflows.map(wf => (
                            <button
                                key={wf}

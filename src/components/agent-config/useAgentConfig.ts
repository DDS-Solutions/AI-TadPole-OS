...
export function useAgentConfig(agent: Agent | undefined, onUpdate: (id: string, updates: Partial<Agent>) => void, onClose: () => void) {
    const [state, dispatch] = useReducer(configReducer, {
        mainTab: 'cognition',
        activeTab: agent?.activeModelSlot === 2 ? 'secondary' : agent?.activeModelSlot === 3 ? 'tertiary' : 'primary',
        identity: {
    ...
    const handleSave = useCallback(async () => {
        ...
        if (!ValidationUtils.isValidName(identity.name)) {
            EventBus.emit({ source: 'System', text: 'Invalid Neural Name: 2-64 characters required.', severity: 'warning' });
            return;
        }

        if (governance.budgetUsd < 0) {
            EventBus.emit({ source: 'System', text: 'Fiscal Burn limit must be non-negative.', severity: 'warning' });
            return;
        }
        ...
        try {
            ...
            updates.activeModelSlot = state.activeTab === 'primary' ? 1 : state.activeTab === 'secondary' ? 2 : 3;
    ...
    const handlePause = useCallback(async () => {
        ...
        onUpdate(agent.id, { status: 'suspended' });
        EventBus.emit({
            source: 'System',

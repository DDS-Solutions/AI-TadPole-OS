 ...
 * - **Subsystem**: UI Components / Agent-Config / useAgentConfig
 * - **Primary Entrypoints**: `useAgentConfig`
 *
...
import { resolve_agent_model_config, resolve_provider, parse_active_model_slot } from '../../utils/model_utils';
...
export function useAgentConfig(
    ...
    useEffect(() => {
        ...
        if (!lastAgent || agent === lastAgent) return; // Skip on initial mount or identical reference

        // Quick check: if only telemetry/tokens changed, skip heavy slot diffing
        const configKeysChanged = (
            agent.id !== lastAgent.id ||
            agent.active_model_slot !== lastAgent.active_model_slot ||
            agent.model !== lastAgent.model ||
            agent.model_2 !== lastAgent.model_2 ||
            agent.model_3 !== lastAgent.model_3 ||
            agent.model_config !== lastAgent.model_config ||
            agent.model_config2 !== lastAgent.model_config2 ||
            agent.model_config3 !== lastAgent.model_config3
        );
        if (!configKeysChanged) return;
        ...
        const expected_tab = (agent.active_model_slot ? SLOT_INDEX_MAP[parse_active_model_slot(agent.active_model_slot)] : undefined) ?? 'primary';
        if (parse_active_model_slot(agent.active_model_slot) !== parse_active_model_slot(lastAgent.active_model_slot)) {
            dispatch({ type: 'SET_TAB', payload: expected_tab });
        }

        // 2. Parameterized synchronization for primary, secondary, and tertiary model slots
        const getSlotModel = (a: Agent, s: Agent_Model_Slot_Key) => s === 'primary' ? a.model : s === 'secondary' ? a.model_2 : a.model_3;
        const getSlotConfig = (a: Agent, s: Agent_Model_Slot_Key) => s === 'primary' ? a.model_config : s === 'secondary' ? a.model_config2 : a.model_config3;
        const DEFAULT_TEMPS: Record<Agent_Model_Slot_Key, number> = { primary: 0.7, secondary: 0.5, tertiary: 0.9 };

        const slotKeys: Agent_Model_Slot_Key[] = ['primary', 'secondary', 'tertiary'];
        slotKeys.forEach(slotKey => {
            const agentModel = getSlotModel(agent, slotKey);
            const lastAgentModel = getSlotModel(lastAgent, slotKey);

            const config = getSlotConfig(agent, slotKey);
            const lastConfig = getSlotConfig(lastAgent, slotKey);
            ...
            const defaultTemp = DEFAULT_TEMPS[slotKey];
            const expectedTemp = config?.temperature ?? defaultTemp;
            const lastTemp = lastConfig?.temperature ?? defaultTemp;
    ...
    const handleSendMessage = useCallback(async () => {
        ...
        try {
            const live_agent = use_agent_store.getState().agents.find(a => a.id === agent.id) || agent;
            const { model_id, provider } = resolve_agent_model_config(live_agent, get_settings().default_model);
    ...
    const handlePromote = useCallback(() => {
        ...
        void (async () => {
            ...
            add_role(blueprint);
            void use_role_store.getState().fetch_blueprints();

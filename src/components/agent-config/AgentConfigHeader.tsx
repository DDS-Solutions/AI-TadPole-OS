...
import { X, Sliders, ChevronDown, ExternalLink, Settings } from 'lucide-react';
            ...
            <div className="flex items-start gap-4 z-10">
                ...
                <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-3 pt-1">
                        <div className="relative group/role flex items-center gap-1.5">
                            <div className="relative">
                                <select
                                    value={role}
                                    onChange={(e) => onRoleChange(e.target.value)}
                                    aria-label={i18n.t('agent_config.aria_role_selector')}
                                    className="appearance-none bg-[color:color-mix(in_srgb,var(--color-surface)_80%,transparent)] border border-zinc-700/50 rounded px-2 py-0.5 text-xs font-bold text-zinc-300 uppercase tracking-[0.2em] cursor-pointer hover:border-emerald-500/50 hover:text-emerald-400 transition-all focus:outline-none pr-6"
                                >
                            ...
                            <Tooltip content="Manage Roles" position="top">
                                <button
                                    ...
                                    className="p-1 hover:bg-zinc-800/80 rounded transition-colors text-zinc-500 hover:text-zinc-300 cursor-pointer flex items-center justify-center border border-zinc-700/30"
                                >
                                    <Settings size={12} />
                                </button>
                            </Tooltip>
                        ...
                        <div className="relative group/dept flex items-center gap-1.5">
                            <div className="relative">
                                <select
                                    value={department}
                                    onChange={(e) => onUpdateIdentity('department', e.target.value)}
                                    aria-label={i18n.t('agent_config.aria_dept_selector')}
                                    className="appearance-none bg-[color:color-mix(in_srgb,var(--color-surface)_80%,transparent)] border border-zinc-700/50 rounded px-2 py-0.5 text-xs font-bold text-green-400 uppercase tracking-[0.2em] cursor-pointer hover:border-green-500/50 hover:text-blue-300 transition-all focus:outline-none pr-6"
                                >
                                    {departments.map(d => (

 ...
 * 
 * ### AI Assist Note
 * **UI Component**: Modal manager for swarm Roles.
 * Handles role blueprints creation, renaming, and deletion.
 * 
 * ### 🔍 Debugging & Observability
 * - **Failure Path**: Failed backend API sync for role deletions or duplicate role validation failures.
 * - **Telemetry Link**: Search `[RoleManagerModal]` in UI logs.
 */
...
export function RoleManagerModal({ isOpen, onClose, currentAgentRole }: RoleManagerModalProps) {
    ...
    return createPortal(
        <>
            <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm pointer-events-auto" onClick={onClose}>
                <motion.div
                ...
                >
                    <div className="p-5 border-b border-zinc-800 flex items-center justify-between bg-zinc-900/40">
                        ...
                        <button onClick={onClose} className="text-zinc-500 hover:text-zinc-300 p-1.5 rounded hover:bg-zinc-800 cursor-pointer">
                            <X size={16} />
                        </button>
                    </div>
                                ...
                                <div key={r} className="flex items-center justify-between gap-2 p-3 bg-zinc-900/20 rounded-xl border border-zinc-800/40">
                                    {isEditing ? (
                                        <input
                                            ...
                                            className="bg-black/45 border border-zinc-700 rounded px-2.5 py-1 text-xs text-zinc-200 focus:outline-none focus:border-emerald-500/50 flex-1"
                                        />
                                    ) : (
                    ...
                    <div className="p-5 border-t border-zinc-800 bg-zinc-900/30 flex gap-2">
                        <input
                            ...
                            className="bg-black/30 border border-zinc-800 rounded-xl px-3.5 py-2 text-xs text-zinc-200 focus:outline-none focus:border-emerald-500/50 flex-1"
                        />
                        <button

 ...
 * - **Subsystem**: UI Components / General / Telemetry_Graph
 * - **Primary Entrypoints**: `Telemetry_Graph`
 *
...
import { type Trace_Span } from '../types';
...
const Telemetry_Node = ({ data }: { data: Trace_Span & { agent_name?: string } }) => {
    const span_name = data?.name && typeof data.name === 'string' ? data.name : '';
    const is_error = data.status === 'error';
    const is_running = data.status === 'running';
    const is_tool = span_name.startsWith('execute_tool');

    return (
        <div className={`
            relative px-4 py-3 rounded-2xl border min-w-[200px] max-w-[220px] overflow-hidden
            ${is_error ? 'bg-rose-950/20 border-rose-500/50' : 'bg-[color:var(--color-surface)]/90 border-[color:var(--color-border)]'}
        ...
        `}>
            ...
            <div className="flex flex-col gap-1">
                ...
                <h3 className="text-[12px] font-semibold text-zinc-100 truncate leading-tight">
                    {span_name ? (span_name.includes(':') ? span_name.split(':')[1] : span_name) : 'UNNAMED STEP'}
                </h3>
...
// Create a fresh dagre graph instance on every layout calculation to eliminate singleton state leakage
...
export const Telemetry_Graph: React.FC<{ initial_mission_id?: string }> = ({ initial_mission_id }) => {
    ...
    // Memoize graph topological structure hash so status updates do not trigger unnecessary DAG layout recalculation
    const structure_hash = useMemo(() => {
        return `${new_nodes.map(n => n.id).join(',')}|${new_edges.map(e => `${e.source}->${e.target}`).join(',')}`;
    }, [new_nodes, new_edges]);
    ...
    }, [structure_hash, new_nodes, new_edges]);
    ...
    return (
        <div className="w-full h-full bg-[color:var(--color-background)] rounded-[2rem] border border-[color:var(--color-surface)] overflow-hidden relative group">
            {nodes.length === 0 && (
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
                    <div className="bg-zinc-900/80 backdrop-blur-md border border-zinc-800 rounded-xl px-4 py-3 text-center space-y-1">
                        <p className="text-xs font-semibold text-zinc-300 uppercase tracking-widest">No Active Telemetry Traces</p>
                        <p className="text-[10px] font-mono text-zinc-500">Telemetry spans will stream into the execution graph automatically.</p>
                    </div>
                </div>
            )}
            ...
            >
                ...
                <Controls className="bg-[color:color-mix(in_srgb,var(--color-surface)_80%,transparent)] backdrop-blur-md border-[color:var(--color-border)] rounded-xl overflow-hidden fill-white z-20" />
                
                <Panel position="top-left" className="m-6 flex flex-col gap-3 z-20">
                    <div className="flex gap-2 items-center">
                        <select 
                            value={filter_mission_id || ''} 
                            onChange={(e) => set_filter_mission_id(e.target.value || undefined)}
                            className="bg-[color:var(--color-surface)]/60 backdrop-blur-xl border border-[color:var(--color-border)] rounded-xl px-3 py-2 text-[10px] font-bold text-zinc-300 uppercase tracking-widest outline-none focus:ring-2 focus:ring-cyan-500/20 transition-all hover:bg-[color:var(--color-surface)]"
                        ...
                        <button 
                            onClick={clear_all}
                            className="bg-[color:var(--color-surface)]/60 backdrop-blur-xl border border-[color:var(--color-border)] rounded-xl px-4 py-2 text-[10px] font-bold text-zinc-500 uppercase tracking-widest hover:text-rose-400 hover:border-rose-900/50 hover:bg-[color:var(--color-surface)] transition-all cursor-pointer"
                ...
                <Panel position="bottom-right" className="m-6 z-20">
                     <div className="px-4 py-2 bg-[color:color-mix(in_srgb,var(--color-background)_80%,transparent)] backdrop-blur-sm border border-[color:var(--color-surface)] rounded-xl flex items-center gap-4">

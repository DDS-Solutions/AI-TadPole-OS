...
export const Buffered_Transcript_View: React.FC<Buffered_Transcript_View_Props> = ({
    ...
    useEffect(() => {
        ...
        const unsubscribe = event_bus.subscribe_logs((entry) => {
            ...
            const entry_to_buffer = entry.id ? entry : { ...entry, id: `local-${entry.timestamp.getTime()}-${local_id_counter_ref.current++}` };
    ...
    return (
        <div className={clsx("flex flex-col h-full bg-black/40 border border-[color:var(--color-border)] rounded-xl overflow-hidden backdrop-blur-md", class_name)}>

...
export const Sovereign_Chat: React.FC<SovereignChatProps> = ({ is_detached_view }) => {
    ...
    const handle_send = useCallback(async (text?: string) => {
        ...
        try {
            ...
            await process_command(text, agents, is_safe_mode, active_scope, target_node);
        } catch (err: unknown) {
    ...
    }, [active_scope, target_node, agents, is_safe_mode, add_message]);
...
interface SovereignChatContentProps {
    ...
    handle_send: (text?: string) => Promise<void>;
    toggle_voice: () => void;
    toggle_speech: () => void;
    toggle_safety: () => void;

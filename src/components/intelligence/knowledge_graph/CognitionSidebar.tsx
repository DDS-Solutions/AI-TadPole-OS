 ...
 * 
 * ### AI Assist Note
 * **CognitionSidebar Component**: Displays detailed metadata about a selected symbol.
 * If the selected symbol is a "memory" node, provides an interactive workspace
 * to query, inject, and manage the agent's long-term memory space.
 */
...
const SymbolInfoSection = React.memo(({ 
    node, affected_nodes, total_nodes_count 
}: { 
            ...
            <div className="flex flex-col gap-2">
                ...
                <div className="mt-2 flex flex-col gap-2">
                    ...
                    <div className="w-full h-1 bg-zinc-800 rounded-full overflow-hidden">
                        <div className="h-full bg-rose-500 transition-all duration-500" style={{ width: `${progress_percent}%` }} />
                    </div>
                </div>
            </div>
...
export const CognitionSidebar: React.FC<CognitionSidebarProps> = ({
    selected_node, is_memory_node, active_info_tab, set_active_info_tab,
    affected_nodes, total_nodes_count, on_close
}) => {
    const workspace = useMemoryWorkspace(is_memory_node, active_info_tab);
        ...
        <div className={`absolute bottom-6 left-6 ${is_memory_node && active_info_tab === 'memory' ? 'w-[400px]' : 'w-80'} bg-zinc-900/80 backdrop-blur-xl border border-zinc-800 p-5 rounded-2xl animate-in fade-in slide-in-from-bottom-4 duration-300 transition-all z-50 shadow-2xl`}>
            <div className="flex flex-col gap-4">
                <div className="flex items-start justify-between">
                    ...
                    <button onClick={on_close} className="text-zinc-500 hover:text-white transition-colors cursor-pointer shrink-0">
                        <RefreshCw size={14} />
                    </button>
                </div>
                {is_memory_node && (
                    <div className="flex border-b border-zinc-850 pb-2">
                        <button onClick={() => set_active_info_tab('info')} className={`flex-1 text-[10px] font-bold uppercase tracking-wider text-center py-1 transition-all cursor-pointer ${active_info_tab === 'info' ? 'text-cyan-400 border-b-2 border-cyan-400 font-black' : 'text-zinc-500 hover:text-zinc-300 font-medium'}`}>Symbol Info</button>
                        <button onClick={() => set_active_info_tab('memory')} className={`flex-1 text-[10px] font-bold uppercase tracking-wider text-center py-1 transition-all cursor-pointer ${active_info_tab === 'memory' ? 'text-cyan-400 border-b-2 border-cyan-400 font-black' : 'text-zinc-500 hover:text-zinc-300 font-medium'}`}>Memory Workspace</button>
                    </div>
                )}
                {(!is_memory_node || active_info_tab === 'info') ? (
                    <SymbolInfoSection node={selected_node} affected_nodes={affected_nodes} total_nodes_count={total_nodes_count} />
                ) : (

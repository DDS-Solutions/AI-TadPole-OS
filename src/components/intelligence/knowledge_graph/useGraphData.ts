...
export interface UseGraphDataResult {
    ...
    viewMode: 'symbols' | 'memory' | 'okf';
    setViewMode: (mode: 'symbols' | 'memory' | 'okf') => void;
    resetSelection: () => void;
...
export function useGraphData(): UseGraphDataResult {
    // UI State
    const [viewMode, setViewModeState] = useState<'symbols' | 'memory' | 'okf'>('symbols');
    ...
    const setViewMode = useCallback((mode: 'symbols' | 'memory' | 'okf') => {
        ...
        const load = async () => {
            ...
            try {
                if (viewMode === 'symbols') {
                    ...
                    const memGraph = await intelligence_api_service.get_markdown_memory_graph(controller.signal);
                    if (controller.signal.aborted) return;
                    const nodes: ExtendedGraphNode[] = (memGraph.nodes || []).map(n => ({
                        id: n.id,
                        name: n.title || n.relative_path,
                    ...
                    const links: ForceGraphLink[] = (memGraph.edges || []).map(e => ({
                        source: e.source,
                        target: e.target,
                ...
                if (!controller.signal.aborted) {
                    setError(
                        viewMode === 'symbols'
                            ? i18n.t('knowledge_graph.error_fetch_symbols')
                            : viewMode === 'memory'
                            ? i18n.t('knowledge_graph.error_fetch_memory', { defaultValue: 'Failed to load markdown memory graph.' })
                            : i18n.t('knowledge_graph.error_fetch_okf')
                    );
                }
            } finally {
                if (!controller.signal.aborted) {

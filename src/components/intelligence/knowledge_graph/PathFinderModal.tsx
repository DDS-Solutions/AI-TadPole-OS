 ...
 * - **Subsystem**: UI Components / Intelligence / PathFinderModal
 * - **Primary Entrypoints**: `PathFinderModal`
 *
...
export const PathFinderModal: React.FC = () => {
    ...
    return (
        <div className="fixed inset-0 bg-zinc-950/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-in fade-in duration-200">
            <div className="w-full max-w-2xl bg-zinc-950 border border-zinc-800 rounded-2xl flex flex-col max-h-[90vh] shadow-2xl overflow-hidden">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-900 bg-zinc-900/50">
                    <div className="flex items-center gap-2.5">
                        ...
                        <h3 className="text-sm font-black text-white uppercase tracking-widest font-mono">
                            {i18n.t('knowledge_graph.pathfinder_title')}
                        </h3>
                    </div>
                    <button 
                        onClick={onClose}
                ...
                <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6 custom-scrollbar">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="flex flex-col gap-2">
                            <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider font-mono">
                                {i18n.t('knowledge_graph.pathfinder_label_start')}
                            </label>
                            <div className="relative">
                                <Search size={12} className="absolute left-3 top-3 text-zinc-500" />
                                <input
                                    type="text"
                                    placeholder={i18n.t('knowledge_graph.pathfinder_placeholder_start')}
                                    value={searchStart}
                                    onChange={(e) => setSearchStart(e.target.value)}
                                    className="w-full pl-9 pr-3 py-2 bg-zinc-900 border border-zinc-850 hover:border-zinc-800 focus:border-cyan-500 rounded-xl text-xs font-mono text-zinc-355 focus:outline-none placeholder-zinc-600 transition-colors animate-none"
                                />
                            </div>
                            <select
                                size={4}
                            ...
                            >
                                ...
                                {filteredStartNodes.length === 0 && (
                                    <option disabled className="text-zinc-600 text-center py-4">
                                        {i18n.t('knowledge_graph.pathfinder_no_symbols')}
                                    </option>
                                )}
                            </select>
                        </div>
                        ...
                        <div className="flex flex-col gap-2">
                            <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider font-mono">
                                {i18n.t('knowledge_graph.pathfinder_label_end')}
                            </label>
                            <div className="relative">
                                <Search size={12} className="absolute left-3 top-3 text-zinc-500" />
                                <input
                                    type="text"
                                    placeholder={i18n.t('knowledge_graph.pathfinder_placeholder_end')}
                                    value={searchEnd}
                    ...
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            className="flex-1 py-2.5 px-4 bg-cyan-500 hover:bg-cyan-400 disabled:bg-zinc-800 disabled:text-zinc-600 text-zinc-950 text-xs font-bold uppercase tracking-widest rounded-xl transition-colors cursor-pointer disabled:cursor-not-allowed font-mono flex items-center justify-center gap-2"
                        >
                            <GitCommit size={14} />
                            {i18n.t('knowledge_graph.pathfinder_btn_calculate')}
                        </button>
                        <button
                            onClick={handleClear}
                        ...
                        >
                            {i18n.t('knowledge_graph.pathfinder_btn_reset')}
                        </button>
                    </div>
                    ...
                    {searched && (
                        <div className="flex flex-col gap-3 border-t border-zinc-900 pt-5">
                            <h4 className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider font-mono">
                                {i18n.t('knowledge_graph.pathfinder_result_title')}
                            </h4>
                            ...
                            {calculatedPath && calculatedPath.length > 0 ? (
                                <div className="flex flex-col gap-2">
                                    <div className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/30 p-3 rounded-xl">
                                        ...
                                        <span className="text-[10px] text-emerald-400 font-bold font-mono">
                                            {i18n.t('knowledge_graph.pathfinder_result_found', { count: calculatedPath.length, edges: calculatedPath.length - 1 })}
                                        </span>
                                    </div>
                                ...
                                <div className="flex items-center gap-2 bg-rose-500/10 border border-rose-500/30 p-3 rounded-xl">
                                    ...
                                    <span className="text-[10px] text-rose-400 font-bold font-mono">
                                        {i18n.t('knowledge_graph.pathfinder_result_none')}
                                    </span>
                                </div>
                            )}
                        </div>
                ...
                <div className="px-6 py-4 border-t border-zinc-900 bg-zinc-900/20 flex items-center justify-between text-[9px] text-zinc-500 font-mono select-none">
                    <span>{i18n.t('knowledge_graph.pathfinder_footer_hint')}</span>
                </div>

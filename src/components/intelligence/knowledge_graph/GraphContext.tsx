...
export const GraphProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    // --- OKF Extensions ---
    const [viewMode, setViewMode] = useState<'symbols' | 'okf'>('symbols');

    // Clear coordinate cache and highlighted paths on view mode toggle to avoid coordinate overlap pollution and pathfinder desync
    useEffect(() => {
        nodesCache.clear();
    ...
    }, [viewMode, setHighlightedPathNodeIds]);
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            try {
                ...
                if (!controller.signal.aborted) {
                    setError(`Failed to fetch ${viewMode === 'symbols' ? 'code symbol' : 'semantic knowledge'} graph data.`);
                }
            } finally {
                if (!controller.signal.aborted) {
                    setLoading(false);
    ...
    const selectNode = useCallback(async (node: ExtendedGraphNode, pushHistory = true) => {
        ...
        try {
            ...
            if (!controller.signal.aborted) {
                setBlastRadiusError(viewMode === 'okf' ? 'Failed to fetch peer links.' : 'Failed to fetch blast radius.');
            }
        } finally {
            if (!controller.signal.aborted) {

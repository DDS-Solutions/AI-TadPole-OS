 ...
 * 
 * ### AI Assist Note
 * **GraphView Component**: Renders the 2D Canvas force-directed graph.
 * Features customizable colors, interactive nodes, and flowing directional particles.
 */

import React, { useMemo, useEffect } from 'react';
import ForceGraph2D from 'react-force-graph-2d';
import { THEME_COLORS, GRAPH_THEME } from '../../../constants/theme';
import type { ExtendedGraphNode, ForceGraphLink, GraphLinkObject, GraphMethods, GraphNodeObject } from './types';
...
interface GraphViewProps {
    ...
    fg_ref: React.MutableRefObject<GraphMethods | undefined>;
}
...
export const GraphView: React.FC<GraphViewProps> = ({
    ...
    useEffect(() => {
        return () => {
            if (fg_ref.current) fg_ref.current = undefined;
        };
    }, [fg_ref]);

    const get_link_source_id = (link: ForceGraphLink | GraphLinkObject): string => {
        if (typeof link.source === 'string') return link.source;
        return link.source?.id?.toString() || '';
    };

    const get_link_target_id = (link: ForceGraphLink | GraphLinkObject): string => {
    ...
    const node_canvas_object = useMemo(() => (node: GraphNodeObject, ctx: CanvasRenderingContext2D, global_scale: number) => {
        ...
        // 1. Glow Halo
        if (node.is_affected || (selected_node && selected_node.id === node.id) || node.is_path_highlighted) {
            ctx.beginPath();
        ...
        // 3. Highlight Border
        if ((hover_node && hover_node.id === node.id) || (selected_node && selected_node.id === node.id) || node.is_path_highlighted) {
        ...
        if (global_scale > 1.2 || (selected_node && selected_node.id === node.id) || node.is_affected || node.is_path_highlighted) {
    ...
    return (
        <ForceGraph2D<ExtendedGraphNode, ForceGraphLink>
            ...
            linkDirectionalParticleSpeed={(link: GraphLinkObject) => {
                return link.is_path_highlighted ? GRAPH_THEME.PARTICLE_SPEED * 2 : GRAPH_THEME.PARTICLE_SPEED;
            }}
            linkDirectionalParticleWidth={(link: GraphLinkObject) => {
                if (link.is_path_highlighted) {
            ...
            d3AlphaDecay={0.01}
            d3VelocityDecay={0.3}
            onNodeClick={(node) => on_node_click(node)}
            onNodeHover={(node) => set_hover_node(node)}
        />
    );
};

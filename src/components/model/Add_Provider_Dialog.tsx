 ...
 * - **Subsystem**: UI Components / Model / Add_Provider_Dialog
 * - **Primary Entrypoints**: `Add_Provider_Dialog`
 *
...
export function Add_Provider_Dialog({
    ...
    return (
        <div className="p-5 bg-[color:var(--color-surface)] border border-emerald-500/30 rounded-2xl flex flex-col space-y-4 animate-in fade-in zoom-in-95 shadow-xl shadow-emerald-500/5">
            <label htmlFor="new-provider-name" className="sr-only">{i18n.t('model_manager.add_provider.placeholder_name')}</label>
            <input
                id="new-provider-name"
                className="bg-[color:var(--color-background)] border border-[color:var(--color-border)] rounded-xl px-4 py-2 text-[10px] text-zinc-200 focus:outline-none focus:border-emerald-500/50 transition-colors font-mono"
            ...
            <input
                ...
                className="bg-[color:var(--color-background)] border border-[color:var(--color-border)] rounded-xl px-4 py-2 text-center text-sm"

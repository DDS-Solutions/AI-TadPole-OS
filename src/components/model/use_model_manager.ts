 ...
 * - **Subsystem**: UI Components / Model / use_model_manager
 * - **Primary Entrypoints**: `useModelManager`
 *
...
export function useModelManager() {
    ...
    const handle_add_provider = async () => {
        try {
            await provider_service.add_provider(provider_form.name, provider_form.icon);
    ...
    const handle_delete_confirm = async () => {
        ...
        try {
            if (confirm_delete.type === 'provider') {
                await provider_service.delete_provider(confirm_delete.id);
            } else {

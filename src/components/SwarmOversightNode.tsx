...
export const SwarmOversightNode: React.FC<SwarmOversightNodeProps> = ({ clusterId, onClose, className = '' }) => {
    const { activeProposals, clusters, dismissProposal } = useWorkspaceStore();
    
    // Filter proposals if clusterId is provided, otherwise show all
    const proposals = clusterId 
        ? (activeProposals[clusterId] ? [activeProposals[clusterId]] : []) 
        : Object.values(activeProposals);
                                                    ...
                                                    <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                                                        ...
                                                        {change.proposedModel && (
                                                            <div className="flex items-center gap-1.5 text-[9px]">
                                                                ...
                                                                <span className="text-blue-400 font-mono">{change.proposedModel}</span>
                                                            </div>
                                                        )}
                                                        {change.addedSkills && (
                                                            <div className="flex items-center gap-1.5 text-[9px] col-span-2">
                                                                ...
                                                                <span className="text-emerald-400 font-mono">+{change.addedSkills.length} {i18n.t('oversight.new_capabilities')}</span>
                                                            </div>
                                                        )}
                                                    </div>
                                                </div>
                            ...
                            <div className="px-4 pb-4 flex justify-end gap-2">
                                <Tooltip content={i18n.t('oversight.dismiss_tooltip')} position="top">
                                    <button 
                                        onClick={(e) => { 
                                            e.stopPropagation(); 
                                            dismissProposal(proposal.clusterId);
                                            onClose?.();
                                        }}
                                ...
                                <button 
                                    onClick={(e) => { 
                                        ...
                                        useWorkspaceStore.getState().applyProposal(proposal.clusterId);
                                        onClose?.();
                                    }}

 ...
 * - **Subsystem**: UI Components / Layout / Sidebar
 * - **Primary Entrypoints**: `Sidebar`
 *
...
export function Sidebar({ nav_item_class }: SidebarProps) {
    ...
    return (
        <aside className="w-16 lg:w-[260px] bg-[color:var(--color-background)] border-r border-[color:var(--color-border)] flex flex-col z-20 transition-all duration-300">
            <div className="p-4 lg:p-6 border-b border-[color:var(--color-border)] flex items-center gap-3 justify-center lg:justify-start">
                <Tooltip content={i18n.t('sidebar.root_tooltip')} position="right">
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center overflow-hidden bg-[color:var(--color-surface)]/50 border border-[color:var(--color-border)] shrink-0 shadow-lg group cursor-pointer">
                        <img 
                            src="/assets/logo.png" 
                            alt={i18n.t('sidebar.brand_name')}
            ...
            <nav className="flex-1 p-3 space-y-4 mt-4 overflow-y-auto custom-scrollbar">
                ...
                <div className="pt-2 border-t border-[color:var(--color-border)] mx-2"></div>
            ...
            <div className="p-4 border-t border-[color:var(--color-surface)] bg-[color:color-mix(in_srgb,var(--color-background)_80%,transparent)] backdrop-blur-sm hidden lg:block">

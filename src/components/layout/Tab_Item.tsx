 ...
 * - **Subsystem**: UI Components / Layout / Tab_Item
 * - **Primary Entrypoints**: `Tab_Item`
 *
...
export const Tab_Item = React.memo(({ tab, is_active }: Tab_Item_Props) => {
    ...
    return (
        <div
            ...
            className={clsx(
                "group relative flex items-center gap-2 px-4 py-2 min-w-[120px] max-w-[200px] cursor-pointer transition-all duration-200 border-r border-[color:var(--color-surface)]",
                is_active 
                    ? "bg-[color:var(--color-surface)] text-zinc-100" 
                    : "text-zinc-500 hover:bg-[color:var(--color-surface)]/40 hover:text-zinc-300"
            )}
        >
            ...
            <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-all">
                <Tooltip content={tab.is_detached ? i18n.t('layout.recall_sector') : i18n.t('layout.pop_out_sector')} position="top">
                    <button
                        onClick={handle_detach}
                        aria-label={tab.is_detached ? i18n.t('layout.recall_sector') : i18n.t('layout.pop_out_sector')}
                        className={clsx(
                ...
                <button
                    ...
                    aria-label={i18n.t('layout.close_sector')}
                    title={i18n.t('layout.close_sector')}
                    className={clsx(
                        "p-0.5 rounded-md hover:bg-zinc-800 transition-all",
                        is_active ? "text-zinc-500 hover:text-zinc-200" : "text-zinc-700 hover:text-zinc-400"
                    )}

 ...
 * - **Subsystem**: UI Components / Layout / Observability_Sidebar
 * - **Primary Entrypoints**: `Observability_Sidebar`
 *
...
export const Observability_Sidebar: React.FC<Observability_Sidebar_Props> = ({ 
    ...
    return (
        <div className={clsx(
            ...
            is_detached_context ? "w-[400px] border-l border-[color:var(--color-surface)] bg-zinc-950/20" : "w-[400px]",
            className
        )}>
            ...
            {(is_lineage_stream_detached && is_trace_stream_detached && is_system_log_detached) && (
                <div className="flex-1 flex flex-col items-center justify-center text-zinc-800 p-8 text-center">
                    <div className="w-12 h-12 rounded-full border border-[color:var(--color-surface)] flex items-center justify-center mb-4">
                        <div className="w-2 h-2 rounded-full bg-[color:var(--color-surface)] animate-pulse" />
                    </div>

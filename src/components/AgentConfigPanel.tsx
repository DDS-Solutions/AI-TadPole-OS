...
export default function AgentConfigPanel({ agent, onClose, onUpdate, isNew = false }: AgentConfigPanelProps): JSX.Element {
    ...
    const { skills, workflows, mcpTools, fetchCapabilities, fetchMcpTools } = useCapabilitiesStore();

    useEffect(() => {
        fetchCapabilities();
        fetchMcpTools();
        TadpoleOSService.getSkills().then(setSkillManifests).catch(console.error);
    }, [fetchCapabilities, fetchMcpTools]);

    const allSkills = useMemo(() => {
        const standardNames = skills.map(s => s.name);
        const mcpNames = mcpTools.map(t => t.name);
        const manifestNames = skillManifests.map(m => m.name);
        const agentAssigned = agent?.capabilities || [];

        // Unify all sources to ensure absolute visibility
        return [...new Set([
            ...standardNames,
            ...mcpNames,
            ...manifestNames,
    ...
    }, [skills, mcpTools, skillManifests, agent?.capabilities]);

    const allWorkflows = useMemo(() => {
        const standardNames = workflows.map(w => w.name);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-6">
            ...
            <div className="relative w-full max-w-2xl bg-zinc-950 border border-zinc-800 rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300 max-h-[90vh]">
                ...
                <div className="p-6 border-b border-zinc-800 bg-zinc-900/80 backdrop-blur-md flex items-start justify-between shrink-0 relative overflow-hidden">
                    ...
                    <button onClick={onClose} className="p-3 w-11 h-11 flex items-center justify-center rounded-lg hover:bg-zinc-800 transition-colors text-zinc-500 hover:text-white z-10 shrink-0">
                        <X size={20} />
                    </button>
                </div>
                                    ...
                                    <div className="flex flex-col h-48">
                                        ...
                                        <div className="flex-1 overflow-y-auto custom-scrollbar border border-zinc-800 rounded-lg bg-zinc-900/30 p-2 space-y-2">
                                            {allSkills.map(skillName => {
                                                const manifest = skillManifests.find((m: SkillManifest) => m.name === skillName);
                                                ...
                                                return (
                                                    <div
                                                        key={skillName}
                                                        onClick={() => dispatch({ type: 'TOGGLE_CAPABILITY', slot: activeTab, kind: 'skills', value: skillName as string })}
                            ...
                            <div className="pt-4 border-t border-zinc-800/50">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <Tooltip content={i18n.t('agent_config.tooltip_inject')} position="left">
                                        <button onClick={handleSendMessage} className="p-3 w-11 h-11 flex items-center justify-center rounded-lg bg-blue-500/10 text-blue-400">
                                            <Send size={14} />
                                        </button>
                                    </Tooltip>
                                </div>

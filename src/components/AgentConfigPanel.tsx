 ...
 * - **Subsystem**: UI Components / General / AgentConfigPanel
 * - **Primary Entrypoints**: `AgentConfigPanel`
 *
 ...
 */
export default function AgentConfigPanel({ agent, onClose, onUpdate, onDelete, isNew = false, isDetachedMode = false }: AgentConfigPanelProps) {
    const {
        state,
        dispatch,
        handleRoleChange,
    ...
    const handleUpdateSlotField = useCallback<SlotFieldUpdater>((slot, field, value) => {
        dispatch({ type: 'UPDATE_SLOT', slot, field, value } as unknown as Parameters<typeof dispatch>[0]);
    }, [dispatch]);
    ...
    type GovernanceField = 'budget_usd' | 'requires_oversight' | 'economic_zone' | 'daily_spend_limit' | 'shadows_human_id';
    ...
    const handleUpdateGovernance = useCallback(
        <F extends GovernanceField>(field: F, value: GovernanceValue<F>) => {
            dispatch({ type: 'UPDATE_GOVERNANCE', field, value } as unknown as Parameters<typeof dispatch>[0]);
        },
        [dispatch]
    );
    ...
    const MAIN_TABS = [
        { id: 'cognition' as const, label: i18n.t('agent_config.tab_cognition'), activeColor: 'text-emerald-400', indicatorBg: 'bg-emerald-500' },
        { id: 'memory' as const, label: i18n.t('agent_config.tab_memory'), activeColor: 'text-green-400', indicatorBg: 'bg-green-500' },
        { id: 'governance' as const, label: i18n.t('agent_config.tab_governance'), activeColor: 'text-amber-400', indicatorBg: 'bg-amber-500' },
    ];
    ...
    const panelContent = (
        <div className="flex-1 flex flex-col min-h-0 bg-[color:var(--color-background)]/40 backdrop-blur-xl">
            <AgentConfigHeader
                ...
                onDelete={agent?.id && onDelete ? () => { onDelete(agent.id); onClose(); } : undefined}
            ...
            <div className="flex border-b border-[color:var(--color-surface)] bg-[color:var(--color-surface)]/40 px-6 shrink-0 z-10">
                {MAIN_TABS.map(tab => (
                    <button
                        key={tab.id}
                        onClick={() => dispatch({ type: 'SET_MAIN_TAB', payload: tab.id })}
                        className={`px-4 py-3 text-[10px] font-bold uppercase tracking-[0.2em] transition-all relative ${mainTab === tab.id ? tab.activeColor : 'text-zinc-500 hover:text-zinc-300'}`}
                    >
                        {tab.label}
                        {mainTab === tab.id && <div className={`absolute bottom-0 left-0 w-full h-0.5 ${tab.indicatorBg}`} />}
                    </button>
                ))}
            </div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar min-h-0">
                    ...
                    {mainTab === 'governance' && (
                        <GovernanceSection
                            ...
                            shadows_human_id={governance.shadows_human_id}
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 pointer-events-none">
                    ...
                    >
                        ...
                        <button
                            ...
                            className="px-6 py-2 bg-zinc-100 text-zinc-950 text-xs font-bold uppercase tracking-[0.2em] rounded-lg hover:bg-white transition-all shadow-lg active:scale-95 z-10"
                        >
        ...
        <AnimatePresence>
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-2 sm:p-6 md:p-10 pointer-events-none">
                <motion.div
                    ...
                    className="absolute inset-0 bg-zinc-950/60 backdrop-blur-md pointer-events-auto"
                />
                ...
                <motion.div
                    ...
                    className="relative w-full max-w-3xl  max-h-[95vh] sm:max-h-[90vh] bg-[color:var(--color-background)]/90 backdrop-blur-2xl rounded-[2.5rem] border border-white/5 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden pointer-events-auto"
                >

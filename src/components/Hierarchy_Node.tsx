 ...
 * - **Subsystem**: UI Components / General / Hierarchy_Node
 * - **Primary Entrypoints**: `Hierarchy_Node`
 *
...
import { use_workspace_store, type Workspace_State, type Mission_Cluster } from '../stores/workspace_store';
...
const Hierarchy_Node_Base: React.FC<Hierarchy_Node_Props> = ({
    ...
    useLayoutEffect(() => {
        if (!is_oversight_open || !node_ref.current) return;

        const update_pos = () => {
            if (!node_ref.current) return;
            const rect = node_ref.current.getBoundingClientRect();
            // Prevent horizontal overflow past viewport width (assuming ~350px portal width)
            const calculated_left = Math.min(rect.right + 20, window.innerWidth - 350);
            set_portal_pos({ 
                top: Math.max(16, rect.top), 
                left: Math.max(16, calculated_left) 
            });
        };

        update_pos();
        window.addEventListener('resize', update_pos);
        window.addEventListener('scroll', update_pos, true);
        return () => {
            window.removeEventListener('resize', update_pos);
            window.removeEventListener('scroll', update_pos, true);
        };
    }, [is_oversight_open]);

    const agent_id = agent?.id;
    const cluster = use_workspace_store(useCallback((s: Workspace_State) => 
        s.clusters.find((c: Mission_Cluster) => (c.collaborators || []).includes(agent_id || '')),
    [agent_id]));
    const cluster_id = cluster?.id;
    const has_oversight = use_workspace_store(useCallback((s: Workspace_State) => 
        cluster_id ? !!s.active_proposals[cluster_id] : false,
    [cluster_id]));
    ...
    return (
        <div 
            ref={node_ref} 
            role="region"
            aria-label={`Agent Node: ${agent.name}`}
            className="relative group transition-all duration-300 w-full z-0"
        >
            <div
                className={`
                    relative z-10 p-3 rounded-xl border backdrop-blur-xl transition-all duration-300
                    bg-[#1b1b1e]/60
                    ${agent.status !== 'offline' && agent.status !== 'idle' ? 'border-[color:var(--color-border)]' : 'border-[color:var(--color-border)]/30'}
            ...
            {!is_root && (
                <div className="absolute -top-1 left-1/2 -translate-x-1/2 w-2 h-2 rounded-full border border-current bg-[color:var(--color-background)] opacity-0 group-hover:opacity-100 transition-opacity" />
            )}
            <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-2 h-2 rounded-full border border-current bg-[color:var(--color-background)] opacity-0 group-hover:opacity-100 transition-opacity" />

            {/* Neural Glow (Active Mission) */}
            <AnimatePresence>
                {(is_active || ['active', 'thinking', 'speaking', 'coding'].includes(agent.status)) && (
                    <motion.div
...
export const Hierarchy_Node = React.memo(Hierarchy_Node_Base, (prev, next) => {
    ...
    // and shallow comparison for primitive props and roles array contents.
    const roles_equal = prev.available_roles === next.available_roles ||
        (Array.isArray(prev.available_roles) && Array.isArray(next.available_roles) &&
         prev.available_roles.length === next.available_roles.length &&
         prev.available_roles.every((role, idx) => next.available_roles ? role === next.available_roles[idx] : false));

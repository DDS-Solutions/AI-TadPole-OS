...
export const Swarm_Visualizer: React.FC<{ is_detached?: boolean, on_detach?: () => void }> = ({ is_detached = false, on_detach }) => {
    ...
    useEffect(() => {
        ...
        // ### 🛰️ Telemetry Orchestration (10Hz)
        // Consumes the engine's binary MessagePack telemetry feed. 
        // This subscription ensures that agent state changes (IDLE -> BUSY) 
        // are reflected in the UI with sub-100ms latency.
        const unsubscribe = tadpole_os_socket.subscribe_swarm_pulse((pulse: Swarm_Pulse) => {
            set_graph_data(prev => {
                // 1. Map Nodes
                // We maintain spatial continuity by cross-referencing incoming pulse IDs 
                // with existing local graph nodes. If a node already has [x, y] 
                // velocity components from the force engine, we preserve them 
                // to prevent "UI Flickering" during high-frequency updates.
                ...
                // 2. Map Links (Neural Edge Topology)
    ...
    const node_canvas_object = useMemo(() => (node: GraphNode, ctx: CanvasRenderingContext2D, global_scale: number) => {
        ...
        // 🛡️ Drawing Layer 1: Glow Halo (Depth/Status focus)
        // Active (Busy) agents emitted a cyan glow, while Error states utilize Rose. 
        // This provides an immediate pre-attentive signal to the Overlord.
        ...
        // Fixed-radius circle representing the agent's presence in the swarm.
        ctx.beginPath();
        ctx.arc(node.x ?? 0, node.y ?? 0, radius, 0, 2 * Math.PI, false);
        ctx.fillStyle = status_color;
        ctx.fill();
        
        // 🛡️ Drawing Layer 3: Busy Pulse Animation (Kinetic Feedback)
        // Uses a sine-wave oscillation on the system clock to create a 
        // "Breathing" effect for agents engaged in missions.
        if (node.status === 1) {
        ...
        // 🛡️ Drawing Layer 4: Semantic Labels (Scale-conditional)
        // We only render text labels when zoomed in to prevent "Label Overlap" 
        // congestion during wide-angle swarm monitoring.
        if (global_scale > 2) {
        ...
        // Renders ephemeral health status (Battery/Signal Strength) directly 
        // onto the node scope without requiring a hover state.

...
export const Swarm_Visualizer: React.FC = () => {
    ...
    const [graph_data, set_graph_data] = useState({ nodes: [] as any[], links: [] as any[] });
    ...
    const node_canvas_object = useMemo(() => (node: any, ctx: CanvasRenderingContext2D, global_scale: number) => {
        ...
        if (node.status === 1) {
             ...
             ctx.beginPath();
             ctx.arc(node.x, node.y, radius + (pulse * 3), 0, 2 * Math.PI, false);
        ...
        if (global_scale > 1.5) {
            ...
            ctx.fillRect(node.x - bar_w/2, node.y - radius - 5, bar_w, bar_h);
            ctx.fillStyle = node.battery > 20 ? '#10b981' : '#f43f5e';
            ctx.fillRect(node.x - bar_w/2, node.y - radius - 5, bar_w * (node.battery / 100), bar_h);
        }
    }, [agents]);

...
export const Swarm_Visualizer: React.FC<{ is_detached?: boolean, on_detach?: () => void }> = ({ is_detached = false, on_detach }) => {
                ...
                const new_nodes = pulse.nodes.map(pulse_node => {
                    ...
                    if (existing) {
                        Object.assign(existing, {
                            ...sanitized_metrics,
                            name: agent?.name || (pulse_node.status === SWARM_NODE_STATUS.MISSION_HUB ? i18n.t('swarm_visualizer.label_mission_hub') : pulse_node.id),
                        });
                        return existing;
                    }
    ...
    const logic_node_canvas_object = useMemo(() => (node: GraphNode, ctx: CanvasRenderingContext2D, global_scale: number) => {
        ...
        if (global_scale > 1.2) {
            ...
            // Sanitize label: remove newlines and extra spaces
            const sanitized_name = (node.name || '').replace(/\n/g, ' ').replace(/\s+/g, ' ').trim();
            const safe_label = sanitized_name.length > 30 ? sanitized_name.substring(0, 27) + '...' : sanitized_name;

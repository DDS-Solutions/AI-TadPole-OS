...
export const LineageStream: React.FC<{ isDetachedView?: boolean }> = ({ isDetachedView = false }): React.ReactElement => {
    const { activeTraceId, getTraceTree } = useTraceStore();
    const { isLineageStreamDetached, toggleLineageStreamDetachment } = useTabStore();
    ...
    return (
        <div
            className={clsx(
                ...
                !isDetachedView && "flex-1 sovereign-card mb-4",
                isDetachedView && "h-full"
            )}
            style={{ width: isDetachedView ? '100%' : sidebarWidth }}
            ref={streamRef}
        >
            ...
            <Tooltip content={i18n.t('trace.tooltip')} position="left">
                <div className="relative z-10 p-3 border-b border-zinc-800 bg-zinc-950 flex items-center justify-between transition-colors cursor-help">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button
                            onClick={() => toggleLineageStreamDetachment()}
                            className="p-1 hover:bg-zinc-800 rounded-md text-zinc-500 hover:text-zinc-200 transition-colors"
                            title={isLineageStreamDetached ? i18n.t('trace_stream.recall_tooltip') : i18n.t('trace_stream.detach_tooltip')}
                        >
                            {isLineageStreamDetached ? <Minimize2 size={14} /> : <ExternalLink size={14} />}
                        </button>
                    </div>
                </div>
            </Tooltip>

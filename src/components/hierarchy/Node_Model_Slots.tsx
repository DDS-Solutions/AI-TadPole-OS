...
export const Node_Model_Slots: React.FC<Node_Model_Slots_Props> = ({
    ...
    const is_model_2_open = use_dropdown_store(s => s.is_open(agent.id, 'model2'));
    const is_model_3_open = use_dropdown_store(s => s.is_open(agent.id, 'model3'));
        ...
        return (
            <div className="relative" key={`slot-${slot_idx}`} role="presentation" onClick={(e) => e.stopPropagation()} onKeyDown={(e) => e.stopPropagation()}>
                ...
                <Model_Badge
                    ...
                    on_click={() => toggle_dropdown(agent.id, slot_idx === 1 ? 'model' : slot_idx === 2 ? 'model2' : 'model3')}
                />
                {is_open && (
    ...
    return (
        <div className={`flex flex-col gap-1.5 border-t border-zinc-800 pt-2 relative ${is_model_1_open || is_model_2_open || is_model_3_open ? 'z-50' : 'z-20'}`}>
            <div className="flex items-center gap-1.5 overflow-visible">
                ...
                {render_slot(2, agent.model2, is_model_2_open, on_model_2_change)}
                {render_slot(3, agent.model3, is_model_3_open, on_model_3_change)}
            </div>
        </div>
    );
};

    ...
    const render_slot = (slot_idx: 1 | 2 | 3, model: string | undefined, is_open: boolean, on_model_update?: (id: string, m: string) => void) => {
        ...
        return (
            <div className="relative" key={`slot-${slot_idx}`} role="presentation" onClick={(e) => e.stopPropagation()} onKeyDown={(e) => e.stopPropagation()}>
                ...
                />
                {is_open && (
                    <div className="absolute top-full left-0 mt-1 w-56 bg-zinc-950 border border-zinc-800 rounded-xl shadow-2xl z-50 py-1.5 max-h-60 overflow-y-auto custom-scrollbar">
                        {available_models.map((m) => (
                            <button key={m.id} onClick={() => {
                                on_model_update?.(agent.id, m.name);
                                close_dropdown();
                            }}
                                className={`w-full text-left px-3 py-2 text-[10px] hover:bg-zinc-900 transition-colors flex items-center justify-between gap-2 ${model === m.name ? 'text-blue-400 font-bold bg-blue-500/5' : 'text-zinc-400'}`}>

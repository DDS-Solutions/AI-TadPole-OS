 ...
 * - **Subsystem**: UI Components / Hierarchy / Node_Header
 * - **Primary Entrypoints**: `Node_Header`
 *
...
export const Node_Header: React.FC<Node_Header_Props> = ({
    ...
    const is_role_dropdown_open = use_dropdown_store(s => s.open_id === agent.id && s.open_type === 'role');
    ...
    const role_badge_class =
        agent.department === 'Executive' ? 'text-amber-400 border-amber-900 bg-amber-900/10' :
            agent.department === 'Engineering' ? 'text-green-400 border-green-500/20 bg-green-500/10' :
                agent.department === 'Product' ? 'text-orange-400 border-orange-900 bg-orange-900/10' :
                    'text-zinc-400 border-[color:var(--color-border)] bg-[color:var(--color-surface)]';
    ...
    const health_tooltip = failure_count >= 3 ? i18n.t('security.throttled') : failure_count > 0 ? i18n.t('security.status_degraded') : i18n.t('security.status_healthy');
    ...
    return (
        <div className={`grid grid-cols-[min-content_1fr_min-content] gap-2 items-start ${is_role_dropdown_open ? 'z-50' : 'z-20'}`}>
            <div className={`
                ...
                ${is_alpha ? 'bg-[color:var(--color-surface)] border-amber-500/50 shadow-[0_0_10px_rgba(245,158,11,0.2)]' : (agent.status !== 'offline' && agent.status !== 'idle' ? 'bg-[color:var(--color-surface)] border-white/10' : 'bg-[color:var(--color-background)] border-white/5')}
            `}>
                ...
                {is_alpha ? (
                    <Tooltip content="Alpha Node (Swarm Commander) — Tactical Decomposer, Fan-In Digest & Concurrency Guard Active" position="top">
            ...
            <div className="flex flex-col min-w-0 overflow-hidden">
                ...
                <div className="relative shrink-0" role="presentation" onClick={(e) => e.stopPropagation()} onKeyDown={(e) => e.stopPropagation()}>
                    ...
                    {is_role_dropdown_open && (
                        <div className="absolute top-full left-0 mt-1 w-48 bg-[color:var(--color-surface)] border border-zinc-700 rounded-lg shadow-xl z-50 py-1 max-h-60 overflow-y-auto custom-scrollbar">
                            {available_roles.map((role) => (
                                <button
                                    ...
                                    className={`w-full text-left px-3 py-1.5 text-xs transition-colors hover:bg-[color:var(--color-surface)] hover:text-white border-l-2 ${agent.role === role ? 'text-green-400 font-bold bg-green-500/10 border-emerald-500' : 'text-zinc-300 border-transparent hover:border-zinc-500'}`}
                                >
                                    {role}
                                </button>
                            ))}
            ...
            <div className="flex items-center gap-2 shrink-0">
                            ...
                            <button
                                ...
                                className={`
                                    ...
                                    ${is_oversight_open
                                        ...
                                        : 'bg-[color:var(--color-surface)] border-zinc-700 text-zinc-500 hover:border-green-500/50 hover:text-green-400 shadow-[0_0_10px_rgba(34,197,94,0.1)]'
                                    }
                                `}
                            >
                ...
                <Tooltip content={i18n.t('agent_card.tooltip_configure')} position="top">
                    <button
                        ...
                        className="p-1 rounded hover:bg-zinc-800/80 text-zinc-600 hover:text-green-400 transition-colors"
                    >
                        <Sliders size={12} />
                    </button>
                </Tooltip>

 ...
 * - **Subsystem**: UI Components / Hierarchy / Node_Health
 * - **Primary Entrypoints**: `Node_Health`
 *
...
export const Node_Health: React.FC<Node_Health_Props> = ({ agent, on_close }) => {
    ...
    const handle_reset = async (e: React.MouseEvent) => {
        ...
        try {
            ...
            if (result.status === 'ok') {
                void agent_service.update_agent(agent.id, { 
                    failure_count: 0, 
    ...
    return (
        <div className="absolute inset-0 bg-[color:var(--color-background)]/95 backdrop-blur-md z-[60] flex flex-col p-3 border border-[color:var(--color-border)]/50 rounded-lg animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between mb-3 border-b border-[color:var(--color-border)]/50 pb-2">
                <div className="flex items-center gap-2">
                    <Shield size={14} className="text-green-400" />
                    <span className="text-[10px] uppercase font-bold tracking-widest text-zinc-400">
                        {i18n.t('security.swarm_health_monitor')}
                    </span>
                </div>
                <button 
            ...
            <div className="flex-1 flex flex-col gap-4">
                <div className="flex items-center justify-between bg-[color:var(--color-surface)]/40 p-2 rounded border border-[color:var(--color-border)]/30">
                    <div className="flex items-center gap-2">
                        {is_healthy ? (
                        ...
                        <span className={`text-xs font-bold uppercase tracking-wide ${
                            ...
                            {is_healthy ? i18n.t('security.status_healthy') : is_throttled ? i18n.t('security.throttled') : i18n.t('security.status_degraded')}
                        </span>
                    </div>
                    <div className="text-[10px] font-mono text-zinc-500">
                        {i18n.t('security.failures_label', { count: failure_count })}
                    </div>
                </div>
                ...
                <div className="mt-auto">
                    <button
                        ...
                        className={`w-full py-2 px-3 rounded flex items-center justify-center gap-2 text-xs font-bold transition-all ${
                            is_healthy 
                                ? 'bg-zinc-800/50 text-zinc-600 cursor-not-allowed border border-[color:var(--color-border)]'
                    ...
                    >
                        ...
                        {is_resetting ? 'Resetting...' : i18n.t('security.reset_agent')}
                    </button>
                </div>
            </div>

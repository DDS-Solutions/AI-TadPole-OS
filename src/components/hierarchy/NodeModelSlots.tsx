...
export const NodeModelSlots: React.FC<NodeModelSlotsProps> = ({
    ...
    const renderSlot = (slotIdx: 1 | 2 | 3, model: string | undefined, isOpen: boolean, onModelUpdate?: (id: string, m: string) => void) => {
        ...
        return (
            <div className="relative" key={`slot-${slotIdx}`} role="presentation" onClick={(e) => e.stopPropagation()} onKeyDown={(e) => e.stopPropagation()}>
                ...
                <ModelBadge
                    ...
                    // The ModelBadge itself also has an onClick, which is now redundant with the parent button's onClick
                    // but keeping it for now as it might have other side effects or styling.
                    // If the parent button handles opening the dropdown, this onClick might be removed.

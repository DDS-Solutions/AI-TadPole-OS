...
export const Node_Stats: React.FC<Node_Stats_Props> = ({ agent, on_skill_trigger }) => {
    ...
    const model_slots = [
        ...
        { label: i18n.t('agent_card.label_secondary'), model: agent.model2, config: agent.model_config2 },
        { label: i18n.t('agent_card.label_tertiary'), model: agent.model3, config: agent.model_config3 },
    ].filter(s => s.model);
                ...
                <div className="relative shrink-0" role="presentation" onClick={(e) => e.stopPropagation()} onKeyDown={(e) => e.stopPropagation()}>
                    <Tooltip content={i18n.t('agent_card.tooltip_skills')} position="top">
                        <button
                            ...
                            className={`p-1 rounded hover:bg-zinc-800 transition-colors ${is_skill_dropdown_open ? 'text-blue-400 bg-blue-900/10' : 'text-zinc-600 hover:text-zinc-300'}`}
                        >

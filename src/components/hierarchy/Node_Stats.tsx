 ...
 * - **Subsystem**: UI Components / Hierarchy / Node_Stats
 * - **Primary Entrypoints**: `Node_Stats`
 *
...
export const Node_Stats: React.FC<Node_Stats_Props> = ({ agent, on_skill_trigger }) => {
    ...
    const is_skill_dropdown_open = use_dropdown_store(s => s.open_id === agent.id && s.open_type === 'skill');
    ...
    return (
        <div className={`grid grid-cols-[1fr_min-content] gap-2 items-center px-1 py-1 border-b border-[color:var(--color-border)]/30 relative ${is_skill_dropdown_open ? 'z-50' : 'z-30'}`}>
                ...
                <div className="relative shrink-0" role="presentation" onClick={(e) => e.stopPropagation()} onKeyDown={(e) => e.stopPropagation()}>
                    <Tooltip content={i18n.t('agent_card.tooltip_skills')} position="top">
                        <button
                            ...
                            className={`p-1 rounded hover:bg-zinc-800 transition-colors ${is_skill_dropdown_open ? 'text-green-400 bg-green-500/10' : 'text-zinc-600 hover:text-zinc-300'}`}
                        >
                    ...
                    {is_skill_dropdown_open && createPortal(
                        <div 
                            className="fixed mt-2 w-64 bg-[color:var(--color-background)]/95 backdrop-blur-md border border-[color:var(--color-border)] rounded-lg shadow-[0_0_30px_rgba(0,0,0,0.5)] z-[110] overflow-hidden animate-in fade-in zoom-in-95 duration-100 max-h-80 overflow-y-auto custom-scrollbar"
                        ...
                        >
                            ...
                            <div className="relative z-10 w-full flex flex-col">
                                {model_slots.map((slot, idx) => {
                                    ...
                                    return (
                                        <div key={slot.label} className={idx > 0 ? 'border-t border-[color:var(--color-border)]' : ''}>
                                            <div className="px-3 py-1.5 text-[10px] uppercase font-bold tracking-widest bg-[color:var(--color-background)]/60 border-b border-[color:var(--color-border)]/50 flex items-center justify-between gap-2">
                                            ...
                                            {skills.length > 0 && (
                                                <React.Fragment key={`${slot.label}-skills`}>
                                                    <div className="px-4 py-1.5 text-[9px] uppercase font-bold text-zinc-600 tracking-[0.2em] flex items-center gap-1.5 bg-[color:color-mix(in_srgb,var(--color-background)_80%,transparent)]">
                                                    ...
                                                    {(skills || []).map((skill) => (
                                                        <button 
                                                            ...
                                                            className="w-full text-left px-4 py-2 text-xs text-zinc-300 hover:bg-[color:var(--color-surface)] border-l-2 border-transparent hover:border-emerald-500 hover:text-white transition-all flex items-center gap-2 group font-mono outline-none focus:bg-[color:var(--color-surface)] focus:border-emerald-500 focus:text-white"
                                            ...
                                            {workflows.length > 0 && (
                                                <React.Fragment key={`${slot.label}-workflows`}>
                                                    <div className="px-4 py-1.5 mt-1 border-t border-[color:var(--color-border)]/30 text-[9px] uppercase font-bold text-zinc-600 tracking-[0.2em] flex items-center gap-1.5 bg-[color:color-mix(in_srgb,var(--color-background)_80%,transparent)]">
                                                    ...
                                                    {(workflows || []).map((wf) => (
                                                        <button key={`${slot.label}-w-${wf}`} onClick={() => on_skill_trigger?.(agent.id, wf, (idx + 1) as 1 | 2 | 3)}
                                                            className="w-full text-left px-4 py-2 text-xs text-zinc-300 hover:bg-[color:var(--color-surface)] border-l-2 border-transparent hover:border-amber-500 hover:text-white transition-all flex items-center gap-2 group font-mono">

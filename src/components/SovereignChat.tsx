...
import { resolve_alpha } from '../utils/agent_hierarchy';
...
export const SovereignChat: React.FC<SovereignChatProps> = ({ isDetachedView }) => {
    ...
    const {
        ...
        toggle_voice,
        toggle_speech,
    } = useChatVoice(messages, selected_agent_id, agents, set_input_text);
    ...
    useEffect(() => {
        ...
        if (agents.length > 0 && !selected_agent_id && is_ungetTarget) {
            const alpha = resolve_alpha(agents);
            if (alpha) {
                set_target_agent(alpha.name);
                set_selected_agent_id(alpha.id);
            }
        }

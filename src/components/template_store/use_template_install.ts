...
export function extractMcpPlaceholders(mcps: Record<string, unknown> | null | undefined): McpPlaceholderVariable[] {
    if (!mcps || typeof mcps !== 'object') return [];
    const placeholders: McpPlaceholderVariable[] = [];
    const seen = new Set<string>();

    const servers = (mcps.mcpServers || mcps.mcp_servers || mcps) as Record<string, unknown>;
    for (const [serverName, serverCfg] of Object.entries(servers)) {
        if (serverCfg && typeof serverCfg === 'object' && serverName !== 'mcpServers' && serverName !== 'mcp_servers') {
            const str = JSON.stringify(serverCfg);
    ...
    const install = useCallback(async (
        ...
        try {
            ...
            try {
                ...
                const department = (template.industry || 'Operations') as any;

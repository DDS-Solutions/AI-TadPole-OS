 ...
 * - **Subsystem**: UI Components / General / Command_Table
 * - **Primary Entrypoints**: `Command_Table`
 *
...
import { calculate_utilization, format_cost_usd } from '../utils/command_table_utils';

/**
 * Command_Table
 * High-density tabular view of the swarm's neural footprint.
 * Refined with glassmorphic depth, neural-grid overlays, and premium transitions.
 */
    ...
    const sorted_agents = React.useMemo(() => {
        return [...(agents || [])].sort((a, b) => {
            ...
            if (sort_key === 'cost') {
                ...
                val_a = calculate_utilization(a.cost_usd || 0, a.budget_usd ?? 1);
                val_b = calculate_utilization(b.cost_usd || 0, b.budget_usd ?? 1);
            } else {
                val_a = a.name;
                val_b = b.name;
            }
    ...
    return (
        ...
        >
                ...
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-zinc-800/20">
                        {(sorted_agents || []).map((agent): React.ReactElement => {
                            ...
                            const util = calculate_utilization(cost_usd, budget_usd);
                                ...
                                <tr key={agent.id} className="hover:bg-zinc-800/20 transition-all duration-300 group/row">
                                    <td className="p-5">
                                        <div className="flex items-center gap-4 min-w-0">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <div className="font-bold text-sm text-zinc-100 group-hover/row:text-white transition-colors truncate max-w-[180px] md:max-w-[220px]" title={agent.name}>
                                                    {agent.name}
                                                </div>
                                                <div className="text-[10px] text-zinc-600 font-bold uppercase tracking-wider font-mono truncate">
                                    ...
                                    <td className="p-5">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-xs font-mono text-zinc-400 uppercase tracking-tighter truncate max-w-[120px]">
                                                {agent.department}
                                            </span>
                                        </div>
                                    </td>
            ...
            <div className="bg-[color:color-mix(in_srgb,var(--color-background)_80%,transparent)] px-4 py-2 border-t border-[color:var(--color-border)]/50 flex items-center justify-between text-[9px] font-mono text-zinc-500 uppercase tracking-widest backdrop-blur-sm">
                ...
                <div className="flex gap-4">
                    <span>{i18n.t('command.version', { ver: pkg.version })}</span>

 ...
 * - **Subsystem**: UI Components / General / Swarm_Telemetry
 * - **Primary Entrypoints**: `Swarm_Telemetry`
 *
...
const TelemetryCard = ({ label, value, subtext, icon: Icon, status, colorClass, tooltip }: TelemetryCardProps) => {
    ...
    return (
        <Tooltip content={tooltip} position="top" class_name="w-full">
            <div className={`p-4 rounded-2xl border bg-[color:var(--color-surface)]/40 backdrop-blur-md transition-all duration-500 hover:bg-[color:var(--color-surface)]/60 ${statusGlow} group relative overflow-hidden h-full cursor-help`}>
...
export const Swarm_Telemetry = ({ agent_count, max_depth, tpm, recruit_count, max_density }: SwarmTelemetryProps) => {
    // Prevent division by zero if backend initializes with zero density
    const safe_max_density = max_density || 1;
    const densityVal = Math.round((agent_count / safe_max_density) * 100);
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full">
            <TelemetryCard
                ...
                value={`${densityVal}%`}
                subtext={i18n.t('telemetry.density_subtext', { active: agent_count, max: max_density })}
                icon={Users}

 ...
 * - **Subsystem**: UI Components / Ui / Empty_State
 * - **Primary Entrypoints**: `Empty_State`, `Tw_Empty_State`
 *
...
export const Empty_State: React.FC<EmptyStateProps> = ({
    ...
    if (variant === 'dashed') {
        return (
            <div className={`col-span-full flex flex-col items-center justify-center py-16 px-6 border border-dashed border-[color:var(--color-border)] rounded-3xl text-zinc-600 gap-4 bg-[color:var(--color-background)]/20 animate-in fade-in zoom-in-95 duration-500 ${class_name}`}>
                {icon && (
                    <div className="p-4 bg-[color:var(--color-surface)] rounded-full border border-[color:var(--color-border)]/50 mb-2 opacity-50">
                        {typeof icon === 'string' ? <span role="img" aria-label={i18n.t('common.icon_aria') || 'icon'}>{icon}</span> : icon}
                    </div>
                )}
    ...
    return (
        <div className={`flex flex-col items-center justify-center py-12 px-6 text-center animate-in fade-in duration-500 ${class_name}`}>
            <div className="text-4xl mb-4 opacity-50 grayscale hover:grayscale-0 transition-all duration-500 transform hover:scale-110">
                {typeof icon === 'string' ? (
                    <span role="img" aria-label={i18n.t('common.icon_aria') || 'icon'}>{icon}</span>
                ) : (
                    icon
                )}
            </div>

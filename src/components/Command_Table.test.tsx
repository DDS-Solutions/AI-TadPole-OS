 ...
 * - **Subsystem**: UI Components / General / Command_Table.test
 *
...
describe('Command_Table Component', () => {
    ...
    it('calculates utilization with zero-budget safety', () => {
        expect(calculate_utilization(0.05, 1.0)).toBe(5.0);
        expect(calculate_utilization(5.0, 0)).toBe(100);
        expect(calculate_utilization(0, 0)).toBe(0);
    });

    it('formats sub-cent costs with micro-precision', () => {
        expect(format_cost_usd(0.000025)).toBe('2.50e-5');
        expect(format_cost_usd(1.2345)).toBe('1.2345');
    });
});

 ...
 * - **Subsystem**: UI Components / Skills / Workflow_Card
 * - **Primary Entrypoints**: `Workflow_Card`
 *
...
export const Workflow_Card: React.FC<Workflow_Card_Props> = ({ workflow, on_edit, on_assign, on_delete }) => {
    return (
        <div className="bg-[color:var(--color-background)] border border-[color:var(--color-border)] p-5 rounded-xl transition-all duration-300 hover:border-amber-500/30 hover:shadow-[0_0_15px_rgba(245,158,11,0.15)] group relative overflow-hidden shadow-sm">
            ...
            <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                <Tooltip content={i18n.t('skills.tooltip_edit_workflow')} position="top">
                    <button onClick={() => on_edit(workflow)} className="text-zinc-500 hover:text-green-400 bg-[color:var(--color-surface)] hover:bg-zinc-800 p-1.5 rounded">
                        <Edit2 className="w-3.5 h-3.5" />
                    </button>
                </Tooltip>
                <Tooltip content={i18n.t('agent_manager.tooltip_assign')} position="top">
                    <button onClick={() => on_assign(workflow.name)} className="text-zinc-500 hover:text-emerald-400 bg-[color:var(--color-surface)] hover:bg-zinc-800 p-1.5 rounded transition-colors">
                        <Users className="w-3.5 h-3.5" />
                    </button>
                </Tooltip>
                <Tooltip content={i18n.t('skills.tooltip_delete_workflow')} position="top">
                    <button onClick={() => on_delete(workflow.name)} className="text-zinc-500 hover:text-red-400 bg-[color:var(--color-surface)] hover:bg-zinc-800 p-1.5 rounded">
            ...
            <div className="relative z-10 flex flex-col h-full">
                ...
                <div className="bg-zinc-950/40 border border-[color:var(--color-border)]/50 p-3 rounded text-[11px] text-zinc-400 font-mono whitespace-pre-wrap max-h-64 overflow-y-auto custom-scrollbar flex-1">

 ...
 * - **Subsystem**: UI Components / Skills / Mcp_Tool_Card
 * - **Primary Entrypoints**: `Mcp_Tool_Card`
 *
...
export const Mcp_Tool_Card: React.FC<Mcp_Tool_Card_Props> = ({ tool, on_edit }) => {
    ...
    return (
        <div 
            className={`bg-[color:var(--color-surface)] border ${tool.is_pulsing ? 'border-green-500 shadow-lg shadow-green-500/20' : 'border-[color:var(--color-border)]'} rounded-2xl overflow-hidden transition-all duration-300 hover:border-zinc-700 group flex flex-col`}
        ...
        >
            <div className="p-5 flex-1 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3 pt-2">
                    <div className="bg-[color:color-mix(in_srgb,var(--color-background)_50%,transparent)] rounded-xl p-2.5 border border-[color:var(--color-border)]/50 space-y-1">
            ...
            <div className="p-3 bg-[color:color-mix(in_srgb,var(--color-background)_50%,transparent)] border-t border-[color:var(--color-border)]/50 flex gap-2">
                <button
                    onClick={() => on_edit(tool)}
                    className="flex-1 px-4 py-2 bg-[color:var(--color-surface)] hover:bg-zinc-800 text-zinc-300 text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-2 border border-[color:var(--color-border)]"
                >
                    <Info size={14} />

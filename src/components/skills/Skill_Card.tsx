        ...
        <div className="bg-[color:var(--color-background)] border border-[color:var(--color-border)] p-5 rounded-xl transition-all duration-300 hover:border-emerald-500/30 hover:shadow-[0_0_15px_rgba(16,185,129,0.15)] group relative overflow-hidden shadow-sm">
            ...
            <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity z-20">
                <Tooltip content={i18n.t('skills.tooltip_edit_skill')} position="top">
                    <button onClick={() => on_edit(skill)} className="text-zinc-500 hover:text-green-400 bg-[color:var(--color-surface)] hover:bg-zinc-800 p-1.5 rounded">
                        <Edit2 className="w-3.5 h-3.5" />
                    </button>
                </Tooltip>
                <Tooltip content={i18n.t('agent_manager.tooltip_assign')} position="top">
                    <button onClick={() => on_assign(skill.name)} className="text-zinc-500 hover:text-emerald-400 bg-[color:var(--color-surface)] hover:bg-zinc-800 p-1.5 rounded transition-colors">
                        <Users className="w-3.5 h-3.5" />
                    </button>
                </Tooltip>
                <Tooltip content={i18n.t('skills.tooltip_delete_skill')} position="top">
                    <button onClick={() => on_delete(skill.name)} className="text-zinc-500 hover:text-red-400 bg-[color:var(--color-surface)] hover:bg-zinc-800 p-1.5 rounded">
            ...
            <div className="relative z-10">
                ...
                <div className="bg-black/40 border border-[color:var(--color-border)]/50 p-2.5 rounded font-mono text-[10px] text-zinc-300 flex items-center gap-2 overflow-x-auto">

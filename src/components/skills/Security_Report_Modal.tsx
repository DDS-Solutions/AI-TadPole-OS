 ...
 * 
 * ### AI Assist Note
 * **UI Component**: Security audit report viewer for agent skills.
 * Displays details of SkillSpector vulnerability scans including risk scores,
 * rule violations, location in files, and recommendations.
 * 
 * ### 🔍 Debugging & Observability
 * - **Failure Path**: Missing or corrupt JSON report payloads from the backend.
 * - **Telemetry Link**: Search for `[Security_Report_Modal]` in logs.
 */
        ...
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
            <div className="bg-[color:var(--color-background)] border border-[color:var(--color-border)] w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh] relative overflow-hidden">
                ...
                <div className="p-5 border-b border-[color:var(--color-border)] flex justify-between items-center shrink-0 relative z-10 bg-[color:color-mix(in_srgb,var(--color-background)_50%,transparent)]">
                    ...
                    <button onClick={on_close} className="text-zinc-500 hover:text-zinc-300 p-1 cursor-pointer transition-colors">
                        <X className="w-4 h-4" />
                    </button>
                </div>
                ...
                <div className="p-6 overflow-y-auto space-y-6 custom-scrollbar relative z-10 bg-[color:color-mix(in_srgb,var(--color-background)_80%,transparent)]">
                    ...
                    <div>
                        ...
                        {findings.length === 0 ? (
                            <div className="border border-[color:var(--color-border)]/50 rounded-xl p-6 text-center bg-black/20 flex flex-col items-center justify-center">
                            ...
                            <div className="space-y-4">
                                {findings.map((finding, idx) => {
                                    ...
                                    return (
                                        <div key={idx} className="border border-[color:var(--color-border)] rounded-xl overflow-hidden bg-black/10">
                                            {/* Finding header */}
...
// Metadata: [Security_Report_Modal]

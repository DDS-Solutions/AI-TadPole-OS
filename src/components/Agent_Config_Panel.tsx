...
import type { Skill_State, Skill_Actions } from '../stores/skill_store';
 ...
 */
export default function Agent_Config_Panel({ agent, on_close, on_update, is_new = false, is_detached_mode = false }: Agent_Config_Panel_Props) {
    const {
        state,
        dispatch,
        handle_role_change,
        ...
        handle_send_message,
        handle_promote
    } = useAgentConfig(agent, on_update, on_close);
    ...
    const manifests = use_skill_store((s: Skill_State) => s.manifests);
    const scripts = use_skill_store((s: Skill_State) => s.scripts);
    const workflows = use_skill_store((s: Skill_State) => s.workflows);
    const mcp_tools = use_skill_store((s: Skill_State) => s.mcp_tools);
    const fetch_skills = use_skill_store((s: Skill_Actions) => s.fetch_skills);
    const fetch_mcp_tools = use_skill_store((s: Skill_Actions) => s.fetch_mcp_tools);
    const is_loading = use_skill_store((s: Skill_State) => s.is_loading);

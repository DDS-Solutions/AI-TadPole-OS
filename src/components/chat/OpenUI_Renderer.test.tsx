...
describe('OpenUI_Renderer', () => {
    ...
    it('halts recursion when layout exceeds maximum depth limit', () => {
        let nested: OpenUI_Layout = {
            kind: 'layout',
            direction: 'column',
            children: [{ kind: 'kpi_card', title: 'Deep Leaf', value: 999 }]
        };
        for (let i = 0; i < 12; i++) {
            nested = {
                kind: 'layout',
        ...
        render(<OpenUI_Renderer dsl={nested} />);
        expect(screen.getByText(/Maximum layout depth exceeded/)).toBeTruthy();

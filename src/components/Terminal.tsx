...
export default function Terminal_Component({ agents }: Terminal_Props) {
    ...
    const [log_buffer, set_log_buffer] = useState<log_entry[]>([]);
    ...
    const update_buffer_ref = useRef<log_entry[]>([]);
    ...
    useEffect(() => {
        ...
        const unsubscribe = event_bus.subscribe_logs((entry) => {
            update_buffer_ref.current.push(entry);
        });

        // Flush buffer to React state on each animation frame
        let raf_id: number;
        const flush = () => {
            if (update_buffer_ref.current.length > 0) {
                const batch = update_buffer_ref.current;
                update_buffer_ref.current = [];
                set_log_buffer((prev) => {
    ...
    const handle_key_down = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === 'Tab') {
            ...
            const agent_names = agents.map(a => a.name).filter(Boolean) as string[];

 ...
 * - **Subsystem**: UI Components / Scheduler / Job_History_View
 * - **Primary Entrypoints**: `Job_History_View`
 *
...
export const Job_History_View: React.FC<JobHistoryViewProps> = ({ job, runs }) => {
    ...
    return (
        <div className="p-6 ml-12 border-l-2 border-[color:var(--color-border)] my-4 space-y-4 max-w-full overflow-hidden">
            <div className="bg-zinc-950/20 rounded p-4 font-mono text-xs text-zinc-400 border border-[color:var(--color-surface)]/50 break-words whitespace-pre-wrap overflow-hidden max-w-full">
                    ...
                    {runs.map(run => {
                        ...
                        return (
                            <div key={run.id} className="rounded border border-[color:var(--color-border)] overflow-hidden max-w-full">
                                ...
                                {run.output_summary && (
                                    <div className="p-3 bg-zinc-950/40 border-t border-[color:var(--color-border)]/50 text-[11px] font-mono text-zinc-300 break-words whitespace-pre-wrap overflow-x-auto max-w-full leading-relaxed">

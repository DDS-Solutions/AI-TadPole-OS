            ...
            {!runs ? (
                ...
                <div className="space-y-2">
                    {runs.map(run => {
                        ...
                        return (
                            <div key={run.id} className="rounded border border-[color:var(--color-border)] overflow-hidden">
                                <div 
                                    onClick={() => toggle_run(run)}
                                    className={`flex items-center justify-between bg-[color:var(--color-background)] p-3 text-xs font-mono group transition-colors ${
                                    ...
                                    <div className="flex items-center gap-4">
                                        {can_expand && (
                                            <ChevronRight 
                                                size={14} 
                                                strokeWidth={1.5}
                                                className={`text-zinc-500 transform transition-transform ${is_expanded ? 'rotate-90' : ''}`} 
                                            />
                                        )}
                                        {run.status === 'completed' ? <CheckCircle2 size={14} strokeWidth={1.5} className="text-emerald-500" /> : <AlertCircle size={14} strokeWidth={1.5} className="text-rose-500" />}

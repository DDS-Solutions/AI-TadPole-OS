 ...
 * - **Subsystem**: UI Components / Scheduler / Job_Table_Row
 * - **Primary Entrypoints**: `Job_Table_Row`
 *
...
import { ChevronRight, Activity, Pause, Play, Edit2, Trash2, Rocket } from 'lucide-react';
...
export const Job_Table_Row: React.FC<JobTableRowProps> = ({
    ...
    const [is_triggering, set_is_triggering] = React.useState(false);

    const handle_trigger = async (e: React.MouseEvent) => {
        e.stopPropagation();
        set_is_triggering(true);
        try {
            await tadpole_os_service.trigger_scheduled_job(job.id);
            event_bus.emit_log({
                source: 'System',
                text: i18n.t('scheduled_jobs.event_triggered_ok', { name: job.name }),
                severity: 'success'
            });
        } catch (error) {
            ...
            event_bus.emit_log({
                source: 'System',
                text: i18n.t('scheduled_jobs.event_triggered_err', { name: job.name }),
                severity: 'error'
            });
        } finally {
            ...
            <tr className={`group hover:bg-[color:var(--color-surface)]/50 transition-colors ${is_expanded ? 'bg-[color:var(--color-surface)]/50' : ''}`}>
                <td className="px-6 py-4 w-10">
                    <button onClick={() => toggle_expand(job.id)} className="text-zinc-500 hover:text-white transition-colors">
                        <ChevronRight size={16} strokeWidth={1.5} className={`transform transition-transform ${is_expanded ? 'rotate-90' : ''}`} />
                    </button>
                </td>
                        ...
                        {is_workflow ? (
                            <>
                                <div className="w-5 h-5 rounded overflow-hidden bg-green-500/20 flex items-center justify-center border border-green-500/30">
                                    <Activity size={10} strokeWidth={1.5} className="text-green-400" />
                                </div>
                ...
                <td className="px-6 py-4 whitespace-nowrap space-x-2 text-right">
                    <Tooltip content={i18n.t('scheduled_jobs.trigger_execution')} position="top">
                        <button 
                             onClick={handle_trigger} 
                             aria-label={i18n.t('scheduled_jobs.trigger_execution', { defaultValue: 'Trigger execution' })}
                             disabled={is_triggering || !job.enabled}
                             className={`p-1.5 rounded bg-zinc-800 hover:bg-zinc-700 transition-colors ${!job.enabled ? 'opacity-50 cursor-not-allowed' : 'text-amber-500 hover:text-amber-400'}`}
                        >
                            <Rocket size={14} strokeWidth={1.5} className={is_triggering ? 'animate-bounce' : ''} />
                        </button>
                    </Tooltip>
                    <Tooltip content={job.enabled ? i18n.t('scheduled_jobs.pause_execution') : i18n.t('scheduled_jobs.resume_execution')} position="top">
                        <button onClick={() => toggle_enable(job)} className="p-1.5 text-zinc-500 hover:text-white rounded bg-zinc-800 hover:bg-zinc-700 transition-colors">
                            {job.enabled ? <Pause size={14} strokeWidth={1.5} /> : <Play size={14} strokeWidth={1.5} />}
                        </button>
                    </Tooltip>
                    ...
                    <Tooltip content={i18n.t('scheduled_jobs.purge_job_tooltip')} position="top">
                            ...
                            >
                                <Trash2 size={16} strokeWidth={1.5} className="pointer-events-none" />
                            </button>
                        </Tooltip>
                </td>
            </tr>

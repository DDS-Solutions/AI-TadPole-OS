 ...
 * - **Subsystem**: UI Components / General / AgentConfigPanel.test
 *
...
describe('AgentConfigPanel', () => {
    ...
    it('allows configuring shadows_human_id under governance tab', async () => {
        render(<AgentConfigPanel agent={mock_agent} onUpdate={mock_on_update} onClose={mock_on_close} />);
        
        // Toggle to governance tab
        const governance_tab = screen.getByText('agent_config.tab_governance');
        fireEvent.click(governance_tab);

        // Fill Shadows Real Person input
        const shadow_input = screen.getByLabelText('Shadows Real Person');
        fireEvent.change(shadow_input, { target: { value: 'usr_human_admin_01' } });

        // Save config
        const save_button = screen.getByText('agent_config.btn_save_config');
        fireEvent.click(save_button);

        await waitFor(() => {
            expect(mock_on_update).toHaveBeenCalledWith('agent-1', expect.objectContaining({
                shadows_human_id: 'usr_human_admin_01'
            }));
        });
    });
});

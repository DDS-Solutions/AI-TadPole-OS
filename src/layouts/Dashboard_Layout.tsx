...
import { Page_Header, Portal_Window, Connection_Banner } from '../components/ui';
...
export default function Dashboard_Layout() {
    ...
    const [connection_state, set_connection_state] = useState<Connection_State>('disconnected');
    ...
    useEffect(() => {
        ...
        const unsubscribe_status = tadpole_os_socket.subscribe_status((state) => {
            set_connection_state(state as Connection_State);
        });
    ...
    const nav_item_class = ({ isActive }: { isActive: boolean }) => clsx(
    ...
    return (
        <div className="flex h-screen bg-zinc-950 text-zinc-100 overflow-hidden font-sans antialiased selection:bg-zinc-700/30">
            <Sidebar navItemClass={nav_item_class} />

            <main className="flex-1 flex flex-col bg-zinc-950 relative overflow-hidden">
                <Connection_Banner state={connection_state} />
                <Page_Header

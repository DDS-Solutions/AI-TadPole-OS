 ...
 * - **Subsystem**: System Core / Dashboard_Route_Smoke.test
 *
...
function MockBenchmarkAnalytics() {
    const location = useLocation();
    const test_id = `route-${location.pathname.replace(/^\//, '').replace(/\//g, '-')}`;
    return <div data-testid={test_id} />;
}

vi.mock('../pages/Benchmark_Analytics', () => ({
    default: MockBenchmarkAnalytics
}));
...
vi.mock('../pages/MCP_Store', () => ({ default: () => <div data-testid="route-mcp-store" /> }));
vi.mock('../pages/Security_Dashboard', () => ({ default: () => <div data-testid="route-security" /> }));
vi.mock('../pages/Neural_Map', () => ({ default: () => <div data-testid="route-intelligence-map" /> }));

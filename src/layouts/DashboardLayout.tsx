...
import { PageHeader, PortalWindow } from '../components/ui';
...
export default function DashboardLayout() {
    ...
    const { tabs, activeTabId, isSystemLogDetached, toggleSystemLogDetachment } = useTabStore();
    ...
    useEffect(() => {
        ...
        const unsubscribePulse = TadpoleOSSocket.subscribePulse((pulse) => {
            useCapabilitiesStore.getState().handlePulse(pulse.tool, pulse.status, pulse.latency);
        });
        return () => {
            unsubscribeStatus();
            unsubscribeHealth();
        ...
        <div className="flex h-screen bg-zinc-950 text-zinc-100 overflow-hidden font-sans antialiased selection:bg-zinc-700/30">
            ...
            <main className="flex-1 flex flex-col bg-zinc-950 relative overflow-hidden">
                ...
                <div className="flex-1 flex overflow-hidden">
                            ...
                            if (isDetached) {
                                return (
                                    <React.Fragment key={tab.id}>
                                        ...
                                        >
                                            <div className="h-screen overflow-auto bg-zinc-950 p-6 custom-scrollbar">
                                                {content}
                                            </div>
                                        </PortalWindow>
                    ...
                    <div className="flex flex-col relative" style={{ width: '400px' }}>
                        <LineageStream />
                        <NeuralWaterfall />
                    </div>

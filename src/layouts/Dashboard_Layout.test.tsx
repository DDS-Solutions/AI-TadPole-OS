 ...
 * - **Subsystem**: System Core / Dashboard_Layout.test
 *
...
vi.mock('../services/socket', () => ({
    get_tadpole_os_socket: () => ({
        ...
        subscribe_agent_updates: vi.fn().mockReturnValue(() => {}),
    }),
...
describe('Dashboard_Layout', () => {
    ...
    it('handles tab detachment placeholders and keeps observability sidebar outside portal window', () => {
        vi.mocked(use_tab_store).mockReturnValue({
            ...mock_tab_store,
        ...
        // The portal window (detached window) should exist
        const portal_window = screen.getByTestId('portal-window');
        expect(portal_window).toBeDefined();
        
        // Observability sidebar (Neural Waterfall & OTEL cards) should NOT be inside portal window
        expect(portal_window.querySelector('[data-testid="observability-sidebar"]')).toBeNull();
    });

 ...
 * - **Subsystem**: System Core / routes
 * - **Primary Entrypoints**: `APP_ROUTES`, `get_route_by_path`, `Route_Config`
 *
...
const MCP_Store = lazy(() => import('../pages/MCP_Store'));
...
const Neural_Map = lazy(() => import('../pages/Neural_Map'));
const Customer_Catalog_Manager = lazy(() => import('../pages/Customer_Catalog_Manager').then(m => ({ default: m.Customer_Catalog_Manager })));
...
export const APP_ROUTES: Route_Config[] = [
  ...
  { path: '/customer-catalog', component: Customer_Catalog_Manager, label: 'Customer Catalog', icon: 'Store' },
  ...
  { path: '/analytics', component: Benchmark_Analytics, label: 'Analytics', icon: 'LineChart' },
  ...
  { path: '/mcp-store', component: MCP_Store, label: 'MCP Connectors', icon: 'Server' },
  { path: '/security', component: Security_Dashboard, label: 'Security', icon: 'Lock' },
  { path: '/intelligence/map', component: Neural_Map, label: 'Neural Map', icon: 'Network' },
];
...
export const get_route_by_path = (path: string) => {
  const clean_path = (path || '').split('?')[0].split('#')[0].trim();
  const normalized = !clean_path || clean_path === '/' ? '/dashboard' : clean_path.replace(/\/$/, '');
  const resolved_path = ROUTE_ALIASES[normalized] || normalized;
  return APP_ROUTES.find(r => r.path === resolved_path) || APP_ROUTES[0];
};

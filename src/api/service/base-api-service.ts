 ...
 * - **Subsystem**: System Core / base-api-service
 * - **Primary Entrypoints**: `BaseApiService`
 *
...
import { is_allowed_origin, is_private_network_origin } from '@/services/socket';
...
export function normalize_headers(headers?: HeadersInit): Record<string, string> {
    if (!headers) return {};
...
export class BaseApiService {
    ...
    public async request<T = unknown>(
        ...
        const allow_private = is_private_network_origin(new URL(base_url).hostname);
        if (!is_allowed_origin(base_url, undefined, allow_private)) {
        ...
        const custom_headers = normalize_headers(options.headers);
        const custom_request_id = custom_headers['X-Request-Id'] || custom_headers['x-request-id'];
        ...
        const all_headers = { ...final_headers, ...custom_headers };
            ...
            const execute_fetch = async (attempt: number): Promise<Response> => {
                ...
                try {
                    ...
                    if (is_timeout && is_retryable && attempt < MAX_RETRIES) {
                        ...
                        await delay_with_signal(backoff, options.signal || undefined, setTimeoutFn, clearTimeoutFn);
            ...
            if (response.status === 204) {
                ...
                if (!text || text.trim() === '') {
                    ...
                    try {
                        ...
                        const sanitized_detail = sanitize_error_detail((parse_err as Error).message);
                        telemetryPort.updateSpan(context.span_id, {
                            end_time: Date.now(),
                            status: 'error',
                            attributes: build_trace_attributes(
                                response,
                                { 'error.code': 'MALFORMED_JSON_RESPONSE', 'error.detail': sanitized_detail }
                            )
                        });
                        const message = `Malformed Response: Failed to parse JSON response from ${clean_path} (${sanitized_detail})`;
                        const base_error = new ApiError(message, 'urn:tadpole:error:malformed_json', response.status, 'MALFORMED_JSON_RESPONSE', null);
                        const mapped_error = map_api_error_to_subclass(base_error);
                        this.emit_api_error(mapped_error);
                        throw mapped_error;
                    }

...
describe('BaseApiService & normalize_headers', () => {
    ...
    describe('BaseApiService request execution', () => {
        ...
        it('safely wraps malformed JSON on 200 response into typed ApiError with error span', async () => {
            mockFetch.mockResolvedValueOnce(
                new Response('<html><body>502 Bad Gateway from upstream proxy</body></html>', {
                    status: 200,
                    headers: { 'Content-Type': 'application/json' }
                })
            );
            ...
            service.subscribe_api_errors(errorListener);

            await expect(service.request('/api/corrupt')).rejects.toThrow(ApiError);
        ...
        it('handles empty string body on 200 OK by returning null without throwing', async () => {
            mockFetch.mockResolvedValueOnce(new Response('', { status: 200 }));

            const result = await service.request('/api/empty-string');
            expect(result).toBeNull();
        });
    });
});

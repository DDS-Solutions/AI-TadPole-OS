...
export const agent_api_service = {
     ...
     */
    get_agents: async (options: RequestInit = {}): Promise<Tadpole_OS_Agent[]> => {
    ...
    update_agent: async (agent_id: string, config: Agent_Config): Promise<boolean> => {
        return track_operation('AgentAPI', `Updating configuration for agent: ${agent_id.toUpperCase()}`, async () => {
            ...
            // ... (rest of the body assignment logic)
            ...
            if (config.model_config2 !== undefined) {
                body.model_2 = config.model_config2.model_id;
                body.model_config2 = config.model_config2;
            }
            if (config.model_config3 !== undefined) {
                body.model_3 = config.model_config3.model_id;
    ...
    create_agent: async (agent: Agent): Promise<boolean> => {
        const body = {
            ...
            model_2: agent.model_2,
            model_3: agent.model_3,

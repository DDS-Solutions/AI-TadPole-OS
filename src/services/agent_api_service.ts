...
let agents_fetch_controller: AbortController | null = null;

export const agent_api_service = {
    /**
     * Fetches agents from TadpoleOS. Supporting Maturity Level 3 HATEOAS envelopes.
     * Handles both direct arrays and object-wrapped 'data' envelopes.
     * Implements request deduplication to prevent polling overlap.
     */
    get_agents: async (options: RequestInit = {}): Promise<AgentDto[]> => {
        // 🛡️ Deduplication: Abort pending request before starting a new one
        if (agents_fetch_controller) {
            agents_fetch_controller.abort();
        }
        const current_controller = new AbortController();
        agents_fetch_controller = current_controller;
        ...
        return track_operation('AgentAPI', 'Fetching agent registry...', async () => {
            try {
                ...
                if (agents_fetch_controller === current_controller) {
                    agents_fetch_controller = null;
                }
            }
        });
    },

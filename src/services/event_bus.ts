...
class event_bus_service {
    ...
    private channel: BroadcastChannel | null = typeof window !== 'undefined' ? new BroadcastChannel('tadpole-neural-hub') : null;
    private trace_listeners: ((span: unknown) => void)[] = [];
    private pulse_listeners: ((pulse: unknown) => void)[] = [];
    private instance_id = Math.random().toString(36).substring(2, 9);
    ...
    private handle_sync_request(): void {
        ...
        if (this.count === 0 && Object.keys(this.processed_ids).length === 0) return;

        if (this.channel) {
            this.channel.postMessage({
                topic: 'SYNC_RESPONSE',
                payload: {
                    logs: this.get_history(),
                    // Note: Traces are in the trace_store, we'd need a cleaner way to pull them
                    // but for now we focus on logs. Traces will follow same pattern if needed.
                },
                timestamp: Date.now(),
                sender_id: this.instance_id
            } as telemetry_message);
    ...
    private handle_sync_response(payload: unknown): void {
        ...
        if (p.logs && Array.isArray(p.logs)) {
            p.logs.forEach((log: log_entry) => this.internal_emit(log, false));
        }
    }
    ...
    emit_log(entry: Omit<log_entry, 'id' | 'timestamp'> & { id?: string; timestamp?: Date }): void {
        ...
        const metadata = entry.metadata ? this.truncate_large_metadata(entry.metadata) : undefined;
     ...
     */
    private truncate_large_metadata(metadata: Record<string, unknown>, depth = 0): Record<string, unknown> {
        if (depth > 3) return { _status: '[MAX_DEPTH_REACHED]' }; // Prevent stack overflow
        const result: Record<string, unknown> = {};
        const MAX_STRING_LEN = 10000;

        for (const [key, value] of Object.entries(metadata)) {
            if (typeof value === 'string' && value.length > MAX_STRING_LEN) {
                result[key] = value.substring(0, MAX_STRING_LEN) + '... [TRUNCATED]';
            } else if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
                result[key] = this.truncate_large_metadata(value as Record<string, unknown>, depth + 1);
            } else {
                result[key] = value;
            }
        }
    ...
    private internal_emit_trace(span: unknown, broadcast: boolean): void {
        this.trace_listeners.forEach(l => l(span));
        if (broadcast && this.channel) {
            this.channel.postMessage({ 
                topic: 'TRACE', 
                payload: span, 

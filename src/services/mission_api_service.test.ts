...
describe('mission_api_service', () => {
    ...
    it('clone_mission', async () => {
        const mock_mission = { id: 'mission-clone-1', name: 'Cloned' };
        vi.mocked(api_request).mockResolvedValueOnce(mock_mission);
        const res = await mission_api_service.clone_mission('mission/1', 'New goal');
        expect(api_request).toHaveBeenCalledWith(
            '/v1/agents/missions/mission%2F1/clone',
            expect.objectContaining({
                method: 'POST',
                body: JSON.stringify({ primary_goal: 'New goal' })
            })
        );
        expect(res).toEqual(mock_mission);
    });
});

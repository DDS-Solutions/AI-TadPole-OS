...
export const normalize_agent = (raw: Raw_Agent, workspace_path?: string, existing_agent?: Agent): Agent => {
    ...
    const raw_status = raw.status || 'idle';
    const status = raw_status === 'working' ? 'active' : raw_status;
    const has_current_task = Object.prototype.hasOwnProperty.call(raw, 'current_task');
    const current_task = (raw_status === 'active' || raw_status === 'working')
        ? (has_current_task ? (raw.current_task ?? undefined) : existing_agent?.current_task)
        : (has_current_task ? (raw.current_task ?? undefined) : undefined);
...
export const persist_agent_update = async (agent_id: string, updates: Agent_Override) => {
    try {
        ...
        if (updates.input_tokens !== undefined) config.input_tokens = Math.floor(updates.input_tokens);
        if (updates.output_tokens !== undefined) config.output_tokens = Math.floor(updates.output_tokens);
        if (updates.tokens_used !== undefined) config.tokens_used = Math.floor(updates.tokens_used);
        if (updates.current_task !== undefined) config.current_task = updates.current_task;
        if (updates.failure_count !== undefined) config.failure_count = Math.floor(updates.failure_count);

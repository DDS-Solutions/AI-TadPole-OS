 ...
 * - **Subsystem**: Frontend Service Layer / system_api_service.test
 *
...
describe('system_api_service', () => {
    ...
    describe('Phase 2 Configuration and Telemetry', () => {
        ...
        it('should reset failure counter on successful execution in CLOSED state (decaying consecutive failures)', async () => {
            const breakers = get_circuit_breakers();
            const infra_breaker = breakers.infra;
            infra_breaker.force_close();

            // Trigger 3 failures (threshold is 5)
            vi.mocked(api_request).mockRejectedValue(new Error('Intermittent network glitch'));
            for (let i = 0; i < 3; i++) {
                await expect(system_api_service.infra.get_nodes()).rejects.toThrow('Intermittent network glitch');
            }
            expect(infra_breaker.get_failures()).toBe(3);
            expect(infra_breaker.get_state()).toBe('CLOSED');

            // Successful call should reset failure counter
            vi.mocked(api_request).mockResolvedValueOnce([{ id: 'node-ok' }]);
            await system_api_service.infra.get_nodes();
            expect(infra_breaker.get_failures()).toBe(0);
            expect(infra_breaker.get_state()).toBe('CLOSED');
        });

        it('should gate concurrent trial probes in HALF_OPEN state with probeInFlight', async () => {
            const breakers = get_circuit_breakers();
            const infra_breaker = breakers.infra;
            infra_breaker.force_open();
            ...
            const realNow = Date.now;
            Date.now = vi.fn().mockReturnValue(realNow() + 11000);

            // Mock a pending long promise for the first trial probe
            let resolveFirstProbe: (val: unknown) => void;
            const pendingProbe = new Promise((resolve) => {
                resolveFirstProbe = resolve;
            });
            vi.mocked(api_request).mockReturnValueOnce(pendingProbe as any);

            // Launch first probe
            const probe1Promise = system_api_service.infra.get_nodes();

            // Concurrent second probe while first is in flight should be rejected with CircuitBreakerOpenError
            const probe2Promise = system_api_service.infra.get_nodes();
            await expect(probe2Promise).rejects.toThrowError(/trial probe request already in flight/);

            // Settle first probe
            resolveFirstProbe!([{ id: 'node-probe-1' }]);
            await probe1Promise;

    ...
    private async handle_socket_message(data: any) {
        ...
        if (this.cache_stale) {
            ...
            for (const a of agents) {
                this.agent_name_cache.set(String(a.id), a.name);
            }
            this.cache_stale = false;
        }

        if (data.type === 'log' || data.type === 'thought') {
            const agent_name = this.agent_name_cache.get(String(data.agent_id)) || data.agent_name;
            ...
            if (data.agent_id && (data.type === 'thought' || (data.type === 'log' && data.level !== 'debug'))) {
                const resolved_name = this.agent_name_cache.get(String(data.agent_id)) || data.agent_name || data.agent_id;
            ...
            const resolved_name = this.agent_name_cache.get(String(data.agent_id)) || data.agent_name || data.agent_id || 'Agent';

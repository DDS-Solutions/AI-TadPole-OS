...
export class Tadpole_OS_Socket_Client {
    ...
    subscribe(channel: 'agentUpdates' | 'health' | 'handoff' | 'status', listener: (data: any) => void): () => void {
        let unsubscribe_func: () => void;

        switch (channel) {
            case 'agentUpdates':
                unsubscribe_func = this.subscribe_agent_updates(listener);
                break;
            case 'health':
                unsubscribe_func = this.subscribe_health(listener);
                break;
            case 'handoff':
                unsubscribe_func = this.subscribe_handoff(listener);
                break;
            case 'status':
                unsubscribe_func = this.subscribe_status(listener);
                break;
            default:
                unsubscribe_func = () => {};
                break;
    ...
    private raw_listeners: ((event: Record<string, unknown>) => void)[] = [];

    subscribe_raw(listener: (event: Record<string, unknown>) => void): () => void {
        this.raw_listeners.push(listener);
        return () => {
            this.raw_listeners = this.raw_listeners.filter(l => l !== listener);
        };
    }
    ...
    private async handle_socket_message(data: Record<string, unknown>): Promise<void> {
        ...
        this.raw_listeners.forEach(l => l(data));
        ...
        if (data.type === 'log' || data.type === 'thought') {
            ...
            use_trace_store.getState().add_span(data.span as import('../types').Trace_Span);
        } else if (data.type === 'trace:span_update') {
            use_trace_store.getState().update_span((data.span_id || data.spanId) as string, data.update as Partial<import('../types').Trace_Span>);

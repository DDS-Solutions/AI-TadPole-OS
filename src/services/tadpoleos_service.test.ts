 ...
 * - **Subsystem**: Frontend Service Layer / tadpoleos_service.test
 *
...
vi.mock('./intelligence_api_service');

describe('tadpole_os_service Facade', () => {
    beforeEach(() => {
        // No set_agent_api_service needed since agent_api_service is statically imported
    });
    ...
    it('delegates System domain calls correctly', async () => {
        (system_api_service.engine.check_health as any).mockResolvedValue({ status: 'ok' });

        const result = await tadpole_os_service.check_health();
        expect(system_api_service.engine.check_health).toHaveBeenCalled();
    ...
    it('delegates scheduled job operations correctly', async () => {
        (system_api_service.continuity.get_scheduled_jobs as any).mockResolvedValue([]);
        await tadpole_os_service.get_scheduled_jobs();
        expect(system_api_service.continuity.get_scheduled_jobs).toHaveBeenCalled();

        (system_api_service.continuity.create_scheduled_job as any).mockResolvedValue({ success: true });
        await tadpole_os_service.create_scheduled_job({} as any);
        expect(system_api_service.continuity.create_scheduled_job).toHaveBeenCalled();
    });

    it('delegates benchmark operations correctly', async () => {
        (system_api_service.benchmarks.run_benchmark as any).mockResolvedValue({ id: 'bench-1' });
        await tadpole_os_service.run_benchmark('llm');
        expect(system_api_service.benchmarks.run_benchmark).toHaveBeenCalledWith('llm');
    });
    ...
    it('verifies all proxy methods are correctly mapped', async () => {
        const services = { agent_api_service, mission_api_service, system_api_service, intelligence_api_service };
        ...
        Object.keys(intelligence_api_service).forEach(k => mapping[k] = 'intelligence_api_service');

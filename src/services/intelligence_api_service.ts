...
export interface MarkdownMemoryNode {
    id: string;
    title: string;
    path: string;
...
export interface MarkdownMemoryEdge {
    source: string;
    target: string;
    link_type: string;
}

export interface MarkdownMemoryGraphResponse {
    status: string;
    nodes: MarkdownMemoryNode[];
    edges: MarkdownMemoryEdge[];
}
...
export const intelligence_api_service = {
    ...
    /**
     * Fetches the Markdown Multi-Parent DAG of directives and memory links.
     */
    async get_markdown_memory_graph(signal?: AbortSignal): Promise<MarkdownMemoryGraphResponse> {
        return api_request<MarkdownMemoryGraphResponse>('/v1/memory/graph', {
            method: 'GET',
            signal,
        });
    },

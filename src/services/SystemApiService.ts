...
export const SystemApiService = {
     ...
     */
    getMissionQuotas: async (): Promise<{ quotas: QuotaDetails[] }> => {
        return apiRequest('/v1/oversight/security/missions/quotas', { method: 'GET' });
    },

    /**
     * Updates the budget quota for a specific mission cluster.
     */
    updateMissionQuota: async (clusterId: string, budgetUsd: number): Promise<{ status: string }> => {
        return apiRequest(`/v1/oversight/security/missions/${clusterId}/quota`, {
            method: 'PUT',
            body: JSON.stringify({ budgetUsd })
        });
    },
     ...
     */
    updateGovernanceSettings: async (settings: Record<string, unknown>): Promise<unknown> => {
        return apiRequest('/v1/oversight/settings', {
            method: 'PUT',
     ...
     */
    getModelCatalog: async (): Promise<StoreModel[]> => {
        return apiRequest<StoreModel[]>('/v1/infra/model-store/catalog', { method: 'GET' });
    },

    /**
     * Initiates a model pull/download sequence on a specific Bunker node.
     */
    pullModel: async (modelId: string, nodeId: string): Promise<{ status: string }> => {
        return apiRequest<{ status: string }>('/v1/infra/model-store/pull', {
            method: 'POST',
            body: JSON.stringify({ tag: modelId, nodeId: nodeId })
        });
    }
};

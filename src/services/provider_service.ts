 ...
 * - **Subsystem**: Frontend Service Layer / provider_service
 * - **Primary Entrypoints**: `provider_service`
 *
...
class Provider_Service {
    ...
    private deletion_timers = new Set<ReturnType<typeof setTimeout>>();
        ...
        const unsubscribe = use_provider_store.subscribe((state) => {
            ...
            if (current !== this.last_broadcast) {
                ...
                this.debounce_timer = setTimeout(() => {
                    const sanitized_providers = (state.providers || []).map((provider) => {
                        const p = { ...provider };
                        delete p.api_key;
                        return {
                            ...p,
    ...
    public async sync_with_backend(): Promise<void> {
        try {
            const raw_providers = (await system_api_service.infra.get_providers()) || [];
            ...
            let final_providers = [...filtered_providers];
            ...
            backend_providers.forEach((bp) => {
                const existing_idx = final_providers.findIndex(p => p.id === bp.id);
                if (existing_idx === -1 && !store.deleting_ids.has(bp.id)) {
                    final_providers.push(bp);
                    providers_changed = true;
                } else if (existing_idx !== -1) {
                    const existing = final_providers[existing_idx];
                    if (bp.base_url && existing.base_url !== bp.base_url) {
                        final_providers = final_providers.map((p, idx) => idx === existing_idx ? { ...p, base_url: bp.base_url, protocol: bp.protocol } : p);
                        providers_changed = true;
                    }
                }
    ...
    public async set_provider_config(
        ...
        const current_store = use_provider_store.getState();
        const prev_base_urls = { ...current_store.base_urls };
        const prev_providers = [...current_store.providers];

        try {
            if (api_key) {
                // Inside try: a locked vault throws 'Store is locked' — caught below and reverted.
            ...
            await system_api_service.infra.update_provider(id, {
                id,
            ...
            log_error('ProviderService', 'Config Update Failed — Reverting optimistic state', error);
            use_provider_store.setState({
                base_urls: prev_base_urls,
    ...
    public async add_provider(name: string, icon: string): Promise<void> {
        ...
        const prev_providers = [...store.providers];
        ...
        try {
            ...
            log_error('ProviderService', 'Provider Creation Failed — Reverting state', error);
            use_provider_store.setState({ providers: prev_providers });
        }
    }
    ...
    public async delete_provider(id: string): Promise<void> {
        ...
        try {
            await system_api_service.infra.delete_provider(id);
            const timer = setTimeout(() => {
                this.deletion_timers.delete(timer);
                use_provider_store.setState(state => {
                    const new_deleting = new Set(state.deleting_ids);
                    new_deleting.delete(id);
                    associated_model_ids.forEach(mid => new_deleting.delete(mid));
                    return { deleting_ids: new_deleting };
                });
            }, 10000);
            this.deletion_timers.add(timer);
        } catch (error) {
            log_error('ProviderService', 'Deletion Failed — Reverting optimistic state', error);
            use_provider_store.setState(state => {
                const new_deleting = new Set(state.deleting_ids);
                new_deleting.delete(id);
                associated_model_ids.forEach(mid => new_deleting.delete(mid));
    ...
    public destroy(): void {
        this.deletion_timers.forEach(timer => clearTimeout(timer));
        this.deletion_timers.clear();
    ...
    // Telemetry trace checkpoint: [ProviderService]
}

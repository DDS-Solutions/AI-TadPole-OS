 ...
 * - **Primary Entrypoints**: `agent_api_service`, `AgentApiService`
 *
...
export class AgentApiService {
    ...
    constructor(
        ...
        this.get_role_blueprints = this.get_role_blueprints.bind(this);
        this.delete_role_blueprint = this.delete_role_blueprint.bind(this);
        ...
        this.get_swarm_graph = this.get_swarm_graph.bind(this);
    }
    ...
    public delete_agent(agent_id: string): Promise<boolean> {
        return this.registry_service.delete_agent(agent_id);
    }
    ...
    public get_role_blueprints(): Promise<Role[]> {
        return this.governance_service.get_role_blueprints();
    }
    ...
    public delete_role_blueprint(id: string): Promise<boolean> {
        return this.governance_service.delete_role_blueprint(id);
    }
    ...
    /**
     * Fetches the complete relational swarm knowledge graph (agents, missions, relationships)
     */
    async get_swarm_graph(options?: RequestInit): Promise<{
        nodes: Array<{ id: string; label: string; type: string; status: string; metadata?: Record<string, unknown> }>;
        edges: Array<{ id: string; source: string; target: string; relationship_type: string; metadata?: Record<string, unknown> }>;
    }> {
        return this.api_request_fn('/v1/agents/graph', {
            method: 'GET',
            ...options,
        });
    }

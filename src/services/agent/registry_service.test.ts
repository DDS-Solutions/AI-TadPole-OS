...
describe('AgentRegistryService', () => {
    ...
    describe('get_agents', () => {
        ...
        it('rejects if already aborted signal is provided', async () => {
            ...
            expect(mock_api_request).not.toHaveBeenCalled();
        });

        it('does not cancel a shared fetch when its first caller aborts', async () => {
            const controller = new AbortController();
            const agents = [{ id: '1', name: 'Shared agent' }];
            let complete!: (value: typeof agents) => void;
            mock_api_request.mockImplementation((_path: string, options: RequestInit) =>
            ...
            const cancelled = service.get_agents({ signal: controller.signal });
            const other = service.get_agents();
            const rejection = expect(cancelled).rejects.toMatchObject({ name: 'AbortError' });
            controller.abort();
            complete(agents);

            await rejection;
            await expect(other).resolves.toEqual(agents);
            expect(mock_api_request).toHaveBeenCalledTimes(1);
        });

        it('preserves a newer cached request when an invalidated request fails', async () => {
            let fail!: (reason: Error) => void;
            mock_api_request.mockImplementationOnce(() => new Promise((_resolve, reject) => { fail = reject; }));
            const old = service.get_agents();
            const rejection = expect(old).rejects.toThrow('Old request failed');
            service.invalidate_agents_cache();
            mock_api_request.mockResolvedValueOnce([{ id: 'new' }]);
            const current = service.get_agents();
            await current;

            fail(new Error('Old request failed'));
            await rejection;
            expect(service.get_agents()).toBe(current);
            expect(mock_api_request).toHaveBeenCalledTimes(2);
        });

        it('retries after the current cached request fails', async () => {
            mock_api_request.mockRejectedValueOnce(new Error('Network failure'));
            await expect(service.get_agents()).rejects.toThrow('Network failure');
            mock_api_request.mockResolvedValueOnce([{ id: 'recovered' }]);
            await expect(service.get_agents()).resolves.toEqual([{ id: 'recovered' }]);
            expect(mock_api_request).toHaveBeenCalledTimes(2);
        });

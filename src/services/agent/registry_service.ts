 ...
 * - **Subsystem**: Frontend Service Layer / registry_service
 * - **Primary Entrypoints**: `AgentRegistryService`
 *
...
export class AgentRegistryService {
    ...
    public get_agents(options: RequestInit = {}): Promise<AgentDto[]> {
        ...
        // Cancellation belongs to each caller, not to the shared registry request.
        const promise = this.get_agents_internal();
    ...
    private get_agents_internal(): Promise<AgentDto[]> {
        if (this.agents_cache_promise) {
            return this.agents_cache_promise;
        }

        const request = track_operation('AgentAPI', 'Fetching agent registry...', async () => {
            try {
        ...
        this.agents_cache_promise = request;
        void request.catch(() => {
            // An invalidated request may fail after a newer request has been cached.
            if (this.agents_cache_promise === request) this.agents_cache_promise = null;
        });
        return request;
    }
    ...
    public async delete_agent(agent_id: string): Promise<boolean> {
        try {
            await this.api_request_fn(`/v1/agents/${agent_id}`, { method: 'DELETE' });
            this.invalidate_agents_cache();
            return true;
        } catch (err) {
            throw map_api_error(err);

 ...
 *
 * ### AI Assist Note
 * - Fake timers verify bounded reconnects before authentication and recovery after authentication.
 *
 ...
 * - **Witness Tests**: this suite covers retry exhaustion and recovery.
 */
...
describe('WebSocketConnection', () => {
    ...
    it('bounds_retries_before_authentication', () => {
        vi.spyOn(Math, 'random').mockReturnValue(0);
        const conn = new WebSocketConnection(
            () => 'ws://localhost:8000/v1/engine/ws',
            () => 'secret-token',
            new ReconnectionPolicy(1000, 10000, 2)
        );

        conn.connect();
        for (const [attempt, delay] of [1000, 2000].entries()) {
            const ws = created_sockets[attempt];
            ws.onopen?.();
            ws.close(4408);
            expect(conn.get_state()).toBe('reconnecting');

            vi.advanceTimersByTime(delay - 1);
            expect(created_sockets).toHaveLength(attempt + 1);
            vi.advanceTimersByTime(1);
            expect(created_sockets).toHaveLength(attempt + 2);
        }

        created_sockets[2].onopen?.();
        created_sockets[2].close(4408);
        expect(conn.get_state()).toBe('error');
        vi.advanceTimersByTime(30000);
        expect(created_sockets).toHaveLength(3);
        conn.disconnect();
    });

    it('resets_retry_budget_after_authentication', () => {
        vi.spyOn(Math, 'random').mockReturnValue(0);
        const conn = new WebSocketConnection(
            () => 'ws://localhost:8000/v1/engine/ws',
            () => 'secret-token',
            new ReconnectionPolicy(1000, 10000, 1)
        );

        conn.connect();
        created_sockets[0].onopen?.();
        created_sockets[0].close(4408);
        vi.advanceTimersByTime(1000);

        const recovered = created_sockets[1];
        recovered.onopen?.();
        recovered.onmessage?.({ data: JSON.stringify({ type: 'auth_ok' }) });
        expect(conn.get_state()).toBe('connected');
        recovered.close(1006);
        expect(conn.get_state()).toBe('reconnecting');

        vi.advanceTimersByTime(999);
        expect(created_sockets).toHaveLength(2);
        vi.advanceTimersByTime(1);
        expect(created_sockets).toHaveLength(3);
        conn.disconnect();
    });

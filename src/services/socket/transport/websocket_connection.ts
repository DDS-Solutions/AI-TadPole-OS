 ...
 * - **Telemetry Link**: Search `[websocket_connection]` in observability traces.
 */
...
export class WebSocketConnection {
    ...
    private binary_listeners = new Set<(type: 'audio' | 'pulse', payload: any) => void>();
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    public on_binary(cb: (type: 'audio' | 'pulse', payload: any) => void): () => void {
            ...
            ws.onmessage = (event) => {
                ...
                if (event.data instanceof ArrayBuffer) {
                    ...
                    try {
                        ...
                        if (decoded.type === 'audio' || decoded.type === 'pulse') {
                            const binType = decoded.type as 'audio' | 'pulse';
                            this.binary_listeners.forEach(cb => cb(binType, decoded.payload));
                        }
                    } catch (e) {
    ...
    private start_heartbeat(): void {
        ...
        // Check for client-side activity idle timeouts (detect dead link/NAT drop)

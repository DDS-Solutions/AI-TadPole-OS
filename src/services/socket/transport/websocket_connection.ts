 ...
 *
 * ### AI Assist Note
 * - Reset reconnect backoff only once authentication confirms a usable connection.
 *
 * ### ⚠️ Invariants & Non-Negotiables
 * - `[Structural]` Retry resets belong to explicit connections and successful authentication.
 *
...
export class WebSocketConnection {
    ...
    /**
     * Counts consecutive heartbeat intervals without server activity.
     * Reset to 0 on every received message. Incremented each heartbeat tick.
     * When it reaches `max_missed_heartbeats`, the connection is considered idle.
     * This is immune to JS event loop stalls (unlike wall-clock Date.now() checks).
     */
    private missed_heartbeat_count = 0;
    ...
    /** Number of consecutive heartbeat intervals without activity before disconnect. */
    private readonly max_missed_heartbeats: number;
    ...
    constructor(
        ...
        this.max_missed_heartbeats = max_missed_heartbeats;
    ...
    public connect(is_reconnect = false): void {
        ...
        try {
            ...
            ws.onmessage = (event) => {
                if (this.socket !== ws || this.generation !== gen) return;
                // Reset missed heartbeat counter on any server activity
                this.missed_heartbeat_count = 0;
    ...
    private start_heartbeat(): void {
        ...
        // Missed-heartbeat idle check: increments a counter each interval.
        // The counter is reset to 0 on every received message (in onmessage).
        // This is immune to JS event loop stalls — it counts ticks, not wall-clock time.
        this.idle_timer = setInterval(() => {
            this.missed_heartbeat_count++;
            if (this.missed_heartbeat_count >= this.max_missed_heartbeats) {
                console.warn(`[Tadpole_OS] No server activity for ${this.missed_heartbeat_count} heartbeat intervals (~${Math.round(this.missed_heartbeat_count * this.heartbeat_interval_ms / 1000)}s). Reconnecting...`);
                this.socket?.close();
            }
        }, this.heartbeat_interval_ms);
    }
    ...
    private schedule_reconnect(): void {
        if (!this.reconnection_policy.should_retry(this.retry_count)) {
            ...
            event_bus.emit_log({
                source: 'System',
                text: `Tadpole_OS: Connection failed after ${this.retry_count} attempts. Verify URL in Settings. Will retry automatically in 90s.`,
                severity: 'error'
            });
            if (this.cooldown_timer) clearTimeout(this.cooldown_timer);
            this.cooldown_timer = setTimeout(() => {
                if (this.state === 'error' && !this.is_explicitly_closed) {
                    this.retry_count = 0;
                    this.connect(true);
                }
            }, 90_000);
            ...
            if (typeof window !== 'undefined' && typeof window.addEventListener === 'function') {
                window.addEventListener('online', handle_online, { once: true });
            }
            return;
        }

...
function run_origin_check(url_string: string, allowed_origins?: string[], allow_private_network = false): boolean {
    try {
        ...
        // 2. Allow same-origin if running in browser — compare full origin (scheme + hostname + port)
        //    hostname-only comparison allowed token exfiltration to a different port on the same host.
        if (typeof window !== 'undefined' && window.location) {
            const loc = window.location;
            const loc_port = loc.port || (loc.protocol === 'https:' ? '443' : '80');
            const url_scheme = url.protocol.replace(':', '');
            const loc_scheme = loc.protocol.replace(':', '');
            if (
                loc.hostname.toLowerCase() === hostname &&
                loc_scheme === url_scheme &&
                loc_port === port
            ) {
                return true;
            }
        ...
        // 3. Allow private network origins when explicitly enabled (e.g. LAN self-hosting mode)
        //    Default is false — private network access requires opt-in via settings.
 ...
 * allow_private_network defaults to false — explicitly pass true only when the
 * operator has configured a non-loopback engine URL in settings.
 */
export function is_allowed_origin(url_string: string, allowed_origins?: string[], allow_private_network = false): boolean {
    const env_allowed = (typeof import.meta !== 'undefined' && import.meta.env?.VITE_ALLOWED_ORIGINS) || '';
    ...
    // Simple eviction: cap unbounded cache growth (e.g. from dynamic URL params)

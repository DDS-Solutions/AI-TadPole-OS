 ...
 * - **Subsystem**: Frontend Service Layer / socket_manager
 * - **Primary Entrypoints**: `SocketManager`
 *
...
interface SocketChannelPayloads {
    agentUpdates: Agent_Update_Event;
    health: Engine_Health_Event;
    handoff: Handoff_Event;
    status: Connection_State;
    pulse: Mcp_Pulse_Event;
    audio_stream: ArrayBuffer;
    swarm_pulse: Swarm_Pulse;
    raw: Record<string, unknown>;
}

function is_swarm_pulse(value: unknown): value is Swarm_Pulse {
    if (!value || typeof value !== 'object') return false;
    const pulse = value as Partial<Swarm_Pulse>;
    return typeof pulse.timestamp === 'number'
        && Array.isArray(pulse.nodes)
        && Array.isArray(pulse.edges);
}
...
export class SocketManager {
    ...
    private recovery_poll_timeout: ReturnType<typeof setTimeout> | null = null;
    private recovery_poll_delay = 2500;
    ...
    public subscribe<K extends keyof SocketChannelPayloads>(
        channel: K,
        listener: (data: SocketChannelPayloads[K]) => void
    ): () => void {
        ...
        return () => {
            unsubscribe_func();
            this.connection_references = Math.max(0, this.connection_references - 1);
    ...
    public connect(is_reconnect = false): void {
        ...
        if (!this.connection) {
            ...
            this.connection.on_state_change((state) => {
                ...
                if (state === 'error' && this.connection_references > 0) {
                    this.start_recovery_poll();
                } else {
                    this.stop_recovery_poll();
                }
            });
            ...
            this.connection.on_binary((type, payload) => {
                if (type === 'audio' && payload instanceof ArrayBuffer) {
                    this.audio_stream_channel.handle_binary(payload);
                } else if (type === 'pulse' && is_swarm_pulse(payload)) {
    ...
    private start_recovery_poll(): void {
        if (this.recovery_poll_timeout) return;
        this.schedule_recovery_poll();
    }

    private schedule_recovery_poll(): void {
        this.recovery_poll_timeout = setTimeout(async () => {
            if (this.get_connection_state() !== 'error' || this.connection_references <= 0) {
                this.stop_recovery_poll();
                return;
            }
            try {
                const { tadpole_os_service } = await import('../tadpoleos_service');
                const pending = await tadpole_os_service.get_pending_oversight();
                if (Array.isArray(pending)) {
                    for (const item of pending) {
                        if (item && item.skill === 'ask_user_question') {
                            const params = (item.params || {}) as Record<string, unknown>;
                            const question_event: Incoming_Socket_Message = {
                                type: 'agent:user_question_requested',
                                id: item.id,
                                question_id: item.id,
                                agent_id: item.agent_id || 'Agent',
                                question: (params.question || item.description || 'Pending question') as string,
                                options: (params.options || []) as string[],
                                context: (params.context || '') as string,
                                mission_id: item.mission_id
                            };
                            this.channels.forEach(channel => {
                                if (channel.matches(question_event)) {
                                    channel.handle(question_event);
                                }
                            });
                        }
                    }
                }
                this.recovery_poll_delay = Math.min(30000, this.recovery_poll_delay * 2);
            } catch {
                this.recovery_poll_delay = Math.min(30000, this.recovery_poll_delay * 2);
            }
        ...
        }, this.recovery_poll_delay);
    }

    private stop_recovery_poll(): void {
        if (this.recovery_poll_timeout) {
            clearTimeout(this.recovery_poll_timeout);
            this.recovery_poll_timeout = null;
        }
        this.recovery_poll_delay = 2500;
    }

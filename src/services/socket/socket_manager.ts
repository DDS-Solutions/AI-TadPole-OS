...
export class SocketManager {
    ...
    private recovery_poll_timer: ReturnType<typeof setInterval> | null = null;
    ...
    public connect(is_reconnect = false): void {
        ...
        if (!this.connection) {
            ...
            this.connection.on_state_change((state) => {
                ...
                } else if (this.connection_references > 0) {
    ...
    private start_recovery_poll(): void {
        if (this.recovery_poll_timer) return;
        this.recovery_poll_timer = setInterval(async () => {
            if (this.get_connection_state() === 'connected') {
                this.stop_recovery_poll();
                return;
            }
            try {
                ...
                // Ignore transient network errors during socket reconnection
            }
        }, 2500);
    }

    private stop_recovery_poll(): void {
        if (this.recovery_poll_timer) {
            clearInterval(this.recovery_poll_timer);
            this.recovery_poll_timer = null;
        }
    }

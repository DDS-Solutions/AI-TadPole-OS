 ...
 * - **Subsystem**: Frontend Service Layer / log_channel
 * - **Primary Entrypoints**: `LogChannel`
 *
...
import type { 
    ...
    Socket_User_Question_Event,
    Socket_Execution_Metrics_Event
} from '../types/events';
...
export class LogChannel extends BaseChannel<Incoming_Socket_Message> {
    ...
    matches(message: Incoming_Socket_Message): boolean {
        return (
            ...
            message.type === 'agent:user_question_requested' ||
            message.type === 'agent:execution_metrics'
        );
    }
    ...
    handle(message: Incoming_Socket_Message): void {
        ...
        switch (message.type) {
            ...
            case 'engine:scheduled_job_complete':
                ...
                break;
            case 'agent:user_question_requested':
                this.handle_user_question(message as Socket_User_Question_Event);
                break;
            case 'agent:execution_metrics':
                this.handle_execution_metrics(message as Socket_Execution_Metrics_Event);
                break;
        }
    }
    ...
    private handle_user_question(data: Socket_User_Question_Event): void {
        const agent_id = this.get_agent_id(data);
        const agent_name = this.get_agent_name(agent_id, data.agent_name);
        const metadata = Object.assign(Object.create(null), data);

        event_bus.emit_log({
            id: (data.question_id || data.id || '') as string,
            source: 'Agent',
            agent_id,
            agent_name,
            text: data.question || 'Awaiting user input',
            severity: 'info',
            metadata: metadata as Record<string, unknown>
        });
    }

    private handle_execution_metrics(data: Socket_Execution_Metrics_Event): void {
        if (data && data.metrics) {
            use_sovereign_store.getState().set_execution_metrics(data.metrics);
        }
    }

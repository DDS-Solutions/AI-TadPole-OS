...
describe('WebSocket Channels', () => {
    ...
    it('SwarmPulseChannel routes binary pulse payloads', () => {
        const channel = new SwarmPulseChannel();
        const listener = vi.fn();
        channel.subscribe(listener);

        expect(channel.matches()).toBe(false);
        channel.handle(); // no-op test

        const mock_pulse = { timestamp: 1234, nodes: [], edges: [] } as any;
        channel.handle_binary(mock_pulse);
        expect(listener).toHaveBeenCalledWith(mock_pulse);
    });

    it('AudioStreamChannel routes binary audio buffers', () => {
        const channel = new AudioStreamChannel();
        const listener = vi.fn();
        channel.subscribe(listener);

...
export async function api_request<T = unknown>(
    ...
    try {
        ...
        if (!response.ok) {
            ...
            // Self-healing guidance for RFC 9457 error codes
            const normalized_code = (error_code || '').toUpperCase().replace('-', '_');
            if (normalized_code === 'BUDGET_LIMIT_EXCEEDED' || normalized_code === 'BUDGET_EXHAUSTED') {
                try {
                    import('../stores/notification_store').then(({ use_notification_store }) => {

 ...
 * - **Subsystem**: Frontend Service Layer / voice_client
 * - **Primary Entrypoints**: `voice_client`, `Voice_Client`, `Voice_Status`
 *
...
export class Voice_Client {
    ...
    private status_callbacks = new Set<(status: Voice_Status) => void>();
    ...
    /** Unsubscribe handle for the socket audio stream. Stored so destroy() can clean it up. */
    private audio_unsub: (() => void) | null = null;
    private features_detected = false;

    constructor() {
        // init_feature_detection is deferred to first use (start_listening/speak) to avoid
        // a REST call at module import time before the engine is reachable.
        ...
        if (SpeechRecognitionClass) {
            ...
            this.recognition.onend = () => {
                if (this.is_listening && this.recognition) {
                    if (this.restart_timer) clearTimeout(this.restart_timer);
                    this.restart_timer = setTimeout(() => {
                        ...
                        if (this.is_listening && this.recognition) {
                            try {
                                ...
                                console.debug('[VoiceClient] Failed to restart recognition:', e);
                            }
                        }
                    }, 250);
        ...
        // Initialize streaming audio handler — store the unsubscribe handle so destroy() cleans it up.
        this.audio_unsub = get_tadpole_os_socket().subscribe_audio_stream((chunk) => {
    ...
    public on_status_change = (callback: (status: Voice_Status) => void): (() => void) => {
        this.status_callbacks.add(callback);
        return () => {
            this.status_callbacks.delete(callback);
        };
    };

    private set_status(status: Voice_Status): void {
        this.status = status;
        this.status_callbacks.forEach(callback => {
            try {
                callback(status);
            } catch (err) {
                console.error('[VoiceClient] Error in status callback:', err);
            }
        });
    }
    ...
    start_listening(callback: (text: string) => void): void {
        ...
        // Deferred feature detection: run once on first use rather than at import time.
        if (!this.features_detected) {
            this.features_detected = true;
            void this.init_feature_detection();
        }
        ...
        try {
            ...
            console.debug('[VoiceClient] Recognition.start() failed (may already be started):', err);
        }
    }
    ...
    async start_recording(): Promise<boolean> {
        try {
    ...
    private async speak_premium(text: string, voice_id?: string, engine: string = 'openai') {
        ...
        try {
            const audio_blob = await system_api_service.engine.speak(text, voice_id, engine);
    ...
    private async handle_audio_chunk(chunk: ArrayBuffer): Promise<void> {
        ...
        if (this.audio_context.state === 'suspended') {
            try {
                ...
                console.debug('[VoiceClient] AudioContext resume deferred pending user gesture:', e);
                return;
            }
        }
    ...
    private async init_feature_detection(): Promise<void> {
        try {
            ...
            console.debug('[VoiceClient] Feature detection failed, engine unreachable:', err);
        }
    }

    public destroy(): void {
        this.stop_listening();
        this.status_callbacks.clear();
        // Tear down audio stream subscription to prevent orphaned callbacks resurrecting AudioContext.
        if (this.audio_unsub) {
            this.audio_unsub();
            this.audio_unsub = null;
        }
        if (this.audio_context) {
            try {
                void this.audio_context.close();
            } catch {
                // Ignore failure on closing already-closed audio context
            }

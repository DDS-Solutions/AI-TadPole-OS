 ...
 * - **Subsystem**: Frontend Service Layer / mission_api_service
 * - **Primary Entrypoints**: `mission_api_service`, `Skill_Manifest`
 *
...
export const mission_api_service = {
    ...
    sync_mission: async (agent_id: string, mission: Mission): Promise<boolean> => {
        return track_operation('MissionAPI', `Synchronizing mission to agent: ${agent_id.toUpperCase()}`, async () => {
            await api_request(`/v1/agents/${encodeURIComponent(agent_id)}/mission`, {
                method: 'POST',
    ...
    save_skill_script: async (name: string, definition: unknown): Promise<void> => {
        await api_request(`/v1/skills/scripts/${encodeURIComponent(name)}`, {
            method: 'PUT',
    ...
    delete_skill_script: async (name: string): Promise<void> => {
        await api_request(`/v1/skills/scripts/${encodeURIComponent(name)}`, { method: 'DELETE' });
    },
    ...
    save_workflow: async (name: string, content: unknown): Promise<void> => {
        await api_request(`/v1/skills/workflows/${encodeURIComponent(name)}`, {
            method: 'PUT',
    ...
    delete_workflow: async (name: string): Promise<void> => {
        await api_request(`/v1/skills/workflows/${encodeURIComponent(name)}`, { method: 'DELETE' });
    },
    ...
    /**
     * Clones a mission into a fresh unique ID.
     */
    clone_mission: async (mission_id: string, primary_goal?: string): Promise<Mission> => {
        return track_operation('MissionAPI', `Cloning mission: ${mission_id}`, async () => {
            return api_request<Mission>(`/v1/agents/missions/${encodeURIComponent(mission_id)}/clone`, {
                method: 'POST',
                body: JSON.stringify({ primary_goal })
            });
        }, { metadata: { mission_id } });
    }
};

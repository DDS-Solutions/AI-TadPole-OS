...
export const oversight_api = {
    ...
    get_oversight_ledger: async (options?: RequestOptions): Promise<LedgerEntry[]> => {
        ...
        // Fetch remaining pages in chunks with concurrency cap (max 5 simultaneous requests)
        const CONCURRENCY_LIMIT = 5;
        const remaining_page_indices = Array.from({ length: total_pages - 1 }, (_, i) => i + 2);
        const rest: LedgerEntry[][] = [];

        for (let i = 0; i < remaining_page_indices.length; i += CONCURRENCY_LIMIT) {
            const batch = remaining_page_indices.slice(i, i + CONCURRENCY_LIMIT);
            const batch_results = await Promise.all(
                batch.map(page =>
                    api_request<{ data?: LedgerEntry[] } | LedgerEntry[]>(
                        `/v1/oversight/ledger?per_page=100&page=${page}`,
                        { method: 'GET', signal: options?.signal, timeout: options?.timeout }
                    ).then(r => Array.isArray(r) ? r : (r.data || []))
                )
            );
            rest.push(...batch_results);
        }

        return [...initial, ...rest.flat()];
    },

    decide_oversight: async (
        id: string, 
        decision: 'approved' | 'rejected', 
        user_answer_or_options?: string | RequestOptions,
        maybe_options?: RequestOptions
    ): Promise<void> => {
        const clean_id = sanitize_id(id);
        const user_answer = typeof user_answer_or_options === 'string' ? user_answer_or_options : undefined;
        const options = typeof user_answer_or_options === 'object' ? user_answer_or_options : maybe_options;

        await use_security_store.getState().generate_keys_if_needed();
        
        const timestamp = Date.now();
        const crypto_obj = (typeof window !== 'undefined' && window.crypto) || (typeof globalThis !== 'undefined' && globalThis.crypto);
        const nonce = crypto_obj && typeof crypto_obj.getRandomValues === 'function'
            ? Array.from(crypto_obj.getRandomValues(new Uint8Array(8)))
        ...
        if (user_answer !== undefined) {
            body_payload.user_answer = user_answer;
        }

...
describe('continuity_api', () => {
    ...
    describe('trigger_scheduled_job', () => {
        it('calls POST on /v1/continuity/jobs/:id/run', async () => {
            vi.mocked(api_request).mockResolvedValueOnce(undefined as any);
            await continuity_api.trigger_scheduled_job('job-1');
            expect(api_request).toHaveBeenCalledWith('/v1/continuity/jobs/job-1/run', expect.objectContaining({
                method: 'POST'
            }));
        });

        it('blocks traversal ids', async () => {
            await expect(continuity_api.trigger_scheduled_job('../malicious'))
                .rejects.toThrow(/Invalid identifier/);
        });
    });
    ...
    describe('get_workflow_run_steps', () => {
        it('handles array format', async () => {
            ...
            vi.mocked(api_request).mockResolvedValueOnce(mock_steps);
            const result = await continuity_api.get_workflow_run_steps('run-1');
            expect(result).toEqual(mock_steps);
            expect(api_request).toHaveBeenCalledWith('/v1/continuity/workflow-runs/run-1/steps', expect.objectContaining({
                method: 'GET'
            }));
        });

        it('handles envelope format and nulls', async () => {
            const mock_steps = [{ id: 'step-run-2', status: 'pending' }];
            vi.mocked(api_request).mockResolvedValueOnce({ step_runs: mock_steps });
            const result = await continuity_api.get_workflow_run_steps('run-1');
            expect(result).toEqual(mock_steps);

            vi.mocked(api_request).mockResolvedValueOnce(null);
            const nullResult = await continuity_api.get_workflow_run_steps('run-1');

 ...
 * - **Subsystem**: Frontend Service Layer / oversight_api.test
 *
...
vi.mock('../socket', () => ({
    ...
    is_private_network_origin: vi.fn(() => false)
}));
...
describe('oversight_api', () => {
    ...
    describe('get_oversight_ledger', () => {
        ...
        it('fetches subsequent pages in parallel when total_pages > 1', async () => {
            const page1_entry = { id: 'p1' };
            const page2_entry = { id: 'p2' };
            ...
            const result = await oversight_api.get_oversight_ledger();
            expect(result).toEqual([page1_entry, page2_entry]);
            expect(api_request).toHaveBeenCalledWith(
                '/v1/oversight/ledger?per_page=100&page=2',
    ...
    describe('decide_oversight', () => {
        it('generates keys, signs oversight, and sends to HTTP + WebSocket fallback', async () => {
            ...
            expect(mock_security_store.sign_oversight).toHaveBeenCalledWith('o-1', 'approved', expect.any(Number), expect.any(String));
            
            expect(api_request).toHaveBeenCalled();
            const apiCallArgs = vi.mocked(api_request).mock.calls[0];
            expect(apiCallArgs[0]).toBe('/v1/oversight/o-1/decide');
            const body = JSON.parse(apiCallArgs[1]!.body as string);
            expect(body).toEqual({
                decision: 'approved',
    ...
    describe('get_governance_settings', () => {
        it('calls GET /v1/oversight/settings', async () => {
            const mock_settings = { auto_approve_safe_skills: true, privacy_mode: false, cluster_privacy_policies: { 'cl-command': false } };
            vi.mocked(api_request).mockResolvedValueOnce(mock_settings);

            const result = await oversight_api.get_governance_settings();

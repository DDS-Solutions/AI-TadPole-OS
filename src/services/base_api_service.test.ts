 ...
 * - **Subsystem**: Frontend Service Layer / base_api_service.test
 *
...
describe('base_api_service utilities', () => {
    ...
    describe('scrub_secrets and scrub_string', () => {
        ...
        it('should redact Google, Groq, Anthropic, HuggingFace, and GitHub keys in strings', () => {
            const googleKey = 'AIzaSyDx123456789012345678901234567';
            const groqKey = 'gsk_1234567890abcdef1234567890abcdef';
            const hfKey = 'hf_1234567890123456789012345678901234';
            const ghpKey = 'ghp_1234567890abcdef1234567890abcdef';
            const antKey = 'sk-ant-api03-1234567890abcdef';

            const rawStr = `Keys: google=${googleKey}, groq=${groqKey}, hf=${hfKey}, ghp=${ghpKey}, ant=${antKey}`;
            const scrubbed = scrub_string(rawStr);

            expect(scrubbed).not.toContain(googleKey);
            expect(scrubbed).not.toContain(groqKey);
            expect(scrubbed).not.toContain(hfKey);
            expect(scrubbed).not.toContain(ghpKey);
            expect(scrubbed).not.toContain(antKey);
            expect(scrubbed).toBe('Keys: google=[REDACTED], groq=[REDACTED], hf=[REDACTED], ghp=[REDACTED], ant=[REDACTED]');
        });
...
describe('BaseApiService hardening additions', () => {
    ...
    describe('large / binary payload telemetry scrubbing', () => {
        ...
        it('should redact composite snake_case keys in FormData', () => {
            const formData = new FormData();
            formData.append('my_api_key', 'secret-val-1');
            formData.append('client_secret', 'secret-val-2');
            formData.append('custom_token', 'secret-val-3');
            formData.append('auth_bearer', 'secret-val-4');
            formData.append('user_name', 'alice');

            const scrubbed = scrub_secrets(formData) as FormData;
            expect(scrubbed.get('my_api_key')).toBe('[REDACTED]');
            expect(scrubbed.get('client_secret')).toBe('[REDACTED]');
            expect(scrubbed.get('custom_token')).toBe('[REDACTED]');
            expect(scrubbed.get('auth_bearer')).toBe('[REDACTED]');
            expect(scrubbed.get('user_name')).toBe('alice');
        });
    });

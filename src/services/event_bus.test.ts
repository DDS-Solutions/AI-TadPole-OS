 ...
 * - **Subsystem**: Frontend Service Layer / event_bus.test
 *
...
describe('event_bus', () => {
    ...
    it('gracefully coerces non-string log text without crashing', () => {
        event_bus.emit_log({ source: 'System', text: 123 as any, severity: 'info' });
        const logs = event_bus.get_history();
        expect(logs[logs.length - 1].text).toBe('123');
    });

    it('gracefully coerces invalid source and severity in emit_log without crashing (R3-M7)', () => {
        const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
        event_bus.emit_log({ source: 'Invalid' as any, text: 'hi', severity: 'invalid' as any });
        const logs = event_bus.get_history();
        expect(logs[logs.length - 1].source).toBe('System');
        expect(logs[logs.length - 1].severity).toBe('info');
        warnSpy.mockRestore();
    });

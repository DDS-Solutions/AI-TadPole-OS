 ...
 * - **Subsystem**: Frontend Service Layer / agent_mappers.test
 *
...
describe('agent_mappers', () => {
    ...
    describe('from_backend_agent (Robustness)', () => {
        ...
        it('should preserve identity from existing_agent when DTO has missing fields', () => {
            ...
            const partial_dto = {
                id: 'agent-1',
                name: 'Updated Name',
                // missing role, department, tokens_used, etc.
            };
            ...
            expect(normalized.tokens_used).toBe(12500);    // Preserved
        });
        ...
        it('should prioritize tokenUsage.totalTokens over legacy tokensUsed', () => {
            const raw = {
                id: 'agent-tokens',
                tokensUsed: 0,
                tokenUsage: {
            ...
            const normalized = from_backend_agent(raw as any);
            expect(normalized.tokens_used).toBe(1500);
            expect(normalized.input_tokens).toBe(500);
            expect(normalized.output_tokens).toBe(1000);
        });
    });
    ...
    describe('to_agent_update_payload', () => {
        ...
        it('should map tokens_used to both tokensUsed and tokenUsage.totalTokens', () => {
            const updates: Partial<Agent> = {
                tokens_used: 2500
            };
            const payload = to_agent_update_payload(updates as any);
            expect(payload.tokensUsed).toBe(2500);
            expect(payload.tokenUsage?.totalTokens).toBe(2500);
        });
    });
});

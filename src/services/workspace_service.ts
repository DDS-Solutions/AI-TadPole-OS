...
let custom_workspace_store: typeof use_workspace_store | null = null;
export const set_workspace_store = (store: typeof use_workspace_store | null) => {
    custom_workspace_store = store;
};

class Workspace_Service {
    private proposal_timeout: ReturnType<typeof setTimeout> | undefined;
    private proposal_resolvers: Array<(value: Swarm_Proposal | null) => void> = [];

    /**
     * Helper to safely access the workspace store and prevent null pointer exceptions.
     */
    private get_store() {
        return custom_workspace_store || use_workspace_store;
    }
    ...
    public async create_mission_cluster(mission: Partial<Mission_Cluster>): Promise<void> {
        ...
        try {
            ...
            log_error('WorkspaceService', 'Cluster Quota Sync Failed — Reverting optimistic UI state', error);
    ...
    public request_proposal(cluster_id: string, immediate = false): Promise<Swarm_Proposal | null> {
        ...
        if (immediate) {
            ...
            const pending = [...this.proposal_resolvers];
            this.proposal_resolvers = [];
            pending.forEach(resolve => resolve(result));
            return Promise.resolve(result);
        }

        return new Promise((resolve) => {
            this.proposal_resolvers.push(resolve);

...
export const system_api_service = {
    ...
    check_health: async (): Promise<boolean> => {
        try {
            await api_request('/v1/engine/health', { method: 'GET', timeout: 5000 });
            return true;
        } catch {
            return false;
        }
     ...
     */
    get_nodes: async (): Promise<Swarm_Node[]> => {
        return api_request<Swarm_Node[]>('/v1/infra/nodes', { method: 'GET' });
    },
    ...
    install_template: async (repository_url: string, path: string): Promise<void> => {
        await api_request('/engine/templates/install', {
            method: 'POST',

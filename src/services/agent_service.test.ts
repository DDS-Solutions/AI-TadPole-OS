 ...
 * - **Subsystem**: Frontend Service Layer / agent_service.test
 *
...
describe('agent_service', () => {
    ...
    describe('delete_agent', () => {
        const agent = { id: 'agent-1', name: 'Agent 1' } as Agent;
        const other = { id: 'agent-2', name: 'Agent 2' } as Agent;
        const cluster = { id: 'team', collaborators: [agent.id, other.id], alpha_id: agent.id } as Mission_Cluster;

        beforeEach(() => {
            use_agent_store.setState({ agents: [agent, other], error: null });
            use_workspace_store.setState({ clusters: [cluster] });
        });

        it('persists store deletion before removing the agent and its workspace assignments', async () => {
            let complete!: (success: boolean) => void;
            vi.mocked(agent_api_service.delete_agent).mockImplementation(() => new Promise(resolve => { complete = resolve; }));
            const deletion = use_agent_store.getState().delete_agent(agent.id);
            await vi.waitFor(() => expect(agent_api_service.delete_agent).toHaveBeenCalledWith(agent.id));
            expect(use_agent_store.getState().agents).toEqual([agent, other]);
            expect(use_workspace_store.getState().clusters).toEqual([cluster]);

            complete(true);
            await deletion;
            expect(use_agent_store.getState().agents).toEqual([other]);
            expect(use_workspace_store.getState().clusters[0]).toMatchObject({ collaborators: [other.id], alpha_id: undefined });
            expect(use_agent_store.getState().error).toBeNull();
        });

        it.each([false, new Error('Server refused deletion')])('preserves agents and assignments on failure: %s', async result => {
            if (result instanceof Error) vi.mocked(agent_api_service.delete_agent).mockRejectedValueOnce(result);
            else vi.mocked(agent_api_service.delete_agent).mockResolvedValueOnce(result);

            await expect(agent_service.delete_agent(agent.id)).resolves.toBe(false);
            expect(use_agent_store.getState().agents).toEqual([agent, other]);
            expect(use_workspace_store.getState().clusters).toEqual([cluster]);
            expect(use_agent_store.getState().error).toContain('Failed to delete agent');
        });
    });

    describe('registry invalidation', () => {
        it('refreshes the registry when an invalidation has no agent id or data', async () => {
            let on_update!: (event: Agent_Update_Event) => void;
            vi.spyOn(get_tadpole_os_socket(), 'subscribe').mockImplementation((_channel, callback) => {
                on_update = callback;
                return vi.fn();
            });
            vi.mocked(system_api_service.engine.check_health).mockResolvedValue(true);
            vi.mocked(agent_api_service.get_agents).mockResolvedValue([{ id: 'fresh-agent', name: 'Fresh agent' } as Raw_Agent]);
            agent_service.init();

            on_update({ type: 'engine:ui_invalidate', resource: 'agents' } as Agent_Update_Event);
            await vi.waitFor(() => expect(use_agent_store.getState().agents.map(agent => agent.id)).toEqual(['fresh-agent']));
            expect(agent_api_service.invalidate_cache).toHaveBeenCalledOnce();
            expect(agent_api_service.get_agents).toHaveBeenCalledOnce();
        });
    });
    ...
    describe('persist_agent_update', () => {
        ...
        it('forwards voice, metadata, and connector updates in camelCase', async () => {
            await persist_agent_update('agent-1', {
                ...
                connector_configs: [{ type: 'fs', uri: 'mock/path/workspace' }]
            });
    ...
    describe('build_socratic_envelope', () => {
        it('compiles a deterministic 4-pillar Socratic context envelope', () => {
            const agent = {
                id: '99',
                name: 'Agent 99 (QA-99)',
            ...
            const envelope = agent_service.build_socratic_envelope(agent, 'Security & Code Review', ['src/components/']);

            expect(envelope.status).toBe('PRE_CLEARED_GATE_PASS');
            expect(envelope.scope_contract.target_paths).toEqual(['src/components/']);
            expect(envelope.performance_threshold.active_model_slot).toBe(2);
            expect(envelope.performance_threshold.target_turn_latency_ms).toBe(1500);
            expect(envelope.performance_threshold.budget_usd_cap).toBe(1.5);
            expect((envelope.architecture_mode.mode_name as string)).toContain('Nexus Engineer Mode');
            expect(envelope.pre_cleared_failure_modes.length).toBeGreaterThan(0);
        });

        it('defaults to general sovereign mode for non-audit tasks', () => {
            const agent = {
                id: 'system_architect',
            ...
            const envelope = agent_service.build_socratic_envelope(agent, 'Feature Blueprint');
            expect(envelope.performance_threshold.target_turn_latency_ms).toBe(10000);
            expect((envelope.architecture_mode.mode_name as string)).toContain('Sovereign 3-Layer Architecture');
        });
    });
});

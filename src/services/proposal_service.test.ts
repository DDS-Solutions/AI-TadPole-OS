...
import type { MissionCluster } from '../stores/workspace_store';
...
describe('proposal_service', () => {
    ...
    it('returns null if cluster or objective is missing', () => {
        expect(proposal_service.generateProposal(null as any)).toBeNull();
        expect(proposal_service.generateProposal({ ...baseCluster, objective: '' } as any)).toBeNull();
    });
    ...
    it('generates security proposal for security-related objectives', () => {
        ...
        const proposal = proposal_service.generateProposal(cluster);

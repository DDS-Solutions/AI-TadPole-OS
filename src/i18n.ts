...
type TranslationParams = (Record<string, string | number | undefined> & TOptions) | TOptions;
...
class I18n {
  ...
  t(key: string, params?: TranslationParams): string | Record<string, unknown> {
    let keys = key.split('.');
    ...
    if (typeof result === 'string') {
      text = result;
    } else if (params && 'defaultValue' in params && typeof (params as TOptions).defaultValue === 'string') {
      text = (params as TOptions).defaultValue!;
    } else {
      text = key;
    }

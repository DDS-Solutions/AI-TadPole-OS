 ...
 * - **Subsystem**: Frontend State Store / provider_store
 * - **Primary Entrypoints**: `use_provider_store`, `Provider_Config`, `Provider_State`
 *
...
const DEFAULT_PROVIDERS: Provider_Config[] = [
    { id: PROVIDERS.OPENAI, name: 'OpenAI', icon: '⚡', protocol: 'openai' },
    { id: PROVIDERS.ANTHROPIC, name: 'Anthropic', icon: '🏺', protocol: 'anthropic' },
    { id: PROVIDERS.GOOGLE, name: 'Google Vertex', icon: '☁️', protocol: 'google' },
    { id: PROVIDERS.GROQ, name: 'Groq', icon: '⚡', protocol: 'groq' },
    { id: PROVIDERS.OLLAMA, name: 'Ollama', icon: '🦙', protocol: 'ollama', base_url: 'http://127.0.0.1:11434' },
    { id: 'meta', name: 'Meta / Llama', icon: '🦙', protocol: 'ollama' },
    { id: PROVIDERS.INCEPTION, name: 'Inception AI', icon: '🧠', protocol: 'custom' },
    { id: PROVIDERS.LOCAL, name: 'Local Infrastructure', icon: '🏠', protocol: 'ollama', base_url: 'http://127.0.0.1:11434' },
];
            ...
            migrate: (persisted_state: unknown) => {
                ...
                return {
                    ...
                    providers: (providers || []).map((p: Record<string, unknown>) => {
                        const safe_p = { ...p };
                        delete safe_p.api_key;
                        delete safe_p.apiKey;
                        return {
                            ...safe_p,

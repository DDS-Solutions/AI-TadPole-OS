...
const merge_agent_state = (existing_agent: Agent, updates: Partial<Agent>, work_path: string): Agent => {
    // Create a temporary combined object
    const combined_raw: Raw_Agent = { ...existing_agent, ...updates } as unknown as Raw_Agent;

    // Custom logic: remove current_task if it is explicitly missing in the updates
    const merged_raw: Partial<Raw_Agent> = { ...combined_raw };
    if (!Object.prototype.hasOwnProperty.call(updates, 'current_task')) {
        delete merged_raw.current_task;
    }

    return normalize_agent(merged_raw as Raw_Agent, work_path, existing_agent);
};
...
export const use_agent_store = create<Agent_State>((set, get) => ({
    ...
    fetch_agents: async (options: RequestInit = {}) => {
    ...
    /// 
    /// ### 🛡️ Sector Defense: Conflict Resolution
    /// Implements a `path_cache` to efficiently map agent IDs to their 
    /// respective workspace directories, avoiding expensive lookups during 
    /// 10Hz burst updates.
    init_telemetry: () => {

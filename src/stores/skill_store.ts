 ...
 * - **Subsystem**: Frontend State Store / skill_store
 * - **Primary Entrypoints**: `use_skill_store`, `Skill_Definition`, `Workflow_Definition`, `Hook_Definition`
 *
...
export interface Skill_Definition {
    ...
    category: 'user' | 'ai' | 'agent_generated';
...
export interface Skill_Actions {
    ...
    handle_pulse: (tool_name: string, status: 'success' | 'error' | string, latency: number) => void;
}

const pulse_timers = new Map<string, ReturnType<typeof setTimeout>>();
...
export const use_skill_store = create<Skill_State & Skill_Actions>()((set, get) => ({
    ...
    fetch_mcp_tools: async () => {
        ...
        try {
            const data = await mission_api_service.get_mcp_tools();
    ...
    fetch_skills: async () => {
        set({ is_loading: true, error: null });
        try {
            const data = await mission_api_service.get_unified_skills();
            ...
            log_error('SkillStore', 'Unified Skills Retrieval Failed', _error);
        }
    },

    save_skill_script: async (skill) => {
        try {
            await mission_api_service.save_skill_script(skill.name, skill);
    ...
    delete_skill_script: async (name) => {
        try {
            await mission_api_service.delete_skill_script(name);
            await get().fetch_skills();
        } catch (error: unknown) {
            const message = error instanceof Error ? error.message : String(error);
            set({ error: message });
    ...
    save_workflow: async (workflow) => {
        try {
            await mission_api_service.save_workflow(workflow.name, workflow);
            await get().fetch_skills();
        } catch (error: unknown) {
            const message = error instanceof Error ? error.message : String(error);
            set({ error: message });
    ...
    delete_workflow: async (name) => {
        try {
            await mission_api_service.delete_workflow(name);
            await get().fetch_skills();
        } catch (error: unknown) {
            const message = error instanceof Error ? error.message : String(error);
            set({ error: message });
    ...
    save_hook: async (hook) => {
        try {
            await mission_api_service.save_hook(hook.name, hook);
            await get().fetch_skills();
        } catch (error: unknown) {
            const message = error instanceof Error ? error.message : String(error);
            set({ error: message });
    ...
    delete_hook: async (name) => {
        try {
            await mission_api_service.delete_hook(name);
            await get().fetch_skills();
        } catch (error: unknown) {
            const message = error instanceof Error ? error.message : String(error);
            set({ error: message });
    ...
    handle_pulse: (tool_name: string, status: 'success' | 'error' | string, latency: number) => {
        const { mcp_tools } = get();
        ...
        // Reset pulsing after 1 second (clearing any existing timer for this tool)
        const existing_timer = pulse_timers.get(tool_name);
        if (existing_timer) clearTimeout(existing_timer);

        const timer = setTimeout(() => {
            pulse_timers.delete(tool_name);
        ...
        pulse_timers.set(tool_name, timer);
    }
}));

...
interface Workspace_State {
    ...
    sync_status: Record<string, {
        ...
        mounted_okf_nodes?: Array<{ id: string; title: string; concept_type: string }>;
        okf_validation?: {
    ...
    persist(
        (set, get) => ({
            ...
            toggle_cluster_privacy: async (cluster_id) => {
                let target_mode = false;
                set(state => ({
                    clusters: state.clusters.map(c => {
                        if (c.id === cluster_id) {
                            target_mode = !c.privacy_mode;
                            return { ...c, privacy_mode: target_mode };
                        }
                        return c;
                    })
                }));
                try {
                    await fetch('/v1/oversight/settings', {
                        method: 'PUT',
                        headers: {
                        ...
                        body: JSON.stringify({
                            ...
                            cluster_privacy_policies: { [cluster_id]: target_mode }
                        })
                    });
                } catch (e) {
                    console.error('Failed to sync cluster privacy settings:', e);
                }
            },

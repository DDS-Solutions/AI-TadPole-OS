 ...
 * - **Subsystem**: Frontend State Store / vault_store.test
 *
...
describe('use_vault_store', () => {
    ...
    describe('Lock/Unlock Mechanism with Sync', () => {
        it('unlocks and updates BroadcastChannel without broadcasting plaintext password', async () => {
            ...
            expect(bc_post_message).toHaveBeenCalledWith(expect.objectContaining({ type: 'UNLOCKED_SIGNAL' }));
            expect(bc_post_message).not.toHaveBeenCalledWith(expect.objectContaining({ payload: 'my-password' }));

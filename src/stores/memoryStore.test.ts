...
describe('useMemoryStore', () => {
    ...
    describe('searchMemories', () => {
        it('proxies raw search via TadpoleOSService.request', async () => {
            const mockSearchReturn = [{ id: 'found', text: 'search hit', mission_id: '1', timestamp: 1 }];
            vi.mocked(TadpoleOSService.request).mockResolvedValue(mockSearchReturn as any);

            const store = useMemoryStore.getState();
            const results = await (store as any).searchMemories('hit');

            expect(results).toEqual(mockSearchReturn);
            expect(TadpoleOSService.request).toHaveBeenCalledWith('/v1/memory/search', expect.objectContaining({
                method: 'POST',
                body: JSON.stringify({ query: 'hit' })
            }));
        });
    });

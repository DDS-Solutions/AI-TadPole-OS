 ...
 * - **Subsystem**: Frontend State Store / agent_store.test
 *
...
// Mock dependencies
vi.mock('../domain/agents/normalizers', async (importOriginal) => {
    const actual = await importOriginal<typeof import('../domain/agents/normalizers')>();
    return {
        ...actual,
        __esModule: true,
        normalize_agent_dto: vi.fn((raw, path, existing) => ({ 
            ...(existing || {}), 
            ...raw, 
...
describe('agent_store', () => {
    ...
    beforeEach(() => {
        ...
        (agent_service.agent_service.load_agents_into_store as Mock).mockImplementation(async () => {
            await Promise.resolve(); // Yield control to preserve synchronous is_loading state check
            use_agent_store.setState({ agents: [mock_agent_1, mock_agent_2], is_loading: false });
        });
        (agent_service.agent_service.update_agent as Mock).mockResolvedValue(true);
    ...
    describe('fetch_agents', () => {
        ...
        it('handles fetch errors correctly and logs the error', async () => {
            (agent_service.agent_service.load_agents_into_store as Mock).mockRejectedValue(new Error('Network error'));
    ...
    describe('update_agent', () => {
        ...
        it('performs an optimistic update and persists successfully', async () => {
            ...
            expect(agent_service.agent_service.update_agent).toHaveBeenCalledWith('1', { name: 'Updated Name', status: 'working' });
            expect(agent_service.agent_service.load_agents_into_store).not.toHaveBeenCalled();
        });

        it('retains optimistic state and logs a warning on persistence failure', async () => {
            (agent_service.agent_service.update_agent as Mock).mockRejectedValue(new Error('Sync failed'));
            (agent_service.agent_service.load_agents_into_store as Mock).mockResolvedValue(undefined);
    ...
    describe('add_agent', () => {
        ...
        it('reverts and logs error on persistence failure', async () => {
            ...
            (agent_service.agent_service.load_agents_into_store as Mock).mockImplementation(async () => {
                ...
                use_agent_store.setState({ agents: [mock_agent_1], is_loading: false });
            });
    ...
    describe('delete_agent', () => {
        it('delegates deletion to the persistence service before changing local state', async () => {
            use_agent_store.setState({ agents: [mock_agent_1, mock_agent_2] });
            
            await use_agent_store.getState().delete_agent('1');
            
            expect(agent_service.agent_service.delete_agent).toHaveBeenCalledWith('1');
            expect(use_agent_store.getState().agents).toEqual([mock_agent_1, mock_agent_2]);
            expect(use_workspace_store.setState).not.toHaveBeenCalled();
        });
    });
    ...
    describe('init_telemetry', () => {
        ...
        beforeEach(() => {
            vi.mocked(get_tadpole_os_socket().subscribe).mockImplementation((channel: any, cb: any) => {
                update_callback = cb;
        ...
        it('subscribes to updates and applies them with workspace resolution', () => {
            ...
            expect(get_tadpole_os_socket().subscribe).toHaveBeenCalledWith('agentUpdates', expect.any(Function));
            ...
            expect(normalize_agent_dto).toHaveBeenCalledWith(

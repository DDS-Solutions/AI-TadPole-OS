 ...
 * - **Subsystem**: Frontend State Store / workspace_store.test
 *
...
describe('use_workspace_store', () => {
    ...
    describe('Cluster CRUD', () => {
        ...
        it('prevents creating a cluster if limit reached', async () => {
            vi.mocked(settings_store.get_settings).mockReturnValue({ max_clusters: 1 } as any);

            const store = use_workspace_store.getState();
            await store.create_cluster({ name: 'Blocked Cluster' });
    ...
    describe('Branching & Handoffs', () => {
        ...
        it('receives a handoff and enforces maximum branch cap', () => {
             const store = use_workspace_store.getState();
             store.receive_handoff('cl-source', 'cl-command', 'Passing the torch');
  
             let state = use_workspace_store.getState();
             expect(state.clusters[0].pending_tasks[0].description).toContain('[HANDOFF FROM cl-source]');

             // Push over 50 handoffs
             for (let i = 0; i < 60; i++) {
                 store.receive_handoff('cl-source', 'cl-command', `Handoff ${i}`);
             }
             state = use_workspace_store.getState();
             expect(state.clusters[0].pending_tasks.length).toBeLessThanOrEqual(50);
        });
    });

    describe('delete_cluster', () => {
        it('deletes cluster and purges associated proposals and sync status', () => {
            use_workspace_store.setState({
                active_proposals: {
                    'cl-command': { cluster_id: 'cl-command', reasoning: 'proposal', changes: [], timestamp: 123 }
                },
                sync_status: {
                    'cl-command': { status: 'synced', last_sync_at: null, file_count: 5, total_bytes: 500 }
                }
            });
            ...
            const state = use_workspace_store.getState();
            expect(state.clusters.find(c => c.id === 'cl-command')).toBeUndefined();
            expect(state.active_proposals['cl-command']).toBeUndefined();
            expect(state.sync_status['cl-command']).toBeUndefined();
        });
    });

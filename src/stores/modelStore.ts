...
if (syncChannel) {
    // Initial fingerprint
    let lastBroadcast = JSON.stringify(useModelStore.getState().models);
    ...
    syncChannel.onmessage = (event) => {
        ...
        if (type === 'models:sync') {
            ...
            useModelStore.setState({ models: payload });
        }
    };
    ...
    useModelStore.subscribe((state) => {
        ...
        if (current !== lastBroadcast) {
            ...
            syncChannel.postMessage({ type: 'models:sync', payload: state.models });
        }
    });
}

            ...
            onRehydrateStorage: () => {
                return (hydrated_state, error) => {
                    ...
                    if (hydrated_state) {
                        ...
                        // Auto-seed from VITE_NEURAL_TOKEN when persisted token is missing, legacy, or weak.
                        ...
                        if (is_local && (!trimmed_token || trimmed_token === '' || LEGACY_DEV_TOKENS.has(trimmed_token) || trimmed_token.length < 16)) {
                            // Attempt to seed from Vite build-time env var before giving up
                            const env_token = (typeof import.meta !== 'undefined' && import.meta.env?.VITE_NEURAL_TOKEN) || '';
                            if (env_token.trim().length >= 16) {
                                console.debug('[SettingsStore] Auto-seeded API token from VITE_NEURAL_TOKEN.');
                                hydrated_state.update_setting('tadpole_os_api_key', env_token.trim());
                            } else {
                                console.warn('[SettingsStore] Local sidecar detected with missing, legacy, or weak token. Configure NEURAL_TOKEN in Settings or set VITE_NEURAL_TOKEN in .env.');

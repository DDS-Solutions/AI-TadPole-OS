    ...
    persist(
        (set, get) => ({
            ...
            save_settings: (new_settings) => {
                ...
                // Token is held in Zustand memory only — not persisted to disk (partialize strips it)
                // and not written to sessionStorage (readable by XSS). Tauri builds auto-seed via IPC.
                set({
                    settings: {
                        ...settings_copy,
            ...
            update_setting: <K extends keyof Tadpole_Settings>(key: K, value: Tadpole_Settings[K]) => {
                ...
                if (key === 'tadpole_os_api_key' && typeof value === 'string') {
                    final_value = sanitize_api_key(value) as unknown as Tadpole_Settings[K];
                    // Token stays in memory only — no sessionStorage write.
                }

                if (current[key] === final_value) return;
            ...
            onRehydrateStorage: () => {
                return (hydrated_state, error) => {
                    ...
                    if (hydrated_state) {
                        ...
                        // Auto-seed from runtime window injection only (Tauri IPC handled below).
                        // SECURITY: VITE_NEURAL_TOKEN build-time injection removed — env vars are
                        // inlined into the bundle and readable by anyone who obtains the dist files.
                        // sessionStorage removed — plaintext and readable by any XSS.
                        // Use Tauri IPC (get_neural_token) for desktop builds, or paste in Settings.
                        ...
                        if (trimmed_token && LEGACY_DEV_TOKENS.has(trimmed_token)) {
                            ...
                            if (runtime_token.trim().length >= 16) {
                                ...
                                console.warn('[SettingsStore] Sidecar detected with missing or legacy token. Configure NEURAL_TOKEN in Settings or verify Tauri IPC bridge.');
                                hydrated_state.update_setting('tadpole_os_api_key', '');
                            }
                        } else {

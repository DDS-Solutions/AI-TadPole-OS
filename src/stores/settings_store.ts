/**
 * @docs ARCHITECTURE:StateManagement
 * @docs OPERATIONS_MANUAL:Settings
 * 
 * ### AI Assist Note
 * **Zustand State**: Manages the Global system and user preference settings. 
 * Handles base64 obfuscation for sensitive keys and URL normalization for the Rust sidecar.
 * 
 * ### 🔍 Debugging & Observability
 * - **Failure Path**: LocalStorage corruption, invalid URL format (fails validation), or legacy migration failures.
 * - **Telemetry Link**: Check `localStorage['tadpole_settings']` or search `[SettingsStore]` in tracing logs.
 * 
 * ```mermaid
 * stateDiagram-v2
 *     [*] --> Initializing
 *     Initializing --> Loading_Local_Storage: getItem(SETTINGS_KEY)
 *     Loading_Local_Storage --> Deobfuscating: API Key deobfuscate()
 *     Deobfuscating --> Migration_Check: check legacy keys
 *     Migration_Check --> Ready: set({settings})
 *     Ready --> Updating: update_setting()
 *     Updating --> Validating: save_settings()
 *     Validating --> Ready: Valid URL
 *     Validating --> Ready: Invalid URL (return error)
 *     Ready --> Persisting: setItem(SETTINGS_KEY)
 *     Persisting --> Obfuscating: API Key obfuscate()
 *     Obfuscating --> Ready
 * ```
 */
            ...
            storage: {
                getItem: (name) => {
                    ...
                    try {
                        ...
                        const legacy = parsed.state.settings as Record<string, unknown>;
                        if (legacy.default_model) settings.default_model = legacy.default_model;
                        if (legacy.default_temperature) settings.default_temperature = legacy.default_temperature;
                        ...
                        if (legacy.max_clusters) settings.max_clusters = legacy.max_clusters;
                        ...
                        if (legacy.privacy_mode) settings.privacy_mode = legacy.privacy_mode;

                        // Legacy URL/Key migration
                        if (legacy.openClawUrl && !settings.tadpole_os_url) settings.tadpole_os_url = legacy.openClawUrl;
                        if (legacy.TadpoleOSUrl && !settings.tadpole_os_url) settings.tadpole_os_url = legacy.TadpoleOSUrl;
                        if (legacy.openClawApiKey && !settings.tadpole_os_api_key) {
                            settings.tadpole_os_api_key = deobfuscate(deobfuscate(legacy.openClawApiKey as string));
                        }
                        ...
                        // HARD MIGRATION: Force standard loopback for local dev consistency
                        let stored_url = (settings.tadpole_os_url || defaults.tadpole_os_url).trim();
                        if (stored_url === 'tauri://localhost' || stored_url.includes('tauri.localhost')) {

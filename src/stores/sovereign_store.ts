...
// Bridge websocket telemetry events from the global event bus into the SovereignChat store
event_bus.subscribe_logs((log) => {
    ...
    if (type === 'thought' || (type === 'log' && log.metadata?.level !== 'debug')) {
        ...
        const new_part: Message_Part = {
            type: 'text' as const,
            content: (log.metadata?.text || log.metadata?.message || 'Mission action complete.') as string
        };
        ...
        if (!did_update) {
            store.add_message({
                ...
                text: (log.metadata?.text || log.metadata?.message || 'Mission action complete.') as string,
                parts: [new_part],
                scope: store.active_scope
            });
        }

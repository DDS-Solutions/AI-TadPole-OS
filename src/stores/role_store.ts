 ...
 * - **Subsystem**: Frontend State Store / role_store
 * - **Primary Entrypoints**: `use_role_store`, `Role_State`, `Role_Actions`
 *
...
export const use_role_store = create<Role_State & Role_Actions>()(
    persist(
        (set) => ({
            ...
            fetch_blueprints: async () => {
                try {
                    const blueprints = await tadpole_os_service.get_role_blueprints();
                    ...
                    console.warn('[RoleStore] Blueprint hydration failed:', err);
                }
            }
        }),
        {

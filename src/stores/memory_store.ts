 ...
 * - **Subsystem**: Frontend State Store / memory_store
 * - **Primary Entrypoints**: `use_memory_store`, `MemoryEntry`
 *
...
export const use_memory_store = create<Memory_State>((set) => ({
    ...
    fetch_memories: async (agent_id: string) => {
        ...
        try {
            const data = await agent_api_service.get_agent_memory(agent_id);
    ...
    save_memory: async (agent_id: string, text: string): Promise<void> => {
        set({ is_loading: true, error: null });
        try {
            const res = await agent_api_service.save_agent_memory(agent_id, text);
    ...
    search_memories: async (query: string): Promise<MemoryEntry[]> => {
        const results = await agent_api_service.search_memory(query);
        return results.entries ?? [];
    },

    delete_memory: async (agent_id: string, row_id: string) => {
        try {
            await agent_api_service.delete_agent_memory(agent_id, row_id);
            // Optimistic update
            set(state => ({

...
let is_internal_emit = false;
let span_count = 0;

const prune_excess_spans = (spans: Record<string, Trace_Span>): Record<string, Trace_Span> => {
    const keys = Object.keys(spans);
    span_count = keys.length;
    if (keys.length <= MAX_SPANS) return spans;
    const to_remove = keys.length - MAX_SPANS;
    const candidates = keys.filter(k => spans[k].status === 'success' || spans[k].status === 'error');
    const pruned = { ...spans };
    for (let i = 0; i < Math.min(to_remove, candidates.length); i++) {
        delete pruned[candidates[i]];
    }
    span_count = Object.keys(pruned).length;
    return pruned;
};
...
export const use_trace_store = create<Trace_Store_State>((set, get) => ({
    ...
    add_span: (span: Trace_Span): void => {
        ...
        set((state) => {
            const is_new = !state.spans[span.id];
            if (is_new) span_count++;
            const next_spans = {
                ...state.spans,
                [span.id]: enriched
            };
            return {
                spans: span_count > MAX_SPANS ? prune_excess_spans(next_spans) : next_spans
            };
        });
        is_internal_emit = true;
        try {
            event_bus.emit_trace(enriched);
        } finally {
            is_internal_emit = false;
        }
    },

    update_span: (id: string, updates: Partial<Trace_Span>): void => {
        let updated: Trace_Span | undefined;
        set((state) => {
    ...
    reap_stale_spans: (now: number = Date.now()): number => {
            ...
            Object.keys(new_spans).forEach((id) => {
                ...
                if (s.status === 'running') {
                    ...
                    if (now - last_act >= ttl_ms) {
                        ...
                        has_mutations = true;
                        reaped_count += 1;
                        reaped_spans.push(new_spans[id]);
                    }
                }
            });
        ...
        if (reaped_spans.length > 0) {
            is_internal_emit = true;
            try {
                reaped_spans.forEach(s => event_bus.emit_trace(s));
            } finally {
                is_internal_emit = false;
            }
        }
    ...
    clear_trace: (trace_id: string): void => {
        set((state) => {
            ...
            span_count = Object.keys(new_spans).length;
            return {
                spans: new_spans,

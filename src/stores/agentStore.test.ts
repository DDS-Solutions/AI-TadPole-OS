...
describe('agentStore', () => {
    ...
    describe('updateAgent', () => {
        ...
        it('performs an optimistic update and persists successfully', async () => {
            await useAgentStore.getState().updateAgent('1', { name: 'Updated Name', status: 'working' });
        ...
        it('reverts state and emits event on persistence failure', async () => {
            ...
            await useAgentStore.getState().updateAgent('1', { name: 'Updated Name' });
            
            // It should NOT revert (to prevent flickers)
            const agent = useAgentStore.getState().agents.find(a => a.id === '1');
            expect(agent?.name).toBe('Updated Name'); // kept optimistic
            expect(useAgentStore.getState().error).toBeNull(); // handled silently with warning
    ...
    describe('getAgent', () => {
        it('retrieves an agent by id', () => {
            ...
            expect(useAgentStore.getState().getAgent('2')).toEqual(mockAgent2);
    ...
    describe('initTelemetry', () => {
        ...
        it('subscribes to updates and applies them with workspace resolution', () => {
            ...
            const agent = useAgentStore.getState().agents.find(a => a.id === '1') as any;
        ...
        it('resolves silo path if agent is not in any cluster', () => {
            useAgentStore.setState({ agents: [{ ...mockAgent1, id: '99' }] });
            useAgentStore.getState().initTelemetry();
            
            updateCallback({
                type: 'agent:update',
                agentId: '99', // Not in cluster mock

...
export const useWorkspaceStore = create<WorkspaceState>()(
    persist(
        (set, get) => ({
            ...
            loadQuotas: async () => {
                try {
                    const { quotas } = await TadpoleOSService.getMissionQuotas();
                    set((state) => ({
                        clusters: state.clusters.map(cluster => {
                            const q = quotas.find(q => q.entity_id === cluster.id);
                            return q ? {
                                ...cluster,
                                budgetUsd: q.budget_usd,
                                costUsd: q.used_usd
                            } : cluster;
                        })
                    }));
             ...
             * Enforces the 'maxClusters' constraint from the Settings Store.
             */
            createCluster: async (mission) => {
                const settings = getSettings();
                const clusters = get().clusters;

                // Threshold Check: Prevent workspace bloat
                ...
                const newClusterId = `cl-${(typeof crypto !== 'undefined' && crypto.randomUUID) ? crypto.randomUUID().split('-')[0] : Date.now().toString(36)}`;
                const newCluster: MissionCluster = {
                    ...mission,
                    id: newClusterId,
                ...
                try {
                    // Sync: Ensure the Rust backend is aware of the new mission's fiscal boundaries
                    await TadpoleOSService.updateMissionQuota(newClusterId, newCluster.budgetUsd || 0);
                } catch (error) {
                    console.error('Failed to sync new cluster quota to backend:', error);
                    // Optionally, revert cluster creation or mark it as unsynced
                }
            },
            ...
            updateClusterBudget: async (clusterId: string, budget: number) => {
                set((state) => ({
                    clusters: state.clusters.map((c) =>
                ...
                try {
                    await TadpoleOSService.updateMissionQuota(clusterId, budget);
                } catch (error) {
             ...
             * If the agent is not in a cluster, it falls back to a secure local silo.
             */
            getAgentPath: (agentId) => {
                const idStr = String(agentId);
                // Search for the first cluster that contains this agent in its collaborator list.

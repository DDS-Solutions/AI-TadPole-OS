 ...
 * 
 * ### AI Assist Note
 * **Test Suite**: Validates the Global system and user preference settings. 
 */
...
describe('settings_store', () => {
    ...
    it('rehydrates from localStorage correctly', async () => {
        ...
        const obfuscated_key = btoa(test_key);

        const mock_saved_settings = {
            state: {
                settings: {
                    tadpole_os_url: 'http://custom-engine:9000',
                    tadpole_os_api_key: obfuscated_key, // Store expects obfuscated key
                    theme: 'zinc',
                    density: 'comfortable',

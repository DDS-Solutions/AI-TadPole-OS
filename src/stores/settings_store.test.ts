...
describe('settings_store', () => {
    ...
    it('initializes with default value if no storage exists', async () => {
        ...
        expect(settings.tadpole_os_url).toBe('http://localhost:8000');
        expect(settings.tadpole_os_api_key).toBe('tadpole-os-sidecar-default-2026');
    });
    ...
    it('strips legacy placeholder tokens during rehydration', async () => {
        ...
        // Legacy token is stripped (via merge/hydrate) and then auto-enrolled to default
        expect(get_settings().tadpole_os_api_key).toBe('tadpole-os-sidecar-default-2026');
    });
});

            ...
            storage: {
                getItem: (name) => {
                    ...
                    try {
                        ...
                        const stateProps = parsed.state.settings as Record<string, unknown>;
                        const legacyUrl = stateProps.openClawUrl as string | undefined;
                        const legacyKey = stateProps.openClawApiKey as string | undefined;

                        if (legacyUrl && !settings.TadpoleOSUrl) {
                            console.debug(`[Settings] Migrating legacy URL: ${legacyUrl} -> TadpoleOSUrl`);
                            settings.TadpoleOSUrl = legacyUrl;
                        }
                        if (legacyKey && !settings.TadpoleOSApiKey) {

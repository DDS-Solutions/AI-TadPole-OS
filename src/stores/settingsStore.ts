...
const getBaseUrl = () => {
    ...
    // Protocols like 'tauri://' or 'https://' (when local) can cause fetch barriers.
            ...
            storage: {
                getItem: (name) => {
                    ...
                    try {
                        ...
                        // to prevent 'failed to fetch' errors on sidecar communication.
                        let storedUrl = (settings.TadpoleOSUrl || defaults.TadpoleOSUrl).trim();
                        if (storedUrl.startsWith('tauri:') || storedUrl.includes('tauri.localhost')) {
                            console.debug(`[Settings] Correcting protocol mismatch: ${storedUrl} -> http://localhost:8000`);
                        ...
                        if (isLocalAlias && isRemoteAccess && !window.location.protocol.startsWith('tauri')) {

 ...
 * - **Subsystem**: Frontend State Store / skill_store.test
 *
...
describe('use_skill_store', () => {
    ...
    describe('fetch_skills', () => {
        it('fetches and sorts skills and workflows successfully', async () => {
            ...
            vi.mocked(mission_api_service.get_unified_skills).mockResolvedValue(mock_data as any);
        ...
        it('handles failure during fetch_skills', async () => {
            vi.mocked(mission_api_service.get_unified_skills).mockRejectedValue(new Error('Network error'));
    ...
    describe('fetch_mcp_tools', () => {
        it('fetches mcp tools successfully', async () => {
            ...
            vi.mocked(mission_api_service.get_mcp_tools).mockResolvedValue(mock_tools as any);
            ...
            expect(mission_api_service.get_mcp_tools).toHaveBeenCalled();
        });

        it('handles failure during fetch_mcp_tools', async () => {
            vi.mocked(mission_api_service.get_mcp_tools).mockRejectedValue(new Error('MCP failure'));
    ...
    describe('CUD Operations', () => {
        it('saves a skill script and refetches skills', async () => {
            vi.mocked(mission_api_service.save_skill_script).mockResolvedValue({} as any);
            vi.mocked(mission_api_service.get_unified_skills).mockResolvedValue({ scripts: [], workflows: [], hooks: [], manifests: [] } as any);
            ...
            expect(mission_api_service.save_skill_script).toHaveBeenCalledWith('NewSkill', mock_skill);
            expect(mission_api_service.get_unified_skills).toHaveBeenCalled();
        });

        it('deletes a skill script and refetches', async () => {
            vi.mocked(mission_api_service.delete_skill_script).mockResolvedValue({} as any);
            vi.mocked(mission_api_service.get_unified_skills).mockResolvedValue({ scripts: [], workflows: [], hooks: [], manifests: [] } as any);

            const store = use_skill_store.getState();
            await store.delete_skill_script('OldSkill');

            expect(mission_api_service.delete_skill_script).toHaveBeenCalledWith('OldSkill');
            expect(mission_api_service.get_unified_skills).toHaveBeenCalled();
        });

        it('saves a workflow and refetches', async () => {
            vi.mocked(mission_api_service.save_workflow).mockResolvedValue({} as any);
            vi.mocked(mission_api_service.get_unified_skills).mockResolvedValue({ scripts: [], workflows: [], hooks: [], manifests: [] } as any);
            ...
            expect(mission_api_service.save_workflow).toHaveBeenCalledWith('NewWF', mock_wf);
            expect(mission_api_service.get_unified_skills).toHaveBeenCalled();
        });

        it('deletes a workflow and refetches', async () => {
            vi.mocked(mission_api_service.delete_workflow).mockResolvedValue({} as any);
            vi.mocked(mission_api_service.get_unified_skills).mockResolvedValue({ scripts: [], workflows: [], hooks: [], manifests: [] } as any);

            const store = use_skill_store.getState();
            await store.delete_workflow('OldWF');

            expect(mission_api_service.delete_workflow).toHaveBeenCalledWith('OldWF');
            expect(mission_api_service.get_unified_skills).toHaveBeenCalled();
        });

        it('saves a hook and refetches', async () => {
            vi.mocked(mission_api_service.save_hook).mockResolvedValue({} as any);
            vi.mocked(mission_api_service.get_unified_skills).mockResolvedValue({ scripts: [], workflows: [], hooks: [], manifests: [] } as any);
            ...
            expect(mission_api_service.save_hook).toHaveBeenCalledWith('NewHook', mock_hook);
            expect(mission_api_service.get_unified_skills).toHaveBeenCalled();
        });

        it('deletes a hook and refetches', async () => {
            vi.mocked(mission_api_service.delete_hook).mockResolvedValue({} as any);
            vi.mocked(mission_api_service.get_unified_skills).mockResolvedValue({ scripts: [], workflows: [], hooks: [], manifests: [] } as any);

            const store = use_skill_store.getState();
            await store.delete_hook('OldHook');

            expect(mission_api_service.delete_hook).toHaveBeenCalledWith('OldHook');
            expect(mission_api_service.get_unified_skills).toHaveBeenCalled();
        });

        it('handles failure during save_skill_script', async () => {
            vi.mocked(mission_api_service.save_skill_script).mockRejectedValue(new Error('Save failed'));

 ...
 * - **Subsystem**: Frontend State Store / model_store
 * - **Primary Entrypoints**: `use_model_store`, `Model_Entry`, `Model_State`
 *
    ...
    persist(
        (set, get) => ({
            ...
            add_model: async (name, provider, modality = 'llm', limits) => {
                ...
                try {
                    await system_api_service.infra.update_model(id, {
                        ...new_model,
            ...
            delete_model: async (id) => {
                ...
                try {
                    await system_api_service.infra.delete_model(id);
                    setTimeout(() => {
                        set(state => {
                            const new_deleting = new Set(state.deleting_ids);
            ...
            sync_models: async () => {
                try {
                    const raw_models = (await system_api_service.infra.get_models()) || [];
                    ...
                    let final_models = [...filtered_models];
                    let changed = filtered_models.length !== models.length;

                    backend_models.forEach((bm) => {
                        const existing_idx = final_models.findIndex(m => m.id === bm.id);
                        if (existing_idx === -1 && !deleting_ids.has(bm.id)) {
                            ...
                            if (existing.provider !== bm.provider || 
                                ...
                                final_models = final_models.map((m, idx) => idx === existing_idx ? { ...m, ...bm } : m);
                                changed = true;
                            }
                        }
                    });
...
if (sync_channel) {
    ...
    sync_channel.onmessage = (event) => {
        ...
        if (type === 'models:sync' && Array.isArray(payload)) {
            const valid_models = payload.filter((m): m is Model_Entry => Boolean(m && typeof m === 'object' && typeof m.id === 'string' && typeof m.name === 'string'));
            // Update fingerprint BEFORE setting state to prevent local subscribe from echoing back
            last_broadcast = JSON.stringify(valid_models);
            use_model_store.setState({ models: valid_models });
        }
    };

...
export default function Missions() {
    ...
    const active_cluster = clusters.find(c => c.id === selected_cluster_id);
    const assigned_agent_ids = new Set(clusters.flatMap(c => c.collaborators).map(String));
    const available_agents = agents.filter(a => !assigned_agent_ids.has(String(a.id)));

    const handle_run_mission = async () => {
        ...
        try {
            ...
            const { model_id, provider } = resolve_agent_model_config(alpha_agent as any);
                    ...
                    {active_cluster ? (
                        <>
                            ...
                            <div className="p-6 flex flex-col gap-8 flex-1 overflow-y-auto custom-scrollbar">
                                ...
                                {/* Handoffs */}
                                {active_cluster.pending_tasks.filter((t: any) => t.id.startsWith('ho-') && t.status === 'pending').length > 0 && (
                                    <div className="p-4 rounded-xl border border-amber-500/30 bg-amber-500/5">
                                        ...
                                        <div className="space-y-3">
                                            {active_cluster.pending_tasks.filter((t: any) => t.id.startsWith('ho-') && t.status === 'pending').map((task: any) => (
                                ...
                                <div key={active_cluster.id} className="p-4 rounded-xl bg-zinc-900/30 border border-zinc-800/50 flex flex-col gap-3">
                                    <h4 className="text-xs font-bold text-zinc-500 uppercase tracking-[0.2em] flex items-center gap-2">
                                        <Target size={12} className={get_theme_colors(active_cluster.theme).text} /> {i18n.t('missions.header_objective')}
                                    </h4>
                                    <textarea
                                ...
                                <Agent_Team_View
                                    ...
                                    on_assign={(id: string) => assign_agent_to_cluster(id, active_cluster.id)}
                                    on_unassign={(id: string) => unassign_agent_from_cluster(id, active_cluster.id)}
                                    on_set_alpha={(id: string) => set_alpha_node(active_cluster.id, id)}
                                />

                                <div className="mt-4 h-64">
                                    <Buffered_Transcript_View 
                                        mission_id={active_cluster.id} 
                                        class_name="shadow-2xl border-zinc-800/50" 
                                    />
                                </div>
                            </div>
                        </>

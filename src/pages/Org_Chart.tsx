 ...
 * - **Subsystem**: UI Pages / Org_Chart
 * - **Primary Entrypoints**: `Org_Chart`
 *
 ...
 * - **Local Errors**: none
 * - **Telemetry Targets**: `[Org_Chart]`, `[OrgChart]`
 */
...
import { resolve_alpha, resolve_nexus } from '../utils/agent_hierarchy';
import { use_workspace_store } from '../stores/workspace_store';
import { use_dropdown_store } from '../stores/dropdown_store';
import { use_agent_store } from '../stores/agent_store';
import { tadpole_os_service } from '../services/tadpoleos_service';
import { i18n } from '../i18n';
import { OrgChartHeader } from '../components/hierarchy/OrgChartHeader';
import { HierarchyConnectionLines } from '../components/hierarchy/HierarchyConnectionLines';
import { ClusterNodeCard, type SharedNodeProps } from '../components/hierarchy/ClusterNodeCard';
...
export default function Org_Chart() {
    const { clusters } = use_workspace_store(
        useShallow(s => ({ clusters: s.clusters }))
    );
    const agents_list = use_agent_store(s => s.agents);
    const { dropdown_open_id, close_dropdowns } = use_dropdown_store(
        useShallow(s => ({
            dropdown_open_id: s.open_id,
            close_dropdowns: s.close_dropdown
        }))
    );
    ...
    // Populate available roles from live registry
    const available_roles = useMemo(() =>
        Array.from(new Set(
            agents_list.map(a => a.role).filter(Boolean)
        )).sort()
        , [agents_list]);
    ...
    const handle_skill_trigger = useCallback(async (agent_id: string, skill: string, slot?: 1 | 2 | 3) => {
        ...
        const target_slot = slot !== undefined ? slot : parse_active_model_slot(agent.active_model_slot);
    ...
    const handle_delete_agent = useCallback(async (id: string) => {
        try {
            await use_agent_store.getState().delete_agent(id);
    ...
    const shared_node_props: SharedNodeProps = useMemo(() => ({
        available_roles,
    ...
    const hierarchy_data = useMemo(() => {
        ...
        // Resolve Root Alpha (CEO / Lead Agent) and Nexus (COO / Sub-Lead) via centralized hierarchy utility
        const alpha = resolve_alpha(combined_agents);
        if (!alpha) return null;

        const nexus = resolve_nexus(combined_agents, alpha);
                ...
                <div className="min-w-max pt-1 pb-12 px-12 flex flex-col items-center gap-12 relative">
                    ...
                    <div className={`relative pt-0 mt-[-18px] w-[350px] ${dropdown_open_id === hierarchy_data.nexus?.id ? 'z-[100]' : 'z-20'}`}>
                        ...
                        <HierarchyConnectionLines
                            ...
                            xCoords={xCoords}
                            hasActiveChains={hierarchy_data.chains.some(c => c.is_active)}
                        />
                    </div>

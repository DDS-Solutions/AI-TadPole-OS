...
export default function Org_Chart() {
    ...
    const hierarchy_data = useMemo(() => {
        ...
        // Force Agent of Nine (ID '1', CEO) to be the Root Alpha (top) and Tadpole Alpha (ID '2', COO) to be the Nexus (Level 2)
        const alpha = combined_agents.find(a => a.id === '1' || a.name === 'Agent of Nine')
            || combined_agents.find(a => a.role === 'CEO')
            || combined_agents[0];

        const nexus = combined_agents.find(a => (a.id === '2' || a.name === 'Tadpole Alpha' || a.name === 'Tadpole') && a.id !== alpha.id)
            || combined_agents.find(a => a.role === 'COO' && a.id !== alpha.id)

...
export default function Org_Chart() {
    ...
    const handle_agent_update = useCallback((agent_id: string, updates: Partial<Agent>) => {
        void agent_service.update_agent(agent_id, updates);
    }, []);
    ...
    const handle_skill_trigger = useCallback(async (agent_id: string, skill: string, slot: 1 | 2 | 3 = 1) => {
        ...
        void agent_service.update_agent(agent_id, { 
            ...
            current_task: i18n.t('org_chart.executing_skill', { defaultValue: `⚡ Executing: ${skill}...` }),
        ...
        try {
            ...
            if (!success) {
                void agent_service.update_agent(agent_id, { status: 'idle' });
            }
        } catch (e) {
    ...
    // Parameterized and unified Model Change Handler to remove three-fold code duplication
    ...
    const hierarchy_data = useMemo(() => {
        ...
        // Level 1: Alpha & Nexus dynamically resolved by CEO/COO role blueprints instead of hardcoded strings
        const rootCluster = clusters.find(c => c.id === ROOT_CLUSTER_ID);
        const dynamicAlphaId = rootCluster?.alpha_id;
        const dynamicNexusId = rootCluster?.collaborators.find(cid => cid !== dynamicAlphaId);

        const alpha = (dynamicAlphaId ? combined_agents.find(a => a.id === dynamicAlphaId) : null)
            || combined_agents.find(a => a.role === 'CEO' || a.name === 'Agent of Nine' || a.id === '1')
            || combined_agents[0];

        const nexus = (dynamicNexusId ? combined_agents.find(a => a.id === dynamicNexusId && a.id !== alpha.id) : null)
            || combined_agents.find(a => (a.role === 'COO' || a.name === 'Tadpole' || a.id === '2') && a.id !== alpha.id)
    ...
    const branchPath = useMemo(() => {
        ...
        for (const x of xCoords) {
            if (Math.abs(x - 500) < 1) {
                ...
                // Curve to the left
                const startTurnX = 500;
                const endTurnX1 = 500 - 20; // 480
                const startTurnX2 = x + 20;  // 120
                const endTurnX2 = x;       // 100
                path += ` M 500 20 Q ${startTurnX} 30 ${endTurnX1} 30 H ${startTurnX2} Q ${endTurnX2} 30 ${endTurnX2} 40 V 52`;
            } else {
                // Curve to the right
                const startTurnX = 500;
                const endTurnX1 = 500 + 20; // 520
                const startTurnX2 = x - 20;  // 880
                const endTurnX2 = x;       // 900
                path += ` M 500 20 Q ${startTurnX} 30 ${endTurnX1} 30 H ${startTurnX2} Q ${endTurnX2} 30 ${endTurnX2} 40 V 52`;
            }
        }
        return path;
    ...
    return (
        <div className="h-full flex flex-col bg-[color:var(--color-background)]">
            ...
            {/* Semantic layout elements without a11y role contradictions */}
            <div 
...
type AgentChainProps = {
    ...
    handle_skill_trigger: (id: string, skill: string, slot?: 1 | 2 | 3) => void;

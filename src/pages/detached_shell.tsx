...
const Agent_Config_Panel = lazy(() => import('../components/Agent_Config_Panel').then(module => ({ default: module.default })));
...
const Sovereign_Chat = lazy(() => import('../components/Sovereign_Chat').then(module => ({ default: module.Sovereign_Chat })));
...
export default function Detached_Shell() {
    ...
    const render_content = () => {
        switch (type) {
            case 'chat':
                return <Sovereign_Chat is_detached_view />;
                ...
                return (
                    <div className="h-screen bg-zinc-950 p-6 flex flex-col overflow-hidden">
                        <Agent_Status_Grid
                            ...
                            clusters={clusters}
                            initial_tab_id={tab_id || undefined}
                            on_skill_trigger={async (agent_id, skill, _slot = 1) => {
                                // Direct integration with service for detached operations
                                const agent = agents_list.find(a => a.id === agent_id);
                                if (!agent) return;
                                const cluster = clusters.find(c => (c.collaborators || []).map(String).includes(String(agent_id)));
                                try {
            ...
            case 'agent-config':
                ...
                return (
                    <div className="h-screen bg-zinc-950 p-6 flex items-center justify-center">
                         <Agent_Config_Panel
                            agent={agent}
                            on_close={() => window.close()} // Detached window close
                            on_update={update_agent}
                            is_new={id === 'new'}

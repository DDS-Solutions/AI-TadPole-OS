    ...
    return (
        <div className="h-screen bg-[color:var(--color-background)] p-6 flex flex-col overflow-hidden">
            <Agent_Status_Grid
                ...
                on_model_change={(agent_id, new_model) => {
                    const agent = agents_list.find(a => a.id === agent_id);
                    const provider = resolve_provider(new_model);
                    const model_config = agent?.model_config
                        ? { ...agent.model_config, modelId: new_model, provider }
                        : { modelId: new_model, provider };
                    update_agent(agent_id, { model: new_model, model_config });
                }}
                on_model_2_change={(agent_id, new_model) => {
                    const agent = agents_list.find(a => a.id === agent_id);
                    const provider = resolve_provider(new_model);
                    const model_config2 = agent?.model_config2
                        ? { ...agent.model_config2, modelId: new_model, provider }
                        : { modelId: new_model, provider };
                    update_agent(agent_id, { model_2: new_model, model_config2 });
                }}
                on_model_3_change={(agent_id, new_model) => {
                    const agent = agents_list.find(a => a.id === agent_id);
                    const provider = resolve_provider(new_model);
                    const model_config3 = agent?.model_config3
                        ? { ...agent.model_config3, modelId: new_model, provider }
                        : { modelId: new_model, provider };
                    update_agent(agent_id, { model_3: new_model, model_config3 });
                }}
                on_role_change={() => {}}
                on_configure_click={() => {}}
...
export default function Detached_Shell() {
    ...
    const render_content = () => {
        switch (type) {
            ...
            case 'tab': {
                ...
                return (
                    <div className="h-screen bg-[color:var(--color-background)] p-6 flex flex-row gap-6 overflow-hidden">
    ...
    return (
        <div className="min-h-screen bg-[color:var(--color-background)] text-zinc-100 font-sans antialiased">
            ...
            <Suspense fallback={<div className="h-screen bg-[color:var(--color-background)] flex items-center justify-center animate-pulse font-mono uppercase tracking-widest text-zinc-700 text-xs">Synchronizing Neural Channels...</div>}>
                {render_content()}
            </Suspense>
        </div>
    );

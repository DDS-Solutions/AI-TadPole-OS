 ...
 * - **Subsystem**: UI Pages / Skills
 * - **Primary Entrypoints**: `Skills`
 *
...
export default function Skills() {
    const {
        ...
        editing_skill, editing_workflow, editing_hook, hook_form,
        ...
        set_selected_tool, set_is_lab_open, set_import_modal_open, set_editing_skill, set_editing_workflow, set_editing_hook, set_hook_form,
        set_assignment_modal_open, set_schema_error, set_confirm_dialog,
        handle_import_click, on_confirm_import,
        handle_delete_skill, handle_delete_workflow, handle_delete_hook,
        handle_save_skill, handle_save_workflow, handle_save_hook,
        handle_toggle_assignment, handle_assign, handle_edit_hook, handle_create_hook
    } = useSkillsManager();

    const [selected_report_skill, set_selected_report_skill] = useState<Skill_Definition | null>(null);

    // Mouse drag scrolling state & event handlers for horizontal overflow tabs
    ...
    const tabs: { id: Tab_Type; label: string; icon: React.ElementType; count: number; tooltip: string }[] = [
        { id: 'all', label: i18n.t('skills.tab_all', { defaultValue: 'ALL ABILITIES' }), icon: Terminal, count: manifests.length + scripts.length + workflows.length, tooltip: i18n.t('skills.tooltip_all', { defaultValue: 'Unified swarm capabilities' }) },
    ...
    const all_tab = tabs.find(t => t.id === 'all')!;
    const scrollable_tabs = tabs.filter(t => t.id !== 'all');
    ...
    return (
        <div className="p-6 space-y-6 max-w-7xl mx-auto">
            ...
            <div className="flex items-center bg-[color:var(--color-background)] p-1 rounded-2xl border border-[color:var(--color-surface)] select-none w-full min-w-0">
                {/* Static 'ALL ABILITIES' Tab */}
                <div className="flex-shrink-0">
                    <Tooltip content={all_tab.tooltip} position="bottom">
                        <button
                        ...
                        >
                            <all_tab.icon size={18} className={active_tab === 'all' ? 'text-green-500' : 'text-zinc-600 group-hover:text-zinc-400'} />
                            {all_tab.label}
                            <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-mono ${
                                active_tab === 'all' ? 'bg-green-500/20 text-green-400' : 'bg-[color:var(--color-surface)] text-zinc-600'
                            }`}>
                                {all_tab.count}
                            </span>
                        </button>
                ...
                {/* Vertical Divider */}
                <div className="h-6 w-px bg-zinc-800 mx-3 shrink-0" />

                {/* Scrollable Container for Dynamic Tabs */}
                <div
                    ref={scrollRef}
                    ...
                    className="flex-1 flex items-center gap-1 overflow-x-auto no-scrollbar cursor-grab active:cursor-grabbing select-none"
                >
            ...
            <div className="min-h-[400px]">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        {scripts.map(script => (
                            <Skill_Card 
                                ...
                                on_view_report={set_selected_report_skill}
                            />
                        ))}
                        {workflows.map(wf => (
                            <Workflow_Card 
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        {scripts.length === 0 && (
                            <div className="col-span-full">
                                <Tw_Empty_State title={i18n.t('skills.empty_skills_title')} description={i18n.t('skills.empty_skills_desc')} />
                            </div>
                        )}
                    </div>
                )}
                ...
                {active_tab === 'hooks' && (
                    <Hook_List 
                        hooks={hooks} 
                        on_edit={handle_edit_hook} 
                        on_delete={handle_delete_hook}
                        on_create={handle_create_hook}
                    />
                )}
            ...
            <Hook_Modal 
                is_open={!!editing_hook}
                on_close={() => set_editing_hook(null)}
                editing_hook={editing_hook || {}}
                hook_form={hook_form}
                set_hook_form={set_hook_form}
                hook_save_error={save_error}
                is_saving={is_saving}
                on_save={handle_save_hook}
            />
            ...
            <Security_Report_Modal
                is_open={!!selected_report_skill}
                on_close={() => set_selected_report_skill(null)}
                skill={selected_report_skill}
            />
        </div>
    );
}

 ...
 * - **Subsystem**: UI Pages / Agent_Manager.test
 *
 ...
 * - **Witness Tests**: `Agent_Manager.test.tsx`
 */
...
describe('Agent_Manager', () => {
    ...
    it('calls add_agent when creating a new agent', () => {
        ...
        // Trigger update in creation mode calls store add_agent
        fireEvent.click(screen.getByText('Update'));
        expect(mock_agent_store_state.add_agent).toHaveBeenCalled();
    });
    ...
    it('shows recruited user agents in the AI Swarm tab', () => {
        ...
        const ai_tab_button = screen.getByText('agent_manager.tab_ai');
    ...
    it('allows selecting Health Status from the role filter select and sorts from worst health to best', () => {
        mock_agent_store_state.agents = [
            { id: '1', name: 'Agent Healthy', role: 'CEO', status: 'idle', category: 'user', failure_count: 0 },
            { id: '2', name: 'Agent Broken', role: 'Dev', status: 'idle', category: 'user', failure_count: 5 },
            { id: '3', name: 'Agent Degraded', role: 'Dev', status: 'idle', category: 'user', failure_count: 2 },
        ];

        render(<Agent_Manager />);
        const select = screen.getByRole('combobox');
        fireEvent.change(select, { target: { value: 'health_status' } });

        const cards = screen.getAllByRole('button').filter(b => b.textContent?.includes('Agent '));
        expect(cards[0].textContent).toContain('Agent Broken');
        expect(cards[1].textContent).toContain('Agent Degraded');
        expect(cards[2].textContent).toContain('Agent Healthy');
    });
});

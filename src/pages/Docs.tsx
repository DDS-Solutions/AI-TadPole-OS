...
export default function Docs() {
    ...
    useEffect(() => {
        const fetchDocs = async () => {
            try {
                const docList = await TadpoleOSService.getKnowledgeDocs();
                setDocs(docList);
    ...
    useEffect(() => {
        const fetchContent = async () => {
            ...
            try {
                let rawData = '';
                if (activeTab === 'manual') {
                    rawData = await TadpoleOSService.getOperationsManual();
                } else if (selectedDoc) {
                    rawData = await TadpoleOSService.getKnowledgeDoc(selectedDoc.category, selectedDoc.name);
                }

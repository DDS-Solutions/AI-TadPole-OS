...
describe('Standups Page', () => {
    ...
    it('rolls back to idle and reports microphone startup failure', async () => {
        ...
        expect(tadpole_os_service.transcribe).not.toHaveBeenCalled();
    });

    it('reports transcription dispatch failures without emitting a command', async () => {
        (tadpole_os_service.transcribe as unknown as ReturnType<typeof vi.fn>).mockRejectedValue(new Error('engine offline'));
        ...
        expect(tadpole_os_service.send_command).not.toHaveBeenCalled();
    });

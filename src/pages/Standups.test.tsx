 ...
 * - **Subsystem**: UI Pages / Standups.test
 *
 ...
 * - **Witness Tests**: `Standups.test.tsx`
 */
...
describe('Standups Page', () => {
    ...
    beforeEach(() => {
        ...
        (tadpole_os_service.send_command as unknown as ReturnType<typeof vi.fn>).mockResolvedValue(undefined);
        // Domain service mocks (hooks now call these directly)
        (system_api_service.engine.transcribe as unknown as ReturnType<typeof vi.fn>).mockResolvedValue('Hello Alpha');
        (agent_api_service.send_command as unknown as ReturnType<typeof vi.fn>).mockResolvedValue(undefined);
        (voice_client.start_recording as unknown as ReturnType<typeof vi.fn>).mockResolvedValue(true);
    ...
    it('renders the neural sync interface and live transcript base', async () => {
        ...
        expect(screen.getByRole('button', { name: /Local microphone capture status/i })).toBeInTheDocument();
    });

    it('disables sync when no valid target is available', async () => {
        use_agent_store.setState({ agents: [] });
        (use_workspace_store as unknown as ReturnType<typeof vi.fn>).mockReturnValue([]);
        (load_agents as unknown as ReturnType<typeof vi.fn>).mockResolvedValue([]);
        ...
        expect(voice_client.start_recording).not.toHaveBeenCalled();
    });
    ...
    it('starts and stops recording, transcribes, and sends command to an agent target', async () => {
        ...
        await waitFor(() => {
            expect(system_api_service.engine.transcribe).toHaveBeenCalled();
    ...
    it('rolls back to idle and reports microphone startup failure', async () => {
        (voice_client.start_recording as unknown as ReturnType<typeof vi.fn>).mockResolvedValue(false);
        ...
        await waitFor(() => {
            expect(screen.getAllByText(/Microphone capture failed/i)).toHaveLength(2);
            expect(screen.getByRole('button', { name: /Start Sync/i })).toBeInTheDocument();
        });
        expect(system_api_service.engine.transcribe).not.toHaveBeenCalled();
    });

    it('reports transcription dispatch failures without emitting a command', async () => {
        (system_api_service.engine.transcribe as unknown as ReturnType<typeof vi.fn>).mockRejectedValue(new Error('engine offline'));
        ...
        await act(async () => {
            fireEvent.click(screen.getByRole('button', { name: /End Sync/i }));
        });

        await waitFor(() => {
            expect(screen.getByText(/Neural Sync dispatch failed/i)).toBeInTheDocument();
        });

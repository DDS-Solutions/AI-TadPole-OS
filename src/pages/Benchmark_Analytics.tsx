 ...
 * - **Witness Tests**: `Benchmark_Analytics.test.tsx`
 */
...
const buildBenchmarkTrace = (bench: BenchmarkResult, isOptimized: boolean): TraceStep[] => {
    ...
    const baseTime = bench.created_at ? new Date(bench.created_at).getTime() || Date.now() : Date.now();
    const meanNum = typeof bench.mean_ms === 'number' && !isNaN(bench.mean_ms) ? bench.mean_ms : 0;
    const meanStr = meanNum.toFixed(2);
    const p95Str = typeof bench.p95_ms === 'number' && !isNaN(bench.p95_ms) ? bench.p95_ms.toFixed(2) : 'N/A';
...
const formatMs = (ms?: number): string => (typeof ms === 'number' && !isNaN(ms) ? `${ms.toFixed(2)}ms` : '—');
...
const calculateDelta = (v1?: number, v2?: number): DeltaMetrics => {
    const num1 = typeof v1 === 'number' && !isNaN(v1) ? v1 : 0;
    const num2 = typeof v2 === 'number' && !isNaN(v2) ? v2 : 0;
    const delta = num1 - num2;
    const percentage = num2 > 0 ? ((delta / num2) * 100).toFixed(1) : '0.0';
...
const Benchmark_Analytics: React.FC = () => {
    ...
    const fetchBenchmarks = useCallback(async () => {
        try {
            ...
            const safeList = Array.isArray(data) 
                ? data 
                : (data && Array.isArray((data as unknown as { data?: BenchmarkResult[] }).data) 
                    ? (data as unknown as { data: BenchmarkResult[] }).data 
                    : []);
            setBenchmarks(safeList);
            if (safeList.length >= 2 && (is_analytics_view || selectedTests.length < 2)) {
                setSelectedTests([safeList[0].id, safeList[1].id]);
            }
        } catch (err) {
    ...
    const safeBenchmarks = useMemo(() => Array.isArray(benchmarks) ? benchmarks : [], [benchmarks]);
    const benchmarkMap = useMemo(() => new Map(safeBenchmarks.map(b => [b.id, b])), [safeBenchmarks]);

    const comparisonData = useMemo(() => {
        if (selectedTests.length < 2 && safeBenchmarks.length >= 2) {
            return { t1: safeBenchmarks[0], t2: safeBenchmarks[1] };
        }
    ...
    }, [selectedTests, benchmarkMap, safeBenchmarks]);

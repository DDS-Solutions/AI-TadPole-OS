 ...
 * - **Subsystem**: UI Pages / Missions.test
 *
 ...
 * - **Witness Tests**: `Missions.test.tsx`
 */
...
vi.mock('../components/missions/Mission_Header', () => ({
    Mission_Header: ({ on_run_mission, on_pause_resume_mission, on_cancel_mission, on_toggle_analysis, active_cluster }: any) => (
        <div>
            <button onClick={on_run_mission}>RUN MISSION</button>
            <button onClick={on_pause_resume_mission}>PAUSE/RESUME MISSION</button>
            <button onClick={on_cancel_mission}>CANCEL MISSION</button>
...
describe('Missions Page', () => {
    ...
    it('handles command rejection from engine', async () => {
        (agent_api_service.send_command as any).mockResolvedValue(false);
        render(<Missions />);
    ...
    it('handles send_command throw exception', async () => {
        (agent_api_service.send_command as any).mockRejectedValue(new Error('Network Crash'));
        render(<Missions />);
    ...
    it('launches a mission correctly', async () => {
        (agent_api_service.send_command as any).mockResolvedValue(true);
        render(<Missions />);

        fireEvent.click(screen.getByText('RUN MISSION'));

        await waitFor(() => {
            expect(agent_api_service.send_command).toHaveBeenCalled();
    ...
    it('updates objective on textarea change', async () => {
        render(<Missions />);
    ...
    it('triggers pause/resume mission action', () => {
        render(<Missions />);
        fireEvent.click(screen.getByText('PAUSE/RESUME MISSION'));
        expect(mock_workspace_state.toggle_cluster_active).toHaveBeenCalledWith('c-001');
    });

    it('triggers cancel mission action', async () => {
        render(<Missions />);
        fireEvent.click(screen.getByText('CANCEL MISSION'));
        await waitFor(() => {
            expect(event_bus.emit_log).toHaveBeenCalledWith(expect.objectContaining({
                severity: 'error',
                text: expect.stringContaining('Emergency cancellation issued')
            }));
        });
    });
});

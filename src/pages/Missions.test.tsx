...
vi.mock('../components/missions/AgentTeamView', () => ({
    AgentTeamView: (props: any) => (
        <div data-testid="team-view">
            <button onClick={() => props.onAssign('agent-2')}>Assign Agent</button>
            <button onClick={() => props.onUnassign('agent-1')}>Unassign Agent</button>
            <button onClick={() => props.onSetAlpha('agent-1')}>Set Alpha</button>
        </div>
    )
}));
...
describe('Missions Page', () => {
    ...
    it('handles agent team management actions', () => {
        ...
        expect(mockWorkspaceState.assignAgentToCluster).toHaveBeenCalledWith('agent-2', 'c-001');

        fireEvent.click(screen.getByText('Unassign Agent'));
        expect(mockWorkspaceState.unassignAgentFromCluster).toHaveBeenCalledWith('agent-1', 'c-001');

        fireEvent.click(screen.getByText('Set Alpha'));
        expect(mockWorkspaceState.setAlphaNode).toHaveBeenCalledWith('c-001', 'agent-1');
    });

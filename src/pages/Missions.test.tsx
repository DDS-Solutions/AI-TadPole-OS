...
vi.mock('../stores/agent_store', () => ({
    use_agent_store: vi.fn((selector) => selector ? selector(mockAgentState) : mockAgentState)
}));
...
vi.mock('../components/missions/Mission_Header', () => ({
    Mission_Header: ({ onRunMission, onToggleAnalysis, activeCluster }: any) => (
        <div>
...
describe('Missions Page', () => {
    ...
    it('calls receiveHandoff when socket event arrives', () => {
        let handoffCb: any;
        (tadpole_os_socket.subscribeHandoff as any).mockImplementation((cb: any) => {
            handoffCb = cb;
            return () => {};
        });

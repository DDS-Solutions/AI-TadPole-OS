 ...
 * - **Subsystem**: UI Pages / Workspaces
 * - **Primary Entrypoints**: `Workspaces`
 *
...
export default function Workspaces() {
    ...
    return (
        <div className="h-full flex flex-col bg-[color:var(--color-background)]">
            ...
            <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 max-w-7xl mx-auto px-6 pt-6">
                {/* Mission Clusters */}
                ...
                <section className="pt-8 border-t border-[color:var(--color-surface)]">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 opacity-40 grayscale group hover:grayscale-0 hover:opacity-100 transition-all">
                        {agents.filter(a => !clusters.some(c => c.collaborators.includes(a.id))).map(agent => (
                            <Tooltip key={agent.id} content={i18n.t('workspaces.tooltip_legacy_silo', { name: agent.name, defaultValue: `${agent.name} is not assigned to any active mission cluster` })} position="top">
                                <div className="p-3 bg-[color:var(--color-surface)]/30 border border-[color:var(--color-border)] rounded-xl flex items-center gap-3 cursor-help">

                        ...
                        <div className="flex items-center justify-between border-b border-zinc-900 pb-2">
                            ...
                            <div className="flex -space-x-2 p-1">
                                {cluster.collaborators.map(id => {
                                    ...
                                    return (
                                        <Tooltip key={id} content={`${agent?.name || 'Unknown Agent'} ${isAlpha ? i18n.t('workspaces.tooltip_alpha') : ''}`}>
                                            <div
                            ...
                            <div className="bg-zinc-900 border border-zinc-800 p-5 rounded-2xl md:col-span-1 xl:col-span-2 flex flex-col gap-4 relative shadow-2xl">
                                <div className="flex items-center justify-between">
                                    <h3 className="text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em] flex items-center gap-2">
                                        ...
                                        {i18n.t('workspaces.header_branches', { count: cluster.pendingTasks.filter(t => t.status === 'pending').length })}
                                    </h3>
                                </div>
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="flex items-center gap-2 mt-1">
                                                            <span className="text-[9px] font-mono text-zinc-500 uppercase">{i18n.t('workspaces.label_from_agent', { id: task.agentId })}</span>
                                                ...
                                                {task.status === 'pending' && (
                                                    <div className="flex items-center gap-2">
                                                        <Tooltip content={i18n.t('workspaces.tooltip_merge')} position="top">
                                                            <button onClick={() => approveBranch(cluster.id, task.id)} className="p-2 hover:bg-emerald-500/10 text-zinc-600 hover:text-emerald-500 transition-all rounded-lg">
                                                                <CheckCircle2 size={16} />
                                                            </button>
                                                        </Tooltip>
                                                        <Tooltip content={i18n.t('workspaces.tooltip_reject')} position="top">
                                                            <button onClick={() => rejectBranch(cluster.id, task.id)} className="p-2 hover:bg-red-500/10 text-zinc-600 hover:text-red-500 transition-all rounded-lg">
                                                                <XCircle size={16} />
                                                            </button>
                                                        </Tooltip>
                                                    </div>

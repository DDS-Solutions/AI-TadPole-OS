...
describe('BenchmarkAnalytics', () => {
    beforeEach(() => {
        vi.clearAllMocks();
        (TadpoleOSService.getBenchmarks as any).mockResolvedValue(mockBenchmarks);
    });

    it('renders and fetches benchmarks on mount', async () => {
        render(<BenchmarkAnalytics />);
        ...
        await waitFor(() => {
            expect(TadpoleOSService.getBenchmarks).toHaveBeenCalled();
    ...
    it('handles benchmark execution', async () => {
        (TadpoleOSService.runBenchmark as any).mockResolvedValue({ status: 'success' });
        render(<BenchmarkAnalytics />);
        ...
        await waitFor(() => {
            expect(TadpoleOSService.runBenchmark).toHaveBeenCalledWith('BM-RUN-01');
            expect(EventBus.emit).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('handles benchmark execution failure', async () => {
        (TadpoleOSService.runBenchmark as any).mockRejectedValue(new Error('Telemetry Timeout'));
        render(<BenchmarkAnalytics />);
        await waitFor(() => expect(screen.getByText('Runner Bench')).toBeInTheDocument());
    ...
    it('shows empty state when no benchmarks available', async () => {
        (TadpoleOSService.getBenchmarks as any).mockResolvedValue([]);
        render(<BenchmarkAnalytics />);

 ...
 * - **Subsystem**: UI Pages / detached_shell.test
 *
 ...
 * - **Witness Tests**: `detached_shell.test.tsx`
 */
...
describe('Detached_Shell subscription ownership', () => {
    ...
    it('renders sector component without Observability_Sidebar for detached tab windows', async () => {
        render(
            <MemoryRouter initialEntries={['/detached-view?type=tab&path=%2Fdashboard']}>
                <Detached_Shell />
            </MemoryRouter>
        );

        expect(screen.queryByTestId('observability-sidebar')).not.toBeInTheDocument();
    });
});

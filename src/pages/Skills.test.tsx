 ...
 * - **Subsystem**: UI Pages / Skills.test
 *
...
vi.mock('../stores/agent_store', () => ({
    use_agent_store: Object.assign(
        vi.fn((selector?: any) => {
            if (selector) return selector(mock_agent_store_state);
            return mock_agent_store_state;
        }),
        {
            getState: () => mock_agent_store_state,
            setState: (updates: any) => {
                Object.assign(mock_agent_store_state, updates);
            },
        }
    ),
}));
...
vi.mock('../components/skills', () => ({
    ...
    Security_Report_Modal: ({ is_open, on_close }: any) => {
        if (!is_open) return null;
        return <div data-testid="security-report-modal"><button onClick={on_close}>Close</button></div>;
    },
    Hook_Modal: ({ is_open, on_close, on_save }: any) => {
        if (!is_open) return null;
        return (
            <div data-testid="hook-modal">
                <button onClick={on_close}>Close Hook Modal</button>
                <button onClick={on_save}>Save Hook</button>
            </div>
        );
    },
    ...
    Hook_List: ({ hooks, on_delete, on_edit, on_create }: any) => (
        <div data-testid="hook-list">
            <button onClick={on_create}>Add Hook</button>
            {hooks.map((h: any) => (
                <div key={h.name}>
                    {h.name}
                    <button onClick={() => on_edit(h)}>Edit Hook</button>
...
describe('Skills Page', () => {
    ...
    beforeEach(() => {
        ...
        mock_agent_store_state.set_loading = vi.fn();
        mock_agent_store_state.set_error = vi.fn();
    });
    ...
    it('opens Hook_Modal on Add Hook click and triggers save', async () => {
        render(<Skills />);
        await act(async () => {
            fireEvent.click(screen.getByText('skills.tab_hooks'));
        });

        // Click Add Hook to open modal
        await act(async () => {
            fireEvent.click(screen.getByText('Add Hook'));
        });
        expect(screen.getByTestId('hook-modal')).toBeInTheDocument();

        // Click Save Hook to trigger save_hook action
        await act(async () => {
            fireEvent.click(screen.getByText('Save Hook'));
        });
        expect(mock_store.save_hook).toHaveBeenCalled();
    });

    it('pre-populates Hook_Modal on Edit Hook click and triggers update', async () => {
        render(<Skills />);
        await act(async () => {
            fireEvent.click(screen.getByText('skills.tab_hooks'));
        });
        ...
        // Click Edit Hook
        await act(async () => {
            fireEvent.click(screen.getByText('Edit Hook'));
        });
        expect(screen.getByTestId('hook-modal')).toBeInTheDocument();

        // Click Save Hook to trigger save_hook action (which does updates)
        await act(async () => {
            fireEvent.click(screen.getByText('Save Hook'));
        });
        expect(mock_store.save_hook).toHaveBeenCalled();
    ...
    it('triggers delete_hook on Delete Hook click', async () => {
        render(<Skills />);
        await act(async () => {
            fireEvent.click(screen.getByText('skills.tab_hooks'));
        });
        ...
        // Click Delete Hook
        await act(async () => {
            fireEvent.click(screen.getByText('Delete Hook'));
        });
        ...
        expect(mock_store.delete_hook).toHaveBeenCalledWith('test_hook');
    });
});

...
import { Search, Download, ShieldCheck, Filter, Store, Star, Clock, AlertTriangle, ArrowLeft, Code, X, RefreshCw, ExternalLink, BookOpen } from 'lucide-react';
...
function Template_Store() {
    ...
    const INDUSTRIES = ['All', ...Array.from(new Set(templates.map(t => t.industry))).sort((a, b) => a.localeCompare(b))];
    ...
    const handlePreview = async (template: Template) => {
        ...
        try {
            ...
            const knowledgeUrl = `https://raw.githubusercontent.com/DDS-Solutions/AI-Tadpole-OS-Industry-Templates/main/${template.path}/knowledge.json`;
            console.debug('Fetching knowledgeUrl:', knowledgeUrl);
            try {
                const kRes = await fetch(knowledgeUrl);
                if (kRes.ok) {
                    const kData = await kRes.json();
                    if (Array.isArray(kData)) {
                        setPreviewKnowledge(kData);
                    }
                }
            } catch (kErr) {
    ...
    return (
        <div className="p-6 space-y-6 max-w-7xl mx-auto w-full">
            ...
            <div className="flex-none">
                ...
                {/* Market Selector & Actions Row */}
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                    {/* Market Selector Tabs/Buttons */}
                    <div className="flex items-center gap-2 bg-[color:var(--color-surface)]/30 border border-[color:var(--color-border)]/50 rounded-2xl p-1.5 w-fit">
                        <NavLink
                            ...
                            className={({ isActive }) => `px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider transition-all duration-200 border ${isActive
                    ...
                    <div className="flex items-center gap-2 w-full sm:w-auto">
                        ...
                        >
                            ...
                            <span>Make your own templates</span>
                        </a>
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <div data-testid="industry-filters" className="flex items-center gap-2 overflow-x-auto pb-2 custom-scrollbar w-full">
                        ...
                        {INDUSTRIES.map(ind => (
                            <button
                                ...
                                className={`px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-colors ${selectedIndustry === ind
                    ...
                    <div data-testid="size-filters" className="flex items-center gap-2 overflow-x-auto pb-2 custom-scrollbar w-full">
                        <span className="sovereign-header-text mr-2 flex-shrink-0">{i18n.t('template_store.label_size')}</span>
                        {COMPANY_SIZES.map(size => (
                            <button
                                key={size}
                                onClick={() => setSelectedCompanySize(size)}
                                className={`px-3 py-1 rounded-md text-[10px] font-bold whitespace-nowrap transition-all ${selectedCompanySize === size
                    ...
                    <>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {filteredTemplates.map(template => (
                                <div key={template.id} className="sovereign-card flex flex-col hover:border-green-500/30 transition-all duration-300 group hover:-translate-y-1">
                                    ...
                                    <div className="mt-auto space-y-4">
                                        ...
                                        <button
                                            ...
                                            className={`w-full py-2 rounded-lg font-bold text-sm flex items-center justify-center gap-2 transition-all ${template.installed
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                    <div className="bg-[color:var(--color-background)] border border-[color:var(--color-border)] rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden relative">
                        ...
                        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                            ...
                            {previewKnowledge && previewKnowledge.length > 0 && (
                                <div className="mt-8 space-y-4">
                                    <div className="flex items-center gap-2 text-xs font-bold text-zinc-500 uppercase tracking-widest">
                                        <BookOpen size={14} className="text-green-500" />
                                        Playbooks & Institutional Knowledge (OKF)
                                    </div>
                                    <div className="grid grid-cols-1 gap-4">
                                        {previewKnowledge.map((item, idx) => (
                                            <div key={idx} className="p-4 bg-[color:var(--color-surface)]/50 border border-[color:var(--color-border)]/50 rounded-xl hover:border-green-500/20 transition-all duration-300">
                                                <div className="flex justify-between items-start mb-2">
                                                    <div className="flex items-center gap-2">
                                                        <h4 className="text-sm font-bold text-zinc-100">
                                                            {item.title || 'Untitled Playbook'}
                                                        </h4>
                                                        {item.concept_type && (
                                                    ...
                                                    {item.resource_uri && (
                                                        <a 
                                                            href={item.resource_uri} 
                                                            target="_blank" 
                                                            rel="noopener noreferrer"
                                                            className="text-zinc-500 hover:text-green-400 transition-colors text-xs font-mono flex items-center gap-1"
                                                        >
                                                            <span>SOP</span>
                                                ...
                                                {item.tags && (
                                                    <div className="flex flex-wrap gap-1.5 mt-2">
                                                        {item.tags.split(',').map((tag, tIdx) => (
                                                            <span key={tIdx} className="text-[9px] font-mono text-zinc-500 bg-zinc-800/50 px-1.5 py-0.5 rounded">
                                                                #{tag.trim()}
                                                            </span>
                                                        ))}
                                                    </div>
                        ...
                        <div className="p-6 bg-[color:var(--color-surface)]/30 border-t border-[color:var(--color-border)]/50 flex items-center justify-between gap-4">
                            ...
                            <button
                                ...
                                className={`px-8 py-2.5 rounded-lg font-bold text-sm flex items-center gap-2 shadow-lg transition-all ${isInstalling === previewTemplate.id

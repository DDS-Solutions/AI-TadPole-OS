 ...
 * - **Subsystem**: UI Pages / Engine_Dashboard
 * - **Primary Entrypoints**: `Engine_Dashboard`
 *
 ...
 * - **Witness Tests**: `Engine_Dashboard.test.tsx`
 */
...
export default function Engine_Dashboard() {
    ...
    const stats = useMemo(() => [
        { 
            ...
            value: `${cpu.toFixed(1)}%`, 
            percent: Math.min(Math.max(cpu, 0), 100),
            tooltip: i18n.t('engine_dashboard.tooltip_cpu'),
            icon: CpuIcon, 
        ...
        { 
            ...
            percent: max_memory > 0 ? Math.min(Math.max((memory / max_memory) * 100, 0), 100) : 0,
            tooltip: i18n.t('engine_dashboard.tooltip_memory'),
            icon: HardDrive, 
        ...
        { 
            ...
            percent: Math.min(Math.max((latency / 500) * 100, 0), 100),
            tooltip: i18n.t('engine_dashboard.tooltip_latency'),
            icon: Zap, 
            color: latency > 150 ? 'text-yellow-400' : 'text-emerald-400' 
        },
    ], [cpu, memory, max_memory, latency]);
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    <Tooltip content={i18n.t('engine_dashboard.tooltip_main')} position="right">
                        <div className="p-2 bg-[color:var(--color-surface)] border border-[color:var(--color-border)] rounded-lg shadow-xl ring-1 ring-zinc-800 cursor-help">
                ...
                <Tooltip content={i18n.t('engine_dashboard.tooltip_conn', { status: connection_state })} position="left">
                    <div className="flex items-center gap-2 px-3 py-1 bg-[color:var(--color-background)] border border-[color:var(--color-border)] rounded-full cursor-help">
                ...
                <div className="md:col-span-3 grid grid-cols-1 md:grid-cols-3 gap-4">
                    {stats.map((stat) => (
                        <Tooltip key={stat.label} content={stat.tooltip} position="top">
                            <div className="sovereign-panel p-5 cursor-help group">
            ...
            <div className="sovereign-panel relative overflow-hidden group h-[500px]">
                ...
                <div className="relative z-10 h-full flex flex-col items-center p-4">
                    ...
                    <div className="w-full flex-grow rounded-2xl overflow-hidden border border-[color:var(--color-surface)] shadow-inner">
            ...
            <div className="h-[400px] w-full relative">
                {is_swarm_pulse_detached ? (
                    <Fragment key="swarm-pulse-detached-state">
                        <div className="absolute inset-0 flex items-center justify-center bg-[color:var(--color-surface)]/20 backdrop-blur-sm border border-[color:var(--color-border)] rounded-[2.5rem] overflow-hidden group">
                        ...
                        >
                            <div className="h-screen bg-[color:var(--color-background)] p-6 flex flex-col overflow-hidden">
                                <SectorBoundary name="Detached Swarm Visualizer">
                    ...
                    <SectorBoundary name="Inline Swarm Visualizer">
                        <Swarm_Visualizer 

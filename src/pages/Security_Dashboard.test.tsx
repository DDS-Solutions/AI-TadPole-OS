 ...
 * - **Subsystem**: UI Pages / Security_Dashboard.test
 *
 ...
 * - **Witness Tests**: `Security_Dashboard.test.tsx`
 */
...
describe('Security_Dashboard Page', () => {
    ...
    beforeEach(() => {
        ...
        (tadpole_os_service.get_security_snapshot as any).mockResolvedValue({
            quotas: mock_quotas,
            audit_trail: mock_audit_trail,
            agent_health: mock_agent_health.agents,
        });
        (system_api_service.oversight.get_security_snapshot as any).mockResolvedValue({
            quotas: mock_quotas,
            audit_trail: mock_audit_trail,
            agent_health: mock_agent_health.agents,
        });
        (system_api_service.oversight.update_security_quota as any).mockResolvedValue({});
    });

    it('renders loading state initially', () => {
        (tadpole_os_service.get_security_snapshot as any).mockReturnValue(new Promise(() => {}));
        (system_api_service.oversight.get_security_snapshot as any).mockReturnValue(new Promise(() => {}));
    ...
    it('handles fetch errors gracefully', async () => {
        ...
        (tadpole_os_service.get_security_snapshot as any).mockRejectedValue(new Error('API Down'));
        (system_api_service.oversight.get_security_snapshot as any).mockRejectedValue(new Error('API Down'));
    ...
    it('prevents division by zero crashes in quota progress bar', async () => {
        ...
        (tadpole_os_service.get_security_snapshot as any).mockResolvedValue(zero_budget_snapshot);
        (system_api_service.oversight.get_security_snapshot as any).mockResolvedValue(zero_budget_snapshot);

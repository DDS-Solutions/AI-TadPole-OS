 ...
 * 
 * ### AI Assist Note
 * **Root View**: Manages the Ops Dashboard.test page. 
        ...
        <div data-testid="agent-grid">
            {agents.map((a: any) => (
                <div key={a.id} data-testid={`node-${a.id}`}>
                    ...
                    <button onClick={() => handle_agent_update(a.id, { model2: 'gemini-pro' })}>Change Model 2</button>
                    <button onClick={() => handle_agent_update(a.id, { model3: 'gemini-pro' })}>Change Model 3</button>
...
vi.mock('../components/Agent_Config_Panel', () => ({
    default: ({ on_close, on_update }: any) => (
        <div data-testid="config-panel">
            Config Panel
            <button onClick={on_close}>Close</button>
            <button onClick={() => on_update('1', { name: 'Updated' })}>Update</button>
        </div>
    )
}));

 ...
 * Orchestrates agent discovery, filtering by Sector (User vs AI), and deep configuration via `Agent_Config_Panel`. 
 ...
 * - Configuration: Access to Agent_Config_Panel for deep editing.
...
export default function Agent_Manager() {
    ...
    const handle_add_new_click = () => {
        ...
        const new_agent: Agent = {
            ...
            theme_color: active_tab === 'user' ? "#10b981" : "#6366f1",
            model_config: {
        ...
        >
            ...
            <div className="flex items-start gap-4">
                <div
                    ...
                    style={{
                        backgroundColor: agent.status === 'active' ? `${agent.theme_color || '#10b981'}15` : '#27272a',
                        color: agent.status === 'active' ? (agent.theme_color || '#10b981') : '#71717a',
                        border: `1px solid ${agent.status === 'active' ? `${agent.theme_color || '#10b981'}30` : '#3f3f46'}`
                    }}
                >
                    {(agent.name || '?')[0]}
                </div>
                    ...
                    <div className="flex items-center gap-2 mb-1">
                        ...
                        {agent.category === 'user' && agent.metadata?.has_participated_in_swarm === true && (
                            <Tooltip content="This specialist has been recruited into the AI Swarm pool." position="top">
                                <span className="flex items-center gap-1 text-[9px] font-bold text-indigo-400 uppercase tracking-wider bg-indigo-500/10 px-1.5 py-0.5 rounded cursor-help">

...
const ScheduledJobs: React.FC = () => {
    ...
    const fetchWorkflows = async () => {
        try {
            const [continuityData, skillsData] = await Promise.all([
                TadpoleOSService.listContinuityWorkflows(),
                TadpoleOSService.getUnifiedSkills()
            ]);

            const continuityWfs = continuityData.map(w => ({ ...w, type: 'continuity' as const }));
            const passiveWfs = ((skillsData.workflows || []) as { name: string; content: string }[]).map(w => ({

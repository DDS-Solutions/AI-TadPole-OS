 ...
 * - **Subsystem**: UI Pages / Workspaces.test
 *
 ...
 * - **Witness Tests**: `Workspaces.test.tsx`
 */
...
describe('Workspaces Page', () => {
    ...
    const mock_refresh_sync_status = vi.fn();

    beforeEach(() => {
        vi.clearAllMocks();
        
        use_agent_store.setState({ agents: mock_agents });
        ...
        (use_workspace_store as unknown as ReturnType<typeof vi.fn>).mockReturnValue({
            ...
            refresh_sync_status: mock_refresh_sync_status,
            sync_status: {}
        });
    ...
    it('renders detected environments, playbooks and safety warnings', async () => {
        (use_workspace_store as unknown as ReturnType<typeof vi.fn>).mockReturnValue({
            clusters: mock_clusters,
            approve_branch: mock_approve_branch,
            reject_branch: mock_reject_branch,
            ...
            sync_status: {
                '/org/frontend': {
                    total_bytes: 500,
                    status: 'idle',
                    detected_environments: ['vs_code', 'docker'],
                    mounted_okf_nodes: [
                        { id: 'okf-1', title: 'Docker Security Guide', concept_type: 'playbook' }
                    ],
                    okf_validation: {
                        status: 'warning',
                        message: 'Missing Docker Sandbox on host'
                    }
                }
            }
        });
        ...
        expect(screen.getByText('Missing Docker Sandbox on host')).toBeInTheDocument();

        // Verify playbook badge
        expect(screen.getByText('Docker Security Guide')).toBeInTheDocument();
    });
});

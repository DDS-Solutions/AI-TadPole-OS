 ...
 * - **Subsystem**: UI Pages / Scheduled_Jobs.test
 *
 ...
 * - **Witness Tests**: `Scheduled_Jobs.test.tsx`
 */
...
const mock_agent_state = {
    ...
    set_loading: vi.fn(),
    set_error: vi.fn(),
    get_agent: vi.fn((id) => mock_agent_state.agents.find(a => a.id === id))
};

vi.mock('../stores/agent_store', () => {
    const mock_store = Object.assign(
        vi.fn((selector) => selector ? selector(mock_agent_state) : mock_agent_state),
        {
            getState: vi.fn(() => mock_agent_state),
            setState: vi.fn((updater) => {
                if (typeof updater === 'function') {
                    Object.assign(mock_agent_state, updater(mock_agent_state));
                } else {
                    Object.assign(mock_agent_state, updater);
                }
            }),
...
describe('Scheduled_Jobs Page', () => {
    ...
    beforeEach(() => {
        ...
        (system_api_service.continuity.get_scheduled_jobs as any).mockResolvedValue(mock_jobs);
        (system_api_service.continuity.list_continuity_workflows as any).mockResolvedValue(mock_workflows);
        (system_api_service.continuity.get_scheduled_job_runs as any).mockResolvedValue(mock_runs);
        (system_api_service.continuity.create_scheduled_job as any).mockResolvedValue({});
        (system_api_service.continuity.update_scheduled_job as any).mockResolvedValue({});
        (system_api_service.continuity.delete_scheduled_job as any).mockResolvedValue({});
        (mission_api_service.get_unified_skills as any).mockResolvedValue({ scripts: [], manifests: [], workflows: [] });
    ...
    it('toggles job enabled state', async () => {
        ...
        expect(system_api_service.continuity.update_scheduled_job).toHaveBeenCalledWith('job-1', { enabled: false });
    ...
    it('deletes a job after confirmation', async () => {
        ...
        expect(system_api_service.continuity.delete_scheduled_job).toHaveBeenCalledWith('job-1');
        expect(event_bus.emit_log).toHaveBeenCalledWith(expect.objectContaining({ text: expect.stringContaining('Daily Summary') }));
    });
    ...
    it('expands a job and fetches its run history', async () => {
        ...
        expect(system_api_service.continuity.get_scheduled_job_runs).toHaveBeenCalledWith('job-1');
    ...
    it('can create a new agent-based scheduled job', async () => {
        ...
        expect(system_api_service.continuity.create_scheduled_job).toHaveBeenCalledWith(expect.objectContaining({
            name: 'New Agent Job',
            agent_id: 'agent-2',
            cron_expr: '0 12 * * *',

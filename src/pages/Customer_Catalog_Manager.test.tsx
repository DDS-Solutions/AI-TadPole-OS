...
describe('Customer_Catalog_Manager Page', () => {
  ...
  it('filters catalog items based on search query input', () => {
    render(<Customer_Catalog_Manager />);

    const searchInput = screen.getByPlaceholderText('Search catalog items (e.g. drill, hammer, tools)...');
    
    expect(screen.getByText('Heavy Duty Claw Hammer 16oz')).toBeInTheDocument();
    expect(screen.getByText('Cordless 20V Power Drill')).toBeInTheDocument();

    fireEvent.change(searchInput, { target: { value: 'Drill' } });

    expect(screen.queryByText('Heavy Duty Claw Hammer 16oz')).not.toBeInTheDocument();
    expect(screen.getByText('Cordless 20V Power Drill')).toBeInTheDocument();
  });

  it('shows no items found message when search query matches nothing', () => {
    render(<Customer_Catalog_Manager />);

    const searchInput = screen.getByPlaceholderText('Search catalog items (e.g. drill, hammer, tools)...');
    fireEvent.change(searchInput, { target: { value: 'NonexistentProductXYZ' } });

    expect(screen.getByText("No matching catalog items found for 'NonexistentProductXYZ'.")).toBeInTheDocument();
  });
  ...
  it('has accessible aria-label attributes on file upload buttons', () => {
    ...
    expect(screen.getByLabelText('Upload PDF Pricelist File')).toBeInTheDocument();
  });
});

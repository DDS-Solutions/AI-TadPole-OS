 ...
 * - **Subsystem**: UI Pages / Template_Store.test
 *
...
describe('Template_Store Page', () => {
    ...
    beforeEach(() => {
        ...
        global.fetch = vi.fn().mockImplementation(async (url: string) => {
            ...
            else if (url.includes('/engine/templates/installed')) res_body = { status: 'ok', swarms: [] };
            else if (url.includes('/engine/templates/uninstall')) res_body = { status: 'ok', uninstalled: true };
            else if (url.includes('/engine/templates/install') || url.includes('/engine/templates/import')) res_body = { status: 'success' };
            else if (url.includes('/system/environment')) res_body = { status: 'success', updated_keys: [] };
    ...
    it('opens preview modal, fetches config, and installs template', async () => {
        ...
        // Wait for modal to close after installation completes
        await waitFor(() => {
    ...
    it('allows importing a downloaded swarm archive file and opening preview modal', async () => {
        ...
        // Click deploy swarm for the imported local swarm
        const deploy_button = screen.getByText(/Deploy Swarm/i);
        await act(async () => {
            fireEvent.click(deploy_button);
        });

        // Verify it calls /engine/templates/import instead of /engine/templates/install
        expect(global.fetch).toHaveBeenCalledWith(
            expect.stringContaining('/engine/templates/import'),
            expect.objectContaining({
                method: 'POST'
            })
        );
    ...
    it('switches between Marketplace and Installed Swarms tabs', async () => {
        render(
            <MemoryRouter>
                <Template_Store />
            </MemoryRouter>
        ...
        const installedTabBtn = screen.getByRole('button', { name: /Installed Swarms/i });
        await act(async () => {
            fireEvent.click(installedTabBtn);
        });

        // Market grid is hidden, installed swarms component is rendered
        expect(screen.queryByText('Finance AI Agents')).not.toBeInTheDocument();

        const marketplaceTabBtn = screen.getByRole('button', { name: /Swarm Marketplace/i });
        await act(async () => {
            fireEvent.click(marketplaceTabBtn);
        });

...
export default function Model_Store() {
    ...
    const categories: string[] = catalog.length > 0 
        ? ['all', ...Array.from(new Set(catalog.flatMap(m => m.tags.map(t => t.toLowerCase())))).sort((a, b) => a.localeCompare(b))]
        : ['all', 'code', 'general', 'vision'];
    ...
    const filtered_catalog = catalog.filter(model => {
        ...
        const matches_category = category === 'all' || model.tags.some(tag => tag.toLowerCase() === category.toLowerCase());

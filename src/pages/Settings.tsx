...
export default function Settings(): React.ReactElement {
    ...
    const [isSwarmScanning] = useState(false); // Default state for Swarm Discovery indicator
    ...
    const handleSave = async (): Promise<void> => {
        ...
        try {
            ...
            setValidationError(i18n.t('settings.error_sync_failed', { defaultValue: 'System synchronization failed. Local changes saved.' }));
            return;
        }
                    ...
                    <div className="space-y-3 z-10 relative p-4 bg-zinc-950/50 rounded-lg border border-zinc-800 group/shield">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                <div className={`p-1.5 rounded ${settings.privacyMode ? 'bg-blue-500/20 text-blue-400' : 'bg-zinc-800 text-zinc-500'}`}>
                                ...
                                <Tooltip content={i18n.t('settings.tooltip_privacy_mode')} position="top">
                                    <label htmlFor="privacyMode" className="text-sm font-bold text-zinc-300 cursor-help w-max">{i18n.t('settings.label_privacy_mode')}</label>
                                </Tooltip>
                                {settings.privacyMode && (
                                    <span className="text-[10px] font-bold bg-blue-500/10 text-blue-500 border border-blue-500/20 px-1.5 py-0.5 rounded uppercase tracking-tighter">
                    ...
                    <div className="flex items-start justify-between relative z-10">
                        <div className="space-y-2">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="bg-blue-500 text-[9px] font-black text-white px-2 py-0.5 rounded-full uppercase tracking-widest animate-pulse">{i18n.t('settings.badge_alpha')}</span>
                            </div>
                            ...
                            <div className="flex items-center gap-4 mt-6">
                                <button
                                    ...
                                    className="flex items-center gap-2 px-6 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl text-[10px] font-bold uppercase tracking-widest transition-all shadow-lg shadow-blue-500/20 active:scale-95"
                                >
                                ...
                                <div className="text-[10px] text-zinc-500 font-mono uppercase tracking-tighter">
                                    {isSwarmScanning ? 'Scanning Cluster...' : 'Swarm Discovery Active'}
                                </div>
                            </div>
                        </div>
                    </div>

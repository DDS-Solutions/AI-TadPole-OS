...
class Command_Registry {
    ...
    public get_definition(cmd: string): Command_Definition | undefined {
        return this.handlers.get(cmd.trim().toLowerCase());
    }

    public async execute(cmd: string, ctx: Command_Context): Promise<Command_Result> {
        const handler = this.handlers.get(cmd.trim().toLowerCase());
        if (handler) {

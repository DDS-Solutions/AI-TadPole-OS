...
export interface AgentConfigState {
    ...
    voice: {
        voiceId: string;
        voiceEngine: 'browser' | 'openai' | 'groq' | 'piper';
...
export type Action =
    | { type: 'SET_MAIN_TAB'; payload: 'cognition' | 'memory' }
...
export function configReducer(state: AgentConfigState, action: Action): AgentConfigState {
    switch (action.type) {
        ...
        case 'UPDATE_GOVERNANCE':
            return {
                ...state,
                governance: { ...state.governance, [action.field]: action.value as number }
            };
        default:
            return state;
    }

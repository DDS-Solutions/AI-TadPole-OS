 ...
 *
 * @param channel    - Which channel to subscribe to: 'agentUpdates' | 'health' | 'handoff' | 'status' | 'pulse' | 'audio_stream' | 'swarm_pulse' | 'raw'
...
export function useWebSocketEvent<T = unknown>(
    channel: 'agentUpdates' | 'health' | 'handoff' | 'status' | 'pulse' | 'audio_stream' | 'swarm_pulse' | 'raw',
    handler: (data: T) => void,
    throttle_ms = 0
) {

 ...
 * - **Subsystem**: Frontend React Hooks / useAgentManager
 * - **Primary Entrypoints**: `useAgentManager`
 *
...
export function useAgentManager() {
    ...
    const handle_update_agent = useCallback((id: string, updates: Partial<Agent>) => {
        if (is_creating) {
            ...
            // Optimistic add & backend persistence
            const new_agent = { ...selected_agent, category: active_tab, ...updates } as Agent;
            void use_agent_store.getState().add_agent(new_agent);
    ...
    const handle_delete_agent = useCallback((id: string) => {
        void use_agent_store.getState().delete_agent(id);
        if (selected_agent && selected_agent.id === id) {
            set_selected_agent(null);
            set_is_creating(false);
        }
    ...
    const handle_add_new_click = useCallback(() => {
        ...
        const new_agent: Agent = {
            id: `node_${crypto.randomUUID ? crypto.randomUUID().substring(0, 8) : Date.now().toString(36)}`,
            ...
            model_config: {
                ...
                provider: settings.default_provider || resolve_provider(settings.default_model),
    ...
    const filtered_agents = useMemo(() => {
        const list = agents.filter(agent => {
            ...
            const matches_role = filter_role === 'all' || filter_role === 'health_status' || agent.role === filter_role;
            return matches_tab && matches_search && matches_role;
        });

        if (filter_role === 'health_status') {
            return [...list].sort((a, b) => {
                const fail_a = a.failure_count || 0;
                const fail_b = b.failure_count || 0;
                if (fail_b !== fail_a) {
                    return fail_b - fail_a; // Worst health (e.g. 5) to best health (0)
                }
                if (a.last_failure_at && b.last_failure_at) {
                    return new Date(b.last_failure_at).getTime() - new Date(a.last_failure_at).getTime();
                }
                if (a.last_failure_at) return -1;
                if (b.last_failure_at) return 1;

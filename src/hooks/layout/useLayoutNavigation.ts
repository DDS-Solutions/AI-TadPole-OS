 ...
 * - **Subsystem**: Frontend React Hooks / useLayoutNavigation
 * - **Primary Entrypoints**: `useLayoutNavigation`
 *
...
export function useLayoutNavigation(set_is_command_palette_open: (open: boolean | ((prev: boolean) => boolean)) => void) {
    ...
    useEffect(() => {
        const handle_key_down = (e: KeyboardEvent) => {
            ...
            // Quick Navigation (1-6) - disabled in detached popups
            if (!e.ctrlKey && !e.metaKey && !e.altKey && !location.pathname.startsWith('/detached')) {
    ...
    }, [navigate, set_is_command_palette_open, location.pathname]);
}

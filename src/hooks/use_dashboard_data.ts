...
export function useDashboardData() {
    ...
    const add_agent = use_agent_store(s => s.add_agent);
    const init_telemetry = use_agent_store(s => s.init_telemetry);
    ...
    const active_agents = useMemo(() =>
        (Array.isArray(agents_list) ? agents_list : []).filter((a: Agent) => (a.status === 'active' || a.status === 'speaking') && assigned_agent_ids.has(a.id)).length,

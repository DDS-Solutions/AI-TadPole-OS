 ...
 * - **Subsystem**: Frontend React Hooks / use_dashboard_data
 * - **Primary Entrypoints**: `useDashboardData`
 *
 ...
 * - **Local Errors**: none
 * - **Telemetry Targets**: `[useDashboardData]`
...
export function useDashboardData() {
    ...
    const assigned_agent_ids = useMemo(() => {
        ...
        (clusters || []).forEach(c => {
            (c.collaborators || []).forEach(id => ids.add(String(id)));
            if (c.alpha_id) ids.add(String(c.alpha_id));
        });
        return ids;
    }, [clusters]);
    ...
    const add_agent = useCallback((agent: Agent) => {
        return use_agent_store.getState().add_agent(agent);
    }, []);

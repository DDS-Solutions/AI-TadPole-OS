...
import { use_engine_status } from '../hooks/use_engine_status';
import { event_bus, type log_entry } from '../services/event_bus';
import { use_workspace_store } from '../stores/workspace_store';
import { use_role_store } from '../stores/role_store';

export function use_dashboard_data() {
    const { is_online } = use_engine_status();
    ...
    const active_agents = useMemo(() =>
        agents_list.filter(a => (a.status === 'active' || a.status === 'speaking') && assigned_agent_ids.has(a.id)).length,
        [agents_list, assigned_agent_ids]);

    const total_cost = useMemo(() => agents_list.reduce((acc, curr) => acc + (curr.cost_usd || 0), 0), [agents_list]);
    const total_budget = useMemo(() => agents_list.reduce((acc, curr) => acc + (curr.budget_usd || 0), 0), [agents_list]);
    const budget_util = total_budget > 0 ? (total_cost / total_budget) * 100 : 0;

    const total_tokens = useMemo(() => agents_list.reduce((acc, curr) => acc + (curr.tokens_used || 0), 0), [agents_list]);
    const total_input_tokens = useMemo(() => agents_list.reduce((acc, curr) => acc + (curr.input_tokens || 0), 0), [agents_list]);
    const total_output_tokens = useMemo(() => agents_list.reduce((acc, curr) => acc + (curr.output_tokens || 0), 0), [agents_list]);

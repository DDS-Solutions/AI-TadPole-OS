...
export const useSwarmMetrics = (): SwarmMetric[] => {
    ...
    const active_clusters_count = clusters_list.filter(c => c.is_active).length;
    const active_agents_count = new Set(
        clusters_list.flatMap(c => [
            ...(c.collaborators || []),
            ...(c.alpha_id ? [c.alpha_id] : [])
        ])
    ).size;
    ...
    return useMemo(() => [
        ...
        },
        {
            ...
            value: `$${(settings.default_budget_usd ?? 0).toFixed(2)}`,
            icon: DollarSign,

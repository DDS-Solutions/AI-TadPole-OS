 ...
 * - **Subsystem**: Frontend React Hooks / useOversightDashboard
 * - **Primary Entrypoints**: `useOversightDashboard`, `OversightStats`, `UseOversightDashboardHook`
 *
 ...
 * - **Local Errors**: none
 * - **Telemetry Targets**: `[OversightDashboard]`
...
export function useOversightDashboard(): UseOversightDashboardHook {
    ...
    const [pending, set_pending] = useState<OversightEntry[]>(() => {
        ...
        if (saved) {
            try { return JSON.parse(saved); } catch (err) { console.debug('[OversightDashboard] Failed to parse pending from localStorage:', err); return []; }
        }
        return [];
    });
    ...
    const [ledger, set_ledger] = useState<LedgerEntry[]>(() => {
        ...
        if (saved) {
            try { return JSON.parse(saved); } catch (err) { console.debug('[OversightDashboard] Failed to parse ledger from localStorage:', err); return []; }
        }
        return [];
    });
    ...
    const is_demo_enabled = useCallback(() => {
        return typeof import.meta !== 'undefined' && (import.meta.env?.VITE_DEMO_MODE === 'true' || import.meta.env?.VITE_ENABLE_MOCK_OVERSIGHT === 'true');
    }, []);

    const fetch_pending = useCallback(async () => {
        if (is_simulated && !is_demo_enabled()) return;
        try {
            const pending_data = await system_api_service.oversight.get_pending_oversight();
            set_pending(pending_data);
            set_is_simulated(false);
            set_has_attempted_fetch(true);
        } catch (err) {
            console.debug('[OversightDashboard] Failed to fetch pending oversight from engine:', err);
    ...
    }, [is_simulated, has_attempted_fetch, is_demo_enabled]);

    const fetch_ledger = useCallback(async () => {
        if (is_simulated && !is_demo_enabled()) return;
        try {
            const ledger_data = await system_api_service.oversight.get_oversight_ledger();
            set_ledger(ledger_data);
            set_is_simulated(false);
            set_has_attempted_fetch(true);
        } catch (err) {
            console.debug('[OversightDashboard] Failed to fetch ledger from engine:', err);
            if (!has_attempted_fetch) {
                if (is_demo_enabled()) {
    ...
    useEffect(() => {
        ...
        if (is_simulated) {
            if (!has_attempted_fetch && is_demo_enabled()) {
                timer = setTimeout(() => {
                    set_pending(prev => (prev.length === 0 ? MOCK_PENDING : prev));
                    set_ledger(prev => (prev.length === 0 ? MOCK_LEDGER : prev));
                    set_has_attempted_fetch(true);
                }, 0);
            }
            return () => {
    ...
    }, [is_simulated, fetch_pending, fetch_ledger, has_attempted_fetch, is_demo_enabled]);
    ...
    }, [fetch_pending, fetch_ledger, is_simulated, is_demo_enabled]);
    ...
    const resolve_agent_name = useCallback((id: string) => {
        ...
        const live_agent = live_agents.find(a => a.id === id || a.id.split('-').pop() === id);
        if (live_agent?.name) return live_agent.name;
    ...
    const handle_decide = async (id: string, decision: 'approved' | 'rejected') => {
        ...
        try {
            await system_api_service.oversight.decide_oversight(id, decision);
            set_pending(prev => prev.filter(p => p.id !== id));
            void fetch_ledger();
        } catch (error) {

...
export function useOversightDashboard(): UseOversightDashboardHook {
    ...
    useEffect(() => {
        if (is_simulated) {
            if (pending.length === 0 && !has_attempted_fetch && is_demo_enabled()) {
                setTimeout(() => {
                    set_pending(MOCK_PENDING);
                    set_ledger(MOCK_LEDGER);
                    set_has_attempted_fetch(true);
    ...
    }, [is_simulated, fetch_pending, fetch_ledger, pending.length, has_attempted_fetch, is_demo_enabled]);
    ...
    }, [fetch_pending, fetch_ledger, is_simulated, pending.length, is_demo_enabled]);

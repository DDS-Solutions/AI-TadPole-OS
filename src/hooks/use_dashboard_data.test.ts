 ...
 * - **Subsystem**: Frontend React Hooks / use_dashboard_data.test
 *
...
vi.mock('../stores/agent_store', () => {
    ...
    return {
        use_agent_store: Object.assign(
            vi.fn((selector: any) => selector ? selector(mockState) : mockState),
            {
...
vi.mock('../stores/node_store', () => ({
    use_node_store: vi.fn((selector: any) => {
        const state = {
...
vi.mock('../stores/workspace_store', () => ({
    use_workspace_store: vi.fn((selector: any) => {
        const state = {

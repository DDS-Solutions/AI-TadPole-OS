...
function skills_manager_reducer(state: SkillsManagerState, action: Action): SkillsManagerState {
    switch (action.type) {
        ...
        case 'RESET_HOOK_FORM':
            return { ...state, hook_form: initial_hook_form, editing_hook: initial_hook_form };
        default:
            return state;
    }
}
...
export function useSkillsManager() {
    ...
    const on_confirm_import = useCallback(async (data: Skill_Definition | Workflow_Definition | Hook_Definition, category: 'user' | 'ai') => {
        ...
        try {
            ...
            } else if (state.preview_type === 'hook') {
                await save_hook({ ...data as Hook_Definition, category });
            }
    ...
    }, [state.preview_type, save_skill_script, save_workflow, save_hook, fetch_skills]);
    ...
    const handle_toggle_assignment = useCallback(async (agent_id: string) => {
        ...
        if (assigning_item.type === 'skill') {
            ...
            updates.workflows = current.includes(assigning_item.name)
                ...
                : [...current, assigning_item.name];
        } else if (assigning_item.type === 'mcp') {
            const current = agent.mcp_tools || [];
            updates.mcp_tools = current.includes(assigning_item.name)
                ? current.filter((m: string) => m !== assigning_item.name)
                : [...current, assigning_item.name];
        }
    ...
    const handle_edit_hook = useCallback((hook: Hook_Definition) => {
        dispatch({
            type: 'SET_FIELDS',
            fields: {
                hook_form: hook,
    ...
    const handle_create_hook = useCallback(() => {
        dispatch({ type: 'RESET_HOOK_FORM' });
    }, []);

    const handle_assign = useCallback((type: 'skill' | 'workflow' | 'mcp', name: string) => {
        dispatch({
            type: 'SET_FIELDS',
            fields: {

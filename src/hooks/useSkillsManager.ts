...
export function useSkillsManager() {
    ...
    const [editing_hook, set_editing_hook] = useState<Partial<Hook_Definition> | null>(null);
    const [hook_form, set_hook_form] = useState<Hook_Definition>({
        name: '',
        description: '',
    ...
    const handle_save_hook = async () => {
        if (!editing_hook) return;
        set_is_saving(true);
        try {
            await save_hook(hook_form);
            set_editing_hook(null);
            fetch_skills();
        } catch (err) {
            set_save_error((err as Error).message);
        } finally {
    ...
    const handle_delete_hook = (name: string) => {
        set_confirm_dialog({
            is_open: true,
            title: i18n.t('skills.confirm_delete_hook_title', { defaultValue: 'Decommission Hook' }),
            message: i18n.t('skills.confirm_delete_hook_msg', { defaultValue: `Are you sure you want to decommission hook '${name}'?` }),
            on_confirm: async () => {
                await delete_hook(name);
                fetch_skills();
                set_confirm_dialog(prev => ({ ...prev, is_open: false }));
            }
        });
    ...
    return {
        ...
        set_import_modal_open, set_editing_skill, set_editing_workflow, set_editing_hook, set_hook_form,

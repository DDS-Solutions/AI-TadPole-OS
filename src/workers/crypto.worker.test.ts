 ...
 * - **Subsystem**: System Core / crypto.worker.test
 *
...
describe('crypto.worker', () => {
    ...
    it('replies with failure on unsupported operation types', async () => {
        const id = 'test-unknown';
        await worker_onmessage({
            data: { id, type: 'unknown_op' as any, payload: {} as any }
        });

        expect(mock_post_message).toHaveBeenCalledWith({
            id,
            success: false,
            error: 'Unsupported worker operation: unknown_op'
        });
    });
});

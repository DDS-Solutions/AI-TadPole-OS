# deploy-bunker-2.ps1
# Automated Deployment Script for Tadpole OS -> Swarm Bunker 2 (High RAM)
# Supports rollback on failure by tagging the previous image before rebuild.

$remoteHost = "tadpole-bunker-2" # Swarm Bunker 2 (High RAM)
$identityFile = "$HOME\.ssh\tadpole_deploy_key"
$remoteDir = "~/Desktop/tadpole-os" # Adjust if your Linux Lite path differs
$tarFile = "tadpole-deploy-b2.tar"

Write-Host "[DEPLOY] Starting Tadpole OS Deployment to $remoteHost (Bunker 2)..." -ForegroundColor Cyan

# Step 1: Package the codebase (Source-only for Remote Build)
Write-Host "[DEPLOY] Packaging codebase for remote compilation on Bunker 2..." -ForegroundColor Yellow
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "[ERROR] Failed to transfer package. check SSH connection to 192.168.50.38." -ForegroundColor Red
    exit 1
}

# Step 3: Remote Reconstruction & Compilation
Write-Host "[DEPLOY] Triggering remote Docker build (Frontend + Backend) on Swarm Bunker 2..." -ForegroundColor Yellow
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "[ERROR] Deployment to Bunker 2 failed or was rolled back." -ForegroundColor Red
}
else {
    Write-Host "[SUCCESS] Deployment Successful! Tadpole OS is live on Bunker 2." -ForegroundColor Green
}

Remove-Item $tarFile -ErrorAction SilentlyContinue
Write-Host "[DONE] Deployment to Bunker 2 complete." -ForegroundColor Cyan

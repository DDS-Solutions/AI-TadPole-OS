...
> - **Subsystem**: Core Sovereign Governance / GEMINI
> - **Architecture**: `@docs ARCHITECTURE:Core`
> - **Failure Path**: Information drift, legacy terminology, or documentation mismatch.
> - **Observability**: Traceability via `execution/parity_guard.py` (`[AGENTS]`)
...
### Active Documentation Guard (ADG v2) Protocol
This repository strictly enforces **ADG v2** (@docs `docs/ADG_V2.md`):
- **No Hand-Written Fact Claims**: Never write `Witness Tests:` or `Invariants:` in source file headers. Those are forbidden v1 anti-patterns flagged by `npm run adg:lint`.
- **Machine-Maintained Manifest**: Test witness relationships are automatically derived by `npm run adg:generate` into `adg.manifest.json`.
- **Executable Invariants**: Critical safety and security guarantees must be verified via test suites in `tests/invariants/` (run via `npx vitest run tests/invariants/`).
- **Pre-Commit Verification**: Run `npm run adg:verify` and `npm run test:witness` before declaring changes complete.

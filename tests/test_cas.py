...
class TestCas(unittest.TestCase):
    ...
    def test_cas_database_schema_and_deduplication(self):
        ...
        migration_dir = os.path.join(root_dir, "server-rs", "migrations")
        if not os.path.exists(migration_dir):
            migration_dir = os.path.join(root_dir, "migrations")
        migration_file_v1 = os.path.join(migration_dir, "20260812000000_cas_file_revisions.sql")
        migration_file_v2 = os.path.join(migration_dir, "20260815000000_cas_blob_dedup.sql")

 ...
 * - **Subsystem**: Test Verification Suite / commandProcessor.test
 *
...
import { process_command, initialize_command_processor, reset_command_processor } from '../src/logic/command_processor';
...
describe('command_processor', () => {
    ...
    it('processes /help command', async () => {
        ...
        expect(history[0].text).toContain('Available Slash Commands');
    });
    ...
    it('processes /pause command for valid agent', async () => {
        ...
        const { agent_api_service } = await import('../src/services/agent');

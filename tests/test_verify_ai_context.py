...
from execution import verify_ai_context as verifier
...
class PythonAIContextVerifierTests(unittest.TestCase):
    ...
    def test_javascript_witnesses_preserve_names_and_ignore_examples(self):
        source = '''
it('real_single', () => {});
test("real_double", () => {});
it(`real_template`, () => {});
// it('commented_line', () => {});
/* test("commented_block", () => {}); */
const example = "it('quoted_example', () => {})";
const template = `test("template_example", () => {})`;
const url = 'https://example.test'; it('after_url', () => {});
fit('not_a_test_call', () => {});
it('freeform name', () => {});
'''
        with patch.object(verifier.os, 'walk', return_value=[(str(ROOT), [], ['sample.test.ts'])]), \
                patch('builtins.open', mock_open(read_data=source)), \
                patch.object(verifier, '_KNOWN_TESTS', None):
            self.assertEqual(get_workspace_test_symbols(), {
                'real_single', 'real_double', 'real_template', 'after_url',
            })
...
'''
        ...
        rust_with_multiline_tracing = '''
fn process(catalog_url: &str) {
    tracing::info!(
        "📥 [rust_tag] Fetching from {}",
        catalog_url
    );
}
'''
        self.assertTrue(tag_is_emitted("rust_tag", rust_with_multiline_tracing))

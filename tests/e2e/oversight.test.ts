...
vi.mock('express', () => {
    ...
    const jsonMock = vi.fn(() => (_req: unknown, _res: unknown, next: () => void) => next());
    const urlencodedMock = vi.fn(() => (_req: unknown, _res: unknown, next: () => void) => next());

    return {
        default: Object.assign(expressMock, { json: jsonMock, urlencoded: urlencodedMock }),
        json: jsonMock,
...
describe('E2E: Oversight Safety Loop', () => {
    ...
    beforeEach(() => {
        ...
        oversight = (gateway as unknown as { oversight: OversightGate }).oversight;
    ...
    it('should pause execution for unsafe skills and resume after approval', async () => {
        ...
        console.debug(`[Test] User approving task ${entryId}`);

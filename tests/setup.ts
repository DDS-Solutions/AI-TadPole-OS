 ...
 * - **Subsystem**: Test Verification Suite / setup
 *
...
vi.mock('../src/i18n', () => ({
    i18n: {
        t: (key: string, options?: any) => {
            ...
            if (typeof t !== 'string') {
                t = (options && typeof options.defaultValue === 'string') ? options.defaultValue : key;
            }

            // Interpolate
            if (options && typeof options === 'object') {
                Object.keys(options).forEach(opt_key => {
                    if (opt_key === 'defaultValue' || opt_key === 'returnObjects') return;
...
// ─── Layout Stubs ────────────────────────────────────────────────────────────
// jsdom does not implement element.scrollTo — stub it globally so components
// that call scroll_ref.current.scrollTo(...) don't throw in tests.
HTMLElement.prototype.scrollTo = vi.fn() as any;

// ─── Web Audio Stub ──────────────────────────────────────────────────────────
// AudioContext is unavailable in jsdom. This base stub is sufficient for most
// tests. Tests requiring specialised behaviour (e.g. Live_Voice_Hub) override
// this per-test via vi.stubGlobal('AudioContext', CustomClass) — which takes
// precedence over this global and is automatically restored after each test.
class AudioContext_Mock {
    sampleRate = 16000;
...
(global as any).AudioContext = AudioContext_Mock;
(window as any).AudioContext = AudioContext_Mock;
(window as any).webkitAudioContext = AudioContext_Mock;

// ─── Console Logger Filtering ────────────────────────────────────────────────
// Silences verbose domain-logic module loading and auto-seeding notices during
// test suite execution, while preserving other debug traces.
const original_debug = console.debug;
console.debug = (...args: any[]) => {
    const message = typeof args[0] === 'string' ? args[0] : '';
    if (
        message.includes('[SettingsStore]') ||
        message.includes('[Serializer]') ||
        message.includes('[RoleNormalizer]') ||
        message.includes('[FormState]') ||
        message.includes('[AgentUIUtils]') ||
        message.includes('[RoleStore]')
    ) {
        return;
    }
    original_debug(...args);

...
> - **Telemetry Link**: Search `[sme_discovery]` in audit logs.
>
> ### AI Assist Note
...
> 💡 **Bound Skills**:
> - Use `@[skills/smb-data-ingestion]` to parse and validate un-normalized SMB data dumps (CSV, Excel, Quickbooks PDFs).
> - Use `@[skills/digital-twin-voice]` to analyze founder communications and calibrate brand tone into `.tmp/twin-persona.md`.
> - Use `@[skills/pii-redaction]` to scrub customer PII before transmitting data to external APIs.

> [!NOTE]
> **Socratic Gate Required** *(IDENTITY.md Directive #9)*: SME discovery is inherently a complex task. Before executing any analysis, the Socratic Gate must be passed: ask 3+ discovery questions, surface trade-offs between connector options, and document edge cases (e.g., businesses with no web presence, multi-location operators).

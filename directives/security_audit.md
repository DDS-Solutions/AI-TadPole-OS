...
> - **Subsystem**: Core Directives / security_audit
...
- **STASIS Behavior Test** *(IDENTITY.md Directive #7)*: Simulate a budget breach in a test environment and verify:
  1. All active Specialist Nodes are suspended (no new tool calls initiated).
  2. Task states are snapshotted to the Merkle Audit Trail.
  3. A `BUDGET_BREACH` alert is emitted to Entity 0 with spend, threshold, and active task inventory.
  4. The system does **NOT** self-resume — it waits for an explicit `RESUME` or `TERMINATE` command.

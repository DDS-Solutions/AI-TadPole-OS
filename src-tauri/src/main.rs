...
//!
//! ### AI Assist Note
//! **Main Entry Point**: Orchestrates the host-side windowing and backend sidecar (server-rs) lifecycle.
//! Implements a resilient spawn-loop with automatic port collision remediation (taskkill/pkill) and AppImage path discovery.
//!
//! ### 🔍 Debugging & Observability
//! - **Failure Path**: Sidecar binary not found (E0404), port 8000 already bound (10048), or AppImage mount point mismatch.
//! - **Telemetry Link**: Check `sidecar_runtime.log` in the project root for detailed spawn and output logs.
